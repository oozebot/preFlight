...
const char *Repetier::get_name() const
{
    return "Repetier";
}
        ...
        .on_complete(
            [&](std::string body, unsigned)
            {
                ...
                try
                {
                    ...
                    if (!res)
                    {
                        msg = DSKY::format_wxstr(_L("Mismatched type of print host: %s"),
...
{
    return DSKY::format_wxstr("%s: %s\n\n%s", _L("Could not connect to Repetier"), msg,
...
bool Repetier::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    BOOST_LOG_TRIVIAL(info)
        << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%, group: %7%") % name %
    ...
    if (upload_data.post_action == PrintHostPostUploadAction::StartPrint)
    {
        ...
        http.form_add("autostart",
                      "true"); // Fix for Repetier compatibility
    }
                ...
                try
                {
                    ...
                    BOOST_FOREACH (boost::property_tree::ptree::value_type &v, ptree.get_child("groupNames."))
                    {
                        ...
                        else
                        {
                            ...
                            groups.push_back(DSKY::from_u8(v.second.data()));
                        }
                    }
                }
                catch (const std::exception &)
...
bool Repetier::get_printers(wxArrayString &printers) const
{
    ...
    http.on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error listing printers: %2%, HTTP %3%, body: `%4%`") %
                                                name % error % status % body;
                res = false;
            })
        .on_complete(
            ...
            {
                BOOST_LOG_TRIVIAL(debug) << boost::format("%1%: Got printers: %2%, HTTP status: %3%") % name % body %
                                                http_status;
                ...
                {
                    throw HostNetworkError(
                        DSKY::format(_L("Parsing of host response failed.\nMessage body: \"%1%\"\nError: \"%2%\""),
                                     body, err.what()));
                }
                ...
                try
                {
                    BOOST_FOREACH (boost::property_tree::ptree::value_type &v, ptree.get_child("data."))
                    {
                        ...
                        printers.push_back(DSKY::from_u8(port));
                    }
                }
                catch (const std::exception &err)
                {
                    throw HostNetworkError(
                        DSKY::format(_L("Enumeration of host printers failed.\nMessage body: \"%1%\"\nError: \"%2%\""),
                                     body, err.what()));
                }
            })
        .perform_sync();
