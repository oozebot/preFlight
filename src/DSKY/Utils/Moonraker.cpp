...
{
    return DSKY::format_wxstr("%s: %s", _L("Could not connect to Moonraker"), msg);
}
...
// Check whether the file already exists on Moonraker before uploading
bool Moonraker::file_exists(const boost::filesystem::path &upload_path, wxString &error) const
{
    bool exists = false;
    auto url = make_url(
        (boost::format("server/files/metadata?filename=gcodes/%1%") % Http::url_encode(upload_path.string())).str());
...
bool Moonraker::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    else
    {
        ...
        url = substitute_host(make_url("server/files/upload"), DSKY::into_u8(test_msg_or_host_ip));
