...
void actualize(RaycastManager::Meshes &meshes, const ModelVolumePtrs &volumes, const RaycastManager::ISkip *skip,
...
std::optional<RaycastManager::Hit> RaycastManager::first_hit(const Vec3d &point, const Vec3d &direction,
                                                             const ISkip *skip) const
{
    ...
    // Anisotropic transformation of normal is not perpendiculat to triangle
...
std::optional<RaycastManager::Hit> RaycastManager::closest_hit(const Vec3d &point, const Vec3d &direction,
                                                               const ISkip *skip) const
{
...
Luminary::Transform3d RaycastManager::get_transformation(const TrKey &tr_key) const
{
               ...
               RaycastManager::Meshes *inputs)
{
    ...
    for (const ModelVolume *volume : volumes)
    {
        ...
        if (skip != nullptr && skip->skip(oid))
            continue;
        auto is_oid = [oid](const RaycastManager::Mesh &it)
        {
            return oid == it.first;
        };
        if (auto item = std::find_if(meshes.begin(), meshes.end(), is_oid); item != meshes.end())
        {
    ...
    if (need_sort)
    {
        auto is_lower = [](const RaycastManager::Mesh &m1, const RaycastManager::Mesh &m2)
        {
...
const Luminary::AABBMesh *get_mesh(const RaycastManager::Meshes &meshes, size_t volume_id)
{
    auto is_lower_index = [](const RaycastManager::Mesh &m, size_t i)
    {
        return m.first < i;
    };
...
RaycastManager::TrItems::iterator find(RaycastManager::TrItems &items, const RaycastManager::TrKey &key)
{
    auto fnc = [](const RaycastManager::TrItem &it, const RaycastManager::TrKey &l_key)
    {
        return is_lower_key(it.first, l_key);
    };
...
std::optional<RaycastManager::Hit> ray_from_camera(const RaycastManager &raycaster, const Vec2d &mouse_pos,
                                                   const Camera &camera, const RaycastManager::ISkip *skip)
{
    Vec3d point;
    Vec3d direction;
