...
#include "Bonjour.hpp"
#include "DSKY/GUI/BonjourDialog.hpp"
...
bool OctoPrint::test_with_resolved_ip(wxString &msg) const
{
    ...
    auto url = substitute_host(make_url("api/version"), DSKY::into_u8(msg));
    msg.Clear();
    ...
    // Not changing the host would work in most cases (where there is 1 service on 1 hostname) but would break when f.e. a reverse proxy is used.
        ...
        .on_complete(
            [&, this](std::string body, unsigned)
            {
                ...
                try
                {
                    ...
                    if (!res)
                    {
                        msg = DSKY::format_wxstr(_L("Mismatched type of print host: %s"), (text ? *text : name));
                    }
                }
...
{
    return DSKY::format_wxstr("%s: %s\n\n%s", _L("Could not connect to OctoPrint"), msg,
                              _L("Note: OctoPrint version at least 1.1.0 is required."));
}

// Check whether the file already exists on OctoPrint before uploading
bool OctoPrint::file_exists(const boost::filesystem::path &upload_path, wxString &error) const
{
    ...
    http.ssl_revoke_best_effort(m_ssl_revoke_best_effort)
        ...
        .on_error(
            [&](std::string, std::string, unsigned status)
            {
                // Only a 404 proves absence; any other failure leaves the question open, and an
                // open question must not turn into a silent overwrite.
                exists = status != 404;
                if (exists)
                    DBG_COUNT("PRINTHOST_FILE_EXISTS_UNKNOWN");
            })
        .perform_sync();
...
bool OctoPrint::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    else if (DSKY::get_app_config()->get_bool("allow_ip_resolve") && boost::algorithm::ends_with(host, ".local"))
    {
        Bonjour("octoprint")
            .set_hostname(host)
    ...
    else if (resolved_addr.size() == 2 && resolved_addr[0].is_v4() != resolved_addr[1].is_v4())
    {
        ...
        if (!upload_inner_with_resolved_ip(
                ...
                { msg = DSKY::format_wxstr("%1%: %2%", resolved_addr.front(), error); }, info_fn,
                resolved_addr.front()) &&
            !upload_inner_with_resolved_ip(
                std::move(upload_data), prorgess_fn, [&msg = error_message, resolved_addr](wxString error)
                { msg += DSKY::format_wxstr("\n%1%: %2%", resolved_addr.back(), error); }, info_fn,
                resolved_addr.back()))
        {
                                              ...
                                              InfoFn info_fn, const boost::asio::ip::address &resolved_addr) const
{
    ...
    wxString test_msg_or_host_ip = DSKY::from_u8(resolved_addr.to_string());
                                       ...
                                       InfoFn info_fn) const
{
    ...
    if (m_host.find("https://") == 0 || test_msg_or_host_ip.empty() ||
        !DSKY::get_app_config()->get_bool("allow_ip_resolve"))
#endif // _WIN32
    {
    ...
    else
    {
        ...
        url = substitute_host(make_url("api/files/local"), DSKY::into_u8(test_msg_or_host_ip));
...
{
    return DSKY::format_wxstr("%s: %s", _L("Could not connect to PrusaLink"), msg);
}
        ...
        .on_complete(
            [&, this](std::string body, unsigned)
            {
                ...
                try
                {
                    ...
                    if (!res)
                    {
                        msg = DSKY::format_wxstr(_L("Mismatched type of print host: %s"), (text ? *text : "OctoPrint"));
                    }
                }
                catch (const std::exception &)
                {
...
bool PrusaLink::get_storage(wxArrayString &storage_path, wxArrayString &storage_name) const
{
    ...
    wxString wlang = DSKY::wxGetApp().current_language_code();
    std::string lang = DSKY::into_u8(wlang.SubString(0, 1));
    ...
    if (res && storage_path.empty())
    {
        ...
        { // otherwise error_msg is already filled
            ...
            for (const auto &si : storage)
            {
                error_msg += DSKY::format_wxstr(si.read_only ?
                                                             // TRN %1% = storage path
                                                    _L("%1% : read only")
                                                             :
                                                             // TRN %1% = storage path
        ...
        // TRN %1% = host
        std::string message = DSKY::format(_L("Upload has failed. There is no suitable storage found at %1%."),
                                           m_host) +
                              DSKY::into_u8(error_msg);
        BOOST_LOG_TRIVIAL(error) << message;
        throw Luminary::IOError(message);
    }
                                       ...
                                       InfoFn info_fn) const
{
    ...
    else
    {
        ...
        // in original address (m_host) replace host for resolved ip
        info_fn(L"resolve", test_msg_or_host_ip);
        url = substitute_host(make_url(storage_path), DSKY::into_u8(test_msg_or_host_ip));
        BOOST_LOG_TRIVIAL(info) << "Upload address after ip resolve: " << url;
    }
#endif // _WIN32
...
#endif // _WIN32
    set_auth(http);
    // PrusaLink reads any Print-After-Upload value as true, so the header is sent only when the print should start.
