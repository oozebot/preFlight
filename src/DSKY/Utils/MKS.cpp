...
{
    return DSKY::format_wxstr("%s: %s", _L("Could not connect to MKS"), msg);
}
...
bool MKS::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("MKS: Uploading file %1%, filepath: %2%, print: %3%, command: %4%") %
                                   upload_data.source_path % upload_data.upload_path %
                                   (upload_data.post_action == PrintHostPostUploadAction::StartPrint) % upload_cmd;
    ...
    http.on_complete(
            [&](std::string body, unsigned status)
            {
                ...
                if (err_code != 0)
                {
                    BOOST_LOG_TRIVIAL(error)
                        << boost::format("MKS: Request completed but error code was received: %1%") % err_code;
        ...
        .on_error(
            ...
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("MKS: Error uploading file: %1%, HTTP %2%, body: `%3%`") %
                                                error % status % body;
