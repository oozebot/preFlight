...
#include "DSKY/GUI/3DScene.hpp" // ::glsafe
#include "DSKY/GUI/Jobs/CreateFontStyleImagesJob.hpp"
#include "DSKY/GUI/ImGuiPureWrap.hpp" // check of font ranges
...
StyleManager::StyleManager(const ImWchar *language_glyph_range,
                           const std::function<EmbossStyles()> &create_default_styles)
...
const HorizontalAlignToName horizontal_align_to_name = boost::assign::list_of<HorizontalAlignToName::relation>(
    FontProp::HorizontalAlign::left, "left")(FontProp::HorizontalAlign::center,
                                             "center")(FontProp::HorizontalAlign::right, "right");
using VerticalAlignToName = boost::bimap<FontProp::VerticalAlign, std::string_view>;
const VerticalAlignToName vertical_align_to_name = boost::assign::list_of<VerticalAlignToName::relation>(
    FontProp::VerticalAlign::top, "top")(FontProp::VerticalAlign::center, "middle")(FontProp::VerticalAlign::bottom,
                                                                                    "bottom");
} // namespace
...
void StyleManager::init(AppConfig *app_config)
{
    ...
    std::optional<size_t> active_index_opt = (app_config != nullptr) ? ::load_style_index(*app_config)
                                                                     : std::optional<size_t>{};
...
ImFont *StyleManager::create_imgui_font(const std::string &text, double scale)
{
    ...
    ImFont *font = m_style_cache.atlas.AddFontFromMemoryTTF((void *) buffer.data(), buffer.size(), font_size,
                                                            &font_config, m_style_cache.ranges.Data);
...
bool read(const Section &section, const std::string &key, Luminary::FontProp::HorizontalAlign &value)
{
    auto item = section.find(key);
    if (item == section.end())
        return false;
    ...
    value = (it != map.end()) ? it->second : Luminary::FontProp::HorizontalAlign::center;
    return true;
}

bool read(const Section &section, const std::string &key, Luminary::FontProp::VerticalAlign &value)
{
    auto item = section.find(key);
    if (item == section.end())
        return false;
    ...
    auto it = map.find(data);
    value = (it != map.end()) ? it->second : Luminary::FontProp::VerticalAlign::center;
    return true;
}
