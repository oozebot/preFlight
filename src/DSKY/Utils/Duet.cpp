...
{
    return DSKY::format_wxstr("%s: %s", _L("Could not connect to Duet"), msg);
}
...
bool Duet::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    if (dsf)
    {
        ...
        // Send the session key when one is present
        if (!connect_msg.empty())
            http.header("X-Session-Key", DSKY::into_u8(connect_msg));
    }
    else
    {
...
// Try DSF first (preferred for oozeBot Rapid / SBC-based Duets), fall back to RRF
...
// Check if the file already exists on the Duet before uploading
bool Duet::file_exists(const boost::filesystem::path &upload_path, wxString &error) const
{
    ...
    http.on_complete(
            [&](std::string body, unsigned status)
            {
                ...
                {
                    // RRF always returns 200, so check the err code in the body (0 = success = file exists)
                      ...
                      ConnectionType connectionType, const std::string &session_key) const
{
    ...
    // For standalone RRF, drain any stale replies from the rr_reply queue
    // before sending the command. The queue is shared with all HTTP clients (e.g. DWC)
    ...
    if (dsf)
    {
        ...
        // Pass the session key for authenticated DSF installations
    ...
    // For standalone RRF, rr_gcode returns only {"buff": N} (buffer status).
    // The G-code reply text must be fetched separately via GET /rr_reply.
    if (success && !dsf)
    {
...
bool Duet::get_machine_limits(wxString &msg, MachineLimitsResult &result) const
{
    ...
    // Extract the session key for DSF authentication
    std::string session_key = dsf ? DSKY::into_u8(connect_msg) : std::string();

    // For standalone RRF, rr_reply can return stale or wrong data because of races
    // with other HTTP clients (e.g. Duet Web Control). Each query is retried up to 3 times,
    // validating the response via parse_mcode_response(). DSF returns replies inline, so no retry.
