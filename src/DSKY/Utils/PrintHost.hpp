...
public:
    ...
    // The mDNS service the network lookup queries. Hosts that announce themselves as OctoPrint keep the default.
    virtual const char *discovery_service() const { return "octoprint"; }
    ...
    // Check whether the file already exists on the print host before uploading.
    // Returns true when the file is known to exist. Returns false when it does not exist
    // or the check failed (network error, unsupported host); the upload proceeds either way.
...
{
public:
    PrintHostJobQueue(DSKY::PrintHostQueueDialog *queue_dialog);
