...
// Excerpt of DSKY::Selection for serialization onto the Undo / Redo stack.
struct Selection : public Luminary::ObjectBase
{
    void clear()
    {
        mode = 0;
...
public:
    ...
    bool undo(Luminary::Model &model, const DSKY::Selection &selection, DSKY::GLGizmosManager &gizmos,
              const SnapshotData &snapshot_data, size_t time_to_load = SIZE_MAX);

    // Jump forward in time. If time_to_load is SIZE_MAX, the next snapshot is activated.
    bool redo(Luminary::Model &model, DSKY::GLGizmosManager &gizmos, size_t time_to_load = SIZE_MAX);
