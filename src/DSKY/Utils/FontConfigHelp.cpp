...
static std::optional<Luminary::ScopeGuard> finalize_guard;
...
std::string DSKY::get_font_path(const wxFont &font, bool reload_fonts)
{
    ...
    Luminary::ScopeGuard sg_mp([matchPattern]() { FcPatternDestroy(matchPattern); });
    ...
    if (resultPattern == nullptr)
        return "";
    Luminary::ScopeGuard sg_rp([resultPattern]() { FcPatternDestroy(resultPattern); });
