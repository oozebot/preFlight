...
const char *FlashAir::get_name() const
{
    return "FlashAir";
}
    ...
    http.on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format(
                                                "%1%: Error getting upload enabled: %2%, HTTP %3%, body: `%4%`") %
...
{
    return DSKY::format_wxstr(
        "%s: %s\n%s", _u8L("Could not connect to FlashAir"), msg,
...
bool FlashAir::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    if (strDest.empty() || strDest.front() != '/') // Needs a leading / else root uploads fail.
    {
        strDest.insert(0, "/");
    }
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Uploading file %2% at %3% / %4%, filename: %5%") % name %
                                   upload_data.source_path % urlPrepare % urlUpload % upload_filename.string();
    ...
    httpPrepare
        .on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error preparing upload: %2%, HTTP %3%, body: `%4%`") %
                                                name % error % status % body;
            ...
            [&, this](std::string body, unsigned)
            {
                ...
                if (!res)
                {
                    BOOST_LOG_TRIVIAL(error)
                        << boost::format("%1%: Request completed but no SUCCESS message was received.") % name;
    ...
    httpDir
        .on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error setting upload dir: %2%, HTTP %3%, body: `%4%`") %
                                                name % error % status % body;
                error_fn(format_error(body, error, status));
                res = false;
            })
...
std::string FlashAir::timestamp_str() const
{
    ...
    unsigned long fattime = ((tm.tm_year - 80) << 25) | ((tm.tm_mon + 1) << 21) | (tm.tm_mday << 16) |
