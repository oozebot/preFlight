 ...
 *
 * This uses the format as of Windows 10 20H2 (latest as of this writing),
 ...
 */
static std::wstring format_user_choice_string(const wchar_t *aExt, const wchar_t *aUserSid, const wchar_t *aProgId,
                                              SYSTEMTIME aTimestamp)
{
    ...
    int userChoiceLen = _scwprintf(userChoiceFmt, aExt, aUserSid, aProgId, fileTime.dwHighDateTime,
    ...
    _snwprintf_s(userChoice.data(), userChoiceLen, _TRUNCATE, userChoiceFmt, aExt, aUserSid, aProgId,
...
static std::wstring generate_user_choice_hash(const wchar_t *aExt, const wchar_t *aUserSid, const wchar_t *aProgId,
                                              SYSTEMTIME aTimestamp)
{
...
static bool set_user_choice(const wchar_t *aExt, const wchar_t *aProgID)
{
    ...
    ls = ::RegCreateKeyExW(assocKey.get(), L"UserChoice", 0, nullptr, 0 /* options */, KEY_READ | KEY_WRITE,
                           0 /* security attributes */, &rawUserChoiceKey, nullptr);
    if (ls != ERROR_SUCCESS)
        return false;

    AutoRegKey userChoiceKey(rawUserChoiceKey);
    DWORD progIdByteCount = (::lstrlenW(aProgID) + 1) * sizeof(wchar_t);
    ls = ::RegSetValueExW(userChoiceKey.get(), L"ProgID", 0, REG_SZ, reinterpret_cast<const unsigned char *>(aProgID),
                          progIdByteCount);
    if (ls != ERROR_SUCCESS)
        return false;

    DWORD hashByteCount = (::lstrlenW(hash.data()) + 1) * sizeof(wchar_t);
    ls = ::RegSetValueExW(userChoiceKey.get(), L"Hash", 0, REG_SZ, reinterpret_cast<const unsigned char *>(hash.data()),
                          hashByteCount);
    if (ls != ERROR_SUCCESS)
        return false;
...
static bool set_as_default_per_file_type(const std::wstring &extension, const std::wstring &prog_id)
{
    ...
    if (res == ERROR_SUCCESS)
    {
        ...
        res = ::RegGetValueW(assoc_key.get(), L"UserChoice", L"ProgId", RRF_RT_REG_SZ, nullptr, nullptr,
                             &data_size_bytes);
        if (res == ERROR_SUCCESS)
        {
            ...
            res = ::RegGetValueW(assoc_key.get(), L"UserChoice", L"ProgId", RRF_RT_REG_SZ, nullptr, curr_prog_id.data(),
                                 &data_size_bytes);
            if (res == ERROR_SUCCESS)
            {
...
void associate_file_type(const std::wstring &extension, const std::wstring &prog_id, const std::wstring &prog_desc,
                         bool set_as_default)
{
