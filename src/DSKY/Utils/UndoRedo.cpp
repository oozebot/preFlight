...
#include <luminary/core/ObjectID.hpp>
#include <luminary/platform/process/Memory.hpp>

#include "DSKY/GUI/3DScene.hpp" // IWYU pragma: keep
...
#ifdef PREFLIGHT_UNDOREDO_DEBUG
...
public:
    ...
    {
        return this->is_valid() && rhs.is_valid() && m_end <= rhs.m_begin;
    }
    // This interval comes strictly after the rhs interval.
    bool strictly_after(const Interval &rhs) const
    {
        return this->is_valid() && rhs.is_valid() && rhs.m_end <= m_begin;
    }

    bool operator<(const Interval &rhs) const
    {
        return (m_begin < rhs.m_begin) || (m_begin == rhs.m_begin && m_end < rhs.m_end);
    }
...
#endif /* PREFLIGHT_UNDOREDO_DEBUG */
...
public:
    ...
    size_t release_between_timestamps(size_t timestamp_start, size_t timestamp_end) override
    {
        ...
        if (!m_history.empty())
        {
            ...
            auto it_lo = std::upper_bound(m_history.begin(), m_history.end(), timestamp_start,
                                          [](size_t l, const auto &r) { return l < r.begin(); });
            auto it_hi = std::upper_bound(it_lo, m_history.end(), timestamp_end,
                                          [](size_t l, const auto &r) { return l < r.end(); });
            if (it_lo != it_hi)
            {
...
{
public:
    ImmutableObjectHistory(std::shared_ptr<const T> shared_object, bool optional)
        : m_shared_object(shared_object), m_optional(optional)
    {
    }
    ...
    std::string format() override
    {
        ...
        out += this->is_serialized() ? std::string(" len:") + std::to_string(m_serialized.size())
...
private:
    struct Data
    {
        ...
        bool matches(const std::string &rhs)
        {
...
public:
    MutableHistoryInterval(const Interval &interval, const std::string &input_data)
        : m_interval(interval), m_data(nullptr)
    {
    ...
    MutableHistoryInterval(MutableHistoryInterval &&rhs) : m_interval(rhs.m_interval), m_data(rhs.m_data)
    {
        rhs.m_data = nullptr;
    }
...
public:
    ...
    std::string format() override
    {
        std::string out = typeid(T).name();
        for (const MutableHistoryInterval &interval : m_history)
            out += std::string(", ptr:") + ptr_to_string(interval.data()) + " len:" + std::to_string(interval.size()) +
                   " <" + std::to_string(interval.begin()) + "," + std::to_string(interval.end()) + ")";
        return out;
    }
#endif /* PREFLIGHT_UNDOREDO_DEBUG */
...
public:
    ...
    StackImpl()
        : m_memory_limit(std::min(Luminary::total_physical_memory() / 10,
                                  size_t(1 * 16384 * 65536 / UNDO_REDO_DEBUG_LOW_MEM_FACTOR)))
    ...
    void take_snapshot(const std::string &snapshot_name, const Luminary::Model &model, const DSKY::Selection &selection,
                       const DSKY::GLGizmosManager &gizmos, const SnapshotData &snapshot_data);
    void reduce_noisy_snapshots(const std::string &new_name);
    void load_snapshot(size_t timestamp, Luminary::Model &model, DSKY::GLGizmosManager &gizmos);

    bool has_undo_snapshot() const;
    bool has_undo_snapshot(size_t time_to_load) const;
    bool has_redo_snapshot() const;
    bool undo(Luminary::Model &model, const DSKY::Selection &selection, DSKY::GLGizmosManager &gizmos,
              const SnapshotData &snapshot_data, size_t jump_to_time);
    bool redo(Luminary::Model &model, DSKY::GLGizmosManager &gizmos, size_t jump_to_time);
    ...
    template<typename T>
    T *load_mutable_object(const Luminary::ObjectID id);
    template<typename T>
    std::shared_ptr<const T> load_immutable_object(const Luminary::ObjectID id, bool optional);
    template<typename T>
    void load_mutable_object(const Luminary::ObjectID id, T &target);
...
private:
    ...
    std::vector<Snapshot>::iterator release_snapshots(std::vector<Snapshot>::iterator begin,
...
struct specialize<Archive, Luminary::Model *, cereal::specialization::non_member_load_save>
{
};
template<class Archive>
struct specialize<Archive, Luminary::ModelObject *, cereal::specialization::non_member_load_save>
{
};
template<class Archive>
struct specialize<Archive, Luminary::ModelVolume *, cereal::specialization::non_member_load_save>
{
};
template<class Archive>
struct specialize<Archive, Luminary::ModelInstance *, cereal::specialization::non_member_load_save>
{
};
template<class Archive>
struct specialize<Archive, Luminary::ModelMaterial *, cereal::specialization::non_member_load_save>
{
};
template<class Archive>
struct specialize<Archive, std::shared_ptr<Luminary::TriangleMesh>, cereal::specialization::non_member_load_save>
{
};
...
{
    ar(cereal::get_user_data<Luminary::UndoRedo::StackImpl>(ar).save_mutable_object<T>(*ptr));
}
...
{
    Luminary::UndoRedo::StackImpl &stack = cereal::get_user_data<Luminary::UndoRedo::StackImpl>(ar);
    size_t id;
    ar(id);
    ptr = stack.load_mutable_object<T>(Luminary::ObjectID(id));
}
...
{
    ar(cereal::get_user_data<Luminary::UndoRedo::StackImpl>(ar).save_mutable_object<T>(*ptr.get()));
}
...
void load(BinaryInputArchive &ar, std::unique_ptr<T> &ptr)
{
    ...
    size_t id;
    ar(id);
    ptr.reset(stack.load_mutable_object<T>(Luminary::ObjectID(id)));
}
...
{
    ar(cereal::get_user_data<Luminary::UndoRedo::StackImpl>(ar).save_mutable_object<T>(cfg));
}
// Load ObjectBase derived class from the Undo / Redo stack as a separate object
// based on the ObjectID loaded from this stream.
template<class T>
void load_by_value(BinaryInputArchive &ar, T &cfg)
{
    ...
    size_t id;
    ar(id);
    stack.load_mutable_object<T>(Luminary::ObjectID(id), cfg);
}
...
{
    ar(cereal::get_user_data<Luminary::UndoRedo::StackImpl>(ar).save_immutable_object<T>(
        const_cast<std::shared_ptr<const T> &>(ptr), false));
}
template<class T>
void save_optional(BinaryOutputArchive &ar, const std::shared_ptr<const T> &ptr)
{
    ar(cereal::get_user_data<Luminary::UndoRedo::StackImpl>(ar).save_immutable_object<T>(
        const_cast<std::shared_ptr<const T> &>(ptr), true));
}
...
void load(BinaryInputArchive &ar, std::shared_ptr<const T> &ptr)
{
    ...
    size_t id;
    ar(id);
    ptr = stack.load_immutable_object<T>(Luminary::ObjectID(id), false);
}
template<class T>
void load_optional(BinaryInputArchive &ar, std::shared_ptr<const T> &ptr)
{
    ...
    size_t id;
    ar(id);
    ptr = stack.load_immutable_object<T>(Luminary::ObjectID(id), true);
}
} // namespace cereal

#include <luminary/model/scene/Model.hpp>      // IWYU pragma: keep
#include <luminary/mesh/core/TriangleMesh.hpp> // IWYU pragma: keep
#include <DSKY/GUI/Selection.hpp>              // IWYU pragma: keep
#include <DSKY/GUI/Gizmos/GLGizmosManager.hpp> // IWYU pragma: keep
...
std::shared_ptr<const T> &ImmutableObjectHistory<T>::shared_ptr(StackImpl &stack)
{
    if (m_shared_object.get() == nullptr && !m_serialized.empty())
    {
        ...
        {
            Luminary::UndoRedo::InputArchive archive(stack, iss);
...
ObjectID StackImpl::save_mutable_object(const T &object)
{
    ...
    if (it_object_history == m_objects.end())
        it_object_history = m_objects.insert(it_object_history,
                                             std::make_pair(object.id(), std::unique_ptr<MutableObjectHistory<T>>(
                                                                             new MutableObjectHistory<T>())));
    ...
    if (needs_to_save)
    {
        ...
        {
            Luminary::UndoRedo::OutputArchive archive(*this, oss);
            archive(object);
        }
...
ObjectID StackImpl::save_immutable_object(std::shared_ptr<const T> &object, bool optional)
{
    ...
    if (it_object_history == m_objects.end())
        it_object_history = m_objects.emplace_hint(it_object_history, object_id,
                                                   std::unique_ptr<ImmutableObjectHistory<T>>(
                                                       new ImmutableObjectHistory<T>(object, optional)));
    else
        assert(it_object_history->second.get()->is_optional() == optional);
    // Then save the interval.
    static_cast<ImmutableObjectHistory<T> *>(it_object_history->second.get())
        ->save(m_active_snapshot_time, m_current_time);
    return object_id;
}

template<typename T>
T *StackImpl::load_mutable_object(const Luminary::ObjectID id)
{
    T *target = new T();
...
template<typename T>
std::shared_ptr<const T> StackImpl::load_immutable_object(const Luminary::ObjectID id, bool optional)
{
...
template<typename T>
void StackImpl::load_mutable_object(const Luminary::ObjectID id, T &target)
{
    // First find a history stack for the ObjectID of this object instance.
    auto it_object_history = m_objects.find(id);
    ...
    Luminary::UndoRedo::InputArchive archive(*this, iss);
    target.m_id = id;
    archive(target);
}

// Store the current application state onto the Undo / Redo stack, remove all snapshots after m_active_snapshot_time.
void StackImpl::take_snapshot(const std::string &snapshot_name, const Luminary::Model &model,
                              const DSKY::Selection &selection, const DSKY::GLGizmosManager &gizmos,
                              const SnapshotData &snapshot_data)
{
    ...
    if (!m_snapshots.empty())
    {
        ...
        topmost_saved = m_active_snapshot_time == m_saved_snapshot_time &&
                        m_active_snapshot_time == m_snapshots.back().timestamp;
        auto it = std::lower_bound(m_snapshots.begin(), m_snapshots.end(), Snapshot(m_active_snapshot_time));
        this->release_snapshots(it, m_snapshots.end());
    }
    // Take new snapshots.
    this->save_mutable_object<Luminary::Model>(model);
    ...
    this->save_mutable_object<DSKY::GLGizmosManager>(gizmos);
...
void StackImpl::reduce_noisy_snapshots(const std::string &new_name)
{
    ...
    assert(!m_snapshots.empty() && m_snapshots.back().is_topmost() &&
           m_snapshots.back().timestamp == m_active_snapshot_time);
    ...
    assert(it_last != m_snapshots.begin() &&
           (it_last->snapshot_data.snapshot_type == SnapshotType::LeavingGizmoNoAction ||
            it_last->snapshot_data.snapshot_type == SnapshotType::LeavingGizmoWithAction));
...
void StackImpl::load_snapshot(size_t timestamp, Luminary::Model &model, DSKY::GLGizmosManager &gizmos)
{
    ...
    if (it_snapshot == m_snapshots.end() || it_snapshot->timestamp != timestamp)
        throw Luminary::RuntimeError((boost::format("Snapshot with timestamp %1% does not exist") % timestamp).str());

    m_active_snapshot_time = timestamp;
    model.clear_objects();
    model.clear_materials();
    this->load_mutable_object<Luminary::Model>(ObjectID(it_snapshot->model_id), model);
    model.update_links_bottom_up_recursive();
    m_selection.volumes_and_instances.clear();
    this->load_mutable_object<Selection>(m_selection.id(), m_selection);
    this->load_mutable_object<DSKY::GLGizmosManager>(gizmos.id(), gizmos);
...
{
    return time_to_load < m_active_snapshot_time &&
           std::binary_search(m_snapshots.begin(), m_snapshots.end(), Snapshot(time_to_load));
}
...
bool StackImpl::undo(Luminary::Model &model, const DSKY::Selection &selection, DSKY::GLGizmosManager &gizmos,
                     const SnapshotData &snapshot_data, size_t time_to_load)
{
    assert(this->valid());
...
bool StackImpl::redo(Luminary::Model &model, DSKY::GLGizmosManager &gizmos, size_t time_to_load)
{
    assert(this->valid());
    if (time_to_load == SIZE_MAX)
    {
...
// Release snapshots between begin and end. Only erases data from m_snapshots, not from m_objects!
// Updates m_saved_snapshot_time.
std::vector<Snapshot>::iterator StackImpl::release_snapshots(std::vector<Snapshot>::iterator begin,
                                                             std::vector<Snapshot>::iterator end)
{
    assert(!m_snapshots.empty());
    assert(begin <= end);
    if (m_saved_snapshot_time != size_t(-1) && m_saved_snapshot_time >= begin->timestamp &&
        (end == m_snapshots.end() || m_saved_snapshot_time < end->timestamp))
    {
...
void StackImpl::release_least_recently_used()
{
    ...
    while (current_memsize > m_memory_limit && m_snapshots.size() >= 3)
    {
        ...
        else
        {
            ...
            if (m_saved_snapshot_time == m_snapshots.front().timestamp)
            {
                // The saved snapshot is going away. The next snapshot captures the same project state if the
                // erased one does not modify it, otherwise the saved state is lost and the project counts as
                // "likely modified" until it is saved again.
                m_saved_snapshot_time = snapshot_modifies_project(m_snapshots.front()) ? size_t(-1)
                                                                                       : m_snapshots[1].timestamp;
            }
...
bool StackImpl::project_modified() const
{
    ...
    assert(m_saved_snapshot_time >= m_snapshots.front().timestamp &&
           m_saved_snapshot_time <= m_snapshots.back().timestamp);
...
void Stack::set_memory_limit(size_t memsize)
{
    pimpl->set_memory_limit(memsize);
}
size_t Stack::get_memory_limit() const
{
    return pimpl->get_memory_limit();
}
size_t Stack::memsize() const
{
    return pimpl->memsize();
}
void Stack::release_least_recently_used()
{
    pimpl->release_least_recently_used();
}
void Stack::take_snapshot(const std::string &snapshot_name, const Luminary::Model &model,
                          const DSKY::Selection &selection, const DSKY::GLGizmosManager &gizmos,
                          const SnapshotData &snapshot_data)
{
    pimpl->take_snapshot(snapshot_name, model, selection, gizmos, snapshot_data);
}
void Stack::reduce_noisy_snapshots(const std::string &new_name)
{
    pimpl->reduce_noisy_snapshots(new_name);
}
bool Stack::has_undo_snapshot() const
{
    return pimpl->has_undo_snapshot();
}
bool Stack::has_undo_snapshot(size_t time_to_load) const
{
    return pimpl->has_undo_snapshot(time_to_load);
}
bool Stack::has_redo_snapshot() const
{
    return pimpl->has_redo_snapshot();
}
bool Stack::undo(Luminary::Model &model, const DSKY::Selection &selection, DSKY::GLGizmosManager &gizmos,
                 const SnapshotData &snapshot_data, size_t time_to_load)
{
    return pimpl->undo(model, selection, gizmos, snapshot_data, time_to_load);
}
bool Stack::redo(Luminary::Model &model, DSKY::GLGizmosManager &gizmos, size_t time_to_load)
{
    return pimpl->redo(model, gizmos, time_to_load);
}
const Selection &Stack::selection_deserialized() const
{
    return pimpl->selection_deserialized();
}

const std::vector<Snapshot> &Stack::snapshots() const
{
    return pimpl->snapshots();
}
const Snapshot &Stack::snapshot(size_t time) const
{
    return pimpl->snapshot(time);
}
size_t Stack::active_snapshot_time() const
{
    return pimpl->active_snapshot_time();
}
bool Stack::temp_snapshot_active() const
{
    return pimpl->temp_snapshot_active();
}
void Stack::mark_current_as_saved()
{
    pimpl->mark_current_as_saved();
}
bool Stack::project_modified() const
{
