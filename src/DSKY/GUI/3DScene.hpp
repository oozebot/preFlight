...
{
public:
    // Selected-object colors follow the active theme accent (read at render time), so they
...
public:
    ...
    struct CompositeID
    {
        ...
        // and which has no associated ModelVolume in ModelObject.volumes.
    ...
    std::unique_ptr<DSKY::MeshRaycaster> mesh_raycaster;
...
struct GLRenderContext
{
    ...
    const DSKY::Camera *camera{nullptr};
    const Model *model{nullptr};
    const AppConfig *app_config{nullptr};
    const DSKY::ImGuiWrapper *imgui{nullptr};
...
private:
    ...
    {
        size_t extruder_id;
        std::unique_ptr<DSKY::TriangleSelectorMmGui> triangle_selector_mm;
...
struct _3DScene
{
    ...
    static void thick_lines_to_verts(const Lines3 &lines, const std::vector<double> &widths,
                                     const std::vector<double> &heights, bool closed,
                                     DSKY::GLModel::Geometry &geometry);
