...
public:
    ...
    const Luminary::UndoRedo::Stack &undo_redo_stack_main() const;
    void clear_undo_redo_stack_main();
    // Enter / leave the Gizmos specific Undo / Redo stack.
    ...
    // Extruder colors: the printer's extruder_colour where set, else the filament color (Tool
    // view, thumbnails).
    std::vector<std::string> get_extruder_color_strings_from_plater_config(
        const GCodeProcessorResult *const result = nullptr) const;
    // Filament colors only (the filament color print view).
    std::vector<std::string> get_filament_color_strings(const GCodeProcessorResult *const result = nullptr) const;
    // Filament colors followed by the color-change colors (the Color print view).
