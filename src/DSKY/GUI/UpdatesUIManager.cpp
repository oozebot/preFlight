...
RepositoryUpdateUIManager::RepositoryUpdateUIManager(wxWindow *parent, Luminary::PresetUpdaterWrapper *puw, int em)
        ...
        for (const auto &entry : m_offline_entries)
        {
...

            {
                ...
                btn->Bind(wxEVT_BUTTON, [&entry](wxCommandEvent &event)
                          { DSKY::desktop_open_folder(entry.source_path.parent_path().make_preferred()); });
                add(btn);
            }
...
ManagePresetRepositoriesDialog::ManagePresetRepositoriesDialog(Luminary::PresetUpdaterWrapper *puw)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr("%1% - %2%", PREFLIGHT_APP_NAME, _L("Manage Updates")), wxDefaultPosition, wxDefaultSize,
