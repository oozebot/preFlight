...
public:
    ...
    // Unhandled error produced by stdlib or a Win32 structured exception, or an unhandled critical exception of
    // preFlight's own.
...
// As of now this class is not declared in Luminary::GUI due to the Perl bindings limits.
...
public:
    ...
    // release_memory: after an invalidation of the exported G-code, compact the heap and trim the working
    // set. Pointless when a re-slice follows at once, so callers that restart automatically pass false.
    PrintBase::ApplyStatus apply(const Model &model, const DynamicPrintConfig &config,
                                 std::vector<std::string> *warnings = nullptr, bool release_memory = true);
    ...
    void direct_prepare_upload(Luminary::PrintHostJob &upload_job);

    // Copy of the sliced G-code text for Export to Script (throws ExportError when there is none), and the
    // script time limit the Print carries, both read on the UI thread before the export job starts.
    std::string copy_gcode_text() const;
    double preprocessing_timeout_seconds() const;
    ...
    // A slice is being computed right now: started or running, its result not yet complete.
    bool in_flight() const { return m_state == STATE_STARTED || m_state == STATE_RUNNING; }
...
private:
    ...
    // Calls thread_proc_safe_seh(), rethrows a Luminary::HardCrash exception based on SEH exception
    ...
    // Calls call_process_seh(), rethrows a Luminary::HardCrash exception based on SEH exception
    ...
    DSKY::ISlicingEventPoster *m_slicing_event_poster{nullptr};
};
