...
// Repeated here because the engine types above are only forward declared.
...
private:
    ...
    bool m_prevent_canvas_selection_update = false; // This flag prevents changing selection on the canvas while the
                                                    // list is rebuilt: updating canvas selection is undesirable there,
    // because it would turn off the gizmos

    wxDataViewItem m_last_selected_item{nullptr};
    wxDataViewItem m_override_press_item{nullptr}; // the row whose override cell took the left button
#ifdef __WXOSX__
    // The hover tooltip. Cocoa arms a view's tooltip once per pointer entry into the view, and the
    // outline view's per-cell tooltips share that one rect, so the list shows the system-styled
    // panel of ObjectListMac for the icon the pointer rests on and hides it when the pointer moves.
    wxTimer m_tooltip_timer;
    wxDataViewItem m_hover_item{nullptr};
    wxDataViewColumn *m_hover_column{nullptr};
    bool m_hover_on_icon{false};
    void show_hover_tooltip();
    void hide_hover_tooltip();
#endif
#ifdef __WXMSW__
...
public:
    ...
    // The tooltip of one cell: the override count, the printable icon's hint, or the mesh warning
    // when x (in control coordinates) is on the Name column's icon. Empty for no tooltip.
    wxString cell_tooltip(const wxDataViewItem &item, const wxDataViewColumn *col, int x);
    ...
    // The override column redraws from the item's config
    void update_override_column(const wxDataViewItem &item);
    // The override panel for an object, a part, a modifier or a layer range: selects the item
    // and opens the panel under the list; close_overrides restores the item's own panels
    void open_overrides(const wxDataViewItem &item);
    void close_overrides();
