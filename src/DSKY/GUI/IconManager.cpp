...
NSVGimage *parse_file(const char *filepath)
{
    ...
    Luminary::ScopeGuard sg([fp]() { fclose(fp); });
...
void subdata(unsigned char *data, size_t data_stride, const std::vector<unsigned char> &data2, size_t data2_row)
{
    ...
    for (size_t data2_offset = 0, data_offset = 0; data2_offset < data2.size();
...
IconManager::Icons IconManager::init(const InitTypes &input)
{
    ...
    int width = (surface_sqrt >= 4096 * 0.7f)   ? 4096
    ...
    std::sort(idx.begin(), idx.end(),
              [&input](size_t i1, size_t i2) { return input[i1].filepath < input[i2].filepath; });
    for (size_t j : idx)
    {
    ...
    glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei) tex_size.x, (GLsizei) tex_size.y, 0, GL_RGBA,
...
std::vector<IconManager::Icons> IconManager::init(const std::vector<std::string> &file_paths, const ImVec2 &size,
                                                  RasterType type)
{
...
bool button(const IconManager::Icon &activ, const IconManager::Icon &hover, const IconManager::Icon &disable,
            bool disabled)
{
    if (disabled)
    {
