...
namespace Luminary::internal::format
{
...
// Accept wxString and convert it to UTF8 to be processed by Luminary::format().
...
// Why Luminary::internal::format::cook(T&& arg) is taken for non-const wxString reference?
...
inline wxString format_wxstr(const char *fmt, TArgs &&...args)
{
    ...
    return wxString::FromUTF8(
        Luminary::internal::format::format_recursive(message, std::forward<TArgs>(args)...).c_str());
}
template<typename... TArgs>
...
{
    return Luminary::format(fmt, std::forward<TArgs>(args)...);
}
template<typename... TArgs>
...
{
    return Luminary::format(fmt.ToUTF8().data(), std::forward<TArgs>(args)...);
}
