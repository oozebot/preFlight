...
ObjectDataViewModelNode::ObjectDataViewModelNode(ObjectDataViewModelNode *parent, const wxString &sub_obj_name,
                                                 Luminary::ModelVolumeType type, const bool is_text_volume,
    ...
    , m_idx(idx)
    , m_extruder(type == Luminary::ModelVolumeType::MODEL_PART || type == Luminary::ModelVolumeType::PARAMETER_MODIFIER
                     ? extruder
                     : "")
{
...
void ObjectDataViewModelNode::set_extruder_icon()
{
    if (m_type & (itInstance | itInstanceRoot | itLayerRoot) ||
        ((m_type & itVolume) && m_volume_type != Luminary::ModelVolumeType::MODEL_PART &&
         m_volume_type != Luminary::ModelVolumeType::PARAMETER_MODIFIER))
...
static int get_root_idx(ObjectDataViewModelNode *parent_node, const ItemType root_type)
{
    ...
    {
        // if there is VolumeItem, then RootItems don't exist in current ObjectItem
        if (parent_node->GetNthChild(root_idx)->GetType() & itVolume)
            break;
...
wxDataViewItem ObjectDataViewModel::AddVolumeChild(const wxDataViewItem &parent_item, const wxString &name,
                                                   const int volume_idx, const Luminary::ModelVolumeType volume_type,
...
wxDataViewItem ObjectDataViewModel::Delete(const wxDataViewItem &item)
{
    ...
    // MyObjectTreeModelNodePtrArray is only an array of _pointers_
...
void ObjectDataViewModel::GetItemInfo(const wxDataViewItem &item, ItemType &type, int &obj_idx, int &idx)
{
    ...
    if (!node || node->GetIdx() < -1 ||
        (node->GetIdx() == -1 && !(node->GetType() & (itObject | itInstanceRoot | itLayerRoot | itInfo))))
        return;
...
{
    if (col == colName || col == colExtruder || col == colEditing)
...
void ObjectDataViewModel::GetValue(wxVariant &variant, const wxDataViewItem &item, unsigned int col) const
{
    ...
    case colEditing:
    {
        const int count = m_override_count ? m_override_count(item) : 0;
        variant << DataViewBitmapText(count > 0 ? wxString::Format("%d", count) : wxString(),
                                      node->m_action_icon.GetBitmapFor(m_ctrl));
        break;
    }
    default:;
    }
