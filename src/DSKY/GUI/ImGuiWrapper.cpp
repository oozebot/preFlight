...
// suggest location
#include "luminary/geometry/clipper/ClipperUtils.hpp" // Luminary::intersection
                                ...
                                bool show_edit_btn /*= true*/)
{
    ...
    if (show_edit_btn)
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                              DSKY::Theme::Primary::GetImGuiColor()); // Brand color hover
        ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                              DSKY::Theme::PrimaryDark::GetImGuiColor()); // Brand color active
...
ImVec2 ImGuiWrapper::suggest_location(const ImVec2 &dialog_size, const Luminary::Polygon &interest,
                                      const ImVec2 &canvas_size)
{
    ...
    assert(Luminary::intersection(interest, Luminary::Polygon(window_polygon)).empty());
    ...
    do
    {
        ...
        for (Point &p : moved_polygon)
            p += move_point;
        if (Luminary::intersection(interest, Luminary::Polygon(moved_polygon)).empty())
...
void ImGuiWrapper::draw(const Luminary::Polygon &polygon, ImDrawList *draw_list /* = ImGui::GetOverlayDrawList()*/,
                                                  ...
                                                  unsigned target_height)
{
    ...
    // Also recolor the brand orange baked into the icon SVGs to the active theme accent
    // (no-op for orange themes). A bare-hex replace covers every fill/stroke format these icons use.
    {
        const wxColour &acc = DSKY::active_palette().accent_primary;
        auto byte_hex = [](int v)
        {
    ...
    NSVGimage *image = BitmapCache::nsvgParseFromFileWithReplace(Luminary::var(bitmap_name + ".svg").c_str(), "px",
                                                                 96.0f, color_replaces);
    if (image == nullptr)
        return empty_vector;
...
void ImGuiWrapper::init_font(bool compress)
{
    ...
    builder.AddChar(ImWchar(0x2026)); // horizontal ellipsis
    ...
    ImFont *font = io.Fonts->AddFontFromFileTTF(
        (Luminary::resources_dir() + "/fonts/" + "NotoSans-Regular.ttf").c_str(), m_font_size, nullptr, ranges.Data);
    if (s_font_cjk)
    {
        ImFontConfig config;
        config.MergeMode = true;
        io.Fonts->AddFontFromFileTTF((Luminary::resources_dir() + "/fonts/" + "NotoSansCJK-Regular.ttc").c_str(),
    ...
    if (font == nullptr)
    {
        ...
        if (font == nullptr)
        {
            throw Luminary::RuntimeError("ImGui: Could not load deafult font");
        }
    }

    // Load a second font at the legend sidebar's native pixel size (11pt) for crisp rendering
    m_legend_font =
        io.Fonts->AddFontFromFileTTF((Luminary::resources_dir() + "/fonts/" + "NotoSans-Regular.ttf").c_str(),
...
void ImGuiWrapper::init_style()
{
    ...
    // ImGui colors come from the active theme palette.
    const DSKY::ThemePalette &pal = DSKY::active_palette();
    auto im = [](const DSKY::RGBAf &c)
    {
    ...
    // Refresh the shared brand-accent ImGui colors from the active theme so every overlay
    ...
    set_color(ImGuiCol_CheckMark, DSKY::Theme::Primary::GetImGuiColor());
    ...
    // Slider
    set_color(ImGuiCol_SliderGrab, DSKY::Theme::PrimaryDark::GetImGuiColor());
    set_color(ImGuiCol_SliderGrabActive, DSKY::Theme::Primary::GetImGuiColor());
