...
static wxString generate_html_row(const Config::Snapshot &snapshot, bool row_even, bool snapshot_active, bool dark_mode)
{
    ...
    text += wxString("<font size=\"5\"><b>") + (snapshot_active ? _(L("Active")) + ": " : "") + datetime + ": " +
            format_reason(snapshot.reason);

    if (!snapshot.comment.empty())
        text += " (" + wxString::FromUTF8(snapshot.comment.data()) + ")";
    text += "</b></font><br>";
    // End of row header.
    text += _(L("preFlight version")) + ": " + snapshot.preflight_version_captured.to_string() + "<br>";
    text += _(L("print")) + ": " + snapshot.print + "<br>";
    text += _(L("filaments")) + ": " + snapshot.filaments.front() + "<br>";
    text += _(L("printer")) + ": " +
            (snapshot.physical_printer.empty() ? snapshot.printer : snapshot.physical_printer) + "<br>";

    bool compatible = true;
    for (const Config::Snapshot::VendorConfig &vc : snapshot.vendor_configs)
    {
        text += _(L("vendor")) + ": " + vc.name + ", " + _(L("version")) + ": " +
                vc.version.config_version.to_string() + ", " + _(L("min preFlight version")) + ": " +
                vc.version.min_preflight_version.to_string();
        if (vc.version.max_preflight_version != Semver::inf())
            text += ", " + _(L("max preFlight version")) + ": " + vc.version.max_preflight_version.to_string();
        text += "<br>";
        ...
        if (!vc.version.is_current_preflight_supported())
        {
            compatible = false;
        }
    }
    ...
    if (!compatible)
    {
        text += "<p align=\"right\">" + format_wxstr(_L("Incompatible with this %s"), PREFLIGHT_APP_NAME) + "</p>";
    }
...
ConfigSnapshotDialog::ConfigSnapshotDialog(const Config::SnapshotDB &snapshot_db, const wxString &on_snapshot)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _(L("Configuration Snapshots")),
                wxDefaultPosition, wxSize(45 * wxGetApp().em_unit(), 40 * wxGetApp().em_unit()),
