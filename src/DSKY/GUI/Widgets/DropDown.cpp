...
{
    return (DSKY::wxGetApp().em_unit() * 12) / 10; // 12px at 100% DPI
}

static int GetScaledItemsPadding()
{
    return DSKY::wxGetApp().em_unit() / 5; // 2px at 100% DPI
}
...
{
    return (DSKY::wxGetApp().em_unit() * 6) / 10; // 6px at 100% DPI
}

static int GetScaledStandardMargin()
{
    return DSKY::wxGetApp().em_unit(); // 10px at 100% DPI
}
...
DropDown::DropDown(std::vector<wxString> &texts, std::vector<wxBitmapBundle> &icons)
    : texts(texts)
    , icons(icons)
    , radius(DSKY::wxGetApp().suppress_round_corners() ? 0 : GetScaledCornerRadius())
    , state_handler(this)
...
void DropDown::render(wxDC &dc)
{
    ...
    wxRect rc(0, 0, size.x, size.y);
    rc.Deflate(1, 1);
    dc.SetPen(wxPen(DSKY::active_palette().static_box_border));
    ...
    const int deflate_x = (DSKY::wxGetApp().em_unit() * 4) / 10; // 4px at 100% DPI
    const int deflate_y = DSKY::wxGetApp().em_unit() / 10;       // 1px at 100% DPI
    ...
    const wxColour selected_fill = DSKY::active_palette().highlight_background;
    const int inset = std::max(1, DSKY::wxGetApp().em_unit() / 10);
