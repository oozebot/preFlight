...
void CollapsibleSection::CreateHeader()
{
    ...
    m_icon->Hide();
    m_icon->Bind(wxEVT_LEFT_UP, &CollapsibleSection::OnHeaderClick, this);
    ...
    m_badge->Hide();
    m_badge->Bind(wxEVT_LEFT_UP, &CollapsibleSection::OnHeaderClick, this);
...
{
    // Icon-set chevrons: the SVG loader themes them (black in light mode, white in dark mode) and
    // renders them per DPI like every other header icon, and a wxStaticBitmap can be swapped between
    // two SVG bundles any number of times.
    const int size = m_compact ? COMPACT_CHEVRON_SIZE : CHEVRON_SIZE;
    m_chevron_expanded = *get_bmp_bundle("chevron_down", size);
    m_chevron_collapsed = *get_bmp_bundle("chevron_right", size);
    m_chevron_blank = *get_bmp_bundle("chevron_none", size);
    ApplyChevronBitmap();
}

void CollapsibleSection::ApplyChevronBitmap()
{
    // The chevron keeps its slot in every state so the icon and title never shift; a header that
    // cannot collapse shows a blank of the same size.
    if (!m_chevron)
        return;
    if (!m_collapsible)
        m_chevron->SetBitmap(m_chevron_blank);
    else
...
void CollapsibleSection::SetExpanded(bool expanded, bool animate, bool relayout)
{
...
void CollapsibleSection::ToggleExpanded()
{
    if (!m_collapsible)
        return;
    // Freeze the list this section sits in so the chevron, the content and the rows below repaint
    // together after the re-layout, instead of the chevron flipping first.
    wxWindowUpdateLocker frozen(GetParent() ? GetParent() : this);
    SetExpanded(!m_expanded, false, m_relayout_on_toggle);
}
...
void CollapsibleSection::SetCollapsible(bool collapsible, bool relayout)
{
    m_collapsible = collapsible;

    // The chevron slot stays; only its bitmap and the hand cursor change
    ApplyChevronBitmap();
    if (m_header_panel)
    {
        const wxCursor cursor = collapsible ? wxCursor(wxCURSOR_HAND) : wxNullCursor;
        m_header_panel->SetCursor(cursor);
...
void CollapsibleSection::SetHeaderVisible(bool visible)
{
    ...
    if (m_chevron)
        m_chevron->Show(visible);
    // Showing or hiding a child does not clear the cached best size of its parents, so the
    // section would keep laying out at its previous minimum height.
    m_header_panel->InvalidateBestSize();
    // No UpdateLayout() here; the caller lays out after batch changes
