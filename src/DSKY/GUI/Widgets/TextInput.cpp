...
{
    return (DSKY::wxGetApp().em_unit() * 4) / 10; // 4px at 100% DPI
}

static int GetScaledSmallMargin()
{
    return DSKY::wxGetApp().em_unit() / 2; // 5px at 100% DPI
}

static int GetScaledHeightPadding()
{
    return (DSKY::wxGetApp().em_unit() * 8) / 10; // 8px at 100% DPI
}
...
{
    if (DSKY::wxGetApp().suppress_round_corners())
        radius = 0;
    border_width = 1;
}
                       ...
                       const wxSize &size, long style)
{
    ...
    text_ctrl = new DSKY::ThemedTextCtrl(this, wxID_ANY, text, {small_padding, small_padding}, size,
                                         style | wxBORDER_NONE);
#ifdef __WXOSX__
    ...
    {
        // Hide the native scrollbar and use the custom themed ScrollBar.
        ...
        // Scrollbar drag scrolls the text control
        ...
        // Only scroll text content when the user has clicked inside the text area.
...
#ifdef __APPLE__
    // Constructor label_color defaults are light-mode. Update for dark mode.
    if (is_dark)
    {
    ...
    // Custom context menu with only the essentials (no IME/Unicode/Reading order clutter)
...
void TextInput::SysColorsChanged()
{
...
#ifdef __APPLE__
    // Update label color for current theme.
    ...
    // Refresh the custom scrollbar on theme change
    if (m_scrollbar)
...
bool TextInput::Enable(bool enable)
{
    ...
    if (changed && text_ctrl)
    {
        ...
        bool dark = DSKY::wxGetApp().dark_mode();
        ...
        // Send EVT_ENABLE_CHANGED before the refresh so state_handler is updated first
    ...
    // macOS takes the same approach as Windows. A disabled native NSTextField ignores
    // the themed colors and shows the system disabled appearance. Keep text_ctrl enabled
