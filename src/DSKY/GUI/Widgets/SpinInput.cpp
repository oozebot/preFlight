...
void SpinInputBase::SysColorsChanged()
{
...
#ifdef __APPLE__
    // Update label color (e.g. "mm" unit text) for current theme.
    ...
    {
        button_inc->Rescale();
        // Rescale() resets radius/border_width via StaticBox::msw_rescale().
...
bool SpinInputBase::Enable(bool enable)
{
    ...
    if (changed && text_ctrl)
    {
        ...
        // Refresh the buttons directly; macOS doesn't always repaint child
...
#ifdef __APPLE__
    // On macOS, child NSViews render on top of the parent's drawing.
                       ...
                       long style, int min, int max, int initial)
{
    ...
    text_ctrl = new DSKY::ThemedTextCtrl(this, wxID_ANY, text, {20, 4}, wxDefaultSize,
                                         style | wxBORDER_NONE | wxTE_PROCESS_ENTER, wxTextValidator(wxFILTER_NUMERIC));
#ifdef __WXOSX__
...
{
    if (Luminary::is_approx(value, val))
        return;
