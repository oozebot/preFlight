...
{
    return DSKY::wxGetApp().em_unit() / 2; // 5px at 100%
}
...
Button::Button() : paddingSize(DSKY::wxGetApp().em_unit(), (DSKY::wxGetApp().em_unit() * 8) / 10) // 10x8 at 100%
{
...
void Button::Rescale()
{
    ...
    paddingSize = wxSize(DSKY::wxGetApp().em_unit(), (DSKY::wxGetApp().em_unit() * 8) / 10);
...
#ifdef __APPLE__
    // On macOS, use wxBufferedPaintDC (matching StaticBox::paintEvent) so
