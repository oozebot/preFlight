...
{
    return (DSKY::wxGetApp().em_unit() * 8) / 10; // 8px at 100%
}

static int GetScaledBorderWidth()
{
    return std::max(1, DSKY::wxGetApp().em_unit() / 10); // 1px at 100%, min 1px
}
...
#ifdef __WXOSX__
    // Suppress native background erase on macOS so doRender() has full
...
#ifdef __WXOSX__
            // On macOS Retina, the drawing rect is deflated to avoid
            // clipping.  Fill the whole rect with the parent/theme background first
            // so the native macOS background doesn't bleed through the gap.
            if (dc.GetContentScaleFactor() > 1.)
            {
                bool is_dark = DSKY::wxGetApp().dark_mode();
                // StateColor::Disabled (1<<16) is a matching rule, not a state flag.
                // The disabled state is the absence of the Enabled bit (1) in states.
