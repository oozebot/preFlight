...
public:
    ...
    // Insert at the front (index 0), shifting existing items down. Used to add a
...
private:
    ...
    // Fire the dismiss callback, notify the menu bar, and release the self-reference.
    ...
    // Guards NotifyDismissed() so the dismiss callback and self-ref release run
    // exactly once regardless of teardown path (OnDismiss, item activation, or destruction).
    bool m_dismissNotified{false};

    // Time the menu was last shown (for is_in_show_grace()).
