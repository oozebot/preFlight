...
{
    return (DSKY::wxGetApp().em_unit() * 4) / 10; // 4px at 100%
}

static int GetScaledThumbWidth()
{
    return (DSKY::wxGetApp().em_unit() * 12) / 10; // 12px at 100%
}

static int GetScaledThumbHeight()
{
    return DSKY::wxGetApp().em_unit() * 2; // 20px at 100%
}

static int GetScaledCornerRadius()
{
    return DSKY::wxGetApp().em_unit() / 5; // 2px at 100%
}
...
void Slider::OnPaint(wxPaintEvent &event)
{
    ...
    wxColour bgColor = DSKY::wxGetApp().get_window_default_clr();
    ...
    wxColour trackColor = bgColor.ChangeLightness(DSKY::wxGetApp().dark_mode() ? 150 : 85);
