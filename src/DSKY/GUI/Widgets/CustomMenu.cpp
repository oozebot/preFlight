...
#ifdef __linux__
// Detect labwc compositor to disable rounded popup corners.
...
int CustomMenuMouseFilter::FilterEvent(wxEvent &event)
{
    ...
    {
        // Ignore an outside-click within the show grace window. Under seamless /
...
void CustomMenu::CleanupAllMenus()
{
    ...
    // Remove menus from their parents to prevent double-delete
...
CustomMenu::~CustomMenu()
{
    // Do not call m_selfRef.reset() here.
    ...
    // Final safety net. If we are torn down without OnDismiss() or item activation
...
#ifdef __APPLE__
    // On macOS, wxWidgets operates in logical (point) coordinates.
...
#ifdef __linux__
    // Square corners on labwc - its XWayland ignores X11 shape clipping
...
#elif defined(__APPLE__)
    // On macOS, make the window non-opaque with a clear background
...
void CustomMenu::ShowAt(const wxPoint &pos, wxWindow *parent)
{
    ...
    // Set the position before the size, to establish the DPI context
...
#ifdef __APPLE__
    // On macOS, wxPopupTransientWindow::Popup() installs Cocoa event
    // tracking that steals focus and doesn't release it on dismiss, blocking
    // left-click events. Bypass it entirely: Show()+Raise() is enough since
...
#ifdef __WXGTK__
    // On Linux/GTK3, clip the popup to a rounded rect using a cairo region.
...
std::shared_ptr<CustomMenu> CustomMenu::FromWxMenu(wxMenu *menu, wxWindow *eventHandler)
{
    ...
    // Use shared_ptr(new ...) instead of make_shared
...
void CustomMenu::OnDismiss()
{
    ...
    // Fire the dismiss callback and release the self-reference exactly once.
...
#ifdef __APPLE__
    // Popup() is bypassed on macOS, so only hide the window here.
...
void CustomMenu::DrawItem(wxDC &dc, const CustomMenuItem &item, const wxRect &rect, bool isHovered)
{
    ...
    if (m_iconWidth > 0)
    {
        ...
        else if (item.icon.IsOk())
        {
            ...
            {
#ifdef __APPLE__
                // On Retina, GetHeight() returns physical pixels but
...
bool CustomMenu::ProcessLeftDown(wxMouseEvent &event)
{
    ...
    // If this is a submenu, check whether the click is on the parent menu's submenu item
...
void CustomMenu::OnAppActivate(wxActivateEvent &evt)
{
    ...
    {
        // Under seamless/virtualized compositors (e.g. Qubes) the popup is its own X
...
void CustomMenu::ActivateItem(int index)
{
    ...
    // Find the root menu before calling CloseAllSubmenus,
    ...
    // Keep the root alive across the action and the dismissal below, in case the action
    ...
    // Item activation bypasses OnDismiss(), so fire the dismiss callback and release
    // the self-reference here. Otherwise the menu and its consumer state leak (e.g.
