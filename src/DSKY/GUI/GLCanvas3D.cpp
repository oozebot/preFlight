...
#include "DSKY/GUI/Gizmos/GLGizmoPainterBase.hpp"
...
#include <DSKY/GUI/Gizmos/TriangleSelectorMmGui.hpp>
...
static const Luminary::ColorRGBA ERROR_BG_DARK_COLOR = {0.478f, 0.192f, 0.039f, 1.0f};
static const Luminary::ColorRGBA ERROR_BG_LIGHT_COLOR = {0.753f, 0.192f, 0.039f, 1.0f};
...
Rect GLCanvas3D::LayersEditing::get_bar_rect_screen(const GLCanvas3D &canvas)
{
    ...
    // Beside the shown sidebar, at the canvas edge while the sidebar is hidden.
    if (wxGetApp().legacy_prepare_layout())
    {
        const float right = w - canvas.get_shown_right_panel();
        return {right - bar_w, 0.0f, right, h};
    }
    const float left = canvas.get_shown_left_panel();
    return {left, 0.0f, left + bar_w, h};
}
...
void GLCanvas3D::LayersEditing::render_active_object_annotations(const GLCanvas3D &canvas)
{
    ...
    // The bar's geometry is built at the canvas edge and placed beside the shown sidebar by a translation, so
    // it stays cached when the sidebar is shown or hidden.
    const float bar_shift_px = wxGetApp().legacy_prepare_layout() ? -canvas.get_shown_right_panel()
                                                                  : canvas.get_shown_left_panel();
...
void GLCanvas3D::LayersEditing::render_profile(const GLCanvas3D &canvas)
{
    ...
    {
        shader->start_using();
        // Placed beside the shown sidebar like the bar's background, with the cached geometry at the canvas edge.
        const float bar_shift_px = wxGetApp().legacy_prepare_layout() ? -canvas.get_shown_right_panel()
                                                                      : canvas.get_shown_left_panel();
        shader->set_uniform("view_model_matrix",
                            Geometry::translation_transform(Vec3d(2.0 * bar_shift_px / cnv_width, 0.0, 0.0)));
...
void GLCanvas3D::LayersEditing::adjust_layer_height_profile()
{
    this->update_slicing_parameters();
    PrintObject::update_layer_height_profile(*m_model_object, *m_slicing_parameters, m_layer_height_profile);
    Luminary::adjust_layer_height_profile(*m_slicing_parameters, m_layer_height_profile, this->last_z, this->strength,
...
void GLCanvas3D::LayersEditing::generate_layer_height_texture()
{
    ...
    m_layers_texture.cells = DSKY::generate_layer_height_texture(
        *m_slicing_parameters, Luminary::generate_object_layers(*m_slicing_parameters, m_layer_height_profile),
...
wxDEFINE_EVENT(EVT_GLCANVAS_PREVIEW_LEFT_CLICK, RBtnEvent);
                                             ...
                                             bool selection_only) const
{
    ...
    auto volumes_to_process_idxs = [this, &volumes, selection_only]()
    {
        ...
        {
            const DSKY::Selection::IndicesList &selected_volume_idxs = m_selection.get_volume_idxs();
    ...
    const Luminary::BuildVolume &build_volume = m_bed.build_volume();
                                     ...
                                     (!volume->is_wipe_tower() && volume->composite_id.volume_id >= 0)))
        {
            ...
            else
            {
                ...
                case BuildVolume::Type::Rectangle:
                    // The bounding box test does not evaluate non-convex objects against the build volume exactly.
                    state = build_volume.volume_state_bbox(volume_bbox(*volume), true, &bed_idx);
                    break;
                case BuildVolume::Type::Circle:
                case BuildVolume::Type::Convex:
                // Tested against the convex hull of the bed, so a concave bed shape is not evaluated exactly.
...
BoundingBoxf3 GLCanvas3D::scene_bounding_box() const
{
    ...
    // Pad the camera scene box by one print height above and below the bed.
...
void GLCanvas3D::animate_pivot_to(const Vec3d &target)
{
    m_pivot_anim.active = true;
    m_pivot_anim.start = std::chrono::steady_clock::now();
    m_pivot_anim.from = get_camera().get_target();
    m_pivot_anim.to = target;
    m_dirty = true;
    request_extra_frame();
}
...
// center_x: horizontal center of the scene beside the Preview's strip, where the window is placed.
void begin_statistics(const char *window_name, float center_x)
{
    ...
    const ImVec2 position{center_x, y_postion};
...
void render_print_statistics(float scale, std::vector<std::unique_ptr<Print>> &prints,
                             const std::vector<ExtruderFilaments> &extruders_filaments, float center_x)
{
    ImGui::SetNextWindowContentSize(ImVec2(content_size_x, 0.0f));

    begin_statistics(_u8L("Statistics").c_str(), center_x);
    ImGui::Spacing();
...
void render_autoslicing_wait(ImGuiWrapper *imgui, float center_x)
{
    const float scale = imgui->get_style_scaling();
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(20.f * scale, 30.f * scale));
    begin_statistics((_u8L("Generating statistics") + " ...").c_str(), center_x);
...
void GLCanvas3D::render()
{
    ...
    m_in_render = true;
    Luminary::ScopeGuard in_render_guard([this]() { m_in_render = false; });
    ...
    {
        // this happens at startup when nothing has been saved under the application data directory yet
    ...
    // The scene is framed beside the strip reserved on the sidebar's side, in both views and whether or not the
    // strip's panel is shown, so nothing moves when a panel hides or the view switches. A hidden panel leaves
    // its strip empty: the viewport then widens by the strip on both sides, which keeps its center, so the
    // scene is drawn across the strip as well; the part past the far edge falls outside the canvas.
    const int scene_inset = int(std::lround(get_left_inset()));
    const int scene_right_inset = int(std::lround(get_right_inset()));
    const int framed_width = std::max(10, cnv_size.get_width() - scene_inset - scene_right_inset);
    const unsigned int viewport_height = std::max(10u, (unsigned int) cnv_size.get_height());
    const bool strip_hidden = scene_inset + scene_right_inset > 0 && is_strip_hidden();
    if (strip_hidden)
        camera.set_viewport(-scene_right_inset, 0,
                            (unsigned int) (cnv_size.get_width() + scene_inset + scene_right_inset), viewport_height,
                            (unsigned int) framed_width);
    else
        camera.set_viewport(scene_inset, 0, (unsigned int) framed_width, viewport_height);
    camera.apply_viewport();
    if (scene_inset + scene_right_inset > 0 && !strip_hidden)
    {
        // The strip keeps the canvas background; the scene passes never paint it.
        glsafe(::glViewport(0, 0, cnv_size.get_width(), cnv_size.get_height()));
    ...
    // on frames where the size hasn't changed. It runs here because a DPI change
    // (e.g. dragging across monitors) resizes the canvas outside the zoom-to-bed
    // path, which would otherwise leave ImGui with a stale display size and fonts.
    ...
    // Smooth pan toward a pivot picked with Alt + middle click.
    if (m_pivot_anim.active)
    {
        constexpr double PIVOT_ANIM_SECONDS = 0.22;
        const double t = std::chrono::duration<double>(std::chrono::steady_clock::now() - m_pivot_anim.start).count() /
                         PIVOT_ANIM_SECONDS;
        if (t >= 1.0)
        {
            camera.set_target(m_pivot_anim.to);
            m_pivot_anim.active = false;
        }
        else
        {
            const double eased = t * t * (3.0 - 2.0 * t);
            camera.set_target(m_pivot_anim.from + eased * (m_pivot_anim.to - m_pivot_anim.from));
            request_extra_frame();
        }
        m_dirty = true;
    }
    ...
    // Supersampled scene rendering (SSAA): the whole 3D scene draws into a scaled
    // offscreen target and resolves to the native framebuffer before the overlays.
    if (m_scene_supersampler == nullptr)
    ...
    {
        const Vec3d bed_offset = s_multiple_beds.get_bed_translation(s_multiple_beds.get_active_bed());
        ScenePasses::ExtraCasters toolpath_casters;
        if (preview_toolpaths)
        {
            toolpath_casters.render =
                [this](const Matrix4d &view, const Matrix4d &projection, const Vec3d &eye, bool gbuffer)
            {
                m_gcode_viewer.render_segments_for_pass(view, projection, eye, gbuffer);
            };
    ...
    if (!s_multiple_beds.is_autoslicing())
    {
        ...
        if (is_looking_downward)
        {
            ...
            if (m_scene_passes->active())
                m_scene_passes->render_bed_overlay(m_bed, camera,
        ...
        // Alt + middle click: resolve the pending pivot pick against this frame's
        // depth buffer (any rendered pixel, toolpaths included); the raycaster and
        // bed plane catch clicks on empty space. Must run before the gizmo renders
        // below invalidate the depth buffer.
        if (m_pivot_center_request.has_value())
        {
            const Point pick = m_pivot_center_request->cast<coord_t>();
            m_pivot_center_request.reset();
            const std::array<int, 4> &vp = camera.get_viewport();
            // The read targets the currently bound scene framebuffer, which is the
            // scaled offscreen target while SSAA is active.
            const bool ssaa_read = m_scene_supersampler != nullptr && m_scene_supersampler->active();
            const double read_scale = ssaa_read ? double(m_scene_supersampler->width()) / double(vp[2]) : 1.0;
            const int read_h = ssaa_read ? m_scene_supersampler->height() : vp[3];
    ...
    {
        // The overview replaces the toolpaths, not the legend: the active bed's legend stays in its strip,
        // and the overview centers on the scene beside it.
        if (m_gcode_viewer.has_data() && is_legend_shown())
            m_gcode_viewer.render_legend_standalone();
        const float overview_center_x = 0.5f * (get_left_inset() + float(cnv_size.get_width()) - get_right_inset());
        ...
        if (!all_finished)
        {
            render_autoslicing_wait(get_imgui(), overview_center_x);
...
// 2) Print bed
// 3) Wipe tower preview
// 4) Out of bed collision status & message overlay (texture)
            ...
            if (replaced_objects.find(obj_idx) == replaced_objects.end())
            {
                ...
                if (is_positive)
                {
                    ...
                    if (preview != nullptr && !preview->empty())
                    {
                        ...
                        v->mesh_raycaster = std::make_unique<DSKY::MeshRaycaster>(
...
void GLCanvas3D::on_idle(wxIdleEvent &evt)
{
    ...
    // Process state updates (toolbars, notifications, gizmos).
    ...
    {
        // In power-saving mode (Preview tab), only continue the idle loop
...
void GLCanvas3D::on_mouse_wheel_internal(MouseInput &mouse)
{
    ...
    if (mouse.shift)
    {
        ...
        // The scene's center is the viewport's, which the Preview offsets past its strip.
        const std::array<int, 4> &vp = get_camera().get_viewport();
        const Vec3d screen_center_3d_pos = _mouse_to_3d({vp[0] + vp[2] * 0.5, cnv_size.get_height() * 0.5});
...
{
    // Ignore schedule requests made during the render cycle when power saving. Code inside
    // render() calls schedule_extra_frame(0), and honouring it would arm a 33ms timer from
    // within the frame it schedules, looping forever. Every real render trigger comes from
    // on_idle's update checks or from event handlers.
...
void GLCanvas3D::on_mouse_internal(MouseInput &mouse)
{
    ...
    // Alt + middle press: center the view and orbit pivot on the point under the
    // cursor (the modifier reserves the whole gesture, so no drag conflict). Handled
    // before the ImGui/toolbar/gizmo early returns; the pick resolves during the
    // next render, when the depth buffer holds the current scene.
    if (mouse.type == MouseEventType::MiddleDown && mouse.alt)
    {
        if (_is_in_scene_viewport(pos.cast<double>()))
            m_pivot_center_request = pos.cast<double>();
        m_dirty = true;
    }
    ...
    {
#ifndef _WIN32
        // Track genuine LeftDown events on the canvas so we can filter
    ...
    else if (mouse.dragging)
    {
        ...
        {
            // If dragging over blank area with left button, rotate. Only the default
            // scheme orbits with the left button; in the other schemes the left
            // button selects, matching their namesake applications.
            if (!m_moving)
            {
                const std::string scheme = m_app_config != nullptr ? m_app_config->get("canvas_mouse_scheme")
                                                                   : std::string();
                const bool left_orbits = scheme.empty() || scheme == "default";
                if (left_orbits && (any_gizmo_active || mouse.cmd || m_hover_volume_idxs.empty()))
                    _camera_orbit_step(pos);
                else
                    m_mouse.drag.start_position_3D = Vec3d((double) pos.x(), (double) pos.y(), 0.0);
            }
        }
        else if ((mouse.middle_down || mouse.right_down) && _nav_action_for_drag(mouse) == ECameraNavAction::Orbit)
        {
            _camera_orbit_step(pos);
        }
        else if ((mouse.middle_down || mouse.right_down) && _nav_action_for_drag(mouse) == ECameraNavAction::Pan)
        {
            Camera &camera = get_camera();
    ...
    else if (mouse.is_any_button_up())
    {
                 ...
                 m_hover_volume_idxs.empty() && !is_layers_editing_enabled())
        {
            ...
            if (!mouse.shift && (!any_gizmo_active || !mouse.cmd) &&
                m_picking_enabled
#ifndef _WIN32
                // GTK/Cocoa deliver phantom LeftUp events to the canvas after popup menus
            ...
            // The Preview has no picking pass: the plater resolves the click against the shells and selects
            // through the object list, as a click does in Prepare.
            if (m_canvas_role == CanvasRole::Preview && is_editor() && _is_in_scene_viewport(pos.cast<double>())
#ifndef _WIN32
                && m_mouse.left_down_on_canvas
#endif
            )
...
#endif // ENABLE_RETINA_GL
                m_event_poster->postEvent(CanvasEventType::PreviewLeftClick, std::make_pair(logical_pos, mouse.shift));
            }
        }
        else if ((mouse.type == MouseEventType::RightUp))
        {
            ...
            if (!m_hover_volume_idxs.empty())
            {
                ...
                {
                    // Force the selection of the volume. check_for_already_contained avoids an extra
                    // "Add-Selection" snapshot.
...
{
    // wxPaintDC validates the paint region, preventing WM_PAINT from being
...
void GLCanvas3D::mouse_up_cleanup()
{
    ...
    // Flush the GPU pipeline and release the GL context after a drag ends
...
bool GLCanvas3D::_render_undo_redo_stack(const bool is_undo, float pos_x)
{
    ...
    ImGuiPureWrap::text(DSKY::format(is_undo ? _L_PLURAL("Undo %1$d Action", "Undo %1$d Actions", hovered + 1)
                                            ...
                                            const GLVolumeCollection &volumes, Camera::EType camera_type)
{
    ...
    for (GLVolume *vol : visible_volumes)
    {
        ...
        if (has_model)
        {
            ...
            // A single-color (unpainted) object renders in the themed "selected" accent color so
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    item.tooltip = _u8L("Add...") + " [" + DSKY::shortkey_ctrl_prefix() + "I]";
    ...
    item.tooltip = _u8L("Delete all") + " [" + DSKY::shortkey_ctrl_prefix() + "Del]";
    item.sprite_id = sprite_id++;
    item.left.action_callback = [this]()
    {
        if (m_canvas != nullptr)
    ...
    item.tooltip = _u8L("Copy") + " [" + DSKY::shortkey_ctrl_prefix() + "C]";
    item.sprite_id = sprite_id++;
    item.left.action_callback = [this]()
    {
        if (m_canvas != nullptr)
    ...
    item.tooltip = _u8L("Paste") + " [" + DSKY::shortkey_ctrl_prefix() + "V]";
    item.sprite_id = sprite_id++;
    item.left.action_callback = [this]()
    {
        if (m_canvas != nullptr)
    ...
    item.tooltip = _u8L("Switch to Settings") + "\n" + "[" + DSKY::shortkey_ctrl_prefix() + "2] - " + _u8L("Print Settings Tab")    + 
                                                "\n" + "[" + DSKY::shortkey_ctrl_prefix() + "3] - " + (current_printer_technology() == ptFFF ? _u8L("Filament Settings Tab") : _u8L("Material Settings Tab") +
                                                "\n" + "[" + DSKY::shortkey_ctrl_prefix() + "4] - " + _u8L("Printer Settings Tab")) ;
    item.sprite_id = sprite_id++;
...
bool GLCanvas3D::_init_undoredo_toolbar()
{
    ...
    item.tooltip = _u8L("Undo") + " [" + DSKY::shortkey_ctrl_prefix() + "Z]\n" +
    ...
    item.tooltip = _u8L("Redo") + " [" + DSKY::shortkey_ctrl_prefix() + "Y]\n" +
                   _u8L("Click right mouse button to open/close History");
    item.sprite_id = sprite_id++;
    item.left.action_callback = [this]()
    {
...
void GLCanvas3D::_rectangular_selection_picking_pass()
{
    ...
    if (m_picking_enabled)
    {
        // Detect which volumes fall inside the selection rectangle by projecting their
...
void GLCanvas3D::_render_objects(GLVolumeCollection::ERenderType type)
{
    ...
    // Full lighting tier: swap in the shadow/AO-aware model shader when this frame's
    // passes ran; a fallback-resolved program keeps the Enhanced look instead.
    const bool full_lighting_pass = m_scene_passes != nullptr && m_scene_passes->active();
    if (full_lighting_pass)
    {
        GLShaderProgram *full_shader = get_shader("phong_full");
        if (full_shader != nullptr && full_shader->get_name() == "phong_full")
            shader = full_shader;
    }
    if (shader != nullptr)
    {
        shader->start_using();
        if (full_lighting_pass && shader->get_name() == "phong_full")
            m_scene_passes->bind_scene_uniforms(*shader);
...
// The Preview shows the active bed only: true when every selected instance sits on it.
static bool selection_on_active_bed(const Selection &selection)
{
    ...
    const std::map<ObjectID, int> &inst_to_bed = s_multiple_beds.get_inst_map();
    for (const auto &[obj_idx, inst_idxs] : selection.get_content())
    {
        if (obj_idx < 0 || obj_idx >= int(model->objects.size()))
            return false;
        const ModelObject *object = model->objects[obj_idx];
        for (const int inst_idx : inst_idxs)
        {
            if (inst_idx < 0 || inst_idx >= int(object->instances.size()))
                return false;
            const auto it = inst_to_bed.find(object->instances[inst_idx]->id());
            if (it == inst_to_bed.end() || it->second != active_bed)
                return false;
        }
    }
    return true;
...
void GLCanvas3D::_render_selection()
{
    ...
    // The wipe tower has no row in the object list, so a box around it would match nothing the sidebar shows.
    if (m_shown_selection_fn)
    {
        Selection *shown = m_shown_selection_fn();
        if (shown != nullptr && !shown->is_wipe_tower() && selection_on_active_bed(*shown))
            shown->render(scale_factor);
    }
...
void GLCanvas3D::_render_overlays()
{
    ...
    // The toolbars and the layer-editing overlay are drawn in canvas coordinates, so they draw in the canvas
    // viewport rather than the scene's, which the strip narrows or a hidden panel widens.
    const Size overlay_size = get_canvas_size();
    glsafe(::glViewport(0, 0, overlay_size.get_width(), overlay_size.get_height()));
...
void DSKY::GLCanvas3D::_render_bed_selector()
{
    ...
    if (s_multiple_beds.get_number_of_beds() != 1 && is_preview_shown())
    {
        ...
        float win_x_pos = get_canvas_size().get_width() - get_right_inset();
        ...
        // The selector holds the plate thumbnails only. The slice-all button that opened the all-plates
        // statistics table is not offered: nothing acts on that table, each plate slices from its own
        // thumbnail, and the Preview re-slices the active plate on every change.
...
#if use_scrolling
        bool is_legend_visible = is_legend_shown() && m_gcode_viewer.has_data();
...
void GLCanvas3D::_render_main_toolbar()
{
    ...
    // Centered on the canvas beside the shown sidebar.
...
float GLCanvas3D::get_left_inset() const
{
    if (!m_strip_width_fn || wxGetApp().legacy_prepare_layout())
        return 0.0f;
    return m_strip_width_fn();
}

float GLCanvas3D::get_right_inset() const
{
    if (!m_strip_width_fn || !wxGetApp().legacy_prepare_layout())
        return 0.0f;
    return m_strip_width_fn();
...
bool GLCanvas3D::is_strip_hidden() const
{
    return m_strip_hidden_fn && m_strip_hidden_fn();
}

float GLCanvas3D::_shown_panel_offset() const
{
    // The center of the canvas beside the shown panel, relative to the canvas center.
    return 0.5f * (get_shown_left_panel() - get_shown_right_panel());
}

bool GLCanvas3D::_is_in_scene_viewport(const Vec2d &pos) const
{
    // A hidden panel's strip shows the scene as well.
    if (is_strip_hidden())
        return true;
    return pos.x() >= double(get_left_inset()) &&
           pos.x() < double(get_canvas_size().get_width()) - double(get_right_inset());
}

void GLCanvas3D::_render_sidebar_toggle_imgui()
{
    // In the Preview the button swaps the sidebar and the legend; the G-code viewer has no sidebar.
    if (is_preview_shown() && !is_editor())
        return;
    ...
    // The button sits at the inner edge of the layout side's panel: in the Preview the reserved strip, in Prepare
    // the floating sidebar, or the canvas edge while the sidebar is collapsed.
    const bool legacy = wxGetApp().legacy_prepare_layout();
    float panel_width = legacy ? get_right_inset() : get_left_inset();
    if (!is_preview_shown() && is_strip_hidden())
        panel_width = 0.0f;
    if (legacy)
        ImGuiPureWrap::set_next_window_pos((float) cnv_size.get_width() - panel_width - button_size, 0.0f,
                                           ImGuiCond_Always);
    else
        ImGuiPureWrap::set_next_window_pos(panel_width, 0.0f, ImGuiCond_Always);
    ...
    const DSKY::ThemePalette &pal = DSKY::active_palette();
...
void GLCanvas3D::_camera_orbit_step(const Point &pos)
{
...
GLCanvas3D::ECameraNavAction GLCanvas3D::_nav_action_for_drag(const MouseInput &mouse) const
{
    // Alt + middle belongs to pivot centering in every scheme.
    if (mouse.middle_down && mouse.alt)
        return ECameraNavAction::None;

    const std::string scheme = m_app_config != nullptr ? m_app_config->get("canvas_mouse_scheme") : std::string();
    if (scheme == "blender")
    {
        if (mouse.middle_down)
            return mouse.shift ? ECameraNavAction::Pan : ECameraNavAction::Orbit;
        return ECameraNavAction::Pan;
    }
    if (scheme == "fusion")
    {
        if (mouse.middle_down)
            return mouse.shift ? ECameraNavAction::Orbit : ECameraNavAction::Pan;
        return ECameraNavAction::Pan;
    }
    if (scheme == "solidworks")
    {
        if (mouse.middle_down)
            return mouse.ctrl ? ECameraNavAction::Pan : ECameraNavAction::Orbit;
        return ECameraNavAction::Pan;
    }
    if (scheme == "tinkercad")
        return mouse.right_down ? ECameraNavAction::Orbit : ECameraNavAction::Pan;

    // Default scheme: middle and right drag pan.
    return ECameraNavAction::Pan;
}
