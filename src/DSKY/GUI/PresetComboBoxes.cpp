...
void PresetComboBox::update(std::string select_preset_name)
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        if (m_type == Preset::TYPE_PRINTER)
            bitmap_key += "_printer";
        std::string main_icon_name = m_main_bitmap_name;
        ...
        // No separator between default and other presets
    }

    // Unified filament list - no System/User/Template/Incompatible separators
...
void PresetComboBox::open_physical_printer_url()
{
    ...
    std::string host = PhysicalPrinter::web_interface_host(pp.config);
    assert(!host.empty());
...
{
    // All presets are editable, no lock icons or compatibility flags
    is_compatible = true;
    is_system = false;
...
{
    if (m_type == Preset::TYPE_FILAMENT && m_preset_bundle->extruders_filaments.size() <= (size_t) m_extruder_idx)
        return;
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        // Bounds guard: ExtruderFilaments may be stale after preset import
        const bool is_compatible = (m_type == Preset::TYPE_FILAMENT && i < extruder_filaments.size())
                                       ? extruder_filaments.filament(i).is_compatible
                                       : preset.is_compatible;

        // Filaments are always shown regardless of compatibility
        if (m_type == Preset::TYPE_FILAMENT)
        {
            if (!preset.is_visible)
                continue;
        ...
        std::string bitmap_type_name = bitmap_key = m_main_bitmap_name;
        ...
        auto bmp = get_bmp(bitmap_key, wide_icons, bitmap_type_name, is_compatible,
                           preset.is_system || preset.is_default, single_bar, filament_rgb, extruder_rgb);
        assert(bmp);
    ...
    // Unified list - no System/User/Template separators
    {
        std::vector<PresetData> all_presets;
        all_presets.insert(all_presets.end(), system_presets.begin(), system_presets.end());
        all_presets.insert(all_presets.end(), nonsys_presets.begin(), nonsys_presets.end());
