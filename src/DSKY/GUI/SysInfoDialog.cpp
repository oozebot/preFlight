...
std::string get_main_info(bool format_as_html)
{
    ...
    if (!format_as_html)
        out << b_start << (wxGetApp().is_editor() ? PREFLIGHT_APP_NAME : GCODEVIEWER_APP_NAME) << b_end << line_end;
    out << b_start << "Version:   " << b_end << PREFLIGHT_VERSION << line_end;
    ...
    out << b_start << "Config:    " << b_end << PREFLIGHT_BUILD_TYPE << line_end;
...
std::string get_mem_info(bool format_as_html)
{
    ...
    const Luminary::UndoRedo::Stack &stack = wxGetApp().plater()->undo_redo_stack_main();
    out << b_start << "RAM size reserved for the Undo / Redo stack: " << b_end
        << Luminary::format_memsize_MB(stack.get_memory_limit()) << line_end;
...
SysInfoDialog::SysInfoDialog()
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                (wxGetApp().is_editor() ? wxString(PREFLIGHT_APP_NAME) : wxString(GCODEVIEWER_APP_NAME)) + " - " +
                ...
                wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    // title
    {
        wxStaticText *title = new wxStaticText(this, wxID_ANY,
                                               wxGetApp().is_editor() ? PREFLIGHT_APP_NAME : GCODEVIEWER_APP_NAME,
...
#endif // WIN32
        const auto text = DSKY::format_wxstr("<html>"
                                             "<body bgcolor= %s link= %s>"
                                             "<font color=%s>"
                                             "%s<br>%s<br>%s<br>%s"
                                             "</font>"
