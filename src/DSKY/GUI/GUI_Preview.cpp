...
bool Preview::init(wxWindow *parent, Bed3D &bed, Model *model)
{
    ...
    // The layers slider column is on the right unless the legacy layout moves it to the left.
    m_canvas->set_right_margin_fn(
        [this]() { return wxGetApp().legacy_prepare_layout() ? 0.0f : get_layers_slider_width(true); });
    m_canvas->allow_multisample(OpenGLManager::can_multisample());
...
void Preview::render_sliders(GLCanvas3D &canvas)
{
    ...
    // The sliders keep clear of the strip that holds the legend or the floating sidebar: the layers
    // slider moves to the left edge in the legacy layout, the moves slider centers beside the strip.
    const bool legacy_layout = wxGetApp().legacy_prepare_layout();
    if (m_layers_slider)
    {
        m_layers_slider->set_on_left(legacy_layout);
        // Use full canvas height now that view toolbar is hidden
        m_layers_slider->Render(canvas_width, canvas_height, extra_scale, 0.f);
    }
    if (m_moves_slider)
    {
        m_moves_slider->set_left_inset(canvas.get_left_inset());
        m_moves_slider->Render(canvas_width - static_cast<int>(canvas.get_right_inset()), canvas_height, extra_scale);
    }
}
...
{
    DSKY::wxGetApp().UpdateDlgDarkUI(dlg);
...
void Preview::create_sliders()
{
    ...
    if (wxGetApp().is_editor())
    {
        ...
        m_layers_slider->set_callback_on_check_gcode(
            [this](CustomGCode::Type type) -> void
            {
                if (type == ColorChange && m_layers_slider->gcode(ColorChange).empty())
                    DSKY::wxGetApp().plater()->get_notification_manager()->push_notification(
                        DSKY::NotificationType::EmptyColorChangeCode);
            });
        ...
        m_layers_slider->set_callback_on_empty_auto_color_change(
            ...
            {
                DSKY::wxGetApp().plater()->get_notification_manager()->push_notification(
                    DSKY::NotificationType::EmptyAutoColorChange);
            });
        ...
        m_layers_slider->set_callback_on_get_print([]() -> const Print &
                                                   { return DSKY::wxGetApp().plater()->active_fff_print(); });
        ...
        m_layers_slider->set_callback_on_get_new_color(
            [](const std::string &color) -> std::string
            {
                ...
                wxColourDialog dialog(DSKY::wxGetApp().GetTopWindow(), data);
        ...
        m_layers_slider->set_callback_on_show_info_msg(
            ...
            {
                DSKY::MessageDialog msg(this, from_u8(message), _L("Notice"), btns_flag);
                                         ...
                                         const std::vector<double> &layers_z)
{
    ...
    // Not called when the last object is deleted, so ticks can survive an emptied plater.
...
void Preview::update_layers_slider(const std::vector<double> &layers_z, bool keep_z_range)
{
    ...
    {
        const Print &print = DSKY::wxGetApp().plater()->active_fff_print();
        std::string idxs;
...
void Preview::update_moves_slider(std::optional<int> visible_range_min, std::optional<int> visible_range_max)
{
    ...
    // Sequential preview always restricts to the top layer, so the range lower thumb is never shown
...
void Preview::load_print_as_fff(bool keep_z_range)
{
    ...
    if (IsShown())
    {
        ...
        {
            // No G-code yet while the print keeps its sliced layers (a re-slice after an edit that
            // left the slice step valid): show the shells, filled to the slicing progress, as a
            // first slice does.
            m_canvas->load_gcode_shells();
        }
...
void Preview::on_layers_slider_scroll_changed()
{
    if (IsShown())
    {
        ...
        if (tech == ptFFF)
        {
            ...
            // Get shifted z-values: position 0 gives z=0, position N gives z=m_values[N-1]
