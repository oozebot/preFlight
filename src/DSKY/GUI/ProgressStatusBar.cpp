...
ProgressStatusBar::ProgressStatusBar(wxWindow *parent, int id)
    : self{new wxStatusBar(parent, id == -1 ? wxID_ANY : id, wxSTB_DEFAULT_STYLE & ~wxSTB_SIZEGRIP)}
...
{
    DSKY::wxGetApp().UpdateDarkUI(self);
    DSKY::wxGetApp().UpdateDarkUI(m_prog);
    DSKY::wxGetApp().UpdateDarkUI(m_cancelbutton);
#ifdef _WIN32
    NppDarkMode::SetDarkExplorerTheme(self->GetHWND());
#endif
}
