...
// The active theme's disabled foreground as "#RRGGBB", used to recolor the disabled-only
...
// The active theme's disabled input background as "#RRGGBB" (the disabled checkbox box fill,
...
// The active theme's static-box border as "#RRGGBB" (the disabled checkbox box border, matching
                                ...
                                const bool grayscale /* = false*/)
{
    ...
    wxImage image;
    if (!image.LoadFile(DSKY::from_u8(Luminary::var(bitmap_name + ".png")), wxBITMAP_TYPE_PNG) ||
                                      ...
                                      const bool dark_mode, const std::string &new_color /*= ""*/)
{
    ...
    // Themed colors are part of the cache key so a theme switch never serves a stale recolor.
    ...
    // Recolor the baked brand orange to the active theme's accent (no-op for orange themes).
    if (accent != "#EAA032")
    {
    ...
    std::string str;
    nsvgGetDataFromFileWithReplace(Luminary::var(bitmap_name + ".svg").c_str(), str, replaces);
    if (str.empty())
        return nullptr;

    return insert_bndl(bitmap_key, str.data(), target_width, target_height);
#else
    // Linux/Mac - wxBitmapBundle::FromSVG() is unreliable on non-Windows platforms.
    // Rasterize with NanoSVG directly for crisp rendering.
    NSVGimage *image = nsvgParseFromFileWithReplace(Luminary::var(bitmap_name + ".svg").c_str(), "px", 96.0f, replaces);
    if (image == nullptr)
        return nullptr;
