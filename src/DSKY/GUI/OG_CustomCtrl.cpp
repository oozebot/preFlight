...
void OG_CustomCtrl::OnMotion(wxMouseEvent &event)
{
    ...
    for (CtrlLine &line : ctrl_lines)
    {
        ...
        if (line.is_focused)
        {
            // A line whose field a rule disabled says why first
            for (const Option &option : line.og_line.get_options())
                if (Field *field = opt_group->get_field(option.opt_id);
                    field != nullptr && !field->m_disabled_reason.IsEmpty())
                    tooltip += field->m_disabled_reason + "\n\n";
