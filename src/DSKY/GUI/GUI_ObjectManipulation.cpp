...
ObjectManipulation::ObjectManipulation(wxWindow *parent) : OG_Settings(parent, true)
{
    ...
    // On GTK3/macOS, views draw in creation order. The FlatStaticBox
    ...
    , m_axis(axis)
{
    ...
    // There is no wxEVT_UPDATE_UI handler here: its continuous polling makes the themed
    // text controls flicker and gray out while a dialog is open. Enable/disable is driven
    // explicitly from ObjectManipulation::Enable().
}
...
void ManipulationEditor::sys_color_changed(ObjectManipulation *parent)
{
    ...
    if (parent->use_colors())
    {
        ...
        if (auto *tc = dynamic_cast<DSKY::ThemedTextCtrl *>(GetTextCtrl()))
