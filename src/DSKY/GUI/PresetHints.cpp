...
std::string PresetHints::cooling_description(const Preset &preset)
{
    ...
    if (cooling)
    {
        ...
        out += DSKY::format(_L("If estimated layer time is below ~%1%s, "
        ...
        if (fan_below_layer_time > slowdown_below_layer_time)
        {
            ...
            if (min_fan_speed != max_fan_speed)
                out += DSKY::format(_L("If estimated layer time is greater, but still below ~%1%s, "
    ...
    if (preset.config.opt_bool("fan_always_on", 0))
    {
        ...
        if (full_fan_speed_layer > disable_fan_first_layers + 1)
            out += DSKY::format(_L("Fan speed will be ramped from zero at layer %1% to %2%%% at layer %3%."),
                                disable_fan_first_layers, min_fan_speed, full_fan_speed_layer);
        else
        {
            out += DSKY::format(cooling ? _L("During the other layers, fan will always run at %1%%%")
                                        : _L("Fan will always run at %1%%%"),
                                min_fan_speed) +
                   " ";
            if (disable_fan_first_layers > 1)
                out += DSKY::format(_L("except for the first %1% layers."), disable_fan_first_layers);
            else if (disable_fan_first_layers == 1)
                out += DSKY::format(_L("except for the first layer."));
        }
    }
    else
    {
        // Point the user at the manual fan controls when auto cooling and fan_always_on are both off
        if (cooling)
    ...
    for (size_t idx_type = (first_layer_extrusion_width.value == 0) ? 1 : 0; idx_type < 3; ++idx_type)
    {
                                         ...
                                         double speed, const char *err_msg)
        {
            ...
            if (max_flow < volumetric_flow)
            {
                ...
                max_flow_extrusion_type = DSKY::into_u8(_(err_msg));
            }
        };
        ...
        if (support_material_interface_extruder_active)
            test_flow(frSupportMaterialInterface, support_material_extrusion_width, support_material_interface_speed,
                      L("support interface"));
        // gap_fill_speed is not part of this hint.
        if (!out.empty())
            out += "\n";
...
std::string PresetHints::recommended_thin_wall_thickness(const PresetBundle &preset_bundle)
{
    ...
    if (num_perimeters > 0)
    {
        ...
        try
        {
            ...
            for (int i = 2; i <= num_lines; i += 2)
            {
                if (i > 2)
                    out += ", ";
                out += (boost::format(_u8L("%d lines: %.2f mm")) % i % width).str() + " ";
                width += perimeter_flow.spacing() * 2.f;
            }
        }
...
std::string PresetHints::top_bottom_shell_thickness_explanation(const PresetBundle &preset_bundle)
{
    ...
    // Minimum layer height is taken from the first extruder.
