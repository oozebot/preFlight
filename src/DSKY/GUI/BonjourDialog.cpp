...
///|/
#include "DSKY/Utils/Bonjour.hpp" // On Windows, boost needs to be included before wxWidgets headers
...
#include "DSKY/Utils/Bonjour.hpp"
...
BonjourDialog::BonjourDialog(wxWindow *parent, Luminary::PrinterTechnology tech, std::string service)
    : wxDialog(parent, wxID_ANY, _(L("Network lookup")), wxDefaultPosition, wxDefaultSize,
...
bool BonjourDialog::show_and_lookup()
{
    ...
    // "version" and "model" are the OctoPrint TXT records; Moonraker supplies "version" too
...
void BonjourDialog::on_reply(BonjourReplyEvent &e)
{
    ...
    for (const auto &reply : *replies)
    {
        ...
        if (tech == ptFFF)
        {
            ...
            {
                list->SetItem(item, 3, DSKY::from_u8(it->second));
            }
        }
    }
...
IPListDialog::IPListDialog(wxWindow *parent, const wxString &hostname, const std::vector<boost::asio::ip::address> &ips,
                           size_t &selected_index)
    : wxDialog(parent, wxID_ANY, _(L("Multiple resolved IP addresses")), wxDefaultPosition, wxDefaultSize,
               wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
    ...
    , m_selected_index(selected_index)
{
    ...
    auto *label = new wxStaticText(
        this, wxID_ANY,
        DSKY::format_wxstr(
            _L("There are several IP addresses resolving to hostname %1%.\nPlease select one that should be used."),
            hostname));
    vsizer->Add(label, 0, wxEXPAND | wxTOP | wxLEFT | wxRIGHT, em);
