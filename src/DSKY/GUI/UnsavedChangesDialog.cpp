...
#include "DSKY/GUI/Sidebar.hpp" // IWYU pragma: keep
#include "format.hpp"
#include "GUI_App.hpp"
...
static std::string get_icon_name(Preset::Type type)
{
    return type_icon_names.at(type);
}
...
// The modified-value highlight follows the active theme accent.
...
wxDataViewItem DiffModel::AddPreset(Preset::Type type, wxString preset_name, wxString new_preset_name /* = wxString()*/)
{
    // "color" strings
    ...
    auto preset = new ModelNode(type, m_parent_win, preset_name, get_icon_name(type), new_preset_name);
...
wxDataViewItem DiffModel::Delete(const wxDataViewItem &item)
{
    ...
    // m_preset_nodes is only a vector of _pointers_
                                       ...
                                       const std::string &new_selected_preset)
{
    ...
    for (PresetCollection *presets : presets_list)
    {
        ...
        m_tree->model->AddPreset(type, from_u8(presets->get_edited_preset().name), from_u8(new_selected_preset));
...
void DiffPresetDialog::create_presets_sizer()
{
    ...
    for (auto new_type : {Preset::TYPE_PRINT, Preset::TYPE_FILAMENT, Preset::TYPE_PRINTER})
    {
...
{
    return PresetBundle::types_list();
}
                                            ...
                                            PresetBundle *preset_bundle)
{
    ...
    bool print_tab = type == Preset::TYPE_PRINT;
    ...
    auto update_compatible_type = [](bool on_page, bool show_incompatible_presets)
    {
        return on_page                     ? PresetSelectCompatibleType::Never
    ...
    if (print_tab || printer_tab)
        preset_bundle->update_compatible(update_compatible_type(print_tab, true), update_compatible_type(false, true));
    ...
    for (auto preset_combos : m_preset_combos)
    {
        ...
        if ((print_tab && presets_type == Preset::TYPE_FILAMENT) ||
            (printer_tab && (presets_type == Preset::TYPE_PRINT || presets_type == Preset::TYPE_FILAMENT)))
            cb->update();
    }
}
