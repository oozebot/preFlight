...
private:
    ...
    // The width the zoom-to-fit functions fill: the viewport's, unless the viewport was widened past the
    // scene's framing to draw across a hidden panel's strip.
    int m_fit_width{0};
...
public:
    ...
    // fit_w is the width the zoom-to-fit functions fill when it is not the viewport's; 0 means the viewport's.
    void set_viewport(int x, int y, unsigned int w, unsigned int h, unsigned int fit_w = 0);
...
private:
    ...
    double calc_zoom_to_bounding_box_factor(const BoundingBoxf3 &box,
                                            double margin_factor = DefaultZoomToBoxMarginFactor) const;
    double calc_zoom_to_volumes_factor(const GLVolumePtrs &volumes, Vec3d &center,
