...
public:
    ...
    // Lay the slider out along the canvas' left edge, mirrored so its labels hang inward.
    void set_on_left(bool on_left)
    {
        m_on_left = on_left;
        m_ctrl.SetMirrored(on_left);
    }
    ...
    void set_imgui_wrapper(DSKY::ImGuiWrapper *imgui) { m_imgui = imgui; }
    ...
    void set_callback_on_get_print(std::function<const Luminary::Print &()> cb) { m_cb_get_print = cb; }
...
private:
    ...
    DSKY::ImGuiWrapper *m_imgui{nullptr};
    ...
    std::function<const Luminary::Print &()> m_cb_get_print{nullptr};
