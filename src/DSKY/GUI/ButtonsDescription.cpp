...
static std::vector<std::pair<std::string, std::vector<std::string>>> MODE_PALETTES = {
    {L("Palette 1 (default)"), {"#00B000", "#FFDC00", "#E70000"}}, {L("Palette 2"), {"#FC766A", "#B0B8B4", "#184A45"}},
    {L("Palette 3"), {"#567572", "#964F4C", "#696667"}},           {L("Palette 4"), {"#DA291C", "#56A8CB", "#53A567"}},
    {L("Palette 5"), {"#F65058", "#FBDE44", "#28334A"}},           {L("Palette 6"), {"#FF3EA5", "#EDFF00", "#00A4CC"}},
    {L("Palette 7"), {"#E95C20", "#006747", "#4F2C1D"}},           {L("Palette 8"), {"#D9514E", "#2A2B2D", "#2DA8D8"}}};
                                        ...
                                        wxColourPickerCtrl **mod_colour)
{
    ...
    auto add_color = [grid_sizer, parent](wxColourPickerCtrl **color_picker, const wxColour &color,
                                          const wxColour &def_color, wxString label_text)
    {
        ...
        *color_picker = new wxColourPickerCtrl(parent, wxID_ANY, color);
        ...
        (*color_picker)
            ->Bind(wxEVT_COLOURPICKER_CHANGED,
                   [color_picker, sys_label](wxCommandEvent &)
                   {
        ...
        btn->Bind(wxEVT_BUTTON,
                  [sys_label, color_picker, def_color](wxEvent &event)
                  {
        ...
        parent->Bind(
            wxEVT_UPDATE_UI, [color_picker, def_color](wxUpdateUIEvent &evt)
            { evt.Enable((*color_picker)->GetColour() != def_color); }, btn->GetId());
    ...
    add_color(sys_colour, wxGetApp().get_label_clr_sys(), wxGetApp().get_label_default_clr_system(),
              _L("Value is the same as the system value"));
    add_color(mod_colour, wxGetApp().get_label_clr_modified(), wxGetApp().get_label_default_clr_modified(),
              _L("Value was changed and is not equal to the system value or the last saved preset"));
}

void FillSizerWithModeColorDescriptions(wxSizer *sizer, wxWindow *parent,
                                        ...
                                        std::vector<wxColour> &mode_palette)
{
    ...
    palette_cb->Bind(wxEVT_COMBOBOX,
                     [clr_pickers, &mode_palette](wxCommandEvent &evt)
                     {
    ...
    h_sizer->Add(new wxStaticText(parent, wxID_ANY, _L("Default palette for mode markers") + ": "), 0,
    ...
    for (size_t mode = 0; mode < names.size(); ++mode)
    {
        ...
        *color_picker = new wxColourPickerCtrl(parent, wxID_ANY, color);
        ...
        (*color_picker)
            ->Bind(wxEVT_COLOURPICKER_CHANGED,
                   [color_picker, &color, palette_cb, &mode_palette](wxCommandEvent &)
                   {
        ...
        btn->Bind(wxEVT_BUTTON,
                  [color_picker, &color, def_color, palette_cb, &mode_palette](wxEvent &event)
                  {
                      color = def_color;
                      (*color_picker)->SetColour(def_color);
                      palette_cb->UpdateSelection(mode_palette);
        ...
        grid_sizer->Add(*color_picker, 0, wxALIGN_CENTRE_VERTICAL);
        grid_sizer->Add(btn, 0, wxALIGN_CENTRE_VERTICAL);
        grid_sizer->Add(new wxStaticText(parent, wxID_ANY, names[mode]), 0, wxALIGN_CENTRE_VERTICAL | wxRIGHT,
                        2 * margin);
    }
}
