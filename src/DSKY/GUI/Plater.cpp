...
///|/
#include "Plater.hpp"
#include "DSKY/GUI/BitmapCache.hpp"
...
#include "luminary/config/thumbnails/ThumbnailData.hpp"
...
// For stl export
#include "luminary/gizmo/csg/ModelToCSGMesh.hpp"
#include "luminary/gizmo/csg/PerformCSGMeshBooleans.hpp"
...
#include "luminary/model/custom_gcode/CustomGCode.hpp"
...
using Luminary::_3DScene;
using Luminary::Preset;
using Luminary::PrintHostJob;
...
struct Plater::priv
{
    ...
    // Data
    Luminary::DynamicPrintConfig *config;
    std::vector<std::unique_ptr<Luminary::Print>> fff_prints;
    Luminary::Model model;
    PrinterTechnology printer_technology = ptFFF;
    std::vector<Luminary::GCodeProcessorResult> gcode_results;
    ...
    // The running slice was started by sidebar processing, not by the user, so its completion must not switch tabs.
    bool sidebar_auto_reslice{false};
            ...
            if (mainframe->can_save_as())
            {
                ...
                if (act.empty())
                {
                    RichMessageDialog dialog(mainframe,
                                             ...
                                             wxString(PREFLIGHT_APP_NAME), wxYES_NO | wxCANCEL);
    ...
    // In the Preview with sidebar processing on, "collapsed" means the legend owns the corner.
    bool is_sidebar_collapsed() const
    {
        return sidebar_processing_active() ? !get_config_bool("preview_sidebar") : sidebar->is_collapsed;
    }
    void collapse_sidebar(bool collapse);
    // Show or hide the sidebar floating over the current panel: in the Preview the persisted choice picks it
    // or the legend, in Prepare it shows unless collapsed.
    void apply_sidebar();
    // Size and raise the sidebar floating over the current panel's canvas.
    void place_sidebar();
    // The floating sidebar follows its panel's size by hand.
    void on_panel_size(wxSizeEvent &evt);
    ...
    fs::path get_export_file_path(DSKY::FileType file_type);
    wxString get_export_file(DSKY::FileType file_type);
    ...
    Luminary::UndoRedo::Stack &undo_redo_stack()
    {
        assert(m_undo_redo_stack_active != nullptr);
        return *m_undo_redo_stack_active;
    }
    Luminary::UndoRedo::Stack &undo_redo_stack_main() { return m_undo_redo_stack_main; }
    ...
    // Sidebar processing: while the Preview is up, the sidebar is available there and its edits re-slice
    // in place. Prepare stays quiet until the user presses Slice.
    bool sidebar_processing_active() const { return current_panel == preview && wxGetApp().is_editor(); }
    // Only an edit made with the Preview and its sidebar on screen re-slices in place. One made from a
    // Settings page (the plater is hidden) or with the legend up invalidates and waits for Slice, as Prepare
    // does. The plater's own shown flag is what the Settings pages clear; the key is what apply_sidebar reads.
    bool background_processing_enabled() const
    {
        return sidebar_processing_active() && q->IsShown() && get_config_bool("preview_sidebar");
    }
    ...
    {
        // update_background_process() reports, that the Print was updated in a way,
        ...
        // update_background_process() reports, that the Print is invalid, and the error message
        ...
        // Restart for G-code export or upload.
    ...
    void clear_warnings();
    void add_warning(const Luminary::PrintStateBase::Warning &warning, size_t oid);
    ...
    // The shell volume under a position on the Preview canvas (logical pixels), nullptr when there is none.
    // Shells keep their geometry while the shells toggle is off, so visibility is not considered.
    const GLVolume *preview_shell_at(const Vec2d &logical_pos);
    // A click on an object in the Preview selects it as a click in Prepare does; Shift adds or removes.
    void on_preview_left_click(RBtnEvent &);
    // Add the "Export G-code" entry (direct save, or a destination submenu mirroring
    ...
    // Prepend the editor right-click's top action above the factory items: "Export G-code"
    // once sliced, otherwise "Slice Platter" when the platter can be sliced.
...
private:
    ...
    Luminary::UndoRedo::Stack m_undo_redo_stack_main;
    Luminary::UndoRedo::Stack m_undo_redo_stack_gizmos;
    Luminary::UndoRedo::Stack *m_undo_redo_stack_active = &m_undo_redo_stack_main;
    ...
    std::vector<std::pair<Luminary::PrintStateBase::Warning, size_t>> current_warnings;
    bool show_warning_dialog{false};
};

// pattern_3mf and pattern_zip match any name ending in those letters; they have no [.] before the extension.
...
Plater::priv::priv(Plater *q, MainFrame *main_frame)
    ...
    , config(Luminary::DynamicPrintConfig::new_from_defaults_keys(
          {"bed_shape", "bed_custom_texture", "bed_custom_model", "complete_objects", "duplicate_distance",
           ...
           "filament_colour", "filament_transmission_distance", "max_print_height", "printer_model", "printer_notes",
           "printer_technology",
...
void Plater::priv::init()
{
    ...
    auto statuscb = [this](const Luminary::PrintBase::SlicingStatus &status)
    {
...
#ifdef __WXGTK3__
    // On GTK the sidebar sits beside the canvas in the sizer. Floating it over the GL canvas needs a native
    // X window of its own, and every repaint of a native child window lands in two frames, the second with
    // its widgets missing: a visible flicker on each pointer move between inputs. The strip callbacks below
    // report no strip while it is shown, so the scene is framed on the whole canvas beside it and keeps its
    // place when the sidebar hides.
    if (wxGetApp().legacy_prepare_layout())
    {
        hsizer->Add(panel_sizer, 1, wxEXPAND | wxALL, 0);
        hsizer->Add(sidebar, 0, wxEXPAND);
    }
    else
    {
        hsizer->Add(sidebar, 0, wxEXPAND);
    ...
    q->SetSizer(hsizer);
#else
    // The sidebar is in no sizer: it floats over the current panel's canvas, on the sidebar's side, so the
    // canvas never resizes when the sidebar is shown or hidden and the scene stays put.
    hsizer->Add(panel_sizer, 1, wxEXPAND | wxALL, 0);
    q->SetSizer(hsizer);

    // The floating sidebar is outside any sizer, so it follows its panel's size by hand.
    view3D->Bind(wxEVT_SIZE, &priv::on_panel_size, this);
    preview->Bind(wxEVT_SIZE, &priv::on_panel_size, this);
#endif
    ...
    // The Preview reserves a strip of the sidebar's width on the sidebar's side for the legend or the floating
    // sidebar; the scene is framed beside it, the rect Prepare's canvas has beside its sidebar, so switching
    // views does not move it. A legend folded to its title bar leaves the strip empty and the scene is drawn
    // across it, framed as before.
    preview->get_canvas3d()->set_strip_width_fn(
        [this]()
        {
            if (!wxGetApp().is_editor())
                return 0.0f;
#ifdef __WXGTK3__
            // The sidebar sits beside the canvas, not over it: no strip while it is shown
            if (sidebar->IsShown())
                return 0.0f;
#endif
            return static_cast<float>(sidebar->GetMinSize().GetWidth()) *
                   preview->get_canvas3d()->get_canvas_size().get_scale_factor();
        });
    // With the sidebar hidden the strip is free whenever the legend does not fill it: folded, turned off, or not
    // yet drawn because the slice has no toolpaths.
    preview->get_canvas3d()->set_strip_hidden_fn(
        [this]()
        {
            const GLCanvas3D *canvas = preview->get_canvas3d();
            return !get_config_bool("preview_sidebar") && (!canvas->is_legend_drawn() || canvas->is_legend_collapsed());
        });
    // Prepare reserves the same strip, under the floating sidebar; a collapsed sidebar leaves the strip empty
    // and the scene, framed as before, is drawn across it.
    view3D->get_canvas3d()->set_strip_width_fn(
        [this]()
        {
            if (!wxGetApp().is_editor())
                return 0.0f;
...
#endif
            return static_cast<float>(sidebar->GetMinSize().GetWidth()) *
                   view3D->get_canvas3d()->get_canvas_size().get_scale_factor();
        });
    view3D->get_canvas3d()->set_strip_hidden_fn([this]() { return sidebar->is_collapsed; });
    // The object list selects into the Prepare canvas's selection; the Preview draws that selection's bounding box.
    preview->get_canvas3d()->set_shown_selection_fn([this]() { return &view3D->get_canvas3d()->get_selection(); });
    ...
    preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_PREVIEW_LEFT_CLICK, &priv::on_preview_left_click, this);
    preview->get_wxglcanvas()->Bind(
        ...
        [this](RBtnEvent &evt)
        {
            ...
            // The clipping plane is offered for the object under the cursor.
            const GLVolume *clicked_shell = preview_shell_at(evt.data.first);
            const int clicked_object_id = clicked_shell != nullptr ? clicked_shell->composite_id.object_id : -1;

#ifdef __APPLE__
            // On macOS, use the native popup menu to avoid wxPopupTransientWindow
            // issues with the Cocoa event loop (mouse freeze after dismiss).
            {
                wxMenu nativeMenu;

                // Navigation + export shortcuts, shown above a divider. Editor only:
                ...
                {
                    prepareId = wxNewId();
                    wxMenuItem *prepareItem = nativeMenu.Append(prepareId, _L("Back to Prepare"));
                    ...
                    if (tabbar->HasSlicedObject())
                    {
                        ...
                        {
                            // Save to disk is the only destination, so invoke it directly.
                            exportId = wxNewId();
                        ...
                        {
                            // Multiple destinations, so mirror the slice-button dropdown as a submenu.
            ...
            if (tabbar != nullptr)
            {
                ...
                customMenu->Append(prepareId, _L("Back to Prepare"), *get_bmp_bundle("switch_presets"));
...
Plater::priv::~priv()
{
    ...
    // The panels outlive this object; their size handlers must not reach back into it.
    if (view3D != nullptr)
        view3D->Unbind(wxEVT_SIZE, &priv::on_panel_size, this);
    if (preview != nullptr)
        preview->Unbind(wxEVT_SIZE, &priv::on_panel_size, this);
...
void Plater::priv::update(unsigned int flags)
{
    ...
    const bool force_background_processing_restart =
        (flags & (unsigned int) UpdateParams::FORCE_BACKGROUND_PROCESSING_UPDATE) != 0;
    if (force_background_processing_restart)
        // Update the background process from the current Model, so that reload_scene()
...
{
    if (sidebar_processing_active())
    {
        // Preview: the sidebar and the legend share the corner, so one replaces the other.
        wxGetApp().app_config->set("preview_sidebar", collapse ? "0" : "1");
        apply_sidebar();
        return;
    }

    // Prepare: the sidebar floats over the canvas, which keeps its size, so the scene does not move.
    sidebar->collapse(collapse);
    apply_sidebar();
    // Rendering before returning to the event loop fills the sidebar's strip with the scene at once.
    if (current_panel == view3D)
...
void Plater::priv::apply_sidebar()
{
    // The sidebar floats over the current panel's canvas instead of taking a layout slot, so the scene and
    // its orbit center stay put when it is shown or hidden. The G-code viewer has no sidebar.
    const bool editor = wxGetApp().is_editor() && current_panel != nullptr;
    const bool show_sidebar = editor &&
                              (current_panel == preview ? get_config_bool("preview_sidebar") : !sidebar->is_collapsed);
#ifdef _WIN32
    // On Windows the sidebar becomes a child of the panel it floats over: as a sibling of the GL canvas
    // its painting is clipped around it. A window raised over the panel from one level up is painted over
    // by every canvas frame, so only a hovered control ever shows.
    if (show_sidebar && sidebar->GetParent() != current_panel)
        sidebar->Reparent(current_panel);
#else
    // Elsewhere it stays a child of the Plater and is raised above the panel it overlaps; reparenting a
    // live window tree corrupts the view hierarchy on macOS.
#endif
    sidebar->Show(show_sidebar);
    // The legend is the sidebar's counterpart in the Preview.
    if (editor && current_panel == preview)
        preview->get_canvas3d()->show_legend(!show_sidebar);

    q->Layout();
#ifndef __WXGTK3__
    // On GTK the sizer has placed the sidebar beside the canvas
    if (show_sidebar)
        place_sidebar();
#endif
...
void Plater::priv::place_sidebar()
{
    // Placed in its parent's coordinates over the current panel: the panel's client rect when the sidebar
    // is the panel's child, the panel's rect in the Plater when it is the panel's sibling. The strip is on
    // the sidebar's side: left, or right in the legacy layout.
    const wxRect area = sidebar->GetParent() == current_panel ? wxRect(wxPoint(0, 0), current_panel->GetClientSize())
                                                              : current_panel->GetRect();
    const int width = sidebar->GetMinSize().GetWidth();
    const int x = wxGetApp().legacy_prepare_layout() ? area.GetRight() + 1 - width : area.GetLeft();
    sidebar->SetSize(x, area.GetTop(), width, area.GetHeight());
    sidebar->Raise();
    sidebar->Layout();
}

void Plater::priv::on_panel_size(wxSizeEvent &evt)
{
    evt.Skip();
    // Only the panel the sidebar floats over, and only while the sidebar is visible.
    if (sidebar->IsShown() && evt.GetEventObject() == current_panel)
        place_sidebar();
}
                                             ...
                                             bool load_config, bool imperial_units /* = false*/)
{
    ...
    // On Linux (only), there are issues when settings pages are swapped during project file loading (disabling of controls,
    ...
    auto *extra_model = (!load_model || one_by_one) ? nullptr : new Luminary::Model();
    ...
    for (size_t i = 0; i < input_files_size; ++i)
    {
        ...
        // On Windows, swap slashes to back slashes: read_from_file() does not understand slashes there and assigns the full path to the names of loaded objects.
        path.make_preferred();
#else   // _WIN32
        ...
        // But if settings pages are unparented from the wxNoteBook during project loading,
        // they will never be enabled after destruction of the ProgressDialog.
        ...
        {
            std::string message = DSKY::format(_L("Failed loading file \"%1%\" due to an invalid configuration."),
                                               filename.string()) +
                                  "\n\n" + e.what();
            DSKY::show_error(q, message);
            continue;
        }
        catch (const std::exception &e)
        {
            DSKY::show_error(q, e.what());
            continue;
        }
        ...
        if (load_config)
        {
            ...
            if (had_config)
            {
                ...
                {
                    // preFlight prints FFF only; the project opens with the FFF base and the load is counted.
                    DBG_COUNT_LOAD("PRINTER_TECHNOLOGY_SLA_REJECTED");
                    loaded_printer_technology = ptFFF;
                    config_loaded.set_key_value("printer_technology", new ConfigOptionEnum<PrinterTechnology>(ptFFF));
                }
            ...
            else
            {
                ...
                if (generator_version && // set only when loaded with configuration
                    *generator_version > Semver(PREFLIGHT_VERSION))
                {
            ...
            // Some other slicers' files carry a partial config, others carry none.
    ...
    if (!obj_idxs.empty() && view3D != nullptr)
    {
        ...
        if (view3D->get_canvas3d()->get_gizmos_manager().is_enabled())
            // this is required because the selected object changed and the flatten on face gizmo needs to be updated accordingly
    ...
    for (size_t idx : obj_idxs)
    {
        if (idx < model.objects.size())
        {
            ...
            if (!stats.manifold())
            {
                std::string msg = DSKY::format(_L("Model \"%1%\" has %2% open edges and may not slice correctly."),
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs &model_objects, bool allow_negative_z,
                                                     bool call_selection_changed /*= true*/)
{
    const Vec3d bed_size = Luminary::to_3d(this->bed.build_volume().bounding_volume2d().size(), 1.0) -
                           2.0 * Vec3d::Ones();
    ...
    for (ModelObject *model_object : model_objects)
    {
        ...
        if (model_object->instances.empty())
        {
            ...
            instance->set_offset(
                Luminary::to_3d(this->bed.build_volume().bed_center(), -object->origin_translation(2)));
#endif /* AUTOPLACEMENT_ON_LOAD */
        }
...
fs::path Plater::priv::get_export_file_path(DSKY::FileType file_type)
{
...
wxString Plater::priv::get_export_file(DSKY::FileType file_type)
{
    wxString wildcard;
    switch (file_type)
    {
...
void Plater::priv::reset()
{
    ...
    if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
    {
        wxGetApp().mainframe->m_modern_tabbar->EnableTab(DSKY::ModernTabBar::TAB_PREVIEW, false);
        wxGetApp().mainframe->m_modern_tabbar->SelectTab(DSKY::ModernTabBar::TAB_PREPARE);
...
void Plater::priv::split_object()
{
    ...
    if (new_objects.size() == 1)
        DSKY::warning_catcher(q, _L("The selected object couldn't be split because it contains only one solid part."));
    else
    {
...
void Plater::priv::process_validation_warning(const std::vector<std::string> &warnings) const
{
    ...
    for (std::string text : warnings)
    {
        ...
        else if (text == "_SUPPORT_NOZZLE_DIAMETER_DIFFER")
        {
            ...
            action_fn = [](wxEvtHandler *)
            {
                DSKY::wxGetApp().jump_to_option("support_material_extruder", Preset::Type::TYPE_PRINT,
...
void Plater::priv::regenerate_thumbnails(SimpleEvent &)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        generate_thumbnail(data, w, h, params, DSKY::Camera::EType::Ortho);
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages)
{
    ...
    std::vector<Print::ApplyStatus> apply_statuses{apply_to_inactive_beds(q->model(), q->p->fff_prints, full_config)};
    ...
    if (invalidated == Print::APPLY_STATUS_INVALIDATED)
    {
        ...
        // Return to the 3D view when slicing becomes invalid, unless a re-slice follows in place:
        // then the Preview stays current and its tab stays enabled.
        const bool stay_in_preview = background_processing_enabled();
        if (current_panel == preview && !stay_in_preview)
        {
            set_current_panel(view3D);
        ...
        // The 3D scene has to be reloaded because of the wipe tower preview box.
        ...
        (return_state & UPDATE_BACKGROUND_PROCESS_RESTART) == 0)
    {
        ...
        wxQueueEvent(DSKY::wxGetApp().mainframe->m_plater,
        ...
        if (background_process.finished())
            show_action_buttons(false);
                 ...
                 !background_process.running()) /* Do not update buttons if background process is running
                                                 * This condition is important when this function is called
                                                 * several times during calculations
                                                 * */
...
bool Plater::priv::restart_background_process(unsigned int state)
{
         ...
         (state & UPDATE_BACKGROUND_PROCESS_FORCE_EXPORT) != 0 || (state & UPDATE_BACKGROUND_PROCESS_RESTART) != 0))
    {
        ...
        {
            // A slice started while the Preview is up was not asked for by the user, so its completion
            // must not move them; a slice started from Prepare still lands in the Preview.
            sidebar_auto_reslice = sidebar_processing_active();
...
void Plater::priv::export_gcode(fs::path output_path, bool output_path_on_removable_media, PrintHostJob upload_job)
{
    ...
    {
        DSKY::show_error(q, _L("Another export job is currently running."));
        return;
    }

    // The slicing thread is still writing the G-code buffer; reading it now would export a torn copy.
    if (background_process.in_flight())
    {
        DSKY::show_error(q, _L("Slicing is in progress. Export the G-code when it finishes."));
        return;
    }
    ...
    {
        // Upload directly from in-memory G-code, no re-slicing needed.
...
public:
    ...
    {
        // wxBusyInfo::UpdateLabel() has no effect, so the message is changed by building a new dialog.
        m_dlg = std::make_unique<wxBusyInfo>(message, m_parent);
...
void Plater::priv::set_current_panel(wxPanel *panel)
{
    ...
    {
        // Back from a Settings tab: re-assert the floating sidebar's place and z-order over the canvas.
        apply_sidebar();
    ...
    // Deactivate the preview clip controller when navigating away from Preview
    if (old_panel == preview && panel != preview)
    {
        // Leaving the Preview with no finished slice: its tab goes dark until a slice lands, as before.
        if (s_print_statuses[s_multiple_beds.get_active_bed()] != PrintStatus::finished && wxGetApp().mainframe &&
            wxGetApp().mainframe->m_modern_tabbar)
            wxGetApp().mainframe->m_modern_tabbar->EnableTab(DSKY::ModernTabBar::TAB_PREVIEW, false);
    ...
    if (current_panel == view3D)
    {
        ...
        if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
        {
            ...
            if (wxGetApp().mainframe->m_modern_tabbar->IsSelected(DSKY::ModernTabBar::TAB_PREVIEW))
            {
                wxGetApp().mainframe->m_modern_tabbar->SelectTab(DSKY::ModernTabBar::TAB_PREPARE);
            }
        }
...
void Plater::priv::add_warning(const Luminary::PrintStateBase::Warning &warning, size_t oid)
{
    ...
    current_warnings.emplace_back(std::pair<Luminary::PrintStateBase::Warning, size_t>(warning, oid));
}
...
{
    std::vector<std::pair<Luminary::PrintStateBase::Warning, size_t>> current_critical_warnings{};
    ...
    InfoDialog msg_wingow(this->q, format_wxstr("%1% %2%", PREFLIGHT_APP_NAME, _L("generated warnings")), from_u8(text),
                          true);
...
{
    // A completion that arrives after a newer slice has already started belongs to the superseded
    // slice; handling it would stop the running one. The newer slice reports on its own.
    if (this->background_process.in_flight())
        return;
    ...
    // Only a slice the user asked for selects the Preview tab on completion.
    const bool auto_reslice = sidebar_auto_reslice;
    sidebar_auto_reslice = false;
    ...
    if (evt.error())
    {
        ...
        // If slicing failed while on Preview (the Slice button switches to Preview
    ...
    if (evt.success())
    {
        ...
        if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
        {
            wxGetApp().mainframe->m_modern_tabbar->EnableTab(DSKY::ModernTabBar::TAB_PREVIEW, true);
            if (!auto_reslice)
                wxGetApp().mainframe->m_modern_tabbar->SelectTab(DSKY::ModernTabBar::TAB_PREVIEW);
            wxGetApp().mainframe->m_modern_tabbar->UpdateSliceButtonState(true);
        }

        // The user controls sidebar visibility manually. The sidebar is hidden in Preview
        // via set_current_panel() and restored when returning to the Prepare view if
        // is_collapsed is false.
    }
    ...
    if (this->background_process.fff_print() != nullptr)
    {
...
const GLVolume *Plater::priv::preview_shell_at(const Vec2d &logical_pos)
{
    GLCanvas3D *canvas3d = preview->get_canvas3d();
    if (canvas3d == nullptr)
        return nullptr;
    const GLVolumeCollection &shells = canvas3d->get_gcode_viewer().get_shells_volumes();
    if (shells.volumes.empty())
        return nullptr;

    // The canvas size is in physical pixels, the position in logical ones.
    const Size cnv_size = canvas3d->get_canvas_size();
    const Vec2d mouse = logical_pos * double(cnv_size.get_scale_factor());

    // The scene is projected into the camera viewport, which is narrower than the canvas while the Preview
    // reserves its strip. The viewport's origin is the canvas's bottom left.
    const std::array<int, 4> &viewport = camera.get_viewport();
    const double gl_y = double(cnv_size.get_height()) - mouse.y();
    const double ndc_x = (2.0 * (mouse.x() - viewport[0]) / viewport[2]) - 1.0;
    const double ndc_y = (2.0 * (gl_y - viewport[1]) / viewport[3]) - 1.0;
    // A position on the strip is outside the scene: no object is drawn there.
    if (std::abs(ndc_x) > 1.0 || std::abs(ndc_y) > 1.0)
        return nullptr;

    const Eigen::Matrix4d inv_vp =
        (camera.get_projection_matrix().matrix() * camera.get_view_matrix().matrix()).inverse();
    Vec4d near_w = inv_vp * Vec4d(ndc_x, ndc_y, -1.0, 1.0);
    Vec4d far_w = inv_vp * Vec4d(ndc_x, ndc_y, 1.0, 1.0);
    near_w /= near_w.w();
    far_w /= far_w.w();
    const Vec3d ray_origin = near_w.head<3>();
    const Vec3d ray_dir = (far_w.head<3>() - near_w.head<3>()).normalized();

    const GLVolume *best = nullptr;
    ...
    for (const GLVolume *vol : shells.volumes)
    {
        ...
        for (int a = 0; a < 3 && !miss; ++a)
        {
            if (std::abs(ray_dir[a]) < 1e-12)
                miss = ray_origin[a] < bb.min[a] || ray_origin[a] > bb.max[a];
            else
            {
        ...
        // Bounding boxes of neighbouring objects overlap on screen, so the mesh decides where the volume has one.
        double t = tmin;
        if (vol->mesh_raycaster != nullptr)
        {
            const Transform3d world = vol->world_matrix();
            Vec3f hit;
            Vec3f normal;
            if (!vol->mesh_raycaster->closest_hit(mouse, world, camera, hit, normal))
                continue;
            t = (world * hit.cast<double>() - ray_origin).dot(ray_dir);
        }
...
void Plater::priv::on_preview_left_click(RBtnEvent &evt)
{
    const bool shift = evt.data.second;
    const GLVolume *shell = preview_shell_at(evt.data.first);

    // The object list and the Preview's bounding box both follow the Prepare canvas's selection.
    GLCanvas3D *canvas = view3D->get_canvas3d();
    Selection &selection = canvas->get_selection();

    if (shell == nullptr)
    {
        // A click beside every object deselects, unless Shift is building a selection.
    ...
    {
        const int object_idx = shell->composite_id.object_id;
        const int instance_idx = shell->composite_id.instance_id;
        if (object_idx < 0 || object_idx >= int(model.objects.size()) || instance_idx < 0 ||
            instance_idx >= int(model.objects[object_idx]->instances.size()))
            return;

        const Selection::ObjectIdxsToInstanceIdxsMap &content = selection.get_content();
        const auto it = content.find(object_idx);
        const bool already_selected = it != content.end() && it->second.count(instance_idx) > 0;
        if (already_selected && shift)
            selection.remove_instance(object_idx, instance_idx);
        else
            selection.add_instance(object_idx, instance_idx, !shift);
    }
...
void Plater::priv::prepend_slice_or_export_entry(const std::shared_ptr<CustomMenu> &menu, bool factory_has_items)
{
    ...
    if (sliced)
    {
        ...
        menu->Prepend(previewId, _L("Show Preview"), *get_bmp_bundle("switch_presets"));
...
void Plater::priv::on_right_click(RBtnEvent &evt)
{
    ...
    if (q != nullptr && menu)
    {
        ...
        if (customMenu)
        {
            // Top action above a divider: Export G-code once sliced, else Slice Platter.
...
bool Plater::priv::init_view_toolbar()
{
    ...
    item.tooltip = _u8L("3D editor view") + " [" + DSKY::shortkey_ctrl_prefix() + "5]";
    ...
    item.tooltip = _u8L("Preview") + " [" + DSKY::shortkey_ctrl_prefix() + "6]";
    item.sprite_id = sprite_id++;
    item.left.action_callback = [this]()
    {
        if (this->q != nullptr)
...
void Plater::priv::take_snapshot(const std::string &snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    if (this->sidebar->obj_list()->is_selected(itLayer))
    {
    ...
    BOOST_LOG_TRIVIAL(info) << "Undo / Redo snapshot taken: " << snapshot_name << ", Undo / Redo stack memory: "
                            << Luminary::format_memsize_MB(this->undo_redo_stack().memsize()) << log_memory_info();
}
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
                                           ...
                                           it_snapshot->timestamp))
    {
        ...
        if (new_selected_layer_on_sidebar)
...
void Plater::priv::update_after_undo_redo(const UndoRedo::Snapshot &snapshot, bool /* temp_snapshot_was_taken */)
{
    ...
    this->view3D->get_canvas3d()->get_selection().set_deserialized(
        DSKY::Selection::EMode(this->undo_redo_stack().selection_deserialized().mode),
...
void Plater::convert_gcode_to_ascii()
{
    ...
    if (input_file == output_file)
    {
        ...
        else
        {
            ...
            wxString msg = DSKY::format_wxstr("The converted binary G-code file has '.gcode' extension.\n"
    ...
    if (exists)
    {
        MessageDialog msg_dlg(this,
                              DSKY::format_wxstr(_L("File %1% already exists. Do you wish to overwrite it?"),
                                                 output_file),
                              _L("Notice"), wxYES_NO);
        if (msg_dlg.ShowModal() != wxID_YES)
            return;
...
void Plater::convert_gcode_to_binary()
{
    ...
    if (input_file == output_file)
    {
        ...
        else
        {
            ...
            wxString msg = DSKY::format_wxstr("The converted ASCII G-code file has '.bgcode' extension.\n"
                                              "The exported file will be renamed to:\n\n%1%\n\nDo you want to continue?",
                                              output_file);
            MessageDialog msg_dlg(this, msg, _L("Warning"), wxYES_NO);
            if (msg_dlg.ShowModal() != wxID_YES)
...
// To be called when providing a list of files to the GUI on the command line.
...
LoadProjectsDialog::LoadProjectsDialog(const std::vector<fs::path> &paths)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr(_L("%1% - Multiple projects file"), PREFLIGHT_APP_NAME), wxDefaultPosition, wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE)
{
    ...
    id++; // the id advances even when the option is not added
    if (contains_projects)
    {
        // one project
...
bool Plater::preview_zip_archive(const boost::filesystem::path &archive_path)
{
    ...
    try
    {
        ...
        if (!open_zip_reader(&archive, archive_path.string()))
        {
            ...
            std::string err_msg = DSKY::format(_u8L("Loading of a ZIP archive on path %1% has failed."),
                                               archive_path.string());
            throw Luminary::FileIOError(err_msg);
        }
                    ...
                    if (mz_zip_reader_file_stat(&archive, i, &stat))
                    {
                        ...
                        try
                        {
                            ...
                            if (res == 0)
                            {
                                ...
                                wxString error_log = DSKY::format_wxstr(_L("Failed to unzip file to %1%: %2%"),
                                                                        final_path.string(),
                            ...
                            {
                                wxString error_log = DSKY::format_wxstr(
                        ...
                        catch (const std::exception &e)
                        {
                            ...
                            throw Luminary::FileIOError(e.what());
                        }
                    }
                }
            }
    ...
    catch (const Luminary::FileIOError &e)
    {
        // zip reader should be already closed or not even opened
        DSKY::show_error(this, e.what());
        return false;
    }
    // none selected
...
{
    const std::regex pattern_drop(".*[.](stl|obj|amf|3mf|step|stp|zip)", std::regex::icase);
    ...
    if (wxGetApp().is_gcode_viewer())
    {
        ...
        if (paths.size() > 1)
        {
            ...
            MessageDialog(static_cast<wxWindow *>(this), _L("You can open only one .gcode file at a time."),
                          wxString(PREFLIGHT_APP_NAME) + " - " + _L("Drag and drop G-code file"),
            ...
            if (!load_just_one_file)
            {
                WarningDialog dlg(static_cast<wxWindow *>(this),
                                  format_wxstr(_L("You have several files for loading and \"%1%\" is one of them.\n"
                                               ...
                                               filename),
                                  format_wxstr("%1% - %2%", PREFLIGHT_APP_NAME, _L("Drag and drop several files")),
                                  wxYES_NO);
    ...
    if (!load_config)
    {
        ...
        WarningDialog dlg(static_cast<wxWindow *>(this),
                          _L("You have several files for loading.\n"
                             ...
                             "Would you like to continue anyway?"),
                          format_wxstr("%1% - %2%", PREFLIGHT_APP_NAME, _L("Drag and drop several files")), wxYES_NO);
        if (dlg.ShowModal() == wxID_NO)
            return false;
    }
...
void Plater::reset_with_confirm()
{
    if (p->model.objects.empty() ||
        //wxMessageDialog(static_cast<wxWindow*>(this), _L("All objects will be removed, continue?"), wxString(PREFLIGHT_APP_NAME) + " - " + _L("Delete all"), wxYES_NO | wxCANCEL | wxYES_DEFAULT | wxCENTRE).ShowModal() == wxID_YES)
        MessageDialog(static_cast<wxWindow *>(this), _L("All objects will be removed, continue?"),
                      wxString(PREFLIGHT_APP_NAME) + " - " + _L("Delete all"),
    ...
    if (w.is_idle())
    {
        ...
        cbs.on_finished = [this](arr2::FillBedTaskResult &result)
        {
            ...
            if (!result.to_add.empty())
            {
                ...
                // Needed for increase_object_instances.
...
static wxString check_binary_vs_ascii_gcode_extension(const std::string &ext, bool binary_output)
{
    wxString err_out;
...
std::optional<fs::path> Plater::get_default_output_file()
{
    try
    {
        ...
        return fs::path(Luminary::fold_utf8_to_ascii(path));
    }
    catch (const Luminary::PlaceholderParserError &ex)
    {
...
std::optional<wxString> Plater::check_output_path_has_error(const boost::filesystem::path &path) const
{
    ...
    const wxString error{check_binary_vs_ascii_gcode_extension(ext, supports_binary && uses_binary)};
    if (!error.IsEmpty())
    {
        return error;
    }
...
std::optional<fs::path> Plater::get_output_path(const std::string &start_dir, const fs::path &default_output_file)
{
    ...
    wxFileDialog dlg(this, _L("Save G-code file as:"), start_dir, from_path(default_output_file.filename()),
                     DSKY::file_wildcards(FT_GCODE, ext), wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
...
{
    std::reference_wrapper<Luminary::Print> print;
    std::reference_wrapper<Luminary::GCodeProcessorResult> processor_result;
    int bed{};
};
...
void Plater::export_stl_obj(bool extended, bool selection_only)
{
    ...
    if (path.Lower().EndsWith(".stl"))
        stored = Luminary::store_stl(path_u8.c_str(), &mesh, true);
    else if (path.Lower().EndsWith(".obj"))
        stored = Luminary::store_obj(path_u8.c_str(), &mesh);
    if (!stored)
        show_error(this, DSKY::format_wxstr("%1%: %2%", _L("Unable to save file"), path));
}
...
bool Plater::export_3mf(const boost::filesystem::path &output_path)
{
    ...
    try
    {
        ret = Luminary::store_3mf(path_u8.c_str(), &p->model, export_config ? &cfg : nullptr, full_pathnames,
                                  &thumbnail_data);
    }
    ...
    {
        // Failure
        const wxString what = DSKY::format_wxstr("%1%: %2%", _L("Unable to save file"), path_u8);
...
void Plater::regenerate_active_preview_data()
{
    if (p->background_process.in_flight())
        return;
    if (active_fff_print().invalidate_gcode_export())
        reslice();
}
...
void Plater::reslice()
{
    ...
    // In case a gizmo is in editing mode, refuse to continue
    ...
    // A reslice is not held back while a G-code export or a host upload is running.
    // bitmask of UpdateBackgroundProcessReturnState
    unsigned int state = this->p->update_background_process(true);
    if (state & priv::UPDATE_BACKGROUND_PROCESS_REFRESH_SCENE)
        this->p->view3D->reload_scene(false);
    // Clear any single-object task so the whole print is sliced.
...
// Runs one Export to Script call on the GUI job worker, so a slow or hung export script never blocks the
// UI: the job's cancel button interrupts the script and the preprocessing time limit applies. The job
// owns a copy of the G-code text, so a re-slice started meanwhile cannot pull the buffer from under it.
class ExportScriptJob : public Job
{
public:
    ExportScriptJob(std::string script_path, std::string gcode_text, std::string output_filename,
                    double timeout_seconds)
        : m_script_path(std::move(script_path))
        , m_gcode_text(std::move(gcode_text))
        , m_output_filename(std::move(output_filename))
        , m_timeout_s(timeout_seconds)
    {
    }

    void process(Ctl &ctl) override
    {
        ctl.update_status(0, _u8L("Exporting G-code via script"));
        ExportScriptResult result = run_export_script(
            m_script_path, m_gcode_text, m_output_filename, [&ctl]() { return ctl.was_canceled(); }, m_timeout_s);
        if (!result.success && !ctl.was_canceled())
            throw Luminary::ExportError(result.error_message);
        ctl.update_status(100, _u8L("G-code exported via script"));
    }
...
void Plater::export_to_script()
{
    ...
    {
        show_error(this, DSKY::format(_L("Export script not found: %1%"), script_path));
        return;
    }
    ...
    {
        show_error(this, _L("Another export job is currently running."));
        return;
    }
    std::string gcode_text;
    try
    {
        gcode_text = p->background_process.copy_gcode_text();
    }
    catch (const std::exception &e)
    {
    ...
    replace_job(get_ui_job_worker(),
                std::make_unique<ExportScriptJob>(script_path, std::move(gcode_text), output_filename,
                                                  p->background_process.preprocessing_timeout_seconds()));
}
...
std::string Plater::get_upload_filename()
{
    ...
    default_output_file = fs::path(Luminary::fold_utf8_to_ascii(default_output_file.string()));
...
void Plater::send_gcode_inner(DynamicPrintConfig *physical_printer_config)
{
    ...
    wxArrayString storage_names;
    {
        ...
        catch (const Luminary::IOError &ex)
        {
            show_error(this, ex.what(), false);
            return;
        }
    ...
    // Loop to allow the user to rename the file if it already exists on the host
    ...
    for (;;)
    {
        ...
        const bool binary_output = wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_bool(
                                       "binary_gcode") &&
                                   wxGetApp().app_config->get_bool("use_binary_gcode_when_supported");
        const wxString error_str = check_binary_vs_ascii_gcode_extension(ext, binary_output);
        ...
        // Check if the file already exists on the host before uploading
        ...
        {
            // Check the stored preference for overwrite behavior
...
void Plater::on_config_change(const DynamicPrintConfig &config)
{
    ...
    for (auto opt_key : p->config->diff(config))
    {
        ...
        else if (boost::starts_with(opt_key, "wipe_tower") || opt_key == "single_extruder_multi_material")
        {
            update_scheduled = true;
        }
...
void Plater::on_activate(bool active)
{
    if (active)
    {
        ...
        // Resume the Preview canvas when the application regains focus.
...
// The printer's extruder_colour entries laid over the filament colors: an entry that is a valid
// hex color wins for its extruder, anything else leaves the filament color in place.
static std::vector<std::string> apply_extruder_color_overrides(const std::vector<std::string> &filament_colors,
                                                               const std::vector<std::string> &extruder_colors)
{
    const size_t n = std::max(filament_colors.size(), extruder_colors.size());
...
std::vector<std::string> Plater::get_filament_color_strings(const GCodeProcessorResult *const result) const
{
...
{
    if (wxGetApp().is_gcode_viewer() && result != nullptr)
        return apply_extruder_color_overrides(result->extruder_colors, result->extruder_color_overrides);
    ...
    if (p && p->config)
    {
        ...
        const std::vector<std::string> &extruder_colors =
            p->config->has("extruder_colour") ? p->config->option<ConfigOptionStrings>("extruder_colour")->values
                                              : std::vector<std::string>();
        return apply_extruder_color_overrides(filament_colors, extruder_colors);
    }

    return {};
}

// Append the colors of the color-change custom G-codes to a base palette.
static void append_color_change_colors(std::vector<std::string> &colors, const Model &model,
                                       const GCodeProcessorResult *const result)
{
    const std::vector<CustomGCode::Item> &items = (wxGetApp().is_gcode_viewer() && result != nullptr)
                                                      ? result->custom_gcode_per_print_z
                                                      : model.custom_gcode_per_print_z().gcodes;
    colors.reserve(colors.size() + items.size());
    for (const CustomGCode::Item &code : items)
    {
...
{
    std::vector<std::string> colors = get_filament_color_strings(result);
    append_color_change_colors(colors, p->model, result);
    return colors;
}
    ...
    cbs.on_finished = [this](arr2::ArrangeTaskResult &t)
    {
        ...
        if (!names.empty())
        {
            get_notification_manager()->push_notification(
                DSKY::format(_L("Arrangement ignored the following objects which "
...
{
    // Invalidates slicing when a setting changes
...
bool Plater::PopupMenu(wxMenu *menu, const wxPoint &pos)
{
    ...
    // No SuppressBackgroundProcessingUpdate here. CustomMenu is non-blocking (ShowAt returns
    // immediately), so the global background-processing gate could only be released by the
    // dismiss callback; if that callback does not fire, the gate leaks "true" and silently
    // blocks every slice invalidation, object and global alike. The menu only needs error
    // messages postponed, which m_tracking_popup_menu handles below.
...
PlaterAfterLoadAutoArrange::PlaterAfterLoadAutoArrange()
{
    ...
    m_enabled = plater->model().objects.empty() && is_XL_printer(plater->active_fff_print().config());
}
