...
#include "luminary/presets/bundle/PresetBundle.hpp" // IWYU pragma: keep
...
wxDataViewItem EditGCodeDialog::add_presets_placeholders()
{
    ...
    const std::set<std::string> print_options = get_set_from_vec(Preset::print_options());
    const std::set<std::string> material_options = get_set_from_vec(Preset::filament_options());
    const std::set<std::string> printer_options = get_set_from_vec(Preset::printer_options());
    ...
    wxDataViewItem material = m_params_list->AppendSubGroup(group, _L("Filament settings"), "spool");
    ...
    wxDataViewItem printer = m_params_list->AppendSubGroup(group, _L("Printer settings"), "printer");
...
wxDataViewItem ParamsModel::Delete(const wxDataViewItem &item)
{
    ...
    // m_group_nodes is only a vector of _pointers_
