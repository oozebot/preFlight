...
enum ColumnNumber
{
    ...
    colEditing,  // the override column: the panel's icon with the item's override count
};
...
public:
    ...
    ObjectDataViewModelNode(ObjectDataViewModelNode *parent, const wxString &sub_obj_name,
                            Luminary::ModelVolumeType type, const bool is_text_volume, const bool is_svg_volume,
                            const wxString &extruder, const int idx = -1);
...
class ObjectDataViewModel : public wxDataViewModel
{
    ...
    // The override count of an item, asked when the column is drawn; the list installs it
    std::function<int(const wxDataViewItem &)> m_override_count;

public:
    ObjectDataViewModel();
    ~ObjectDataViewModel();

    void SetOverrideCountFunction(std::function<int(const wxDataViewItem &)> fn) { m_override_count = std::move(fn); }
    ...
    wxDataViewItem AddVolumeChild(const wxDataViewItem &parent_item, const wxString &name, const int volume_idx,
                                  const Luminary::ModelVolumeType volume_type, const bool is_text_volume,
