...
{
    return DSKY::format("{ \"type\" : \"%1%\" , \"data\" : \"%2%\"}", type, data);
}

#ifdef _WIN32

static HWND l_preflight_hwnd;
static BOOL CALLBACK EnumWindowsProc(_In_ HWND hwnd, _In_ LPARAM lParam)
{
    //checks for other instances of preFlight, if found brings it to front and return false to stop enumeration and quit this instance
    ...
    if (wndTextString.find(L"preFlight") != std::wstring::npos && classNameString == L"wxWindowNR")
    {
        ...
        uint64_t my_instance_hash = DSKY::wxGetApp().get_instance_hash_int();
...
static bool send_message(const std::string &message, const std::string &version)
{
    if (!EnumWindows(EnumWindowsProc, 0))
    {
        ...
        SendMessage(l_preflight_hwnd, WM_COPYDATA, 0, (LPARAM) &data_to_send);
        return true;
    }
    return false;
}
...
{
    if (hwnd == DSKY::wxGetApp().mainframe->GetHandle())
    {
        return true;
    }
...
static void delete_lockfile()
{
    ...
    {
        std::string path = data_dir() + "/cache/" + DSKY::wxGetApp().get_instance_hash_string() + ".lock";
        ...
        if (wxPlatformInfo::Get().GetOSMajorVersion() > 12)
            send_message_mac_closing(DSKY::wxGetApp().get_instance_hash_string(),
                                     DSKY::wxGetApp().get_instance_hash_string());
#endif
    }
}
...
static void list_matching_objects(const std::string &pattern, std::vector<std::string> &result)
{
    ...
    DBusMessage *message = dbus_message_new_method_call("org.freedesktop.DBus",  // Destination (the D-Bus daemon)
...
static bool multicast_one_message(const std::string &message_text, const std::string &interface_name)
{
    ...
    //std::string		interface_name = "com.oozebot.preflight.MulticastListener.Object" + reciever_id;
    //std::string		object_name = "/com/oozebot/preflight/MulticastListener/Object" + reciever_id;
    ...
    msg = dbus_message_new_method_call(interface_name.c_str(), object_name.c_str(), interface_name.c_str(),
                                       method_name.c_str());
    if (NULL == msg)
    {
...
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__;
    std::string pattern = R"(com\.oozebot\.preflight\.MulticastListener\.Object\d+)";
...
static bool send_message(const std::string &message_text, const std::string &version)
{
		...
		if ( !checker->IsAnotherRunning() ) */
    {
        ...
        dbus_uint32_t serial = 0;
        const char *sigval = message_text.c_str();
        //std::string		interface_name = "com.oozebot.preflight.InstanceCheck";
        std::string interface_name = "com.oozebot.preflight.InstanceCheck.Object" + version;
        std::string method_name = "AnotherInstance";
        //std::string		object_name = "/com/oozebot/preflight/InstanceCheck";
        std::string object_name = "/com/oozebot/preflight/InstanceCheck/Object" + version;
...
bool instance_check(int argc, char **argv, bool app_config_single_instance)
{
    ...
    if (appimage_env)
    {
        ...
        if (!appimage_env_valid)
            BOOST_LOG_TRIVIAL(error) << "APPIMAGE environment variable was set, but it does not point to a valid file: "
                                     << appimage_env;
    }
    if (!appimage_env_valid)
#endif // __linux__
        hashed_path = std::hash<std::string>{}(
            boost::filesystem::canonical(boost::filesystem::system_complete(argv[0]), ec).string());
    if (ec.value() > 0)
    { // canonical was not able to find the executable (can happen with appimage on some systems. Does it fail on Fuse file systems?)
        ec.clear();
    ...
    DSKY::wxGetApp().set_instance_hash(hashed_path);
...
#ifdef _WIN32
    DSKY::wxGetApp().init_single_instance_checker(lock_name + ".lock", data_dir() + "\\cache\\");
    if (cla.should_send.value() && DSKY::wxGetApp().single_instance_checker()->IsAnotherRunning())
    {
#else // mac & linx
    ...
    {
#endif
        instance_check_internal::send_message(instance_check_internal::compose_message_json("CLI", cla.cl_string),
                                              lock_name);
        BOOST_LOG_TRIVIAL(error)
...
void OtherInstanceMessageHandler::multicast_message(const std::string &message_type, const std::string &message_data)
{
    ...
    instance_check_internal::multicast_message_inner(
        instance_check_internal::compose_message_json(message_type, message_data));
}
...
#ifdef _WIN32
        else if (it->rfind("preflight://open/?file=", 0) == 0)
#else
        else if (it->rfind("preflight://open?file=", 0) == 0)
#endif
            downloads.emplace_back(*it);
        else if (it->rfind("preflight://login", 0) == 0)
        {
            wxPostEvent(m_callback_evt_handler,
                        LoginOtherInstanceEvent(DSKY::EVT_LOGIN_OTHER_INSTANCE, std::string(*it)));
        }
    }
    if (!paths.empty())
    {
        wxPostEvent(m_callback_evt_handler,
                    LoadFromOtherInstanceEvent(DSKY::EVT_LOAD_MODEL_OTHER_INSTANCE,
                                               std::vector<boost::filesystem::path>(std::move(paths))));
    }
    if (!downloads.empty())
    {
        wxPostEvent(m_callback_evt_handler,
                    StartDownloadOtherInstanceEvent(DSKY::EVT_START_DOWNLOAD_OTHER_INSTANCE,
...
{
    wxPostEvent(m_callback_evt_handler, SimpleEvent(DSKY::EVT_STORE_READ_REQUEST));
}
...
void OtherInstanceMessageHandler::handle_message(const std::string &message)
{
    ...
    assert(!type.empty());
    assert(m_message_handlers.find(type) !=
           m_message_handlers.end()); // this assert means there is an message type that has no handling.
...
static void respond_to_introspect(DBusConnection *connection, DBusMessage *request)
{
    ...
    const char *introspection_data = " <!DOCTYPE node PUBLIC \"-//freedesktop//DTD D-BUS Object Introspection 1.0//EN\" "
                                     ...
                                     "     </method>"
                                     "   </interface>"
                                     "   <interface name=\"com.oozebot.preflight.InstanceCheck\">"
...
//method AnotherInstance receives message from another preFlight instance
...
static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
{
    ...
    std::string our_interface = "com.oozebot.preflight.InstanceCheck.Object" + wxGetApp().get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen_instance_check()
{
    ...
    std::string interface_name = "com.oozebot.preflight.InstanceCheck.Object" + instance_hash;
    std::string object_name = "/com/oozebot/preflight/InstanceCheck/Object" + instance_hash;
    ...
    {
        BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another preFlight instance is running.";
        BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
        dbus_connection_unref(conn);
        return;
    }
    ...
    for (;;)
    {
        ...
        // Cancellable.
        {
            ...
            m_instance_check_thread_stop_condition.wait_for(lck, std::chrono::seconds(1),
                                                            [this] { return m_instance_check_thread_stop; });
        }
...
static void respond_to_introspect(DBusConnection *connection, DBusMessage *request)
{
    ...
    const char *introspection_data = " <!DOCTYPE node PUBLIC \"-//freedesktop//DTD D-BUS Object Introspection 1.0//EN\" "
                                     ...
                                     "     </method>"
                                     "   </interface>"
                                     "   <interface name=\"com.oozebot.preflight.MulticastListener\">"
                                     "     <method name=\"Message\">"
                                     "       <arg name=\"data\" direction=\"in\" type=\"s\" />"
                                     "     </method>"
                                     "	  <method name=\"Introspect\">"
...
//method Message receives message from another preFlight instance
...
static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
{
    const char *interface_name = dbus_message_get_interface(message);
    const char *member_name = dbus_message_get_member(message);
    std::string our_interface = "com.oozebot.preflight.MulticastListener.Object" + std::to_string(get_current_pid());
...
void OtherInstanceMessageHandler::listen_multicast()
{
    ...
    std::string interface_name = "com.oozebot.preflight.MulticastListener.Object" + pid;
    std::string object_name = "/com/oozebot/preflight/MulticastListener/Object" + pid;
    ...
    {
        BOOST_LOG_TRIVIAL(error)
            << "listen_multicast: Not primary owner of DBus name - probably another preFlight instance is running.";
        BOOST_LOG_TRIVIAL(error) << "listen_multicast: Dbus Messages listening terminating.";
        dbus_connection_unref(conn);
        return;
    }
    ...
    BOOST_LOG_TRIVIAL(debug) << "listen_multicast: Dbus object " << object_name
                             << " registered. Starting listening for messages.";
    ...
    for (;;)
    {
        ...
        // Cancellable.
        {
            ...
            m_multicast_listener_thread_stop_condition.wait_for(lck, std::chrono::seconds(1),
                                                                [this] { return m_multicast_listener_thread_stop; });
        }
