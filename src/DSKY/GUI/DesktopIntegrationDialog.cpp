...
bool create_desktop_file(const std::string &path, const std::string &data)
{
    ...
    output << data;
    output.close();
    // The stream state decides, not the presence of a file: a packaged desktop file in a read-only
    // system directory would otherwise count as written.
    if (output)
    {
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    std::string version(PREFLIGHT_VERSION);
    ...
    if (flatpak_id)
    {
        ...
        target_dir_icons = DSKY::format("%1%/.local/share", wxFileName::GetHomeDir());
        std::string icon_path = DSKY::format("%1%/icons/preFlight.svg", resources_dir());
        std::string dest_path = DSKY::format("%1%/icons/%2%preFlight%3%.svg", target_dir_icons, icon_theme_path,
                                             version_suffix);
    ...
    else
    {
        ...
        {
            // Copy SVG icon from resources_dir()/icons to target_dir_icons/icons/
    ...
    if (target_dir_icons.empty())
    {
        ...
        app_config->set("desktop_integration_icon_slicer_path",
                        DSKY::format("%1%/icons/%2%preFlight%3%.svg", target_dir_icons, icon_theme_path,
                                     version_suffix));
    ...
    std::string exec_line = flatpak_id ? DSKY::format("Exec=%1% %%F", excutable_path)
                                       : DSKY::format("Exec=\"%1%\" %%F", excutable_path);

    std::string desktop_file = DSKY::format(
        "[Desktop Entry]\n"
        "Name=preFlight%1%\n"
        ...
        "Keywords=3D;Printing;Slicer;slice;3D;printer;convert;gcode;stl;obj;amf\n"
        "StartupNotify=false\n"
    ...
    if (flatpak_id)
    {
        ...
        target_dir_desktop = DSKY::format("%1%/.local/share", wxFileName::GetHomeDir());
        std::string path = DSKY::format("%1%/applications/preFlight%2%.desktop", target_dir_desktop, version_suffix);
    ...
    else
    {
        ...
        for (size_t i = 0; i < target_candidates.size(); ++i)
        {
            if (contains_path_dir(target_candidates[i], "applications"))
            {
                ...
                std::string path = DSKY::format("%1%/applications/preFlight%2%.desktop", target_dir_desktop,
                                                version_suffix);
                if (create_desktop_file(path, desktop_file))
                {
    ...
    app_config->set("desktop_integration_app_path",
                    DSKY::format("%1%/applications/preFlight%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium)
    {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = DSKY::format("%1%/icons/preFlight-gcodeviewer.svg", resources_dir());
            std::string dest_path = DSKY::format("%1%/icons/%2%preFlight-gcodeviewer%3%.svg", target_dir_icons,
        ...
        // Desktop file
        std::string exec_line_viewer = flatpak_id ? DSKY::format("Exec=%1% --gcodeviewer %%F", excutable_path)
                                                  : DSKY::format("Exec=\"%1%\" --gcodeviewer %%F", excutable_path);
        std::string desktop_file_viewer = DSKY::format("[Desktop Entry]\n"
        ...
        std::string desktop_path = DSKY::format("%1%/applications/preFlightGcodeViewer%2%.desktop", target_dir_desktop,
                                                version_suffix);
    ...
    // Also create a desktop shortcut on ~/Desktop/
    {
        std::string desktop_dir = DSKY::format("%1%/Desktop", wxFileName::GetHomeDir());
        if (contains_path_dir(into_u8(wxFileName::GetHomeDir()), "Desktop"))
        {
            std::string desktop_shortcut_path = DSKY::format("%1%/Desktop/preFlight%2%.desktop",
    ...
    // Plater may not exist yet if called during early startup
    if (wxGetApp().plater())
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration()
{
    ...
    std::string desktop_file_downloader = DSKY::format("[Desktop Entry]\n"
    ...
    std::string desktop_path = DSKY::format("%1%/applications/preFlightURLProtocol%2%.desktop", target_dir_desktop,
                                            version_suffix);
    if (create_desktop_file(desktop_path, desktop_file_downloader))
    {
        ...
        std::string command =
            DSKY::format("xdg-mime default preFlightURLProtocol%1%.desktop x-scheme-handler/preflight", version_suffix);
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i)
    {
        if (contains_path_dir(target_candidates[i], "applications"))
        {
            target_dir_desktop = target_candidates[i];
            // Write slicer desktop file
            std::string path = DSKY::format("%1%/applications/preFlightURLProtocol%2%.desktop", target_dir_desktop,
                                            version_suffix);
    ...
    // finish registration on mime type
    std::string command = DSKY::format("xdg-mime default preFlightURLProtocol%1%.desktop x-scheme-handler/preflight",
                                       version_suffix);
    BOOST_LOG_TRIVIAL(debug) << "system command: " << command;
    int r = system(command.c_str());
    BOOST_LOG_TRIVIAL(debug) << "system result: " << r;
...
void DesktopIntegrationDialog::undo_downloader_registration_rigid()
{
    ...
    // If $XDG_DATA_DIRS is either not set or empty, a value equal to /usr/local/share/:/usr/share/ should be used.
    std::vector<std::string> target_candidates;
    target_candidates.emplace_back(DSKY::into_u8(wxFileName::GetHomeDir()) + "/.local/share");
    resolve_path_from_var("XDG_DATA_HOME", target_candidates);
    resolve_path_from_var("XDG_DATA_DIRS", target_candidates);
    for (const std::string &cand : target_candidates)
    {
        ...
        {
            boost::filesystem::path file_path = apps_path / DSKY::format("preFlightURLProtocol%1%.desktop", suffix);
            boost::system::error_code ec;
...
void DesktopIntegrationDialog::find_all_desktop_files(std::vector<boost::filesystem::path> &results)
{
    ...
    std::vector<std::string> target_candidates;
    if (auto home_config_dir = Luminary::get_home_local_dir(); home_config_dir)
    {
    ...
    for (const std::string &cand : target_candidates)
    {
        ...
        for (const std::string &filename : {"preFlight", "preFlightGcodeViewer", "preFlightURLProtocol"})
            for (const std::string &suffix : {"", "-beta", "-alpha", "_beta", "_alpha"})
            {
                boost::filesystem::path file_path = apps_path / DSKY::format("%1%%2%.desktop", filename, suffix);
                boost::system::error_code ec;
                if (!boost::filesystem::exists(file_path, ec) || ec)
                {
                    continue;
