...
void glAssertRecentCallImpl(const char *file_name, unsigned int line, const char *function_name)
{
    ...
    if (Luminary::get_logging_level() < 5)
        return;
#endif // NDEBUG
...
void GLVolume::SinkingContours::update()
{
    ...
    m_model.reset();
    DSKY::GLModel::Geometry init_data;
    init_data.format = {DSKY::GLModel::Geometry::EPrimitiveType::Triangles, DSKY::GLModel::Geometry::EVertexLayout::P3};
...
void GLVolume::NonManifoldEdges::update()
{
    ...
    if (0 <= object_idx && object_idx < int(model.objects.size()))
    {
        ...
        if (0 <= volume_idx && volume_idx < int(model_object->volumes.size()))
        {
            ...
            {
                DSKY::GLModel::Geometry init_data;
                init_data.format = {DSKY::GLModel::Geometry::EPrimitiveType::Lines,
                                    DSKY::GLModel::Geometry::EVertexLayout::P3};
...
// The selected-object highlight follows the theme accent, read at render time so it tracks
// live theme switches. Unselected volumes keep their filament/extruder color.
ColorRGBA GLVolume::selected_color()
{
    const wxColour &c = DSKY::active_palette().accent_primary;
...
{
    return m_instance_transformation.get_matrix() * m_volume_transformation.get_matrix();
}
                                           ...
                                           int instance_idx)
{
    ...
    if (m_use_raycasters)
        v.mesh_raycaster = std::make_unique<DSKY::MeshRaycaster>(mesh);
#else
    ...
    // Alignment Box modifiers are visible but not scene-pickable.
    ...
    for (GLVolumeWithIdAndZ &volume : to_render)
    {
        ...
        if (render_as_mmu_painted)
        {
            ...
            if (cache_miss)
            {
                ...
                entry.triangle_selector_mm = std::make_unique<DSKY::TriangleSelectorMmGui>(model_volume.mesh(),
            ...
            DSKY::TriangleSelectorMmGui *ts = it->second.triangle_selector_mm.get();
...
static void thick_lines_to_geometry(const Lines &lines, const std::vector<double> &widths,
                                    ...
                                    DSKY::GLModel::Geometry &geometry)
{
    assert(!lines.empty());
    if (lines.empty())
        return;
...
static void thick_lines_to_geometry(const Lines3 &lines, const std::vector<double> &widths,
                                    const std::vector<double> &heights, bool closed, DSKY::GLModel::Geometry &geometry)
{
    assert(!lines.empty());
    if (lines.empty())
        return;
...
void _3DScene::extrusionentity_to_verts(const ExtrusionMultiPath &extrusion_multi_path, float print_z,
                                        const Point &copy, DSKY::GLModel::Geometry &geometry)
{
    Lines lines;
    std::vector<double> widths;
    std::vector<double> heights;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (extrusion_entity_collection != nullptr)
                        ...
                        throw Luminary::RuntimeError("Unexpected extrusion_entity type in to_verts()");
                }
            }
        }
    }
