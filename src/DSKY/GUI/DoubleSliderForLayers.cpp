...
#include "luminary/core/text/Duration.hpp" // -> get_time_dhms()
#include "luminary/core/format.hpp"        // -> format()
#include "I18N.hpp"
#include "ImGuiWrapper.hpp"
#include "luminary/core/Prelude.hpp"
#include "DSKY/GUI/ImGuiDoubleSlider.hpp"
#include "DSKY/GUI/ImGuiPureWrap.hpp"
#include "DSKY/GUI/RulerForDoubleSlider.hpp"
#include "DSKY/GUI/TickCodesManager.hpp"
...
void DSForLayers::SetDrawMode(bool is_sequential_print)
{
...
void DSForLayers::draw_ticks(const ImRect &slideable_region)
{
    ...
    while (tick_it != m_ticks.ticks.end())
    {
        ...
        // Tick icons sit on the control's outer side.
        ImVec2 icon_pos = ImVec2(m_on_left ? m_ctrl.GetCtrlPos().x - icon_side : m_ctrl.GetCtrlPos().x + GetWidth(),
...
void DSForLayers::draw_ruler(const ImRect &slideable_region)
{
    ...
    if (m_show_ruler_bg)
    {
        ...
        // From the groove to the control's outer edge, where the ruler is.
        if (m_on_left)
        {
            bg_rect.Max.x += tick_width;
            bg_rect.Min.x = m_ctrl.GetCtrlPos().x;
        }
        else
        {
    ...
    auto draw_text = [max_val, x_center, label_height, long_outer_x, this](const int tick, const float tick_pos)
    {
        ...
        std::string label = (tick >= 0 && tick < (int) m_values.size())
                                ...
                                : "";
        // The ruler reads on the control's outer side: right of the groove, or left when on the left edge.
        const float gap = long_outer_x + 1.0f * m_scale;
        const float x = m_on_left ? x_center - gap - ImGui::CalcTextSize(label.c_str()).x : x_center + gap;
        ImGui::RenderText(ImVec2(x, tick_pos - (0.5f * label_height)), label.c_str());
    };

    auto draw_tick = [x_center, tick_width, inner_x, tick_clr, this](const float tick_pos, const float outer_x)
    {
        ImRect tick = m_on_left ? ImRect(x_center - outer_x, tick_pos - tick_width, x_center - inner_x, tick_pos)
                                : ImRect(x_center + inner_x, tick_pos - tick_width, x_center + outer_x, tick_pos);
        ImGui::RenderFrame(tick.Min, tick.Max, tick_clr, false);
    };
...
void DSForLayers::render_add_tick_menu()
{
    ...
    // The menu opens on the label side of the thumb: left of it, or right of it on the left-edge slider.
    ImGui::SetNextWindowPos(
        ImVec2(m_on_left ? pos.x + active_thumb_rect.GetWidth() : pos.x - label_size.x - active_thumb_rect.GetWidth(),
               pos.y));
            ...
            for (int i = 1; i <= extruders_cnt; i++)
            {
                const bool is_used_extruder =
                    used_extruders_for_tick.empty()
                        ? true
                        : // an empty set means every extruder; it is not filled for the multi-extruder mode
                         ...
                         float offset /*= 0.f*/)
{
    ...
    // On the left edge the slider keeps its layout; only its column moves.
    pos.x = m_on_left ? tick_offset : canvas_width - VERTICAL_SLIDER_WIDTH - tick_offset;
