...
int em_unit(wxWindow *win)
{
    if (win)
    {
        wxTopLevelWindow *toplevel = DSKY::find_toplevel_parent(win);
        DSKY::DPIDialog *dlg = dynamic_cast<DSKY::DPIDialog *>(toplevel);
        if (dlg)
            return dlg->em_unit();
        DSKY::DPIFrame *frame = dynamic_cast<DSKY::DPIFrame *>(toplevel);
        if (frame)
            return frame->em_unit();
    }

    return DSKY::wxGetApp().em_unit();
}

int mode_icon_px_size()
{
    // DPI-scaled mode icon size (10/12px base at 100% DPI)
    int em = DSKY::wxGetApp().em_unit();
#ifdef __APPLE__
                               ...
                               const std::string &new_color /* = std::string()*/)
{
    ...
    wxBitmapBundle *bmp = cache.from_svg(bmp_name, width, height, DSKY::wxGetApp().dark_mode(), new_color);
    if (bmp == nullptr)
    {
        bmp = cache.from_png(bmp_name, width, height);
        if (!bmp)
            // Neither SVG nor PNG has been found, raise error
            throw Luminary::RuntimeError("Could not load bitmap: " + bmp_name);
    }
    return bmp;
}
...
wxBitmapBundle *get_solid_bmp_bundle(int width, int height, const std::string &color)
{
    static DSKY::BitmapCache cache;
#ifdef __WXGTK2__
    return cache.mksolid_bndl(width * scale(), height * scale(), color, 1, DSKY::wxGetApp().dark_mode());
#else
    return cache.mksolid_bndl(width, height, color, 1, DSKY::wxGetApp().dark_mode());
#endif // __WXGTK2__
}
...
std::vector<wxBitmapBundle *> get_extruder_color_icons(bool thin_icon /* = false*/)
{
    ...
    std::vector<std::string> colors = DSKY::wxGetApp().plater()->get_extruder_color_strings_from_plater_config();
...
void apply_extruder_selector(DSKY::BitmapComboBox **ctrl, wxWindow *parent, const std::string &first_item /* = ""*/,
                             ...
                             bool use_thin_icon /* = false*/)
{
    ...
    if (!*ctrl)
    {
        *ctrl = new DSKY::BitmapComboBox(parent, wxID_ANY, wxEmptyString, pos, size, 0, nullptr, wxCB_READONLY);
        DSKY::wxGetApp().UpdateDarkUI(*ctrl);
    }
    else
    {
    ...
    const bool use_full_item_name = dynamic_cast<DSKY::ObjectList *>(parent) == nullptr;
    ...
    for (wxBitmapBundle *bmp : icons)
    {
        ...
        (*ctrl)->Append(use_full_item_name ? DSKY::from_u8((boost::format("%1% %2%") % str % i).str())
                                           : wxString::Format("%d", i),
                        *bmp);
        ++i;
    }
...
{
    wxString path = DSKY::from_u8(icon_path.string());
    wxBitmap bitmap;
...
#ifdef __APPLE__
        scales.emplace(DSKY::mac_max_scaling_factor());
#elif _WIN32
...
{
    DSKY::wxGetApp().UpdateDarkUI(this, m_has_border);
    if (m_current_icon_name.empty())
        return;
    wxBitmapBundle bmp = *get_bmp_bundle(m_current_icon_name, m_bmp_width, m_bmp_height);
    SetBitmap(bmp);
...
BlinkingBitmap::BlinkingBitmap(wxWindow *parent, const std::string &icon_name)
    : wxStaticBitmap(parent, wxID_ANY, wxNullBitmap, wxDefaultPosition,
                     wxSize(int(1.6 * DSKY::wxGetApp().em_unit()), -1))
{
