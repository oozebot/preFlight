...
#include <luminary/model/scene/Model.hpp> // ModelVolume
...
constexpr Luminary::MinMax<double> surface_distance_sq{1e-4, 10.}; // [in mm]
...
bool on_mouse_surface_drag(const DSKY::MouseInput &mouse, const Camera &camera,
                           ...
                           RaycastManager &raycast_manager, const std::optional<double> &up_limit)
{
    ...
    if (surface_drag.has_value() && !mouse.dragging)
    {
        ...
        return mouse.type == DSKY::MouseEventType::LeftUp;
    }

    if (mouse.type == DSKY::MouseEventType::Motion)
        return false;

    if (mouse.type == DSKY::MouseEventType::LeftDown)
...
std::optional<float> calc_distance(const GLVolume &gl_volume, const RaycastManager &raycaster,
                                   ...
                                   const std::optional<Luminary::Transform3d> &fix)
{
    ...
    const RaycastManager::Hit &hit = *hit_opt;
    // hit.squared_distance is in volume space not world
...
void selection_transform(Selection &selection, const std::function<void()> &selection_transformation_fnc)
{
    ...
    {
        // need editable gl volume .. can't use selection.get_first_volume()
                    ...
                    GLCanvas3D &canvas, RaycastManager &raycast_manager, const std::optional<double> &up_limit)
{
    ...
    const Vec3d volume_center = to_world.translation();
    // screen coordinate of volume center
    const Point coor = CameraUtils::project(camera, volume_center);
    const Vec2d mouse_offset = coor.cast<double>() - mouse_pos;
    ...
    {
        const std::optional<Luminary::Transform3d> &fix = es_opt->fix_3mf_tr;
        if (fix.has_value())
            volume_tr = volume_tr * fix->inverse();
    }
    ...
    surface_drag = SurfaceDrag{mouse_offset, world_tr,    instance_tr_inv, gl_volume_ptr,
                               condition,    start_angle, start_distance,  true};
