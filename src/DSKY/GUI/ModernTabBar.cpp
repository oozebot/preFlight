...
void ModernTabBar::AddSettingsDropdownButton(std::function<void(TabType)> callback)
{
    ...
    // --- Search action button (always visible with expanded settings) ---
    {
    ...
    m_settings_dropdown_btn->Bind(
        ...
        [this, select_settings_tab](wxMouseEvent &)
        {
            ...
            // Use wxID_HIGHEST+1 to avoid collision with TabType enum values
...
void ModernTabBar::SelectTab(TabType type)
{
    ...
    // Settings tabs (Print/Filament/Printer) are not in m_tabs; they are handled
    // via m_settings_callback from AddSettingsDropdownButton.
...
{
    // A wxPanel rather than a wxButton: wxButton on GTK3 has native theme rendering that cannot
    // be fully suppressed, which makes the custom-drawn buttons inconsistent. wxPanel with
    // wxBG_STYLE_PAINT gives a clean canvas on all platforms.
...
void ModernTabBar::AddSliceButton(std::function<void()> slice_callback, std::function<void()> export_callback)
{
    ...
    m_slice_button->Bind(wxEVT_PAINT,
                         [this](wxPaintEvent &event)
                         {
                             ...
                             // Colors
                             // Fully themed CTA - accent body, darker-accent dropdown zone + border, and the
...
#ifdef __APPLE__
                                 // Defer the callback to after the mouse event completes.
...
void ModernTabBar::FitPrinterWebViewButton()
{
    if (!m_printer_webview_btn)
        return;
...
void ModernTabBar::SetPrinterWebViewName(const wxString &printerName)
{
    if (!m_printer_webview_btn || printerName == m_printer_webview_name)
        return;

    m_printer_webview_name = printerName;
    FitPrinterWebViewButton();
    // The new width changes the space left for the settings buttons
    UpdateSettingsLayout(true);
    m_printer_webview_btn->Refresh();
}
