...
wxSize BitmapTextRenderer::GetSize() const
{
    ...
    // An icon alone (the override column while the item follows the project) reports its own
    // size, so the column's alignment centres it as it centres the icon-and-count pair
    if (m_value.GetBitmap().IsOk())
        return get_size(m_value.GetBitmap());
...
bool BitmapTextRenderer::GetValueFromEditorCtrl(wxWindow *ctrl, wxVariant &value)
{
    ...
    m_was_unusable_symbol = DSKY::has_illegal_characters(text_editor->GetValue());
...
#ifdef _WIN32
    DSKY::BitmapComboBox *c_editor = new DSKY::BitmapComboBox(parent, wxID_ANY, wxEmptyString,
#else
...
#ifdef _WIN32
    DSKY::wxGetApp().UpdateDarkUI(c_editor);
#endif
...
{
#ifdef _WIN32
    DSKY::BitmapComboBox *c = static_cast<DSKY::BitmapComboBox *>(ctrl);
#else
