        ...
        : P(parent, id, title, pos, size, style, name)
    {
        ...
        this->Bind(wxEVT_MOVE_END,
                   [this](wxMoveEvent &event)
                   {
                       ...
                       // If the DPI changed during the drag (cross-monitor move), perform the
                       // full rescale now that the move is complete, so every active control
                       // resizes to the new DPI.
