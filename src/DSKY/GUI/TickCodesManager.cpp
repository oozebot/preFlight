...
bool TickCodeManager::auto_color_change(Mode main_mode)
{
    ...
    for (auto object : m_print->objects())
    {
        ...
        check_color_change(object, 1, object->layers().size(), false,
                           [this, extruders_cnt, main_mode](const Layer *layer)
                           {
...
{
    if (mode == main_mode || (type != ColorChange && type != ToolChange) ||
...
std::set<int> TickCodeManager::get_used_extruders_for_tick(int tick, double print_z, Mode force_mode /* = Undef*/) const
{
    ...
    if (e_mode == MultiExtruder)
    {
        ...
        auto it_layer_tools = std::lower_bound(tool_ordering.begin(), tool_ordering.end(), print_z,
                                               [](const LayerTools &lhs, double rhs) { return lhs.print_z < rhs; });
...
bool TickCodeManager::edit_tick(std::set<TickCode>::iterator it, double print_z)
{
    ...
    // and the app will crash if tick codes overlap
...
{
    if ((tick.type == ColorChange && ((mode == SingleExtruder && main_mode == MultiExtruder) ||
                                      (mode == MultiExtruder && main_mode == SingleExtruder))) ||
        (tick.type == ToolChange && (mode == MultiAsSingle && main_mode != MultiAsSingle)))
