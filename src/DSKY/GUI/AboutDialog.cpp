...
CopyrightsDialog::CopyrightsDialog()
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr("%1% - %2%", wxGetApp().is_editor() ? PREFLIGHT_APP_NAME : GCODEVIEWER_APP_NAME,
...
AboutDialog::AboutDialog()
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr(_L("About %s"), wxGetApp().is_editor() ? PREFLIGHT_APP_NAME : GCODEVIEWER_APP_NAME),
                wxDefaultPosition, wxDefaultSize, /*wxCAPTION*/ wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    // title
    {
        ...
        wxFont title_font = DSKY::wxGetApp().bold_font();
    ...
    // version
    {
        auto version_string = _L("Version") + " " + std::string(PREFLIGHT_VERSION);
...
{
    wxTheClipboard->Open();
    wxTheClipboard->SetData(new wxTextDataObject(_L("Version") + " " + std::string(PREFLIGHT_VERSION)));
