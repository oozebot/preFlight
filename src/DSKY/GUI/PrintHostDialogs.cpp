    ...
    , m_paths(storage_paths)
{
        ...
        if (!path.Lower().EndsWith(m_valid_suffix.Lower()))
        {
            MessageDialog msg_wingow(
                ...
                wxString::Format(_L("Upload filename doesn't end with \"%s\". Do you wish to continue?"),
                                 m_valid_suffix),
                wxString(PREFLIGHT_APP_NAME), wxYES | wxNO);
    ...
    // Add the "Reset Upload Preferences" button between the action buttons and Cancel
...
{
    if (!combo_storage)
        return DSKY::format("%1%", m_preselected_storage);
    ...
    , on_info_evt(this, EVT_PRINTHOST_INFO, &PrintHostQueueDialog::on_info, this)
{
    ...
    btn_cancel->Bind(wxEVT_BUTTON,
                     [this](wxCommandEvent &)
                     {
                         ...
                         if (state < ST_ERROR)
                         {
                             DSKY::wxGetApp().printhost_job_queue().cancel(selected);
                         }
                     });
    ...
    btn_error->Bind(wxEVT_BUTTON,
                    [this](wxCommandEvent &)
                    {
                        ...
                        DSKY::show_error(nullptr, job_list->GetTextValue(selected, COL_ERRORMSG));
                    });
}
...
void PrintHostQueueDialog::on_progress(Event &evt)
{
    ...
    else if (get_state(evt.job_id) != ST_COMPLETED)
    {
        ...
        // Offer to switch to the Printer tab if available and not already viewing it
...
void PrintHostQueueDialog::on_error(Event &evt)
{
    ...
    DSKY::show_error(nullptr, errormsg);
