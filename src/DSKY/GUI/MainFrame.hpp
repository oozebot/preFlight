...
public:
    ...
    // The embedded view's content process died: reload once, and on a second death within a minute
    // tear the panel down so the printer tab opens the system browser from then on
    void on_printer_webview_terminated(const std::string &reason);
    // False when the browser preference is on or the embedded view failed this session
    bool printer_webview_embed_wanted() const;

    void refresh_account_menu(bool avatar = false);

    PrintHostQueueDialog *printhost_queue_dlg() { return m_printhost_queue_dlg; }

    ModernTabBar *m_modern_tabbar{nullptr};
    ...
    std::chrono::steady_clock::time_point m_printer_webview_last_termination{};
    // The mode the tab was last built for; a settings change to the other mode rebuilds it
    bool m_printer_webview_built_for_embed{false};
