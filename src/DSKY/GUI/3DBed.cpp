...
static const Luminary::ColorRGBA PICKING_MODEL_COLOR = Luminary::ColorRGBA::BLACK();
static const Luminary::ColorRGBA DISABLED_MODEL_COLOR = {0.6f, 0.6f, 0.6f, 0.75f};
...
static Luminary::ColorRGBA get_bed_color()
{
    return DSKY::IsDarkMode() ? wxColourToColorRGBA(UIColors::BedSurfaceDark())
                              : wxColourToColorRGBA(UIColors::BedSurfaceLight());
}

static Luminary::ColorRGBA get_grid_color(float alpha = 0.66f)
{
    return DSKY::IsDarkMode() ? wxColourToColorRGBA(UIColors::BedGridDark(), alpha)
                      ...
                      const std::string &custom_model, bool force_as_custom)
{
    ...
    m_contour = ExPolygon(Luminary::Polygon::new_scale(rounded_bed_shape));
...
void Bed3D::init_gridlines()
{
    ...
    for (const Luminary::Line &l : gridlines)
    {
...
void Bed3D::init_contourlines()
{
    ...
    for (const Luminary::Line &l : contour_lines)
    {
...
void Bed3D::render_shadow_catcher_geometry()
{
    init_triangles();
    if (m_triangles.is_initialized())
        m_triangles.render();
...
void Bed3D::render_logo(GLCanvas3D &canvas, const Transform3d &view_matrix, const Transform3d &projection_matrix)
{
    ...
    {
        std::string logo_path = Luminary::var("preFlight_platter.png");
