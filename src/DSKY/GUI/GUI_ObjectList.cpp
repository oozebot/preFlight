...
#include "luminary/model/build_volume/BuildVolume.hpp" // IWYU pragma: keep
...
#include "DSKY/Utils/FixModelByWin10.hpp"
                         ...
                         wxDV_MULTIPLE)
{
...
#ifdef __WXOSX__
    mac_track_mouse_moves(this);
    m_tooltip_timer.SetOwner(this);
    Bind(wxEVT_TIMER, [this](wxTimerEvent &) { show_hover_tooltip(); }, m_tooltip_timer.GetId());
    GetMainWindow()->Bind(wxEVT_MOTION,
                          [this](wxMouseEvent &event)
                          {
                              event.Skip();
                              const wxPoint pt = this->get_mouse_position_in_control();
                              wxDataViewItem item;
                              wxDataViewColumn *col = nullptr;
                              HitTest(pt, item, col);
                              const int em = wxGetApp().em_unit();
                              const bool on_icon = col != nullptr && col->GetTitle() == _("Name") && pt.x >= 2 * em &&
                                                   pt.x <= 4 * em;
                              if (item == m_hover_item && col == m_hover_column && on_icon == m_hover_on_icon)
                                  return;
                              m_hover_item = item;
                              m_hover_column = col;
                              m_hover_on_icon = on_icon;
                              hide_hover_tooltip();
                              if (item.IsOk() && col != nullptr)
                                  m_tooltip_timer.StartOnce(500);
                          });
    GetMainWindow()->Bind(wxEVT_LEAVE_WINDOW,
                          [this](wxMouseEvent &event)
                          {
                              event.Skip();
                              m_hover_item = wxDataViewItem(nullptr);
                              m_hover_column = nullptr;
                              hide_hover_tooltip();
                          });
    auto dismiss_hover_tooltip = [this](wxMouseEvent &event)
    {
        event.Skip();
        hide_hover_tooltip();
    };
    GetMainWindow()->Bind(wxEVT_LEFT_DOWN, dismiss_hover_tooltip);
    GetMainWindow()->Bind(wxEVT_RIGHT_DOWN, dismiss_hover_tooltip);
    GetMainWindow()->Bind(wxEVT_MOUSEWHEEL, dismiss_hover_tooltip);
#endif

    // The override column: a left click that goes down and up on the same cell opens the panel
    // for that row, once the click's own selection change has been processed.
    GetMainWindow()->Bind(wxEVT_LEFT_DOWN,
                          [this](wxMouseEvent &event)
                          {
                              event.Skip();
                              wxDataViewItem item;
                              wxDataViewColumn *col = nullptr;
                              HitTest(this->get_mouse_position_in_control(), item, col);
                              m_override_press_item = (item.IsOk() && col != nullptr &&
                                                       col->GetModelColumn() == colEditing)
                                                          ? item
    ...
    GetMainWindow()->Bind(wxEVT_LEFT_UP,
                          ...
                          {
                              event.Skip();
                              const wxDataViewItem pressed = m_override_press_item;
                              m_override_press_item = wxDataViewItem(nullptr);
                              if (!pressed.IsOk())
                                  return;
                              wxDataViewItem item;
                              wxDataViewColumn *col = nullptr;
#ifdef __WXOSX__
                              // The native table keeps the real mouse-up; wx synthesizes this event from
                              // the press once the table has handled the click, so its position is the
                              // press position and always lands on the pressed cell. The live pointer only
                              // has to be on the same row, which still rejects a drag released elsewhere
                              // without dropping a trackpad click that drifted out of the icon column.
                              HitTest(event.GetPosition(), item, col);
                              wxDataViewItem live;
                              wxDataViewColumn *live_col = nullptr;
                              HitTest(this->get_mouse_position_in_control(), live, live_col);
                              const bool released_on_cell = live == pressed;
#else
                              HitTest(this->get_mouse_position_in_control(), item, col);
                              const bool released_on_cell = true;
#endif
                              if (released_on_cell && item == pressed && col != nullptr &&
                                  col->GetModelColumn() == colEditing)
                                  CallAfter([this, item]() { open_overrides(item); });
                          });
#ifdef __WXGTK__
    // The tree view's click gesture claims a left press, so wxEVT_LEFT_DOWN never reaches the
    // control. The tree view's own press signal runs before that gesture and before the row is
    // selected, so the override cell's press is recorded from there. Returning FALSE leaves
    // selection, drag and the context menu to GTK.
    g_signal_connect(GtkGetTreeView(), "button-press-event",
                     G_CALLBACK(+[](GtkWidget *widget, GdkEventButton *event, gpointer data) -> gboolean
                                {
                                    if (event->type != GDK_BUTTON_PRESS || event->button != 1 ||
                                        ...
                                        return FALSE;
                                    ObjectList *list = static_cast<ObjectList *>(data);
                                    wxDataViewItem item;
                                    wxDataViewColumn *col = nullptr;
                                    list->HitTest(list->get_mouse_position_in_control(), item, col);
                                    list->m_override_press_item = (item.IsOk() && col != nullptr &&
                                                                   col->GetModelColumn() == colEditing)
                                                                      ? item
                                                                      : wxDataViewItem(nullptr);
                                    // A GTK tree view keeps its selection on a press below the last
                                    // row; the other platforms clear it, so the list does too. The
                                    // programmatic clear raises no selection event, so the
                                    // selection handling runs here.
                                    if (!item.IsOk() && list->GetSelectedItemsCount() > 0)
                                    {
                                        list->UnselectAll();
                     ...
                     this);
    // The tooltip follows the pointer as on Windows. The tree view's own motion signal is used
    // for the same reason as its press signal above: the native tree view keeps the pointer
    // events to itself, so nothing reaches the control as a wx event.
    g_signal_connect(GtkGetTreeView(), "motion-notify-event",
                     G_CALLBACK(+[](GtkWidget *, GdkEventMotion *, gpointer data) -> gboolean
                                {
                                    ObjectList *list = static_cast<ObjectList *>(data);
                                    list->set_tooltip_for_item(list->get_mouse_position_in_control());
                                    return FALSE;
                                }),
                     this);
    g_signal_connect(GtkGetTreeView(), "leave-notify-event",
                     G_CALLBACK(+[](GtkWidget *, GdkEventCrossing *, gpointer data) -> gboolean
                                {
                                    ObjectList *list = static_cast<ObjectList *>(data);
                                    list->set_tooltip_for_item(wxPoint(-1, -1)); // no cell: clears it
                                    return FALSE;
                                }),
                     this);
#endif // __WXGTK__
...
void ObjectList::create_objects_ctrl()
{
    ...
    // The override column: the icon that opens the panel, with the item's override count beside
    // it while it differs from the project
    AppendColumn(new wxDataViewColumn(_L("Overrides"), new BitmapTextRenderer(false, wxDATAVIEW_CELL_INERT), colEditing,
                                      6 * em, wxALIGN_CENTER_HORIZONTAL, 0));
    m_objects_model->SetOverrideCountFunction([](const wxDataViewItem &item)
                                              { return ObjectSettings::override_count(item); });
...
#ifdef __APPLE__
    // Disable the horizontal scrollbar and fit the columns once the native
    ...
    CallAfter(
        [this]()
        {
            DSKY::mac_disable_horizontal_scroll(GetHandle());
            GetColumn(colPrint)->SetWidth(m_print_col_width);
            GetColumn(colExtruder)->SetWidth(m_extruder_col_width);
            GetColumn(colEditing)->SetWidth(m_editing_col_width);
            int name_w = DSKY::mac_get_outlineview_name_width(GetHandle());
            if (name_w > 0)
...
#ifdef __APPLE__
                     // Re-assert fixed column widths (NSOutlineView's autoresizing
...
void ObjectList::set_tooltip_for_item(const wxPoint &pt)
{
     ...
     */
    // Every SetToolTip re-creates the native tooltip, which redraws it under a moving pointer,
    // so the text is only set when the cell under the pointer changes it.
    const wxString tooltip = cell_tooltip(item, col, pt.x);
    if (tooltip != GetMainWindow()->GetToolTipText())
        GetMainWindow()->SetToolTip(tooltip);
}

wxString ObjectList::cell_tooltip(const wxDataViewItem &item, const wxDataViewColumn *col, int x)
{
    if (!item.IsOk() || col == nullptr || GetSelectedItemsCount() > 1)
        return {};

    wxString tooltip;

    if (col->GetTitle() == _L("Overrides"))
    {
        OverrideScope scope;
        if (ObjectSettings::scope_of(item, scope))
        {
            const int count = ObjectSettings::override_count(item);
            tooltip = count > 0 ? format_wxstr(_L_PLURAL("%1% override", "%1% overrides", count), count)
                                : _L("Open the overrides for this item");
        }
    }
...
#endif //__WXMSW__
    else if (col->GetTitle() == _("Name") && (x >= 2 * wxGetApp().em_unit() && x <= 4 * wxGetApp().em_unit()))
    {
...
#ifdef __WXOSX__
void ObjectList::show_hover_tooltip()
{
    const wxPoint pt = this->get_mouse_position_in_control();
    wxDataViewItem item;
    wxDataViewColumn *col = nullptr;
    HitTest(pt, item, col);
    if (!(item.IsOk() && item == m_hover_item && col == m_hover_column))
        return; // the pointer moved on before the delay ran out

    const wxString text = cell_tooltip(item, col, pt.x);
    if (text.empty())
        return;

    // Below and to the right of the pointer, where the system puts its tooltips.
    const int em = wxGetApp().em_unit();
    mac_tooltip_show(text, wxGetMousePosition() + wxPoint(em, em * 3 / 2));
}

void ObjectList::hide_hover_tooltip()
{
...
void ObjectList::update_extruder_values_for_items(const size_t max_extruder)
{
    for (size_t i = 0; i < m_objects->size(); ++i)
    {
        ...
        // A missing key, 0 and an extruder the printer no longer has all read "default"
        auto column_text = [max_extruder](const ModelConfig &config)
        {
            const int extruder = config.has("extruder") ? config.extruder() : 0;
            return (extruder <= 0 || size_t(extruder) > max_extruder) ? wxString(_L("default"))
                                                                      : wxString::Format("%d", extruder);
        };

        auto object = (*m_objects)[i];
        m_objects_model->SetExtruder(column_text(object->config), item);
        ...
        if (object->volumes.size() > 1)
        {
            for (size_t id = 0; id < object->volumes.size(); id++)
            {
                ...
                if (!item)
                    continue;
                m_objects_model->SetExtruder(column_text(object->volumes[id]->config), item);
            }
        }
    }
}
...
void ObjectList::paste_layers_into_list()
{
    ...
    if (obj_idx < 0 || (int) m_objects->size() <= obj_idx || cache_ranges.empty())
        return;
...
void ObjectList::list_manipulation(const wxPoint &mouse_pos, bool evt_context_menu /* = false*/)
{
    ...
    if (col != nullptr)
    {
        ...
        else if (title == _L("Overrides"))
        {
            // A left click opens the override panel from the mouse-up handler; the context menu
    ...
    else
    {
        ...
        if (item)
        {
            ...
            if (type & itVolume)
            {
                ...
                // A layer range has no menu of its own: its settings are the override panel's
                menu = type & itInstance ? plater->instance_menu()
                       : type & itLayer  ? static_cast<wxMenu *>(nullptr)
                                         : plater->object_menu();
        }
        else if (evt_context_menu)
...
bool ObjectList::del_subobject_item(wxDataViewItem &item)
{
    ...
    if (type & itInstanceRoot && obj_idx != -1)
...
void ObjectList::del_instances_from_object(const int obj_idx)
{
    ...
    (*m_objects)[obj_idx]->invalidate_bounding_box();
...
bool ObjectList::del_subobject_from_object(const int obj_idx, const int idx, const int type)
{
    ...
    if (type == itVolume)
    {
        ...
        {
            DSKY::show_error(nullptr, _L("From Object List You can't delete the last solid part from object."));
            return false;
        }
        ...
        if (object->volumes.size() == 1)
        {
            ...
            if (!last_volume->config.empty())
            {
                ...
                update_override_column(obj_item);
            }
    ...
    else if (type == itInstance)
    {
        ...
        {
            DSKY::show_error(nullptr, _L("Last instance of an object cannot be deleted."));
            return false;
        }
        if (object->is_cut())
        {
            DSKY::show_error(nullptr, _L("Instance cannot be deleted from cut object."));
            return false;
        }
...
wxDataViewItem ObjectList::add_layer_root_item(const wxDataViewItem obj_item)
{
    ...
    if (obj_idx < 0 || object(obj_idx)->layer_config_ranges.empty())
...
bool ObjectList::has_selected_cut_object() const
{
    ...
    {
        const int obj_idx = m_objects_model->GetObjectIdByItem(item);
        // A deleted item can still be in sels, so obj_idx is range checked.
...
void ObjectList::part_selection_changed()
{
    ...
    // The override panel stays open while its item is the selection and closes when the item
    // leaves it. While it is open the manipulation and layer panels give it the room. A press on
    // the item's Overrides cell opens it with the selection itself, so the manipulation panel is
    // never shown in between the press and the release that would open it.
    ObjectSettings *overrides = wxGetApp().obj_settings();
    if (item && !multiple_selection() && m_override_press_item == item && !overrides->is_open_for(item))
        overrides->open_for(item);
    if (overrides->is_open())
    {
        if (item && !multiple_selection() && overrides->is_open_for(item))
            overrides->refresh();
        else
            overrides->close();
        update_and_show_settings = overrides->is_open();
    }
    if (update_and_show_settings)
    {
        update_and_show_manipulations = false;
    ...
    // Minimize ObjectList when the override panel is shown
...
void ObjectList::update_override_column(const wxDataViewItem &item)
{
    if (!item.IsOk())
        return;
    wxDataViewItem target = item;
    if (m_objects_model->GetItemType(target) & itInstance)
        target = m_objects_model->GetTopParent(target);
    m_objects_model->ItemChanged(target);
}
         ...
         item != items_for_delete.rend(); ++item)
    {
        ...
        else
        {
            ...
            if (item->type & itVolume)
            {
                ...
                wxGetApp().plater()->canvas3D()->ensure_on_bed(item->obj_idx, true);
            }
            else
...
void ObjectList::select_object_item(bool is_msr_gizmo)
{
    ...
    {
        ItemType type = m_objects_model->GetItemType(item);
        if ((is_msr_gizmo && type == itVolume) || type == itObject)
            return;
                                ...
                                const int layer_idx /* = -1*/)
{
    ...
    m_objects_model->AddLayersChild(layers_item, range, extruder2str(config.opt_int("extruder")), layer_idx);
}
...
int ObjectList::get_selected_layers_range_idx() const
{
    ...
    const ItemType type = m_objects_model->GetItemType(item);
    return type & itLayer ? m_objects_model->GetLayerIdByItem(item) : -1;
}
...
void ObjectList::update_selections()
{
    ...
    if ((m_selection_mode & (smLayer | smLayerRoot)) == 0)
        m_selection_mode = smInstance;

    // We doesn't update selection if itLayerRoot | itLayer Item for the current object/part is selected
    if (GetSelectedItemsCount() == 1 && m_objects_model->GetItemType(GetSelection()) & (itLayerRoot | itLayer))
    {
        const auto item = GetSelection();
        if (selection.is_single_full_object())
        {
        ...
        // Can be the case, when we have selected itLayerRoot | itLayer in the ObjectList and selected object/instance in the Scene
    ...
    else if (selection.is_single_full_object() || selection.is_multiple_full_object())
    {
        ...
        // it's impossible to select Layer or LayerRoot for several objects
        if (!selection.is_multiple_full_object() && (m_selection_mode & (smLayer | smLayerRoot)))
        {
            auto obj_idx = objects_content.begin()->first;
            wxDataViewItem obj_item = m_objects_model->GetItemById(obj_idx);
            if (m_selection_mode & smLayerRoot)
...
void ObjectList::update_selections_on_canvas()
{
    ...
    // The Preview draws this selection's bounding box and is not repainted by the Prepare canvas's render().
    ...
    if (sel_cnt == 1)
    {
        ...
        if (m_objects_model->GetInfoItemType(item) == InfoItemType::CutConnectors)
            selection.remove_all();
        if (m_objects_model->GetItemType(item) & (itInstanceRoot | itLayerRoot | itLayer))
...
void ObjectList::update_selection_mode()
{
    ...
    m_selection_mode = type & itLayer ? smLayer : type & itVolume ? smVolume : smInstance;
}
...
void ObjectList::change_part_type()
{
    ...
    if (type == ModelVolumeType::MODEL_PART)
    {
        ...
        if (model_part_cnt == 1)
        {
            DSKY::show_error(nullptr, _(L("You can't change a type of the last solid part of the object.")));
            return;
        }
    }
...
// A print preset change: the selected object's settings digest and the open override panel follow it
// A print preset change: the open override panel follows it
void ObjectList::update_and_show_object_settings_item()
{
    if (wxGetApp().obj_settings()->is_open())
        wxGetApp().obj_settings()->refresh();
}

void ObjectList::open_overrides(const wxDataViewItem &item)
{
    if (!item.IsOk())
        return;
    const wxDataViewItem target = item;
    OverrideScope scope;
    if (!ObjectSettings::scope_of(target, scope))
        return;

    if (!(GetSelectedItemsCount() == 1 && GetSelection() == target))
    {
        select_item(target);
        if (!m_prevent_canvas_selection_update)
            update_selections_on_canvas();
    }
    // Already open for the item (the selection change opened it): nothing to do
    if (wxGetApp().obj_settings()->is_open_for(target))
        return;
    ...
    if (open)
    {
        m_config = &get_item_config(target);
        wxGetApp().obj_manipul()->UpdateAndShow(false);
        wxGetApp().obj_layers()->UpdateAndShow(false);
    }
...
void ObjectList::update_object_list_by_printer_technology()
{
    ...
    for (auto &object_item : object_items)
    {
        ...
        update_override_column(object_item);
...
void ObjectList::set_extruder_for_selected_items(const int extruder) const
{
    ...
    for (const wxDataViewItem &item : sels)
    {
        ...
        wxGetApp().plater()->canvas3D()->ensure_on_bed(obj_idx, true);
    }
