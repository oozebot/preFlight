...
{
static bool fuzzy_match_recursive(const char_type *pattern, const char_type *str, int &outScore,
                                  const char_type *const strBegin, pos_type const *srcMatches, pos_type *newMatches,
                                  int nextMatch, int recursionCount, const int recursionLimit);
                                         ...
                                         pos_type *matches)
{
    ...
    return fuzzy_internal::fuzzy_match_recursive(pattern, str, outScore, str, nullptr, matches, 0, recursionCount,
                                                 recursionLimit);
}
    ...
    int recursionCount, const int recursionLimit)
{
    ...
    while (*pattern != '\0' && *str != '\0')
    {
        ...
        {
            wchar_t tmp[4];
            wchar_t *end = Luminary::fold_to_ascii(*str, tmp);
            wchar_t *c = tmp;
    ...
    if (matched)
    {
        ...
        static constexpr int leading_letter_penalty =
            -1 /*-5*/; // penalty applied for every letter in str before the first match
        ...
        outScore += matches[0] == int(group_start - strBegin)
                        ? first_letter_bonus
                        : std::max((matches[0] - int(group_start - strBegin)) * leading_letter_penalty,
