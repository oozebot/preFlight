...
gboolean webkit_authorize_handler(WebKitWebView *web_view, WebKitAuthenticationRequest *request, gpointer user_data)
{
    ...
    webkit_authentication_request_authenticate(request,
                                               webkit_credential_new(creds.username.c_str(), creds.password.c_str(),
...
void setup_webview_with_credentials(wxWebView *web_view, const std::string &username, const std::string &password)
{
    ...
    auto handler = g_signal_connect_data(native_backend, "authenticate", G_CALLBACK(webkit_authorize_handler),
                                         user_data, &free_credentials, static_cast<GConnectFlags>(0));
...
{
    WebKitWebView *native_backend = static_cast<WebKitWebView *>(web_view->GetNativeBackend());
    if (auto it = g_webview_authorize_handlers.find(native_backend); it != g_webview_authorize_handlers.end())
    {
...
void get_cookie_callback(GObject *source_object, GAsyncResult *result, void *user_data)
{
    ...
    for (GList *l = cookies; l != nullptr; l = l->next)
    {
        ...
        */
        webkit_cookie_manager_delete_cookie(cookie_manager, cookie, nullptr,
                                            (GAsyncReadyCallback) DSKY::delete_cookie_callback, nullptr);
...
void delete_cookies(wxWebView *web_view, const std::string &url)
{
    ...
    webkit_cookie_manager_get_cookies(cookieManager, uri, nullptr, (GAsyncReadyCallback) DSKY::get_cookie_callback,
                                      nullptr);
}
...
void free_termination_handler(gpointer user_data, GClosure *)
{
    delete static_cast<TerminationHandler *>(user_data);
}

void webkit_terminated_handler(WebKitWebView *, WebKitWebProcessTerminationReason reason, gpointer user_data)
{
    ...
    case WEBKIT_WEB_PROCESS_EXCEEDED_MEMORY_LIMIT:
        why = "exceeded_memory_limit";
        break;
    case WEBKIT_WEB_PROCESS_TERMINATED_BY_API:
        why = "terminated_by_api";
        break;
    default:
        break;
    }
    static_cast<TerminationHandler *>(user_data)->on_terminated(why);
}
} // namespace

void setup_webview_termination_handler(wxWebView *web_view, std::function<void(const std::string &)> on_terminated)
{
    WebKitWebView *native_backend = static_cast<WebKitWebView *>(web_view->GetNativeBackend());
    if (!native_backend)
        return;
    // The handler and its closure live as long as the native view; the view's destruction disconnects it.
    g_signal_connect_data(native_backend, "web-process-terminated", G_CALLBACK(webkit_terminated_handler),
                          new TerminationHandler{std::move(on_terminated)}, &free_termination_handler,
