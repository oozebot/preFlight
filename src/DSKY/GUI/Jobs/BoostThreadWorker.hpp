...
class BoostThreadWorker : public Worker, private Job::Ctl
{
    ...
    public:
        using RawQueue<El>::RawQueue;
        void push_back(El &&entry) { this->emplace_back(std::move(entry)); }
...
public:
    explicit BoostThreadWorker(std::shared_ptr<ProgressIndicator> pri, boost::thread::attributes &attr,
                               const char *name = "");
    ...
    explicit BoostThreadWorker(std::shared_ptr<ProgressIndicator> pri, const char *name = "")
