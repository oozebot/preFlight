...
#include <luminary/format/mesh/OBJ.hpp>                     // load_obj for default mesh
#include <luminary/gizmo/emboss/surface_cut/CutSurface.hpp> // use surface cuts
#include <luminary/model/build_volume/BuildVolume.hpp>      // create object
...
#include "DSKY/GUI/Jobs/Worker.hpp"
#include "DSKY/Utils/UndoRedo.hpp"
#include "DSKY/Utils/RaycastManager.hpp"
...
/// EmbossDataBase::font_file doesn't have to be valid
/// </summary>
...
void DSKY::Emboss::DataBase::write(ModelVolume &volume) const
{
...
void CreateObjectJob::process(Ctl &ctl)
{
    ...
    Luminary::Polygon bed(bed_shape_);
...
void CreateObjectJob::finalize(bool canceled, std::exception_ptr &eptr)
{
...
#endif /* _DEBUG */
    {
        ...
        // set transformation
        Luminary::Geometry::Transformation tr(m_transformation);
    ...
    for (size_t text_line_index = 0; text_line_index < input.text_lines.size(); ++text_line_index)
    {
        ...
        { // Debug store polygon
            ...
            for (size_t i = 0; i < samples.size(); i++)
            {
                ...
                svg.draw(Luminary::Line(p - n, p + n), "Lime");
            }
        }
#endif // STORE_SAMPLING
    }
...
TriangleMesh try_create_mesh(DataBase &input, const Fnc &was_canceled)
{
    ...
    // SHAPE_SCALE is applied in ProjectZ
...
TriangleMesh create_default_mesh()
{
    ...
    std::string path = Luminary::resources_dir() + "/data/embossed_text.obj";
                   ...
                   const std::optional<Transform3d> &trmat, const DataBase &data, GLGizmosManager::EType gizmo)
{
    ...
    // be carefull add volume also center mesh
                                        ...
                                        std::function<bool()> was_canceled)
{
    ...
    bool is_text_reflected = Luminary::has_reflection(tr);
    if (is_text_reflected)
    {
        ...
        for (ExPolygon &shape : shapes_data)
        {
            ...
            for (Luminary::Polygon &hole : shape.holes)
                hole.reverse();
        }
                             ...
                             const ModelObjectPtrs &objects, Vec2d *closest_center)
{
    ...
    for (unsigned int id : indices)
    {
        ...
        if (const ModelVolume *volume = get_model_volume(*gl_volume, objects);
            ...
            continue;
        Luminary::Polygon hull = CameraUtils::create_hull2d(camera, *gl_volume);
