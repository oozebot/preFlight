...
#include "DSKY/GUI/Jobs/ProgressIndicator.hpp"
#include "DSKY/GUI/Jobs/ThreadSafeQueue.hpp"
...
void BoostThreadWorker::run()
{
    ...
    while (!stop)
    {
        m_input_queue.consume_one(BlockingWait{0},
                                  [this, &stop](JobEntry &e)
                                  {
                                      if (!e.job)
                                          stop = true;
                                      else
...
BoostThreadWorker::BoostThreadWorker(std::shared_ptr<ProgressIndicator> pri, boost::thread::attributes &attribs,
                                     const char *name)
...
bool BoostThreadWorker::wait_for_current_job(unsigned timeout_ms)
{
    ...
    if (!is_idle())
    {
        ...
        {
            timeout_reached = !m_output_queue.consume_one(BlockingWait{timeout_ms},
                                                          [this, &was_finish](WorkerMessage &msg)
                                                          {
                                                              msg.deliver(*this);
...
bool BoostThreadWorker::wait_for_idle(unsigned timeout_ms)
{
    ...
    {
        timeout_reached = !m_output_queue.consume_one(BlockingWait{timeout_ms},
                                                      [this](WorkerMessage &msg) { msg.deliver(*this); });
    }
