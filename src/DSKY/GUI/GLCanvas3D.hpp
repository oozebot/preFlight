...
// A left click without a drag on the Preview canvas: the position and whether Shift was held.
wxDECLARE_EVENT(EVT_GLCANVAS_PREVIEW_LEFT_CLICK, RBtnEvent);
...
#ifndef _WIN32
        bool left_down_on_canvas{false}; // tracks genuine LeftDown to filter phantom LeftUp (Linux/macOS)
#endif
        Drag drag;
...
private:
    ...
    // Width of the strip reserved on the sidebar's side: the Preview's legend or floating sidebar, or the slot
    // of Prepare's collapsed sidebar. The scene is framed beside it in both views.
    std::function<float()> m_strip_width_fn;
    // The strip's panel is hidden (the legend folded, Prepare's sidebar collapsed): the scene keeps its framing
    // beside the strip and is drawn across it as well.
    std::function<bool()> m_strip_hidden_fn;
    // The selection whose bounding box the Preview draws. The Preview keeps none of its own: the object list and
    // clicks on its shells select into the Prepare canvas's selection, and the Preview shows that one.
    std::function<Selection *()> m_shown_selection_fn;
...
private:
    ...
    // Offscreen shadow/AO passes for the Full lighting tier.
    ...
    // What a middle/right drag does to the camera under the active mouse scheme.
    enum class ECameraNavAction
    {
        None,
        Orbit,
        Pan
    };
    ECameraNavAction _nav_action_for_drag(const MouseInput &mouse) const;
    void _camera_orbit_step(const Point &pos);
...
public:
    ...
    void set_strip_hidden_fn(std::function<bool()> fn) { m_strip_hidden_fn = std::move(fn); }
    void set_shown_selection_fn(std::function<Selection *()> fn) { m_shown_selection_fn = std::move(fn); }
    // Canvas pixels of the strip on each side (one of them is zero); the scene is framed beside it and the
    // overlays anchored on that side keep clear of it.
    float get_left_inset() const;
    float get_right_inset() const;
    bool is_strip_hidden() const;
    // Canvas pixels of the panel shown at each edge, zero while it is hidden; overlays that sit beside the
    // panel use these, overlays that keep the framing use the insets.
    float get_shown_left_panel() const { return is_strip_hidden() ? 0.0f : get_left_inset(); }
    float get_shown_right_panel() const { return is_strip_hidden() ? 0.0f : get_right_inset(); }
    ...
    // Release the GL context so the GPU driver can drop to idle power state.
...
public:
    ...
    // Pans the view so the given point sits at the center of the scene and becomes the orbit pivot.
    // View direction and zoom are kept.
    void animate_pivot_to(const Vec3d &target);
    ...
    // Returns true when GPU power-saving optimizations should be active.
    ...
    bool is_legend_collapsed() const { return m_gcode_viewer.is_legend_collapsed(); }
    bool is_legend_drawn() const { return m_gcode_viewer.is_legend_drawn(); }
...
private:
    ...
    // Offset of the center of the canvas beside the shown panel from the canvas center, in canvas pixels.
    float _shown_panel_offset() const;
    // Canvas position inside the scene viewport, outside the strip reserved for the legend or the sidebar.
    bool _is_in_scene_viewport(const Vec2d &pos) const;
#if ENABLE_SHOW_CAMERA_TARGET
