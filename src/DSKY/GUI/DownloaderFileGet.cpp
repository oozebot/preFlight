...
struct FileGet::priv
{
    ...
    bool m_load_after;
    priv(int ID, std::string &&url, const std::string &filename, wxEvtHandler *evt_handler,
         const boost::filesystem::path &dest_folder, bool load_after);

    void get_perform();
};

FileGet::priv::priv(int ID, std::string &&url, const std::string &filename, wxEvtHandler *evt_handler,
                    const boost::filesystem::path &dest_folder, bool load_after)
    : m_id(ID)
    , m_url(std::move(url))
    , m_filename(filename)
...
void FileGet::priv::get_perform()
{
    ...
    if (m_written == 0)
    {
        ...
        try
        {
            ...
            while (boost::filesystem::exists(m_dest_folder / (final_filename + extension)) ||
                   boost::filesystem::exists(m_dest_folder / (final_filename + extension + "." +
                                                              std::to_string(get_current_pid()) + ".download")))
            {
                ++version;
                if (version > 999)
                {
                    wxCommandEvent *evt = new wxCommandEvent(EVT_DWNLDR_FILE_ERROR);
                    evt->SetString(DSKY::format_wxstr(L"Failed to find suitable filename. Last name: %1%.",
                                                      (m_dest_folder / (final_filename + extension)).string()));
                    evt->SetInt(m_id);
                    m_evt_handler->QueueEvent(evt);
                    return;
                }
                final_filename = DSKY::format("%1%(%2%)", just_filename, std::to_string(version));
            }
        }
    ...
    BOOST_LOG_TRIVIAL(info) << DSKY::format("Starting download from %1% to %2%. Temp path is %3%", m_url, dest_path,
                                            m_tmp_path);
    ...
    if (file == NULL)
    {
        wxCommandEvent *evt = new wxCommandEvent(EVT_DWNLDR_FILE_ERROR);
        // TRN %1% = file path
        evt->SetString(DSKY::format_wxstr(_L("Can't create file at %1%"), m_tmp_path.string()));
        evt->SetInt(m_id);
        m_evt_handler->QueueEvent(evt);
        return;
    }
    ...
    Http::get(m_url)
            ...
            [&](Http::Progress progress, bool &cancel)
            {
                ...
                if (progress.dlnow != 0)
                {
                    ...
                    int percent_total = m_absolute_size == 0
                                            ? 0
                                            : (written_previously + progress.dlnow) * 100 / m_absolute_size;
                    ...
                    while (boost::filesystem::exists(m_dest_folder / (final_filename + extension)))
                    {
                        ...
                        if (version > 999)
                        {
                            BOOST_LOG_TRIVIAL(error)
                                << DSKY::format("Failed to find suitable filename. Last name: %1%.",
                                                (m_dest_folder / (final_filename + extension)).string());
                            return;
                        }
                        final_filename = DSKY::format("%1%(%2%)", just_filename, std::to_string(version));
        ...
        .on_error(
            [&](std::string body, std::string error, unsigned http_status)
            {
                ...
                wxCommandEvent *evt = new wxCommandEvent(EVT_DWNLDR_FILE_ERROR);
                if (!error.empty())
                    evt->SetString(DSKY::from_u8(error));
                else
                    evt->SetString(DSKY::from_u8(body));
                evt->SetInt(m_id);
                m_evt_handler->QueueEvent(evt);
            })
        .on_complete(
            [&](std::string body, unsigned /* http_status */)
            {
                ...
                {
                    wxCommandEvent *evt = new wxCommandEvent(EVT_DWNLDR_FILE_ERROR);
                    evt->SetString(DSKY::format("Failed to write and move %1% to %2%", m_tmp_path, dest_path));
                    evt->SetInt(m_id);
                    m_evt_handler->QueueEvent(evt);
                    return;
                }
...
FileGet::FileGet(int ID, std::string url, const std::string &filename, wxEvtHandler *evt_handler,
                 const boost::filesystem::path &dest_folder, bool load_after)
