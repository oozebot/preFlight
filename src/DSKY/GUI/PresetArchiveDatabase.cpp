...
// #include "DSKY/GUI/UserAccount.hpp"
#include "DSKY/Utils/PresetUpdaterWrapper.hpp"
...
bool LocalArchiveRepository::get_file_inner(const fs::path &source_path, const fs::path &target_path) const
{
    ...
    CopyFileResult cfr = Luminary::copy_file(source_path.string(), target_path.string(), error_message, false);
    ...
    for (const std::string &uuid : selected_uuids)
    {
        ...
        for (const auto &archive : m_archive_repositories)
        {
            ...
            if (!archive->is_extracted())
            {
                ...
                msg = DSKY::format(_L("Cannot select local source from path: %1%. It was not extracted."),
        ...
        {
            msg = DSKY::format(_L("Cannot select two sources with the same id: %1% and %2%"), it->second, name);
            return false;
        }
...
std::string PresetArchiveDatabase::add_local_archive(const boost::filesystem::path path, std::string &msg)
{
    ...
    {
        msg = DSKY::format(_L("Failed to add local archive %1%. Path already used."), path);
    ...
    {
        msg = DSKY::format(_L("Failed to extract local archive %1%."), path);
        BOOST_LOG_TRIVIAL(error) << msg;
        return std::string();
    }
...
{
    return (boost::filesystem::path(Luminary::data_dir()) / "ArchiveRepositoryManifest.json").make_preferred();
}
