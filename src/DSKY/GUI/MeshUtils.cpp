...
#include "luminary/gizmo/csg/SliceCSGMesh.hpp"
...
int MeshClipper::is_projection_inside_cut(const Vec3d &point_in) const
{
    ...
    for (int i = 0; i < int(m_result->cut_islands.size()); ++i)
    {
        ...
        if (isl.expoly_bb.contains(pt_2d) && isl.expoly.contains(pt_2d))
            return i; // the first containing contour wins, also where contours overlap
    }
    return -1;
}
...
void MeshClipper::recalculate_triangles()
{
    ...
    {
        expolys = std::move(
            csg::slice_csgmesh_ex(range(m_csgmesh), {height_mesh}, MeshSlicingParamsEx{slicing_params}).front());
    }
    ...
    if (m_limiting_plane != ClippingPlane::ClipsNothing())
    {
        ...
        else
        {
            ...
            const coord_t size = (std::numeric_limits<coord_t>::max() / 2 -
                                 ...
                                 4;
            Polygons ep{Luminary::Polygon({Point(-size, coord_t(0)), Point(size, coord_t(0)),
                                           Point(size, coord_t(2) * size), Point(-size, coord_t(2) * size)})};
    ...
    for (const ExPolygon &exp : expolys)
    {
        ...
        isl.hash = isl.expoly.contour.size() + size_t(std::abs(100. * centroid_world.x())) +
                   size_t(std::abs(100. * centroid_world.y())) + size_t(std::abs(100. * centroid_world.z()));
    }
...
std::vector<unsigned> MeshRaycaster::get_unobscured_idxs(const Geometry::Transformation &trafo, const Camera &camera,
                                                         ...
                                                         const ClippingPlane *clipping_plane) const
{
    ...
    Vec3d direction_to_camera_mesh =
        (instance_matrix_no_translation_no_scaling.inverse() * direction_to_camera).normalized().eval();
    ...
    for (size_t i = 0; i < points.size(); ++i)
    {
        ...
        if (!hits.empty())
        {
            ...
            // Only the camera direction is known, so a hit behind the camera also counts as obscuring.
            // The epsilon above is in mesh coordinates.
            if (!hits.empty())
                is_obscured = true;
        }
        if (!is_obscured)
...
bool MeshRaycaster::closest_hit(const Vec2d &mouse_pos, const Transform3d &trafo, const Camera &camera, Vec3f &position,
                                Vec3f &normal, const ClippingPlane *clipping_plane, size_t *facet_idx) const
{
    Vec3d point;
    Vec3d direction;
    line_from_mouse_pos(mouse_pos, trafo, camera, point, direction);
...
Vec3f MeshRaycaster::get_closest_point(const Vec3f &point, Vec3f *normal) const
{
    ...
    if (normal)
        // The face normal, so a point on an edge or a vertex reports one of its faces rather than a blend.
