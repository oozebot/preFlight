...
static char marker_by_type(Preset::Type type, PrinterTechnology pt)
{
    ...
    case Preset::TYPE_PRINTER:
        return ImGui::PrinterIconMarker;
...
void OptionsSearcher::append_options(DynamicPrintConfig *config, Preset::Type type)
{
    auto emplace = [this, type](std::string key, const wxString &label, int id = -1)
    {
        ...
        else if (gc.group == "Overhang speed" && id >= 0)
        {
        ...
        if (!label.IsEmpty())
            options.emplace_back(Option{boost::nowide::widen(key), type, (label + suffix).ToStdWstring(),
                                        ...
                                        DSKY::Tab::translate_category(gc.category, type).ToStdWstring()});
    };
...
void OptionsSearcher::append_preferences_option(const DSKY::Line &opt_line)
{
...
void OptionsSearcher::append_preferences_options(const std::vector<DSKY::Line> &opt_lines)
{
    for (const DSKY::Line &line : opt_lines)
    {
                            ...
                            const GroupAndCategory &gc)
{
    ...
    return Option{boost::nowide::widen(get_key(opt_key, type)),
                  ...
                  DSKY::Tab::translate_category(category, type).ToStdWstring()};
}
...
{
    // The dialog is centered on the parent at creation and user-movable via the title bar;
...
void OptionsSearcher::show_dialog(bool show /*= true*/)
{
    ...
    // The dialog has its own filter input that receives focus in Popup()
    search_dialog->Popup();
}
...
void OptionsSearcher::edit_search_input()
{
    ...
    {
        DSKY::wxGetApp().show_search_dialog();
    }
}
...
// Owner-drawn search results list with a custom ScrollBar, so the scrollbar follows the theme.

// Maps icon marker characters to icon indices 0-3
static const std::map<const char, int> icon_idxs = {
    {ImGui::PrintIconMarker, 0},
    {ImGui::PrinterIconMarker, 1},
    {ImGui::FilamentIconMarker, 2},
    {ImGui::PreferencesButton, 3},
};
...
private:
    int RowHeight() const { return static_cast<int>(DSKY::wxGetApp().em_unit() * 1.8); }
    ...
    void OnPaint(wxPaintEvent &event)
    {
        ...
        const int rowH = RowHeight();
        const int em = DSKY::wxGetApp().em_unit();
        ...
        dc.SetFont(GetFont().IsOk() ? GetFont() : DSKY::wxGetApp().normal_font());
...
SearchDialog::SearchDialog(OptionsSearcher *searcher, wxWindow *parent)
    : DSKY::DPIDialog(parent ? parent : DSKY::wxGetApp().tab_panel(), wxID_ANY, _L("Search"), wxDefaultPosition,
                      wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
    , searcher(searcher)
{
    SetFont(DSKY::wxGetApp().normal_font());
#if _WIN32
    DSKY::wxGetApp().UpdateDarkUI(this);
#elif __WXGTK__
    SetBackgroundColour(DSKY::wxGetApp().get_window_default_clr());
#endif

    int em = em_unit();
    int border = em;

    // Initialize category icons, in the order icon_idxs indexes them
    int icon_id = 0;
    for (const std::string &icon : {"cog", "printer", "spool", "notification_preferences"})
        m_icons[icon_id++] = ScalableBitmap(this, icon);

    // Filter input field within the dialog
    m_filter_input = new ::TextInput(this, wxEmptyString, "", "search", wxDefaultPosition, wxSize(em * 50, -1),
                                     wxTE_PROCESS_ENTER);
    m_filter_input->SetFont(DSKY::wxGetApp().normal_font());
    DSKY::wxGetApp().UpdateDarkUI(m_filter_input);
#ifdef __APPLE__
    ...
    DSKY::mac_set_focus_ring_none(m_filter_input->GetHandle());
    if (wxTextCtrl *tc = m_filter_input->GetTextCtrl())
    {
        DSKY::mac_set_focus_ring_none(tc->GetHandle());
        ...
        tc->Bind(wxEVT_SET_FOCUS,
                 [tc](wxFocusEvent &e)
                 {
                     ...
                     DSKY::mac_set_insertion_point_color(tc->GetHandle(), c.Red(), c.Green(), c.Blue());
                 });
    }
#endif
    ...
    // Custom ScrollBar + owner-drawn results panel for a themed scrollbar
    ...
    m_results_panel->SetFont(DSKY::wxGetApp().normal_font());
    ...
    DSKY::wxGetApp().UpdateDarkUI(static_cast<wxButton *>(this->FindWindowById(wxID_CANCEL, this)));
    ...
    // Results panel + custom scrollbar side by side
    ...
    // Center on the parent so the dialog never appears off-screen
    CenterOnParent();

#ifdef _WIN32
    DSKY::wxGetApp().UpdateDlgDarkUI(this);
#endif
}
...
void SearchDialog::Popup(wxPoint position /*= wxDefaultPosition*/)
{
    ...
    // Center on the main application window each time
    if (wxWindow *top = DSKY::wxGetApp().GetTopWindow())
    {
...
void SearchDialog::ProcessSelection(int row_index)
{
    ...
    wxPostEvent(DSKY::wxGetApp().mainframe, event);
}
...
{
#ifdef _WIN32
    DSKY::wxGetApp().UpdateAllStaticTextDarkUI(this);
    DSKY::wxGetApp().UpdateDarkUI(static_cast<wxButton *>(this->FindWindowById(wxID_CANCEL, this)), true);
    for (wxWindow *win : std::vector<wxWindow *>{check_category, check_english})
        if (win)
            DSKY::wxGetApp().UpdateDarkUI(win);
#endif

    // Update icons for new theme. Only the leading icon_idxs.size() slots of m_icons are populated;
    // the trailing ones have no icon name and would throw.
    for (size_t i = 0; i < icon_idxs.size(); ++i)
        m_icons[i].sys_color_changed();
