...
#include "DSKY/GUI/Search.hpp" // IWYU pragma: keep
...
const t_field &OptionsGroup::build_field(const t_config_option_key &id, const ConfigOptionDef &opt)
{
    ...
    default:
        ...
        case coNone:
            break;
        default:
            throw Luminary::LogicError("This control doesn't exist till now");
            break;
        }
    }
...
void OptionsGroup::activate_line(Line &line)
{
    ...
    if (line.full_width && (line.widget != nullptr || !line.get_extra_widgets().empty()))
    {
        if (line.widget != nullptr)
        {
            ...
            // Consistent padding on all platforms: FlatStaticBox has no native wxStaticBox
            // padding to lean on, so the margins are explicit. LEFT+TOP only, because the
            // bottom padding is handled by the group border itself.
#ifdef __WXOSX__
...
#ifdef __APPLE__
                // wxBG_STYLE_PAINT on macOS doesn't auto-fill the background,
                            ...
                            int horiz_alignment /* = wxALIGN_LEFT*/)
{
    ...
    try
    {
        ...
        // On GTK, the GtkFrame label is removed in FlatStaticBox.
...
boost::any ConfigOptionsGroup::config_value(const std::string &opt_key, int opt_index, bool deserialize)
{
    if (deserialize)
    {
        ...
        // Currently used for the post_process config value only.
        if (opt_index != -1)
            throw Luminary::OutOfRange("Can't deserialize option indexed value");
...
void ogStaticText::FocusText(bool focus)
{
    ...
    SetFont(focus ? DSKY::wxGetApp().link_font() : DSKY::wxGetApp().normal_font());
#ifdef __linux__
