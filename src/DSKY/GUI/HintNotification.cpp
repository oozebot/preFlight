...
struct HintsCerealData
{
    ...
    void serialize(Archive &ar, std::uint32_t const version)
    {
        ...
        if (version > HINTS_CEREAL_VERSION)
            throw Luminary::IOError("Version of hints.cereal is higher than current version.");
        else
            ar(my_data);
    }
};
...
{
    boost::nowide::ofstream file((boost::filesystem::path(data_dir()) / "cache" / "hints.cereal").string(),
                                 std::ios::binary);
...
TagCheckResult tag_check_mode(const std::string &tag)
{
    ...
    if (std::find(allowed_tags.begin(), allowed_tags.end(), tag) != allowed_tags.end())
    {
        ...
        else if (config_mode == ConfigOptionMode::comExpert)
...
{
    std::vector<std::string> allowed_tags = {"FFF", "MMU"};
    if (std::find(allowed_tags.begin(), allowed_tags.end(), tag) != allowed_tags.end())
    {
...
{
    if (const DSKY::Tab *tab = wxGetApp().get_tab(Preset::Type::TYPE_FILAMENT))
    {
...
// hints.ini names the settings tab a hint should open. The field was a bare enumerator ordinal,
// which silently pointed at a different tab whenever Preset::Type gained or lost a value; a name
// binds to the tab instead. An unrecognised name opens no tab and is counted rather than guessed at.
static Preset::Type settings_type_from_name(const std::string &name)
{
    if (name == "print")
    ...
    DBG_COUNT_LOAD("HINT_SETTINGS_TYPE_UNKNOWN");
...
void HintDatabase::load_hints_from_file(const boost::filesystem::path &path)
{
    ...
    catch (const boost::property_tree::ini_parser::ini_parser_error &err)
    {
        throw Luminary::RuntimeError(
            format("Failed loading hints file \"%1%\"\nError: \"%2%\" at line %3%", path, err.message(), err.line())
                .c_str());
    }
        ...
        if (boost::starts_with(section.first, "hint:"))
        {
            ...
            if (hypertext_start != std::string::npos)
            {
                ...
                if (fulltext.find(HYPERTEXT_MARKER_END) != std::string::npos)
                {
                    ...
                    BOOST_LOG_TRIVIAL(error)
                        << "Hint notification with multiple hypertext end markers: " << dict["text"];
                    continue;
                }
            ...
            if (dict.find("hypertext_type") != dict.end())
            {
                ...
                if (dict["hypertext_type"] == "link")
                {
                    ...
                    HintData hint_data{id_string,
                                       ...
                                       false,
                                       documentation_link,
                                       [hypertext_link]() { launch_browser_if_allowed(hypertext_link); }};
                ...
                else if (dict["hypertext_type"] == "settings")
                {
                    ...
                    Preset::Type type = settings_type_from_name(dict["hypertext_settings_type"]);
                    ...
                    HintData hint_data{id_string,
                                       ...
                                       [opt, type, category]()
                                       {
                                           if (type != Preset::TYPE_INVALID)
                                               DSKY::wxGetApp().jump_to_option(opt, type, category);
                                       }};
                    m_loaded_hints.emplace_back(hint_data);
                    // open preferences
                }
                else if (dict["hypertext_type"] == "preferences")
                {
                    ...
                    HintData hint_data{id_string,
                                       ...
                                       false,
                                       documentation_link,
                                       [page, item]() { wxGetApp().open_preferences(item, page); }};
                    m_loaded_hints.emplace_back(hint_data);
                }
                else if (dict["hypertext_type"] == "plater")
                {
                    ...
                    HintData hint_data{id_string,
                                       ...
                                       true,
                                       documentation_link,
                                       [item]() { wxGetApp().plater()->canvas3D()->highlight_toolbar_item(item); }};
                    m_loaded_hints.emplace_back(hint_data);
                }
                else if (dict["hypertext_type"] == "gizmo")
                {
                    ...
                    HintData hint_data{id_string,
                                       ...
                                       true,
                                       documentation_link,
                                       [item]() { wxGetApp().plater()->canvas3D()->highlight_gizmo(item); }};
                    m_loaded_hints.emplace_back(hint_data);
                }
            ...
            else
            {
                ...
                HintData hint_data{id_string,   text1,         weight,       was_displayed, hypertext_text,
                                   follow_text, disabled_tags, enabled_tags, false,         documentation_link};
                m_loaded_hints.emplace_back(hint_data);
            }
        }
    }
...
size_t HintDatabase::get_next()
{
    if (!m_sorted_hints)
    {
        auto compare_wieght = [](const HintData &a, const HintData &b)
        {
    ...
    for (size_t i = 0; i < m_loaded_hints.size(); i++)
    {
        if (!m_loaded_hints[i].was_displayed &&
            tags_check(m_loaded_hints[i].disabled_tags, m_loaded_hints[i].enabled_tags))
        {
...
void NotificationManager::HintNotification::render_text(const float win_size_x, const float win_size_y,
                                                        const float win_pos_x, const float win_pos_y)
{
    ...
    for (size_t i = 0; i < (m_multiline ? /*m_lines_count*/ m_endlines.size() : 2); i++)
    {
        ...
        if (m_endlines.size() > i && m_text1.size() >= m_endlines[i])
        {
            if (i == 1 && m_endlines.size() > 2 && !m_multiline)
            {
                ...
                line = m_text1.substr(m_endlines[0] +
    ...
    {
        render_hypertext(x_offset + ImGui::CalcTextSize((line + " ").c_str()).x, starting_y + shift_y, _u8L("More"),
                         true);
    }
...
void NotificationManager::HintNotification::render_close_button(const float win_size_x, const float win_size_y,
                                                                const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - win_size.x / 10.f, win_pos.y),
                                   ImVec2(win_pos.x, win_pos.y + win_size.y - 2 * m_line_height), true))
    {
...
void NotificationManager::HintNotification::render_preferences_button(const float win_pos_x, const float win_pos_y)
{
    ...
    //hover
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_window_width / 15.f,
                                          win_pos_y + m_window_height - 1.75f * m_line_height),
...
void NotificationManager::HintNotification::render_right_arrow_button(const float win_size_x, const float win_size_y,
                                                                      const float win_pos_x, const float win_pos_y)
{
    // Used for debuging
...
void NotificationManager::HintNotification::render_logo(const float win_size_x, const float win_size_y,
                                                        const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::HintNotification::render_documentation_button(const float win_size_x, const float win_size_y,
                                                                        const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
