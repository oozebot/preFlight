...
public:
    ...
    // Report which export destinations the slice-button dropdown currently offers,
    ...
    void SetPrinterWebViewName(const wxString &printerName); // Relabel the existing tab for a newly selected printer
...
private:
    ...
    void FitPrinterWebViewButton(); // Size the printer tab to its name, never narrower than a standard tab
