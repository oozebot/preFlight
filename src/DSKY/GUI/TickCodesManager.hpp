...
using namespace Luminary::CustomGCode;
namespace Luminary
{
class PrintObject;
class Print;
...
bool check_color_change(const Luminary::PrintObject *object, size_t frst_layer_id, size_t layers_cnt,
                        bool check_overhangs,
                        // what to do with detected color change
                        // and return true when detection have to be desturbed
                        std::function<bool(const Luminary::Layer *)> break_condition);
enum ConflictType
{
    ctNone,
    ctModeConflict,
...
struct ExtrudersSequence
{
    ...
    {
        return (other.is_mm_intervals == this->is_mm_intervals) && (other.interval_by_mm == this->interval_by_mm) &&
               (other.interval_by_layers == this->interval_by_layers) &&
               (other.random_sequence == this->random_sequence) && (other.color_repetition == this->color_repetition) &&
...
class TickCodeManager
{
    ...
    const Luminary::Print *m_print{nullptr};
...
public:
    ...
    std::string gcode(Luminary::CustomGCode::Type type);
    ...
    void set_print(const Luminary::Print &print)
    {
        if (!m_print)
            m_print = &print;
    }
