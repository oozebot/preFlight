...
#ifdef __WIN32__
// WebView2.h is a MIDL-generated header and relies on the COM base declarations.
#include <objbase.h>
#include "WebView2.h"
#include <wrl.h>
#include <atlbase.h>
...
template<typename T>
using wxCOMPtr = Microsoft::WRL::ComPtr<T>;
#include "GUI_App.hpp"
#include "format.hpp"
    ...
    if (FAILED(wv2_10->add_BasicAuthenticationRequested(
            Microsoft::WRL::Callback<ICoreWebView2BasicAuthenticationRequestedEventHandler>(
                [username, password](ICoreWebView2 *sender, ICoreWebView2BasicAuthenticationRequestedEventArgs *args)
                {
                    ...
                    if (FAILED(basicAuthenticationResponse->put_UserName(DSKY::from_u8(username).c_str())))
                    {
                        return -1;
                    }
                    if (FAILED(basicAuthenticationResponse->put_Password(DSKY::from_u8(password).c_str())))
                    {
                        return -1;
                    }
                    return 0;
...
void remove_webview_credentials(wxWebView *webview)
{
    ...
    if (auto it = g_basic_auth_handler_tokens.find(webView2); it != g_basic_auth_handler_tokens.end())
    {
...
void delete_cookies(wxWebView *webview, const std::string &url)
{
    ...
    */
    wxString parameters = DSKY::format_wxstr(L"{\"urls\": [\"%1%\"]}", url);
    webView2->CallDevToolsProtocolMethod(
        L"Network.getCookies", parameters.c_str(),
        Microsoft::WRL::Callback<ICoreWebView2CallDevToolsProtocolMethodCompletedHandler>(
            [webView2, url](HRESULT errorCode, LPCWSTR resultJson) -> HRESULT
            {
                ...
                try
                {
                    std::stringstream ss(DSKY::into_u8(resultJson));
                ...
                for (const auto &cookie : ptree.get_child("cookies"))
                {
                    ...
                    wxString name_and_domain = DSKY::format_wxstr(L"{\"name\": \"%1%\", \"domain\": \"%2%\"}", name,
                                                                  domain);
                    BOOST_LOG_TRIVIAL(debug) << "Deleting cookie: " << name_and_domain;
                    webView2->CallDevToolsProtocolMethod(
                        L"Network.deleteCookies", name_and_domain.c_str(),
                        Microsoft::WRL::Callback<ICoreWebView2CallDevToolsProtocolMethodCompletedHandler>(
                            [](HRESULT errorCode, LPCWSTR resultJson) -> HRESULT { return S_OK; })
                            .Get());
                }
                return S_OK;
            })
    ...
    if (FAILED(webView2->add_WebResourceRequested(
            Microsoft::WRL::Callback<ICoreWebView2WebResourceRequestedEventHandler>(
                [token](ICoreWebView2 *sender, ICoreWebView2WebResourceRequestedEventArgs *args)
                {
                    ...
                    hr = headers->SetHeader(L"Authorization", DSKY::from_u8(val).c_str());
...
void load_request(wxWebView *web_view, const std::string &address, const std::string &token)
{
    ...
    if (FAILED(webViewEnvironment2->CreateWebResourceRequest(L"https://www.printables.com/", L"GET", NULL,
...
{
    // unused on Windows: WebView2 keeps the control alive across a content process failure
}
} // namespace DSKY
#endif // WIN32
