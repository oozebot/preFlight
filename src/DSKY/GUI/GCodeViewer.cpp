...
void GCodeViewer::SequentialView::Marker::render_position_window(const libvgcode::Viewer *viewer)
{
    ...
    else
    {
        ImGuiWrapper &imgui = *m_imgui;
        const Size cnv_size = m_canvas->get_canvas_size();
        // Centered on the scene beside the Preview's strip, like the moves slider above it.
        ImGuiPureWrap::set_next_window_pos(0.5f *
                                               (m_canvas->get_left_inset() + static_cast<float>(cnv_size.get_width()) -
                                                m_canvas->get_right_inset()),
...
void GCodeViewer::SequentialView::GCodeWindow::render(float top, float bottom, size_t curr_line_id, float legend_width)
{
    ...
    // Below the legend, on the legend's side of the canvas.
    const bool strip_on_right = wxGetApp().legacy_prepare_layout();
    ImGuiPureWrap::set_next_window_pos(strip_on_right ? ImGui::GetIO().DisplaySize.x : 0.0f, top, ImGuiCond_Always,
                                       strip_on_right ? 1.0f : 0.0f, 0.0f);
                                ...
                                const std::vector<std::string> &str_color_print_colors)
{
    ...
    if (is_editor())
    {
        m_contained_in_bed = m_get_build_volume
                                 ...
                                 : true;
        // An empty load has no box to judge; only real toolpaths can lie outside the bed.
        if (!m_contained_in_bed && m_paths_bounding_box.defined)
        {
    ...
    // This shows "Slicing finished. Export G-Code." with the export hyperlink, offered in the
    // Preview whichever panel holds the strip beside the scene.
...
void GCodeViewer::load_as_preview(libvgcode::GCodeInputData &&data)
{
    ...
    m_contained_in_bed = m_get_build_volume
                             ...
                             : true;
    if (!m_contained_in_bed && paths_bounding_box.defined)
    {
        s_print_statuses[s_multiple_beds.get_active_bed()] = PrintStatus::toolpath_outside;
    }
}
...
void GCodeViewer::render()
{
    ...
    // Render the clipping plane ImGui overlay if active
    ...
    if (m_viewer.get_layers_count() > 0)
    {
        ...
        // The G-code window yields its space to the sidebar; a negative top hides it, as a collapsed legend does.
        const bool sidebar_out = m_is_sidebar_collapsed && !m_is_sidebar_collapsed();
        m_sequential_view.render(sidebar_out ? -1.0f : legend_height, &m_viewer, gcode_id_to_use, get_camera(),
                                 get_shader("tool_marker"), m_legend_width);
    }
...
public:
    ...
    void export_to(const std::string &filename)
    {
        ...
        fprintf(f_mat.f, "# Generated by %s-%s\n", PREFLIGHT_APP_NAME, PREFLIGHT_VERSION);
...
void GCodeViewer::render_segments_for_pass(const Matrix4d &view_world, const Matrix4d &projection,
                                           const Vec3d &camera_pos_world, bool gbuffer)
{
    const Vec3d bed_offset = s_multiple_beds.get_bed_translation(s_multiple_beds.get_active_bed());
    const Matrix4d view_local = view_world * Transform3d(Eigen::Translation3d(bed_offset)).matrix();
    const Vec3d cam_local = camera_pos_world - bed_offset;
    const libvgcode::Mat4x4 v = libvgcode::convert(static_cast<Matrix4f>(view_local.cast<float>()));
    const libvgcode::Mat4x4 p = libvgcode::convert(static_cast<Matrix4f>(projection.cast<float>()));
    const libvgcode::Vec3 c = {float(cam_local.x()), float(cam_local.y()), float(cam_local.z())};
    m_viewer.render_segments_pass(v, p, c, gbuffer);
}
...
void GCodeViewer::render_shells()
{
    ...
    // When the preview clip controller is active, render only the clipped object shell
...
// While the slice has no toolpaths the legend shows as its folded title bar, so the sidebar toggle beside it
// keeps its anchor. It takes no input and cannot be opened; it has its own window id so the real legend keeps
// its folded state.
void GCodeViewer::render_legend_pending()
{
    m_legend_width = 0.0f;
    if (!is_legend_shown() || !m_canvas || !get_imgui() || !wxGetApp().is_editor())
        return;

    const Size cnv_size = m_canvas->get_canvas_size();
    const bool strip_on_right = wxGetApp().legacy_prepare_layout();
    ImGuiPureWrap::set_next_window_pos(strip_on_right ? static_cast<float>(cnv_size.get_width()) : 0.0f, 0.0f,
                                       ImGuiCond_Always, strip_on_right ? 1.0f : 0.0f, 0.0f);
...
#if ENABLE_RETINA_GL
    em *= m_canvas->get_canvas_size().get_scale_factor();
#endif
    const float sidebar_width = 45.0f * em;
    ImGui::SetNextWindowSizeConstraints({sidebar_width, 0.0f}, {sidebar_width, sidebar_width});
    ImFont *legend_font = get_imgui()->get_legend_font();
    if (legend_font)
        ImGui::PushFont(legend_font);
    ImGuiPureWrap::begin(std::string("Legend###LegendPending"),
...
void GCodeViewer::render_legend(float &legend_height)
{
    ...
    // The legend owns the strip on the sidebar's side: left, or right in the legacy layout.
    const bool strip_on_right = wxGetApp().legacy_prepare_layout();
    ImGuiPureWrap::set_next_window_pos(strip_on_right ? static_cast<float>(cnv_size.get_width()) : 0.0f, 0.0f,
                                       ImGuiCond_Always, strip_on_right ? 1.0f : 0.0f, 0.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
    ...
    // em_unit() is in logical pixels; ImGui operates in physical pixels on
    ...
    if (legend_collapsed != m_legend_collapsed)
    {
        // The scene viewport for this frame was set before the legend rendered; the next frame reframes it.
        m_legend_collapsed = legend_collapsed;
    ...
    // Band-based legend rendering: shows actual data ranges instead of
    ...
    // Band-based time legend rendering
        ...
        [this](unsigned char extruder_id, const std::vector<CustomGCode::Item> &custom_gcode_per_print_z)
    {
        ...
        for (const auto &item : custom_gcode_per_print_z)
        {
            ...
            auto lower_b = std::lower_bound(zs.begin(), zs.end(),
                                            static_cast<float>(item.print_z - Luminary::CustomGCode::epsilon()));
            if (lower_b == zs.end())
                continue;
    ...
    if (!view_type_changed)
    {
        ...
        {
            // The base color of an extruder before any color change is its filament color; the
            // palette holds the extruders first, then the color-change colors.
            auto base_color = [this](size_t extruder_id) -> ColorRGBA
            {
                const libvgcode::Palette &palette = m_viewer.get_color_print_colors();
                if (extruder_id < palette.size())
                    return libvgcode::convert(palette[extruder_id]);
                if (extruder_id < m_viewer.get_tool_colors().size())
                    return libvgcode::convert(m_viewer.get_tool_colors()[extruder_id]);
                return ColorRGBA(0.25f, 0.25f, 0.25f, 1.0f);
            };
            ...
            { // single extruder use case
                ...
                if (items_cnt == 0) // There are no color changes, but there are some pause print or custom Gcode
                    append_item(EItemType::Rect, base_color(0), _u8L("Default color"));
                else
                {
                    for (int i = items_cnt; i >= 0; --i)
                    {
                        ...
                        if (i == 0)
                        {
                            append_item(EItemType::Rect, base_color(0), upto_label(cp_values.front().second.first));
                            break;
                        }
                        else if (i == items_cnt)
                        {
            ...
            { // multi extruder use case
                ...
                for (uint8_t extruder_id : used_extruders_ids)
                {
                    ...
                    if (items_cnt == 0)
                        ...
                        append_item(EItemType::Rect, base_color(extruder_id),
                    ...
                    else
                    {
                        for (int j = items_cnt; j >= 0; --j)
                        {
                            ...
                            if (j == 0)
                            {
                                ...
                                append_item(EItemType::Rect, base_color(extruder_id), label);
                                break;
                            }
                            else if (j == items_cnt)
                            {
        ...
        if (!m_job_estimate.per_extruder.empty())
        {
            ...
            if (ImGui::BeginTable("JobEstFilament", num_cols, ImGuiTableFlags_SizingStretchProp))
            {
                ...
                if (show_cost)
                {
                    ...
                    ImGui::TableNextRow();
                    // Subtle translucent emphasis band on the Total row instead of a solid header
    ...
    // When the legend content changes size (e.g. switching view types), ImGui needs several
    // frames to recalculate the window layout. On any size transition, allow up to 5 extra
    // frames for the layout to settle. A bounded counter rather than "request while dirty",
    // because the fixed-width constraint fights AlwaysAutoResize and never settles.
