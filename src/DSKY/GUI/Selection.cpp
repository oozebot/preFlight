...
// The scale UI (uniform-scale arrows + the scale plane) uses the theme's secondary accent so it stays
// distinct from the selected object (primary accent). Read at runtime, so it follows the active theme.
static Luminary::ColorRGBA scale_accent_color()
{
    const wxColour &c = DSKY::active_palette().accent_secondary;
    return {c.Red() / 255.0f, c.Green() / 255.0f, c.Blue() / 255.0f, 1.0f};
}
static const Luminary::ColorRGBA TRANSPARENT_PLANE_COLOR = {0.8f, 0.8f, 0.8f, 0.5f};
...
void Selection::clear()
{
    ...
    // Fake a KillFocus from the sidebar.
    if (m_canvas)
...
void Selection::flattening_rotate(const Vec3d &normal)
{
    ...
    assert(Luminary::is_approx(normal.norm(), 1.));
...
void Selection::scale_to_fit_print_volume(const BuildVolume &volume)
{
    ...
    auto fit_circle = [this, fit](const BuildVolume &volume, bool undoredo_snapshot, double *max_height = nullptr)
    {
        ...
        for (unsigned int i : m_list)
        {
            ...
            const Luminary::Polygon hull_2d = hull_3d.convex_hull();
    ...
    // The fit uses the bounding box of the whole selection, including modifiers, support enforcers and
    // blockers and negative meshes.
        ...
        (is_single_volume_or_modifier() && !is_local_coordinates()))
    {
        ...
        if (needs_baking)
        {
            MessageDialog dlg(
                ...
                PREFLIGHT_APP_NAME, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION);
...
void Selection::erase()
{
    ...
    else
    {
        ...
        for (unsigned int i : m_list)
        {
            ...
            // Temporary meshes are not managed by the object list.
            if (v->volume_idx() >= 0)
...
void Selection::update_type()
{
    ...
    else
    {
        ...
        {
            unsigned int aux_volumes_count = 0;
            // Selected volumes with no ModelVolume behind them (negative volume index), counted per object
            // rather than per instance like model_volumes_count.
            for (unsigned int i : m_list)
            {
            ...
            if (m_cache.content.size() == 1) // single object
            {
                ...
                if (model_volumes_count * instances_count + aux_volumes_count == (unsigned int) m_list.size())
                {
                    m_type = SingleFullObject;
                    // ensures the correct mode is selected
                    m_mode = Instance;
                ...
                {
                    if (model_volumes_count + aux_volumes_count == (unsigned int) m_list.size())
                    {
                        m_type = SingleFullInstance;
                        // ensures the correct mode is selected
                        m_mode = Instance;
                    ...
                    else
                    {
                        ...
                        requires_disable = true;
                         ...
                         selected_instances_count * model_volumes_count + aux_volumes_count ==
            ...
            else
            {
                ...
                if (sels_cntr + aux_volumes_count == (unsigned int) m_list.size())
                {
