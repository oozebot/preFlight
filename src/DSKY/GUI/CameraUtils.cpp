...
#include "DSKY/GUI/3DScene.hpp" // GLVolume
...
{
    Vec4i viewport(Eigen::Map<const Eigen::Matrix<int, 4, 1>>(camera.get_viewport().data()).cast<coord_t>().eval());
    ...
    igl::project(pts, camera.get_view_matrix().matrix(), camera.get_projection_matrix().matrix(), viewport,
                 projections);
...
Luminary::Polygon CameraUtils::create_hull2d(const Camera &camera, const GLVolume &volume)
{
    ...
    const Transform3d &trafoMat = volume.get_instance_transformation().get_matrix() *
