...
void PhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup *m_optgroup)
{
    m_optgroup->on_change = [this](t_config_option_key opt_key, boost::any value)
    {
        if (opt_key == "host_type" || opt_key == "printhost_authorization_type")
            this->update();
        if (opt_key == "print_host" || opt_key == "print_host_webui")
    ...
    auto printhost_browse = [=](wxWindow *parent)
    {
        ...
        m_printhost_browse_btn->Bind(wxEVT_BUTTON,
                                     [=](wxCommandEvent &e)
                                     {
                                         // The host type decides which mDNS service the lookup asks for
                                         std::unique_ptr<PrintHost> host(PrintHost::get_print_host(m_config));
                                         BonjourDialog dialog(this, Preset::printer_technology(m_printer.config),
                                                              host ? host->discovery_service() : "octoprint");
    ...
    auto webui_test = [this, create_sizer_with_btn](wxWindow *parent)
    {
        auto sizer = create_sizer_with_btn(parent, &m_webui_test_btn, "test", _L("Test"));
        m_webui_test_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { this->test_web_interface(); });
        return sizer;
    };
    ...
    if (is_secret_store_ok())
    {
        ...
        line.widget = [ca_file_hint](wxWindow *parent)
        {
            wxString info = DSKY::format_wxstr(
    ...
    else
    {
        ...
        line.widget = [ca_file_hint](wxWindow *parent)
        {
            wxString info = DSKY::format_wxstr(
                L"%1%:\n\t%2%\n\t%3%\n", _L("Storing passwords"),
                DSKY::format_wxstr(_L("On this system, %1% cannot access the system password store."),
                                   PREFLIGHT_APP_NAME),
    ...
    // Both address fields drop the leading and trailing spaces a paste brings along
    for (const char *address_key : {"print_host", "print_host_webui"})
    {
        Field *printhost_field = m_optgroup->get_field(address_key);
        if (!printhost_field)
            continue;
...
void PhysicalPrinterDialog::update_printhost_buttons()
{
    ...
    m_webui_test_btn->Enable(!m_config->opt_string("print_host_webui").empty());
}

// The web interface is a page rather than an API, so the test is a GET that has to return a page
void PhysicalPrinterDialog::test_web_interface()
{
    std::string url = m_config->opt_string("print_host_webui");
    ...
    wxString detail;
    {
        // The request is synchronous, so the busy cursor covers the wait
        wxBusyCursor wait;
        auto http = Http::get(std::move(url));
        if (const std::string cafile = m_config->opt_string("printhost_cafile"); !cafile.empty())
            http.ca_file(cafile);
#ifdef _WIN32
        http.ssl_revoke_best_effort(m_config->opt_bool("printhost_ssl_ignore_revoke"));
#endif
        http.on_complete([&](std::string, unsigned) { ok = true; })
            .on_error([&](std::string, std::string error, unsigned status)
                      { detail = status != 0 ? format_wxstr("HTTP %1%", status) : from_u8(error); })
            .perform_sync();
    }

    if (ok)
        show_info(this, _L("Connection to the web interface works correctly."), _L("Success!"));
    else
        show_error(this, format_wxstr("%1%: %2%", _L("Could not connect to the web interface"), detail));
}
...
void PhysicalPrinterDialog::update(bool printer_change)
{
    ...
    if (tech == ptFFF)
    {
        ...
        else
        {
            ...
            has_web_interface = opt && opt->value == htMoonraker;
        }
    ...
    m_optgroup->show_field("print_host_webui", has_web_interface);
    m_webui_test_btn->Show(has_web_interface);
...
void PhysicalPrinterDialog::update_host_type(bool printer_change)
{
    ...
    // so the mapping does not depend on the order of the host_type enum.
