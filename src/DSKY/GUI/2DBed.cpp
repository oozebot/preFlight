...
void Bed_2D::repaint(const std::vector<Vec2d> &shape)
{
    ...
    if (m_user_drawn_background)
    {
        ...
        // Themed canvas surround so the bed preview matches the active theme.
    ...
    auto bed_polygon = Luminary::Polygon::new_scale(shape);
    ...
    // draw bed fill - themed build-plate surface
    ...
    {
        polylines.push_back(Luminary::Polyline::new_scale({Vec2d(x, bb.min(1)), Vec2d(x, bb.max(1))}));
    }
    for (auto y = bb.min(1) - fmod(bb.min(1), step) + step; y < bb.max(1); y += step)
    {
        polylines.push_back(Luminary::Polyline::new_scale({Vec2d(bb.min(0), y), Vec2d(bb.max(0), y)}));
    }
    polylines = intersection_pl(polylines, bed_polygon);

    dc.SetPen(wxPen(UIColors::HeaderDivider(), 1, wxPENSTYLE_SOLID)); // themed grid lines
    ...
    // draw bed contour - themed outline
