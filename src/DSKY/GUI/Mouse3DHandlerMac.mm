...
static DSKY::Mouse3DController* mouse_3d_controller = nullptr;
    ...
    if (s->client == clientID) {
      switch (s->command) {
        case kConnexionCmdHandleAxis: {
          ...
          DSKY::Mouse3DController::DataPacketAxis packet;
          for (int i = 0; i < 6; ++ i)
...
void Mouse3DController::init()
{
  ...
  if (load_driver_functions()) {
    ...
    if (! error) {
      ...
      // The registered name must equal the executable name; on macOS the target is built as "preFlight".
      clientID = RegisterConnexionClient(
          0, "\011preFlight", kConnexionClientModeTakeOver, kConnexionMaskAxis);
