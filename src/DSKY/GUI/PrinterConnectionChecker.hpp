...
public:
    ...
    // A different printer resets the state to Unknown (the callback fires) and, while polling,
    // checks it immediately instead of waiting for the next timer tick
...
private:
    ...
    void PerformCheck();
    void OnCheckComplete(bool success, unsigned generation);
    ...
    // Incremented when the printer changes; a check that started under an older value is discarded
