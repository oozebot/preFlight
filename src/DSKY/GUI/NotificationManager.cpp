...
#include "DSKY/Utils/PrintHost.hpp" // IWYU pragma: keep
#include "luminary/config/model/Config.hpp"
                                                  ...
                                                  float overlay_width)
{
    ...
    // Centered beside the strip the Preview reserves for the legend or the sidebar, like the scene.
    ImVec2 win_pos(0.5f * (canvas.get_left_inset() + (float) cnv_size.get_width() - canvas.get_right_inset()),
...
bool NotificationManager::PopNotification::push_background_color()
{
    ...
    if (m_data.level == NotificationLevel::ErrorNotificationLevel)
    {
        ...
        const wxColour &c = DSKY::active_palette().error; // themed error red, blended over the window bg
    ...
    {
        ImVec4 backcolor = ImGui::GetStyleColorVec4(ImGuiCol_WindowBg);
        const wxColour &c = DSKY::active_palette().warning; // themed warning amber, blended over the window bg
        backcolor.x += (c.Red() / 255.0f - backcolor.x) * 0.55f;
        backcolor.y += (c.Green() / 255.0f - backcolor.y) * 0.55f;
        backcolor.z += (c.Blue() / 255.0f - backcolor.z) * 0.55f;
        push_style_color(ImGuiCol_WindowBg, backcolor, m_state == EState::FadingOut, m_current_fade_opacity);
                                                                          ...
                                                                          const float win_pos_y)
{
                                   ...
                                   ImVec2(win_pos.x - m_line_height * 2.5f, win_pos.y + win_size.y), true))
    {
        ...
        if (m_hover_time > 0 && m_hover_time < time_now)
        {
            ...
            ImGui::BeginTooltip();
            ImGuiPureWrap::text(_u8L("Eject drive") + " " + DSKY::shortkey_ctrl_prefix() + "T");
            ImGui::EndTooltip();
            ImGui::PopStyleColor();
                                                                  ...
                                                                  const float win_pos_x, const float win_pos_y)
{
    ...
    switch (m_uj_state)
    {
    case DSKY::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_PROGRESS:
    {
        ProgressBarNotification::render_bar(win_size_x, win_size_y, win_pos_x, win_pos_y);
    ...
    case DSKY::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_RESOLVING:
        text = _u8L("RESOLVING ADDRESS");
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / 2 + win_size_y / 6 - (m_multiline ? m_line_height / 4 : m_line_height / 2));
        break;
    case DSKY::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_ERROR:
        text = _u8L("ERROR");
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / 2 + win_size_y / 6 - (m_multiline ? m_line_height / 4 : m_line_height / 2));
        break;
    case DSKY::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_CANCELLED:
        text = _u8L("CANCELED");
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / 2 + win_size_y / 6 - (m_multiline ? m_line_height / 4 : m_line_height / 2));
        break;
    case DSKY::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_COMPLETED:
    case DSKY::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_COMPLETED_WITH_WARNING:
        break;
    }
                                                                            ...
                                                                            const float win_pos_y)
{
                                   ...
                                   true))
    {
        ...
        if (m_hover_time > 0 && m_hover_time < time_now)
        {
            ImGui::PushStyleColor(ImGuiCol_PopupBg, ImGuiPureWrap::COL_WINDOW_BACKGROUND);
            ImGui::BeginTooltip();
            ImGuiPureWrap::text(_u8L("Cancel upload") + " " + DSKY::shortkey_ctrl_prefix() + "T");
            ImGui::EndTooltip();
            ImGui::PopStyleColor();
        }
        if (m_hover_time == 0)
    ...
    NotificationManager::SlicingProgressNotification::SlicingProgressState state, float percent /* = 0.f*/)
{
    switch (state)
    {
    case DSKY::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_NO_SLICING:
    case DSKY::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_BEGAN:
        ...
        m_sp_state = state;
        return true;
    case DSKY::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_PROGRESS:
        ...
        m_sp_state = state;
        return true;
    case DSKY::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_RENDERING:
        ...
        m_sp_state = state;
        return true;
    case DSKY::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_CANCELLED:
        set_percentage(-1);
        m_has_cancel_button = false;
        m_has_print_info = false;
        set_export_possible(false);
        m_sp_state = state;
        return true;
    case DSKY::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_COMPLETED:
        if (m_sp_state != SlicingProgressState::SP_BEGAN && m_sp_state != SlicingProgressState::SP_PROGRESS &&
...
void NotificationManager::ProgressIndicatorNotification::init()
{
    ...
    {
    case DSKY::NotificationManager::ProgressIndicatorNotification::ProgressIndicatorState::PIS_HIDDEN:
        m_state = EState::Hidden;
        break;
    case DSKY::NotificationManager::ProgressIndicatorNotification::ProgressIndicatorState::PIS_PROGRESS_REQUEST:
    case DSKY::NotificationManager::ProgressIndicatorNotification::ProgressIndicatorState::PIS_PROGRESS_UPDATED:
        m_state = EState::NotFading;
        break;
    case DSKY::NotificationManager::ProgressIndicatorNotification::ProgressIndicatorState::PIS_COMPLETED:
...
bool NotificationManager::update_notifications(GLCanvas3D &canvas)
{
    ...
    // Only schedule future frames (ms > 0) for notification animations.
