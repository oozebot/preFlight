...
double calc_line_height_in_mm(const Luminary::Emboss::FontFile &ff, const FontProp &fp); // return lineheight in mm

// Be careful it is not water tide and contain self intersections
// It is only for visualization purposes
indexed_triangle_set its_create_torus(const Luminary::Polygon &polygon, float radius, size_t steps = 20)
{
    ...
    auto calc_line_norm = [](const Vec2f &f, const Vec2f &s) -> Vec2f
    {
...
TextLines select_closest_contour(const std::vector<Polygons> &line_contours)
{
    ...
    for (const Polygons &polygons : line_contours)
    {
        ...
        // Luminary::Polygons polygons = union_(polygons);
        // std::vector<Luminary::Line> lines = to_lines(polygons);
        ...
        const Luminary::Polygon &polygon = index.is_contour()
...
{
    return Eigen::AngleAxis(-M_PI_2, Vec3d::UnitX());
}
...
indexed_triangle_set create_its(const TextLines &lines, float radius)
{
    ...
    {
        const Luminary::Polygon &polygon = line.polygon;
        if (polygon.empty())
            continue;
...
GLModel::Geometry create_geometry(const TextLines &lines, float radius, bool is_mirrored)
{
    ...
    geometry.format = {GLModel::Geometry::EPrimitiveType::Triangles, DSKY::GLModel::Geometry::EVertexLayout::P3};
                          ...
                          /*const*/ Emboss::StyleManager &style_manager, unsigned count_lines)
{
    ...
    m_lines = Luminary::Emboss::create_text_lines(text_tr, volumes_to_slice, ff, fp, count_lines, &line_height_mm);
    if (m_lines.empty())
        return;
...
namespace
{
double calc_line_height_in_mm(const Luminary::Emboss::FontFile &ff, const FontProp &fp)
{
    int line_height = Luminary::Emboss::get_line_height(ff, fp); // In shape size
    double scale = Luminary::Emboss::get_text_shape_scale(fp, ff);
    return line_height * scale;
}
} // namespace

Luminary::Emboss::TextLines Luminary::Emboss::create_text_lines(const Transform3d &text_tr,
                                                                const ModelVolumePtrs &volumes_to_slice,
                                                                const FontFile &ff, const FontProp &fp,
                                                                unsigned count_lines, double *line_height_mm_ptr)
{
    ...
    for (const ModelVolume *volume : volumes_to_slice)
    {
        ...
        for (size_t i = 0; i < count_lines; ++i)
        {
            const Polygons polys = Luminary::slice_mesh(volume->mesh().its, line_centers[i], slicing_params);
            if (polys.empty())
                continue;
    ...
    for (Polygons &contours : line_contours)
    {
        ...
        for (const ModelVolume *volume : volumes_to_slice)
        {
            MeshSlicingParams slicing_params;
            slicing_params.trafo = c_trafo_inv * volume->get_matrix();
            const Polygons polys = Luminary::slice_mesh(volume->mesh().its, line_center, slicing_params);
            if (polys.empty())
                continue;
            contours.insert(contours.end(), polys.begin(), polys.end());
        }
