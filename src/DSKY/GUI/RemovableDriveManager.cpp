    ...
    for (char drive = 'C'; drive <= 'Z'; ++drive, drives_mask >>= 1)
        if (drives_mask & 1)
        {
            ...
            if (drive_type == DRIVE_REMOVABLE)
            {
                ...
                BOOL error = ::GetVolumeInformationW(wpath.c_str(), volume_name.data(), sizeof(volume_name), nullptr,
                                                     nullptr, nullptr, file_system_name.data(),
...
bool eject_inner(const std::string &path)
{
    ...
    if (!SUCCEEDED(hr))
    {
        BOOST_LOG_TRIVIAL(error) << DSKY::format("Ejecting of %1% has failed: Attempt to get Shell pointer has failed.",
                                                 path);
        CoUninitialize();
        return false;
    }
    ...
    if (!SUCCEEDED(hr))
    {
        BOOST_LOG_TRIVIAL(error) << DSKY::format("Ejecting of %1% has failed: Attempt to create Namespace has failed.",
                                                 path);
        CoUninitialize();
        return false;
    }
    ...
    if (!SUCCEEDED(hr))
    {
        BOOST_LOG_TRIVIAL(error) << DSKY::format("Ejecting of %1% has failed: Attempt to Parse name has failed.", path);
        CoUninitialize();
        return false;
    }
    VARIANT vtEject;
    ...
    if (!SUCCEEDED(hr))
    {
        BOOST_LOG_TRIVIAL(error) << DSKY::format("Ejecting of %1% has failed: Attempt to Invoke Verb has failed.",
                                                 path);
            ...
            if (m_callback_evt_handler)
                wxPostEvent(m_callback_evt_handler,
                            RemovableDriveEjectEvent(EVT_REMOVABLE_DRIVE_EJECTED,
                                                     std::pair<DriveData, bool>(std::move(*it_drive_data), true)));
        }
        else
        {
            // failed to eject
            ...
            if (m_callback_evt_handler)
                wxPostEvent(m_callback_evt_handler,
                            RemovableDriveEjectEvent(EVT_REMOVABLE_DRIVE_EJECTED,
                                                     std::pair<DriveData, bool>(*it_drive_data, false)));
        }
    }
    else
    {
        ...
        assert(m_callback_evt_handler);
        if (m_callback_evt_handler)
            wxPostEvent(m_callback_evt_handler, RemovableDriveEjectEvent(EVT_REMOVABLE_DRIVE_EJECTED,
                                                                         std::pair<DriveData, bool>({"", ""}, false)));
    }
}
...
void RemovableDriveManager::eject_drive()
{
    ...
    // The wait blocks the main thread from pumping Cocoa messages, so a slow diskutil eject can stall
...
#if __APPLE__
    // On Apple, run the eject asynchronously on a worker thread.
    m_eject_thread = new boost::thread(
        [this, correct_path, drive_data]()
#endif
        {
...
#if __APPLE__
            auto exe_path = bp2::environment::find_executable("diskutil");
            std::vector<std::string> args = {"eject", correct_path};
#else
        auto exe_path = bp2::environment::find_executable("umount");
        std::vector<std::string> args = {correct_path};
#endif

            bp2::process child(ctx, exe_path, args, bp2::process_stdio{{}, out_pipe, out_pipe});
            ...
            while (true)
            {
                size_t n = out_pipe.read_some(asio::buffer(buffer), read_ec);
                if (read_ec || n == 0)
                    break;
            ...
            int err = child.wait(wait_ec);
            bool success = false;
            if (wait_ec)
            {
                // The wait call can fail during drive ejection
                ...
                BOOST_LOG_TRIVIAL(error)
                    << "boost::process::v2::process::wait() failed during Ejection. State of Ejection is unknown. Error code: "
                    << wait_ec.value();
            }
            else
            {
...
{
    return Luminary::binary_find_by_predicate(
        m_current_drives.begin(), m_current_drives.end(),
