...
class OptionsSearcher
{
    ...
    void sort_options()
    {
        std::sort(options.begin(), options.end(),
                  [](const Option &o1, const Option &o2) { return o1.label < o2.label; });
    }
    void sort_found()
    {
...
public:
    ...
    void append_preferences_option(const DSKY::Line &opt_line);
    void append_preferences_options(const std::vector<DSKY::Line> &opt_lines);
    ...
    const GroupAndCategory &get_group_and_category(const std::string &opt_key)
    {
        return groups_and_categories[opt_key];
    }
    ...
    SearchDialog *get_dialog() const { return search_dialog; }
};

//------------------------------------------
//          SearchDialog
//------------------------------------------
class SearchDialog : public DSKY::DPIDialog
{
    wxString search_str;

    // Filter input within the dialog itself
    TextInput *m_filter_input{nullptr};

    // Owner-drawn results panel with a custom ScrollBar
