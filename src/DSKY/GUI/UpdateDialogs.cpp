                                 ...
                                 PREFLIGHT_APP_NAME))
{
    ...
    // Release notes section - a ScrollablePanel gives a themed scrollbar
...
AppUpdateDownloadDialog::AppUpdateDownloadDialog(const Semver &ver_online, boost::filesystem::path &path)
    : MsgDialog(nullptr, _L("App Update download"),
                format_wxstr(_L("New version of %1% is available."), PREFLIGHT_APP_NAME))
{
    ...
    content_sizer->Add(versions);
    content_sizer->AddSpacer(GetScaledVertSpacing());
    // No installer yet, just a portable zip, so the "run installer" checkbox stays hidden
    // #ifndef __linux__
    ...
    content_sizer->AddSpacer(GetScaledVertSpacing());
    txtctrl_path = new wxTextCtrl(this, wxID_ANY, DSKY::format_wxstr(path.parent_path().string()));
    filename = DSKY::format_wxstr(path.filename().string());
    ...
    btn->Bind(wxEVT_BUTTON, (
                                [this, path](wxCommandEvent &e)
                                {
                                    ...
                                    if (!extension.empty())
                                    {
                                        ...
                                        wildcard = DSKY::format_wxstr("%1% Files (*.%2%)|*.%2%", wxext.Upper(), wxext);
                                    }
                                    boost::system::error_code ec;
                                    boost::filesystem::path dir = boost::filesystem::absolute(
                                        boost::filesystem::path(DSKY::format(txtctrl_path->GetValue())), ec);
                                    if (ec)
                                        dir = DSKY::format(txtctrl_path->GetValue());
                                    wxDirDialog save_dlg(this, _L("Select directory") + ":",
                                                         DSKY::format_wxstr(dir.string())
                                                         /*
    ...
    if (auto *btn_ok = get_button(wxID_OK); btn_ok != NULL)
    {
        ...
        btn_ok->Bind(
            ...
            (
                ...
                {
                    boost::system::error_code ec;
                    std::string input = DSKY::into_u8(txtctrl_path->GetValue());
                    ...
                    boost::filesystem::path path = dir / DSKY::format(filename);
                    ec.clear();
                    if (dir.string().empty())
                    {
                    ...
                    if (!boost::filesystem::exists(dir, ec) || !boost::filesystem::is_directory(dir, ec) || ec)
                    {
                        ...
                        MessageDialog msgdlg(
                            nullptr,
                            DSKY::format_wxstr(_L("Directory %1% doesn't exists. Do you wish to create it?"),
                                               dir.string()),
                            _L("Notice"), wxYES_NO);
                    ...
                    if (show_change)
                    {
                        MessageDialog msgdlg(nullptr,
                                             DSKY::format_wxstr(_L("Download path is %1%. Do you wish to continue?"),
                                                                path.string()),
                                             _L("Notice"), wxYES_NO);
                        if (msgdlg.ShowModal() != wxID_YES)
                            return;
...
{
    // No installer yet, so there is no cbox_run to read; always return false
    // #ifndef __linux__
...
boost::filesystem::path AppUpdateDownloadDialog::get_download_path() const
{
    ...
    if (ec)
        dir = boost::filesystem::path(input);
    return dir / DSKY::format(filename);
}
    ...
    : MsgDialog(nullptr,
                ...
                update_params == PresetUpdater::UpdateParams::FORCED_BEFORE_WIZARD
                    ? _L("preFlight is not using the newest configuration available.\n"
                         "Configuration Wizard may not offer the latest printers and filaments to be installed.")
...
MsgUpdateForced::MsgUpdateForced(const std::vector<Update> &updates)
    : MsgDialog(nullptr, wxString::Format(_(L("%s incompatibility")), PREFLIGHT_APP_NAME),
                _(L("You must install a configuration update.")) + " ", wxOK | wxICON_ERROR)
{
    ...
    add_button(wxID_EXIT, false, wxString::Format(_L("Exit %s"), PREFLIGHT_APP_NAME));
...
MsgDataIncompatible::MsgDataIncompatible(const std::unordered_map<std::string, wxString> &incompats)
    : MsgDialog(nullptr, wxString::Format(_(L("%s incompatibility")), PREFLIGHT_APP_NAME),
                wxString::Format(_(L("%s configuration is incompatible")), PREFLIGHT_APP_NAME), wxICON_ERROR)
{
    auto *text = new wxStaticText(
        this, wxID_ANY,
        wxString::Format(
            _(L("This version of %s is not compatible with currently installed configuration bundles.\n"
                ...
                "\n",
            PREFLIGHT_APP_NAME, PREFLIGHT_APP_NAME, PREFLIGHT_APP_NAME, PREFLIGHT_APP_NAME));
    text->Wrap(CONTENT_WIDTH * wxGetApp().em_unit());
    content_sizer->Add(text);

    auto *text2 = new wxStaticText(this, wxID_ANY,
                                   wxString::Format(_(L("This %s version: %s")), PREFLIGHT_APP_NAME,
                                                    PREFLIGHT_VERSION));
...
{
    //TRN %1% is preFlight
    auto *text = new wxStaticText(this, wxID_ANY, format_wxstr(_L("Your %1% is up to date."), PREFLIGHT_APP_NAME));
    text->Wrap(CONTENT_WIDTH * wxGetApp().em_unit());
    content_sizer->Add(text);
    content_sizer->AddSpacer(GetScaledVertSpacing());
