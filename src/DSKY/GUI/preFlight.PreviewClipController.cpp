...
void PreviewClipController::activate(int object_id)
{
    ...
    // Center the view and the orbit pivot on the object, so orbiting turns around the cut being inspected.
    if (m_object_bbox.defined)
        wxGetApp().plater()->get_current_canvas3D()->animate_pivot_to(m_object_bbox.center());
}
