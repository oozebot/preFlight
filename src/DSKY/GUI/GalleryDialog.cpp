...
GalleryDialog::GalleryDialog(wxWindow *parent)
    : DPIDialog(parent, wxID_ANY, _L("Shape Gallery"), wxDefaultPosition, wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    m_list_ctrl = new wxListCtrl(this, wxID_ANY, wxDefaultPosition,
                                 wxSize(50 * wxGetApp().em_unit(), 35 * wxGetApp().em_unit()),
    ...
    m_list_ctrl->Bind(wxEVT_LIST_ITEM_ACTIVATED,
    ...
    auto add_btn = [this, buttons](
                       size_t pos, int &ID, wxString title, wxString tooltip, void (GalleryDialog::*method)(wxEvent &),
    ...
    size_t btn_pos = 0;
    add_btn(btn_pos++, ID_BTN_ADD_CUSTOM_SHAPE, _L("Add"), _L("Add one or more custom shapes"),
            &GalleryDialog::add_custom_shapes);
    add_btn(btn_pos++, ID_BTN_DEL_CUSTOM_SHAPE, _L("Delete"),
...
void GalleryDialog::on_dpi_changed(const wxRect &suggested_rect)
{
    ...
    msw_buttons_rescale(this, em,
                        {ID_BTN_ADD_CUSTOM_SHAPE, ID_BTN_DEL_CUSTOM_SHAPE, ID_BTN_REPLACE_CUSTOM_PNG, wxID_OK,
                         wxID_CLOSE});
...
static void generate_thumbnail_from_model(const std::string &filename)
{
    ...
    wxGetApp().plater()->canvas3D()->render_thumbnail(thumbnail_data, 256, 256, thumbnail_params, volumes,
...
void GalleryDialog::load_label_icon_list()
{
    ...
    for (const auto &item : list_items)
    {
        ...
        wxImage image;
        if (!image.CanRead(from_u8(img_name)) || !image.LoadFile(from_u8(img_name), wxBITMAP_TYPE_PNG) ||
...
void GalleryDialog::add_custom_shapes(wxEvent &event)
{
    ...
    wxFileDialog dialog(this, _L("Choose one or more files (STL, OBJ):"),
                        from_u8(wxGetApp().app_config->get_last_dir()), "", file_wildcards(FT_GALLERY),
...
void GalleryDialog::change_thumbnail()
{
    ...
    wxFileDialog dialog(this, _L("Choose one PNG file:"), from_u8(wxGetApp().app_config->get_last_dir()), "",
...
void GalleryDialog::deselect(wxListEvent &event)
{
    ...
    m_selected_items.erase(std::remove_if(m_selected_items.begin(), m_selected_items.end(),
                                          [name](Item item) { return item.name == name; }));
}
...
bool GalleryDialog::add_files_to_custom_dir(const wxArrayString &input_files)
{
    ...
    for (size_t i = 0; i < input_files.size(); ++i)
    {
        ...
        if (is_gallery_file(input_file, ".stl") || is_gallery_file(input_file, ".obj"))
        {
            ...
            {
                show_warning(format_wxstr(_L("Loading of the \"%1%\""), input_file),
                             is_gallery_file(input_file, ".obj") ? "OBJ" : "STL");
                continue;
            }
        }
            ...
            else
            {
                ...
                for (auto &dir_entry : fs::directory_iterator(dest_dir))
                    if (is_gallery_file(dir_entry, ".stl") || is_gallery_file(dir_entry, ".obj"))
                    {
                        ...
                        if (name.find(filename) != 0 || name[filename.size()] != ' ' ||
                            name[filename.size() + 1] != '(' || name[name.size() - 1] != ')')
                            continue;
                ...
                if (file_idx > 0)
                {
                    filename += " (" + std::to_string(file_idx) + ")." +
                                (is_gallery_file(input_file, ".stl") ? "stl" : "obj");
