...
wxDEFINE_EVENT(EVT_FORCE_INVALIDATE_SLICE, SimpleEvent);
...
FreqChangedParams::FreqChangedParams(wxWindow *parent)
{
    ...
    support_def.set_enum_labels(ConfigOptionDef::GUIType::select_close,
                                {L("None"), L("Support on build plate only"), L("For support enforcers only"),
                                 L("Everywhere")});
    ...
    auto empty_widget = [this](wxWindow *parent)
    {
        ...
        auto btn = new ScalableButton(parent, wxID_ANY, "mirroring_transparent", wxEmptyString, wxDefaultSize,
    ...
    {
        m_wiping_dialog_button = new wxButton(parent, wxID_ANY, _L("Purging volumes") + dots, wxDefaultPosition,
        ...
        m_wiping_dialog_button->Bind(
            wxEVT_BUTTON,
            (
                [parent](wxCommandEvent &e)
                {
                    PresetBundle *preset_bundle = wxGetApp().preset_bundle;
                    DynamicPrintConfig &project_config = preset_bundle->project_config;
                    const bool use_custom_matrix =
                        (project_config.option<ConfigOptionBool>("wiping_volumes_use_custom_matrix"))->value;
                    ...
                    const std::vector<std::string> extruder_colours =
                        wxGetApp().plater()->get_extruder_color_strings_from_plater_config();
                    ...
                    const double default_purge = static_cast<const ConfigOptionFloat *>(
                                                     ...
                                                     ->value;
                    std::vector<double> filament_purging_multipliers =
                        preset_bundle->get_config_options_for_current_filaments<ConfigOptionPercents>(
                            "filament_purge_multiplier");

                    WipingDialog dlg(parent, cast<float>(init_matrix), extruder_colours, default_purge,
                                     filament_purging_multipliers, use_custom_matrix);
...
void FreqChangedParams::Show(bool show) const
{
...
ConfigOptionsGroup *FreqChangedParams::get_og()
{
    return m_og_fff.get();
}
