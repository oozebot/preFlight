...
public:
    ...
    // Stored rather than derived from m_children: wxDataViewCtrl on GTK needs to know a node is a
    // container before its children are added.
    ...
    // option node
    ModelNode(ModelNode *parent, const wxString &text, const wxString &old_value, const wxString &mod_value,
...
class DiffModel : public wxDataViewModel
{
    ...
    ModelNode *AddOption(ModelNode *group_node, wxString option_name, wxString old_value, wxString mod_value,
                         wxString new_value);
    ModelNode *AddOptionWithGroup(ModelNode *category_node, wxString group_name, wxString option_name,
                                  wxString old_value, wxString mod_value, wxString new_value);
    ModelNode *AddOptionWithGroupAndCategory(ModelNode *preset_node, wxString category_name, wxString group_name,
...
public:
    ...
    wxDataViewItem AddPreset(Preset::Type type, wxString preset_name, wxString new_preset_name = wxString());
...
public:
    ...
    void Append(const std::string &opt_key, Preset::Type type, wxString category_name, wxString group_name,
...
public:
    ...
    UnsavedChangesDialog(Preset::Type type, PresetCollection *dependent_presets,
                         const std::string &new_selected_preset = std::string());
    // show unsaved changes for all another cases
    UnsavedChangesDialog(const wxString &caption, const wxString &header, const std::string &app_config_key,
                         int act_buttons);
...
{
public:
    FullCompareDialog(const wxString &option_name, const wxString &old_value, const wxString &mod_value,
                      const wxString &new_value, const wxString &old_value_header, const wxString &mod_value_header,
