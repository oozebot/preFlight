...
// data_dir, which this file reached transitively until Slicing.hpp stopped carrying the Utils.hpp
// umbrella.
...
{
#if __APPLE__
    CFStringRef reasonForActivity = CFSTR("preFlight");
...
{
    //wxMessageDialog msg_wingow(parent, message, wxString(PREFLIGHT_APP_NAME " - ") + (title.empty() ? _L("Notice") : title), wxOK | wxICON_INFORMATION);
    MessageDialog msg_wingow(parent, message,
                             wxString(PREFLIGHT_APP_NAME " - ") + (title.empty() ? _L("Notice") : title),
...
static void add_config_substitutions(const ConfigSubstitutions &conf_substitutions, wxString &changes)
{
    ...
    for (const ConfigSubstitution &conf_substitution : conf_substitutions)
    {
        ...
        case coEnum:
        {
            ...
            new_val = opt.has_value() ? wxString("\"") + def->enum_def->value(*opt) + "\"" + " (" +
        ...
        case coBools:
            if (conf_substitution.new_value->nullable())
                for (const char v :
                     static_cast<const ConfigOptionBoolsNullable *>(conf_substitution.new_value.get())->values)
                    new_val += std::string(v == ConfigOptionBoolsNullable::nil_value() ? "nil"
                                           : v                                         ? "true"
                                                                                       : "false") +
                               ", ";
            else
        ...
        changes += format_wxstr("<tr><td><b>\"%1%\" (%2%)</b></td><td>: ", def->opt_key, _(def->label)) +
                   format_wxstr(_L("%1% was substituted with %2%"), bold_string(conf_substitution.old_value),
                                bold(new_val)) +
                   "</td></tr>";
    }
    changes += "</table>";
...
{
    return _L("This configuration was likely produced by another slicer or by an older version of preFlight.") + " " +
...
void show_substitutions_info(const PresetsConfigSubstitutions &presets_config_substitutions)
{
    ...
    {
        changes += "\n\n" + format_wxstr("%1% : %2%", preset_type_name(substitution.preset_type),
                                         bold_string(from_u8(substitution.preset_name)));
    ...
    InfoDialog msg(nullptr,
                   _L("Configuration bundle was loaded, however some configuration values were not recognized."),
...
void show_substitutions_info(const ConfigSubstitutions &config_substitutions, const std::string &filename)
{
    ...
    InfoDialog msg(
        nullptr,
        format_wxstr(_L("Configuration file \"%1%\" was loaded, however some configuration values were not recognized."),
                     from_u8(filename)),
        substitution_message(changes), true);
    msg.ShowModal();
}
