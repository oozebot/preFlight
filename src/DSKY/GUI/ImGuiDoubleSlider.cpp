...
#include "DSKY/GUI/ImGuiPureWrap.hpp"
#include "DSKY/GUI/Widgets/UIColors.hpp"

namespace DoubleSlider
{

// Themed accent fill for the slider thumbs / value labels. A runtime function rather than a static
// const: a const initialized at program start would freeze the default orange before the theme
// loads, and would not follow an in-process theme switch.
...
ImRect ImGuiControl::DrawOptions::groove(const ImVec2 &pos, const ImVec2 &size, bool is_horizontal) const
{
    ImVec2 groove_start = is_horizontal
                              ...
                              : ImVec2(mirrored ? pos.x + dummy_sz().x : pos.x + size.x - groove_sz().x - dummy_sz().x,
                                       pos.y + text_dummy_sz().y);
                              ...
                              bool with_border /*= false*/)
{
    ...
    // A vertical label hangs to the left of the thumb, or to the right when the slider is laid out mirrored.
    const bool hang_right = !is_horizontal() && m_draw_opts.mirrored;
    const float vertical_text_x = hang_right ? thumb.Max.x + triangle_offset_x
                                             : thumb.Min.x - text_size.x - triangle_offset_x;
    ImVec2 text_start = is_horizontal() ? ImVec2(thumb.Max.x + triangle_offset_x, thumb_center.y - text_size.y)
                                        : ImVec2(vertical_text_x, thumb_center.y - text_size.y);

    if (is_mirrored)
        text_start = is_horizontal()
                         ? ImVec2(thumb.Min.x - text_size.x - triangle_offset_x, thumb_center.y - text_size.y)
                         : ImVec2(vertical_text_x, thumb_center.y);
    ...
    if (with_border)
    {
        ...
        // The vertical pointer sits on the label's edge that faces the thumb.
        const float pointer_x_b = hang_right ? text_rect_b.Min.x + rounding_b : text_rect_b.Max.x - rounding_b;
        const float pointer_dx_b = hang_right ? -triangle_offset_x_b : triangle_offset_x_b;
        ImVec2 pos_1 = is_horizontal() ? ImVec2(text_rect_b.Min.x + rounding_b, text_rect_b.Max.y)
                                       : ImVec2(pointer_x_b, text_rect_b.Max.y);
        ImVec2 pos_2 = is_horizontal() ? pos_1 - ImVec2(triangle_offset_x_b, 0.f)
                                       : pos_1 - ImVec2(0.f, triangle_offset_y_b);
        ImVec2 pos_3 = is_horizontal() ? pos_1 - ImVec2(0.f, triangle_offset_y_b) : pos_1 + ImVec2(pointer_dx_b, 0.f);

        if (is_mirrored)
        {
            pos_1 = is_horizontal() ? ImVec2(text_rect_b.Max.x - rounding_b - 1, text_rect_b.Max.y - 1)
                                    : ImVec2(pointer_x_b, text_rect_b.Min.y);
            pos_2 = is_horizontal() ? pos_1 + ImVec2(triangle_offset_x_b, 0.f)
                                    : pos_1 + ImVec2(0.f, triangle_offset_y_b);
            pos_3 = is_horizontal() ? pos_1 - ImVec2(0.f, triangle_offset_y_b) : pos_1 + ImVec2(pointer_dx_b, 0.f);
        }
    ...
    const float pointer_x = hang_right ? text_rect.Min.x + rounding : text_rect.Max.x - rounding;
    const float pointer_dx = hang_right ? -triangle_offset_x : triangle_offset_x;
    ImVec2 pos_1 = is_horizontal() ? ImVec2(text_rect.Min.x + rounding, text_rect.Max.y)
                                   : ImVec2(pointer_x, text_rect.Max.y);
    ImVec2 pos_2 = is_horizontal() ? pos_1 - ImVec2(triangle_offset_x, 0.f) : pos_1 - ImVec2(0.f, triangle_offset_y);
    ImVec2 pos_3 = is_horizontal() ? pos_1 - ImVec2(0.f, triangle_offset_y) : pos_1 + ImVec2(pointer_dx, 0.f);

    if (is_mirrored)
    {
        pos_1 = is_horizontal() ? ImVec2(text_rect.Max.x - rounding - 1, text_rect.Max.y - 1)
                                : ImVec2(pointer_x, text_rect.Min.y);
        pos_2 = is_horizontal() ? pos_1 + ImVec2(triangle_offset_x, 0.f) : pos_1 + ImVec2(0.f, triangle_offset_y);
        pos_3 = is_horizontal() ? pos_1 - ImVec2(0.f, triangle_offset_y) : pos_1 + ImVec2(pointer_dx, 0.f);
    }

    // Accent fill with the themed glyph color (matches the Slice/Export button).
    const wxColour &atc = DSKY::active_palette().accent_text;
                               ...
                               const ImVec2 &pos, const ImVec2 &size, float scale)
{
    ...
    // The label side of the control does not react to hovering; the ruler side does.
    auto move_size = item_size;
    if (!is_horizontal() && m_draw_opts.mirrored)
        move_size.Max.x -= left_dummy_sz().x;
    else
    ...
    if (!pos_changed && ImGui::ItemHoverable(move_size, id) && !ImGui::IsMouseDragging(0))
    {
        ...
        {
            if (m_draw_opts.mirrored)
                sl_region.Min.x -= m_draw_opts.dummy_sz().x;
            else
