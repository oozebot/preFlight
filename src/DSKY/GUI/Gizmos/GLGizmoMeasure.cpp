...
#include "luminary/gizmo/measure/MeasureUtils.hpp"
...
static const Luminary::ColorRGBA SELECTED_1ST_COLOR = {0.25f, 0.75f, 0.75f, 1.0f};
static const Luminary::ColorRGBA SELECTED_2ND_COLOR = {0.75f, 0.25f, 0.75f, 1.0f};
static const Luminary::ColorRGBA NEUTRAL_COLOR = {0.5f, 0.5f, 0.5f, 1.0f};
...
static GLModel::Geometry init_plane_data(const indexed_triangle_set &its, const std::vector<int> &triangle_indices)
{
    ...
    init_data.format = {DSKY::GLModel::Geometry::EPrimitiveType::Triangles, GLModel::Geometry::EVertexLayout::P3N3};
...
bool GLGizmoMeasure::gizmo_event(GizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                                 bool control_down)
{
    if (action == GizmoEventType::ShiftDown)
    {
    ...
    else if (action == GizmoEventType::ShiftUp)
    {
        m_shift_kar_filter.reset_count();
        m_mode = EMode::FeatureSelection;
        restore_scene_raycasters_state();
    }
    else if (action == GizmoEventType::Delete)
    {
        m_selected_features.reset();
        m_selected_sphere_raycasters.clear();
        m_parent.request_extra_frame();
    }
    else if (action == GizmoEventType::Escape)
    {
        if (!m_selected_features.first.feature.has_value())
        {
            update_measurement_result();
