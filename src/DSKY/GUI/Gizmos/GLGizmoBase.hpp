...
#include "DSKY/GUI/SceneRaycaster.hpp"
...
// The gizmo highlight (scale/rotate grabbers) uses the theme's secondary accent so it stays
...
protected:
    ...
    int m_group_id; // picking group for the grabbers, set only by the rotate gizmo
    EState m_state;
    int m_shortcut_key;
