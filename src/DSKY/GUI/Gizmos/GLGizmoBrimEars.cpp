    ...
    else if (mouse.dragging)
    {
        ...
        if (control_down && (mouse.left_down || mouse.right_down))
        {
            if (mouse.left_down)
                ...
                gizmo_event(GizmoEventType::RightUp, mouse_pos, mouse.shift, mouse.alt, true);
            return false;
        }
    }
    else if (mouse.type == MouseEventType::LeftUp)
    {
        if (gizmo_event(GizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, control_down) &&
...
bool GLGizmoBrimEars::gizmo_event(GizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                                  bool control_down)
{
    if (action != GizmoEventType::MouseWheelDown || action != GizmoEventType::MouseWheelUp ||
    ...
    else if (action == GizmoEventType::LeftDown && (shift_down || alt_down || control_down))
    {
    ...
    if (action == GizmoEventType::LeftDown && !m_selection_rectangle.is_dragging() && !shift_down)
    {
    ...
    if (action == GizmoEventType::LeftUp)
    {
    ...
    if (action == GizmoEventType::RightDown)
    {
        if (m_hover_id != -1)
        {
            select_point(NoPoints);
    ...
    // mouse wheel up
    if (action == GizmoEventType::MouseWheelUp)
    {
        if (control_down)
        {
    ...
    if (action == GizmoEventType::MouseWheelDown)
    {
        if (control_down)
        {
            float initial_value = m_new_point_head_diameter;
    ...
    if (action == GizmoEventType::MouseWheelUp && alt_down)
    {
    ...
    if (action == GizmoEventType::MouseWheelDown && alt_down)
    {
        double pos = m_c->object_clipper()->get_position();
...
Points GLGizmoBrimEars::generate_points(Luminary::Polygon &obj_polygon, float ear_detection_length,
                                        float brim_ears_max_angle, bool is_outer)
{
...
void GLGizmoBrimEars::auto_generate()
{
    ...
    {
        Luminary::Polygon out_poly = ex_poly.contour;
        ...
        for (Luminary::Polygon &pl : inner_poly)
        {
