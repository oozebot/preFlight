...
#include "luminary/geometry/diagnostics/SVG.hpp"    // debug store
#include "luminary/geometry/transform/Geometry.hpp" // covex hull 2d
#include "luminary/core/diagnostics/Timer.hpp"      // covex hull 2d
#include "luminary/gizmo/emboss/Emboss.hpp"         // heal_shape
...
CreateVolumeParams GLGizmoSVG::create_input(ModelVolumeType volume_type, std::string_view svg_filepath)
{
    ...
    // During selection of file it could change hovered volume
...
void GLGizmoSVG::on_stop_dragging()
{
    ...
    // Reset the grabber angle so a second rotation starts from a known handle position.
    m_rotate_gizmo.set_angle(PI / 2);
...
#include "luminary/geometry/index/AABBTreeLines.hpp" // aabb lines for draw filled expolygon
...
void draw(const ExPolygonsWithIds &shapes_with_ids, unsigned max_size)
{
    ...
    auto draw_polygon = [&scale, offset](Luminary::Polygon p)
    {
    ...
    for (const ExPolygonsWithId &shape : shapes_with_ids)
    {
        for (const ExPolygon &expoly : shape.expoly)
        {
            ...
            for (const Luminary::Polygon &hole : expoly.holes)
                draw_polygon(hole);
        }
    }
}
                       ...
                       std::vector<unsigned char> &data, size_t data_width, double scale)
{
    ...
    {
        // y has opposit direction in texture
                 ...
                 size_t data_width, double scale)
{
    ...
    auto tree = Luminary::AABBTreeLines::build_aabb_tree_over_indexed_lines(lines);
    ...
    for (int y = std::max(0, static_cast<int>(std::round(bb.min.y()))); y < max_y; ++y)
    {
        ...
        Intersections intersections = Luminary::AABBTreeLines::get_intersections_with_line<false, Vec2d, Linef>(lines,
                                                                                                                tree,
                                                                                                                line);
...
std::string create_fill_warning(const NSVGshape &shape)
{
    ...
    if ((shape.opacity - 1.f + warning_preccission) <= 0.f)
        add_comma_separated(warning, DSKY::format(_L("Opacity (%1%)"), shape.opacity));

    // if(shape->flags != NSVG_FLAGS_VISIBLE) add_warning(_u8L("Visibility flag"));
    bool is_fill_gradient = shape.fillGradient[0] != '\0';
    if (is_fill_gradient)
        add_comma_separated(warning, DSKY::format(_L("Color gradient (%1%)"), shape.fillGradient));
...
std::string create_stroke_warning(const NSVGshape &shape)
{
    ...
    if (is_stroke_gradient)
        add_comma_separated(warning, DSKY::format(_L("Color gradient (%1%)"), shape.strokeGradient));
...
std::vector<std::string> create_shape_warnings(const EmbossShape &shape, float scale)
{
    ...
    for (NSVGshape *shape = image.shapes; shape != NULL; shape = shape->next, ++shape_index)
    {
        ...
        {
            add_warning(shape_index * 2, DSKY::format(_L("Shape is marked as invisible (%1%)."), shape->id));
            continue;
        }
        ...
        if (!fill_warning.empty())
        {
            ...
            add_warning(shape_index * 2,
                        DSKY::format(_L("Fill of shape (%1%) contains unsupported: %2%."), shape->id, fill_warning));
        }
        ...
        {
            add_warning(shape_index * 2,
                        DSKY::format(_L("Stroke of shape (%1%) is too thin (minimal width is %2% mm)."), shape->id,
                                     minimal_width_in_mm));
            continue;
        }
        std::string stroke_warning = create_stroke_warning(*shape);
        if (!stroke_warning.empty())
            add_warning(shape_index * 2 + 1, DSKY::format(_L("Stroke of shape (%1%) contains unsupported: %2%."),
                                                          shape->id, stroke_warning));
    }
    return result;
}
...
void GLGizmoSVG::draw_preview()
{
    ...
    if (m_texture.id != 0)
    {
        ...
        //    std::string tooltip = DSKY::format("%1% shapes, which create %2% polygons with %3% line segments",
...
void GLGizmoSVG::draw_filename()
{
    ...
    if (is_hovered)
    {
        std::string tooltip = DSKY::format(_L("SVG file path is \"%1%\""), svg.path);
        ImGui::SetTooltip("%s", tooltip.c_str());
    }
    ...
    if (ImGui::BeginCombo("##file_options", nullptr, flags))
    {
        ...
        if (ImGui::Selectable((_L("Save as") + dots).ToUTF8().data()))
        {
            ...
            DSKY::FileType file_type = FT_SVG;
        ...
        //        Luminary::save(*m_volume_shape.svg_file.image, path);
        //    }
        //} else if (ImGui::IsItemHovered()) {
...
void GLGizmoSVG::draw_size()
{
    ...
    if (ImGui::IsItemHovered())
    {
        ...
        for (const auto &s : m_volume_shape.shapes_with_ids)
            count_points += Luminary::count_points(s.expoly);
        // TRN: The placeholder contains a number.
        ImGui::SetTooltip("%s",
                          DSKY::format(_L("Scale also changes amount of curve samples (%1%)"), count_points).c_str());
    }
...
EmbossShape select_shape(std::string_view filepath, double tesselation_tolerance)
{
    ...
    if (!boost::filesystem::exists(path))
    {
        show_error(nullptr, DSKY::format(_u8L("File does NOT exist (%1%)."), svg.path));
        return {};
    }

    if (!boost::algorithm::iends_with(svg.path, ".svg"))
    {
        show_error(nullptr, DSKY::format(_u8L("Filename has to end with \".svg\" but you selected %1%"), svg.path));
        return {};
    }

    if (init_image(svg) == nullptr)
    {
        show_error(nullptr, DSKY::format(_u8L("Nano SVG parser can't load from file (%1%)."), svg.path));
        return {};
    }
    ...
    {
        show_error(nullptr,
                   DSKY::format(_u8L("SVG file does NOT contain a single path to be embossed (%1%)."), svg.path));
        return {};
    }
