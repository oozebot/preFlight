...
#include "DSKY/GUI/GUI_ObjectManipulation.hpp"
...
#include "DSKY/GUI/Jobs/EmbossJob.hpp"
#include "DSKY/GUI/Jobs/CreateFontNameImageJob.hpp"
#include "DSKY/GUI/Jobs/NotificationProgressIndicator.hpp"
#include "DSKY/Utils/WxFontUtils.hpp"
#include "DSKY/Utils/UndoRedo.hpp"

#include "luminary/geometry/transform/Geometry.hpp" // to range pi pi
...
#include "luminary/geometry/clipper/ClipperUtils.hpp" // union_ex
#include "luminary/presets/app_config/AppConfig.hpp"  // store/load font list
#include "luminary/format/mesh/OBJ.hpp"               // load obj file for default object
...
/// Embossed object must be selected
/// </summary>
...
{
    // change style manager - be carefull with order changes
...
void GLGizmoEmboss::on_shortcut_key()
{
    ...
    if (m_volume == nullptr)
    {
        ...
        // After finish job for creation emboss Text volume,
...
bool GLGizmoEmboss::do_mirror(size_t axis)
{
    ...
    // Staff around volume transformation change is done in job finish
    return process();
}
...
bool GLGizmoEmboss::on_init()
{
    ...
    // It has special handling in GLGizmosManager::handle_shortcut
...
static void draw_fine_position(const Selection &selection, const Size &canvas, const ImVec2 &windows_size)
{
    ...
    const Camera &camera = m_parent.get_camera();
    Luminary::Polygon hull = CameraUtils::create_hull2d(camera, *volume);
    ...
    Luminary::Polygon rect({Point(offset.x, offset.y), Point(offset.x + windows_size.x, offset.y),
...
static void draw_place_to_add_text()
{
    ...
    ImGuiWrapper::draw(Luminary::Polygon(rect2d));
}
...
EmbossStyles GLGizmoEmboss::create_default_styles()
{
    ...
    else
    {
        ...
        // use the font packed with preFlight
        std::string font_path = Luminary::resources_dir() + "/fonts/NotoSans-Regular.ttf";
...
void GLGizmoEmboss::draw_text_input()
{
    ...
    // Symbol fonts doesn't have atlas
            ...
            if (ImGuiPureWrap::selectable_contrast(face.name_truncated.c_str(), is_selected, flags, selectable_size))
            {
                if (!select_facename(wx_face_name))
                {
                    ...
                    MessageDialog(m_parent.get_wxglcanvas_parent(),
                                  DSKY::format_wxstr(_L("Font \"%1%\" can't be selected."), wx_face_name));
                }
            }
...
void GLGizmoEmboss::draw_model_type()
{
    ...
    if (m_parent.get_mode() != ConfigOptionMode::comSimple)
    {
        ImGui::SameLine();
    ...
    if (m_volume != nullptr && new_type.has_value() && !is_last_solid_part)
    {
        ...
        // on linux, function reorder_volumes_and_get_selection call GLCanvas3D::reload_scene(refresh_immediately = false)
        // which discard m_volume pointer and set it to nullptr also selection is cleared so gizmo is automaticaly closed
        auto &mng = m_parent.get_gizmos_manager();
        if (mng.get_current_type() != GLGizmosManager::Emboss)
            mng.open_gizmo(GLGizmosManager::Emboss);
        // The volume is not selected again after the reload, only the gizmo is reopened.
    }
}
...
void GLGizmoEmboss::draw_style_rename_popup()
{
    ...
    std::string text_in_popup = DSKY::format(_L("Rename style(%1%) for embossing text"), old_name) + ": ";
...
void GLGizmoEmboss::draw_style_save_button(bool is_modified)
{
    ...
    else if (ImGui::IsItemHovered())
    {
        ...
        {
            tooltip = DSKY::format(_L("Save %1% style"), m_style_manager.get_style().name);
        }
        else
        {
...
void GLGizmoEmboss::draw_delete_style_button()
{
    ...
    if (draw_button(m_icons, IconType::erase, !can_delete))
    {
        ...
        while (true)
        {
            // can't use previous loaded activ index -> erase could change index
            ...
            wxString message = DSKY::format_wxstr(_L("Are you sure you want to permanently remove the \"%1%\" style?"),
                                                  style_name);
    ...
    if (ImGui::IsItemHovered())
    {
        ...
        std::string tooltip;
        if (can_delete)
            tooltip = DSKY::format(_L("Delete \"%1%\" style."), style_name);
        else if (is_last)
            tooltip = DSKY::format(_L("Can't delete \"%1%\". It is last style."), style_name);
        else /*if(!is_stored)*/
            tooltip = DSKY::format(_L("Can't delete temporary style \"%1%\"."), style_name);
        ImGui::SetTooltip("%s", tooltip.c_str());
    }
}
...
void GLGizmoEmboss::draw_style_list()
{
    ...
    else
    {
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            std::string tooltip = is_modified ? DSKY::format(_L("Modified style \"%1%\""), current_style.name)
                                              : DSKY::format(_L("Current style is \"%1%\""), current_style.name);
            ImGui::SetTooltip(" %s", tooltip.c_str());
        }
    }
    ...
    if (selected_style_index.has_value() && is_modified)
    {
        ...
        wxString message = DSKY::format_wxstr(
    ...
    if (selected_style_index.has_value())
    {
        ...
        else
        {
            ...
            wxString message = DSKY::format_wxstr(_L("Style \"%1%\" can't be used and will be removed from a list."),
                                                  style.name);
                               ...
                               const std::string &undo_tooltip, float undo_offset, Draw draw) const
{
    ...
    if (changed)
    {
        ...
        if (draw_button(m_icons, IconType::undo))
        {
            ...
            // Detect change of value after revert of float-slider
...
bool GLGizmoEmboss::choose_font_by_wxdialog()
{
        ...
        (!use_deserialized_font && !m_style_manager.load_style(emboss_style, wx_font)))
    {
        ...
        wxString message = DSKY::format_wxstr("Font \"%1%\" can't be used. Please select another.", emboss_style.name);
...
void GLGizmoEmboss::create_notification_not_valid_font(const TextConfiguration &tc)
{
    ...
    std::string text = DSKY::format(_L("Can't load exactly same font(\"%1%\"). "
...
{
    return DSKY::button(get_icon(icons, type, IconState::activable), get_icon(icons, type, IconState::hovered),
...
bool TextDataBase::create_text_lines(const Transform3d &tr, const ModelVolumePtrs &vols)
{
    ...
    text_lines = Luminary::Emboss::create_text_lines(tr, vols, *m_font_file.font_file, m_text_configuration.style.prop,
                                                     count_lines);
    if (text_lines.empty())
