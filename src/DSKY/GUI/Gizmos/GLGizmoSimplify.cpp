                                                           ...
                                                           NotificationManager &manager, GLCanvas3D &canvas)
{
    ...
    {
        std::string t = DSKY::format(_L("Processing model \"%1%\" with more than 1M triangles "
...
void GLGizmoSimplify::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    { // apply or preview
        ImGui::SameLine(m_gui_cfg->bottom_left_width);
        // draw progress bar
        std::string progress_text = DSKY::format(_L("Process %1% / 100"), std::to_string(progress));
...
void GLGizmoSimplify::apply_simplify()
{
    ...
    const Selection &selection = m_parent.get_selection();
    m_parent.take_typed_snapshot(Luminary::format(_u8L("Simplify %1%"), create_volumes_name(m_volume_ids, selection)),
