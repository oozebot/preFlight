...
GLGizmoPainterBase::ClippingPlaneDataWrapper GLGizmoPainterBase::get_clipping_plane_data() const
{
    ...
    // saved with opposite sign than we need to pass to OpenGL
...
void GLGizmoPainterBase::render_cursor_height_range(const Transform3d &trafo) const
{
    ...
    {
        for (const Luminary::Polygon &polygon : polygons_at_z.polygons)
        {
...
bool GLGizmoPainterBase::is_mesh_point_clipped(const Vec3d &point, const Transform3d &trafo) const
{
    ...
    const Vec3d transformed_point = trafo * point;
...
bool GLGizmoPainterBase::gizmo_event(GizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                                     bool control_down)
{
    if (action == GizmoEventType::MouseWheelUp || action == GizmoEventType::MouseWheelDown)
    {
        ...
        if (!alt_down && control_down)
        {
            ...
            pos = action == GizmoEventType::MouseWheelDown ? std::max(0., pos - 0.01) : std::min(1., pos + 0.01);
        ...
        else if (alt_down)
        {
            ...
            {
                m_cursor_radius = action == GizmoEventType::MouseWheelDown
            ...
            else if (m_tool_type == ToolType::SMART_FILL || m_tool_type == ToolType::BUCKET_FILL)
            {
                ...
                fill_angle = (action == GizmoEventType::MouseWheelDown)
            ...
            {
                m_height_range_z_range = action == GizmoEventType::MouseWheelDown
    ...
    if (action == GizmoEventType::ResetClippingPlane)
    {
        m_c->object_clipper()->set_position_by_ratio(-1., false);
        return true;
    }

    if (action == GizmoEventType::Escape)
    {
        if (m_line_start_set)
        {
    ...
    bool is_line_action = (control_down && !m_line_start_set &&
                          ...
                          (m_line_start_set &&
                           (action == GizmoEventType::LeftDown || action == GizmoEventType::RightDown ||
                            action == GizmoEventType::Moving));

    if (is_line_action && m_tool_type == ToolType::BRUSH)
    {
        if (action == GizmoEventType::LeftDown || action == GizmoEventType::RightDown)
        {
            ...
            if (!m_line_start_set)
            {
                ...
                // The arming click is not a stroke: m_button_down stays clear so the release
                // after it does not take a zero-delta snapshot or start a drag.
                m_line_button_type = (action == GizmoEventType::LeftDown) ? Button::Left : Button::Right;
                m_parent.set_as_dirty();
                return true;
            }
            else
        ...
        if (action == GizmoEventType::Moving && m_line_start_set)
        {
    ...
    if (action == GizmoEventType::LeftDown || action == GizmoEventType::RightDown ||
        (action == GizmoEventType::Dragging && m_button_down != Button::None))
    {
        ...
        if (!shift_down)
        {
            if (action == GizmoEventType::Dragging)
                new_state = m_button_down == Button::Left ? this->get_left_button_state_type()
                                                          : this->get_right_button_state_type();
            else
                new_state = action == GizmoEventType::LeftDown ? this->get_left_button_state_type()
                                                               : this->get_right_button_state_type();
        }
        ...
        for (const std::vector<ProjectedMousePosition> &projected_mouse_positions : projected_mouse_positions_by_mesh)
        {
            ...
            const bool dragging_while_painting = (action == GizmoEventType::Dragging && m_button_down != Button::None);
            ...
            if (mesh_idx != -1)
                if (m_button_down == Button::None)
                    m_button_down = ((action == GizmoEventType::LeftDown) ? Button::Left : Button::Right);
    ...
    if (action == GizmoEventType::Moving &&
    ...
    if ((action == GizmoEventType::LeftUp || action == GizmoEventType::RightUp) && m_button_down != Button::None)
    {
        // Take snapshot and update ModelVolume data.
        // Shift is the eraser (the stroke painted NONE); Ctrl arms the line tool.
...
bool GLGizmoPainterBase::on_mouse(const MouseInput &mouse)
{
    ...
    {
        gizmo_event(GizmoEventType::Moving, mouse_pos, mouse.shift, mouse.alt, false);
        return false;
    }
    ...
    {
        if (gizmo_event(GizmoEventType::LeftDown, mouse_pos, mouse.shift, mouse.alt, control_down))
            return true;
    }
    else if (mouse.type == MouseEventType::RightDown)
    {
        if (selected_object_idx != -1 &&
            gizmo_event(GizmoEventType::RightDown, mouse_pos, mouse.shift, mouse.alt, control_down))
            return true;
    }
    else if (mouse.dragging)
    {
        if (m_parent.get_move_volume_id() != -1)
            return true;
        if (gizmo_event(GizmoEventType::Dragging, mouse_pos, mouse.shift, mouse.alt, control_down))
        {
            m_parent.set_as_dirty();
            return true;
        }
    ...
    else if (mouse.type == MouseEventType::LeftUp)
    {
        ...
        {
            gizmo_event(GizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, control_down);
            return true;
        }
    }
    else if (mouse.type == MouseEventType::RightUp)
    {
        if (!m_parent.is_mouse_dragging())
        {
            gizmo_event(GizmoEventType::RightUp, mouse_pos, mouse.shift, mouse.alt, control_down);
            return true;
        }
    }
    return false;
...
{
    const ::DSKY::ClippingPlane *const clipping_plane = m_c->object_clipper()->get_clipping_plane();
...
void GLGizmoPainterBase::draw_line_between_points(const Vec3f &start, const Vec3f &end, int mesh_idx)
{
    ...
    // Use DoublePointCursor to paint between points. The cursor takes the camera in mesh
    // coordinates, as the drag path does.
    const Vec3f camera_pos = (trafo_matrix.inverse() * camera.get_position()).cast<float>();
    std::unique_ptr<TriangleSelector::Cursor> cursor = TriangleSelector::DoublePointCursor::cursor_factory(
        start, end, camera_pos, m_cursor_radius, m_cursor_type, trafo_matrix, clp);
