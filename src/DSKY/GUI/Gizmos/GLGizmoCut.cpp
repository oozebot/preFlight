...
#include "DSKY/GUI/GUI_Factories.hpp"
#include "DSKY/GUI/format.hpp"
#include "DSKY/GUI/Field.hpp"
...
bool GLGizmoCut3D::on_mouse(const MouseInput &mouse)
{
    ...
    if (mouse.shift && mouse.type == MouseEventType::LeftDown)
        return gizmo_event(GizmoEventType::LeftDown, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
    ...
    if (cut_line_processing())
    {
        if (mouse.shift)
        {
            if (mouse.type == MouseEventType::Motion || mouse.dragging)
                return gizmo_event(GizmoEventType::Moving, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
            if (mouse.type == MouseEventType::LeftUp)
                return gizmo_event(GizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
        }
    ...
    if (use_grabbers(mouse))
    {
        if (m_hover_id >= m_connectors_group_id)
        {
            ...
            if (mouse.type == MouseEventType::LeftUp && !mouse.shift)
                gizmo_event(GizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
        }
    ...
    if (mouse.type == MouseEventType::LeftDown)
    {
        ...
        if ((!mouse.shift || grabber_contains_mouse) &&
            gizmo_event(GizmoEventType::LeftDown, mouse_pos, mouse.shift, mouse.alt, false))
            return true;
    }
    else if (mouse.dragging)
    {
        ...
        if (m_parent.get_move_volume_id() != -1)
            return true;
        if (!control_down && gizmo_event(GizmoEventType::Dragging, mouse_pos, mouse.shift, mouse.alt, false))
        {
        ...
        {
            if (mouse.left_down)
                gizmo_event(GizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, true);
    ...
    else if (mouse.type == MouseEventType::RightDown)
    {
        ...
        if (m_parent.get_selection().get_object_idx() != -1 &&
            gizmo_event(GizmoEventType::RightDown, mouse_pos, false, false, false))
        {
...
void GLGizmoCut3D::put_connectors_on_cut_plane(const Vec3d &cp_normal, double cp_offset)
{
    ...
    if (CutConnectors &connectors = mo->cut_connectors; !connectors.empty())
    {
        ...
        for (auto &connector : connectors)
        {
            ...
            const Vec3d pos = connector.pos + instance_offset;
...
bool GLGizmoCut3D::on_init()
{
    ...
    const std::string ctrl = DSKY::shortkey_ctrl_prefix();
    const std::string alt = DSKY::shortkey_alt_prefix();
...
void GLGizmoCut3D::process_contours()
{
    ...
    else
    {
        ...
        m_part_selection = PartSelection(model_objects[object_idx], get_cut_matrix(selection), instance_idx,
                                         m_plane_center, m_cut_normal, *m_c->object_clipper());
    }
                                             ...
                                             float &in_tolerance)
{
    ...
    if (render_slider_double_input(label, val, tolerance, -0.1f, std::min(0.3f * in_val, 1.5f)))
    {
        ...
        {
            m_parent.take_gizmo_snapshot(Luminary::format(_u8L("Groove change") + ": %1%", label));
...
void GLGizmoCut3D::render_input_window_warning() const
{
    ...
    if (!m_invalid_connectors_idxs.empty())
    {
        ...
        if (m_info_stats.outside_cut_contour > size_t(0))
            out += "\n - " +
                   DSKY::format(_L_PLURAL("%1$d connector is out of cut contour",
        ...
        if (m_info_stats.outside_bb > size_t(0))
            out += "\n - " + DSKY::format(_L_PLURAL("%1$d connector is out of object",
...
Transform3d GLGizmoCut3D::get_cut_matrix(const Selection &selection)
{
    ...
    const Vec3d cut_center_offset = m_plane_center - instance_offset;
        ...
        if (dlg.ShowModal() == wxID_YES)
        {
                                        ...
                                        std::vector<std::pair<std::string, std::string>> &failed_models) -> bool
            {
                ...
                if (model_names.size() == 1)
                    msg = DSKY::format(_L("Repairing object %1%"), model_name) + "\n";
                else
                {
                                          ...
                                          bool respect_contours /* = true*/)
{
    ...
    // Disabled: this check does not work yet.
...
bool GLGizmoCut3D::process_cut_line(GizmoEventType action, const Vec2d &mouse_position)
{
    const Camera &camera = m_parent.get_camera();
    ...
    if (action == GizmoEventType::LeftDown && !cut_line_processing())
    {
        m_line_beg = pt;
        m_line_end = pt;
        on_unregister_raycasters_for_picking();
    ...
    if (cut_line_processing())
    {
        ...
        m_line_end = pt;
        if (action == GizmoEventType::LeftDown || action == GizmoEventType::LeftUp)
        {
        ...
        else if (action == GizmoEventType::Moving)
            this->set_dirty();
        return true;
    }
    return false;
...
void GLGizmoCut3D::process_selection_rectangle(CutConnectors &connectors)
{
    ...
    const Transformation trafo = mo->instances[active_inst]->get_transformation();
...
bool GLGizmoCut3D::gizmo_event(GizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                               bool control_down)
{
    ...
    if ((m_hover_id < 0 || m_hover_id == CutPlane) && shift_down && !m_connectors_editing &&
        (action == GizmoEventType::LeftDown || action == GizmoEventType::LeftUp || action == GizmoEventType::Moving))
    ...
    if (action == GizmoEventType::LeftDown)
    {
    ...
    if (action == GizmoEventType::LeftUp && !m_selection_rectangle.is_dragging())
    {
    ...
    if ((action == GizmoEventType::LeftUp || action == GizmoEventType::ShiftUp || action == GizmoEventType::AltUp) &&
    ...
    if (action == GizmoEventType::RightDown && !shift_down)
    {
    ...
    if (action == GizmoEventType::Delete)
        return delete_selected_connectors(connectors);

    if (action == GizmoEventType::SelectAll)
    {
