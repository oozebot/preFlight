...
#include "DSKY/GUI/InputEvents_wx.hpp"
#include "DSKY/GUI/ThemePalette.hpp"
...
#include "DSKY/GUI/Gizmos/GLGizmoSeam.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoSimplify.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoEmboss.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoBrimEars.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoSVG.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoMeasure.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoAlign.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoRelief.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoCounterboreBridge.hpp"
#include "DSKY/GUI/Gizmos/GLGizmoColorMixing.hpp"
...
bool GLGizmosManager::gizmo_event(GizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                                  bool control_down)
{
...
bool GLGizmosManager::on_mouse_wheel(const MouseInput &mouse)
{
        ...
        m_current == BrimEars || m_current == CounterboreBridge)
    {
        ...
        if (gizmo_event((rot > 0.f ? GizmoEventType::MouseWheelUp : GizmoEventType::MouseWheelDown), Vec2d::Zero(),
...
bool GLGizmosManager::on_char(const KeyInput &key)
{
...
#endif
        {
            if ((m_current == Cut) && gizmo_event(GizmoEventType::SelectAll))
                processed = true;
            break;
        }
        }
    ...
    else if (!key.shift && !key.ctrl && !key.alt && !key.meta)
    {
        ...
        case WXK_ESCAPE:
        {
            if (m_current != Undefined)
            {
                if ((m_current == Measure || m_current == Seam) && gizmo_event(GizmoEventType::Escape))
                {
                }
                else
                    reset_all_states();
        ...
        {
            if ((m_current == FdmSupports || m_current == Seam || m_current == ColorMixing || m_current == FuzzySkin) &&
                gizmo_event(GizmoEventType::ResetClippingPlane))
                processed = true;
            break;
        }
        case WXK_BACK:
        case WXK_DELETE:
        {
            if ((m_current == Cut || m_current == Measure || m_current == BrimEars) &&
                gizmo_event(GizmoEventType::Delete))
                processed = true;
            break;
        }
        case 'A':
...
bool GLGizmosManager::process_delete_key()
{
    ...
    if (m_current == Cut || m_current == Measure || m_current == BrimEars)
    {
        if (gizmo_event(GizmoEventType::Delete))
        {
            m_parent.set_as_dirty();
            return true;
        }
...
bool GLGizmosManager::on_key(const KeyInput &key)
{
    ...
    if (key.type == KeyEventType::KeyUp)
    {
        if (m_current == Cut)
        {
            ...
            {
                if (gizmo_event(GizmoEventType::ShiftUp) || (is_editing && is_rectangle_dragging))
                    processed = true;
            }
            else if (keyCode == WXK_ALT)
            {
                if (gizmo_event(GizmoEventType::AltUp) || (is_editing && is_rectangle_dragging))
                    processed = true;
            }
        }
        else if (m_current == Measure)
        {
            if (keyCode == WXK_CONTROL)
                gizmo_event(GizmoEventType::CtrlUp, Vec2d::Zero(), key.shift, key.alt, key.cmd);
            else if (keyCode == WXK_SHIFT)
                gizmo_event(GizmoEventType::ShiftUp, Vec2d::Zero(), key.shift, key.alt, key.cmd);
        }
    }
    else if (key.type == KeyEventType::KeyDown)
    {
        ...
        {
            if (keyCode == WXK_CONTROL)
                gizmo_event(GizmoEventType::CtrlDown, Vec2d::Zero(), key.shift, key.alt, key.cmd);
            else if (keyCode == WXK_SHIFT)
                gizmo_event(GizmoEventType::ShiftDown, Vec2d::Zero(), key.shift, key.alt, key.cmd);
        }
    }
...
void GLGizmosManager::do_render_overlay() const
{
    ...
    // Themed solid backdrop, corners rounded only on the canvas-facing side; the side flips
    ...
    {
        const DSKY::RGBAf &bg = DSKY::active_palette().toolbar_background;
