...
ExtruderSequenceDialog::ExtruderSequenceDialog(const DoubleSlider::ExtrudersSequence &sequence)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                wxString(PREFLIGHT_APP_NAME) + " - " + _(L("Set extruder sequence")), wxDefaultPosition, wxDefaultSize,
    ...
    , m_sequence(sequence)
{
    ...
    wxRadioButton *rb_by_layers = new wxRadioButton(this, ID_RADIO_BUTTON, "", wxDefaultPosition, wxDefaultSize,
                                                    wxRB_GROUP);
    ...
    m_interval_by_layers = new wxTextCtrl(this, wxID_ANY, wxString::Format("%d", m_sequence.interval_by_layers),
    ...
    m_interval_by_layers->Bind(wxEVT_TEXT,
                               [this, rb_by_layers](wxEvent &)
                               {
                                   wxString str = m_interval_by_layers->GetValue();
                                   if (str.IsEmpty())
                                   {
                                       m_interval_by_layers->SetValue(
    ...
    m_interval_by_mm = new wxTextCtrl(this, wxID_ANY, double_to_string(sequence.interval_by_mm), wxDefaultPosition,
                                      editor_sz,
                                      wxTE_PROCESS_ENTER
#ifdef _WIN32
                                          | wxBORDER_SIMPLE
    ...
    m_interval_by_mm->Bind(wxEVT_TEXT,
                           [this, rb_by_mm](wxEvent &)
                           {
                               m_sequence.is_mm_intervals = true;
                               rb_by_mm->SetValue(true);
                           });

    m_interval_by_mm->Bind(wxEVT_KILL_FOCUS,
                           [change_value](wxFocusEvent &event)
                           {
                               change_value();
                               event.Skip();
                           });

    m_interval_by_mm->Bind(wxEVT_TEXT_ENTER, [change_value](wxEvent &) { change_value(); });
    ...
    m_random_sequence->Bind(wxEVT_CHECKBOX,
    ...
    m_color_repetition->Bind(wxEVT_CHECKBOX,
                             [this](wxCommandEvent &e) { m_sequence.color_repetition = e.IsChecked(); });
...
void ExtruderSequenceDialog::apply_extruder_sequence()
{
    ...
    for (size_t extruder = 0; extruder < m_sequence.extruders.size(); ++extruder)
    {
        ...
        extruder_selector->Bind(wxEVT_COMBOBOX,
                                [this, extruder_selector, extruder](wxCommandEvent &evt)
                                {
        ...
        del_btn->Bind(wxEVT_BUTTON,
                      [this, extruder](wxEvent &)
                      {
        ...
        add_btn->Bind(wxEVT_BUTTON,
                      [this, extruder, extruder_selector](wxEvent &)
                      {
