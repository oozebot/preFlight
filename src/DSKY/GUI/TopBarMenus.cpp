...
#include "luminary/config/model/Config.hpp" //ConfigOptionMode
...
wxString TopBarMenus::get_workspace_name(int mode /* = -1*/)
{
    ...
    return mode == Luminary::ConfigOptionMode::comSimple     ? _L("Beginner mode")
           : mode == Luminary::ConfigOptionMode::comAdvanced ? _L("Normal mode")
                                                             : _L("Expert mode");
}
...
void TopBarMenus::ApplyWorkspacesMenu()
{
    ...
    for (const Luminary::ConfigOptionMode &mode :
         {Luminary::ConfigOptionMode::comSimple, Luminary::ConfigOptionMode::comAdvanced,
          Luminary::ConfigOptionMode::comExpert})
    {
        const wxString label = get_workspace_name(mode);
        append_menu_item(
            &workspaces, wxID_ANY, label, label,
        ...
        if (mode < Luminary::ConfigOptionMode::comExpert)
...
{
    m_login_item = append_menu_item(
        &account, wxID_ANY, "", "",
        [this](wxCommandEvent &)
        {
            if (m_cb_act_with_user_account)
                m_cb_act_with_user_account();
        },
        "login");
    ...
    m_hide_login_item = append_menu_item(&account, wxID_ANY, _L("Hide \"Log in\" button"), "",
                                         [this](wxCommandEvent &)
                                         {
                                             if (m_cb_hide_user_account)
                                                 m_cb_hide_user_account();
                                         });
}
