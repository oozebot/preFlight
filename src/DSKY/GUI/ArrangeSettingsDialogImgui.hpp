...
#include <luminary/arrange/scene/ArrangeSettingsView.hpp>
...
class ArrangeSettingsDialogImgui : public arr2::ArrangeSettingsView
{
    ...
    std::function<bool()> m_show_xl_combo_predicate = []
    {
        return true;
    };
...
public:
    ...
    void show_xl_align_combo(std::function<bool()> pred) { m_show_xl_combo_predicate = pred; }

    void on_arrange_btn(std::function<void()> on_arrangefn) { m_on_arrange_btn = on_arrangefn; }

    void on_arrange_bed_btn(std::function<void()> on_arrangefn) { m_on_arrange_bed_btn = on_arrangefn; }

    void on_reset_btn(std::function<void()> on_resetfn) { m_on_reset_btn = on_resetfn; }
