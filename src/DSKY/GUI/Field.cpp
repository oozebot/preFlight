...
void TextCtrl::BUILD()
{
    ...
    temp->SetFont(m_opt.is_code ? DSKY::wxGetApp().code_font() : DSKY::wxGetApp().normal_font());
    ...
    // For multiline fields (G-code editors etc.), dismiss tooltips on the internal
...
void SpinCtrl::BUILD()
{
    ...
    temp->SetFont(DSKY::wxGetApp().normal_font());
    if (!wxOSX)
        temp->SetBackgroundStyle(wxBG_STYLE_PAINT);
    wxGetApp().UpdateDarkUI(temp);
...
void PointCtrl::BUILD()
{
    ...
    x_textctrl->SetFont(DSKY::wxGetApp().normal_font());
    if (!wxOSX)
        x_textctrl->SetBackgroundStyle(wxBG_STYLE_PAINT);
    y_textctrl->SetFont(DSKY::wxGetApp().normal_font());
    if (!wxOSX)
    ...
    static_text_x->SetFont(DSKY::wxGetApp().normal_font());
    static_text_x->SetBackgroundStyle(wxBG_STYLE_PAINT);
    static_text_y->SetFont(DSKY::wxGetApp().normal_font());
...
void SliderCtrl::BUILD()
{
    ...
    m_slider = new wxSlider(m_parent, wxID_ANY, def_val * m_scale, min * m_scale, max * m_scale, wxDefaultPosition,
                            size);
    m_slider->SetFont(DSKY::wxGetApp().normal_font());
    ...
    m_textctrl->SetFont(DSKY::wxGetApp().normal_font());
