...
double Camera::get_near_left() const
{
    ...
    default:
    case EType::Ortho:
        return -1.0 / m_projection_matrix.matrix()(0, 0) -
               0.5 * m_projection_matrix.matrix()(0, 0) * m_projection_matrix.matrix()(0, 3);
    }
}
...
double Camera::get_near_top() const
{
    ...
    default:
    case EType::Ortho:
        return 1.0 / m_projection_matrix.matrix()(1, 1) -
               0.5 * m_projection_matrix.matrix()(1, 1) * m_projection_matrix.matrix()(1, 3);
    }
}
...
void Camera::set_viewport(int x, int y, unsigned int w, unsigned int h, unsigned int fit_w)
{
    m_viewport = {x, y, int(w), int(h)};
    m_fit_width = fit_w > 0 ? int(fit_w) : int(w);
}
...
void Camera::apply_projection(double left, double right, double bottom, double top, double near_z, double far_z)
{
    ...
    case EType::Ortho:
    {
        m_projection_matrix.matrix() << 2.0 * inv_dx, 0.0, 0.0, -(left + right) * inv_dx, 0.0, 2.0 * inv_dy, 0.0,
            -(bottom + top) * inv_dy, 0.0, 0.0, -2.0 * inv_dz, -(near_z + far_z) * inv_dz, 0.0, 0.0, 0.0, 1.0;
        break;
    }
    case EType::Perspective:
    {
        m_projection_matrix.matrix() << 2.0 * near_z * inv_dx, 0.0, (left + right) * inv_dx, 0.0, 0.0,
...
void Camera::rotate_on_sphere(double delta_azimut_rad, double delta_zenit_rad, bool apply_limits)
{
    ...
    m_view_matrix.fromPositionOrientationScale(m_view_rotation * (-m_target) + translation, m_view_rotation,
                                               Vec3d(1., 1., 1.));
}
...
double Camera::calc_zoom_to_bounding_box_factor(const BoundingBoxf3 &box, double margin_factor) const
{
    ...
    return std::min((double) m_fit_width / dx, (double) m_viewport[3] / dy);
}
...
{
    m_zenit = 45.0f; // 45 degrees = angled view
    const double theta_rad = Geometry::deg2rad(-(double) m_zenit);
    const double phi_rad = Geometry::deg2rad(45.0); // 45 degrees rotation
    const double sin_theta = ::sin(theta_rad);
    const Vec3d camera_pos = m_target + m_distance * Vec3d(sin_theta * ::sin(phi_rad), sin_theta * ::cos(phi_rad),
                                                           ::cos(theta_rad));
