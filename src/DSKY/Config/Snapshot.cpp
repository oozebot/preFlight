...
#define PREFLIGHT_SNAPSHOTS_DIR "snapshots"
#define PREFLIGHT_SNAPSHOT_FILE "snapshot.ini"
...
void Snapshot::clear()
{
    ...
    this->preflight_version_captured = Semver::invalid();
    this->comment.clear();
    ...
    for (auto &section : tree)
    {
        if (section.first == "snapshot")
        {
            ...
            for (auto &kvp : section.second)
            {
                ...
                {
                    this->time_captured = Luminary::Utils::parse_iso_utc_timestamp(kvp.second.data());
                    if (this->time_captured == (time_t) -1)
                        throw_on_parse_error("invalid timestamp");
                }
                else if (kvp.first == "preflight_version_captured" || kvp.first == "slic3r_version_captured")
                {
                    // A snapshot written under the earlier key spelling keeps loading; that read is counted.
                    if (kvp.first != "preflight_version_captured")
                        DBG_COUNT_LOAD("CONFIG_LEGACY_VERSION_KEY");
                    auto semver = Semver::parse(kvp.second.data());
                    if (!semver)
                        throw_on_parse_error("invalid " + kvp.first + " semver");
                    this->preflight_version_captured = *semver;
                }
        ...
        {
            // Load the names of the active presets. Keys this build does not know (for example the
            // selections an older build wrote for a removed preset type) are skipped, not treated as
            // an error.
            for (auto &kvp : section.second)
            {
                if (kvp.first == "print")
        ...
        else if (boost::starts_with(section.first, group_name_vendor) &&
                 section.first.size() > group_name_vendor.size())
        {
            ...
            for (auto &kvp : section.second)
            {
                const bool legacy_version_key = kvp.first == "min_slic3r_version" || kvp.first == "max_slic3r_version";
                if (kvp.first == "version" || kvp.first == "min_preflight_version" ||
                    kvp.first == "max_preflight_version" || legacy_version_key)
                {
                    // Version of the vendor specific config bundle bundled with this snapshot. A snapshot
                    // written under the earlier key spellings keeps loading; that read is counted.
                    ...
                    else if (boost::starts_with(kvp.first, "min_"))
                        vc.version.min_preflight_version = *semver;
                    else
                        vc.version.max_preflight_version = *semver;
                }
...
void Snapshot::save_ini(const std::string &path)
{
    ...
    c << "time_captured = " << Luminary::Utils::iso_utc_timestamp(this->time_captured) << std::endl;
    c << "preflight_version_captured = " << this->preflight_version_captured.to_string() << std::endl;
    ...
    for (const VendorConfig &vc : this->vendor_configs)
    {
        ...
        c << "min_preflight_version = " << vc.version.min_preflight_version.to_string() << std::endl;
        c << "max_preflight_version = " << vc.version.max_preflight_version.to_string() << std::endl;
...
static constexpr auto snapshot_subdirs = {"print", "filament", "printer", "physical_printer", "vendor"};

// Perform a deep compare of the active print / filament / printer / physical_printer / vendor directories.
// Return true if the content of the current print / filament / printer / physical_printer / vendor directories
...
bool Snapshot::equal_to_active(const AppConfig &app_config) const
{
    ...
    boost::filesystem::path snapshot_dir = boost::filesystem::path(Luminary::data_dir()) / PREFLIGHT_SNAPSHOTS_DIR /
                                           this->id;
...
size_t SnapshotDB::load_db()
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(snapshots_dir))
        if (boost::filesystem::is_directory(dir_entry.status()))
        {
            ...
            boost::filesystem::path path_ini = dir_entry.path() / PREFLIGHT_SNAPSHOT_FILE;
            Snapshot snapshot;
            try
            {
            ...
            {
                errors_cummulative += std::string("Snapshot ID ") + snapshot.id +
                                      " does not match the snapshot directory " + dir_entry.path().filename().string() +
                                      "\n";
                continue;
            }
...
void SnapshotDB::update_preflight_versions(std::vector<Index> &index_db)
{
    for (Snapshot &snapshot : m_snapshots)
    {
        for (Snapshot::VendorConfig &vendor_config : snapshot.vendor_configs)
        {
            ...
            if (it != index_db.end())
            {
                ...
                {
                    vendor_config.version.min_preflight_version = it_version->min_preflight_version;
                    vendor_config.version.max_preflight_version = it_version->max_preflight_version;
                }
            }
        }
    }
                                         ...
                                         const boost::filesystem::path &path_dst)
{
    if (!boost::filesystem::is_directory(path_dst) && !boost::filesystem::create_directory(path_dst))
        throw Luminary::RuntimeError(std::string("preFlight was unable to create a directory at ") + path_dst.string());

    for (auto &dir_entry : boost::filesystem::directory_iterator(path_src))
        if (Luminary::is_ini_file(dir_entry))
            if (std::string error_message;
                copy_file(dir_entry.path().string(), (path_dst / dir_entry.path().filename()).string(), error_message,
                          false) != SUCCESS)
                throw Luminary::RuntimeError(format("Failed copying \"%1%\" to \"%2%\": %3%", path_src.string(),
                                                    path_dst.string(), error_message));
}
...
static void delete_existing_ini_files(const boost::filesystem::path &path)
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(path))
        if (boost::filesystem::is_regular_file(dir_entry.status()) &&
            boost::algorithm::iends_with(dir_entry.path().filename().string(), ".ini"))
            boost::filesystem::remove(dir_entry.path());
}

const Snapshot &SnapshotDB::take_snapshot(const AppConfig &app_config, Snapshot::Reason reason,
                                          const std::string &comment)
{
    ...
    Snapshot snapshot;
    // Snapshot header.
    snapshot.time_captured = Luminary::Utils::get_current_time_utc();
    snapshot.id = Luminary::Utils::iso_utc_timestamp(snapshot.time_captured);
    snapshot.preflight_version_captured = Luminary::SEMVER;
    ...
    for (const auto &vendor : app_config.vendors())
    {
        ...
        PresetBundle bundle;
        bundle.load_configbundle((data_dir / "vendor" / (cfg.name + ".ini")).string(),
        ...
        // Fill in the min/max application version from the config index, if possible.
        try
        {
            ...
            if (it != index.end())
            {
                cfg.version.min_preflight_version = it->min_preflight_version;
                cfg.version.max_preflight_version = it->max_preflight_version;
            }
        }
        catch (const std::runtime_error & /* err */)
        {
    ...
    catch (...)
    {
        if (boost::filesystem::is_directory(snapshot_dir))
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << "Failed taking snapshot and failed removing the snapshot directory "
                                         << snapshot_dir;
            }
        }
        throw;
...
const Snapshot &SnapshotDB::restore_snapshot(const std::string &id, AppConfig &app_config)
{
    ...
    throw Luminary::RuntimeError(std::string("Snapshot with id " + id + " was not found."));
}
...
SnapshotDB::const_iterator SnapshotDB::snapshot_with_vendor_preset(const std::string &vendor_name,
                                                                   const Semver &config_version)
{
    ...
    for (auto it = m_snapshots.begin(); it != m_snapshots.end(); ++it)
    {
        ...
        auto it_vendor_config = std::lower_bound(snapshot.vendor_configs.begin(), snapshot.vendor_configs.end(), key,
                                                 [](const Snapshot::VendorConfig &cfg1,
                                                    const Snapshot::VendorConfig &cfg2)
...
{
    boost::filesystem::path data_dir = boost::filesystem::path(Luminary::data_dir());
    boost::filesystem::path snapshots_dir = data_dir / PREFLIGHT_SNAPSHOTS_DIR;
    for (const boost::filesystem::path &path : {data_dir, snapshots_dir})
    {
        ...
        if (!boost::filesystem::is_directory(subdir) && !boost::filesystem::create_directory(subdir))
            throw Luminary::RuntimeError(std::string("preFlight was unable to create a directory at ") +
                                         subdir.string());
    }
...
SnapshotDB &SnapshotDB::singleton()
{
    ...
    if (!loaded)
    {
        try
        {
            ...
            // Update the min / max application versions compatible with the configurations stored inside the snapshots
            // based on the min / max application versions defined by the vendor specific config indices.
            instance.update_preflight_versions(index_db);
        }
...
const Snapshot *take_config_snapshot_report_error(const AppConfig &app_config, Snapshot::Reason reason,
                                                  const std::string &comment)
{
    try
    {
...
bool take_config_snapshot_cancel_on_error(const AppConfig &app_config, Snapshot::Reason reason,
                                          const std::string &comment, const std::string &message,
                                          Snapshot const **psnapshot)
{
    ...
    catch (std::exception &err)
    {
        RichMessageDialog dlg(static_cast<wxWindow *>(wxGetApp().mainframe),
                              _L("preFlight has encountered an error while taking a configuration snapshot.") + "\n\n" +
                                  from_u8(err.what()) + "\n\n" + from_u8(message),
                              _L("preFlight error"), wxYES_NO);
