...
public:
    ...
    // Which application version captured this snapshot?
    Semver preflight_version_captured = Semver::invalid();
    ...
    // of the configuration stored in the snapshot with the running application version.
    struct VendorConfig
    {
...
public:
    ...
    size_t load_db();
    void update_preflight_versions(std::vector<Index> &index_db);

    // Create a snapshot directory, copy the vendor config bundles, user print / filament / printer / physical_printer profiles,
    // create an index.
    const Snapshot &take_snapshot(const AppConfig &app_config, Snapshot::Reason reason,
...
bool take_config_snapshot_cancel_on_error(const AppConfig &app_config, Snapshot::Reason reason,
                                          ...
                                          Snapshot const **psnapshot = nullptr);
