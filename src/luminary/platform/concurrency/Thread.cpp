...
///|/
///|/ the worker-count policy folded in from utils.cpp:
...
// Live-updatable TBB thread cap. Pass count == 0 to release the cap entirely (full speed).
// The cap is destroyed and recreated when the user changes the preference, but never at process
// exit: TBB tears its scheduler down in the DLL's TLS-destructor phase, before the atexit table
// runs, and with a one-thread cap that teardown completes synchronously, so a static destructor
// releasing the global_control afterwards dereferences freed control storage (an access
// violation at exit). Leaking the object is harmless; TBB never touches it during shutdown.
#ifdef TBB_HAS_GLOBAL_CONTROL
static tbb::global_control *g_tbb_thread_cap = nullptr;
#else
static tbb::task_scheduler_init *g_tbb_thread_cap = nullptr;
#endif

void enforce_thread_count(const std::size_t count)
{
    delete g_tbb_thread_cap;
    g_tbb_thread_cap = nullptr;
#ifdef TBB_HAS_GLOBAL_CONTROL
    if (count > 0)
        g_tbb_thread_cap = new tbb::global_control(tbb::global_control::max_allowed_parallelism, count);
#else  // TBB_HAS_GLOBAL_CONTROL
    if (count > 0)
        g_tbb_thread_cap = new tbb::task_scheduler_init(count);
...
void name_tbb_thread_pool_threads_set_locale()
{
    ...
    // Fix for Linux hang when run with non standard task affinity
