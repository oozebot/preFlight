...
public:
    ...
    struct ToolChangeResult
    {
        ...
        Vec2f start_pos;
        // Last point, at which the normal G-code generator shall continue.
        // At this position the extruder is loaded and there is no Z-hop applied.
        Vec2f end_pos;
        ...
        // (leading to wipe tower skip issues) or do
    ...
    struct box_coordinates
    {
        box_coordinates(float left, float bottom, float width, float height)
            : ld(left, bottom), lu(left, bottom + height), rd(left + width, bottom), ru(left + width, bottom + height)
        {
        }
    ...
    WipeTower(const Vec2f &position, double rotation_deg, const PrintConfig &config,
    ...
    bool layer_finished() const { return m_current_layer_finished; }
...
private:
    ...
    const float Width_To_Nozzle_Ratio =
        1.25f; // desired line width (oval) in multiples of nozzle diameter - may not be actually neccessary to adjust
    ...
    int m_old_temperature =
        -1; // To keep track of what was the last temp that we set (so we don't issue the command when not neccessary)
    float m_travel_speed = 0.f;
    float m_infill_speed = 0.f;
    float m_perimeter_speed = 0.f;
    float m_first_layer_speed = 0.f;
    // The project's Max volumetric flow (mm3/s, 0 = none); clamps each extruder's filament flow.
    float m_print_volumetric_cap = 0.f;
    ...
    float m_perimeter_width =
        0.4f * Width_To_Nozzle_Ratio; // Width of an extrusion line, also a perimeter spacing for 100% infill.
    float m_extrusion_flow =
        0.038f; //0.029f;// Extrusion flow is derived from m_perimeter_width, layer height and filament diameter.
    ...
    struct WipeTowerInfo
    {
        struct ToolChange
        {
            ...
            ToolChange(size_t old, size_t newtool, float depth = 0.f, float ramming_depth = 0.f, float fwl = 0.f,
                       float wv = 0.f)
                : old_tool{old}
                , new_tool{newtool}
    ...
    std::vector<WipeTowerInfo>
        m_plan; // Stores information about all layers and toolchanges for the future wipe tower (filled by plan_toolchange(...))
    ...
    void toolchange_Unload(WipeTowerWriter &writer, const box_coordinates &cleaning_box,
                           const std::string &current_material, const int old_temperature, const int new_temperature);
