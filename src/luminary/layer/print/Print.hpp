...
#include "luminary/layer/tool_ordering/ToolOrdering.hpp"
...
enum PrintObjectStep : unsigned int
{
    ...
    posPerimeters,
    posPrepareInfill,
    // Bridging over sparse infill, infill combination and the infill-above-bridges split:
    // the fill-surface stages that read the fill parameters, re-run from the surfaces
    // posPrepareInfill stored.
    posBridgeOverInfill,
    posInfill,
    posIroning,
...
public:
    ...
    // Islands that left the Serpentine pattern for ordinary walls, counted from the parallel
    // perimeter pass; make_perimeters() turns the total into one notification per object.
    void record_serpentine_wall_fallback() const
    {
        m_serpentine_wall_fallbacks.fetch_add(1, std::memory_order_relaxed);
    }
    auto layers() const
    {
    ...
    // The seam placer's mesh visibility for aligned seams, kept across exports: it depends on
    // the mesh, the object transformation and the raycasting parameters only. Reset when the
    // slicing is invalidated; the seam placer compares the transformation and the parameters.
    struct SeamVisibilityCache
    {
        std::shared_ptr<const ModelInfo::Visibility> visibility;
        Transform3d trafo = Transform3d::Identity();
        size_t samples_count = 0;
        size_t decimation_target = 0;
        size_t sqr_rays = 0;
    };
    SeamVisibilityCache &seam_visibility_cache() const { return m_seam_visibility_cache; }
    ...
    bool has_brim() const
    {
        if (this->has_raft())
            return false;
        // Painted mouse ears don't use brim_width - each point has its own radius; without a
        // painted point there is nothing to generate.
        if (this->config().brim_type == btPainted)
            return !this->model_object()->brim_points.empty();
    ...
    // Removes the empty support layers the wipe tower inserted for its layers above every
    // object and support layer (id size_t(-1)); the tool ordering derives has_support from
    // non-empty fills, so they must not survive into the next wipe-tower step.
    void remove_synthetic_support_layers();
    ...
    // Every region the config can produce for this object; slicing may leave some of them empty.
...
private:
    ...
    // The steps a changed object or region key invalidates, from the invalidation table.
    bool table_invalidation_steps_for_key(const ConfigOptionResolver &old_config,
                                          const ConfigOptionResolver &new_config, const t_config_option_key &opt_key,
                                          std::vector<PrintObjectStep> &steps,
                                          std::vector<PrintStep> &print_steps) const;
    void prepare_infill();
    void prepare_bridge_over_infill();
    // Whether any region of any layer has a fill surface left after the surface typing.
    bool has_fill_surfaces() const;
    void clear_fills();
    void clear_fill_structures();
    void infill();
    void ironing();
    ...
    // Luminary::Point objects in scaled G-code coordinates
    ...
    mutable std::atomic<size_t> m_serpentine_wall_fallbacks{0};

    std::pair<FillAdaptive::OctreePtr, FillAdaptive::OctreePtr> m_adaptive_fill_octrees;
    FillLightning::GeneratorPtr m_lightning_generator;
    mutable SeamVisibilityCache m_seam_visibility_cache;
};
...
public:
    ...
    // The G-code export step is driven from outside the class (the generator starts and finishes
    // it, the processor reports warnings against it, both poll for cancellation), so the state
    // machine's protected entry points are exposed here instead of through friendship.
    bool set_gcode_export_started() { return this->set_started(psGCodeExport); }
    void set_gcode_export_done() { this->set_done(psGCodeExport); }
    using PrintBase::throw_if_canceled;
    using PrintBase::update_object_placeholders;
    using PrintBaseWithState<PrintStep, psCount>::active_step_add_warning;
    ...
    // If preview_data is not null, the preview_data is filled in for the G-code visualization (not used on the command line).
    ...
    // The steps a changed print-level key invalidates, from the invalidation table.
    bool table_invalidation_steps_for_key(const t_config_option_key &opt_key, std::vector<PrintStep> &steps,
    ...
    // Wall-clock limit for one script call (preprocessing or export); 0 disables it.
    void set_preprocessing_timeout_seconds(double v) { m_preprocessing_timeout_s.store(v, std::memory_order_relaxed); }
    double preprocessing_timeout_seconds() const { return m_preprocessing_timeout_s.load(std::memory_order_relaxed); }
#endif

    // Drops only the G-code export, so the next run regenerates the G-code and its preview data from
    // the cached slicing steps.
    bool invalidate_gcode_export() { return this->invalidate_step(psGCodeExport); }
...
private:
    ...
    // One synchronization pass of apply(); reports the shrinkage compensation the object
    // transformations were built with.
    ApplyStatus apply_once(const Model &model, DynamicPrintConfig new_full_config, std::vector<std::string> *warnings,
                           Vec3d &shrinkage_applied);
    ...
    std::atomic<double> m_preprocessing_timeout_s{300.0};
#endif
...
// The keys that have a row in the print-key and object-key invalidation tables, for the
// settings-spec check: a key without a row would fall on a catch-all.
