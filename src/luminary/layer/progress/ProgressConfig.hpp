...
// std::string is named below and this header has no other include: src/luminary uses no
// precompiled header, so the include cannot arrive from pchheader.hpp any more.
#include <string>
...
//
// Tuning:
// - Start with an estimate based on typical print complexity
// - Test with various models (simple, complex, with supports, and so on)
// - Adjust weights where progress feels stuck or jumps too fast
// - The total weight does not matter, only the ratio between the weights
//
...
struct ProgressConfig
{
    ...
    struct SlicingPhase
    {
        ...
        float prepare_infill = 17.0f;     // Surface typing, shells, external surfaces (about half of the
                                          // 33.12% the infill preparation measured as one step)
        float bridge_over_infill = 16.1f; // Bridging over infill, infill combination (the other half)
        ...
        float total_base() const
        {
            return prepare_layers + slice_volumes + process_sliced_regions + process_geometry + build_layer_graph +
                   perimeters + prepare_infill + bridge_over_infill + making_infill + skirt_brim;
        }
