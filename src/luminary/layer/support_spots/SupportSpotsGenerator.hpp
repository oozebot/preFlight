...
struct Params
{
    Params(const std::vector<std::string> &filament_types, float max_acceleration, int raft_layers_count,
           BrimType brim_type, float brim_width)
        : max_acceleration(max_acceleration)
        , raft_layers_count(raft_layers_count)
        , brim_type(brim_type)
    ...
    const float
        max_acceleration; // mm/s^2 ; max acceleration of object in XY -- should be applicable only to printers with bed slinger,
    ...
    const double filament_density =
        1.25e-3f; // g/mm^3  ; Common filaments are very lightweight, so precise number is not that important
    const double material_yield_strength =
        33.0f *
        1e6f; // (g*mm/s^2)/mm^2; 33 MPa is yield strength of ABS, which has the lowest yield strength from common materials.
    ...
    const float malformations_additive_conflict_extruder_force =
        65.0f * gravity_constant; // for areas with possible high layered curled filaments
    ...
    double get_bed_adhesion_yield_strength() const
    {
        ...
        {
            return 0.1 * 1e6; // estimated, not measured
        }
        else
        { //PLA default value - defensive approach, PLA has quite low adhesion
            return 0.02 * 1e6;
        }
    }
...
public:
    ...
    ObjectPart() = default;
    ObjectPart(const std::vector<const ExtrusionEntityCollection *> &extrusion_collections, const bool connected_to_bed,
               const coordf_t print_head_z, const coordf_t layer_height, const std::optional<Polygons> &brim);

    void add(const ObjectPart &other);

    void add_support_point(const Vec3f &position, float sticking_area);

    float compute_elastic_section_modulus(const Vec2f &line_dir, const Vec3f &extreme_point,
...
void estimate_supports_malformations(std::vector<SupportLayer *> &layers, float supports_flow_width,
                                     const Params &params);
void estimate_malformations(std::vector<Layer *> &layers, const Params &params);

// the boolean marks whether the issue is blocking or not
