...
namespace Luminary::ModelProcessing
{
...
void convert_to_multipart_object(Model &model, unsigned int /*max_extruders*/)
{
    ...
    // Only the name and the input file carry over; per object config of the sources is dropped.
    ...
    for (const ModelObject *o : model.objects)
        for (const ModelVolume *v : o->volumes)
        {
            ...
            auto copy_volume = [o, &counter](ModelVolume *new_v)
            {
                ...
                // 0 follows the object's extruder; a pinned value would outrank it for good
                new_v->config.set("extruder", 0);
                return new_v;
            };
...
size_t split(ModelVolume *volume, unsigned int /*max_extruders*/)
{
    ...
    const auto orig_color_mixing = volume->color_mixing_facets.get_data();
    const auto orig_color_mixing_palette = volume->color_mixing_palette;
    ...
    for (SplitResultMesh &sr : split_results)
    {
        ...
        vol->color_mixing_facets.assign_remapped(orig_color_mixing, sr.face_mapping);
        vol->color_mixing_palette = orig_color_mixing_palette;

        vol->set_offset(Vec3d::Zero());
        vol->center_geometry_after_creation();
        vol->translate(offset);
        vol->name = name + "_" + std::to_string(idx + 1);
        vol->config.set("extruder", 0); // follows the object's extruder
...
void split(ModelObject *object, ModelObjectPtrs *new_objects)
{
    for (ModelVolume *volume : object->volumes)
    {
        ...
        for (SplitResultMesh &sr : split_results)
        {
            // Degenerate parts break the code below, so drop them here.
            ...
            new_vol->color_mixing_facets.assign_remapped(orig_color_mixing, sr.face_mapping);
            new_vol->color_mixing_palette = orig_color_mixing_palette;
