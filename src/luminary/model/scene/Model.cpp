...
// tbb::parallel_for and tbb::blocked_range for the convex hull loop. They arrived through the
// precompiled header while this file sat in the retiring engine; Luminary uses none.
...
bool ModelObject::is_color_mixing_painted() const
{
    return std::any_of(this->volumes.cbegin(), this->volumes.cend(),
...
const BoundingBoxf3 &ModelObject::raw_bounding_box() const
{
    if (!m_raw_bounding_box_valid)
    {
        ...
        if (this->instances.empty())
            throw Luminary::InvalidArgument("Can't call raw_bounding_box() with no instances");
                               ...
                               TypeFilterFn type_filter)
{
    ...
    for (i_old = 0, i_new = 0; i_old < model_object_old.volumes.size() && i_new < model_object_new.volumes.size();)
    {
        ...
        if (mv_old.type() != mv_new.type() || mv_old.id() != mv_new.id())
            return true;
        // Compared by id and transform; a mesh edit produces a new volume id, so content is not compared.
...
{
    std::vector<ColorMixingRecipe> out = mv.color_mixing_palette;
    ...
    for (size_t s : actually_used)
    {
        ...
        if (i < palette.colors().size())
            ...
            DBG_COUNT("COLORMIX_RECIPE_SEEDED_BLACK"); // a painted state past the palette
        if (rec.rgb == new_rgb)
            continue;
        rec.rgb = new_rgb;
    }
...
{
    mv.color_mixing_palette = seeded_color_mixing_recipes(palette, mv);
}
