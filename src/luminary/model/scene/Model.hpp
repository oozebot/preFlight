...
{
public:
    // Attributes are defined by the AMF file format and are not used for any purpose here.
...
public:
    ...
    // Configuration parameters specific to a single ModelObject, overriding the global settings.
    ModelConfigObject config;
    ...
    // Checks if any of object volume is painted using the colour mixing gizmo.
    bool is_color_mixing_painted() const;
...
private:
    ...
    void serialize(Archive &ar)
    {
        ...
        ar(name, input_file, notes, instances, volumes, config_wrapper, layer_config_ranges,
           layer_heigth_profile_wrapper, brim_points, printable, origin_translation, m_bounding_box_approx,
...
public:
    ...
    // Low-resolution preview mesh for CSG boolean preview.
    ...
    // overriding the global settings and the ModelObject settings.
    ModelConfigObject config;
    ...
    // Returns -1 if the extruder ID is not applicable (support volumes).
...
// The volume's recipe table grown to its painted states and synced to the palette, without
// touching the volume (the slicer resolves foreign paint against it; the gizmo stores it).
std::vector<ColorMixingRecipe> seeded_color_mixing_recipes(const MixedColorPalette &palette, const ModelVolume &mv);
...
{
template<class Archive>
struct specialize<Archive, Luminary::ModelVolume, cereal::specialization::member_load_save>
{
};
template<class Archive>
struct specialize<Archive, Luminary::ModelConfigObject, cereal::specialization::member_serialize>
{
};
} // namespace cereal
