...
///|/ Copyright (c) Prusa Research 2016 - 2023 Lukáš Matěna @lukasmatena, Enrico Turri @enricoturri1966, Oleksandra Iushchenko @YuSanka, Vojtěch Bubník @bubnikv, Pavel Mikuš @Godrak, Lukáš Hejl @hejllukas, Filip Sykala @Jony01, David Kocík @kocikdav
///|/
...
std::vector<std::pair<double, unsigned int>> custom_tool_changes(const Info &custom_gcode_per_print_z,
    ...
    for (const Item &custom_gcode : custom_gcode_per_print_z.gcodes)
        if (custom_gcode.type == ToolChange)
        {
            ...
            custom_tool_changes.emplace_back(custom_gcode.print_z,
                                             static_cast<unsigned int>(size_t(custom_gcode.extruder) > num_extruders
                                                                           ? 1
...
std::vector<std::pair<double, unsigned int>> custom_color_changes(const Info &custom_gcode_per_print_z,
                                                                  size_t num_extruders)
{
    ...
    for (const Item &custom_gcode : custom_gcode_per_print_z.gcodes)
        if (custom_gcode.type == ColorChange)
        {
            ...
            {
                custom_color_changes.emplace_back(custom_gcode.print_z,
                                                  static_cast<unsigned int>(custom_gcode.extruder));
            }
        }
