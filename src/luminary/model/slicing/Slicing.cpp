...
SlicingParameters SlicingParameters::create_from_config(const PrintConfig &print_config,
                                                        const PrintObjectConfig &object_config, coordf_t object_height,
                                                        ...
                                                        const Vec3d &object_shrinkage_compensation)
{
    ...
    // Resolve repeating-decimal heights (1/3, 1/6, 1/7, 1/9) to their exact fraction so the
    ...
    coordf_t support_material_extruder_dmr = print_config.nozzle_diameter.get_at(
        object_config.support_material_extruder.value - 1);
    coordf_t support_material_interface_extruder_dmr = print_config.nozzle_diameter.get_at(
        object_config.support_material_interface_extruder.value - 1);
    ...
    if (object_config.support_material.value || params.base_raft_layers > 0)
    {
        ...
        params.min_layer_height =
            std::max(min_layer_height_from_nozzle(print_config, object_config.support_material_extruder),
                     min_layer_height_from_nozzle(print_config, object_config.support_material_interface_extruder));
        params.max_layer_height =
            std::min(max_layer_height_from_nozzle(print_config, object_config.support_material_extruder),
    ...
    else
    {
        for (unsigned int extruder_id : object_extruders)
        {
            params.min_layer_height = std::max(params.min_layer_height,
                                               min_layer_height_from_nozzle(print_config, extruder_id + 1));
            params.max_layer_height = std::min(params.max_layer_height,
                                               max_layer_height_from_nozzle(print_config, extruder_id + 1));
        }
    }
    ...
    if (!soluble_interface)
    {
        ...
        // Without interface layers, only No gap or Full layer height are valid options.
        // Half layer and Custom require interface layers for precise gap control.
        bool has_interface_layers = object_config.support_material_interface_layers.value > 0;

        switch (object_config.support_material_contact_distance.value)
        {
        case stcgNoGap:
            params.gap_support_object = 0.0;
            break;
        case stcgHalfLayer:
            // Half layer requires interface layers; snap to full layer without them
            params.gap_support_object = has_interface_layers ? params.layer_height * 0.5 : params.layer_height;
            break;
        case stcgFullLayer:
            params.gap_support_object = params.layer_height;
            break;
        case stcgCustom:
            // Custom requires interface layers; snap to full layer without them
            if (has_interface_layers)
            {
                params.gap_support_object = std::max(0.0, object_config.support_material_contact_distance_custom.value);
            }
            else
            {
                params.gap_support_object = (object_config.support_material_contact_distance_custom.value > 0)
                                                ? params.layer_height
                                                : 0.0;
            }
            break;
        ...
        // For half-layer gap, we don't offset Z position - the gap is created by thinner extrusion.
        // The bottom contact prints at object layer Z but with 50% height, creating a natural gap.
        switch (object_config.support_material_bottom_contact_distance.value)
        {
        case sbcgHalfLayer:
            params.gap_object_support = 0; // No Z offset - gap from thinner extrusion
            params.half_layer_bottom_gap = true;
            break;
        case sbcgFullLayer:
            params.gap_object_support = params.layer_height;
            params.half_layer_bottom_gap = false;
            break;
        case sbcgNoGap:
        default:
            params.gap_object_support = 0;
            params.half_layer_bottom_gap = false;
            break;
        }
    }
    ...
    if (params.base_raft_layers > 0)
    {
        ...
        params.interface_raft_layer_height = std::max(params.layer_height,
                                                      0.75 * support_material_interface_extruder_dmr);
        params.first_object_layer_bridging = false;
        params.contact_raft_layer_height = std::max(params.layer_height,
                                                    0.75 * support_material_interface_extruder_dmr);
    ...
    if (params.has_raft())
    {
        ...
        // The last raft layer is the contact layer; it is printed with the ordinary flow, not a bridging flow.
        ...
        else
        {
            ...
            params.raft_interface_top_z = params.raft_base_top_z + coordf_t(params.interface_raft_layers - 1) *
                                                       ...
                                                       const t_layer_config_ranges &layer_config_ranges)
{
    ...
    if (slicing_params.first_object_layer_height_fixed())
        ranges_non_overlapping.push_back(
            std::pair<t_layer_height_range, coordf_t>(t_layer_height_range(0.,
    ...
    for (t_layer_config_ranges::const_iterator it_range = layer_config_ranges.begin();
         it_range != layer_config_ranges.end(); ++it_range)
    {
        ...
        if (lo + EPSILON < hi)
            ...
            ranges_non_overlapping.push_back(
                std::pair<t_layer_height_range, coordf_t>(t_layer_height_range(lo, hi), height));
    }
...
std::vector<double> layer_height_profile_adaptive(const SlicingParameters &slicing_params, const ModelObject &object,
                                                  float quality_factor)
{
    ...
    while (print_z + EPSILON < slicing_params.object_print_z_uncompensated_height())
    {
        ...
        */
        ...
        print_z += height;
        // Snap to whole millimeters when very close (within 0.005mm)
    ...
    if (z_gap > 0.0)
    {
        ...
        layer_height_profile.push_back(
            std::clamp(z_gap, slicing_params.min_layer_height, slicing_params.max_layer_height));
    }
...
void adjust_layer_height_profile(const SlicingParameters &slicing_params, std::vector<coordf_t> &layer_height_profile,
                                 ...
                                 LayerHeightEditActionType action)
{
    ...
    std::pair<coordf_t, coordf_t> z_span_variable = std::pair<coordf_t, coordf_t>(
    ...
    assert(std::abs(layer_height_profile[layer_height_profile.size() - 2] -
                    slicing_params.object_print_z_uncompensated_height()) < EPSILON);
    ...
    case LAYER_HEIGHT_EDIT_ACTION_INCREASE:
        if (layer_thickness_delta > 0)
        {
            if (current_layer_height >= slicing_params.max_layer_height - EPSILON)
                return;
            layer_thickness_delta = std::min(layer_thickness_delta,
        ...
        else
        {
            if (current_layer_height <= slicing_params.min_layer_height + EPSILON)
                return;
            layer_thickness_delta = std::max(layer_thickness_delta,
                                             slicing_params.min_layer_height - current_layer_height);
        }
        break;
    ...
    case LAYER_HEIGHT_EDIT_ACTION_SMOOTH:
        ...
        layer_thickness_delta = std::min(layer_thickness_delta,
                                         std::abs(slicing_params.layer_height - current_layer_height));
    ...
    while (zz < hi)
    {
        ...
        coordf_t weight = std::abs(zz - z) < 0.5 * band_width ? (0.5 + 0.5 * cos(2. * M_PI * (zz - z) / band_width))
                                                              : 0.;
        switch (action)
        {
        case LAYER_HEIGHT_EDIT_ACTION_INCREASE:
    ...
    if (action == LAYER_HEIGHT_EDIT_ACTION_SMOOTH)
    {
        ...
        for (size_t i_round = 0; i_round < n_rounds; ++i_round)
        {
            ...
            for (size_t i = i_resampled_start; i < i_resampled_end; i += 2)
            {
                ...
                coordf_t t = std::abs(zz - z) < 0.5 * band_width
                                             ...
                                             const std::vector<coordf_t> &layer_height_profile)
{
    ...
    // The last layer may end slightly above the object top; the surplus slices empty.
    return out;
}
