...
// IsTriviallyCopyable, which the static_assert under SlicingParameters uses. It arrived through the
// Utils.hpp umbrella before; the piece that defines it is named directly.
...
// 1/N mm layer heights for N = 3, 6, 7, 9 are repeating decimals, so the user enters a
...
struct SlicingParameters
{
    ...
    static SlicingParameters create_from_config(const PrintConfig &print_config, const PrintObjectConfig &object_config,
    ...
    coordf_t object_print_z_uncompensated_height() const
    {
        return object_print_z_uncompensated_max - object_print_z_min;
    }
    ...
    // When true, bottom contacts need special layer height handling to maintain Z sync
    bool half_layer_bottom_gap{false};
...
static_assert(IsTriviallyCopyable<SlicingParameters>::value,
              "SlicingParameters class is not POD (and it should be - see constructor).");
...
inline bool equal_layering(const SlicingParameters &sp1, const SlicingParameters &sp2)
{
    ...
    return sp1.base_raft_layers == sp2.base_raft_layers && sp1.interface_raft_layers == sp2.interface_raft_layers &&
           ...
           sp1.soluble_interface == sp2.soluble_interface && sp1.gap_raft_object == sp2.gap_raft_object &&
           sp1.gap_object_support == sp2.gap_object_support && sp1.half_layer_bottom_gap == sp2.half_layer_bottom_gap &&
           sp1.gap_support_object == sp2.gap_support_object && sp1.raft_base_top_z == sp2.raft_base_top_z &&
           sp1.raft_interface_top_z == sp2.raft_interface_top_z && sp1.raft_contact_top_z == sp2.raft_contact_top_z &&
...
void serialize(Archive &archive, Luminary::t_layer_height_range &lhr)
{
    archive(lhr.first, lhr.second);
}
} // namespace cereal
