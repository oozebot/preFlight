...
{
template<>
struct hash<Luminary::FloatOrPercent>
{
    std::size_t operator()(const Luminary::FloatOrPercent &v) const noexcept
    {
...
template<>
struct hash<Luminary::Vec2d>
{
    std::size_t operator()(const Luminary::Vec2d &v) const noexcept
    {
...
template<>
struct hash<Luminary::Vec3d>
{
    std::size_t operator()(const Luminary::Vec3d &v) const noexcept
    {
        std::size_t seed = std::hash<double>{}(v.x());
        boost::hash_combine(seed, std::hash<double>{}(v.y()));
...
class ConfigurationError : public Luminary::RuntimeError
{
public:
...
public:
    ...
    // Hides the base get_at, which returns the stored unsigned char.
...
// Why a key is a property of the whole print rather than of one object. A print, filament or
// printer key that is in neither PrintObjectConfig nor PrintRegionConfig must carry one; the
// settings-spec check refuses a print-level key without it, so every new setting answers
// "project or object?" when it is created.
enum class ScopeReason
{
    None,      // a per-object key (or a key outside the FFF presets)
    Plate,     // one bed, one chamber, one skirt, one wipe tower, one arrangement
    Machine,   // firmware, hardware, per-extruder writer state
    Material,  // a property of the filament, not of the part
    File,      // one G-code file: name, comments, thumbnails, scripts, once-per-print values
    Stream,    // a whole-stream filter over the emitted sequence
    Travel,    // travels connect objects; a travel has no single owner
    LayerTime, // every object on a layer shares the layer time
    Mmu,       // multi-material purge, ramming and tool-change plumbing
    Editor,    // editing-time check, switch or preset identity; nothing generated depends on it
    Engine,    // physically per object, parked behind named engine work
    Ir,        // the fan family, waits for the toolpath IR
};
...
public:
    ...
    Luminary::clonable_ptr<const ConfigOption> default_value;
    void set_default_value(const ConfigOption *ptr)
    {
        this->default_value = Luminary::clonable_ptr<const ConfigOption>(ptr);
    }
    template<typename T>
    ...
    // Why the key is a property of the whole print; None for a per-object key.
    ScopeReason scope_reason = ScopeReason::None;
    ...
    Luminary::clonable_ptr<ConfigOptionEnumDef> enum_def;
...
private:
    void enum_def_new()
    {
        if (enum_def)
            ...
            enum_def = Luminary::clonable_ptr<ConfigOptionEnumDef>(new ConfigOptionEnumDef{});
    }
};
...
// The dynamic config is mostly used at the user interface layer.
...
// The static configuration stores are used during slicing and G-code generation for efficiency reasons,
