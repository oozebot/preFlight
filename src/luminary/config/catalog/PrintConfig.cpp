...
// "tree" is not here: handle_legacy migrates it to "organic" with a counter before a value is parsed,
// so nothing can reach the retired style through a name.
...
static t_config_enum_values s_keys_map_PerimeterGeneratorType{
    ...
    {"classic", int(PerimeterGeneratorType::Athena)} // Legacy alias for Athena
};
...
PrintConfigDef::PrintConfigDef()
{
    ...
    // Scope reasons name keys from every group, so this runs once all of them exist.
    this->init_scope_reasons();
    this->finalize();
}
...
void PrintConfigDef::init_common_params()
{
    ...
    def->tooltip = L(
        "Picture sizes to be stored into a .gcode / .bgcode file, in the following format: \"XxY/EXT, XxY/EXT, ...\"\n"
    ...
    def->tooltip = L("preFlight uploads G-code to the printer host at this address. Enter a hostname, an IP address "
                     "or a full URL. Add the port when the host does not listen on the default one, for example "
                     "192.168.1.50:7126 for a second Moonraker instance or octopi.local:5000 for OctoPrint without "
                     "its proxy. A host behind HAProxy with basic authentication takes the user name and password "
                     "in the URL: https://username:password@your-host-address/");
    def->mode = comAdvanced;
    def->cli = ConfigOptionDef::nocli;
    def->set_default_value(new ConfigOptionString(""));

    // A Klipper host may serve its web interface apart from Moonraker; the Printer tab opens this address instead
    def = this->add("print_host_webui", coString);
    def->label = L("Web interface hostname, IP or URL");
    def->tooltip = L("Optional. The address the Printer tab opens when the web interface is served apart from "
                     "Moonraker, as on a multi-instance host where Mainsail or Fluidd is on port 80 and each "
                     "printer's Moonraker is on 7125, 7126 and so on. Enter a hostname, an IP address or a full "
                     "URL, with the port when it is not the default. Uploads always go to the host above. Leave "
                     "empty to open the host above in the Printer tab.");
    def->mode = comAdvanced;
    def->cli = ConfigOptionDef::nocli;
    def->set_default_value(new ConfigOptionString(""));
    ...
    def->tooltip = L("preFlight can upload G-code files to a printer host. This field should contain "
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip = L("This custom code is inserted at every layer change, right before the Z move. "
                     "Note that you can use placeholder variables for all preFlight settings as well "
    ...
    def->tooltip = L(
        "This code is inserted between objects when using sequential printing. By default extruder and bed temperature are reset using non-wait command; however if M104, M109, M140 or M190 are detected in this custom code, preFlight will not add temperature commands. Note that you can use placeholder variables for all preFlight settings, so you can put a \"M109 S[first_layer_temperature]\" command wherever you want.");
    def->multiline = true;
    def->full_width = true;
    def->height = 12;
    def->mode = comExpert;
    ...
    def->tooltip = L("When printing multiple objects or copies, this feature will complete "
                     ...
                     "preFlight warns about and prevents extruder collisions, but beware.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    // solid_fill_pattern is a key of its own (the internal solid pattern), so it is neither a
    // command-line nor a legacy alias of the top and bottom patterns.
    def->cli = "top-fill-pattern|external-fill-pattern";
    ...
    def->enum_def = Luminary::clonable_ptr<Luminary::ConfigOptionEnumDef>(def_top_fill_pattern->enum_def->clone());
    ...
    def->tooltip = L("Add more perimeters when needed for avoiding gaps in sloping walls. "
                     "preFlight keeps adding perimeters until more than 70% of the loop immediately above "
                     "is supported.");
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionBool(true));
    ...
    def->tooltip = L("Used only in the preFlight interface as a visual aid.");
    ...
    def->tooltip = L(
        ...
        "Before purging the print head into an infill or a sacrificial object, preFlight always primes "
    ...
    def->tooltip = L("Default base angle for infill orientation. Cross-hatching will be applied to this. "
                     "Bridges will be infilled using the best direction preFlight can detect, so this setting "
    ...
    def->tooltip = L("Speed limit for every feature type on the first layer, except solid infill when First layer "
                     "solid infill speed is set. As an absolute value in mm/s, nothing prints faster than this, "
                     "while features already set lower (small perimeters, gap fill, skirt and brim, supports) "
                     "keep their own speed. As a percentage (for example: 40%) it scales each feature's own "
                     "speed. Set to 0 for no limit.");
    def->sidetext = L("mm/s or %");
    def->min = 0;
    def->max_literal = 20;
    def->mode = comAdvanced;
    ...
    def->tooltip = L("Speed limit for solid infill on the first layer. As an absolute value in mm/s, first layer "
                     "solid infill never prints faster than this. As a percentage (for example: 40%) it is a "
                     "percentage of the solid infill speed. Set to 0 to fall back to the First layer speed limit.");
    def->sidetext = L("mm/s or %");
    def->min = 0;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0, false));
    ...
    def->tooltip = L("Speed limit for the first object layer above the raft interface. As an absolute value in "
                     "mm/s, nothing on that layer prints faster than this, while features already set lower keep "
                     "their own speed. As a percentage (for example: 40%) it scales each feature's own speed. "
    ...
    def->tooltip = L("This custom code is inserted at every layer change, right after the Z move "
                     ...
                     "placeholder variables for all preFlight settings as well as [layer_num] and [layer_z].");
    ...
    def->tooltip = L("preFlight will not slow down below this speed.");
    def->sidetext = L("mm/s");
    def->min = 0;
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionFloats{10.});
    ...
    def->tooltip = L("preFlight can upload G-code files to a printer host. This field must contain "
    ...
    def->tooltip = L("If you want to process the output G-code through custom scripts, "
                     ...
                     "and they can access the preFlight config settings by reading environment variables.");
    ...
    // Below half a width the fixed-depth V is a perpendicular stab and the Nip/Tuck clip on the
    // non-notched side approaches a full bead.
    def->min = 0.5;
    def->max = 3;
    def->mode = comAdvanced;
    ...
    def->category = L("Speed");
    def->tooltip = L("Speed for perimeters no larger than the small perimeter diameter (usually holes). "
                     ...
                     "Set to zero to calculate it from the filament max volumetric flow.");
    def->sidetext = L("mm/s or %");
    def->ratio_over = "perimeter_speed";
    def->min = 0;
    def->mode = comAdvanced;
    ...
    def = this->add("small_perimeter_diameter", coFloat);
    def->label = L("Small perimeter diameter");
    def->category = L("Speed");
    def->tooltip = L("A perimeter no longer than the circumference of a circle of this diameter is a small "
                     "perimeter and prints at the small perimeter speed. Set to zero to never apply the "
                     "small perimeter speed.");
    def->sidetext = L("mm");
    def->min = 0;
    def->set_default_value(new ConfigOptionFloat(13));
    ...
    def->tooltip = L("This feature allows to force a solid layer every given number of layers. "
                     ...
                     "preFlight will automatically choose the maximum possible number of layers "
    ...
    def->category = L("Support material");
    def->tooltip = L(
        "Only generate support on the build plate. Supports that cannot grow from the build plate are "
        "removed instead of resting on the object. Applies only to auto-generated Snug, Grid and Organic "
        "supports and to painted Organic supports. Painted Snug and Grid supports are always placed "
        "exactly where painted. Baobab supports use their own \"Plant trunks on object\" setting instead.");
    def->mode = comSimple;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def = this->add("support_alerts", coBool);
    def->label = L("Support alerts");
    def->category = L("Support material");
    def->tooltip = L("Analyze the stability of objects without automatic supports and report where supports are "
                     "needed. Turning this off skips the analysis and shortens every slice.");
    def->set_default_value(new ConfigOptionBool(true));
    ...
    def = this->add("support_baobab_trunk_consolidation", coFloat);
    def->label = L("Trunk consolidation");
    def->category = L("Support material");
    // TRN PrintSettings: "Baobab supports" > "Trunk consolidation"
    def->tooltip = L("Fewer, larger trunks at higher values; more, slimmer trunks at lower values. A trunk is "
                     "dropped when the interface it serves lies within this distance of another trunk's canopy, "
                     "and the support layer directly under the interface bridges the difference. Set to 0 to "
                     "build every trunk that serves any part of the interface; values larger than the support "
                     "region collapse it to a single trunk where one canopy and the bridged layer can hold it.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 100.f;
    ...
    def = this->add("support_baobab_min_opening", coFloat);
    def->label = L("Minimum opening");
    def->category = L("Support material");
    // TRN PrintSettings: "Baobab supports" > "Minimum opening"
    def->tooltip = L("Trunks and branches do not route through an opening in the object narrower than this, such as "
                     "a screw hole or a slot in a floor, even when they would fit through it with the XY separation. "
                     "Such an opening counts as solid while the tree looks for a way down, so it goes around it. A "
                     "cavity that is closed at the top, such as the inside of a cap, is not an opening and keeps its "
                     "support. Set to 0 to let the tree use any opening it fits through.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 100.f;
    ...
    def = this->add("support_tree_min_opening", coFloat);
    def->label = L("Minimum opening");
    def->category = L("Support material");
    // TRN PrintSettings: "Organic supports" > "Minimum opening"
    def->tooltip = L("Branches do not route through an opening in the object narrower than this, such as a screw "
                     "hole or a slot in a floor, even when a branch would fit through it with the XY separation. "
                     "Such an opening counts as solid while branches look for a way down, so the tree goes around it. "
                     "A cavity that is closed at the top, such as the inside of a cap, is not an opening and keeps "
                     "its support. Set to 0 to let branches use any opening they fit through.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 100.f;
    def->set_default_value(new ConfigOptionFloat(8));
    ...
    // TRN PrintSettings: label of an "Organic supports" setting
    ...
    def->tooltip = L("This experimental setting outputs the E values in cubic millimeters "
                     ...
                     "diameter of the filament selected in preFlight. The firmware must support "
                     "volumetric extrusion: Marlin and RepRapFirmware do, Klipper does not.");
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def = this->add("first_travel_combine_z", coBool);
    def->label = L("Combine Z into first travel");
    def->tooltip = L("Merges the first Z move and XY move into a single XYZ travel at the start of a print and after "
                     "each tool change that occurs at a layer change. Warning: this changes how your machine "
                     "approaches the print bed. Only enable it if your start and tool-change G-code leave the nozzle "
                     "raised.");
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def = this->add("min_wall_length", coFloatOrPercent);
    def->label = L("Minimum wall length");
    def->category = L("Advanced");
    def->tooltip = L(
        "A single thin wall (an unpaired bead between two features) shorter than this is not printed. "
        "Expressed as a percentage, it is measured against that wall's own width; the default of 50% keeps "
        "every wall longer than half its width. Raise it to drop the short stubs that drain nozzle pressure "
        "before a longer wall. The first and topmost layers keep the default so their surfaces do not open gaps.");
    def->sidetext = L("mm or %");
    def->min = 0;
    def->set_default_value(new ConfigOptionFloatOrPercent(50, true));
...
// Why each print-level FFF key is a property of the whole print. Every print, filament or printer
// key that is in neither PrintObjectConfig nor PrintRegionConfig gets one; the settings-spec check
// refuses a print-level key without.
void PrintConfigDef::init_scope_reasons()
{
    const auto set = [this](ScopeReason reason, std::initializer_list<const char *> keys)
    {
        for (const char *key : keys)
        {
            auto it = this->options.find(key);
            assert(it != this->options.end());
            if (it != this->options.end())
                it->second.scope_reason = reason;
        }
    };
    // One bed, one chamber, one skirt, one wipe tower, one arrangement
    set(ScopeReason::Plate, {"bed_temperature",
                             "chamber_temperature",
    ...
    // Firmware, hardware, per-extruder writer state, the machine's G-code blocks and identity
    set(ScopeReason::Machine, {"bed_shape",
                               "bed_custom_texture",
                               "bed_custom_model",
                               "extruder_colour",
                               "extruder_offset",
                               ...
                               "printhost_apikey",
                               "printhost_cafile"});
    // A property of the filament, not of the part
    set(ScopeReason::Material, {"filament_colour",
    ...
    set(ScopeReason::File, {"notes",
                            ...
                            "export_script",
                            "preprocessing_enabled_filament",
                            "preprocessing_enabled_print",
                            "preprocessing_enabled_printer",
                            "preprocessing_scripts_filament",
                            "preprocessing_scripts_print",
                            "preprocessing_scripts_printer",
                            "time_cost",
                            "currency_symbol"});
    // A whole-stream filter over the emitted sequence
    set(ScopeReason::Stream,
        {"max_volumetric_extrusion_rate_slope_positive", "max_volumetric_extrusion_rate_slope_negative"});
    // Travels connect objects
    set(ScopeReason::Travel, {"first_layer_travel_speed", "travel_acceleration", "travel_short_distance_acceleration",
                              "travel_speed", "travel_speed_z"});
    // Every object on a layer shares the layer time
    set(ScopeReason::LayerTime,
        {"cooling", "cooling_slowdown_logic", "cooling_perimeter_transition_distance", "fan_below_layer_time",
         "min_print_speed", "slowdown_below_layer_time", "dont_slow_down_outer_wall"});
    // Multi-material purge, ramming and tool-change plumbing
    set(ScopeReason::Mmu, {"filament_loading_speed",
    ...
    // Editing-time checks, switches and preset identity; nothing generated depends on them
    set(ScopeReason::Editor,
        {"nozzle_width_warning_min", "nozzle_width_warning_max", "print_nozzle_diameters", "print_high_flow_nozzle",
         "automatic_extrusion_widths", "inherits", "compatible_printers", "compatible_printers_condition",
         "compatible_prints", "compatible_prints_condition", "printer_technology", "printer_vendor", "printer_variant",
         "filament_vendor", "default_print_profile", "default_filament_profile"});
    // Physically per object, parked behind named engine work
    set(ScopeReason::Engine,
        {"first_layer_height", "first_layer_temperature", "spiral_vase", "temperature", "extrusion_multiplier",
         "filament_enable_pressure_advance", "filament_pressure_advance", "filament_shrinkage_compensation_x",
         "filament_shrinkage_compensation_y", "filament_shrinkage_compensation_z"});
    // The fan family, waits for the toolpath IR
    set(ScopeReason::Ir, {"bridge_fan_speed",
                          ...
                          "manual_fan_speed_serpentine",
                          "manual_fan_speed_serpentine_overhang",
                          ...
                          "fan_spinup_serpentine_overhang",
...
static std::set<std::string> PrintConfigDef_ignore = {
    ...
    // Replaced by interlock_perimeter_overlap; older projects and profiles still carry it.
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    else if (opt_key == "support_material_style" && value == "tree")
    {
        // The legacy tree drawer is retired: the UI has offered grid, snug, organic and baobab for
        // some time, but the name map still accepted "tree" from a saved project and reached the old
        // drawer. Such a project loads as organic, which is what the tree style grew into, and the
        // load is counted so a migration that surprises someone can be seen rather than guessed at.
        DBG_COUNT_LOAD("SUPPORT_STYLE_TREE_MIGRATED");
        value = "organic";
    }
    ...
    {
        // A legacy host-type value written by internal builds; it maps to the PrusaLink host type.
        value = "prusalink";
    }
    else if (opt_key == "fill_density" && value.find("%") == std::string::npos)
    {
        try
        {
            // fill_density is a percent (0-100). Legacy configurations stored it as a decimal (0-1).
    ...
    {
        // The pillars pattern is not supported.
        value = "rectilinear";
    }
    ...
    {
        // Legacy bool form: 1 meant "octoprint" and 0 meant no labelling.
    ...
    // External slicers leave an *_extrusion_width field blank when it is driven
...
void DynamicPrintConfig::normalize_fdm()
{
    ...
    // A sparse config (a CLI delta, an object's override set) carries no extruder count; the
    // extruder indices are validated once the full config is assembled.
    ...
    if (this->has("bed_temperature_extruder") && this->has("nozzle_diameter"))
    {
...
std::string DynamicPrintConfig::validate()
{
    ...
    {
        // preFlight prints FFF only. A config asking for another technology is counted and read as
        // FFF rather than refused, so an old project still opens and slices; printer_technology keeps
        // its definition and its SLA enum value so the rejected value can be named here.
        DBG_COUNT_LOAD("PRINTER_TECHNOLOGY_SLA_REJECTED");
        this->option<ConfigOptionEnum<PrinterTechnology>>("printer_technology", true)->value = ptFFF;
    }

    FullPrintConfig fpc;
    fpc.apply(*this, true);
    // Verify this print options through the FullPrintConfig.
    return Luminary::validate(fpc);
}

// Callers show the returned message to the user as it is, so each message is translated here.
std::string validate(const FullPrintConfig &cfg)
{
    // --layer-height
    if (cfg.get_abs_value("layer_height") <= 0)
        return _u8L("Invalid value for --layer-height");
    if (fabs(fmod(cfg.get_abs_value("layer_height"), SCALING_FACTOR)) > 1e-4)
        return _u8L("--layer-height must be a multiple of print resolution");

    // --first-layer-height
    if (cfg.first_layer_height.value <= 0)
        return _u8L("Invalid value for --first-layer-height");

    // --filament-diameter
    for (double fd : cfg.filament_diameter.values)
        if (fd < 1)
            return _u8L("Invalid value for --filament-diameter");

    // --nozzle-diameter
    for (double nd : cfg.nozzle_diameter.values)
        if (nd < 0.005)
            return _u8L("Invalid value for --nozzle-diameter");

    // --perimeters
    if (cfg.perimeters.value < 0)
        return _u8L("Invalid value for --perimeters");

    // --solid-layers
    if (cfg.top_solid_layers < 0)
        return _u8L("Invalid value for --top-solid-layers");
    if (cfg.bottom_solid_layers < 0)
        return _u8L("Invalid value for --bottom-solid-layers");
    ...
    if (cfg.use_firmware_retraction.value && cfg.gcode_flavor.value != gcfSmoothie &&
        ...
        return _u8L("--use-firmware-retraction is only supported by oozeBot Rapid, Marlin, Klipper, Smoothie, "
                    "RepRapFirmware, Repetier and Machinekit firmware");

    if (cfg.use_firmware_retraction.value)
        for (unsigned char wipe : cfg.wipe.values)
            if (wipe)
                return _u8L("--use-firmware-retraction is not compatible with --wipe");

    // --gcode-flavor
    if (!print_config_def.get("gcode_flavor")->has_enum_value(cfg.gcode_flavor.serialize()))
        return _u8L("Invalid value for --gcode-flavor");

    // --fill-pattern
    if (!print_config_def.get("fill_pattern")->has_enum_value(cfg.fill_pattern.serialize()))
        return _u8L("Invalid value for --fill-pattern");

    // --top-fill-pattern
    if (!print_config_def.get("top_fill_pattern")->has_enum_value(cfg.top_fill_pattern.serialize()))
        return _u8L("Invalid value for --top-fill-pattern");

    // --bottom-fill-pattern
    if (!print_config_def.get("bottom_fill_pattern")->has_enum_value(cfg.bottom_fill_pattern.serialize()))
        return _u8L("Invalid value for --bottom-fill-pattern");
    ...
    if (fabs(cfg.fill_density.value - 100.) < EPSILON &&
        ...
        return _u8L("The selected fill pattern is not supposed to work at 100% density");

    // --infill-every-layers
    if (cfg.infill_every_layers < 1)
        return _u8L("Invalid value for --infill-every-layers");

    // --skirt-height
    if (cfg.skirt_height < 0)
        return _u8L("Invalid value for --skirt-height");

    // --bridge-flow-ratio
    if (cfg.bridge_flow_ratio <= 0)
        return _u8L("Invalid value for --bridge-flow-ratio");

    // extruder clearance
    if (cfg.extruder_clearance_radius <= 0)
        return _u8L("Invalid value for --extruder-clearance-radius");
    if (cfg.extruder_clearance_height <= 0)
        return _u8L("Invalid value for --extruder-clearance-height");

    // --extrusion-multiplier
    for (double em : cfg.extrusion_multiplier.values)
        if (em <= 0)
            return _u8L("Invalid value for --extrusion-multiplier");
    ...
    if (cfg.spiral_vase)
    {
        ...
        // solid layers.
        if (cfg.perimeters > 1)
            return _u8L("Can't make more than one perimeter when spiral vase mode is enabled");
        else if (cfg.perimeters < 1)
            return _u8L("Can't make less than one perimeter when spiral vase mode is enabled");
        if (cfg.fill_density > 0)
            return _u8L("Spiral vase mode can only print hollow objects, so you need to set Fill density to 0");
        if (cfg.top_solid_layers > 0)
            return _u8L("Spiral vase mode is not compatible with top solid layers");
        if (cfg.support_material)
            return _u8L("Spiral vase mode is not compatible with support material");
    }
    ...
    // extrusion widths
    {
        ...
        for (size_t i = 0; i < sizeof(widths) / sizeof(widths[i]); ++i)
        {
            ...
            if (cfg.get_abs_value(key, max_nozzle_diameter) > 10. * max_nozzle_diameter)
                return format(_u8L("Invalid extrusion width (too large): %1%"), key);
        }
    }
    ...
    for (const std::string &opt_key : cfg.keys())
    {
        ...
        if (out_of_range)
            return format(_u8L("Value out of range: %1%"), opt_key);
    }
...
#define PRINT_CONFIG_CACHE_ELEMENT_DEFINITION(r, data, CLASS_NAME) \
    StaticPrintConfig::StaticCache<class Luminary::CLASS_NAME> BOOST_PP_CAT(CLASS_NAME::s_cache_, CLASS_NAME);
#define PRINT_CONFIG_CACHE_ELEMENT_INITIALIZATION(r, data, CLASS_NAME) Luminary::CLASS_NAME::initialize_cache();
...
CLIInputConfigDef::CLIInputConfigDef()
{
    ...
    def->tooltip = ("Name(s) of the filament preset(s) used for slicing.");
    def->set_default_value(new ConfigOptionStrings());
}
...
CLIActionsConfigDef::CLIActionsConfigDef()
{
    ...
    //! slice/export_gcode is the same action
    ...
    // Developer-facing; deliberately not translated.
    ...
    def = this->add("check_settings_spec", coBool);
    def->label = "Check the settings row specification";
    def->tooltip = "Verify that every settable key has exactly one row in the settings specification or a "
                   "recorded reason to have none; prints each violation and exits with their count.";
    def->cli = "check-settings-spec";
    def->set_default_value(new ConfigOptionBool(false));
}
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    // Developer and automation switch; deliberately not translated. The GUI collects preprocessing consent
    // through its dialog; the console never reads that setting and runs scripts only on this explicit request.
    def = this->add("allow-scripts", coBool);
    def->label = "Allow preprocessing scripts";
    def->tooltip = "Run the Python preprocessing scripts enabled in the loaded profiles (print, filament, printer "
                   "order; relative script paths resolve against the resources directory, then the project "
                   "directory). Without this flag configured scripts are skipped with a warning.";
    def->cli = "allow-scripts";
    def->set_default_value(new ConfigOptionBool(false));

    def = this->add("script-timeout", coFloat);
    def->label = "Preprocessing script timeout";
    def->tooltip = "Wall-clock limit in minutes for one preprocessing or export script call; a script past the "
                   "limit is interrupted and the slice fails with an error. 0 disables the limit. Default 5.";
    def->cli = "script-timeout";
    def->min = 0;
    def->set_default_value(new ConfigOptionFloat(5.0));
    ...
    // Developer-facing companions of --debug; deliberately not translated.
    def = this->add("debug-z", coString);
    def->label = "Debug output z window";
    def->tooltip = "Restrict --debug output to lines stamped inside this print z window: a single z "
                   "(--debug-z 27.3) or a range (--debug-z 27.1-27.4). Run-scoped lines (z 0) always pass.";
    def->cli = "debug-z";
    def->set_default_value(new ConfigOptionString());

    def = this->add("debug-geom", coBool);
    def->label = "Debug geometry sidecar";
    def->tooltip = "With a CLI export, write the pipeline geometry that the enabled --debug categories dump "
                   "as WKT records to <output>.geom.wkt (one G|z|category.tag|attrs|WKT record per line).";
    def->cli = "debug-geom";
    def->set_default_value(new ConfigOptionBool(false));

    // Developer-facing invalidation probe; deliberately not translated.
    def = this->add("reslice", coString);
    def->label = "Re-slice with a config delta";
    def->tooltip = "After the export, apply the key = value lines of this config file on top of the print "
                   "configuration, run the invalidation and a second process and export to <output>.re.gcode, "
                   "and print [RESLICE] lines naming the steps the change invalidated and the phase times. A "
                   "fresh slice with the same delta must be byte-identical to the re-sliced file.";
    def->cli = "reslice";
    def->set_default_value(new ConfigOptionString());
...
#if (defined(_MSC_VER) || defined(__MINGW32__)) && defined(PREFLIGHT_GUI)
...
CEREAL_REGISTER_TYPE(Luminary::DynamicPrintConfig)
CEREAL_REGISTER_POLYMORPHIC_RELATION(Luminary::DynamicConfig, Luminary::DynamicPrintConfig)
