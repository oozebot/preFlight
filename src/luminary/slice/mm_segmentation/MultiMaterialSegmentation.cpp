...
#include "luminary/geometry/voronoi/VoronoiUtils.hpp"
...
#include "luminary/geometry/medial_axis/VoronoiOffset.hpp"
...
inline double calc_triangle_area(const Point &a_pt, const Point &b_pt, const Point &c_pt)
{
    ...
    return Luminary::cross2(ba, ca) / 2.;
}
                                                        ...
                                                        const size_t num_facets_states, const size_t layer_idx)
{
    ...
    for (auto &segmented_expolygons : segmented_expolygons_per_extruder)
    {
        ...
        segmented_expolygons = Luminary::closing_ex(segmented_expolygons, static_cast<float>(SCALED_EPSILON));
    }
...
// Determinism fingerprint (debug only) of the top/bottom stage output, order-insensitive per
// layer and state.
static void dbg_state_expolys_fp(const char *tag, size_t layer_idx, double z, size_t slot,
                                 const std::vector<std::vector<ExPolygons>> &by_state)
{
    if (!debug_enabled(DBG_PERIMETERS))
        return;
    for (size_t s = 0; s < by_state.size(); ++s)
    {
        if (slot >= by_state[s].size() || by_state[s][slot].empty())
            continue;
        std::vector<uint64_t> hs;
        for (const ExPolygon &ep : by_state[s][slot])
        {
            uint64_t h = 1469598103934665603ull;
            for (const Point &pt : ep.contour.points)
                h = (h ^ uint64_t(pt.x())) * 1099511628211ull, h = (h ^ uint64_t(pt.y())) * 1099511628211ull;
            for (const Polygon &hole : ep.holes)
        ...
        dbg_log(DBG_PERIMETERS, z, "DETERM", "%s layer=%zu state=%zu polys=%zu hash=%016llx", tag, layer_idx, s,
                by_state[s][slot].size(), (unsigned long long) h);
    }
}
    ...
    const size_t num_facets_states, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    for (size_t i = 0; i < print_object.num_printing_regions(); ++i)
    {
        ...
        // A layer writes its top surface into the top_solid_layers layers below it and its
        // bottom surface into the bottom_solid_layers layers above it, inclusive. The group
        // size must cover that reach so a group only ever writes into its own half and its
        // neighbour's, never into a group two away that shares its half.
        granularity = std::max(granularity, std::max(config.top_solid_layers.value, config.bottom_solid_layers.value));
    }
    ...
    // Filter out polygons less than 0.1mm^2, because they are unprintable and cause dimples on outer primers
    filter_out_small_polygons(top_raw, Luminary::sqr(POLYGON_FILTER_MIN_AREA_SCALED));
    filter_out_small_polygons(bottom_raw, Luminary::sqr(POLYGON_FILTER_MIN_AREA_SCALED));
    ...
    if constexpr (MM_SEGMENTATION_DEBUG_TOP_BOTTOM)
    {
        ...
        {
            std::vector<std::pair<Luminary::ExPolygons, SVG::ExPolygonAttributes>> svg;
            for (size_t extruder_idx = 0; extruder_idx < num_facets_states; ++extruder_idx)
            {
    ...
    // Layers are processed in groups of `granularity` layers. A group writes up to
    // granularity - 1 layers beyond its own range, so consecutive groups write into
    // alternating halves of the output and never touch the same layer at once. That holds
    // only when every parallel chunk is exactly one group: a blocked_range over layers is
    // split by halving, its chunk boundaries drift off the group grid, and two neighbouring
    // chunks can share a half and append to the same layer concurrently. The range is
    // therefore over groups; a chunk of several groups runs them in order.
    const size_t num_groups = (num_layers + size_t(granularity) - 1) / size_t(granularity);
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, num_groups, 1),
        [&granularity, &num_layers, &num_facets_states, &layer_color_stat, &top_raw, &triangles_by_color_top,
         ...
         &triangles_by_color_bottom](const tbb::blocked_range<size_t> &groups)
        {
            for (size_t group_idx = groups.begin(); group_idx < groups.end(); ++group_idx)
            {
                    ...
                    for (size_t color_idx = 0; color_idx < num_facets_states; ++color_idx)
                    {
                            ...
                            if (ExPolygons top_ex = union_ex(top[layer_idx]); !top_ex.empty())
                            {
                                ...
                                if (!top_ex.empty())
                                {
                                    ...
                                    for (int last_idx = int(layer_idx) - 1;
                                         last_idx >= std::max(int(layer_idx - stat.top_solid_layers), int(0));
                                         --last_idx)
                                    {
                                        offset -= stat.extrusion_width;
                                        layer_slices_trimmed = intersection_ex(layer_slices_trimmed,
                                                                               input_expolygons[last_idx]);
                                        ExPolygons last = intersection_ex(top_ex,
                            ...
                            !bottom.empty() && !bottom[layer_idx].empty())
                        {
                            if (ExPolygons bottom_ex = union_ex(bottom[layer_idx]); !bottom_ex.empty())
                            {
                                ...
                                if (!bottom_ex.empty())
                                {
                                    append(triangles_by_color_bottom[color_idx][layer_idx + layer_idx_offset],
                                           bottom_ex);
                                    float offset = 0.f;
                                    ExPolygons layer_slices_trimmed = input_expolygons[layer_idx];
                                    for (size_t last_idx = layer_idx + 1;
                                         last_idx < std::min(layer_idx + stat.bottom_solid_layers, num_layers);
                                         ++last_idx)
                                    {
                                        offset -= stat.extrusion_width;
                                        layer_slices_trimmed = intersection_ex(layer_slices_trimmed,
    ...
    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
        dbg_state_expolys_fp("SEG_TB_OUT_FP", layer_idx, double(zs[layer_idx]), layer_idx, triangles_by_color_merged);
            ...
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                for (size_t extruder_id = 1; extruder_id < num_facets_states; ++extruder_id)
                {
                    ...
                    if (!top_and_bottom_layers.empty() && !top_and_bottom_layers[extruder_id][layer_idx].empty())
                    {
                        ...
                        // Remove dimples appearing after merging side segmentation of the model with tops and bottoms painted layers.
...
static ColorLines flatten_color_lines(const std::vector<ColorLines> &color_polygons_lines)
{
    ...
    {
        Luminary::append(color_lines_out, color_lines);
    }
                                                        ...
                                                        const double max_distance, const double angle_threshold)
{
    ...
    const double max_distance_sqr = Luminary::sqr(max_distance);
...
inline void update_color_changes_using_color_projection_ranges(ColorProjectionLine &projection_line)
{
    ...
    Luminary::sort_remove_duplicates(ranges);
    ...
    Luminary::sort_remove_duplicates(event_points);
    ...
    const std::vector<float> &layer_zs, const PrintObject &print_object, const size_t num_facets_states)
{
    ...
    for (size_t layer_idx = 0; layer_idx < color_polygons_per_layer.size() && layer_idx < layer_zs.size(); ++layer_idx)
        if (!color_polygons_per_layer[layer_idx].empty())
        {
            DBG_COUNT("SEG_PAINTED");
            dbg_log(DBG_PERIMETERS, double(layer_zs[layer_idx]), "DETERM", "SEG_PAINTED layer=%zu polys=%zu", layer_idx,
                    color_polygons_per_layer[layer_idx].size());
        }
    ...
    ColorProjectionLines &color_projection_lines, const AABBTreeLines::LinesDistancer<ColorLine> &color_lines_distancer)
{
    for (ColorProjectionLine &projection_line : color_projection_lines)
    {
        ...
        Luminary::append(nearest_color_line_indices,
        ...
        Luminary::append(nearest_color_line_indices,
                         color_lines_distancer.all_lines_in_radius(projection_line.b,
                                                                   MM_SEGMENTATION_MAX_PROJECTION_DISTANCE_SCALED));
        Luminary::sort_remove_duplicates(nearest_color_line_indices);
    ...
    const AABBTreeLines::LinesDistancer<ColorProjectionLineWrapper> &color_projection_lines_distancer)
{
    for (const ColorLines &color_polygon : color_polygons_lines)
    {
        for (const ColorLine &color_line : color_polygon)
        {
            ...
            Luminary::append(nearest_projection_line_indices,
            ...
            Luminary::sort_remove_duplicates(nearest_projection_line_indices);
...
// Determinism fingerprint (debug only) of a layer's coloured slice: the polygons are hashed
// in a canonical order, so two runs on the same input must print the same line.
static void dbg_color_polygons_fp(const char *tag, size_t layer_idx, double z, const ColorPolygons &polys)
{
    if (!debug_enabled(DBG_PERIMETERS) || polys.empty())
        return;
    std::vector<uint64_t> hs;
    hs.reserve(polys.size());
    size_t pts = 0;
    for (const ColorPolygon &cp : polys)
    {
        uint64_t h = 1469598103934665603ull;
        for (const Point &pt : cp.points)
    ...
    std::sort(hs.begin(), hs.end());
    uint64_t sorted = 1469598103934665603ull;
    for (uint64_t h : hs)
        sorted = (sorted ^ h) * 1099511628211ull;
    dbg_log(DBG_PERIMETERS, z, "DETERM", "%s layer=%zu polys=%zu pts=%zu hash=%016llx", tag, layer_idx, polys.size(),
            pts, (unsigned long long) sorted);
}

// Order-insensitive determinism fingerprints (debug only) of the segmentation stages.
static inline uint64_t fp_mix(uint64_t h, uint64_t v)
{
    return (h ^ v) * 1099511628211ull;
}
static uint64_t fp_sorted(std::vector<uint64_t> &hs)
{
    std::sort(hs.begin(), hs.end());
    uint64_t h = 1469598103934665603ull;
    for (uint64_t v : hs)
...
static void dbg_color_lines_fp(size_t layer_idx, double z, const std::vector<ColorLines> &polys)
{
    if (!debug_enabled(DBG_PERIMETERS) || polys.empty())
        return;
    std::vector<uint64_t> hs;
    for (const ColorLines &lines : polys)
        for (const ColorLine &l : lines)
            hs.push_back(
                fp_mix(fp_mix(fp_mix(fp_mix(fp_mix(1469598103934665603ull, uint64_t(l.a.x())), uint64_t(l.a.y())),
                                     uint64_t(l.b.x())),
                              uint64_t(l.b.y())),
                       uint64_t(l.color)));
    dbg_log(DBG_PERIMETERS, z, "DETERM", "SEG_LINES_FP layer=%zu lines=%zu hash=%016llx", layer_idx, hs.size(),
            (unsigned long long) fp_sorted(hs));
}
static void dbg_projection_fp(size_t layer_idx, double z, const ColorProjectionExPolygons &expolys)
{
    if (!debug_enabled(DBG_PERIMETERS) || expolys.empty())
        return;
    std::vector<uint64_t> hs;
    auto line_hash = [&](const ColorProjectionLine &pl)
    {
        uint64_t h = fp_mix(fp_mix(fp_mix(fp_mix(1469598103934665603ull, uint64_t(pl.a.x())), uint64_t(pl.a.y())),
                                   uint64_t(pl.b.x())),
                            uint64_t(pl.b.y()));
        std::vector<uint64_t> rs;
        for (const ColorProjectionRange &r : pl.color_projection_ranges)
            rs.push_back(fp_mix(fp_mix(fp_mix(1469598103934665603ull, uint64_t(std::llround(r.from_t * 1e6))),
                                       uint64_t(std::llround(r.to_t * 1e6))),
                                uint64_t(r.color)));
        return fp_mix(h, fp_sorted(rs));
    };
    for (const ColorProjectionExPolygon &ep : expolys)
    {
        for (const ColorProjectionLine &pl : ep.contour)
            hs.push_back(line_hash(pl));
        for (const ColorProjectionLines &hole : ep.holes)
            for (const ColorProjectionLine &pl : hole)
                hs.push_back(line_hash(pl));
    }
    dbg_log(DBG_PERIMETERS, z, "DETERM", "SEG_PROJ_FP layer=%zu lines=%zu hash=%016llx", layer_idx, hs.size(),
            (unsigned long long) fp_sorted(hs));
}
static void dbg_buckets_fp(const char *tag, size_t layer_idx, double z, const std::vector<ExPolygons> &buckets)
{
    if (!debug_enabled(DBG_PERIMETERS))
        return;
    for (size_t b = 0; b < buckets.size(); ++b)
    {
        ...
        std::vector<uint64_t> hs;
        double area = 0.;
        for (const ExPolygon &ep : buckets[b])
        {
            area += ep.area();
            uint64_t h = 1469598103934665603ull;
            for (const Point &pt : ep.contour.points)
                h = fp_mix(fp_mix(h, uint64_t(pt.x())), uint64_t(pt.y()));
            for (const Polygon &hole : ep.holes)
                for (const Point &pt : hole.points)
                    h = fp_mix(fp_mix(h, uint64_t(pt.x())), uint64_t(pt.y()));
            hs.push_back(h);
        }
        dbg_log(DBG_PERIMETERS, z, "DETERM", "%s layer=%zu bucket=%zu polys=%zu area=%.4fmm2 hash=%016llx", tag,
                layer_idx, b, buckets[b].size(), area * 1e-12, (unsigned long long) fp_sorted(hs));
    }
}
    ...
    const std::function<void()> &throw_on_cancel_callback, const bool preserve_holes)
{
         ...
         &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                for (LayerRegion *region : layers[layer_idx]->regions())
                {
                    for (const Surface &surface : region->slices())
                    {
                        Luminary::append(ex_polygons, offset_ex(surface.expolygon, float(10 * SCALED_EPSILON)));
                    }
                }
                ...
                remove_small_and_small_holes(ex_polygons, Luminary::sqr(POLYGON_FILTER_MIN_AREA_SCALED));
    ...
    for (const ModelVolume *mv : print_object.model_object()->volumes)
    {
        std::vector<ColorPolygons> color_polygons_per_layer = slice_model_volume_with_color(*mv, extract_facets_info,
                                                                                            ...
                                                                                            num_facets_states);
        for (size_t layer_idx = 0; layer_idx < color_polygons_per_layer.size() && layer_idx < num_layers; ++layer_idx)
            dbg_color_polygons_fp("SEG_SLICED_FP", layer_idx, layers[layer_idx]->print_z,
                                  color_polygons_per_layer[layer_idx]);
    ...
    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
        dbg_color_lines_fp(layer_idx, layers[layer_idx]->print_z, color_polygons_lines_layers[layer_idx]);
    ...
    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
        dbg_projection_fp(layer_idx, layers[layer_idx]->print_z, input_expolygons_projection_lines_layers[layer_idx]);
                ...
                for (ColorProjectionExPolygon &input_expolygon_projection_lines : input_expolygons_projection_lines)
                {
                    ...
                    else
                    {
                        ...
                        for (size_t state_idx = 0; state_idx < num_facets_states; ++state_idx)
                        {
                            ...
                            Luminary::append(segmented_regions[layer_idx][state_idx],
    ...
    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
        dbg_buckets_fp("SEG_LAYERS_FP", layer_idx, layers[layer_idx]->print_z, segmented_regions[layer_idx]);
    throw_on_cancel_callback();
    ...
    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
        dbg_buckets_fp("SEG_MERGED_FP", layer_idx, layers[layer_idx]->print_z, segmented_regions_merged[layer_idx]);
    throw_on_cancel_callback();
    ...
    const PrintObject &print_object, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    // Unlike MMU painted regions, painted fuzzy skin does not take over the slice geometry. The
    // painted areas are stored in Layer::fuzzy_skin_painted_areas and used to create
    // PerimeterRegions in make_perimeters(), which applies fuzzy skin to perimeters via
    // polygon_segmentation() without modifying the underlying slice geometry.
    //
    // preserve_holes=true keeps holes from expanding during the negative offset in
    // cut_segmented_layers(); without it, holes eat into painted regions on the outer surface at
    // large cut widths ("All perimeters" mode).
