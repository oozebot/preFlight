...
// Six member signatures below take an std::unordered_set, which this header names for itself.
...
private:
     ...
     */
    InterlockingGenerator(PrintObject &print_object, const size_t region_a_index, const size_t region_b_index,
                          const coord_t beam_width, const coord_t boundary_avoidance, const float rotation,
                          const Vec3crd &cell_size, const coord_t beam_layer_count,
                          const DilationKernel &interface_dilation, const DilationKernel &air_dilation,
     ...
     */
    std::pair<ExPolygons, ExPolygons> growBorderAreasPerpendicular(const ExPolygons &a, const ExPolygons &b,
     ...
     */
    void addBoundaryCells(const std::vector<ExPolygons> &layers, const DilationKernel &kernel,
     ...
     */
    void applyMicrostructureToOutlines(const std::unordered_set<GridPoint3> &cells,
