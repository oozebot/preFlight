...
struct DilationKernel
{
    ...
    std::vector<GridPoint3>
        relative_cells_; //!< All offset positions relative to some reference cell which is to be dilated
...
public:
     ...
     * \return Whether executing was stopped short as indicated by the \p cell_processing_function
     */
    bool walkPolygons(const ExPolygon &polys, coord_t z,
     ...
     * \return Whether executing was stopped short as indicated by the \p cell_processing_function
     */
    bool walkDilatedPolygons(const ExPolygon &polys, coord_t z, const DilationKernel &kernel,
                             const std::function<bool(GridPoint3)> &process_cell_func) const;
    bool walkDilatedPolygons(const ExPolygons &polys, coord_t z, const DilationKernel &kernel,
                             const std::function<bool(GridPoint3)> &process_cell_func) const
    {
...
private:
     ...
     */
    bool _walkAreas(const ExPolygon &ex_polygon, coord_t z,
                    const std::function<bool(GridPoint3)> &process_cell_func) const;
...
public:
     ...
     * \return Whether executing was stopped short as indicated by the \p cell_processing_function
     */
    bool walkDilatedAreas(const ExPolygon &polys, coord_t z, const DilationKernel &kernel,
                          const std::function<bool(GridPoint3)> &process_cell_func) const;
    bool walkDilatedAreas(const ExPolygons &polys, coord_t z, const DilationKernel &kernel,
                          const std::function<bool(GridPoint3)> &process_cell_func) const
    {
        for (const auto &poly : polys)
        {
     ...
     * \param process_cell_func Function to perform on each voxel cell
     */
    std::function<bool(GridPoint3)> dilate(const DilationKernel &kernel,
                                           const std::function<bool(GridPoint3)> &process_cell_func) const;
    ...
    {
        assert(dim < 3);
        // Floor division: step down one cell only when truncation left a negative remainder, so
        // an exact negative multiple of the cell size stays in the cell whose lower corner it is.
        return coord / cell_size_[dim] - (coord % cell_size_[dim] < 0);
    }
