...
{
template<>
struct hash<Luminary::GridPoint3>
{
    size_t operator()(const Luminary::GridPoint3 &pp) const noexcept
    {
        static int prime = 31;
        int result = 89;
...
void InterlockingGenerator::generate_interlocking_structure(PrintObject &print_object)
{
    ...
    const DilationKernel interface_dilation(GridPoint3(interface_depth, interface_depth, interface_depth),
                                            DilationKernel::Type::PRISM);

    const bool air_filtering = boundary_avoidance > 0;
    const DilationKernel air_dilation(GridPoint3(boundary_avoidance, boundary_avoidance, boundary_avoidance),
                                      DilationKernel::Type::PRISM);
    ...
    for (size_t region_a_index = 0; region_a_index < print_object.num_printing_regions(); region_a_index++)
    {
        ...
        for (size_t region_b_index = region_a_index + 1; region_b_index < print_object.num_printing_regions();
             region_b_index++)
        {
            ...
            InterlockingGenerator gen(print_object, region_a_index, region_b_index, beam_width, boundary_avoidance,
                                      rotation, cell_size, beam_layer_count, interface_dilation, air_dilation,
                                      air_filtering);
...
{
    const coord_t min_line = std::min(
        print_object.printing_region(region_a_index).flow(print_object, frExternalPerimeter, 0.1).scaled_width(),
...
void InterlockingGenerator::handleThinAreas(const std::unordered_set<GridPoint3> &has_all_meshes) const
{
    ...
    for (const auto &cell : has_all_meshes)
    {
        ...
        for (coord_t layer_nr = bottom_corner.z(); layer_nr < bottom_corner.z() + cell_size.z() &&
                                                   layer_nr < static_cast<coord_t>(near_interlock_per_layer.size());
             ++layer_nr)
        {
    ...
    for (size_t layer_nr = 0; layer_nr < print_object.layer_count(); layer_nr++)
    {
        ...
        const ExPolygons thin_expansion_a = offset_ex(
            intersection_ex(intersection_ex(intersection_ex(large_b, offset_ex(diff_ex(polys_a, large_a), expand)),
                                            near_interlock_per_layer[layer_nr]),
                            from_border_a),
            rounding_errors);
        const ExPolygons thin_expansion_b = offset_ex(
            intersection_ex(intersection_ex(intersection_ex(large_a, offset_ex(diff_ex(polys_b, large_b), expand)),
                                            near_interlock_per_layer[layer_nr]),
                            from_border_b),
            rounding_errors);
        ...
        layer->get_region(region_a_index)
            ->m_slices.set(closing_ex(diff_ex(union_ex(polys_a, thin_expansion_a), thin_expansion_b), close_gaps),
                           stInternal);
        layer->get_region(region_b_index)
            ->m_slices.set(closing_ex(diff_ex(union_ex(polys_b, thin_expansion_b), thin_expansion_a), close_gaps),
                           stInternal);
    }
}
...
void InterlockingGenerator::addBoundaryCells(const std::vector<ExPolygons> &layers, const DilationKernel &kernel,
                                             std::unordered_set<GridPoint3> &cells) const
{
    ...
    for (size_t layer_nr = 0; layer_nr < layers.size(); layer_nr++)
    {
        ...
        skin = opening_ex(
            skin, cell_size.x() /
                      2.f); // remove superfluous small areas, which would anyway be included because of walkPolygons
...
std::vector<std::vector<ExPolygons>> InterlockingGenerator::generateMicrostructure() const
{
    ...
    {
        Point offset(mesh_idx ? middle : coord_t(0), coord_t(0));
        Point area_size(width[mesh_idx], cell_size.y());

        Polygon poly;
        poly.append(offset);
        poly.append(offset + Point(area_size.x(), coord_t(0)));
        poly.append(offset + area_size);
        poly.append(offset + Point(coord_t(0), area_size.y()));
                                                          ...
                                                          const std::vector<ExPolygons> &layer_regions) const
{
    ...
    for (const GridPoint3 &grid_loc : cells)
    {
        ...
        for (size_t mesh_idx = 0; mesh_idx < 2; mesh_idx++)
        {
                 ...
                 layer_nr += beam_layer_count)
            {
                ...
                expolygons_append(structure_per_layer[mesh_idx][static_cast<size_t>(layer_nr / beam_layer_count)],
                                  areas_here);
            }
        }
    }
    ...
    for (size_t region_idx = 0; region_idx < 2; region_idx++)
    {
        ...
        for (size_t layer_nr = 0; layer_nr < max_layer_count; layer_nr++)
        {
            ...
            const ExPolygons areas_here = intersection_ex(
                structure_per_layer[region_idx][layer_nr / static_cast<size_t>(beam_layer_count)], layer_outlines);
            const ExPolygons &areas_other =
                structure_per_layer[!region_idx][layer_nr / static_cast<size_t>(beam_layer_count)];
