...
// Only the CONTOUR_DISTANCE_DEBUG_SVG and TESTS_EXPORT_SVGS blocks below name SVG, and both are
// off in every shipping build; the include is here so that defining either one still compiles.
...
std::vector<float> contour_distance(const EdgeGrid::Grid &grid, const size_t idx_contour,
                                    const Luminary::Points &contour,
