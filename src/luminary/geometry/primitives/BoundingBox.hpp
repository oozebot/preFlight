...
public:
    ...
    {
        if (from == to)
            throw Luminary::InvalidArgument("Empty point set supplied to BoundingBox3Base constructor");
    ...
    {
        return is_approx(this->min.x(), other.max.x()) || is_approx(this->max.x(), other.min.x()) ||
               is_approx(this->min.y(), other.max.y()) || is_approx(this->max.y(), other.min.y()) ||
               is_approx(this->min.z(), other.max.z()) || is_approx(this->max.z(), other.min.z());
    }
};
...
{
    return {scaled(bb.min), scaled(bb.max)};
}

template<class T = coord_t, class Tin>
BoundingBoxBase<Vec<2, T>> scaled(const BoundingBoxBase<Vec<2, Tin>> &bb)
{
    return {scaled<T>(bb.min), scaled<T>(bb.max)};
}

template<class T = coord_t>
BoundingBoxBase<Vec<2, T>> scaled(const BoundingBox &bb)
{
    return {scaled<T>(bb.min), scaled<T>(bb.max)};
}

template<class T = coord_t, class Tin>
BoundingBox3Base<Vec<3, T>> scaled(const BoundingBox3Base<Vec<3, Tin>> &bb)
{
    return {scaled<T>(bb.min), scaled<T>(bb.max)};
}
...
template<class T = double, class Tin>
BoundingBox3Base<Vec<3, T>> unscaled(const BoundingBox3Base<Vec<3, Tin>> &bb)
{
    return {unscaled<T>(bb.min), unscaled<T>(bb.max)};
}
...
{
    return BoundingBoxBase<Vec<3, Tout>>{b.min.template cast<Tout>(), b.max.template cast<Tout>()};
}
    ...
    if (pt.x() < bbox.min.x())
        return pt.y() < bbox.min.y()   ? (bbox.min - pt).cast<double>().squaredNorm()
               : pt.y() > bbox.max.y() ? (Point(bbox.min.x(), bbox.max.y()) - pt).cast<double>().squaredNorm()
                                       : Luminary::sqr(double(bbox.min.x() - pt.x()));
    else if (pt.x() > bbox.max.x())
        return pt.y() < bbox.min.y()   ? (Point(bbox.max.x(), bbox.min.y()) - pt).cast<double>().squaredNorm()
               : pt.y() > bbox.max.y() ? (bbox.max - pt).cast<double>().squaredNorm()
                                       : Luminary::sqr<double>(pt.x() - bbox.max.x());
    else
        return Luminary::sqr<double>(pt.y() < bbox.min.y()   ? bbox.min.y() - pt.y()
                                     : pt.y() > bbox.max.y() ? pt.y() - bbox.max.y()
                                                             : coord_t(0));
}
...
inline double bbox_bbox_distance(const BoundingBox &first_bbox, const BoundingBox &second_bbox)
{
    ...
    {
        const coord_t axis_distance = std::max(std::max(coord_t(0), first_bbox.min[axis] - second_bbox.max[axis]),
                                               second_bbox.min[axis] - first_bbox.max[axis]);
        bboxes_distance_squared += Luminary::sqr(static_cast<double>(axis_distance));
    }
...
void serialize(Archive &archive, Luminary::BoundingBox &bb)
{
    archive(bb.min, bb.max, bb.defined);
}
template<class Archive>
void serialize(Archive &archive, Luminary::BoundingBox3 &bb)
{
    archive(bb.min, bb.max, bb.defined);
}
template<class Archive>
void serialize(Archive &archive, Luminary::BoundingBoxf &bb)
{
    archive(bb.min, bb.max, bb.defined);
}
template<class Archive>
void serialize(Archive &archive, Luminary::BoundingBoxf3 &bb)
{
    archive(bb.min, bb.max, bb.defined);
}
} // namespace cereal
