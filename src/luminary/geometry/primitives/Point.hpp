...
// Eigen types, to replace the engine's own types in the future.
// Vector types with a fixed point coordinate base type.
using Vec2crd = Eigen::Matrix<coord_t, 2, 1, Eigen::DontAlign>;
using Vec3crd = Eigen::Matrix<coord_t, 3, 1, Eigen::DontAlign>;
// Vec2i/Vec3i/Vec4i now use coord_t (int64_t) instead of int for consistency
// If you specifically need 32-bit, use Vec2i32/Vec3i32 explicitly
using Vec2i = Eigen::Matrix<coord_t, 2, 1, Eigen::DontAlign>;
using Vec3i = Eigen::Matrix<coord_t, 3, 1, Eigen::DontAlign>;
using Vec4i = Eigen::Matrix<coord_t, 4, 1, Eigen::DontAlign>;
// Explicit 32-bit and 64-bit versions for when you need a specific size
...
template<int N, class T>
Transform<N, T> identity()
{
    return Transform<N, T>::Identity();
}
...
// With int32_t: overflows, so it is not used
// With int64_t: CAN overflow with very large coordinates, but acceptable risk
// For guaranteed no-overflow, would need int128_t (not available on MSVC) or use double
...
inline typename Derived::Scalar cross2(const Eigen::MatrixBase<Derived> &v1, const Eigen::MatrixBase<Derived2> &v2)
{
    static_assert(Derived::IsVectorAtCompileTime && int(Derived::SizeAtCompileTime) == 2,
                  "cross2(): first parameter is not a 2D vector");
    static_assert(Derived2::IsVectorAtCompileTime && int(Derived2::SizeAtCompileTime) == 2,
                  "cross2(): second parameter is not a 2D vector");
    static_assert(!std::is_same<typename Derived::Scalar, int32_t>::value,
                  "cross2(): Scalar type must not be int32_t, otherwise the cross product would overflow.");
...
{
    static_assert(Derived::IsVectorAtCompileTime && int(Derived::SizeAtCompileTime) == 2,
                  "perp(): parameter is not a 2D vector");
    return {-v.y(), v.x()};
}
...
{
    static_assert(Derived::IsVectorAtCompileTime && int(Derived::SizeAtCompileTime) == 2,
                  "angle(): first parameter is not a 2D vector");
    static_assert(Derived2::IsVectorAtCompileTime && int(Derived2::SizeAtCompileTime) == 2,
                  "angle(): second parameter is not a 2D vector");
...
{
    static_assert(Derived::IsVectorAtCompileTime && int(Derived::SizeAtCompileTime) == 2,
                  "to_3d(): first parameter is not a 2D vector");
    return {pt.x(), pt.y(), z};
}

inline Vec2d unscale(coord_t x, coord_t y)
{
    return Vec2d(unscale<double>(x), unscale<double>(y));
}
inline Vec2d unscale(const Vec2crd &pt)
{
    return Vec2d(unscale<double>(pt.x()), unscale<double>(pt.y()));
}
inline Vec2d unscale(const Vec2d &pt)
{
    return Vec2d(unscale<double>(pt.x()), unscale<double>(pt.y()));
}
inline Vec3d unscale(coord_t x, coord_t y, coord_t z)
{
    return Vec3d(unscale<double>(x), unscale<double>(y), unscale<double>(z));
}
inline Vec3d unscale(const Vec3crd &pt)
{
    return Vec3d(unscale<double>(pt.x()), unscale<double>(pt.y()), unscale<double>(pt.z()));
}
inline Vec3d unscale(const Vec3d &pt)
{
    return Vec3d(unscale<double>(pt.x()), unscale<double>(pt.y()), unscale<double>(pt.z()));
}
...
/// In code is sometime function named is_left_handed
/// </summary>
/// <param name="transform">Transformation to check</param>
/// <returns>Is positive determinant</returns>
inline bool has_reflection(const Transform3d &transform)
{
    return transform.matrix().determinant() < 0;
}
...
inline const Vec3d get_base(unsigned index, const Transform3d &transform)
{
    return transform.linear().col(index);
}
inline const Vec3d get_x_base(const Transform3d &transform)
{
    return get_base(0, transform);
}
inline const Vec3d get_y_base(const Transform3d &transform)
{
    return get_base(1, transform);
}
inline const Vec3d get_z_base(const Transform3d &transform)
{
    return get_base(2, transform);
}
inline const Vec3d get_base(unsigned index, const Transform3d::LinearPart &transform)
{
    return transform.col(index);
}
inline const Vec3d get_x_base(const Transform3d::LinearPart &transform)
{
    return get_base(0, transform);
}
inline const Vec3d get_y_base(const Transform3d::LinearPart &transform)
{
    return get_base(1, transform);
}
inline const Vec3d get_z_base(const Transform3d::LinearPart &transform)
{
    return get_base(2, transform);
}
...
public:
    ...
    template<typename OtherDerived>
    static Point new_scale(const Eigen::MatrixBase<OtherDerived> &v)
    {
...
inline bool shorter_then(const Point &p0, const coord_t len)
{
    ...
    if (p0.y() > len || p0.y() < -len)
        return false;
    return p0.cast<int64_t>().squaredNorm() <= Luminary::sqr(int64_t(len));
}
...
public:
    ...
    std::pair<const ValueType *, double> find(const Vec2crd &pt)
    {
        ...
        Vec2crd grid_corner((pt.x() + (m_grid_resolution >> 1)) >> m_grid_log2,
                            (pt.y() + (m_grid_resolution >> 1)) >> m_grid_log2);
...
// Truncates rather than rounds, so the conversion stays constexpr and noexcept. One unit is a nanometre.
...
template<class Tout = double, class Tin, int N, class = ArithmeticOnly<Tin>, class = FloatingOnly<Tout>,
         int... EigenArgs>
...
{
template<>
struct geometry_concept<Luminary::Point>
{
    using type = point_concept;
};

template<>
struct point_traits<Luminary::Point>
{
    using coordinate_type = coord_t;

    static inline coordinate_type get(const Luminary::Point &point, orientation_2d orient)
    {
...
template<>
struct point_mutable_traits<Luminary::Point>
{
    using coordinate_type = coord_t;
    static inline void set(Luminary::Point &point, orientation_2d orient, coord_t value)
    {
        point((orient == HORIZONTAL) ? 0 : 1) = value;
    }
    static inline Luminary::Point construct(coord_t x_value, coord_t y_value)
    {
        return Luminary::Point(x_value, y_value);
    }
};
} // namespace polygon
} // namespace boost
...
namespace cereal
{
//    template<class Archive> void serialize(Archive& archive, Luminary::Vec2crd &v) { archive(v.x(), v.y()); }
//    template<class Archive> void serialize(Archive& archive, Luminary::Vec3crd &v) { archive(v.x(), v.y(), v.z()); }
template<class Archive>
void serialize(Archive &archive, Luminary::Vec2i &v)
{
    archive(v.x(), v.y());
}
template<class Archive>
void serialize(Archive &archive, Luminary::Vec3i &v)
{
    archive(v.x(), v.y(), v.z());
}
//    template<class Archive> void serialize(Archive& archive, Luminary::Vec2i64 &v) { archive(v.x(), v.y()); }
//    template<class Archive> void serialize(Archive& archive, Luminary::Vec3i64 &v) { archive(v.x(), v.y(), v.z()); }
template<class Archive>
void serialize(Archive &archive, Luminary::Vec2f &v)
{
    archive(v.x(), v.y());
}
template<class Archive>
void serialize(Archive &archive, Luminary::Vec3f &v)
{
    archive(v.x(), v.y(), v.z());
}
template<class Archive>
void serialize(Archive &archive, Luminary::Vec2d &v)
{
    archive(v.x(), v.y());
}
template<class Archive>
void serialize(Archive &archive, Luminary::Vec3d &v)
{
    archive(v.x(), v.y(), v.z());
}

template<class Archive>
void serialize(Archive &archive, Luminary::Matrix4d &m)
{
    archive(binary_data(m.data(), 4 * 4 * sizeof(double)));
}
template<class Archive>
void serialize(Archive &archive, Luminary::Matrix2f &m)
{
...
T *begin(Luminary::Mat<N, M, T> &mat)
{
    return mat.data();
}

template<class T, int N, int M>
T *end(Luminary::Mat<N, M, T> &mat)
{
    return mat.data() + N * M;
}

template<class T, int N, int M>
const T *begin(const Luminary::Mat<N, M, T> &mat)
{
    return mat.data();
}

template<class T, int N, int M>
const T *end(const Luminary::Mat<N, M, T> &mat)
{
    return mat.data() + N * M;
}
} // namespace Eigen
