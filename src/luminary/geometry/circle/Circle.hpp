...
template<typename Derived, typename Derived2, typename Derived3>
std::optional<Eigen::Matrix<typename Derived::Scalar, 2, 1, Eigen::DontAlign>> try_circle_center(
    const Derived &a, const Derived2 &bsrc, const Derived3 &csrc, typename Derived::Scalar epsilon)
{
    static_assert(Derived ::IsVectorAtCompileTime && int(Derived ::SizeAtCompileTime) == 2,
...
struct CircleSq
{
    ...
    {
        return (p - this->center).squaredNorm() < this->radius2 + epsilon2;
    }
...
Point circle_center_taubin_newton(const Points::const_iterator &input_start, const Points::const_iterator &input_end,
                                  size_t cycles = 20);
...
Vec2d circle_center_taubin_newton(const Vec2ds::const_iterator &input_start, const Vec2ds::const_iterator &input_end,
                                  size_t cycles = 20);
...
// Variant whose name spells out its arguments: squared radius, line coefficients, squared line vector length
    ...
    std::pair<Eigen::Matrix<T, 2, 1, Eigen::DontAlign>, Eigen::Matrix<T, 2, 1, Eigen::DontAlign>> &out)
{
    ...
    {
        // Degenerate line direction, no meaningful intersection.
        return false;
    }
