...
Intersections line_point_equal_distance_points(const Line &line, const Point &ipt, const double d)
{
    ...
    assert(std::abs(Geometry::ray_point_distance<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(),
                                                        out.pts[0]) -
                    d) < SCALED_EPSILON);
    assert(std::abs(Geometry::ray_point_distance<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(),
                                                        out.pts[1]) -
...
double dist_to_site(const Lines &lines, const VD::cell_type &cell, const Vec2d &point)
{
    ...
    return cell.contains_point()
               ? (((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b)
                      .cast<double>() -
                  point)
                     .norm()
               : (Geometry::foot_pt<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(), point) - point)
                     .norm();
};
...
{
    const Line &line = lines[cell.source_index()];
    auto on_contour = [&pt](const Point &ipt)
    {
        return detail::vertex_equal_to_point(pt, ipt);
    };
    ...
    const double threshold_cos_alpha)
{
    ...
    double x2 = r * r - Luminary::sqr(l - r);
    double x = sqrt(x2);
    // Project voronoi_point1/2 to line_site.
    Vec2d dir_x = (line_site.b - line_site.a).cast<double>().normalized();
    double t1 = (voronoi_point1 - ft).dot(dir_x);
...
bool verify_twin_halfedges_successive(const VD &vd, const Lines &lines)
{
    for (size_t i = 0; i < vd.num_edges(); i += 2)
    {
        ...
        if (e.is_secondary())
        {
            ...
            const Point &pt = (ex.cell()->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT)
                                  ? line0.a
                                  : line0.b;
...
bool verify_inside_outside_annotations(const VD &vd)
{
    ...
    for (const VD::cell_type &cell : vd.cells())
    {
        ...
        do
        {
            ...
            {
                VertexCategory vc = (edge->vertex1() == nullptr) ? VertexCategory::Outside
                                                                 : vertex_category(edge->vertex1());
                switch (vc)
                {
...
bool verify_offset_intersection_points(const VD &vd, const Lines &lines, const double offset_distance,
...
void annotate_inside_outside(VD &vd, const Lines &lines)
{
    ...
    {
        // Degenerate Voronoi diagrams can have null vertices - skip them safely.
        if (vertex == nullptr)
            return;
#ifndef NDEBUG
    ...
    {
        CellCategory cc = cell_category(cell);
        assert(cc == CellCategory::Inside || cc == CellCategory::Outside || cc == CellCategory::Boundary ||
               cc == CellCategory::Unknown);
    ...
    for (const VD::edge_type &edge : vd.edges())
    {
        if (edge.is_secondary() && edge.vertex0() != nullptr)
        {
            ...
            {
                return detail::vertex_equal_to_point(v, pt_on_contour);
            };
    ...
    for (const VD::edge_type &edge : vd.edges())
        if (edge.vertex1() == nullptr)
        {
            ...
            assert(edge.is_linear());
            // Degenerate input geometry can produce edges where both vertices are null.
            // Skip such edges as they cannot be meaningfully processed.
            ...
            annotate_edge(edge.twin(),
                          edge.is_secondary() ? EdgeCategory::PointsToContour : EdgeCategory::PointsOutside);
            ...
            if (line)
            {
                ...
                else
                {
                    ...
                    annotate_cell(cell, on_contour ? CellCategory::Boundary
                                                   : (vc == VertexCategory::Outside ? CellCategory::Outside
                                                                                    : CellCategory::Inside));
                    annotate_cell(cell2,
                                  (on_contour && cell2->contains_segment())
    ...
    for (const VD::edge_type &edge : vd.edges())
    {
        ...
        if (edge_category(edge) == EdgeCategory::Unknown)
        {
            ...
            if (cc_new != CellCategory::Unknown)
            {
                VertexCategory vc = (cc_new == CellCategory::Outside) ? VertexCategory::Outside
                                                                      : VertexCategory::Inside;
                annotate_vertex(edge.vertex0(), vc);
                annotate_vertex(edge.vertex1(), vc);
                auto ec_new = (cc_new == CellCategory::Outside) ? EdgeCategory::PointsOutside
    ...
    for (const VD::vertex_type &vertex : vd.vertices())
    {
        ...
        else
        {
            ...
            if (point_cell == 0)
            {
                ...
                dist = Geometry::ray_point_distance<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(),
                                                           vertex_point(vertex));
            }
            else
            {
...
std::vector<Vec2d> edge_offset_contour_intersections(const VD &vd, const Lines &lines,
    ...
    for (const VD::edge_type &edge : vd.edges())
    {
...
#ifndef NDEBUG
        {
            ...
            if (v1 != nullptr)
            {
                ...
                double err4 = std::abs(detail::dist_to_site(lines, *edge.twin()->cell(), vertex_point(v1)) -
                                       std::abs(d1));
        ...
        if (v1 == nullptr)
        {
            ...
            if (offset_distance > dmin)
            {
                ...
                if (cell->contains_point() && cell2->contains_point())
                {
                    ...
                    Vec2d pt(v0->x(), v0->y());
                    double t = detail::first_circle_segment_intersection_parameter(Vec2d(pt0.x(), pt0.y()),
                                                                                   offset_distance, pt, dir);
                    assert(t > 0.);
                ...
                else
                {
                    ...
                    assert(edge.is_secondary());
                    const Point &ipt = cell->contains_segment() ? contour_point(*cell2, line1)
                                                                : contour_point(*cell, line0);
#ifndef NDEBUG
                    if (cell->contains_segment())
                    {
                        const Point &pt1 = contour_point(*cell2, line1);
                    ...
                    out[edge_idx] = ipt.cast<double>() +
                                    offset_distance *
                                        Vec2d(line.b.y() - line.a.y(), line.a.x() - line.b.x()).normalized();
                }
            }
        ...
        else
        {
            ...
            if (bisector || edge.is_secondary())
            {
                ...
                if (!bisector || (dmin != dmax && offset_distance >= dmin))
                {
                    ...
                    if (d1 < d0)
                    {
                        out[edge_idx2] = Luminary::lerp(vertex_point(v1), vertex_point(v0), t);
                        // mark visited
                        out[edge_idx].y() = 1.;
                    }
                    else
                    {
                        out[edge_idx] = Luminary::lerp(vertex_point(v0), vertex_point(v1), t);
                        // mark visited
                        out[edge_idx2].y() = 1.;
                    }
                    done = true;
            ...
            else
            {
                ...
                if (num_intersections > 0)
                {
                    ...
                    {
                        assert(cell->contains_point() || cell2->contains_point());
                        intersections = detail::line_point_equal_distance_points(cell->contains_segment() ? line0
                                                                                                          : line1,
                                                                                 pt0, offset_distance);
                    }
                    else
                    ...
                    if (num_intersections == 2)
                    {
                        ...
                        case 0:
                            ...
                            // Accepted: the offset curves may intersect at this edge.
                            break;
                        case 1:
                            // Tangential point found.
                            // Accepted: the offset curves may touch at this edge.
                            break;
                        default:
                        {
...
Polygons offset(const Geometry::VoronoiDiagram &vd, const Lines &lines,
                const std::vector<double> &signed_vertex_distances, double offset_distance, double discretization_error)
{
#ifdef VORONOI_DEBUG_OUT
    BoundingBox bbox;
    {
    ...
    ++irun;
    {
        Lines helper_lines;
        for (const VD::edge_type &edge : vd.edges())
            if (edge_category(edge) ==
                    (offset_distance > 0 ? EdgeCategory::PointsOutside : EdgeCategory::PointsInside) &&
                edge.vertex0() != nullptr)
            {
                ...
                if (v1 == nullptr)
                {
                    ...
                    if (cell->contains_point() && cell2->contains_point())
                    {
                        const Point &pt0 = (cell->source_category() ==
                                            boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT)
                                               ? line0.a
                                               : line0.b;
                        const Point &pt1 = (cell2->source_category() ==
                                            boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT)
                                               ? line1.a
                                               : line1.b;
                ...
                else
                {
                    ...
                    Geometry::liang_barsky_line_clipping(pt1, pt2,
        ...
        dump_voronoi_to_svg(debug_out_path("voronoi-offset-candidates1-%d.svg", irun).c_str(), vd, Points(), lines,
                            Polygons(), helper_lines);
    }
#endif // VORONOI_DEBUG_OUT

    std::vector<Vec2d> edge_points = edge_offset_contour_intersections(vd, lines, signed_vertex_distances,
                                                                       offset_distance);
    const VD::edge_type *front_edge = &vd.edges().front();
    ...
    Lines helper_lines;
    {
        for (const VD::edge_type &edge : vd.edges())
            if (edge_offset_has_intersection(edge_points[&edge - front_edge]))
                helper_lines.emplace_back(Line(Point(edge.vertex0()->x(), edge.vertex0()->y()),
                                               Point(edge_points[&edge - front_edge].cast<coord_t>())));
        ...
        if (edge_offset_has_intersection(last_pt))
        {
            ...
            do
            {
                ...
                if (next_edge == nullptr)
                {
                    ...
                    dump_voronoi_to_svg(debug_out_path("voronoi-offset-open-loop-%d.svg", irun).c_str(), vd, Points(),
                                        lines, Polygons(), hl);
                }
#endif // VORONOI_DEBUG_OUT
                assert(next_edge);
...
#ifndef NDEBUG
                {
                    ...
                    {
                        Lines helper_lines;
                        dump_voronoi_to_svg(debug_out_path("voronoi-offset-incorrect_pt-%d.svg", irun).c_str(), vd,
                                            Points(), lines, Polygons(), to_lines(poly));
                    }
#endif // VORONOI_DEBUG_OUT
                ...
                if (cell->contains_point())
                {
                    ...
                    const Line &line0 = lines[cell->source_index()];
                    const Vec2d &center = ((cell->source_category() ==
                                            boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT)
                                               ? line0.a
                                               : line0.b)
                                              .cast<double>();
                                        ...
                                        const double threshold_alpha)
{
    ...
    for (const VD::edge_type &edge : vd.edges())
    {
        ...
        if (
            ...
            &edge > edge.twin() ||
            // Ignore outer edges.
            (edge_category(edge) != EdgeCategory::PointsInside &&
             edge_category(edge.twin()) != EdgeCategory::PointsInside))
            continue;
        const VD::vertex_type *v0 = edge.vertex0();
        const VD::vertex_type *v1 = edge.vertex1();
        const VD::cell_type *cell = edge.cell();
        ...
        else
        {
            ...
            if (cell->contains_point() != cell2->contains_point())
            {
                ...
                const Point &pt0 = cell->contains_point() ? contour_point(*cell, line0) : contour_point(*cell2, line1);
                const Line &line = cell->contains_segment() ? line0 : line1;
                std::tie(out[edge_idx],
                         out[edge_idx2]) = detail::point_segment_dr_dl_thresholds(pt0, line, vertex_point(v0),
                                                                                  vertex_point(v1),
            ...
            else
            {
                ...
                const Point &pt1 = contour_point(*cell2, line1);
                std::tie(out[edge_idx],
                         out[edge_idx2]) = detail::point_point_dr_dl_thresholds(pt0, pt1, vertex_point(v0),
                                                                                vertex_point(v1),
                                                                                threshold_tan_alpha_half);
            }
        }
            ...
            if (&edge < edge.twin() && edge.is_finite())
            {
                ...
                if (has_skeleton_pt && has_skeleton_pt2)
                {
                    ...
                    helper_lines.emplace_back(
                        Line(Point(vertex_pt.x(), vertex_pt.y()), Point(vertex_pt2.x(), vertex_pt2.y())));
                }
                else
                {
                    if (has_skeleton_pt)
                        helper_lines.emplace_back(
                            Line(Point(vertex_pt2.x(), vertex_pt2.y()), Point(skeleton_pt.x(), skeleton_pt.y())));
                    if (has_skeleton_pt2)
                        helper_lines.emplace_back(
                            Line(Point(vertex_pt.x(), vertex_pt.y()), Point(skeleton_pt2.x(), skeleton_pt2.y())));
                }
            }
        dump_voronoi_to_svg(debug_out_path("voronoi-skeleton-edges-%d.svg", irun).c_str(), vd, Points(), lines,
                            Polygons(), helper_lines);
    }
#endif // VORONOI_DEBUG_OUT
