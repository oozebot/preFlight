...
static ClipperZUtils::ZPaths expolygons_to_zpaths_expanded_opened(const ExPolygons &src, const float expansion,
                                                                  coord_t &base_idx)
{
    ClipperZUtils::ZPaths out;
    out.reserve(2 * std::accumulate(src.begin(), src.end(), size_t(0), [](const size_t acc, const ExPolygon &expoly)
                                    { return acc + expoly.num_contours(); }));
    Clipper2Lib::ClipperOffset offsetter;
    offsetter.MiterLimit(2.0);
    Polygons expansion_cache;
    for (const ExPolygon &expoly : src)
    {
        for (size_t icontour = 0; icontour < expoly.num_contours(); ++icontour)
        {
            ...
            offsetter.Clear();
            offsetter.AddPath(Points_to_ClipperPath(expoly.contour_or_hole(icontour).points), JoinType::Square,
                              EndType::Polygon);
            Clipper2Lib::Paths64 clipper_result;
            offsetter.Execute(icontour == 0 ? expansion : -expansion, clipper_result);
            expansion_cache = ClipperPaths_to_Polygons(clipper_result);
...
static inline void merge_splits(ClipperZUtils::ZPaths &paths,
                                std::vector<std::pair<ClipperZUtils::ZPoint, int>> &splits)
{
    for (auto it_path = paths.begin(); it_path != paths.end();)
    {
        ...
        if (path.size() >= 2)
        {
            ...
            if (front.x != back.x || front.y != back.y)
            {
                ...
                {
                    auto it = std::lower_bound(splits.begin(), splits.end(), pt, [](const auto &l, const auto &r)
                                               { return ClipperZUtils::zpoint_lower(l.first, r); });
    ...
    bool sorted)
{
    ...
    {
        // With USINGZ enabled, Clipper2 tracks Z coordinates natively via SetZCallback.
        // Z encodes polygon identity:
        //   Z in [1, idx_boundary_end) = boundary polygon index
        //   Z in [idx_boundary_end, idx_src_end) = source polygon index
        //   Z < 0 = intersection point, -Z-1 is index into intersections vector
        ...
        // Convert ZPaths to Paths64 for boundary (clip paths) - Z is preserved
        ClipperZUtils::ZPaths zboundary = ClipperZUtils::expolygons_to_zpaths(boundary, idx_boundary_end);
        Clipper2Lib::Paths64 boundary_paths = ClipperZUtils::zpaths_to_paths64(zboundary);
        clipper.AddClip(boundary_paths);

        // Convert ZPaths to Paths64 for source (subject open paths) - Z is preserved
        ...
        Clipper2Lib::Paths64 src_paths = ClipperZUtils::zpaths_to_paths64(zsrc);
        clipper.AddOpenSubject(src_paths);

        // Track split points for later merging
        ...
        // Execute clipping - open paths go to separate output parameter
        Clipper2Lib::PolyTree64 polytree;
        Clipper2Lib::Paths64 open_paths;
        clipper.Execute(Clipper2Lib::ClipType::Intersection, Clipper2Lib::FillRule::NonZero, polytree, open_paths);

        // Convert results back to ZPaths - Z coordinates are already set by callback
        segments = ClipperZUtils::paths64_to_zpaths(open_paths);
    ...
    for (const ClipperZUtils::ZPath &path : segments)
    {
        ...
        assert(
            (front == back && front.z >= idx_boundary_end && front.z < idx_src_end) ||
            //(front.z < 0 && back.z < 0));
        ...
        else if (front == back && (front.z < idx_boundary_end))
        {
            ...
            // Fix for region expansion issue.
        ...
        {
            return is.first >= 1 && is.first < idx_boundary_end && is.second >= idx_boundary_end &&
        ...
        if (intersection)
        {
            ...
            out.push_back({uint32_t(intersection->second - idx_boundary_end), uint32_t(intersection->first - 1),
        ...
        else
        {
            ...
            if (boundary_id >= 0)
                out.push_back(
                    {uint32_t(front.z - idx_boundary_end), uint32_t(boundary_id), ClipperZUtils::from_zpath(path)});
        }
        ++iseed;
    }
...
static Polygons wavefront_initial(Clipper2Lib::ClipperOffset &co, const Polygons &polylines, float offset)
{
    Polygons out;
    out.reserve(polylines.size());
    for (const Polygon &path : polylines)
    {
        assert(path.size() >= 2);
        co.Clear();
        Clipper2Lib::Path64 clipper_path = Points_to_ClipperPath(path.points);
        co.AddPath(clipper_path, JoinType::Round,
                   clipper_path.front() == clipper_path.back() ? EndType::Joined : EndType::Round);
        Clipper2Lib::Paths64 out_this;
        co.Execute(offset, out_this);
        append(out, ClipperPaths_to_Polygons(out_this));
    }
    return out;
}
...
static Polygons wavefront_step(Clipper2Lib::ClipperOffset &co, const Polygons &polygons, float offset)
{
    Polygons out;
    ...
    for (const Polygon &polygon : polygons)
    {
        ...
        // contours will be CCW oriented even though the input paths are CW oriented.
        // Offset is applied after contour reorientation, thus the signum of the offset value is reversed.
        Clipper2Lib::Path64 clipper_path = Points_to_ClipperPath(polygon.points);
        co.AddPath(clipper_path, JoinType::Round, EndType::Polygon);
        bool ccw = Clipper2Lib::IsPositive(clipper_path);
        Clipper2Lib::Paths64 out_this;
        co.Execute(ccw ? offset : -offset, out_this);
        if (!ccw)
        {
            // Reverse the resulting contours.
            for (Clipper2Lib::Path64 &path : out_this)
...
{
    Clipper2Lib::Clipper64 clipper;
    clipper.AddSubject(Polygons_to_ClipperPaths(wavefront));
    clipper.AddClip(Polygons_to_ClipperPaths(clipping));
    Clipper2Lib::Paths64 out;
    clipper.Execute(Clipper2Lib::ClipType::Intersection, Clipper2Lib::FillRule::Positive, out);
    ...
    const float max_inflation)
{
    ...
    Polygons clipping = ClipperUtils::clip_clipper_polygons_with_subject_bbox(boundary, get_extents(seed).inflated(
                                                                                            max_inflation));
    Polygons polygons = wavefront_clip(wavefront_initial(co, seed, initial_step), clipping);
...
std::vector<RegionExpansion> propagate_waves(const WaveSeeds &seeds, const ExPolygons &boundary,
                                             const RegionExpansionParameters &params)
{
    ...
    for (auto it_seed = seeds.begin(); it_seed != seeds.end();)
    {
        ...
        RegionExpansion re;
        for (Polygon &polygon :
             propagate_wave_from_boundary(co, paths, boundary[it_seed->boundary], params.initial_step,
                                          params.other_step, params.num_other_steps, params.max_inflation))
...
std::vector<RegionExpansionEx> propagate_waves_ex(const WaveSeeds &seeds, const ExPolygons &boundary,
                                                  const RegionExpansionParameters &params)
{
...
std::vector<ExPolygon> expand_merge_expolygons(ExPolygons &&src, const ExPolygons &boundary,
                                               const RegionExpansionParameters &params)
{
