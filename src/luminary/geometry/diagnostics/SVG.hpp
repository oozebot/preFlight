...
public:
    ...
    bool open(const std::string &filename, const BoundingBox &bbox, const coord_t bbox_offset = scale_(1.),
              bool flipY = true)
    {
    ...
    void draw_outline(const ExPolygon &polygon, std::string stroke_outer = "black", std::string stroke_holes = "blue",
                      coordf_t stroke_width = 0);
    void draw(const ExPolygons &expolygons, std::string fill = "grey", const float fill_opacity = 1.f);
    void draw_outline(const ExPolygons &polygons, std::string stroke_outer = "black", std::string stroke_holes = "blue",
                      coordf_t stroke_width = 0);

    void draw(const Surface &surface, std::string fill = "grey", const float fill_opacity = 1.f);
    void draw_outline(const Surface &surface, std::string stroke_outer = "black", std::string stroke_holes = "blue",
                      coordf_t stroke_width = 0);
    void draw(const Surfaces &surfaces, std::string fill = "grey", const float fill_opacity = 1.f);
    void draw_outline(const Surfaces &surfaces, std::string stroke_outer = "black", std::string stroke_holes = "blue",
                      coordf_t stroke_width = 0);
    void draw(const SurfacesPtr &surfaces, std::string fill = "grey", const float fill_opacity = 1.f);
    void draw_outline(const SurfacesPtr &surfaces, std::string stroke_outer = "black",
                      std::string stroke_holes = "blue", coordf_t stroke_width = 0);
    ...
    void draw(const ThickLines &thicklines, const std::string &fill = "lime", const std::string &stroke = "black",
              coordf_t stroke_width = 0);
    void draw(const ThickPolylines &polylines, const std::string &stroke = "black", coordf_t stroke_width = 0);
    void draw(const ThickPolylines &thickpolylines, const std::string &fill, const std::string &stroke,
    ...
    // Support for rendering Clipper2 paths
    void draw(const Clipper2Lib::Path64 &polygon, double scale, std::string fill = "grey", coordf_t stroke_width = 0);
    void draw(const Clipper2Lib::Paths64 &polygons, double scale, std::string fill = "grey", coordf_t stroke_width = 0);
...
private:
    ...
    std::string get_path_d(const Clipper2Lib::Path64 &mp, double scale, bool closed = false) const;

public:
    static void export_expolygons(const char *path, const BoundingBox &bbox, const Luminary::ExPolygons &expolygons,
                                  std::string stroke_outer = "black", std::string stroke_holes = "blue",
                                  coordf_t stroke_width = 0);
    static void export_expolygons(const std::string &path, const BoundingBox &bbox,
                                  const Luminary::ExPolygons &expolygons, std::string stroke_outer = "black",
                                  std::string stroke_holes = "blue", coordf_t stroke_width = 0)
    {
        export_expolygons(path.c_str(), bbox, expolygons, stroke_outer, stroke_holes, stroke_width);
    }
    static void export_expolygons(const char *path, const Luminary::ExPolygons &expolygons,
                                  std::string stroke_outer = "black", std::string stroke_holes = "blue",
                                  coordf_t stroke_width = 0)
    {
        export_expolygons(path, get_extents(expolygons), expolygons, stroke_outer, stroke_holes, stroke_width);
    }
    static void export_expolygons(const std::string &path, const Luminary::ExPolygons &expolygons,
                                  std::string stroke_outer = "black", std::string stroke_holes = "blue",
                                  coordf_t stroke_width = 0)
    {
    ...
    struct ExPolygonAttributes
    {
        ...
        ExPolygonAttributes(const std::string &color_fill, const std::string &color_contour,
                            const std::string &color_holes, const coord_t outline_width = scale_(0.05),
                            const float fill_opacity = 0.5f, const std::string &color_points = "black",
        ...
        ExPolygonAttributes(const std::string &legend, const std::string &color_fill, const std::string &color_contour,
                            const std::string &color_holes, const coord_t outline_width = scale_(0.05),
                            const float fill_opacity = 0.5f, const std::string &color_points = "black",
                            const coord_t radius_points = 0)
            : legend(legend)
        ...
        ExPolygonAttributes(const std::string &legend, const std::string &color_fill, const float fill_opacity)
            : legend(legend), color_fill(color_fill), fill_opacity(fill_opacity)
        {
        }
    ...
    static void export_expolygons(
        const char *path,
        const std::vector<std::pair<Luminary::ExPolygons, ExPolygonAttributes>> &expolygons_with_attributes);
    static void export_expolygons(
