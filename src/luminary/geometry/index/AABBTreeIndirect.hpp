...
#include "luminary/core/containers/Sizing.hpp" // for next_highest_power_of_2()
...
private:
    ...
    template<typename SourceNode>
    void partition_input(std::vector<SourceNode> &input, const size_t dimension, size_t left, size_t right,
...
// Wrap the engine's own 2D BoundingBox to be passed to Tree::build() and similar
// to build an AABBTree over coord_t 2D bounding boxes.
class BoundingBoxWrapper
{
public:
    using BoundingBox = Eigen::AlignedBox<coord_t, 2>;
    BoundingBoxWrapper(const size_t idx, const Luminary::BoundingBox &bbox)
        : m_idx(idx)
        ,
    ...
    {
        return ((m_bbox.min().cast<int64_t>() + m_bbox.max().cast<int64_t>()) / 2).cast<coord_t>();
    }
...
inline bool ray_box_intersect_invdir(const Eigen::MatrixBase<Derivedsource> &origin,
                                     const Eigen::MatrixBase<Deriveddir> &inv_dir, Eigen::AlignedBox<Scalar, 3> box,
...
std::enable_if_t<std::is_same<typename V::Scalar, double>::value && std::is_same<typename W::Scalar, double>::value,
                 bool>
intersect_triangle(const V &orig, const V &dir, const W &vert0, const W &vert1, const W &vert2, double &t, double &u,
                   double &v, double eps)
{
...
std::enable_if_t<std::is_same<typename V::Scalar, double>::value && !std::is_same<typename W::Scalar, double>::value,
                 bool>
intersect_triangle(const V &origin, const V &dir, const W &v0, const W &v1, const W &v2, double &t, double &u,
                   double &v, double eps)
{
    return intersect_triangle(origin, dir, v0.template cast<double>(), v1.template cast<double>(),
                              v2.template cast<double>(), t, u, v, eps);
}
...
static inline bool intersect_ray_recursive_first_hit(RayIntersectorType &ray_intersector, size_t node_idx, Scalar min_t,
                                                     igl::Hit &hit)
{
    ...
    if (node.is_leaf())
    {
        ...
        if (intersect_triangle(ray_intersector.origin, ray_intersector.dir, ray_intersector.vertices[face(0)],
                               ray_intersector.vertices[face(1)], ray_intersector.vertices[face(2)], t, u, v,
                               ray_intersector.eps) &&
            t > 0.)
        {
...
struct IndexedTriangleSetDistancer
{
    ...
    inline VectorType closest_point_to_origin(size_t primitive_index, ScalarType &squared_distance) const
    {
        ...
        VectorType closest_point =
            closest_point_to_triangle<VectorType>(origin, this->vertices[triangle(0)].template cast<ScalarType>(),
...
static inline Scalar squared_distance_to_indexed_primitives_recursive(
    IndexedPrimitivesDistancerType &distancer, size_t node_idx, Scalar low_sqr_d, Scalar up_sqr_d, size_t &i,
    Eigen::PlainObjectBase<typename IndexedPrimitivesDistancerType::VectorType> &c)
{
    ...
    auto set_min =
        [&i, &c, &up_sqr_d](const Scalar sqr_d_candidate, const size_t i_candidate, const Vector &c_candidate)
    {
    ...
    else
    {
        ...
        const auto &look_left = [&]()
        {
            size_t i_left;
            Vector c_left = c;
            Scalar sqr_d_left = squared_distance_to_indexed_primitives_recursive(distancer, left_node_idx, low_sqr_d,
                                                                                 up_sqr_d, i_left, c_left);
        ...
        const auto &look_right = [&]()
        {
            size_t i_right;
            Vector c_right = c;
            Scalar sqr_d_right = squared_distance_to_indexed_primitives_recursive(distancer, right_node_idx, low_sqr_d,
                                                                                  up_sqr_d, i_right, c_right);
...
static inline void indexed_primitives_within_distance_squared_recurisve(const IndexedPrimitivesDistancerType &distancer,
                                                                        size_t node_idx, Scalar squared_distance_limit,
...
inline Tree<3, typename VertexType::Scalar> build_aabb_tree_over_indexed_triangle_set(
    ...
    const std::vector<IndexedFaceType> &faces, const typename VertexType::Scalar eps = 0)
{
    ...
    const double eps = 0.000001)
{
    ...
    return !tree.empty() && detail::intersect_ray_recursive_first_hit(ray_intersector, size_t(0),
                                                                      std::numeric_limits<Scalar>::infinity(), hit);
}
...
{
    using Scalar = typename VectorType::Scalar;
    auto distancer = detail::IndexedTriangleSetDistancer<VertexType, IndexedFaceType, TreeType, VectorType>{vertices,
                                                                                                            faces, tree,
                                                                                                            point};
    return tree.empty()
               ? Scalar(-1)
    ...
    typename VectorType::Scalar &max_distance_squared)
{
    ...
    detail::squared_distance_to_indexed_primitives_recursive(distancer, size_t(0), Scalar(0), max_distance_squared,
                                                             hit_idx, hit_point);
    ...
    typename VectorType::Scalar max_distance_squared)
{
    ...
    detail::indexed_primitives_within_distance_squared_recurisve(distancer, size_t(0), max_distance_squared,
                                                                 found_triangles);
