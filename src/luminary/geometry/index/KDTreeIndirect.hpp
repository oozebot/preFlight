...
public:
    ...
    KDTreeIndirect(CoordinateFn coordinate, std::vector<size_t> indices) : coordinate(coordinate)
    {
        this->build(indices);
    }
    ...
    // Copy constructor cause failing FDM tests but not each run only from time to time.
    // KDTreeIndirect(const KDTreeIndirect &rhs) : m_nodes(rhs.m_nodes), coordinate(rhs.coordinate) {}
    KDTreeIndirect get_copy() const
    {
        KDTreeIndirect copy(coordinate);
    ...
    unsigned int descent_mask(const CoordType &point_coord, const double &search_radius, size_t idx,
                              size_t dimension) const
    {
        ...
        return (double(dist) * dist < search_radius + EPSILON)
                   ...
                   ((unsigned int) (VisitorReturnMask::CONTINUE_LEFT) |
                    (unsigned int) (VisitorReturnMask::CONTINUE_RIGHT))
                   :
...
private:
    ...
    void build_recursive(std::vector<size_t> &input, size_t node, const size_t dimension, const size_t left,
                         const size_t right)
    {
        if (left > right)
            return;
    ...
    void partition_input(std::vector<size_t> &input, const size_t dimension, size_t left, size_t right,
...
// Find nearby points (spherical neighbourhood) using Euclidian metrics.
template<typename KDTreeIndirectType, typename PointType, typename FilterFn>
std::vector<size_t> find_nearby_points(const KDTreeIndirectType &kdtree, const PointType &bb_min,
                                       const PointType &bb_max, FilterFn filter)
{
    struct Visitor
    {
        const KDTreeIndirectType &kdtree;
