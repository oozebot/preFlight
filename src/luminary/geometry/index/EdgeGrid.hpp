...
public:
    Contour() = default;
    Contour(const Luminary::Point *begin, const Luminary::Point *end, bool open)
        : m_begin(begin), m_end(end), m_open(open)
    {
    }
    Contour(const Luminary::Point *data, size_t size, bool open) : Contour(data, data + size, open) {}
    Contour(const Points &pts, bool open) : Contour(pts.data(), pts.size(), open) {}

    const Luminary::Point *begin() const { return m_begin; }
    const Luminary::Point *end() const { return m_end; }
    bool open() const { return m_open; }
    bool closed() const { return !m_open; }

    const Luminary::Point &front() const { return *m_begin; }
    const Luminary::Point &back() const { return *(m_end - 1); }

    // Start point of a segment idx.
    const Luminary::Point &segment_start(size_t idx) const
    {
    ...
    const Luminary::Point &segment_end(size_t idx) const
    {
        assert(idx < this->num_segments());
        const Luminary::Point *ptr = m_begin + idx + 1;
        return ptr == m_end ? *m_begin : *ptr;
    }

    // Start point of a segment preceding idx.
    const Luminary::Point &segment_prev(size_t idx) const
    {
        assert(idx < this->num_segments());
        assert(idx > 0 || !m_open);
...
private:
    ...
    const Luminary::Point *m_begin{nullptr};
    const Luminary::Point *m_end{nullptr};
    bool m_open{false};
};
...
public:
    ...
    bool signed_distance_edges(const Point &pt, coord_t search_radius, coordf_t &result_min_dist,
                               bool *pon_segment = nullptr) const;
    ...
    void visit_cells_intersecting_line(Luminary::Point p1, Luminary::Point p2, VISITOR &visitor) const
    {
    ...
    void visit_cells_intersecting_box(BoundingBox bbox, VISITOR &visitor) const
    {
        // End points of the line segment.
        bbox.min -= m_bbox.min;
        bbox.max -= m_bbox.min + Point(coord_t(1), coord_t(1));
        // Get the cells of the end points.
    ...
    std::pair<std::vector<std::pair<size_t, size_t>>::const_iterator,
              std::vector<std::pair<size_t, size_t>>::const_iterator>
    cell_data_range(coord_t row, coord_t col) const
    {
    ...
    std::pair<const Luminary::Point &, const Luminary::Point &> segment(
        const std::pair<size_t, size_t> &contour_and_segment_idx) const
    {
        const Contour &contour = m_contours[contour_and_segment_idx.first];
        size_t iseg = contour_and_segment_idx.second;
        return std::pair<const Luminary::Point &, const Luminary::Point &>(contour.segment_start(iseg),
                                                                           contour.segment_end(iseg));
    }
...
protected:
    ...
    // This format allows one to work with any fixed point contour format
...
extern std::vector<std::pair<EdgeGrid::Grid::ContourEdge, EdgeGrid::Grid::ContourEdge>> intersecting_edges(
