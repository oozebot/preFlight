...
{
    m_contours.clear();
    m_contours.reserve((expoly.contour.empty() ? 0 : 1) + std::count_if(expoly.holes.begin(), expoly.holes.end(),
...
void EdgeGrid::Grid::create(const ExPolygons &expolygons, coord_t resolution)
{
    ...
    for (const ExPolygon &expoly : expolygons)
    {
        if (!expoly.contour.empty())
            ++ncontours;
        ncontours += std::count_if(expoly.holes.begin(), expoly.holes.end(),
                                   [](const Polygon &p) { return !p.empty(); });
    }
...
void EdgeGrid::Grid::create_from_m_contours(coord_t resolution)
{
    ...
    for (const Contour &contour : m_contours)
    {
        ...
        for (const Luminary::Point &pt : contour)
            m_bbox.merge(pt);
    }
    ...
    for (size_t i = 0; i < m_contours.size(); ++i)
    {
        ...
        for (size_t j = 0; j < contour.num_segments(); ++j)
        {
            ...
            Luminary::Point p1(contour.segment_start(j));
            Luminary::Point p2(contour.segment_end(j));
            p1(0) -= m_bbox.min(0);
            p1(1) -= m_bbox.min(1);
            p2(0) -= m_bbox.min(0);
            p2(1) -= m_bbox.min(1);
    ...
    for (; visitor.i < m_contours.size(); ++visitor.i)
    {
        ...
        for (visitor.j = 0; visitor.j < contour.num_segments(); ++visitor.j)
            this->visit_cells_intersecting_line(contour.segment_start(visitor.j), contour.segment_end(visitor.j),
                                                visitor);
    }
}
    ...
    for (int r = 0; r < (int) m_rows; ++r)
    {
        for (int c = 0; c < (int) m_cols; ++c)
        {
            ...
            for (size_t i = cell.begin; i != cell.end; ++i)
            {
                ...
                // End points of the line segment.
                const Luminary::Point &p1 = contour.segment_start(ipt);
                const Luminary::Point &p2 = contour.segment_end(ipt);
                // Segment vector
                const Luminary::Point v_seg = p2 - p1;
                // l2 of v_seg
                ...
                for (int corner_y = -1; corner_y < 3; ++corner_y)
                {
                    ...
                    for (int corner_x = -1; corner_x < 3; ++corner_x)
                    {
                        ...
                        Luminary::Point pt(m_bbox.min(0) + corner_c * m_resolution,
                                           m_bbox.min(1) + corner_r * m_resolution);
                        Luminary::Point v_pt = pt - p1;
                        // dot(p2-p1, pt-p1)
                        int64_t t_pt = int64_t(v_seg(0)) * int64_t(v_pt(0)) + int64_t(v_seg(1)) * int64_t(v_pt(1));
                        if (t_pt < 0)
                        {
                            // Closest to p1.
                            double dabs = sqrt(int64_t(v_pt(0)) * int64_t(v_pt(0)) +
                                               int64_t(v_pt(1)) * int64_t(v_pt(1)));
                            if (dabs < d_min)
                            {
                                // Previous point.
                                const Luminary::Point &p0 = contour.segment_prev(ipt);
                                Luminary::Point v_seg_prev = p1 - p0;
                                int64_t t2_pt = int64_t(v_seg_prev(0)) * int64_t(v_pt(0)) +
                                                int64_t(v_seg_prev(1)) * int64_t(v_pt(1));
                                if (t2_pt > 0)
                                {
                                    ...
                                    int64_t det = int64_t(v_seg_prev(0)) * int64_t(v_seg(1)) -
                                                  int64_t(v_seg_prev(1)) * int64_t(v_seg(0));
                                    assert(det != 0);
                                    d_min = dabs;
...
EdgeGrid::Grid::ClosestPointResult EdgeGrid::Grid::closest_point_signed_distance(const Point &pt,
                                                                                 coord_t search_radius) const
{
    BoundingBox bbox;
    ...
    for (int r = bbox.min(1); r <= bbox.max(1); ++r)
    {
        for (int c = bbox.min(0); c <= bbox.max(0); ++c)
        {
            ...
            for (size_t i = cell.begin; i < cell.end; ++i)
            {
                ...
                const Luminary::Point &p2 = contour.segment_end(ipt);
                const Luminary::Point v_seg = p2 - p1;
                const Luminary::Point v_pt = pt - p1;
                // dot(p2-p1, pt-p1)
                int64_t t_pt = int64_t(v_seg(0)) * int64_t(v_pt(0)) + int64_t(v_seg(1)) * int64_t(v_pt(1));
                // l2 of seg
                ...
                else
                {
                    ...
                    if (dabs < d_min)
                    {
                        ...
                        result.t = t_pt;
#ifndef NDEBUG
                        Vec2d foot = p1.cast<double>() * (1. - result.t / l2_seg_min) +
                                     p2.cast<double>() * (result.t / l2_seg_min);
    ...
    if (result.contour_idx != size_t(-1) && d_min <= double(search_radius))
    {
...
#ifndef NDEBUG
        {
            ...
            const Luminary::Point &p1 = contour.segment_start(result.start_point_idx);
            const Luminary::Point &p2 = contour.segment_end(result.start_point_idx);
            Vec2d vfoot;
            if (result.t == 0)
...
bool EdgeGrid::Grid::signed_distance_edges(const Point &pt, coord_t search_radius, coordf_t &result_min_dist,
    ...
    const
{
    ...
    for (int r = 0; r < (int) m_rows; ++r)
    {
        for (int c = 0; c < (int) m_cols; ++c)
        {
            ...
            for (size_t i = cell.begin; i != cell.end; ++i)
            {
                ...
                const Luminary::Point &ip1 = icontour.segment_start(ipt);
                const Luminary::Point &ip2 = icontour.segment_end(ipt);
                for (size_t j = i + 1; j != cell.end; ++j)
                {
                    const Contour &jcontour = m_contours[m_cell_data[j].first];
                    size_t jpt = m_cell_data[j].second;
                    // End points of the line segment and their vector.
                    const Luminary::Point &jp1 = jcontour.segment_start(jpt);
                    const Luminary::Point &jp2 = jcontour.segment_end(jpt);
                    ...
                    if (Geometry::segments_intersect(ip1, ip2, jp1, jp2))
                    {
                        ...
                        out.emplace_back(
                            jfirst ? std::make_pair(std::make_pair(&icontour, ipt), std::make_pair(&jcontour, jpt))
    ...
    Luminary::sort_remove_duplicates(out);
    return out;
}
...
{
    std::vector<std::pair<EdgeGrid::Grid::ContourEdge, EdgeGrid::Grid::ContourEdge>> intersections = intersecting_edges(
        polygons);
