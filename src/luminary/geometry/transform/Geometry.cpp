...
void simplify_polygons(const Polygons &polygons, double tolerance, Polygons *retval)
{
    ...
    *retval = Luminary::simplify_polygons(simplified_raw);
}
...
void assemble_transform(Transform3d &transform, const Transform3d &translation, const Transform3d &rotation,
                        const Transform3d &scale, const Transform3d &mirror)
{
    transform = translation * rotation * scale * mirror;
}

Transform3d assemble_transform(const Transform3d &translation, const Transform3d &rotation, const Transform3d &scale,
...
{
    const Transform3d scale = extract_scale(m_matrix);
    return {scale(0, 0) / std::abs(scale(0, 0)), scale(1, 1) / std::abs(scale(1, 1)),
            scale(2, 2) / std::abs(scale(2, 2))};
}
...
void Transformation::reset_skew()
{
    ...
    const Geometry::TransformationSVD svd(*this);
    m_matrix = get_offset_matrix() * Transform3d(svd.u) * scale_transform(new_scale_factor(svd.s)) *
...
bool is_point_inside_polygon_corner(const Point &a, const Point &b, const Point &c, const Point &query_point)
{
    ...
    if ((proj_a_on_bq_normal > 0. && proj_c_on_bq_normal <= 0.) ||
        (proj_a_on_bq_normal <= 0. && proj_c_on_bq_normal > 0.))
    {
    ...
    else
    {
        ...
        return (proj_a_on_bq_normal > 0. && proj_c_on_bq < proj_a_on_bq) ||
               (proj_a_on_bq_normal <= 0. && proj_c_on_bq >= proj_a_on_bq);
    }
}
