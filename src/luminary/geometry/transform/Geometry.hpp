...
// With 64-bit coordinates, we need 128-bit intermediates to avoid overflow during multiplication.
// For int32_t coords: 30 bits + sign gives 61 bits + sign in products, which fits in int64_t
// For int64_t coords: 62 bits + sign gives 125 bits + sign in products, needing 128-bit arithmetic
//
// MSVC doesn't have __int128, so we use double (53-bit mantissa) for cross products.
// This loses some precision but is sufficient for orientation tests (we only need sign).
// Alternative: Use _mul128 intrinsic for exact 128-bit math (more complex).
static inline Orientation orient(const Point &a, const Point &b, const Point &c)
{
    // Use double for intermediate calculations (53-bit mantissa handles most cases)
    // For exact arithmetic with very large coordinates, would need 128-bit integers
    double u = double(b.x()) * double(c.y()) - double(b.y()) * double(c.x());
    double v = double(a.x()) * double(c.y()) - double(a.y()) * double(c.x());
    double w = double(a.x()) * double(b.y()) - double(a.y()) * double(b.x());
    double d = u - v + w;
...
inline bool segments_intersect(const Luminary::Point &ip1, const Luminary::Point &ip2, const Luminary::Point &jp1,
                               const Luminary::Point &jp2)
{
    assert(ip1 != ip2);
    assert(jp1 != jp2);

    auto segments_could_intersect = [](const Luminary::Point &ip1, const Luminary::Point &ip2,
                                       const Luminary::Point &jp1, const Luminary::Point &jp2) -> std::pair<int, int>
    {
...
inline bool liang_barsky_line_clipping_interval(
    ...
    const Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x0, const Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &v,
    // Bounding box to clip with.
    const BoundingBoxBase<Eigen::Matrix<T, 2, 1, Eigen::DontAlign>> &bbox, std::pair<double, double> &out_interval)
{
    double t0 = 0.0;
    double t1 = 1.0;
...
inline bool liang_barsky_line_clipping(
    // Start and end points of the source line, result will be stored there as well.
    Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x0, Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x1,
    // Bounding box to clip with.
...
bool liang_barsky_line_clipping(
    ...
    const Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x0src, const Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x1src,
    // Bounding box to clip with.
    const BoundingBoxBase<Eigen::Matrix<T, 2, 1, Eigen::DontAlign>> &bbox,
    // Start and end points of the clipped line.
    Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x0clip, Eigen::Matrix<T, 2, 1, Eigen::DontAlign> &x1clip)
{
    x0clip = x0src;
    x1clip = x1src;
...
// 5) rotate Z
// 6) translate
void assemble_transform(Transform3d &transform, const Vec3d &translation = Vec3d::Zero(),
                        const Vec3d &rotation = Vec3d::Zero(), const Vec3d &scale = Vec3d::Ones(),
                        const Vec3d &mirror = Vec3d::Ones());
...
void assemble_transform(Transform3d &transform, const Transform3d &translation = Transform3d::Identity(),
                        const Transform3d &rotation = Transform3d::Identity(),
                        const Transform3d &scale = Transform3d::Identity(),
...
// T = translation * rotation * scale * mirror
Transform3d assemble_transform(const Transform3d &translation = Transform3d::Identity(),
                               const Transform3d &rotation = Transform3d::Identity(),
                               const Transform3d &scale = Transform3d::Identity(),
                               const Transform3d &mirror = Transform3d::Identity());
...
// The matrix should not contain any scale or shear
...
// The transform should not contain any shear
...
{
    return {T(std::cos(azimuth) * std::sin(polar)), T(std::sin(azimuth) * std::sin(polar)), T(std::cos(polar))};
}
