...
std::tuple<float, Vec2f, Vec2f, float> compute_moments_of_area_of_triangle(const Vec2f &a, const Vec2f &b,
                                                                           const Vec2f &c)
{
    // The triangle S with vertices a, b, c is the image of the unit triangle
    // S' = {(u,v) : u >= 0, v >= 0, u + v <= 1} under the affine map
    //   g(u,v) = a + u*(b - a) + v*(c - a),
    // that is x(u,v) = a1 + u*(b1 - a1) + v*(c1 - a1) and y(u,v) = a2 + u*(b2 - a2) + v*(c2 - a2).
    // g is a bijection from S' onto S, so by the change of variables formula
    //   integral over S of f(x,y) = |det Jg| * integral over S' of f(x(u,v), y(u,v)),
    // and the Jacobian determinant is the constant
    //   det Jg = (b1 - a1)*(c2 - a2) - (c1 - a1)*(b2 - a2),
    // taken in absolute value. The inner integral runs v from 0 to 1 - u and u from 0 to 1.
    ...
    float second_moment_of_area_covariance = jacobian_determinant_abs * (1.0f / 24.0f) *
                                             (a.y() * (b.x() + c.x()) + a.x() * (2.0f * a.y() + b.y() + c.y()) +
                                              b.y() * c.x() + b.x() * c.y() + 2.0f * b.x() * b.y() +
                                              2.0f * c.x() * c.y());
...
std::tuple<Vec2f, Vec2f> compute_principal_components(const Polygons &polys)
{
    ...
    float eigenvalue_a = 0.5f * (variance.x() + variance.y() +
                                 sqrt((variance.x() - variance.y()) * (variance.x() - variance.y()) +
