...
#include "luminary/geometry/index/arachne/PolygonsSegmentIndex.hpp"
#include "luminary/geometry/index/athena/PolygonsSegmentIndex.hpp"
#include "luminary/geometry/primitives/ColoredLine.hpp"
...
template VD::SegmentIt::reference VoronoiUtils::get_source_segment(const VoronoiDiagram::cell_type &, VD::SegmentIt,
                                                                   VD::SegmentIt);
template ColoredLinesIt::reference VoronoiUtils::get_source_segment(const VoronoiDiagram::cell_type &, ColoredLinesIt,
                                                                    ColoredLinesIt);
template ColoredLinesConstIt::reference VoronoiUtils::get_source_segment(const VoronoiDiagram::cell_type &,
                                                                         ColoredLinesConstIt, ColoredLinesConstIt);
...
template Point VoronoiUtils::get_source_point(const VoronoiDiagram::cell_type &, ColoredLinesConstIt,
                                              ColoredLinesConstIt);
template Point VoronoiUtils::get_source_point(const VoronoiDiagram::cell_type &, PolygonsSegmentIndexConstIt,
                                              PolygonsSegmentIndexConstIt);
template SegmentCellRange<Point> VoronoiUtils::compute_segment_cell_range(const VoronoiDiagram::cell_type &, LinesIt,
                                                                          LinesIt);
template SegmentCellRange<Point> VoronoiUtils::compute_segment_cell_range(const VoronoiDiagram::cell_type &,
                                                                          VD::SegmentIt, VD::SegmentIt);
template SegmentCellRange<Point> VoronoiUtils::compute_segment_cell_range(const VoronoiDiagram::cell_type &,
                                                                          ColoredLinesConstIt, ColoredLinesConstIt);
template SegmentCellRange<Point> VoronoiUtils::compute_segment_cell_range(const VoronoiDiagram::cell_type &,
                                                                          PolygonsSegmentIndexConstIt,
...
template PointCellRange<Point> VoronoiUtils::compute_point_cell_range(const VoronoiDiagram::cell_type &,
                                                                      PolygonsSegmentIndexConstIt,
                                                                      PolygonsSegmentIndexConstIt);
template Points VoronoiUtils::discretize_parabola(const Point &, const Arachne::PolygonsSegmentIndex &, const Point &,
                                                  const Point &, coord_t, float);
template Arachne::PolygonsSegmentIndex VoronoiUtils::get_source_point_index(const VoronoiDiagram::cell_type &,
                                                                            PolygonsSegmentIndexConstIt,
                                                                            PolygonsSegmentIndexConstIt);

template AthenaPolygonsSegmentIndexConstIt::reference VoronoiUtils::get_source_segment(
    const VoronoiDiagram::cell_type &, AthenaPolygonsSegmentIndexConstIt, AthenaPolygonsSegmentIndexConstIt);
template Point VoronoiUtils::get_source_point(const VoronoiDiagram::cell_type &, AthenaPolygonsSegmentIndexConstIt,
                                              AthenaPolygonsSegmentIndexConstIt);
template SegmentCellRange<Point> VoronoiUtils::compute_segment_cell_range(const VoronoiDiagram::cell_type &,
                                                                          AthenaPolygonsSegmentIndexConstIt,
                                                                          AthenaPolygonsSegmentIndexConstIt);
template PointCellRange<Point> VoronoiUtils::compute_point_cell_range(const VoronoiDiagram::cell_type &,
                                                                      AthenaPolygonsSegmentIndexConstIt,
                                                                      AthenaPolygonsSegmentIndexConstIt);
template Points VoronoiUtils::discretize_parabola(const Point &, const Athena::PolygonsSegmentIndex &, const Point &,
                                                  const Point &, coord_t, float);
...
VoronoiUtils::get_source_segment(const VoronoiDiagram::cell_type &cell, const SegmentIterator segment_begin,
                                 const SegmentIterator segment_end)
{
    if (!cell.contains_segment())
        throw Luminary::InvalidArgument("Voronoi cell doesn't contain a source segment!");

    if (cell.source_index() >= size_t(std::distance(segment_begin, segment_end)))
        throw Luminary::OutOfRange("Voronoi cell source index is out of range!");
...
VoronoiUtils::get_source_point(const VoronoiDiagram::cell_type &cell, const SegmentIterator segment_begin,
                               const SegmentIterator segment_end)
{
    using Segment = typename std::iterator_traits<SegmentIterator>::value_type;

    if (!cell.contains_point())
        throw Luminary::InvalidArgument("Voronoi cell doesn't contain a source point!");
    ...
    {
        throw Luminary::InvalidArgument("Function get_source_point() should only be called on point cells!");
    }
}
...
VoronoiUtils::get_source_point_index(const VD::cell_type &cell, const SegmentIterator segment_begin,
                                     const SegmentIterator segment_end)
{
    if (!cell.contains_point())
        throw Luminary::InvalidArgument("Voronoi cell doesn't contain a source point!");
    ...
    else if (cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_END_POINT)
    {
        assert(int(cell.source_index()) < std::distance(segment_begin, segment_end));
        const SegmentIterator segment_it = segment_begin + cell.source_index();
        auto next_point = (*segment_it).next();
        return typename std::iterator_traits<SegmentIterator>::value_type(next_point.polygons, next_point.poly_idx,
    ...
    {
        throw Luminary::InvalidArgument("Function get_source_point_index() should only be called on point cells!");
    }
}
...
typename boost::polygon::enable_if<typename boost::polygon::gtl_if<typename boost::polygon::is_segment_concept<
                                   ...
                                   Points>::type
VoronoiUtils::discretize_parabola(const Point &source_point, const Segment &source_segment, const Point &start,
                                  const Point &end, const coord_t approximate_step_size, float transitioning_angle)
{
    Points discretized;
    ...
    const coord_t marking_start_end_h = msx * msx / (coord_t(2) * d) + d / coord_t(2);
    ...
    bool add_marking_start = msx * int64_t(dir) > int64_t(sx - px) * int64_t(dir) &&
                             msx * int64_t(dir) < int64_t(ex - px) * int64_t(dir);
    bool add_marking_end = mex * int64_t(dir) > int64_t(sx - px) * int64_t(dir) &&
                           mex * int64_t(dir) < int64_t(ex - px) * int64_t(dir);

    const Point apex = Point(coord_t(0), d / coord_t(2)).rotated(rot_cos_theta, rot_sin_theta) + pxx;
...
typename boost::polygon::enable_if<
    typename boost::polygon::gtl_if<typename boost::polygon::is_segment_concept<typename boost::polygon::geometry_concept<
        typename std::iterator_traits<SegmentIterator>::value_type>::type>::type>::type,
    Geometry::SegmentCellRange<typename boost::polygon::segment_point_type<
        typename std::iterator_traits<SegmentIterator>::value_type>::type>>::type
VoronoiUtils::compute_segment_cell_range(const VD::cell_type &cell, const SegmentIterator segment_begin,
                                         const SegmentIterator segment_end)
{
    using Segment = typename std::iterator_traits<SegmentIterator>::value_type;
    ...
    // Infinite edges are skipped when picking the range endpoints; the range itself is assumed finite.
...
typename boost::polygon::enable_if<
    typename boost::polygon::gtl_if<typename boost::polygon::is_segment_concept<typename boost::polygon::geometry_concept<
        typename std::iterator_traits<SegmentIterator>::value_type>::type>::type>::type,
    Geometry::PointCellRange<typename boost::polygon::segment_point_type<
        typename std::iterator_traits<SegmentIterator>::value_type>::type>>::type
VoronoiUtils::compute_point_cell_range(const VD::cell_type &cell, const SegmentIterator segment_begin,
                                       const SegmentIterator segment_end)
{
    using Segment = typename std::iterator_traits<SegmentIterator>::value_type;
    using Point = typename boost::polygon::segment_point_type<Segment>::type;
    ...
    const PointIndex source_point_idx = Geometry::VoronoiUtils::get_source_point_index(cell, segment_begin,
                                                                                       segment_end);
    ...
    if (!Geometry::is_point_inside_polygon_corner(source_point_idx.prev().p(), source_point_idx.p(),
                                                  source_point_idx.next().p(), edge_query_point))
    {
    ...
    do
    {
        ...
        {
            // Polygons with collinear edges are not supported here, because collinear edges produce
            // secondary edges that run through the endpoints of the input segments.
            assert(
                (Geometry::VoronoiUtils::to_point(edge->vertex0()) == source_point_i64 || edge->is_primary()) &&
