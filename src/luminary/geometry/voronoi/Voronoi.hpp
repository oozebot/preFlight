...
#ifdef _MSC_VER
// Suppress warning C4146 in boost::polygon: unary minus operator applied to unsigned type, result still unsigned
#pragma warning(push)
...
public:
    ...
    typename boost::polygon::enable_if<
        typename boost::polygon::gtl_if<
            typename boost::polygon::is_segment_concept<typename boost::polygon::geometry_concept<
    ...
    typename boost::polygon::enable_if<
        typename boost::polygon::gtl_if<
            typename boost::polygon::is_point_concept<typename boost::polygon::geometry_concept<
    ...
    typename boost::polygon::enable_if<
        ...
        void>::type
    construct_voronoi(const PointIterator p_first, const PointIterator p_last, const SegmentIterator s_first,
    ...
    static typename boost::polygon::enable_if<
        typename boost::polygon::gtl_if<
            typename boost::polygon::is_segment_concept<typename boost::polygon::geometry_concept<
                typename std::iterator_traits<SegmentIterator>::value_type>::type>::type>::type,
        IssueType>::type
    detect_known_issues(const VoronoiDiagram &voronoi_diagram, SegmentIterator segment_begin,
    ...
    try_to_repair_degenerated_voronoi_diagram_by_rotation(SegmentIterator segment_begin, SegmentIterator segment_end,
                                                          double fix_angle);
    ...
    static typename boost::polygon::enable_if<
        typename boost::polygon::gtl_if<
            typename boost::polygon::is_segment_concept<typename boost::polygon::geometry_concept<
                typename std::iterator_traits<SegmentIterator>::value_type>::type>::type>::type,
        IssueType>::type
    detect_known_voronoi_cell_issues(const VoronoiDiagram &voronoi_diagram, SegmentIterator segment_begin,
                                     SegmentIterator segment_end);
...
public:
    using SegmentIt = std::vector<Luminary::Geometry::VoronoiDiagram::Segment>::iterator;

    friend struct boost::polygon::segment_traits<Luminary::Geometry::VoronoiDiagram::Segment>;
};
...
{
template<>
struct geometry_concept<Luminary::Geometry::VoronoiDiagram::Segment>
{
    typedef segment_concept type;
};

template<>
struct segment_traits<Luminary::Geometry::VoronoiDiagram::Segment>
{
    using coordinate_type = coord_t;
    using point_type = Luminary::Point;
    using segment_type = Luminary::Geometry::VoronoiDiagram::Segment;

    static inline point_type get(const segment_type &segment, direction_1d dir)
    {
        return dir.to_int() ? segment.to : segment.from;
    }
};
} // namespace boost::polygon
