...
#include "luminary/geometry/voronoi/VoronoiUtilsCgal.hpp"
                                    ...
                                    SegmentIterator segment_end)
{
    ...
    {
        // Detection of a non-planar Voronoi diagram.
            ...
            for (const VD::edge_type *edge = cell_range.edge_begin; edge != cell_range.edge_end; edge = edge->next())
            {
                if (edge->is_infinite())
                {
                    ...
                    return IssueType::MISSING_VORONOI_VERTEX;
                         ...
                         Luminary::cross2(source_segment_vec, edge_v1 - source_segment_from) < 0)
                {
                                                                      ...
                                                                      const double fix_angle)
{
    ...
    for (cell_type cell : m_cells)
    {
        ...
        // Only vertices on source segments are mapped; the input carries segments, not standalone points.
    }
