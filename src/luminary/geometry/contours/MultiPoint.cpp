...
/*
    ...
    location, it could probably be dropped into a namespace to avoid polluting the engine namespace.
...
Points MultiPoint::visivalingam(const Points &pts, const double tolerance)
{
    ...
    auto effective_area = [pts](const size_t &curr_pt_idx, const size_t &prev_pt_idx,
                                const size_t &next_pt_idx) -> coordf_t
    {
