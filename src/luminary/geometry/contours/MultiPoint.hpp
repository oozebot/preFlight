...
template<typename SquareLengthType, typename InputIterator, typename OutputIterator, typename TakeFloaterPredicate,
         typename PointGetter>
inline OutputIterator douglas_peucker(InputIterator begin, InputIterator end, OutputIterator out,
                                      TakeFloaterPredicate take_floater_predicate, PointGetter point_getter)
{
            ...
            for (;;)
            {
                ...
                else
                {
                    ...
                    if (const SquareLengthType l2 = v.squaredNorm(); l2 == 0)
                    {
                        ...
                        {
                            if (SquareLengthType dist_sq =
                                    (point_getter(*it) - a).template cast<SquareLengthType>().squaredNorm();
...
inline OutputIterator douglas_peucker(InputIterator begin, InputIterator end, OutputIterator out,
                                      const double tolerance, PointGetter point_getter)
{
    const auto tolerance_sq = static_cast<SquareLengthType>(sqr(tolerance));

    const auto take_floater_predicate = [&tolerance_sq](InputIterator, InputIterator,
                                                        const SquareLengthType max_dist_sq) -> bool
    {
...
inline OutputIterator douglas_peucker(Points::const_iterator begin, Points::const_iterator end, OutputIterator out,
...
template<typename OutputIterator>
inline OutputIterator douglas_peucker(Pointfs::const_iterator begin, Pointfs::const_iterator end, OutputIterator out,
                                      const double tolerance)
{
...
public:
    ...
    static Points douglas_peucker(const Points &src, const double tolerance)
    {
        return Luminary::douglas_peucker(src, tolerance);
    }
