...
Polyline Polygon::split_at_vertex(const Point &point) const
{
    ...
    throw Luminary::InvalidArgument("Point not found");
    return Polyline();
}
...
{
    return Clipper2Lib::IsPositive(Points_to_ClipperPath(this->points));
}
...
static inline void simplify_polygon_impl(const Points &points, double tolerance, bool strictly_simple, Polygons &out)
{
    ...
    Clipper2Lib::Path64 clipper_path = Points_to_ClipperPath(simplified);
    ...
    {
        Points contour_points = ClipperPath_to_Points(path);
        ...
        out.emplace_back(std::move(contour_points));
    }
}
...
bool contains(const Polygons &polygons, const Point &p, bool border_result)
{
    ...
    {
        Clipper2Lib::Path64 clipper_path = Points_to_ClipperPath(poly.points);
        Clipper2Lib::PointInPolygonResult result = Clipper2Lib::PointInPolygon(clipper_pt, clipper_path);
