...
{
    if (!Luminary::contains(contour, point, border_result))
        // Outside the outer contour, not on the contour boundary.
        return false;
    for (const Polygon &hole : this->holes)
        if (Luminary::contains(hole, point, !border_result))
...
{
    // init helper object
    Luminary::Geometry::MedialAxis ma(min_width, max_width, *this);
    ...
    */
    ...
    for (size_t i = 0; i < pp.size(); ++i)
    {
        ...
        // Use each polyline's average width instead of the global max_w for fairer filtering.
        // A narrow gap (0.1mm) shouldn't need to be as long as a wide gap (0.4mm).
        // Calculate this polyline's average width
        double polyline_avg_width = 0.0;
        if (!polyline.width.empty())
        {
            double sum = 0.0;
            for (auto w : polyline.width)
            {
                sum += w;
            }
            polyline_avg_width = sum / polyline.width.size();
        }
        // Use 1.5x instead of 2x for more aggressive gap filling
        double length_threshold = (polyline_avg_width > 0) ? polyline_avg_width * 1.5 : max_w * 1.5;

        if ((polyline.endpoints.first || polyline.endpoints.second) && polyline.length() < length_threshold)
        {
            pp.erase(pp.begin() + i);
            --i;
            removed = true;
