...
void Polyline::split_at(const Point &point, Polyline *p1, Polyline *p2) const
{
    ...
    {
        // Both halves carry the split point, so the degenerate half is a single point, not empty.
        *p1 = {point};
        *p2 = *this;
        return;
    }
...
bool remove_same_neighbor(Polylines &polylines)
{
    ...
    polylines.erase(std::remove_if(polylines.begin(), polylines.end(),
                                   [](const Polyline &p) { return p.points.size() <= 1; }),
                    polylines.end());
    return exist;
}
...
{
    if (polylines.empty())
        throw Luminary::InvalidArgument("leftmost_point() called on empty Polylines");
