...
void remove_duplicates(MutablePolygon &polygon, coord_t scaled_eps, const double max_angle)
{
    if (polygon.size() >= 3)
    {
        auto cos_max_angle_2 = Luminary::sqr<double>(cos(max_angle));
        auto scaled_eps_sqr = Luminary::sqr<int64_t>(scaled_eps);
        auto begin = polygon.begin();
        auto it = begin;
        for (++it; it != begin;)
        {
            ...
            if (v1_sqr_norm < scaled_eps_sqr)
            {
                if (Vec2i64 v2 = (*next - *prev).cast<int64_t>();
                    Luminary::sqr<double>(double(v1.dot(v2))) >
                    cos_max_angle_2 * double(v1_sqr_norm) * double(v2.squaredNorm()))
                {
                    it = it.remove();
                    continue;
                }
...
static bool clip_narrow_corner(const Vec2i64 p1, MutablePolygon::iterator &it0, MutablePolygon::iterator &it2,
                               MutablePolygon::range &unprocessed_range, int64_t dist2_current,
...
void smooth_outward(MutablePolygon &polygon, coord_t clip_dist_scaled)
{
    ...
    while (!unprocessed_range.empty() && polygon.size() > 2)
    {
        ...
        if (cross2(v1, v2) > 0)
        {
            ...
            if (dot > 0 || Luminary::sqr(double(dot)) * 2. < l2v1 * l2v2)
            {
