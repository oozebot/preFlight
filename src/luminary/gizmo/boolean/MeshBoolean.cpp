...
#include "luminary/platform/signals/TryCatchSignal.hpp"
...
void _cgal_do(Op &&op, CGALMesh &A, CGALMesh &B)
{
    ...
    if (hw_fail)
        throw Luminary::HardCrash("CGAL mesh boolean operation crashed.");

    if (!success)
        throw Luminary::RuntimeError("CGAL mesh boolean operation failed.");
}
