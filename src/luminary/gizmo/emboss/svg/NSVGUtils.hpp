...
struct NSVGLineParams
{
    ...
    // Every point coor from image(float) is multiplied by scale and rounded to integer -> Luminary::Point
    ...
    explicit NSVGLineParams(double tesselation_tolerance)
        : tesselation_tolerance(tesselation_tolerance), arc_tolerance(std::pow(tesselation_tolerance, 1 / 3.))
    {
    }
};
