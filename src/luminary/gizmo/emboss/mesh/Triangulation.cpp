...
Triangulation::Indices Triangulation::triangulate(const Points &points, const HalfEdges &constrained_half_edges)
{
    ...
    assert(std::adjacent_find(constrained_half_edges.begin(), constrained_half_edges.end()) ==
    ...
    for (CDT::Face_handle fh : faces)
        if (inside(fh))
            indices2.emplace_back(coord_t(fh->vertex(0)->info()), coord_t(fh->vertex(1)->info()),
                                  coord_t(fh->vertex(2)->info()));
    ...
    for (CDT::Face_handle fh : faces)
        if (inside(fh))
            indices.emplace_back(coord_t(fh->vertex(0)->info()), coord_t(fh->vertex(1)->info()),
                                 coord_t(fh->vertex(2)->info()));
...
Triangulation::Indices Triangulation::triangulate(const Polygons &polygons)
{
    ...
    {
        Luminary::append(points, polygon.points);
        priv::insert_edges(edges, offset, polygon);
    }
