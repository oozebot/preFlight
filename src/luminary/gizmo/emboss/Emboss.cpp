...
#include "Emboss.hpp"
#include "luminary/geometry/intersections/IntersectionPoints.hpp"
...
#include <luminary/gizmo/emboss/mesh/Triangulation.hpp> // CGAL project
...
// to heal shape
#include "luminary/geometry/clipper/ClipperUtils.hpp" // union_ex + for boldness(polygon extend(offset))
#include "luminary/gizmo/expolygon_index/ExPolygonsIndex.hpp"
#include "luminary/geometry/index/AABBTreeLines.hpp" // search structure for found close points
...
// approach to heal shape by Clipper::Closing is not working
...
// check scale when store svg
#include "luminary/geometry/diagnostics/SVG.hpp" // for visualize_heal

Points get_unique_intersections(const Luminary::IntersectionsLines &intersections); // fast forward declaration
...
void visualize_heal(const std::string &svg_filepath, const ExPolygons &expolygons)
{
    ...
    Luminary::IntersectionsLines intersections_f = get_intersections(expolygons);
...
namespace
{
using Polygon = Luminary::Polygon;
...
bool remove_when_spike(Luminary::Polygon &polygon, size_t index, const SpikeDesc &spike_desc)
{
...
void remove_spikes_in_duplicates(ExPolygons &expolygons, const Points &duplicates)
{
    if (duplicates.empty())
        return;
    auto check = [](Luminary::Polygon &polygon, const Point &d) -> bool
    {
...
bool Emboss::divide_segments_for_close_point(ExPolygons &expolygons, double distance)
{
    ...
    const std::vector<Linef> lines = Luminary::to_linesf(expolygons, ids.get_count());
...
HealedExPolygons Emboss::heal_polygons(const Polygons &shape, bool is_non_zero, unsigned int max_iteration)
{
    ...
    Clipper2Lib::Paths64 paths = Clipper2Lib::SimplifyPaths(Polygons_to_ClipperPaths(shape), 2.0);
    Polygons polygons = ClipperPaths_to_Polygons(paths);
    polygons.erase(std::remove_if(polygons.begin(), polygons.end(), [](const Polygon &p) { return p.size() < 3; }),
                   polygons.end());
    ...
    ExPolygons res = Luminary::union_ex(polygons, fill_type);
...
Points get_unique_intersections(const Luminary::IntersectionsLines &intersections)
{
    Points result;
    ...
    std::transform(intersections.begin(), intersections.end(), std::back_inserter(result),
                   [](const Luminary::IntersectionLines &i)
...
{
    Polygons result;
    for (const Luminary::Polygon &hole : holes)
...
bool heal_dupl_inter(ExPolygons &shape, unsigned max_iteration)
{
    ...
    while (--max_iteration)
    {
        ...
        shape = Luminary::diff_ex(shape, holes, ApplySafetyOffset::No);
...
const Glyph *get_glyph(int unicode, const FontFile &font, const FontProp &font_prop, Glyphs &cache,
                       fontinfo_opt &font_info_opt)
{
    const float RESOLUTION = 0.0125f; // [in mm] flatness tolerance for glyph outline tessellation
    ...
    if (!glyph.shape.empty())
    {
        if (font_prop.boldness.has_value())
        {
            ...
            glyph.shape = Luminary::union_ex(offset_ex(glyph.shape, delta));
        }
        if (font_prop.skew.has_value())
        {
            ...
            {
                for (Luminary::Point &p : polygon.points)
...
ExPolygons Luminary::union_with_delta(EmbossShape &shape, float delta, unsigned max_heal_iteration)
{
...
void Luminary::translate(ExPolygonsWithIds &expolygons_with_ids, const Point &p)
{
    for (ExPolygonsWithId &expolygons_with_id : expolygons_with_ids)
        translate(expolygons_with_id.expoly, p);
}

BoundingBox Luminary::get_extents(const ExPolygonsWithIds &expolygons_with_ids)
{
    BoundingBox bb;
...
void Luminary::center(ExPolygonsWithIds &e)
{
...
bool point_in_distance(const Coord2 &distance_sq, PolygonPoint &polygon_point, const size_t &i,
                       const Luminary::Polygon &polygon, bool is_first, bool is_reverse = false)
{
...
void point_in_distance(int32_t distance, PolygonPoint &p, const Luminary::Polygon &polygon)
{
...
void point_in_reverse_distance(int32_t distance, PolygonPoint &p, const Luminary::Polygon &polygon)
{
    Coord2 distance_sq = static_cast<Coord2>(distance) * distance;
    bool is_first = true;
