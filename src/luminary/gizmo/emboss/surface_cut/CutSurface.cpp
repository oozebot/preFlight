...
///|/
// In CGAL 6.0:
//   - add_property_map() returns std::pair<PropertyMap, bool> - use .first
//   - property_map() returns std::optional<PropertyMap> - use * or .value()
//   - AABB_traits renamed to AABB_traits_3
...
#include "luminary/gizmo/expolygon_index/ExPolygonsIndex.hpp"
#include "luminary/mesh/core/TriangleMesh.hpp" // its_merge
#include "luminary/core/containers/Sizing.hpp" // next_highest_power_of_2
...
void set_skip_for_out_of_aoi(std::vector<bool> &skip_indicies, const indexed_triangle_set &its,
                             const Project &projection, const BoundingBox &shapes_bb);
...
void set_skip_by_angle(std::vector<bool> &skip_indicies, const indexed_triangle_set &its, const Project3d &projection,
                       double max_angle = 89.);
...
struct IntersectingElement
{
    ...
    // type must be storable to 3bit -> max value is 7
...
/// will be extended by corefine edge </param>
/// <param name="shapes">2d contours</param>
/// <param name="cgal_shape">[const]Model made by shapes
/// Can't be definde as const because of corefine function input definition,
/// but it is.</param>
...
CutAOIs cut_from_model(CutMesh &cgal_model, const ExPolygons &shapes,
                       /*const*/ CutMesh &cgal_shape, float projection_ratio, const ExPolygonsIndices &s2i);
...
/// Create Reduction map as mesh property - clean on end</param>
/// <param name="models">Original models without cut modifications
/// used for differenciation
/// Clip function modify Mesh</param>
...
SurfacePatches diff_models(VCutAOIs &cuts,
                           ...
                           /*const*/ CutMeshes &models, const Project3d &projection);
...
/// <param name="shape">Source shape</param>
/// <returns>True when cover whole expolygon otherwise false</returns>
bool is_over_whole_expoly(const SurfacePatch &patch, const ExPolygons &shapes, const VCutAOIs &cutAOIs,
...
VDistances calc_distances(const SurfacePatches &patches, const CutMeshes &models, const CutMesh &shapes_mesh,
                          size_t count_shapes_points, float projection_ratio);
...
ProjectionDistances choose_best_distance(const VDistances &distances, const ExPolygons &shapes, const Point &start,
                                         const ExPolygonsIndices &s2i, const SurfacePatches &patches);
...
std::vector<bool> select_patches(const ProjectionDistances &best_distances, const SurfacePatches &patches,
                                 const ExPolygons &shapes, const BoundingBox &shapes_bb, const ExPolygonsIndices &s2i,
                                 const VCutAOIs &cutAOIs, const CutMeshes &meshes, const Project &projection);
...
/// Not const because One needs to add property for Convert indices</param>
...
/// Not const because need to [optionaly] append color property map</param>
...
void store(const ExPolygons &shapes, const std::vector<bool> &mask_distances, const Connections &connections,
...
void store(const Emboss::IProjection &projection, const Point &point_to_project, float projection_ratio,
...
SurfaceCut Luminary::cut_surface(const ExPolygons &shapes, const std::vector<indexed_triangle_set> &models,
                                 const Emboss::IProjection &projection, float projection_ratio)
{
    assert(!models.empty());
    assert(!shapes.empty());
    ...
    priv::VDistances distances = priv::calc_distances(patches, cgal_models, cgal_shape, shapes_points,
                                                      projection_ratio);
    ...
    std::vector<bool> use_patch = priv::select_patches(best_projection, patches, shapes, shapes_bb, s2i, model_cuts,
                                                       cgal_models, projection);
...
indexed_triangle_set Luminary::cut2model(const SurfaceCut &cut, const Emboss::IProject3d &projection)
{
    assert(!cut.empty());
...
void priv::set_skip_for_out_of_aoi(std::vector<bool> &skip_indicies, const indexed_triangle_set &its,
                                   const Project &projection, const BoundingBox &shapes_bb)
{
    ...
    int index = 0;
    for (Point v : {shapes_bb.min, Point{shapes_bb.min.x(), shapes_bb.max.y()}, shapes_bb.max,
...
indexed_triangle_set Luminary::its_mask(const indexed_triangle_set &its, const std::vector<bool> &mask)
{
    ...
    for (const stl_triangle_vertex_indices &f : its.indices)
        if (mask[&f - &its.indices.front()])
            result.indices.push_back(
                stl_triangle_vertex_indices(cvt_vetices[f[0]], cvt_vetices[f[1]], cvt_vetices[f[2]]));

    return result;
}

indexed_triangle_set Luminary::its_cut_AoI(const indexed_triangle_set &its, const BoundingBox &bb,
...
void priv::set_skip_by_angle(std::vector<bool> &skip_indicies, const indexed_triangle_set &its,
                             const Project3d &projection, double max_angle)
{
...
priv::CutMesh priv::to_cgal(const indexed_triangle_set &its, const std::vector<bool> &skip_indicies, bool flip)
{
    ...
    if (!flip)
    {
        for (const stl_triangle_vertex_indices &f : indices)
        {
            if (skip_indicies[&f - &indices.front()])
                continue;
            result.add_face(to_filtrated_vertices_index[f[0]], to_filtrated_vertices_index[f[1]],
    ...
    else
    {
        for (const stl_triangle_vertex_indices &f : indices)
        {
            if (skip_indicies[&f - &indices.front()])
                continue;
            result.add_face(to_filtrated_vertices_index[f[2]], to_filtrated_vertices_index[f[1]],
...
{
    Visitor(const CutMesh &object, const CutMesh &shape, EdgeShapeMap edge_shape_map, FaceShapeMap face_shape_map,
            VertexShapeMap vert_shape_map, bool *is_valid)
        : object(object)
        , shape(shape)
    ...
    void intersection_point_detected(std::size_t i_id, int sdim, HI h_f, HI h_e, const CutMesh &tm_f,
                                     const CutMesh &tm_e, bool is_target_coplanar, bool is_source_coplanar);
...
bool is_face_inside(HI hi, const CutMesh &mesh, const CutMesh &shape_mesh, const VertexShapeMap &vertex_shape_map,
...
/// <param name="shape_mesh">Vertices of mesh made by shapes</param>
/// <param name="shape2index">Convert index to shape point from ExPolygons</param>
/// <returns>False when a constrained edge has an endpoint without a shape element</returns>
bool set_face_type(FaceTypeMap &face_type_map, const CutMesh &mesh, const VertexShapeMap &vertex_shape_map,
                   const EdgeBoolMap &ecm, const CutMesh &shape_mesh, const ExPolygonsIndices &shape2index);
...
/// Modify in function: inside -> inside_processed</param>
/// <param name="mesh">mesh to process</param>
void collect_surface_data(std::queue<FI> &process, std::vector<FI> &faces, std::vector<HI> &outlines,
                          FaceTypeMap &face_type_map, const CutMesh &mesh);
...
/// Not const because it need to flag proccessed faces</param>
...
void priv::Visitor::intersection_point_detected(std::size_t i_id, int sdim, HI h_f, HI h_e, const CutMesh &tm_f,
                                                const CutMesh &tm_e, bool is_target_coplanar, bool is_source_coplanar)
{
    if (i_id >= intersections.size())
    {
        size_t capacity = Luminary::next_highest_power_of_2(i_id + 1);
    ...
    {
        // A shape edge passing exactly through a model vertex has no shape element to attribute
        // the new vertex to, so the faces along it cannot be classified: fail the cut instead.
        *is_valid = false;
        intersections[i_id] = nullptr;
        return;
    }
...
void priv::Visitor::new_vertex_added(std::size_t i_id, VI v, const CutMesh &tm)
{
    ...
    // Null when intersection_point_detected found no shape element for this intersection.
    if (intersection_ptr == nullptr)
        *is_valid = false;
    vert_shape_map[v] = intersection_ptr;
}

bool priv::is_face_inside(HI hi, const CutMesh &mesh, const CutMesh &shape_mesh, const VertexShapeMap &vertex_shape_map,
                          const ExPolygonsIndices &shape2index)
{
    ...
    if (i_from == i_to && type_from == type_to)
    {
        ...
        CGAL::Sign abcp = type_from == IntersectingElement::Type::face_1
                              ? CGAL::orientation(shape_mesh.point(VI(i)), shape_mesh.point(VI(i + 1)),
                                                  shape_mesh.point(VI(j)), p)
                              :
                              // type_from == IntersectingElement::Type::face_2
                              CGAL::orientation(shape_mesh.point(VI(j)), shape_mesh.point(VI(i + 1)),
...
bool priv::set_face_type(FaceTypeMap &face_type_map, const CutMesh &mesh, const VertexShapeMap &vertex_shape_map,
                         const EdgeBoolMap &ecm, const CutMesh &shape_mesh, const ExPolygonsIndices &shape2index)
{
    for (EI ei : mesh.edges())
    {
        ...
        // Every endpoint of a constrained edge was attributed to a shape element by the visitor;
        // one without is a cut that cannot be classified.
...
priv::CutAOIs priv::cut_from_model(CutMesh &cgal_model, const ExPolygons &shapes, CutMesh &cgal_shape,
                                   float projection_ratio, const ExPolygonsIndices &s2i)
{
    // pointer to edge or face shape_map
    VertexShapeMap vert_shape_map =
        cgal_model.add_property_map<VI, const IntersectingElement *>(vert_shape_map_name, nullptr).first;

    // detect anomalities in visitor.
    bool is_valid = true;
    // map are created when convert shapes to cgal model
    ...
    FaceTypeMap face_type_map =
        cgal_model.add_property_map<FI, FaceType>(face_type_map_name, FaceType::not_constrained).first;

    // Select inside and outside face in model
    if (!set_face_type(face_type_map, cgal_model, vert_shape_map, ecm, cgal_shape, s2i))
        return {};
#ifdef DEBUG_OUTPUT_DIR
...
void priv::collect_surface_data(std::queue<FI> &process, std::vector<FI> &faces, std::vector<HI> &outlines,
                                FaceTypeMap &face_type_map, const CutMesh &mesh)
{
    assert(!process.empty());
    assert(faces.empty());
...
priv::CutAOIs priv::create_cut_area_of_interests(const CutMesh &mesh, const ExPolygons &shapes,
                                                 FaceTypeMap &face_type_map)
{
...
priv::VDistances priv::calc_distances(const SurfacePatches &patches, const CutMeshes &models,
                                      const CutMesh &shapes_mesh, size_t count_shapes_points, float projection_ratio)
{
    priv::VDistances result(count_shapes_points);
    for (const SurfacePatch &patch : patches)
    {
        // map is created during intersection by corefine visitor
        const VertexShapeMap &vert_shape_map = *models[patch.model_id].property_map<VI, const IntersectingElement *>(
            vert_shape_map_name);
...
uint32_t get_closest_point_index(const SearchData &sd, size_t line_idx, const Vec2d &hit_point,
                                 const ExPolygons &shapes, const ExPolygonsIndices &s2i);

// use AABB Tree Lines to find closest point
uint32_t find_closest_point_index(const Point &p, const ExPolygons &shapes, const ExPolygonsIndices &s2i,
                                  const std::vector<bool> &mask);

std::pair<uint32_t, uint32_t> find_closest_point_pair(const ExPolygons &shapes, const std::vector<bool> &done_shapes,
                                                      const ExPolygonsIndices &s2i, const std::vector<bool> &mask);
...
void fill_polygon_distances(const ProjectionDistance &pd, uint32_t index, const ExPolygonsIndex &id,
                            ProjectionDistances &result, const ExPolygon &shape, const VDistances &distances);

// search for closest projection for expolygon
// choose correct cut by source point
void fill_shape_distances(uint32_t start_index, const ProjectionDistance *start_pd, ProjectionDistances &result,
                          const ExPolygonsIndices &s2i, const ExPolygon &shape, const VDistances &distances);

// find close points between finished and unfinished ExPolygons
ClosePoint find_close_point(const Point &p, ProjectionDistances &result, std::vector<bool> &finished_shapes,
                            const ExPolygonsIndices &s2i, const ExPolygons &shapes);

} // namespace priv

float priv::calc_size_sq(const Point &p)
{
    // p.squaredNorm() can't be use due to overflow max int value
...
uint32_t priv::get_closest_point_index(const SearchData &sd, size_t line_idx, const Vec2d &hit_point,
                                       const ExPolygons &shapes, const ExPolygonsIndices &s2i)
{
...
// use AABB Tree Lines
uint32_t priv::find_closest_point_index(const Point &p, const ExPolygons &shapes, const ExPolygonsIndices &s2i,
                                        const std::vector<bool> &mask)
{
    ...
    Vec2d hit_point;
    Vec2d p_d = p.cast<double>();
    [[maybe_unused]] double distance_sq = AABBTreeLines::squared_distance_to_indexed_lines(sd.lines, sd.tree, p_d,
                                                                                           line_idx, hit_point);
    ...
    for (size_t shape_index = 0; shape_index < shapes.size(); shape_index++)
    {
        ...
        auto search_in_polygon = [&index, &cp, &sd, &mask](const Polygon &polygon)
        {
            for (size_t i = 0; i < polygon.size(); ++i, ++index)
            {
                ...
                size_t line_idx = std::numeric_limits<size_t>::max();
                Vec2d hit_point;
                double distance_sq = AABBTreeLines::squared_distance_to_indexed_lines(sd.lines, sd.tree, p_d, line_idx,
                                                                                      hit_point, cp.distance_sq);
...
void priv::fill_polygon_distances(const ProjectionDistance &pd, uint32_t index, const ExPolygonsIndex &id,
                                  ProjectionDistances &result, const ExPolygon &shape, const VDistances &distances)
{
...
void priv::fill_shape_distances(uint32_t start_index, const ProjectionDistance *start_pd, ProjectionDistances &result,
                                const ExPolygonsIndices &s2i, const ExPolygon &shape, const VDistances &distances)
{
...
priv::ClosePoint priv::find_close_point(const Point &p, ProjectionDistances &result, std::vector<bool> &finished_shapes,
                                        const ExPolygonsIndices &s2i, const ExPolygons &shapes)
{
    // result
    ClosePoint cp;
...
// IMPROVE: when select distance fill in all distances from Patch
priv::ProjectionDistances priv::choose_best_distance(const VDistances &distances, const ExPolygons &shapes,
                                                     const Point &start, const ExPolygonsIndices &s2i,
...
void create_face_types(FaceTypeMap &map, const CutMesh &tm1, const CutMesh &tm2, const EdgeBoolMap &ecm,
                       const VertexSourceMap &sources);

/// <summary>
/// Implement 'cut' Minus 'clipper', where clipper is reverse input Volume
/// clipper will be modified (corefined by cut)
/// </summary>
...
/// Need temporary add property map for convert vertices</param>
/// <param name="rmap">Options to reduce vertices from fis.
/// Used for skip vertices made by diagonal edge in rectangle of shape side</param>
/// <returns>Patch</returns>
SurfacePatch create_surface_patch(const std::vector<FI> &fis,
                                  /*const*/ CutMesh &mesh, const ReductionMap *rmap = nullptr);

} // namespace priv

void priv::create_face_types(FaceTypeMap &map, const CutMesh &tm1, const CutMesh &tm2, const EdgeBoolMap &ecm,
                             const VertexSourceMap &sources)
{
...
bool priv::clip_cut(SurfacePatch &cut, CutMesh clipper)
{
    ...
    {
        // A cut that lies wholly inside or wholly outside the clipper is discarded along with the failures.
...
priv::SurfacePatch priv::create_surface_patch(const std::vector<FI> &fis,
                                              /* const */ CutMesh &mesh, const ReductionMap *rmap)
{
...
/// Can't be const. For indexing vetices need temporary add property map</param>
...
SurfacePatch separate_patch(const std::vector<FI> &fis,
                            /* const*/ SurfacePatch &patch, const CvtVI2VI &cvt_from);
...
{
    // Parity of ray intersections along the projection direction; a mesh open in that direction gives a
    // meaningless answer.
    ...
    assert(
        ((tree.number_of_intersected_primitives(Ray(a, P3(2 * a.x() - b.x(), 2 * a.y() - b.y(), 2 * a.z() - b.z()))) %
          2) == 1) == is_in);
    return is_in;
}
            ...
            for (size_t model_index2 = 0; model_index2 < models.size(); ++model_index2)
            {
                ...
                for (SurfacePatchEx &patch_ex : aoi_patches)
                {
                    ...
                    else
                    {
                        ...
                        // it is possible not neccessary: e.g. one model
...
bool priv::is_over_whole_expoly(const SurfacePatch &patch, const ExPolygons &shapes, const VCutAOIs &cutAOIs,
                                const CutMeshes &meshes)
{
...
std::vector<bool> priv::select_patches(const ProjectionDistances &best_distances, const SurfacePatches &patches,
                                       const ExPolygons &shapes, const BoundingBox &shapes_bb,
                                       const ExPolygonsIndices &s2i, const VCutAOIs &cutAOIs, const CutMeshes &meshes,
                                       const Project &projection)
{
    ...
    for (size_t shape_index = 0; shape_index < shapes.size(); shape_index++)
    {
        ...
        for (size_t i = patch_shapes.size(); i > 0; --i)
        {
            ...
            Point text_pos = get_extents(p.shape).center().cast<coord_t>();
            svg.draw_text(text_pos, std::to_string(i - 1).c_str(), "orange",
    ...
    for (HI hi : outlines)
    {
        ...
        else if (loop_connect != nullptr)
        {
            ...
            if (loop_tail != nullptr)
            {
                ...
                loop_connect->insert(loop_connect->end(), make_move_iterator(loop_tail->begin()),
...
Polygons priv::unproject_loops(const SurfacePatch &patch, const Project &projection, Vec2d &depth_range)
{
    ...
    // this method is working only when patch did not contain outward faces
    Polygons polys;
...
ExPolygon priv::to_expoly(const SurfacePatch &patch, const Project &projection, Vec2d &depth_range)
{
    ...
    FillRule fill_type = FillRule::EvenOdd;
    ExPolygons expolys = Luminary::union_ex(polys, fill_type);
    if (expolys.size() == 1)
    ...
    if (!sc_add.contours.empty())
    {
        ...
        for (std::vector<SurfaceCut::Index> &cut : sc_add.contours)
            for (SurfaceCut::Index &i : cut)
                i += offset;
        Luminary::append(sc.contours, std::move(sc_add.contours));
    }
...
indexed_triangle_set priv::create_indexed_triangle_set(const std::vector<FI> &faces, const CutMesh &mesh)
{
    ...
    for (FI fi : faces)
    {
        ...
        do
        {
            ...
            t[ti++] = coord_t(res - vertices.begin());
            if (res == vertices.end())
...
void priv::store(const CutAOIs &aois, const CutMesh &mesh, const std::string &dir)
{
    auto create_outline_its = [&mesh](const std::vector<HI> &outlines) -> indexed_triangle_set
    {
        ...
        for (HI hi : outlines)
        {
            ...
            its.indices.push_back(Vec3i(coord_t(ai), coord_t(ai2), coord_t(bi)));
            its.indices.push_back(Vec3i(coord_t(ai2), coord_t(bi2), coord_t(bi)));
        }
        return its;
    };
...
void priv::store(const ExPolygons &shapes, const std::vector<bool> &mask, const Connections &connections,
                 const std::string &file_svg)
{
...
indexed_triangle_set priv::create_contour_its(const indexed_triangle_set &its, const std::vector<unsigned int> &contour)
{
    ...
    for (unsigned int vi : contour)
    {
        ...
        result.indices.push_back(Vec3i(coord_t(ai), coord_t(bi), coord_t(ai2)));
        result.indices.push_back(Vec3i(coord_t(ai2), coord_t(bi), coord_t(bi2)));
        prev_vi = vi;
    }
    return result;
}
...
void priv::store(const Emboss::IProjection &projection, const Point &point_to_project, float projection_ratio,
                 const std::string &obj_filename)
{
...
bool Luminary::corefine_test(const std::string &model_path, const std::string &shape_path)
{
