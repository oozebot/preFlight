...
#include "luminary/gizmo/halfedge/SurfaceMesh.hpp"
...
static std::tuple<Vec3d, double, double> get_center_and_radius(const std::vector<Vec3d> &points,
                                                               const Transform3d &trafo, const Transform3d &trafo_inv)
{
    Vec2ds out;
    double z = 0.;
...
public:
    ...
    struct PlaneData
    {
        ...
        std::vector<std::vector<Vec3d>> borders;
...
void MeasuringImpl::update_planes()
{
    ...
    m_planes.clear();
    m_planes.reserve(num_of_facets /
                     5); // empty plane data object is quite lightweight, let's save the initial reallocations
...
void MeasuringImpl::extract_features(int plane_idx)
{
    ...
    for (const std::vector<Vec3d> &border : plane.borders)
    {
            ...
            if (err < 0.05)
            {
                ...
                bool lengths_match =
                    std::all_of(border.begin() + 2, border.end(),
                                [is_polygon](const Vec3d &pt)
                                {
                                    return Luminary::is_approx((pt - *((&pt) - 1)).squaredNorm(),
                    ...
                    if (is_polygon)
                    {
                        ...
                        for (int j = 0; j < int(border.size()); ++j)
                            plane.surface_features.emplace_back(
                                SurfaceFeature(SurfaceFeatureType::Edge, border[j == 0 ? border.size() - 1 : j - 1],
                                               border[j], std::make_optional(center)));
                    }
                    else
                    {
        ...
        if (!done)
        {
            ...
            {
                return Luminary::is_approx(a, b, 0.01);
            };
            ...
            while (i != first_pt_idx || first_iter)
            {
                ...
                {
                    if (circle && single_circle.size() >= 5)
                    { // Less than 5 vertices? Not a circle.
                        ...
                        if (accept_circle)
                        {
                            ...
                            if (accept_circle)
                            {
                                ...
                                circles.emplace_back(SurfaceFeature(SurfaceFeatureType::Circle, center, plane.normal,
                                                                    std::nullopt, radius));
                            }
                        }
                    }
            ...
            else if (circles_idxs.size() > 1 || circles_idxs.front().first != circles_idxs.front().second)
            {
                ...
                while (true)
                {
                    i = offset_to_index(i, 1);
                    edges.emplace_back(
                        SurfaceFeature(SurfaceFeatureType::Edge, border[offset_to_index(i, -1)], border[i]));
            ...
            for (int i = edges.size() - 1; i >= 0; --i)
            {
                ...
                if (Luminary::is_approx(first_end, second_start) &&
                    Luminary::is_approx(
                        (first_end - first_start).normalized().dot((second_end - second_start).normalized()), 1.))
                {
                    // The edges have the same direction and share a point. Merge them.
                    edges[i == 0 ? edges.size() - 1 : i - 1] = SurfaceFeature(SurfaceFeatureType::Edge, first_start,
                                                                              second_end);
...
std::optional<SurfaceFeature> MeasuringImpl::get_feature(size_t face_idx, const Vec3d &point)
{
    ...
    for (size_t i = 0; i < plane.surface_features.size() - 1; ++i)
    {
        ...
        { // feature pairs that do not produce a strict distance are skipped
...
const AngleAndEdges AngleAndEdges::Dummy = {
    0.0, Vec3d::Zero(), {Vec3d::Zero(), Vec3d::Zero()}, {Vec3d::Zero(), Vec3d::Zero()}, 0.0, true};
...
static AngleAndEdges angle_edge_plane(const std::pair<Vec3d, Vec3d> &e, const std::tuple<int, Vec3d, Vec3d> &p)
{
    ...
    if (are_parallel(e1e2_unit, normal))
    {
        ...
        return AngleAndEdges(0.5 * double(PI), inters, std::make_pair(e1, e2), edge_on_plane, radius,
                             inters.isApprox(e1));
    }
...
static AngleAndEdges angle_plane_plane(const std::tuple<int, Vec3d, Vec3d> &p1, const std::tuple<int, Vec3d, Vec3d> &p2)
{
    ...
    const auto [intersection_line_direction, intersection_line_origin] = intersection_plane_plane(normal1, origin1,
                                                                                                  normal2, origin2);

    // Project planes' origin on intersection line
    const Eigen::ParametrizedLine<double, 3> intersection_line =
        Eigen::ParametrizedLine<double, 3>(intersection_line_origin, intersection_line_direction);
...
MeasurementResult get_measurement(const SurfaceFeature &a, const SurfaceFeature &b, const Measuring *measuring)
{
    ...
    if (f1.get_type() == SurfaceFeatureType::Point)
    {
        ...
        else if (f2.get_type() == SurfaceFeatureType::Edge)
        {
            ...
            { // the result is the closer of the endpoints
                ...
                result.distance_strict = std::make_optional(
                    DistAndPoints{std::sqrt(std::min(dist_start_sq, dist_end_sq) + sqr(dist_inf)), f1.get_point(),
                                  s_is_closer ? s : e});
            }
        ...
        else if (f2.get_type() == SurfaceFeatureType::Plane)
        {
            ...
            result.distance_infinite = std::make_optional(
                DistAndPoints{plane.absDistance(f1.get_point()), f1.get_point(), plane.projection(f1.get_point())});
            // Only the infinite plane distance is produced for a point and a plane.
        }
        ///////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////
    ...
    else if (f1.get_type() == SurfaceFeatureType::Edge)
    {
        if (f2.get_type() == SurfaceFeatureType::Edge)
        {
            ...
            auto add_point_edge_distance = [&distances](const Vec3d &v, const std::pair<Vec3d, Vec3d> &e)
            {
                const MeasurementResult res = get_measurement(SurfaceFeature(v),
                                                              SurfaceFeature(SurfaceFeatureType::Edge, e.first,
                                                                             e.second));
        ...
        else if (f2.get_type() == SurfaceFeatureType::Circle)
        {
            ...
            const Eigen::ParametrizedLine<double, 3> line = Eigen::ParametrizedLine<double, 3>::Through(e.first,
                                                                                                        e.second);
    ...
    else if (f1.get_type() == SurfaceFeatureType::Circle)
    {
        if (f2.get_type() == SurfaceFeatureType::Circle)
        {
            ...
            if (!are_parallel(n0, n1))
            {
                ...
                // Parameters for polynomial root finding. roots[] holds the roots, uniqueRoots keeps only
                // the distinct ones, and pairs[] holds the matching (cosine, sine) values.
            ...
            result.distance_infinite = std::make_optional(DistAndPoints{std::sqrt(candidates[0].sqrDistance),
