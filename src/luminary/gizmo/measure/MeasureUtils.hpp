...
// src/luminary has no precompiled header, so this header owns every include it uses instead of
// reaching them through whatever its caller happened to include first. Point.hpp is here for the
// same reason as the standard ones: get_orthogonal takes and returns a Vec3d.
#include <algorithm>
#include <cmath>
#include <cstddef>
...
public:
    ...
    void SetCoefficients(double value) { std::fill(m_coefficient.begin(), m_coefficient.end(), value); }
...
public:
    ...
    static int32_t FindRecursive(int32_t degree, double const *c, double tmin, double tmax, uint32_t maxIterations,
                                 double *roots)
    {
        ...
        const int32_t numDerivRoots = FindRecursive(degree - 1, &derivCoeff[0], tmin, tmax, maxIterations,
                                                    &derivRoots[0]);
