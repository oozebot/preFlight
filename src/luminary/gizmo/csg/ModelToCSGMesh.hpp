...
#include "luminary/mesh/core/MeshSplitImpl.hpp"
                      ...
                      int parts_to_include = mpartsPositive)
{
    ...
    for (const ModelVolume *vol : mo.volumes)
    {
            ...
            ((do_positives && vol->is_model_part()) || (do_negatives && vol->is_negative_volume())))
        {
            ...
            if (attempt_split)
            {
                ...
                its_split(vol->mesh().its,
                          SplitOutputFn{[&meshes_union, &meshes_difference, &split_failed](indexed_triangle_set &&its)
                                        {
                                            if (its.empty())
                                                return;
    ...
    // if (do_drillholes) { ... }
    (void) do_drillholes; // suppress unused variable warning
}
