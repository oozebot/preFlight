...
#include "luminary/core/containers/MutablePriorityQueue.hpp"
...
public:
    ...
    {
        T det = m[a11] * m[a22] * m[a33] + m[a13] * m[a21] * m[a32] + m[a12] * m[a23] * m[a31] -
...
std::tuple<TriangleInfos, VertexInfos, EdgeInfos, Errors> init(const indexed_triangle_set &its,
                                                               ThrowOnCancel &throw_on_cancel, StatusFn &status_fn);
std::optional<uint32_t> find_triangle_index1(uint32_t vi, const VertexInfo &v_info, uint32_t ti,
...
bool degenerate(uint32_t vi, uint32_t ti0, uint32_t ti1, const VertexInfo &v_info, const EdgeInfos &e_infos,
                const Indices &indices);
bool create_no_volume(uint32_t vi0, uint32_t vi1, uint32_t ti0, uint32_t ti1, const VertexInfo &v_info0,
                      const VertexInfo &v_info1, const EdgeInfos &e_infos, const Indices &indices);
...
Error calculate_error(uint32_t ti, const Triangle &t, const Vertices &vertices, const VertexInfos &v_infos,
                      unsigned char &min_index);
void remove_triangle(EdgeInfos &e_infos, VertexInfo &v_info, uint32_t ti);
void change_neighbors(EdgeInfos &e_infos, VertexInfos &v_infos, uint32_t ti0, uint32_t ti1, uint32_t vi0, uint32_t vi1,
                      uint32_t vi_top0, const Triangle &t1, CopyEdgeInfos &infos, EdgeInfos &e_infos1);
void compact(const VertexInfos &v_infos, const TriangleInfos &t_infos, const EdgeInfos &e_infos,
...
bool check_neighbors(const indexed_triangle_set &its, const TriangleInfos &t_infos, const VertexInfos &v_infos,
...
void Luminary::its_quadric_edge_collapse(indexed_triangle_set &its, uint32_t triangle_count, float *max_error,
                                         std::function<void(void)> throw_on_cancel, std::function<void(int)> status_fn)
{
    // check input
    ...
    auto setter = [&ti_2_mpqi](const Error &e, size_t index)
    {
        ti_2_mpqi[e.triangle_index] = index;
    };
    auto less = [](const Error &e1, const Error &e2) -> bool
    {
    ...
    while (actual_triangle_count > triangle_count && !mpq.empty())
    {
            ...
            is_flipped(new_vertex0, ti0, ti1, v_info1, t_infos, e_infos, its))
        {
            ...
            Vec3i ord = (errors[0] < errors[1])
                            ? ((errors[0] < errors[2])
                                   ? ((errors[1] < errors[2]) ? Vec3i(coord_t(0), coord_t(1), coord_t(2))
                                                              : Vec3i(coord_t(0), coord_t(2), coord_t(1)))
                                   : Vec3i(coord_t(2), coord_t(0), coord_t(1)))
                            : ((errors[1] < errors[2])
                                   ? ((errors[0] < errors[2]) ? Vec3i(coord_t(1), coord_t(0), coord_t(2))
                                                              : Vec3i(coord_t(1), coord_t(2), coord_t(0)))
                                   : Vec3i(coord_t(2), coord_t(1), coord_t(0)));
...
std::optional<uint32_t> QuadricEdgeCollapse::find_triangle_index1(uint32_t vi, const VertexInfo &v_info, uint32_t ti0,
                                                                  const EdgeInfos &e_infos, const Indices &indices)
{
...
bool QuadricEdgeCollapse::is_flipped(const Vec3f &new_vertex, uint32_t ti0, uint32_t ti1, const VertexInfo &v_info,
                                     const TriangleInfos &t_infos, const EdgeInfos &e_infos,
                                     const indexed_triangle_set &its)
{
    ...
    for (size_t ei = v_info.start; ei < v_info_end; ++ei)
    {
        ...
        float dot = d1.dot(d2);
        if (dot > triangle_beauty_threshold || dot < -triangle_beauty_threshold)
        { // OK, the new triangle is suspiciously ugly, but it can still be better than the original
            const Vec3f &v_orig = its.vertices[t[(e_info.edge) % 3]];
            Vec3f d1_orig = vf - v_orig;
            d1_orig.normalize();
            Vec3f d2_orig = vs - v_orig;
            d2_orig.normalize();
            if (std::fabs(d1_orig.dot(d2_orig)) < std::fabs(dot))
            { // original was not that ugly, so return flipped
                return true;
...
bool QuadricEdgeCollapse::degenerate(uint32_t vi, uint32_t ti0, uint32_t ti1, const VertexInfo &v_info,
                                     const EdgeInfos &e_infos, const Indices &indices)
{
...
bool QuadricEdgeCollapse::create_no_volume(uint32_t vi0, uint32_t vi1, uint32_t ti0, uint32_t ti1,
...
Error QuadricEdgeCollapse::calculate_error(uint32_t ti, const Triangle &t, const Vertices &vertices,
                                           const VertexInfos &v_infos, unsigned char &min_index)
{
...
void QuadricEdgeCollapse::change_neighbors(EdgeInfos &e_infos, VertexInfos &v_infos, uint32_t ti0, uint32_t ti1,
                                           uint32_t vi0, uint32_t vi1, uint32_t vi_top0, const Triangle &t1,
                                           CopyEdgeInfos &infos, EdgeInfos &e_infos1)
{
...
void QuadricEdgeCollapse::compact(const VertexInfos &v_infos, const TriangleInfos &t_infos, const EdgeInfos &e_infos,
                                  indexed_triangle_set &its)
{
    uint32_t vi_new = 0;
...
void QuadricEdgeCollapse::store_surround(const char *obj_filename, size_t triangle_index, int depth,
                                         const indexed_triangle_set &its, const VertexInfos &v_infos,
                                         const EdgeInfos &e_infos)
{
...
bool QuadricEdgeCollapse::check_neighbors(const indexed_triangle_set &its, const TriangleInfos &t_infos,
                                          const VertexInfos &v_infos, const EdgeInfos &e_infos)
{
    ...
    for (size_t vi = 0; vi < its.vertices.size(); vi++)
    {
        ...
        std::copy(e_infos.begin() + v_info.start, e_infos.begin() + v_info.start + v_info.count,
