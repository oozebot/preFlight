...
bool is_first_path_touching_second_path(const AABBTreeLines::LinesDistancer<Line> &first_distancer,
                                        ...
                                        const BoundingBox &second_distancer_bbox, const double touch_distance_threshold)
{
    for (const Line &line : first_distancer.get_lines())
    {
        if (bbox_point_distance(second_distancer_bbox, line.a) < touch_distance_threshold &&
            second_distancer.distance_from_lines<false>(line.a) < touch_distance_threshold)
        {
            return true;
        }
    }
    ...
    if (bbox_point_distance(second_distancer_bbox, first_distancer_last_pt) &&
        second_distancer.distance_from_lines<false>(first_distancer_last_pt) < touch_distance_threshold)
    {
        return true;
    }
...
bool are_paths_touching(const AABBTreeLines::LinesDistancer<Line> &first_distancer,
                        const BoundingBox &first_distancer_bbox,
                        const AABBTreeLines::LinesDistancer<Line> &second_distancer,
                        const BoundingBox &second_distancer_bbox, const double touch_distance_threshold)
{
    if (is_first_path_touching_second_path(first_distancer, second_distancer, second_distancer_bbox,
                                           touch_distance_threshold))
    {
        return true;
    }
    else if (is_first_path_touching_second_path(second_distancer, first_distancer, first_distancer_bbox,
                                                touch_distance_threshold))
    {
        return true;
    }
...
void sort_paths(RandomAccessIterator begin, RandomAccessIterator end, Point start,
                const double touch_distance_threshold, ToLines convert_to_lines)
{
    ...
    for (size_t curr_path_idx = 0; curr_path_idx < paths_count; ++curr_path_idx)
    {
        for (size_t next_path_idx = curr_path_idx + 1; next_path_idx < paths_count; ++next_path_idx)
        {
            ...
            if (are_paths_touching(distancers[curr_path_idx], curr_path_bbox, distancers[next_path_idx], next_path_bbox,
                                   touch_distance_threshold))
            {
    ...
    while (unsorted_idx < paths_count)
    {
        ...
        unsorted_idx++;
        current_point = reverse ? distancers[next_idx].get_lines().front().a
                                : distancers[next_idx].get_lines().back().b;
