...
///|/ Copyright (c) Prusa Research 2016 - 2023 Vojtěch Bubník @bubnikv, Lukáš Matěna @lukasmatena, Lukáš Hejl @hejllukas, David Kocík @kocikdav, Tomáš Mészáros @tamasmeszaros, Pavel Mikuš @Godrak, Filip Sykala @Jony01
    ...
    else
    {
        ...
        for (int iter = int(num_segments) - 2;; --iter)
        {
            ...
            else
            {
                ...
                // Connected end points stay in the KD tree and are rejected by the search
                // predicate; rebuilding the tree costs more than the stale entries.
                assert(validate_graph_and_queue());
            }
        }
        assert(queue.empty());
        ...
        if (REVERSE_COULD_FAIL)
        {
            ...
            if (failed)
                // As a last resort, chain by closest point, which is not sensitive to edge reversal constraints.
                                              ...
                                              std::vector<std::pair<size_t, bool>> &chain)
{
    if (chain.size() < 3)
        return;
    // The two-exchange polish is quadratic in the entity count; the greedy chain stands on
    // its own above the cap (the same guard as chain_polylines: a large canopy's fill puts
    // eleven thousand support paths on one layer, and this runs per layer at emission).
    constexpr size_t improve_cap = 2000;
    if (chain.size() > improve_cap)
    {
        DBG_COUNT("CHAIN_IMPROVE_SKIPPED");
        return;
    }
...
Polylines chain_polylines(Polylines &&polylines, const Point *start_near)
{
    ...
    if (!polylines.empty())
    {
        ...
        // The two-exchange polish is quadratic in the segment count: at a few thousand
        // polylines one call costs minutes (a large canopy's lightning fill reached eleven
        // thousand lines on one layer). The greedy chain above stands on its own there; the
        // polish runs only under the cap.
        constexpr size_t improve_cap = 2000;
        if (out.size() > improve_cap)
            DBG_COUNT("CHAIN_IMPROVE_SKIPPED");
        else if (out.size() > 1)
        {
...
PolyPaths chain_clipper_polypaths(const Points &points, const PolyPaths &items)
{
...
PolyPaths order_nodes(const PolyPaths &nodes)
{
    ...
    for (const PolyPath64 *node : nodes)
    {
    ...
    PolyPaths ordered_nodes = chain_clipper_polypaths(ordering_points, nodes);

    return ordered_nodes;
}

static void traverse_pt_outside_in(PolyPaths &&nodes, Polygons *retval)
{
    // collect ordering points
    Points ordering_points;
    ordering_points.reserve(nodes.size());
    ...
    // Each nesting level is chained on its own, without carrying the previous level's end point.
    for (PolyPath64 *node : chain_clipper_polypaths(ordering_points, nodes))
    {
        const auto &polygon = node->Polygon();
        retval->emplace_back(ClipperPath_to_Polygon(polygon));
        if (node->IsHole())
            retval->back().reverse();
