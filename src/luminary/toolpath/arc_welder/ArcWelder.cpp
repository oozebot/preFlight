...
///|/ Copyright (c) ArcWelderLib 2021 Brad Hochgesang @FormerLurker
///|/
...
static std::optional<Circle> try_create_circle(const Point &p1, const Point &p2, const Point &p3,
                                               const double max_radius)
{
    if (auto center = Luminary::Geometry::try_circle_center(p1.cast<double>(), p2.cast<double>(), p3.cast<double>(),
                                                            SCALED_EPSILON);
        center)
    {
...
static inline bool circle_approximation_sufficient(const Circle &circle, const Points::const_iterator begin,
                                                   const Points::const_iterator end, const double tolerance)
{
...
static std::optional<Circle> try_create_circle(const Points::const_iterator begin, const Points::const_iterator end,
                                               const double max_radius, const double tolerance)
{
    ...
    if (size == 3)
    {
        ...
        if (out)
        {
            ...
            {
                if (std::optional<Vec2d> opt_center = ArcWelder::arc_fit_center_gauss_newton_ls(
                        first_point, last_point, center_point, fpts.begin(), fpts.end(), 3);
                    opt_center)
                {
    ...
    else
    {
        ...
        if (!circle)
        {
            ...
            {
                Vec2i64 c = (first_point.cast<int64_t>() + last_point.cast<int64_t>()) / int64_t(2);
                ...
                assert(point_on_bisector.x() != std::numeric_limits<coord_t>::max() &&
                       point_on_bisector.y() != std::numeric_limits<coord_t>::max());
        ...
        if (circle)
        {
            ...
            std::optional<Vec2d> opt_center = ArcWelder::arc_fit_center_gauss_newton_ls(first_point, last_point,
                                                                                        circle->center.cast<double>(),
                                                                                        fpts.begin(), fpts.end(), 5);
            if (opt_center)
            {
                ...
                if (const double r2 = (*opt_center - first_point).squaredNorm(); r2 < max_radius * max_radius)
                {
                    ...
                    {
                        // The fit is rejected; no second attempt is made around the worst offender.
                    }
                }
            }
        }
        ...
        // Such a loop makes the time complexity of the arc fitting O(n^3).
        else {
...
static inline std::optional<Arc> try_create_arc_impl(const Circle &circle, const Points::const_iterator begin,
                                                     const Points::const_iterator end, const double tolerance,
                                                     const double path_tolerance_percent)
{
    ...
    else
    {
        ...
        return std::make_optional<Arc>(
            Arc{*begin, *std::prev(end), circle.center, angle > M_PI ? -circle.radius : circle.radius, orientation});
    }
}

static inline std::optional<Arc> try_create_arc(const Points::const_iterator begin, const Points::const_iterator end,
...
static inline Segments::iterator douglas_peucker_in_place(Segments::iterator begin, Segments::iterator end,
...
Path fit_path(const Points &src_in, double tolerance, double fit_circle_percent_tolerance)
{
    ...
    double tolerance2 = Luminary::sqr(tolerance);
    ...
    if (tolerance <= 0 || src_in.size() <= 2)
    {
        ...
        std::transform(src_in.begin(), src_in.end(), std::back_inserter(out),
                       [](const Point &p) -> Segment { return {p}; });
    }
    ...
    else
    {
        ...
        for (auto it = std::next(src.begin()); it != src.end();)
        {
            ...
            while (end != src.end())
            {
                ...
                if (std::optional<Arc> this_arc = try_create_arc(begin, next_end, ArcWelder::default_scaled_max_radius,
                    ...
                    this_arc)
                {
                    ...
                    while (std::next(end) != src.end())
                    {
                        assert(end == next_end);
                        {
                            ...
                            do
                            {
                                if (std::abs((arc->center.cast<double>() - next_end->cast<double>()).norm() -
                                             arc->radius) >= tolerance ||
                                    inside_arc_wedge_vectors(v1, v2, arc->radius > 0,
                                                             arc->direction == Orientation::CCW,
                        ...
                        for (;;)
                        {
                            this_arc = try_create_arc(begin, next_end, ArcWelder::default_scaled_max_radius, tolerance,
                                                      fit_circle_percent_tolerance);
                            if (this_arc)
                            {
                                arc = this_arc;
                                end = next_end;
        ...
        fit_end:
            ...
            if (arc)
            {
                ...
                if (out.size() - begin_pl_idx > 2)
                {
                    ...
                    assert(std::all_of(out.begin() + begin_pl_idx + 1, out.end(),
                                       [](const Segment &seg) { return seg.linear(); }));
    ...
    while (distance > 0)
    {
        ...
        else
        {
            ...
            if (len > distance)
            {
                ...
                const bool flip_radius_sign = last.radius < 0 && std::abs(angle) > M_PI &&
                                              std::abs(angle - rotate_by_angle) <= M_PI;

                path.push_back({last.point.rotated(rotate_by_angle, arc_center(path.back().point.cast<double>(),
                                                                               last.point.cast<double>(),
                                                                               double(last.radius), last.ccw())
                                                                        .cast<coord_t>()),
...
PathSegmentProjection point_to_path_projection(const Path &path, const Point &point, double search_radius2)
{
    ...
    else
    {
        ...
        for (auto it = std::next(path.cbegin()); it != path.cend(); ++it)
        {
            ...
            {
                // Circular arc
                Vec2i64 center = arc_center(prev.cast<double>(), it->point.cast<double>(), double(it->radius),
                                            it->ccw())
                                     .cast<int64_t>();
...
std::pair<Path, Path> split_at(const Path &path, const PathSegmentProjection &proj, const double min_segment_length)
{
    ...
    if (!proj.valid() || proj.segment_id + 1 == path.size() ||
        (proj.segment_id + 2 == path.size() && proj.point == path.back().point))
        out.first = path;
