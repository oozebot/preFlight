...
static inline FlowRole opt_key_to_flow_role(const std::string &opt_key)
{
    if (opt_key == "perimeter_extrusion_width" ||
        ...
        throw Luminary::RuntimeError("opt_key_to_flow_role: invalid argument");
};
...
{
    if (height <= 0)
        throw Luminary::InvalidArgument("Invalid flow height supplied to new_from_config_width()");
...
Flow Flow::with_spacing(float new_spacing) const
{
    ...
    else
    {
        ...
        if (out.m_width < out.m_height)
            throw Luminary::InvalidArgument("Invalid spacing supplied to Flow::with_spacing()");
    }
