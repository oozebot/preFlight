...
public:
    ...
    ExtrusionEntityCollection(const ExtrusionEntityCollection &other) : no_sort(other.no_sort)
    {
    ...
    void append(ExtrusionEntitiesPtr &&src)
    {
        ...
        {
            entities.insert(entities.end(), std::make_move_iterator(src.begin()), std::make_move_iterator(src.end()));
    ...
    // When gap=0 with multi-type support, degenerate geometry can create empty collections.
    // These functions return const Point& and can't signal "no point exists", so we return
    // a static origin point for empty collections rather than crash on undefined behavior.
    const Point &first_point() const override
    {
        static const Point s_origin(0, 0);
        return this->entities.empty() ? s_origin : this->entities.front()->first_point();
    }
    const Point &last_point() const override
    {
        static const Point s_origin(0, 0);
        return this->entities.empty() ? s_origin : this->entities.back()->last_point();
    }
    const Point &middle_point() const override
    {
        static const Point s_origin(0, 0);
        return this->entities.empty() ? s_origin : this->entities[this->entities.size() / 2]->middle_point();
    }
    ...
    {
        throw Luminary::RuntimeError("Calling as_polyline() on a ExtrusionEntityCollection");
        return Polyline();
    };
    ...
    {
        throw Luminary::RuntimeError("Calling length() on a ExtrusionEntityCollection");
        return 0.;
    }
};
