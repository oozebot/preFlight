...
void ExtrusionPath::polygons_covered_by_spacing(Polygons &out, const float scaled_epsilon) const
{
    ...
    auto flow = bridge ? Flow::bridging_flow(m_attributes.width, 0.f)
                       : Flow(m_attributes.width, m_attributes.height, 0.f);
...
Polyline ExtrusionMultiPath::as_polyline() const
{
    ...
    if (!paths.empty())
    {
        ...
        for (size_t i_path = 0; i_path < paths.size(); ++i_path)
            out.points.insert(out.points.end(), paths[i_path].polyline.points.begin() + 1,
...
{
    throw Luminary::LogicError("ExtrusionLoop::reverse() must NOT be called");
}
    ...
    for (ExtrusionPaths::iterator path = this->paths.begin(); path != this->paths.end(); ++path)
        if (int idx = path->polyline.find_point(point, scaled_epsilon); idx != -1)
        {
            if (this->paths.size() == 1)
            {
                ...
                path->polyline.points.insert(path->polyline.points.end(), path->polyline.points.begin() + 1,
                                             path->polyline.points.begin() + idx + 1);
...
ExtrusionLoop::ClosestPathPoint ExtrusionLoop::get_closest_path_and_point(const Point &point,
                                                                          bool prefer_non_overhang) const
{
    ...
    if (this->paths.size() == 1)
    {
        if (p2.polyline.is_valid())
        {
            if (p1.polyline.is_valid())
                p2.polyline.points.insert(p2.polyline.points.end(), p1.polyline.points.begin() + 1,
                                          p1.polyline.points.end());
