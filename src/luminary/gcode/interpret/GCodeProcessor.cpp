...
#include <chrono>
#include <cmath>
#include <deque>   // std::deque, which the precompiled header had been supplying
#include <fstream> // std::fstream, which the precompiled header had been supplying
#include <sstream>
#include <limits>
...
static const Luminary::Vec3f DEFAULT_EXTRUDER_OFFSET = Luminary::Vec3f::Zero();
...
static void recalculate_trapezoids(std::vector<GCodeProcessor::TimeBlock> &blocks)
{
    ...
    for (size_t i = 0; i < blocks.size(); ++i)
    {
        ...
        if (curr != nullptr)
        {
            ...
            {
                // Entry and exit factors always > 0 by all previous logic operations.
                                                 ...
                                                 bool skip_speed_interpolation, float minimum_cruise_ratio)
{
    ...
    // Block processing is fast, the bottleneck is move insertion later
...
GCodeProcessorResult::GCodeProcessorResult(GCodeProcessorResult &&other) noexcept
    ...
    , extruder_color_overrides(std::move(other.extruder_color_overrides))
...
void GCodeProcessorResult::reset()
{
    ...
    extruder_color_overrides = std::vector<std::string>();
                                            ...
                                            std::vector<std::string> &found_tag)
{
    ...
    parser.parse_buffer(gcode,
                        [&ret, &found_tag, max_count](GCodeReader &parser, const GCodeReader::GCodeLine &line)
                        {
                            ...
                            {
                                comment = comment.substr(1);
                                // The cooling buffer's in-flight markers are parsed by substring
                                // after the custom G-code is spliced in, so user text carrying one
                                // corrupts that pass the same way a reserved tag would.
                                static const char *internal_markers[] = {"_BRIDGE_FAN_END",     "_BRIDGE_FAN_START",
                                                                         "_EXTERNAL_PERIMETER", "_EXTRUDE_END",
                                                                         "_EXTRUDE_SET_SPEED",  "_EXTRUSION_ROLE",
                                                                         "_INTERNAL_PERIMETER", "_RESET_FAN_SPEED",
                                                                         "_SET_FAN_SPEED",      "_WIPE"};
                                auto flag = [&](const std::string &tag)
                                {
                                    if (boost::starts_with(comment, tag))
                                    {
                                        ret = true;
                                ...
                                for (const std::string &s : Reserved_Tags)
                                    if (flag(s))
                                        return;
                                for (const char *s : internal_markers)
                                    if (flag(s))
                                        return;
                            }
                        });
...
MachineEnvelopeConfig GCodeProcessor::resolve_machine_limits(const PrintConfig &config)
{
    GCodeProcessor processor;
    processor.apply_config(config);
    return processor.m_time_processor.machine_limits;
}
...
void GCodeProcessor::apply_config(const DynamicPrintConfig &config)
{
    ...
    // filament_colour is the color of what each extruder prints and drives the filament color
    // print view; the printer's extruder_colour is kept separately as the override the extruder
    // color print and tool views apply on top of it.
    ...
    m_result.extruder_color_overrides = extruder_colour != nullptr ? extruder_colour->values
...
void GCodeProcessor::apply_klipper_isotropic_limits(float max_velocity, float max_accel, float scv)
{
    ...
    const float jd = klipper_junction_deviation(scv, max_accel);
...
#ifdef PREFLIGHT_PYTHON_PREPROCESSOR
// The moves, the time model and the M73 line table were built for the text the scripts received.
// After the scripts changed it, the same pass the export ran first is run again on the
// materialized text (scripts excluded), so the estimate, the statistics and the progress lines
// describe what will print.
void GCodeProcessor::reprocess_after_scripts()
{
    GCodeObject *gco = m_result.gcode_object;
    if (gco == nullptr || m_print == nullptr)
        return;
    // Keep the materialized text alive across reset(), which destroys the owned result
    m_result.gcode_object = nullptr;
    m_result.owns_gcode_object = false;
    const std::string filename = m_result.filename;
    Print *print = m_print;
    const bool stealth =
        m_time_processor.machines[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Stealth)].enabled;
    const size_t preview_detail = m_preview_detail_threshold;
    const bool consent = m_preprocessing_consent;

    reset();
    initialize_result_moves();
    apply_config(print->config());
    enable_stealth_time_estimator(stealth);
    set_preview_detail_threshold(preview_detail);
    initialize(filename);
    set_print(print);
    m_preprocessing_consent = false;
    set_gcode_object(gco);
    process_buffer(gco->text_buffer());
    finalize(false);
    m_preprocessing_consent = consent;
    DBG_COUNT("PP_REPARSE");
}

// Runs the profiles' preprocessing scripts on the sliced text, then re-parses it when they changed
// anything. Called before the post-processing pass reads any move or time data.
void GCodeProcessor::run_preprocessing_scripts()
{
    GCodeObject *input_gco = m_result.gcode_object;
    if (input_gco == nullptr)
        return;
    ...
    if (m_preprocessing_consent && m_print)
    {
        ...
        if (any_enabled)
        {
            ...
            if (!all_scripts.empty())
            {
                ...
                // A script past the time limit fails the slice: the user lands back on Prepare with the
                // reason, instead of a G-code that silently lacks that script's work.
                if (pp_result.timed_out)
                {
                    if (gco_for_scripts != input_gco)
                        delete gco_for_scripts;
                    throw Luminary::RuntimeError(Luminary::format(
                        _u8L("Preprocessing timed out: script \"%1%\" ran longer than %2% minutes. Raise the limit "
                             "in Preferences > Preprocessing or fix the script."),
                        pp_result.timed_out_script, m_preprocessing_timeout_s / 60.0));
                }

                // Scripts materialized a new text: adopt it and rebuild the moves and time model from it
                if (gco_for_scripts != input_gco)
                {
                    if (m_result.owns_gcode_object)
                        delete m_result.gcode_object;
...
void GCodeProcessor::post_process_gcode_object()
{
    ...
    // Moves and time estimation data (g1_times_cache, machine.time, stop_times) were built during
    // G-code generation via streaming process_buffer() calls and finalized by the preceding
    // finalize(true); when preprocessing scripts changed the text they were rebuilt from it.
    ...
    {
        return Luminary::float_to_string_decimal_point(time, 2);
    };
    ...
    // A pass-through line is a view into the input object's text; an inserted or rewritten
    // line owns its text in owned_lines, which only grows at its end, so the views into it
    // stay valid while output_lines is inserted into.
    struct BufferedLine
    {
    ...
    std::deque<std::string> owned_lines;
    std::array<float, static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count)> current_times{0.0f, 0.0f};

    auto own_text = [&owned_lines](std::string line) -> std::string_view
    {
        owned_lines.push_back(std::move(line));
        return owned_lines.back();
    };
    // Append a line of the input to the output buffer
    auto append_view = [&output_lines, &current_times](std::string_view line, size_t input_id)
    {
        output_lines.push_back({line, current_times, input_id});
    };
    // Append an inserted or rewritten line to the output buffer
    auto append_owned = [&append_view, &own_text](std::string line, size_t input_id = 0)
    {
        append_view(own_text(std::move(line)), input_id);
    };
    ...
    auto process_line_G1 = [&](size_t g1_counter)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
        {
            ...
            {
                append_owned(format_line_M73_main(machine.line_m73_main_mask.c_str(), to_export_main.first,
            ...
            if (it_stop != machine.stop_times.end())
            {
                ...
                if (last_exported_stop[i] != to_export_stop)
                {
                    if (to_export_stop > 0)
                    {
                        if (last_exported_stop[i] != to_export_stop)
                        {
                            append_owned(format_line_M73_stop_int(machine.line_m73_stop_mask.c_str(), to_export_stop));
                    ...
                    else
                    {
                        ...
                        if (is_last)
                        {
                            if (std::distance(machine.stop_times.begin(), it_stop) ==
                                ...
                                append_owned(format_line_M73_stop_float(machine.line_m73_stop_mask.c_str(),
                                                                        time_in_last_minute(it_stop->elapsed_time -
                                                                                            cache_entry.elapsed_time)));
    ...
    auto process_line_T = [&](const std::string &gcode_line)
    {
        ...
        for (unsigned int step = 0; step < backtrace_steps; ++step)
        {
            ...
            std::string curr_cmd = GCodeReader::GCodeLine::extract_cmd(rev_it->text);
                   ...
                   curr_cmd != "G28" && curr_cmd != "G29")
            {
                ...
                if (GCodeReader::GCodeLine::cmd_is(rev_it->text, "M104"))
                {
                    GCodeReader::GCodeLine gline;
                    GCodeReader reader;
                    reader.parse_line(std::string(rev_it->text),
                        ...
                        m_is_XL_printer)
                    {
                        if (static_cast<int>(val) == tool_number)
                            rev_it->text = own_text("; removed M104\n");
                    }
                }
                ++rev_it;
                if (rev_it != output_lines.rend())
                    curr_cmd = GCodeReader::GCodeLine::extract_cmd(rev_it->text);
            }
            ...
            if (rev_it != output_lines.rend() && rev_it != start_rev_it && rev_it->times[Normal] != last_time_insertion)
            {
                ...
                output_lines.insert(rev_it.base(), {own_text(std::move(out_line)), rev_it->times, 0});
            }
        }
    };
    ...
    for (size_t line_idx = 0; line_idx < input_line_count; ++line_idx)
    {
        ...
        std::string_view gcode_line;
        if (!input.empty() && input.back() == '\n' && (input.size() < 2 || input[input.size() - 2] != '\r'))
            gcode_line = input;
        else
        {
            ...
            if (!body.empty() && body.back() == '\r')
                body.remove_suffix(1);
            rewritten = std::string(body) + "\n";
            gcode_line = rewritten;
        }
        ...
        // The placeholder and filament-statistics comments are the only lines rewritten in
        // place; they are copied and handed to the rewriters, every other line stays a view.
        bool processed = false;
        bool emitted = false;
        {
            std::string_view body = gcode_line.substr(0, gcode_line.size() - 1);
            const bool placeholder = body.size() > 1 &&
                                     (body.substr(1) == reserved_tag(ETags::First_Line_M73_Placeholder) ||
                                      body.substr(1) == reserved_tag(ETags::Last_Line_M73_Placeholder) ||
                                      body.substr(1) == reserved_tag(ETags::Estimated_Printing_Time_Placeholder));
            const bool statistics = gcode_line.size() >= 8 && gcode_line[0] == ';' && gcode_line[1] == ' ' &&
                                    (gcode_line[2] == 'f' || gcode_line[2] == 't');
            if (placeholder || statistics)
            {
                std::string comment(gcode_line);
                // Replace placeholder comments (the rewriter appends its own lines)
                processed = process_placeholders(comment);
                if (processed)
                    emitted = true;
                // Replace filament stat comments
                if (!processed && process_used_filament(comment))
                {
                    processed = true;
                    emitted = true;
                    append_owned(std::move(comment), line_idx + 1);
                }
            }
        }
        ...
        if (!processed)
        {
            ...
            {
                process_line_T(std::string(gcode_line));
            }
        }

        // Append remaining lines (G28, T commands, other)
        if (!emitted)
            append_view(gcode_line, line_idx + 1);
    }
    ...
    // Build post-processed GCodeObject with move linkage from output_lines.
    {
        ...
        for (const auto &buf_line : output_lines)
            output_gco->append_text(buf_line.text.data(), buf_line.text.size());
                                                  ...
                                                  bgcode::binarize::BinaryData binary_data)
{
    try
    {
        ...
        if (out.f == nullptr)
            throw Luminary::RuntimeError(std::string("Cannot open file for binary gcode export: ") + binary_path);

        const bgcode::core::EResult res = binarizer.initialize(*out.f, s_binarizer_config);
        if (res != bgcode::core::EResult::Success)
            throw Luminary::RuntimeError(
        ...
        if (!in.is_open())
            throw Luminary::RuntimeError(std::string("Cannot open text gcode for binarization: ") + text_path);
        ...
        while (std::getline(in, line))
        {
            ...
            if (binarizer.append_gcode(line) != bgcode::core::EResult::Success)
                throw Luminary::RuntimeError("Error while sending gcode to the binarizer.");
        }

        if (in.bad())
            throw Luminary::RuntimeError(std::string("I/O error reading text gcode: ") + text_path);

        in.close();

        if (binarizer.finalize() != bgcode::core::EResult::Success)
            throw Luminary::RuntimeError("Error while finalizing the gcode binarizer.");

        if (ferror(out.f))
            throw Luminary::RuntimeError(std::string("I/O error writing binary gcode: ") + binary_path);
    }
    catch (...)
    {
                                  ...
                                  std::function<void(void)> cancel_callback)
{
    ...
    if (file == nullptr)
        throw Luminary::RuntimeError(format("Error opening file %1%", filename));
                                         ...
                                         std::function<void()> cancel_callback)
{
    ...
    auto throw_error = [progress_callback](const std::string &msg)
    {
        ...
        throw Luminary::RuntimeError(msg.c_str());
    };
    ...
    if ((EBlockType) block_header.type == EBlockType::FileMetadata)
    {
        ...
        if (producer_it != file_metadata_block.raw_data.end() &&
            boost::starts_with(producer_it->second, std::string(PREFLIGHT_APP_NAME)))
            m_producer = EProducer::preFlight;
        else
            m_producer = EProducer::Unknown;
        res = read_next_block_header(*file.f, file_header, block_header, cs_buffer.data(), cs_buffer.size());
...
void GCodeProcessor::finalize(bool perform_post_process)
{
    ...
    // Compute max commands-per-second metrics by scanning 1-second windows.
    {
        ...
        // A window starts at a timed move and takes the following moves of the same layer up to
        // and including the one that brings its time to one second. Within a layer the window
        // end never moves backwards as the start advances, so one forward sweep per layer with
        // a time prefix and running per-role counts visits every window once. The overall count
        // is every move in the window; a role count is the extrusions of the start move's role.
        std::chrono::steady_clock::time_point cps_t0;
        if constexpr (PERF_TIMING)
            cps_t0 = std::chrono::steady_clock::now();
        std::vector<double> time_prefix(num_moves + 1, 0.0);
        for (size_t k = 0; k < num_moves; ++k)
        {
            const float t = m_result.moves[k].time[NORMAL_MODE];
            time_prefix[k + 1] = time_prefix[k] + (t > 0.0f ? double(t) : 0.0);
        }
        std::array<size_t, NUM_ROLES> role_in_window{};
        auto role_of = [&](size_t k) -> size_t
        {
            const auto &move = m_result.moves[k];
            return move.type == EMoveType::Extrude ? static_cast<size_t>(move.extrusion_role) : NUM_ROLES;
        };
        size_t run_begin = 0;
        while (run_begin < num_moves)
        {
            const unsigned int layer = m_result.moves[run_begin].layer_id;
            size_t run_end = run_begin;
            while (run_end < num_moves && m_result.moves[run_end].layer_id == layer)
                ++run_end;
            role_in_window.fill(0);
            size_t j = run_begin; // the window is [i, j)
            for (size_t i = run_begin; i < run_end; ++i)
            {
                while (j < run_end && time_prefix[j] - time_prefix[i] < 1.0)
                {
                    if (const size_t r = role_of(j); r < NUM_ROLES)
                        ++role_in_window[r];
                    ++j;
                }
                ...
                {
                    const size_t command_count = j - i;
                    const double window_time = time_prefix[j] - time_prefix[i];
                    if (command_count > overall_stats.max_commands)
                    {
                        overall_stats.max_commands = command_count;
                        overall_stats.max_at_move = i;
                        overall_stats.max_window_time = window_time;
                        overall_stats.max_layer = layer;
                    }
                    if (const size_t r = role_of(i); r < NUM_ROLES && role_in_window[r] > role_stats[r].max_commands)
                    {
                        role_stats[r].max_commands = role_in_window[r];
                        role_stats[r].max_at_move = i;
                        role_stats[r].max_window_time = window_time;
                        role_stats[r].max_layer = layer;
                    }
                }
                // The start leaves the window (it always entered it: an empty prefix span is under
                // one second).
                if (const size_t r = role_of(i); r < NUM_ROLES)
                    --role_in_window[r];
            }
            run_begin = run_end;
        }
        ...
        if constexpr (PERF_TIMING)
        {
            perf_print("[TIMING]   cps: sweep %7.1fms over %zu moves\n",
                       std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - cps_t0).count(),
                       num_moves);
            auto cps_ref_t0 = std::chrono::steady_clock::now();
            // Cross-check against the rescan-from-every-move form of the same statistic.
            std::array<RoleStats, NUM_ROLES> ref_role_stats{};
            RoleStats ref_overall{};
            for (size_t i = 0; i < num_moves; i++)
            {
                const auto &start_move = m_result.moves[i];
                if (start_move.time[NORMAL_MODE] <= 0.0f)
                    continue;
                const unsigned int start_layer = start_move.layer_id;
                {
                    ...
                    for (size_t j = i; j < num_moves && accumulated_time < 1.0; j++)
                    {
                        ...
                        if (move.layer_id != start_layer)
                            break;
                        const float move_time = move.time[NORMAL_MODE];
                        if (move_time > 0.0f)
                            accumulated_time += move_time;
                        command_count++;
                    }
                    if (command_count > ref_overall.max_commands)
                        ref_overall = {command_count, i, accumulated_time, start_layer};
                }
                if (start_move.type == EMoveType::Extrude)
                {
                    double accumulated_time = 0.0;
                    size_t command_count = 0;
                    const GCodeExtrusionRole start_role = start_move.extrusion_role;
                    for (size_t j = i; j < num_moves && accumulated_time < 1.0; j++)
                    {
                        const auto &move = m_result.moves[j];
                        if (move.layer_id != start_layer)
                    ...
                    const size_t role_idx = static_cast<size_t>(start_role);
                    if (role_idx < NUM_ROLES && command_count > ref_role_stats[role_idx].max_commands)
                        ref_role_stats[role_idx] = {command_count, i, accumulated_time, start_layer};
                }
            }
            auto same = [](const RoleStats &a, const RoleStats &b)
            {
                return a.max_commands == b.max_commands && a.max_at_move == b.max_at_move &&
                       a.max_layer == b.max_layer && std::abs(a.max_window_time - b.max_window_time) < 1e-6;
            };
            size_t mismatches = same(ref_overall, overall_stats) ? 0 : 1;
            for (size_t idx = 0; idx < NUM_ROLES; ++idx)
                if (!same(ref_role_stats[idx], role_stats[idx]))
                    ++mismatches;
            perf_print("[TIMING]   cps: rescan %7.1fms, cross-check mismatches=%zu (overall ref max=%zu at=%zu, "
                       "sweep max=%zu at=%zu)\n",
                       std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - cps_ref_t0).count(),
                       mismatches, ref_overall.max_commands, ref_overall.max_at_move, overall_stats.max_commands,
                       overall_stats.max_at_move);
        }
        ...
        if constexpr (PERF_TIMING)
        {
            // The exact window maxima, so two binaries can be compared on these statistics.
            perf_print("[TIMING]   cps: overall max=%zu at=%zu window=%.9f layer=%u\n", overall_stats.max_commands,
                       overall_stats.max_at_move, overall_stats.max_window_time, overall_stats.max_layer);
            for (size_t idx = 0; idx < NUM_ROLES; ++idx)
                if (role_stats[idx].max_commands > 0)
                    perf_print("[TIMING]   cps: role %zu max=%zu at=%zu window=%.9f layer=%u\n", idx,
                               role_stats[idx].max_commands, role_stats[idx].max_at_move,
                               role_stats[idx].max_window_time, role_stats[idx].max_layer);
        }
    }
                                                     ...
                                                     ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (key_value_pairs < 80)
        throw Luminary::RuntimeError(format("Suspiciously low number of configuration values extracted from %1%: %2%",
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    // layer change tag
    // Use starts_with, since LAYER_CHANGE carries the layer number (e.g., "LAYER_CHANGE: 123")
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        ...
        if (!blocks.empty() && prev.feedrate > PREVIOUS_FEEDRATE_THRESHOLD)
        {
            ...
            if (prev.has_direction && curr.has_direction)
            {
                ...
                // The safe_feedrate limit is deliberately not applied here: it treats jerk as a
                // maximum speed rather than a maximum velocity change, and the corner speed above
                // already accounts for the jerk limits from the direction change, so the cap would
                // be redundant and too conservative.
            }
            else
            {
...
void GCodeProcessor::post_process()
{
            ...
            if (!out_string.empty())
            {
                if (!m_binarizer.is_enabled())
                {
                    ...
                    if (ferror(out.f))
                    {
                        ...
                        throw Luminary::RuntimeError("GCode processor post process export failed.\nIs the disk full?");
                    }
                }
            }
        }
    ...
    auto g1_times_cache_it = Luminary::reserve_vector<std::vector<TimeMachine::G1LinesCacheItem>::const_iterator>(
    ...
    if (rename_file(out_path, result_filename))
        throw Luminary::RuntimeError(std::string("Failed to rename the output G-code file from ") + out_path + " to " +
                                     result_filename + '\n' + "Is " + out_path + " locked?" + '\n');
}
...
{
    if (m_is_XL_printer)
        // A tool changer parks and picks a head instead of loading filament; charge a fixed time.
        return 4.5f;
...
{
    if (m_is_XL_printer)
        // A tool changer has no unload step; the whole tool change is charged as load time.
        return 0.f;
...
{
    // Custom G-code is treated as draining the planner queue, so the estimate may exceed the real
    // print time.
    simulate_st_synchronize();
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
...
{
    if (line.has(Luminary::Axis(axis)))
    {
        bool is_relative = (m_global_positioning_type == EPositioningType::Relative);
        if (axis == E)
            is_relative |= (m_e_local_positioning_type == EPositioningType::Relative);
        ...
        const double lengthsScaleFactor = (m_units == EUnits::Inches) ? double(INCHES_TO_MM) : 1.0;
        double ret = line.value(Luminary::Axis(axis)) * lengthsScaleFactor;
        if (axis == E && m_use_volumetric_e)
            ret /= area_filament_cross_section;
        return is_relative ? m_start_position[axis] + ret : m_origin[axis] + ret;
    }
...
bool BuildVolume::all_paths_inside(const GCodeProcessorResult &paths, const BoundingBoxf3 &paths_bbox,
                                   bool ignore_bottom) const
{
    auto move_valid = [](const GCodeProcessorResult::MoveVertex &move)
    {
        return move.type == EMoveType::Extrude && move.extrusion_role != GCodeExtrusionRole::Custom &&
    ...
    case Type::Circle:
    {
        ...
        return m_max_print_height == 0.0
                   ...
                   : std::all_of(paths.moves.begin(), paths.moves.end(),
                                 [move_valid, c, r2,
                                  z = m_max_print_height + epsilon](const GCodeProcessorResult::MoveVertex &move)
                                 {
                                     return !move_valid(move) ||
                                            ((to_2d(move.position) - c).squaredNorm() <= r2 && move.position.z() <= z);
                                 });
    }
    case Type::Convex:
    // A convex bed is answered by the hull decomposition alone; a custom one is tested against its own
    // edges, so a move end point in a concave notch of the bed reads as outside.
    case Type::Custom:
        return m_max_print_height == 0.0
                   ? std::all_of(paths.moves.begin(), paths.moves.end(),
                                 ...
                                 {
                                     return !move_valid(move) ||
                                            this->point_inside_bed_outline(m_top_bottom_convex_hull_decomposition_bed,
                                                                           epsilon,
