...
struct GCodeProcessorResult
{
    ...
    // Per-extruder filament colors (filament_colour), the color source of the preview.
    std::vector<std::string> extruder_colors;
    // The printer's extruder_colour values as written; empty entries mean no override.
    std::vector<std::string> extruder_color_overrides;
...
public:
    ...
    struct TimeBlock
    {
        ...
        // - min_segment_time_us (M205 B parameter, typically 20000us)
...
private:
    ...
    double m_preprocessing_timeout_s{300.0};

    // The script stage of post-processing, and the second parse of the materialized text after it
    void run_preprocessing_scripts();
    void reprocess_after_scripts();
#endif

    GCodeProcessorResult m_result;
    // Declared after m_result, which it holds a reference to, so it is constructed second.
...
public:
    ...
    // The machine limits the time estimator would use for this config: the profile's arrays
    // where the flavor trusts them, the Klipper / RepRapFirmware fields where it does not.
    static MachineEnvelopeConfig resolve_machine_limits(const PrintConfig &config);
    ...
    void set_preprocessing_timeout_seconds(double s) { m_preprocessing_timeout_s = s; }
#endif
