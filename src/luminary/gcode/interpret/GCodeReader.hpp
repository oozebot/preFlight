...
public:
    ...
    public:
        ...
        // The line may be a view into a larger buffer: the scans stay within it.
        static bool cmd_is(std::string_view gcode_line, const char *cmd_test)
        {
            size_t pos = 0;
            while (pos < gcode_line.size() && (gcode_line[pos] == ' ' || gcode_line[pos] == '\t'))
                ++pos;
            const size_t len = strlen(cmd_test);
            if (gcode_line.size() - pos < len || gcode_line.compare(pos, len, cmd_test) != 0)
                return false;
            return pos + len == gcode_line.size() || GCodeReader::is_end_of_word(gcode_line[pos + len]);
        }

        static bool cmd_starts_with(std::string_view gcode_line, const char *cmd_test)
        {
            size_t pos = 0;
            while (pos < gcode_line.size() && (gcode_line[pos] == ' ' || gcode_line[pos] == '\t'))
                ++pos;
            const size_t len = strlen(cmd_test);
            return gcode_line.size() - pos >= len && gcode_line.compare(pos, len, cmd_test) == 0;
        }

        static std::string extract_cmd(std::string_view gcode_line)
        {
            GCodeLine temp;
            temp.m_raw = std::string(gcode_line);
...
private:
    ...
    bool parse_file_raw_internal(const std::string &filename, ParseLineCallback parse_line_callback,
                                 LineEndCallback line_end_callback);
    template<typename ParseLineCallback, typename LineEndCallback>
    bool parse_file_internal(const std::string &filename, ParseLineCallback parse_line_callback,
                             LineEndCallback line_end_callback);
