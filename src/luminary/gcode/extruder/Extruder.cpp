...
Extruder::Extruder(unsigned int id, GCodeConfig *config) : m_id(id), m_config(config)
{
...
void Extruder::set_retracted(double retracted, double restart_extra)
{
    if (retracted < -EPSILON)
        throw Luminary::RuntimeError("Custom G-code reports negative z_retracted.");
    if (restart_extra < -EPSILON)
        throw Luminary::RuntimeError("Custom G-code reports negative z_restart_extra.");
...
{
    return m_config->use_volumetric_e ? m_absolute_E + m_retracted
...
{
    return m_config->use_volumetric_e ? this->extruded_volume() / this->filament_crossection()
