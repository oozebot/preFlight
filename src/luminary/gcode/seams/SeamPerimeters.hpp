...
struct LayerInfo
{
    static LayerInfo create(const Luminary::Layer &object_layer, std::size_t index,
...
struct Perimeter
{
    ...
    Perimeter(const double slice_z, const std::size_t layer_index, const bool is_hole, std::vector<Vec2d> &&positions,
              std::vector<double> &&angles, std::vector<PointType> &&point_types,
              std::vector<PointClassification> &&point_classifications, std::vector<AngleType> &&angle_types);

    static Perimeter create(const Polygon &polygon, const Geometry::Overhangs &overhangs,
                            const ModelInfo::Painting &painting, const LayerInfo &layer_info,
                            const PerimeterParams &params);

    static Perimeter create_degenerate(std::vector<Vec2d> &&points, const double slice_z,
...
inline std::vector<Vec2d> extract_points(const Perimeter &perimeter, const PointType point_type)
{
