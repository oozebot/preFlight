...
namespace Luminary::Seams::Geometry
{
...
std::vector<std::size_t> assign_buckets(const std::vector<std::optional<Link>> &links, std::size_t &new_bucket_id)
{
    ...
    std::transform(links.begin(), links.end(), std::back_inserter(result),
...
{
    return Vec2d{vector.y(), -vector.x()}.normalized();
}
...
Vec2d get_polygon_normal(const std::vector<Vec2d> &points, const std::size_t index, const double min_arm_length)
{
    ...
    visit_forward(index, points.size(),
                  [&](const std::size_t index_candidate)
                  {
    ...
    visit_backward(index, points.size(),
                   [&](const std::size_t index_candidate)
                   {
                       const double distance{(points[index_candidate] - points[index]).norm()};
                       if (distance > min_arm_length)
...
std::pair<Mapping, std::size_t> get_mapping(const std::vector<std::size_t> &list_sizes,
                                            const MappingOperator &mapping_operator)
{
...
Extrusion::Extrusion(Polygon &&polygon, BoundingBox bounding_box, const double width, const ExPolygon &island_boundary,
                     Overhangs &&overhangs)
    ...
    , overhangs(std::move(overhangs))
{
    ...
    std::transform(this->island_boundary.holes.begin(), this->island_boundary.holes.end(),
...
Overhangs get_overhangs(const ExtrusionEntity *entity)
{
    ...
    if (auto path{dynamic_cast<const ExtrusionPath *>(entity)})
    {
        ...
        {
            result.emplace_back(Overhang{path->first_point(), path->last_point()});
        }
    }
    ...
    {
        const bool bridge_loop{std::all_of(path->paths.begin(), path->paths.end(),
                                           [](const ExtrusionPath &path) { return path.role().is_bridge(); })};
...
Geometry::Extrusions get_external_perimeters(const Luminary::Layer &layer, const LayerSlice &slice)
{
    ...
    for (const LayerIsland &island : slice.islands)
    {
        ...
        for (const uint32_t perimeter_id : island.perimeters)
        {
            const auto collection{
                static_cast<const ExtrusionEntityCollection *>(layer_region.perimeters().entities[perimeter_id])};
...
std::vector<Extrusions> get_extrusions(tcb::span<const Luminary::Layer *const> object_layers)
{
    std::vector<Extrusions> result;
    result.reserve(object_layers.size());

    for (const Luminary::Layer *object_layer : object_layers)
    {
...
BoundedPolygons project_to_geometry(const Geometry::Extrusions &external_perimeters, const double max_bb_distance)
{
    ...
    transform(external_perimeters.begin(), external_perimeters.end(), back_inserter(result),
              [&](const Geometry::Extrusion &external_perimeter)
              {
                  const auto [choosen_index, _]{
                      Geometry::pick_closest_bounding_box(external_perimeter.bounding_box,
                                                          external_perimeter.island_boundary_bounding_boxes)};

                  const double distance{
                      Geometry::bounding_box_distance(external_perimeter.island_boundary_bounding_boxes[choosen_index],
                                                      external_perimeter.bounding_box)};

                  if (distance > max_bb_distance)
                  {
                      const Polygons expanded_extrusion{
                          expand(external_perimeter.polygon, Luminary::scaled(external_perimeter.width / 2.0))};
                      ...
                      return BoundedPolygon{external_perimeter.polygon, external_perimeter.overhangs,
                                            external_perimeter.polygon.bounding_box(),
                                            external_perimeter.polygon.is_clockwise()};
                  }

                  const bool is_hole{choosen_index != 0};
                  const Polygon &adjacent_boundary{!is_hole
...
void visit_forward(const std::size_t start_index, const std::size_t loop_size,
...
void visit_backward(const std::size_t start_index, const std::size_t loop_size,
                    const std::function<bool(std::size_t)> &visitor)
{
    std::size_t last_index{loop_size - 1};
...
std::vector<Vec2d> unscaled(const Points &points)
{
    ...
    transform(begin(points), end(points), back_inserter(result), [](const Point &point) { return unscaled(point); });
    return result;
}
...
std::vector<Linef> unscaled(const Lines &lines)
{
    ...
    std::transform(lines.begin(), lines.end(), std::back_inserter(result),
                   [](const Line &line) { return Linef{unscaled(line.a), unscaled(line.b)}; });
    return result;
}
...
Points scaled(const std::vector<Vec2d> &points)
{
    ...
    {
        result.push_back(Luminary::scaled(point));
    }
    return result;
}
