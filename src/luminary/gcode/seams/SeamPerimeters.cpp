...
#include "luminary/gcode/seams/model_queries/SeamPainting.hpp"
#include "luminary/geometry/contours/MultiPoint.hpp"
#include "tcbspan/span.hpp"

namespace Luminary::Seams::Perimeters::Impl
{
PerimeterPoints oversample_painted(PerimeterPoints &points, const std::function<bool(Vec3f, double)> &is_painted,
                                   const double slice_z, const double max_distance)
{
...
PerimeterPoints remove_redundant_points(const PerimeterPoints &points, const double tolerance)
{
    PerimeterPoints result;
    ...
    for (auto iterator{points.begin()}; iterator != points.end(); ++iterator)
    {
        ...
        if (next(iterator) == points.end() || points[index].type != points[index + 1].type ||
            points[index].classification != points[index + 1].classification)
        {
            douglas_peucker<double>(range_start, next(iterator), std::back_inserter(result), tolerance,
                                    [](const PerimeterPoint &point) { return point.position; });
...
PerimeterPoints get_point_types(const PerimeterPoints &perimeter_points, const ModelInfo::Painting &painting,
                                const double slice_z, const double painting_radius)
{
    PerimeterPoints result;
    result.reserve(perimeter_points.size());
    using std::transform, std::back_inserter;
    transform(perimeter_points.begin(), perimeter_points.end(), back_inserter(result),
...
void project_overhang(PerimeterPoints &points, const AABBTreeLines::LinesDistancer<Linef> &points_distancer,
                      const Geometry::Overhang &overhang, std::map<int, PerimeterPoints> &output)
{
    const auto [start_distance, start_line_index,
                start_point]{points_distancer.distance_from_lines_extra<false>(unscaled(overhang.start))};
    ...
    const auto [end_distance, end_line_index,
                end_point]{points_distancer.distance_from_lines_extra<false>(unscaled(overhang.end))};
...
double get_overhang_angle(const Vec2d &point,
...
PerimeterPoints classify_overhangs(PerimeterPoints &&points, const Geometry::Overhangs &overhangs,
                                   const LayerInfo &layer_info, const double overhang_threshold)
{
    ...
    for (const auto &overhang : overhangs)
    {
        apply_visitor(overload_linearly(
                          ...
                          { project_overhang(classified_points, points_distancer, overhang, points_to_add_to_lines); },
    ...
    for (std::size_t i{0}; i < classified_points.size(); ++i)
    {
        ...
        if (point.classification != PointClassification::overhang)
        {
            ...
            point.classification = overhang_aangle > overhang_threshold ? PointClassification::overhang
...
PerimeterPoints classify_points(PerimeterPoints &&points, const Geometry::Overhangs &overhangs,
                                const double embedding_threshold, const LayerInfo &layer_info,
...
std::vector<AngleType> get_angle_types(const std::vector<double> &angles, const double convex_threshold,
                                       const double concave_threshold)
{
    std::vector<AngleType> result;
    using std::transform, std::back_inserter;
    transform(angles.begin(), angles.end(), back_inserter(result),
              [&](const double angle)
              {
...
std::vector<AngleType> merge_angle_types(const std::vector<AngleType> &angle_types,
                                         const std::vector<AngleType> &smooth_angle_types,
                                         const std::vector<Vec2d> &points, const double min_arm_length)
{
    std::vector<AngleType> result;
    ...
    for (std::size_t index{0}; index < angle_types.size(); ++index)
    {
        ...
        if (smooth_angle_type != angle_type && smooth_angle_type != AngleType::smooth)
        {
            ...
            Geometry::visit_forward(index, angle_types.size(),
            ...
            Geometry::visit_backward(index, angle_types.size(),
...
PerimeterPoints get_perimeter_points(const std::vector<Vec2d> &points)
{
    ...
    std::transform(points.begin(), points.end(), std::back_inserter(perimeter_points),
                   [](const Vec2d &point)
                   {
                       PerimeterPoint perimeter_point;
...
namespace Luminary::Seams::Perimeters
{

LayerInfos get_layer_infos(tcb::span<const Luminary::Layer *const> object_layers,
                           const double elephant_foot_compensation)
{
    ...
    tbb::parallel_for(range,
                      [&](Range range)
                      {
                          ...
                          {
                              result[layer_index] = LayerInfo::create(*object_layers[layer_index], layer_index,
...
LayerInfo LayerInfo::create(const Luminary::Layer &object_layer, const std::size_t index,
                            const double elephant_foot_compensation)
{
    AABBTreeLines::LinesDistancer<Linef> perimeter_distancer{to_unscaled_linesf({object_layer.lslices})};
    ...
    return {std::move(perimeter_distancer), std::move(previous_layer_perimeter_distancer), index, object_layer.height,
            object_layer.slice_z,           index == 0 ? elephant_foot_compensation : 0.0};
}
...
Perimeter::PointTrees get_kd_trees(const PointType point_type, const std::vector<PointType> &all_point_types,
    ...
    for (std::size_t i{0}; i < all_point_types.size(); ++i)
    {
        if (all_point_types[i] == point_type)
        {
            ...
            case PointClassification::overhang:
                overhang_indexes.push_back(i);
                break;
            case PointClassification::embedded:
                embedded_indexes.push_back(i);
                break;
            case PointClassification::common:
                common_indexes.push_back(i);
                break;
            }
        }
    }
...
Perimeter::Perimeter(const double slice_z, const std::size_t layer_index, const bool is_hole,
                     std::vector<Vec2d> &&positions, std::vector<double> &&angles, std::vector<PointType> &&point_types,
                     std::vector<PointClassification> &&point_classifications, std::vector<AngleType> &&angle_types)
    : slice_z(slice_z)
    ...
    , enforced_points(
          get_kd_trees(PointType::enforcer, this->point_types, this->point_classifications, this->index_to_coord))
    , common_points(
          get_kd_trees(PointType::common, this->point_types, this->point_classifications, this->index_to_coord))
    , blocked_points(
          get_kd_trees(PointType::blocker, this->point_types, this->point_classifications, this->index_to_coord))
{
}

Perimeter Perimeter::create_degenerate(std::vector<Vec2d> &&points, const double slice_z, const std::size_t layer_index)
{
    std::vector<PointType> point_types(points.size(), PointType::common);
    std::vector<PointClassification> point_classifications(points.size(), PointClassification::common);
    std::vector<double> angles(points.size());
    std::vector<AngleType> angle_types(points.size(), AngleType::smooth);
    Perimeter perimeter{slice_z,
                        layer_index,
                        false,
                        std::move(points),
                        std::move(angles),
...
Perimeter Perimeter::create(const Polygon &polygon, const Geometry::Overhangs &overhangs,
                            const ModelInfo::Painting &painting, const LayerInfo &layer_info,
                            const PerimeterParams &params)
{
    if (polygon.size() < 3)
    {
        return Perimeter::create_degenerate(Geometry::unscaled(polygon.points), layer_info.slice_z, layer_info.index);
    }
    ...
    perimeter_points = Impl::classify_points(std::move(perimeter_points), overhangs, params.embedding_threshold,
                                             layer_info, params.overhang_threshold);
    ...
    perimeter_points = Impl::oversample_painted(perimeter_points, is_painted, layer_info.slice_z,
    ...
    return Perimeter{layer_info.slice_z,
                     layer_info.index,
                     is_hole,
                     std::move(positions),
                     std::move(angles),
...
LayerPerimeters create_perimeters(const std::vector<Geometry::BoundedPolygons> &polygons,
                                  const std::vector<LayerInfo> &layer_infos, const ModelInfo::Painting &painting,
                                  const PerimeterParams &params)
{
    ...
    Geometry::iterate_nested(polygons,
                             [&](const std::size_t layer_index, const std::size_t polygon_index)
                             {
                                 ...
                                 result[layer_index][polygon_index] = BoundedPerimeter{
                                     Perimeter::create(bounded_polygon.polygon, bounded_polygon.overhangs, painting,
                                                       layer_info, params),
...
std::optional<PointOnPerimeter> offset_along_perimeter(
    const PointOnPerimeter &point, const Perimeter &perimeter, const double offset,
    ...
    const std::function<bool(const Perimeter &, const std::size_t)> &early_stop_condition)
{
    ...
    const Linef initial_line{perimeter.positions[point.previous_index], perimeter.positions[point.next_index]};
    double distance{direction == Dir::forward ? (initial_line.b - point.position).norm()
                                              : (point.position - initial_line.a).norm()};

    if (distance >= offset)
    {
        const Vec2d edge_direction{(initial_line.b - initial_line.a).normalized()};
        const Vec2d offset_point{direction == Dir::forward ? Vec2d{point.position + offset * edge_direction}
                                                           : Vec2d{point.position - offset * edge_direction}};
    ...
    const auto visitor{
        [&](std::size_t index)
        {
            ...
            const std::size_t previous_index{direction == Dir::forward
