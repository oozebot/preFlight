...
namespace Impl
{
std::vector<PerimeterSegment> get_segments(const Perimeters::Perimeter &perimeter, const PointType point_type,
...
PerimeterSegment pick_random_segment(const std::vector<PerimeterSegment> &segments, std::mt19937 &random_engine)
{
    ...
    return *std::find_if(segments.begin(), segments.end(),
                         [&](const PerimeterSegment &segment)
                         {
...
SeamChoice pick_random_point(const PerimeterSegment &segment, const Perimeters::Perimeter &perimeter,
...
std::optional<SeamChoice> Random::operator()(const Perimeters::Perimeter &perimeter, const PointType point_type,
                                                               ...
                                                               const unsigned fixed_seed)
{
    ...
    for (std::vector<Perimeters::BoundedPerimeter> &layer : perimeters)
    {
        ...
        for (Perimeters::BoundedPerimeter &perimeter : layer)
        {
            ...
            {
                result.back().push_back(SeamPerimeterChoice{Seams::choose_seam_point(perimeter.perimeter, random),
