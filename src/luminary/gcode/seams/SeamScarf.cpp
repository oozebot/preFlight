...
#include "luminary/gcode/emit/path/SmoothPath.hpp"

namespace Luminary::Seams::Scarf
{

namespace Impl
{
PathPoint get_path_point(const ExtrusionPaths &paths, const Point &point, const std::size_t global_index)
{
...
std::pair<ExtrusionPath, ExtrusionPath> split_path(const ExtrusionPath &path, const Point &point,
                                                   const std::size_t point_previous_index)
{
    if (static_cast<int>(point_previous_index) >= static_cast<int>(path.size()) - 1)
    {
        throw std::runtime_error("Invalid path split index " + std::to_string(point_previous_index) +
...
Points linspace(const Point &from, const Point &to, const std::size_t count)
{
    ...
    Point offset{(to - from) / coord_t(count - 1)};
...
Points ensure_max_distance(const Points &points, const double max_distance)
{
    ...
    for (std::size_t i{1}; i < points.size(); ++i)
    {
        ...
        {
            const std::size_t points_count{static_cast<std::size_t>(std::ceil(distance / max_distance)) + 1};
...
ExtrusionPaths ensure_scarf_resolution(ExtrusionPaths &&paths, const std::size_t scarf_paths_count,
...
GCode::SmoothPath lineary_increase_extrusion_height(GCode::SmoothPath &&smooth_path, const double start_height)
{
...
GCode::SmoothPath lineary_readuce_extrusion_amount(GCode::SmoothPath &&smooth_path)
{
    GCode::SmoothPath result{std::move(smooth_path)};
    const double length{get_length(result)};
    double distance{0};
...
GCode::SmoothPath elevate_scarf(const ExtrusionPaths &paths, const std::size_t scarf_paths_count,
                                ...
                                const double start_height)
{
    ...
    first_segment = lineary_increase_extrusion_height(std::move(first_segment), start_height);

    std::size_t normal_extrusions_size{paths.size() - 2 * scarf_paths_count};
    const auto normal_extrusions{tcb::span{paths}.subspan(scarf_paths_count, normal_extrusions_size)};
    ...
    last_segment = lineary_readuce_extrusion_amount(std::move(last_segment));
...
std::optional<PathPoint> find_path_point_from_end(const ExtrusionPaths &paths, const Point &point,
                                                  const double tolerance)
{
    ...
    for (int path_index{static_cast<int>(paths.size() - 1)}; path_index >= 0; --path_index)
    {
        ...
        {
            throw std::runtime_error("Invalid path: less than two points: " + std::to_string(path.size()) + "!");
        }
        for (int point_index{static_cast<int>(path.polyline.size() - 2)}; point_index >= 0; --point_index)
        {
            ...
            {
                return PathPoint{point, static_cast<size_t>(path_index), static_cast<size_t>(point_index)};
            }
        }
    }
    return std::nullopt;
...
std::optional<PathPoint> get_point_offset_from_end(const ExtrusionPaths &paths, const double length)
{
    ...
    for (int path_index{static_cast<int>(paths.size() - 1)}; path_index >= 0; --path_index)
    {
        ...
        for (int point_index{static_cast<int>(path.polyline.size() - 2)}; point_index >= 0; --point_index)
        {
            ...
            {
                return PathPoint{previous_point + (edge_direction * (length - distance)).cast<coord_t>(),
...
std::pair<GCode::SmoothPath, std::size_t> add_scarf_seam(
    ExtrusionPaths &&paths, const Scarf &scarf,
    const std::function<GCode::SmoothPath(tcb::span<const ExtrusionPath>)> &apply_smoothing, const bool flipped)
{
    Impl::PathPoint end_point{Impl::get_path_point(paths, scarf.end_point, scarf.end_point_previous_index)};
    ...
    if (!start_point)
    {
        start_point = Impl::PathPoint{paths.front().first_point(), 0, 0};
    }
