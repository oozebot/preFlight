...
bool has_painted_points(const Perimeters::LayerPerimeters &layer_perimeters)
{
    for (const Perimeters::BoundedPerimeters &layer : layer_perimeters)
        for (const Perimeters::BoundedPerimeter &perimeter : layer)
            for (const Perimeters::PointType type : perimeter.perimeter.point_types)
                if (type == Perimeters::PointType::enforcer)
                    return true;
    return false;
}

using LayerColumns = std::vector<std::vector<std::optional<Vec2d>>>;

// The painted column of a painted object for the modes that do not build shells themselves:
// the perimeters regrouped by layer in shell order, and beside each one its column position.
std::pair<Perimeters::LayerPerimeters, LayerColumns> sort_to_layers_with_columns(
    Perimeters::LayerPerimeters &&perimeters, const double max_distance)
{
    Shells::Shells<> shells{Shells::create_shells(std::move(perimeters), max_distance)};
    const std::vector<Aligned::PaintedColumn> columns{Aligned::get_painted_columns(shells)};
    LayerColumns layer_columns(Shells::get_layer_count(shells));
    for (std::size_t shell_index{0}; shell_index < shells.size(); ++shell_index)
        for (std::size_t slice_index{0}; slice_index < shells[shell_index].size(); ++slice_index)
            layer_columns[shells[shell_index][slice_index].layer_index].push_back(columns[shell_index][slice_index]);
    return {sort_to_layers(std::move(shells)), std::move(layer_columns)};
}
                               ...
                               const std::function<void(void)> &throw_if_canceled)
{
    ...
    for (auto &[print_object, layer_perimeters] : seam_data)
    {
        ...
        case spAligned:
        {
            ...
            // The visibility raycast depends on the mesh, the transformation and the
            // raycasting parameters only; the print object keeps it across exports.
            PrintObject::SeamVisibilityCache &cache{print_object->seam_visibility_cache()};
            if (!cache.visibility || cache.trafo.matrix() != transformation.matrix() ||
                cache.samples_count != params.visibility.raycasting_visibility_samples_count ||
                cache.decimation_target != params.visibility.fast_decimation_triangle_count_target ||
                cache.sqr_rays != params.visibility.sqr_rays_per_sample_point)
            {
                cache.visibility = std::make_shared<const Luminary::ModelInfo::Visibility>(transformation, volumes,
                                                                                           params.visibility,
                                                                                           throw_if_canceled);
                cache.trafo = transformation;
                cache.samples_count = params.visibility.raycasting_visibility_samples_count;
                cache.decimation_target = params.visibility.fast_decimation_triangle_count_target;
                cache.sqr_rays = params.visibility.sqr_rays_per_sample_point;
            }
            const Luminary::ModelInfo::Visibility &points_visibility{*cache.visibility};
            throw_if_canceled();
            if (debug_enabled(DBG_PERIMETERS))
            {
                // Run scoped: the raycast visibility samples the aligned seam scores come from.
                DetermFingerprint fp;
                for (size_t i = 0; i < points_visibility.mesh_samples.positions.size(); ++i)
                {
                    const Vec3f &pos = points_visibility.mesh_samples.positions[i];
                    uint64_t h = DetermFingerprint::mix(1469598103934665603ull, DetermFingerprint::bits(pos.x()));
                    h = DetermFingerprint::mix(h, DetermFingerprint::bits(pos.y()));
                    h = DetermFingerprint::mix(h, DetermFingerprint::bits(pos.z()));
                    h = DetermFingerprint::mix(h,
                                               DetermFingerprint::bits(points_visibility.mesh_samples_visibility[i]));
                    fp.add(h);
                }
                dbg_log(DBG_PERIMETERS, 0., "DETERM", "SEAM_VIS_FP samples=%zu radius=%.6f hash=%016llx ord=%016llx",
                        fp.count, double(points_visibility.mesh_samples_radius), (unsigned long long) fp.hash,
        ...
        case spRear:
        {
            ...
            if (has_painted_points(layer_perimeters))
                std::tie(layer_perimeters, columns) = sort_to_layers_with_columns(std::move(layer_perimeters),
                                                                                  params.max_distance);
            result[print_object] = Rear::get_object_seams(std::move(layer_perimeters), columns, params.rear_tolerance,
...
Params Placer::get_params(const DynamicPrintConfig &config)
{
    ...
    params.perimeter.overhang_threshold = Luminary::Geometry::deg2rad(55.0);
    params.perimeter.convex_threshold = Luminary::Geometry::deg2rad(10.0);
    params.perimeter.concave_threshold = Luminary::Geometry::deg2rad(15.0);
                  ...
                  const std::function<void(void)> &throw_if_canceled)
{
    ...
    this->painted_columns_per_layer.clear();
    ...
    for (auto &[print_object, layer_perimeters] : perimeters)
    {
        if (print_object->config().seam_position.value == spNearest)
        {
            if (has_painted_points(layer_perimeters))
            {
                std::tie(this->perimeters_per_layer[print_object], this->painted_columns_per_layer[print_object]) =
                    sort_to_layers_with_columns(std::move(layer_perimeters), params.max_distance);
            }
            else
            {
    ...
    if (debug_enabled(DBG_PERIMETERS))
    {
        // Per layer: the precalculated seam of every perimeter (position and perimeter size).
        for (const auto &[print_object, layer_seams] : this->seams_per_object)
            for (std::size_t layer_index{0}; layer_index < layer_seams.size(); ++layer_index)
            {
                const std::vector<SeamPerimeterChoice> &seams{layer_seams[layer_index]};
                if (seams.empty() || layer_index >= print_object->layers().size())
                    continue;
                DetermFingerprint fp;
                for (const SeamPerimeterChoice &seam : seams)
                {
                    const Point position{Point::new_scale(seam.choice.position.x(), seam.choice.position.y())};
                    fp.add(DetermFingerprint::mix(DetermFingerprint::mix(1469598103934665603ull, position),
                                                  uint64_t(seam.perimeter.positions.size())));
                }
                dbg_log(DBG_PERIMETERS, print_object->layers()[layer_index]->print_z, "DETERM",
                        "SEAM_FP layer=%zu seams=%zu hash=%016llx ord=%016llx", layer_index, fp.count,
...
std::pair<SeamChoice, std::size_t> place_seam_near(const std::vector<Perimeters::BoundedPerimeter> &layer_perimeters,
                                                   const std::vector<std::optional<Vec2d>> &columns,
                                                   ...
                                                   const double max_detour)
{
    ...
    // A painted perimeter seams on the paint centerline, as in aligned mode, not at the painted
    // point nearest the previous position: that let the seam wander across the stroke's width.
    // The column is the centerline filtered across the layers; a perimeter outside any column
    // (a painted object with fewer than five painted layers) takes its own run midpoint.
    if (choice_index < columns.size() && columns[choice_index])
    {
        return {Seams::Aligned::choose_column_seam(layer_perimeters[choice_index].perimeter, *columns[choice_index],
                                                   max_detour),
                choice_index};
    }
    if (const std::optional<SeamChoice> painted{
            Seams::Aligned::Impl::choose_painted_seam(layer_perimeters[choice_index].perimeter, unscaled(position),
                                                      max_detour)})
    {
        return {*painted, choice_index};
    }
                                                       ...
                                                       const Point &last_pos) const
{
    ...
    if (po->config().seam_position.value == spNearest)
    {
        ...
        static const std::vector<std::optional<Vec2d>> no_columns;
        const auto columns_it{this->painted_columns_per_layer.find(po)};
        const std::vector<std::optional<Vec2d>> &columns{columns_it != this->painted_columns_per_layer.end() &&
                                                                 layer_index < columns_it->second.size()
                                                             ? columns_it->second[layer_index]
                                                             : no_columns};
        const auto [seam_choice, perimeter_index] = place_seam_near(perimeters, columns, loop, last_pos,
