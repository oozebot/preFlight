...
#include "luminary/mesh/decimate/ShortEdgeCollapse.hpp"
...
std::vector<float> raycast_visibility(const AABBTreeIndirect::Tree<3, float> &raycasting_tree,
                                      const indexed_triangle_set &triangles, const TriangleSetSamples &samples,
                                      size_t negative_volumes_start_index, const Visibility::Params &params)
{
    ...
    std::vector<Vec3f> precomputed_sample_directions(params.sqr_rays_per_sample_point *
                                                     params.sqr_rays_per_sample_point);
         ...
         &raycasting_tree, &result, &samples, &params](tbb::blocked_range<size_t> r)
        {
            ...
            for (size_t s_idx = r.begin(); s_idx < r.end(); ++s_idx)
            {
                ...
                for (const auto &dir : precomputed_sample_directions)
                {
                    ...
                    {
                        igl::Hit hitpoint;
                        // The AABB tree ray query takes a double origin and direction.
                    ...
                    { // Hits are counted as a parity of solid and negative space; the order in
                        // which volumes are applied is not taken into account.
                        ...
                        { // if casting from negative volume face, invert direction, change start pos
                            final_ray_dir = -1.0 * final_ray_dir;
                            ray_origin_d = (center - normal * 0.01f).cast<double>();
                        }
                        Vec3d final_ray_dir_d = final_ray_dir.cast<double>();
                        bool some_hit = AABBTreeIndirect::intersect_ray_all_hits(triangles.vertices, triangles.indices,
                                                                                 raycasting_tree, ray_origin_d,
                                                                                 final_ray_dir_d, hits);
                        if (some_hit)
                        {
                            int counter = 0;
                            // iterating in reverse, from the last hit, because we know the state of the ray at that point;
                            ...
                            for (int hit_index = int(hits.size()) - 1; hit_index >= 0; --hit_index)
                            {
                                ...
                                if (hits[hit_index].id >= int(negative_volumes_start_index))
                                { //negative volume hit
                                    counter -= sgn(face_normal.dot(
                                        final_ray_dir)); // if volume face aligns with ray dir, we are leaving negative space
...
Visibility::Visibility(const Transform3d &obj_transform, const ModelVolumePtrs &volumes, const Params &params,
                       const std::function<void(void)> &throw_if_canceled)
{
    ...
    this->mesh_samples = sample_its_uniform_parallel(params.raycasting_visibility_samples_count, triangle_set);
    ...
    BOOST_LOG_TRIVIAL(debug) << "SeamPlacer: Mesh sample raidus: " << this->mesh_samples_radius;
