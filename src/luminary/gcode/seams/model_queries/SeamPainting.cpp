...
Painting::Painting(const Transform3d &obj_transform, const ModelVolumePtrs &volumes)
{
    ...
    this->enforcers_tree = AABBTreeIndirect::build_aabb_tree_over_indexed_triangle_set(this->enforcers.vertices,
                                                                                       this->enforcers.indices);
    this->blockers_tree = AABBTreeIndirect::build_aabb_tree_over_indexed_triangle_set(this->blockers.vertices,
                                                                                      this->blockers.indices);
}
...
bool Painting::is_enforced(const Vec3f &position, float radius) const
{
    ...
    return AABBTreeIndirect::is_any_triangle_in_radius(enforcers.vertices, enforcers.indices, enforcers_tree, position,
                                                       radius_sqr);
}
...
bool Painting::is_blocked(const Vec3f &position, float radius) const
{
    ...
    float radius_sqr = radius * radius;
    return AABBTreeIndirect::is_any_triangle_in_radius(blockers.vertices, blockers.indices, blockers_tree, position,
                                                       radius_sqr);
}
} // namespace Luminary::Seams::ModelInfo
