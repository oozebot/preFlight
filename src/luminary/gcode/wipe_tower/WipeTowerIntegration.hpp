...
{
public:
    WipeTowerIntegration(Vec2f pos, double rotation, const PrintConfig &print_config,
    ...
    std::optional<WipeTower::ToolChangeResult> get_toolchange(std::size_t index, bool ignore_sparse) const
    {
        ...
        if (ignore_sparse && m_tool_changes.at(m_layer_idx).size() == 1 &&
            (m_tool_changes.at(m_layer_idx).front().initial_tool == m_tool_changes.at(m_layer_idx).front().new_tool) &&
            m_layer_idx != 0)
        {
            // Ignore sparse
            return std::nullopt;
...
private:
    ...
    float get_alpha() const { return m_wipe_tower_rotation / 180.f * float(M_PI); }

    WipeTowerIntegration &operator=(const WipeTowerIntegration &);
    std::string append_tcr(GCodeGenerator &gcodegen, const WipeTower::ToolChangeResult &tcr, int new_extruder_id,
                           double z = -1.) const;

    // Postprocesses gcode: rotates and moves G1 extrusions and returns result
    std::string post_process_wipe_tower_moves(const WipeTower::ToolChangeResult &tcr, const Vec2f &translation,
                                              float angle) const;
