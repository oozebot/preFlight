...
std::string WipeTowerIntegration::append_tcr(GCodeGenerator &gcodegen, const WipeTower::ToolChangeResult &tcr,
                                             int new_extruder_id, double z) const
{
    if (new_extruder_id != -1 && new_extruder_id != tcr.new_tool)
        throw Luminary::InvalidArgument(
    ...
    const bool should_travel_to_tower = !tcr.priming &&
                                        (tcr.force_travel     // wipe tower says so
                                         ...
                                         || is_ramming || will_go_down); // don't dig into the print
    if (should_travel_to_tower)
    {
        ...
        else
        {
            if (gcodegen.last_position)
            {
                ...
                gcode += gcodegen.travel_to(from, to, ExtrusionRole::Mixed, comment, []() { return ""; });
            }
            else
            {
    ...
    if (tcr.priming || (new_extruder_id >= 0 && needs_toolchange))
    {
        ...
        // layer_z and toolchange_z both come from this print_z.
    ...
    // The wipe tower is a plate feature: its accelerations follow the project value, never an
    // object's override.
    const double default_acceleration = gcodegen.m_print->default_object_config().default_acceleration.value;
    ...
    gcode += tcr_gcode;
    gcode += gcodegen.writer().set_print_acceleration(fast_round_up<unsigned int>(default_acceleration));
    ...
    else
    {
        ...
        std::transform(tcr.wipe_path.begin(), tcr.wipe_path.end(), std::back_inserter(path),
                       ...
                       {
                           return Geometry::ArcWelder::Segment{
                               wipe_tower_point_to_object_point(gcodegen, transform_wt_pt(wipe_pt))};
                       });
...
std::string WipeTowerIntegration::post_process_wipe_tower_moves(const WipeTower::ToolChangeResult &tcr,
                                                                const Vec2f &translation, float angle) const
{
    ...
    if (gcodegen.writer().need_toolchange(extruder_id) || finish_layer)
    {
        if (m_layer_idx < (int) m_tool_changes.size())
        {
            if (!(size_t(m_tool_change_idx) < m_tool_changes[m_layer_idx].size()))
                throw Luminary::RuntimeError("Wipe tower generation failed, possibly due to empty first layer.");
            ...
            if (gcodegen.config().wipe_tower_no_sparse_layers.value)
            {
                ...
                ignore_sparse = (m_tool_changes[m_layer_idx].size() == 1 &&
                                 m_tool_changes[m_layer_idx].front().initial_tool ==
                                     m_tool_changes[m_layer_idx].front().new_tool &&
                                 m_layer_idx != 0);
...
std::string WipeTowerIntegration::finalize(GCodeGenerator &gcodegen)
{
    ...
    if (std::abs(gcodegen.writer().get_position().z() - purge_z) > EPSILON)
        gcode += gcodegen.generate_travel_gcode({{gcodegen.last_position->x(), gcodegen.last_position->y(),
                                                  scaled(purge_z)}},
