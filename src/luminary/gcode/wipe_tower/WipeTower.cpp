...
public:
    ...
    // Suppress / resume G-code preview. The preview has difficulty differentiating the various
    ...
    WipeTowerWriter &wait(float time)
    {
        if (time == 0.f)
            return *this;
        m_gcode += "G4 S" + Luminary::float_to_string_decimal_point(time, 3) + "\n";
        return *this;
    }
...
private:
    ...
    {
        m_current_pos.x() = x;
        return " X" + Luminary::float_to_string_decimal_point(x, 3);
    }

    std::string set_format_Y(float y)
    {
        m_current_pos.y() = y;
        return " Y" + Luminary::float_to_string_decimal_point(y, 3);
    }

    std::string set_format_Z(float z) { return " Z" + Luminary::float_to_string_decimal_point(z, 3); }

    std::string set_format_E(float e) { return " E" + Luminary::float_to_string_decimal_point(e, 4); }
...
WipeTower::WipeTower(const Vec2f &pos, double rotation_deg, const PrintConfig &config,
                     const PrintObjectConfig &default_object_config, const PrintRegionConfig &default_region_config,
...
{
    // The tower is a plate feature: its first layer takes the project's first-layer speed, never
    // an object's override. A percentage is taken over this default, because the role speeds
    // are not accessible here.
    const float default_speed = 60.f;
    m_first_layer_speed = float(default_object_config.first_layer_speed.get_abs_value(default_speed));
    m_print_volumetric_cap = float(default_object_config.max_volumetric_flow.value);
...
void WipeTower::set_extruder(size_t idx, const PrintConfig &config)
{
    ...
    // The project's Max volumetric flow clamps the filament flow when set lower; the tower is a
    // plate feature, so an object's own cap never reaches it.
    if (m_print_volumetric_cap > 0.f)
        max_vol_speed = (max_vol_speed > 0.f) ? std::min(max_vol_speed, m_print_volumetric_cap)
                                              : m_print_volumetric_cap;
...
void WipeTower::toolchange_Change(WipeTowerWriter &writer, const size_t new_tool, const std::string &new_material)
{
    ...
    writer
        ...
        .append(std::string("G1 X") + Luminary::float_to_string_decimal_point(current_pos.x()) + " Y" +
                Luminary::float_to_string_decimal_point(current_pos.y()) + never_skip_tag() + "\n");
...
static WipeTower::ToolChangeResult merge_tcr(WipeTower::ToolChangeResult &first, WipeTower::ToolChangeResult &second)
{
    ...
    if (first.end_pos != second.start_pos)
        out.gcode += "G1 X" + Luminary::float_to_string_decimal_point(second.start_pos.x(), 3) + " Y" +
                     Luminary::float_to_string_decimal_point(second.start_pos.y(), 3) + " F7200\n";
