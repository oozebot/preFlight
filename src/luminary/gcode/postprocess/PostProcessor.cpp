...
static DWORD execute_process_winapi(const std::wstring &command_line)
{
    ...
    if (!::CreateProcessW(nullptr /* lpApplicationName */, (LPWSTR) command_line.c_str(),
        ...
        throw Luminary::RuntimeError(std::string("Failed starting the script ") + boost::nowide::narrow(command_line) +
...
static int run_script(const std::string &script, const std::string &gcode, std::string & /*std_err*/)
{
    ...
    if (szArglist == nullptr || nArgs <= 0)
    {
        ...
        throw Luminary::RuntimeError(std::string("Post processing script ") + script + " on file " + gcode +
    ...
    if (!boost::filesystem::exists(boost::filesystem::path(command)))
        throw Luminary::RuntimeError(std::string("The configured post-processing script does not exist: ") +
                                     boost::nowide::narrow(command));
    if (boost::iends_with(command, L".pl"))
    {
        // This is a perl script. Run it through the perl interpreter.
        // The current process may be preFlight.exe or preFlight-console.exe.
        ...
        {
            LocalFree(szArglist);
            throw Luminary::RuntimeError(std::string("Perl interpreter ") + path_perl.string() + " does not exist.");
        }
                              ...
                              const DynamicPrintConfig &config)
{
    ...
    if ( // the config carries no such option
    ...
    if (!boost::filesystem::exists(gcode_file))
        throw Luminary::RuntimeError(std::string("Post-processor can't find exported gcode file"));

    // Store print configuration into environment variables, under the documented PREFLIGHT_ prefix and
    // under the SLIC3R_ twin (see ConfigBase::setenv_).
    config.setenv_();
    // The PREFLIGHT_PP_HOST and PREFLIGHT_PP_OUTPUT_NAME names are the documented ones; the two twins below
    // carry the same values so scripts written against the old names keep working.
    // Let the post-processing script know the target host ("File", "PrusaLink", "Repetier", "OctoPrint", "FlashAir", "Duet", "AstroBox" ...)
    boost::nowide::setenv("PREFLIGHT_PP_HOST", host.c_str(), 1);
    ...
    boost::nowide::setenv("PREFLIGHT_PP_OUTPUT_NAME", output_name.c_str(), 1);
    ...
    try
    {
        ...
        if (boost::filesystem::exists(path_output_name))
        {
                ...
                if (host == "File")
                {
                    ...
                    else
                    {
                        if (!op.is_absolute() || !op.has_filename())
                            throw Luminary::RuntimeError("Unable to parse desired new path from output name file");
                    }
                    if (!fs::exists(fs::path(new_output_name).parent_path()))
                        throw Luminary::RuntimeError(
                            Luminary::format("Output directory does not exist: %1%",
            ...
            {
                throw Luminary::RuntimeError(Luminary::format("run_post_process_scripts: Failed reading a file %1% "
