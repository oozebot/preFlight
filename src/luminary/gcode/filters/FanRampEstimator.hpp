...
// part cooling) to transition between arbitrary speed setpoints (e.g., 0 to 100%, 60 to 100%,
// 80 to 30%).
//
// Model: Acceleration decreases at higher speeds.
//   acceleration(s) is proportional to (1 - s)^p, where p varies by fan type
//
// Closed-form solution for acceleration:
//   time(s1 to s2) = [(1-s1)^exp - (1-s2)^exp] * t_up
//   where exp = 1 - p (configurable via fan response type)
//
// Coast-down (deceleration) uses inverse-speed model:
//   time(s1 to s2) = (1/s2 - 1/s1) * t_down / 19
//
// Key properties:
...
//   0% to 100%:  1.000s (100%)
//   0% to 20%:   0.145s (14.5%)
//   20% to 80%:  0.531s (53.1%)
//   80% to 100%: 0.324s (32.4%)

class FanRampEstimator
{
public:
    // Construct with measured fan characteristics
    // t_up: Time in seconds for 0% to 100% spin-up (from fan spec or measured)
    // exponent: Curve shape (0.7=axial/fast, 0.5=blower, 0.4=high-inertia)
    // t_down: Time in seconds for the 100% to ~5% coast (0 = derive from 2x t_up)
...
private:
    ...
    float m_t_up;     // Measured 0 to 100% time (seconds)
    float m_exponent; // Curve shape exponent (0.7=fast, 0.5=moderate, 0.4=slow)
    float m_t_down;   // Measured 100% to ~5% coast time (seconds)
};
