    ...
    : m_config(gcodegen.config()), m_toolchange_prefix(gcodegen.writer().toolchange_prefix()), m_current_extruder(0)
{
    ...
    m_machine_limits = GCodeProcessor::resolve_machine_limits(m_config);
...
public:
    ...
    // The accelerations and the Z feedrate come from the limits the time estimator resolved for
    // the flavor, not from the raw profile arrays (empty or hidden under Klipper and RRF).
    MiniTimeEstimator(const PrintConfig &config, const MachineEnvelopeConfig &limits, unsigned int extruder_id)
        : m_print_accel(get_config_float(limits.machine_max_acceleration_extruding, extruder_id, DEFAULT_ACCEL))
        , m_retract_accel(
              get_config_float(limits.machine_max_acceleration_retracting, extruder_id, DEFAULT_RETRACT_ACCEL))
        , m_travel_accel(get_config_float(limits.machine_max_acceleration_travel, extruder_id, DEFAULT_TRAVEL_ACCEL))
        , m_z_accel(get_config_float(limits.machine_max_acceleration_z, extruder_id, DEFAULT_ACCEL))
        , m_z_max_feedrate(
              get_config_float(limits.machine_max_feedrate_z, extruder_id, 10.0f)) // mm/s, fallback 10 if not set
...
private:
    ...
    static float get_config_float(const ConfigOptionFloats &option, unsigned int id, float default_val)
    {
        ...
        {
            DBG_COUNT("COOLING_TIME_ESTIMATOR_DEFAULT_LIMIT");
            return default_val;
        }
...
// Deferred definitions: they need the CoolingLine::Type enum.
...
// Internal linkage: no other translation unit names it.
static std::string emit_feedrate(const float feedrate)
{
                                                                ...
                                                                const bool use_relative_e_distances)
{
    ...
    const Vec4f position_middle = Luminary::lerp(segment.position_start, segment.position_end, t);
                                                             ...
                                                             const bool use_relative_e_distances)
{
    ...
    const Vec4f position_middle_4d = Vec4f(position_middle.x(), position_middle.y(),
                                           Luminary::lerp(segment.position_start.z(), segment.position_end.z(), t),
                                           Luminary::lerp(segment.position_start.w(), segment.position_end.w(), t));
...
struct PerExtruderAdjustments
{
    ...
    {
        // Sorted on first use by a solver that needs the adjustable prefix; a layer whose
        // extruders never reach that solver keeps its lines in parse order.
        if (lines_sorted)
            return;
        lines_sorted = true;
    ...
    bool lines_sorted = false;
    // Whether the parse keeps the per-move segments of adjustable blocks; only the
    // ConsistentSurface logic splits blocks and reads them.
    bool capture_move_segments = true;
...
// Used by non-proportional slow down. Internal linkage: no other translation unit names it.
static float new_feedrate_to_reach_time_stretch(std::vector<PerExtruderAdjustments *>::const_iterator it_begin,
                                                                     ...
                                                                     std::array<float, 5> &current_pos) const
{
    ...
    for (size_t i = 0; i < m_extruder_ids.size(); ++i)
    {
        ...
        adj.capture_move_segments = adj.cooling_slowdown_logic == CoolingSlowdownLogicType::ConsistentSurface &&
                                    m_config.cooling_perimeter_transition_distance.get_at(extruder_id) >= 0.;
    ...
    for (; *line_start != 0; line_start = line_end)
    {
        ...
        if (line.type)
        {
            ...
            if ((line.type & CoolingLine::TYPE_G92) == 0)
            {
                    ...
                    (line.type & (CoolingLine::TYPE_G1 | CoolingLine::TYPE_G2G3)))
                {
                    ...
                    if (!adjustment->capture_move_segments)
                    {
                        // Nothing splits this extruder's blocks, so the segments are never read.
                    }
                    else if (line.type & CoolingLine::TYPE_G2G3_IJ)
                    {
        ...
        else if (boost::starts_with(sline, ";_EXTRUDE_END"))
        {
            ...
            if (active_speed_modifier != size_t(-1))
            {
                ...
                if (sm.time() <= 0)
                {
                    ...
                    DBG_COUNT("COOLING_ZERO_LENGTH_BLOCK");
                }
            }
            active_speed_modifier = size_t(-1);
        }
        ...
        else if (boost::starts_with(sline, "G4 "))
        {
            ...
            bool has_S = pos_S != std::string_view::npos;
            bool has_P = pos_P != std::string_view::npos;
            if (has_S || has_P)
            {
                //auto [pend, ec] =
                ...
                if (has_P && !has_S)
                {
                    // Custom G-code dwells in milliseconds; the generator emits the S form only.
                    DBG_COUNT("COOLING_G4_P_DWELL");
                                                          ...
                                                          float slowdown_below_layer_time)
{
    ...
    if (max_time_nep > slowdown_below_layer_time)
    {
        ...
        if (!converged)
            DBG_COUNT("COOLING_SLOWDOWN_SOLVER_BAILOUT");
    }
    else
    {
...
#undef EXTRUDER_CONFIG
    ...
    const int emitted_before = emitted_fan_speed;
    ...
    if (manual_fan_active && int(layer_id) >= disable_first)
    {
        ...
        // The fan is still at what was last emitted, so the tracked state goes back too;
        // otherwise a following per-feature marker equal to the un-emitted value is skipped.
        new_gcode.resize(gcode_before);
        emitted_fan_speed = emitted_before;
    }

    const CoolingLine *line_waiting_for_split = nullptr;

    // Initialize mini time estimator for physics-based time calculation
    MiniTimeEstimator time_estimator(m_config, m_machine_limits, m_current_extruder);
    ...
    for (size_t line_idx = 0; line_idx < lines.size(); ++line_idx)
    {
        ...
        if (line->type & CoolingLine::TYPE_SET_TOOL)
        {
            ...
            if (res.ec != std::errc::invalid_argument && new_extruder != m_current_extruder)
            {
                ...
                const int emitted_before_tool = emitted_fan_speed;
                change_extruder_set_fan();
                if (manual_fan_active)
                {
                    new_gcode.resize(gcode_before_tool);
                    emitted_fan_speed = emitted_before_tool;
                }
            }
        ...
        else if (line->type & CoolingLine::TYPE_BRIDGE_FAN_START)
        {
            ...
            // The SET_FAN_SPEED marker comes after BRIDGE_FAN_START in the G-code, so the target
            // fan speed is found by looking ahead in the lines vector
            ...
            // bridge_fan_control compares the bridge speed with the automatic layer speed, which
            // manual mode never emits; there the ramp is decided by the target against the fan's
            // actual state alone.
            const bool ramp_allowed = manual_fan_active || bridge_fan_control;
            if (ramp_allowed && target_fan_speed != emitted_fan_speed && !bridge_ramp_done && needs_early_ramp)
            {
                ...
                if (estimator.enabled() && !emitted_moves.empty())
                {
                    ...
                    { // Only bother if ramp time is significant
                        ...
                        for (size_t i = emitted_moves.size(); i > 0; --i)
                        {
                            ...
                            if (time_after >= ramp_time)
                            {
                                ...
                                else
                                {
                                    ...
                                    DBG_COUNT("FAN_RAMP_PLACED_UNSPLIT");
                                    need_segment_split = false;
                                }
                                break;
                            }
                        ...
                        if (split_move_idx < emitted_moves.size())
                        {
                            ...
                            {
                                DBG_COUNT("FAN_RAMP_ABORTED_FAN_OFF");
                                abort_ramp = true;
                                emitted_moves.clear();
                            }
                            else
                ...
                {
                    DBG_COUNT("FAN_RAMP_FALLBACK_AT_FEATURE");
            ...
            // For other bridge fan features not handled above, just set the fan normally
            ...
            if (new_feedrate == current_feedrate)
            {
                    ...
                    line->length() == 0.)
                {
                    ...
                    remove = true;
                     ...
                     line->length() == 0.)
            {
                // Different feedrate, standalone F-only line (no movement).
