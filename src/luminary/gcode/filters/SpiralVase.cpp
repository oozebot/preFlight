...
std::string SpiralVase::process_layer(const std::string &gcode, bool last_layer)
{
    ...
    const bool transition_in = m_transition_layer && relative_e;
    const bool transition_out = last_layer && relative_e;
    ...
    m_reader.parse_buffer(
        ...
        [z, total_layer_length, layer_height, transition_in, transition_out, smooth_spiral,
         max_xy_smoothing = m_max_xy_smoothing, &len, &last_point, &new_gcode, &transition_gcode, &current_layer,
