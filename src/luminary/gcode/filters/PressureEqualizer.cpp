...
PressureEqualizer::PressureEqualizer(const Luminary::GCodeConfig &config)
    : m_use_relative_e_distances(config.use_relative_e_distances.value)
{
    ...
    m_max_volumetric_extrusion_rate_slope_positive = float(config.max_volumetric_extrusion_rate_slope_positive.value) *
                                                     60.f * 60.f;
    m_max_volumetric_extrusion_rate_slope_negative = float(config.max_volumetric_extrusion_rate_slope_negative.value) *
                                                     60.f * 60.f;
...
void PressureEqualizer::process_layer(const std::string &gcode)
{
    if (!gcode.empty())
    {
        ...
        while (*gcode_begin != 0)
        {
            ...
            // End of lines are always generated in Unix style.
    ...
    for (auto current_extrusion_end_it = m_gcode_lines.cbegin(); current_extrusion_end_it != m_gcode_lines.cend();)
    {
        ...
        const auto current_extrusion_begin_it = std::find_if(current_extrusion_end_it, m_gcode_lines.cend(),
        ...
        while (current_extrusion_end_it != m_gcode_lines.cend())
        {
            ...
            const auto travel_begin_it = std::find_if(std::next(current_extrusion_end_it), m_gcode_lines.cend(),
                                                      [](const GCodeLine &line) { return !line.extruding(); });
        ...
        const size_t current_extrusion_begin_idx = size_t(
            std::distance(m_gcode_lines.cbegin(), current_extrusion_begin_it));
        const size_t current_extrusion_end_idx = size_t(
            std::distance(m_gcode_lines.cbegin(), current_extrusion_end_it));
        this->adjust_volumetric_rate_segment(current_extrusion_begin_idx, current_extrusion_end_idx);
...
PressureEqualizer::GCodeLinesConstIt PressureEqualizer::advance_segment_beyond_small_gap(
    const GCodeLinesConstIt &last_extruding_line_it) const
{
    ...
    for (auto current_line_it = std::next(last_extruding_line_it); current_line_it != m_gcode_lines.cend();
         ++current_line_it)
    {
...
static inline bool is_ws_or_eol(const char c)
{
...
static inline int parse_int(const char *&line)
{
    ...
    if (endptr == nullptr || !is_ws_or_eol(*endptr))
        throw Luminary::InvalidArgument("PressureEqualizer: Error parsing an int");
    line = endptr;
    return int(result);
}

// Internal linkage: this three-argument form is local to the parser and is not the
// LocalesUtils overload every other caller in the tree uses.
...
static inline float parse_float(const char *&line, const size_t line_length)
{
    ...
    if (endptr == 0 || !is_ws_or_eol(*(line + endptr)))
        throw Luminary::RuntimeError("PressureEqualizer: Error parsing a float");
    line = line + endptr;
    return result;
}
...
bool PressureEqualizer::process_line(const char *line, const char *line_end, GCodeLine &buf)
{
    ...
    case 'G':
    {
        ...
        catch (Luminary::InvalidArgument &)
        {
        ...
        case 1:
        {
            ...
            else if (m_current_extrusion_role == GCodeExtrusionRole::Perimeter)
            {
                ...
                if (!internal_perimeter_it_range.empty())
                {
                    ...
                    const char *start_ptr = str_line.data() +
                                            std::distance(str_line.begin(), internal_perimeter_it_range.end());
            ...
            if (changed[3])
            {
                ...
                else
                {
                    ...
                    {
                        printf(
                            "Extremely low flow rate: %f. Line %d, Length: %f, extrusion: %f Old position: (%f, %f, %f), new position: (%f, %f, %f)\n",
                            rate, int(line_idx), sqrt(len2), sqrt((diff[3] * diff[3]) / len2), m_current_pos[0],
                            m_current_pos[1], m_current_pos[2], new_pos[0], new_pos[1], new_pos[2]);
                    }
#endif
                }
            }
...
void PressureEqualizer::GCodeLine::update_end_position(const float *position_end,
                                                       const bool *position_provided_original)
{
...
void PressureEqualizer::GCodeLine::update_end_position(const float *position_start, const float *position_end,
                                                       const float t, const bool *position_provided_original)
{
...
void PressureEqualizer::output_gcode_line(const size_t line_idx)
{
    ...
    const float feedrate_start = line.volumetric_extrusion_rate_start * line.feedrate() /
    ...
    else if (auto nSegments = size_t(ceil(l / max_segment_length)); nSegments == 1)
    ...
    else
    {
        ...
        for (size_t i = 1; i < nSegments; ++i)
        {
            ...
            push_line_to_output(line_idx,
                                pos_start[4] + (pos_end[4] - pos_start[4]) * (float(i) - 0.5f) / float(nSegments),
                                comment);
            comment = nullptr;
            memcpy(line.pos_start, line.pos_end, sizeof(float) * 5);
        }
...
// The sliding-window sweep of a continuous extrusion segment: for every line i of [begin, end) a
// backward pass from i over the window [max(begin, i - look_back), i] limits the deceleration
// into i, then a forward pass over the window limits the acceleration out of the earlier lines.
// The passes are cut short where they can no longer change anything, with the same per-line
// results as running both in full for every line. The backward pass from line i stops at the first line whose
// carried per-role rates are no tighter than at its last visit, provided no forward pass has
// lowered a line further back since (such lines are marked dirty until a backward pass reaches
// them): every earlier line is then a fixed point of the constraints it would receive. The
// forward pass applies its limits from that line on; the lines before it are fixed points of the
// previous forward pass under an equal or looser window, so only their carried rates are walked.
void PressureEqualizer::adjust_volumetric_rate_segment(const size_t begin_idx, const size_t end_idx)
{
    constexpr size_t role_count = size_t(GCodeExtrusionRole::Count);
    constexpr size_t none = std::numeric_limits<size_t>::max();
    constexpr float unlimited = std::numeric_limits<float>::max();
    size_t lowest_dirty = none;
    size_t prev_first_idx = none;

    for (size_t last_line_idx = begin_idx; last_line_idx < end_idx; ++last_line_idx)
    {
        const size_t first_line_idx = size_t(
            std::max<std::ptrdiff_t>(std::ptrdiff_t(begin_idx), std::ptrdiff_t(last_line_idx) - max_look_back_limit));
        if (last_line_idx <= first_line_idx || last_line_idx - first_line_idx < 2)
            continue;
        if (!m_gcode_lines[last_line_idx].extruding())
            continue;
        if (lowest_dirty != none && lowest_dirty < first_line_idx)
        {
            // The dirty line left the window; find the next one inside it.
            lowest_dirty = none;
            for (size_t j = first_line_idx; j <= last_line_idx; ++j)
                if (m_gcode_lines[j].dirty)
                {
                    lowest_dirty = j;
                    break;
                }
        ...
        std::array<float, role_count> feedrate_per_extrusion_role{};
        feedrate_per_extrusion_role.fill(unlimited);
        feedrate_per_extrusion_role[int(m_gcode_lines[last_line_idx].extrusion_role)] =
            m_gcode_lines[last_line_idx].volumetric_extrusion_rate_start;
        m_gcode_lines[last_line_idx].backward_state = feedrate_per_extrusion_role;
        m_gcode_lines[last_line_idx].backward_succ = m_gcode_lines[last_line_idx].volumetric_extrusion_rate_start;
        m_gcode_lines[last_line_idx].backward_visited = true;

        size_t line_idx = last_line_idx;
        size_t restart_idx = first_line_idx;
        while (line_idx != first_line_idx)
        {
            ...
            {
                line_idx = idx_prev;
                m_gcode_lines[line_idx].dirty = false;
                continue;
            }
            ...
            for (size_t iRole = 1; iRole < role_count; ++iRole)
            {
                const float &rate_slope = m_max_volumetric_extrusion_rate_slopes[iRole].negative;
                if (rate_slope == 0 || feedrate_per_extrusion_role[iRole] == unlimited)
                    continue;
                ...
                if (!line.adjustable_flow || line.extrusion_role == GCodeExtrusionRole::BridgeInfill ||
                ...
                {
                    float rate_start = sqrt(rate_end * rate_end + 2 * line.volumetric_extrusion_rate * line.dist_xyz() *
                                                                      rate_slope / line.feedrate());
            ...
            bool tighter = !line.backward_visited || line.volumetric_extrusion_rate_start < line.backward_succ;
            for (size_t iRole = 0; !tighter && iRole < role_count; ++iRole)
                tighter = feedrate_per_extrusion_role[iRole] < line.backward_state[iRole];
            line.backward_state = feedrate_per_extrusion_role;
            line.backward_succ = line.volumetric_extrusion_rate_start;
            line.backward_visited = true;
            if (!tighter && (lowest_dirty == none || lowest_dirty >= line_idx))
            {
                restart_idx = line_idx;
                break;
            }
        }
        // Every dirty line of the window was visited above, or lies before the exit line and
        // was clean.
        lowest_dirty = none;

        // Each pass limits through the slot of one role, the role of the window's first line
        // (only an initialised slot is ever written). The lines before the restart line are
        // fixed points of the previous forward pass only if that pass ran the same chain from
        // the same state: the window start unchanged, or advanced by one line of the same role
        // (its end rate was that pass's carried value after it). Otherwise re-apply from first.
        if (!(prev_first_idx == first_line_idx ||
              (prev_first_idx != none && first_line_idx == prev_first_idx + 1 &&
               m_gcode_lines[first_line_idx].extrusion_role == m_gcode_lines[prev_first_idx].extrusion_role)))
            restart_idx = first_line_idx;
        prev_first_idx = first_line_idx;

        // The forward pass starts where the full backward walk ends: at first when it is an
        // extruding line, else at the lowest extruding line above it.
        size_t forward_begin_idx = first_line_idx;
        while (!m_gcode_lines[forward_begin_idx].extruding() && forward_begin_idx < last_line_idx)
            ++forward_begin_idx;
        restart_idx = std::max(restart_idx, forward_begin_idx);
        feedrate_per_extrusion_role.fill(unlimited);
        feedrate_per_extrusion_role[size_t(m_gcode_lines[forward_begin_idx].extrusion_role)] =
            m_gcode_lines[forward_begin_idx].volumetric_extrusion_rate_end;
        line_idx = forward_begin_idx;
        while (line_idx != last_line_idx)
        {
            ...
            if (line_idx < restart_idx)
            {
                // A fixed point of the previous forward pass: only the carried rates move.
                for (size_t iRole = 1; iRole < role_count; ++iRole)
                {
                    if (m_max_volumetric_extrusion_rate_slopes[iRole].positive == 0 ||
                        feedrate_per_extrusion_role[iRole] == unlimited)
                        continue;
                    if (line.extrusion_role != GCodeExtrusionRole::Ironing)
            ...
            for (size_t iRole = 1; iRole < role_count; ++iRole)
            {
                ...
                if (line.volumetric_extrusion_rate_start > rate_start)
                {
                    ...
                    line.modified = true;
                    line.dirty = true;
                    lowest_dirty = std::min(lowest_dirty, line_idx);
                }
                else if (iRole == size_t(line.extrusion_role))
                {
                    rate_start = line.volumetric_extrusion_rate_start;
                ...
                if (line.adjustable_flow)
                {
                    float rate_end = sqrt(rate_start * rate_start + 2 * line.volumetric_extrusion_rate *
                                                                        line.dist_xyz() * rate_slope / line.feedrate());
...
void PressureEqualizer::push_line_to_output(const size_t line_idx, float new_feedrate, const char *comment)
{
    ...
    if (line_idx > 0 && output_buffer_length > 0)
    {
        ...
        if (is_just_line_with_extrude_set_speed_tag(prev_line_str))
            this->output_buffer_length =
                this->output_buffer_prev_length; // Remove the last line because it only sets the speed for an empty block of g-code lines, so it is useless.
        else
    ...
    {
        feedrate_formatter.emit_string(std::string(INTERNAL_PERIMETER_TAG.data(), INTERNAL_PERIMETER_TAG.length()) +
                                       std::to_string(*line.perimeter_index));
    }
    ...
    for (size_t axis_idx = 0; axis_idx < 3; ++axis_idx)
        if (line.pos_provided[axis_idx])
            extrusion_formatter.emit_axis(char('X' + axis_idx), line.pos_end[axis_idx],
                                          GCodeFormatter::XYZF_EXPORT_DIGITS);
    extrusion_formatter.emit_axis('E',
                                  m_use_relative_e_distances ? (line.pos_end[3] - line.pos_start[3]) : line.pos_end[3],
