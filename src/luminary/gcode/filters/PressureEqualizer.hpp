...
public:
    ...
    explicit PressureEqualizer(const Luminary::GCodeConfig &config);
...
private:
    ...
    struct GCodeLine
    {
        ...
        bool moving_xy() const
        {
            return fabs(pos_end[0] - pos_start[0]) > 0.f || fabs(pos_end[1] - pos_start[1]) > 0.f;
        }
        ...
        float dist_xy2() const
        {
            return (pos_end[0] - pos_start[0]) * (pos_end[0] - pos_start[0]) +
                   (pos_end[1] - pos_start[1]) * (pos_end[1] - pos_start[1]);
        }
        float dist_xyz2() const
        {
            return (pos_end[0] - pos_start[0]) * (pos_end[0] - pos_start[0]) +
                   (pos_end[1] - pos_start[1]) * (pos_end[1] - pos_start[1]) +
                   (pos_end[2] - pos_start[2]) * (pos_end[2] - pos_start[2]);
        }
        ...
        float volumetric_correction_avg() const
        {
            ...
            float avg_correction = std::max(0.05f,
                                            0.5f * (volumetric_extrusion_rate_start + volumetric_extrusion_rate_end) /
                                                volumetric_extrusion_rate);
        ...
        // Incremental sweep state: the per-role rates carried backward past this line at its
        // last visit (valid when backward_visited), and whether a forward pass lowered the
        // line since, a pending constraint the next backward pass has to reach.
        std::array<float, size_t(GCodeExtrusionRole::Count)> backward_state{};
        // The line's start rate at that visit: the previous line is limited by it directly
        // (rate_succ), whether or not its role's slot of the array was written.
        float backward_succ = 0.f;
    ...
    // The sliding-window volumetric-rate sweep of a continuous extrusion segment.
    void adjust_volumetric_rate_segment(size_t begin_idx, size_t end_idx);
