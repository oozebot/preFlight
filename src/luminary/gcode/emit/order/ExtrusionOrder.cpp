...
namespace Luminary::GCode::ExtrusionOrder
{
        ...
        for (size_t entity_idx = 0; entity_idx < entities.size(); ++entity_idx)
        {
            ...
            if (as_extra_ring(ee) != nullptr)
            {
                ...
                if (loop_idx < entities.size())
                {
                    ...
                    {
                        const double fit_resolution = scaled<double>(layer.object()->config().gcode_resolution.value);
                                                        ...
                                                        std::optional<Point> &previous_position)
{
    ...
    for (const LayerIsland &island : ordered_islands)
    {
        ...
        island_extrusions.infill_first = layer.object()->config().infill_first;
...
std::vector<SliceExtrusions> get_slices_extrusions(const Print &print, const Layer &layer,
                                                   ...
                                                   const unsigned extruder_id, std::optional<Point> &previous_position)
{
    // Ironing runs as a second pass over the islands: it changes the extrusion rate sharply and a
    // single ironing run may cover several regions, which costs some travel but keeps the order
    // predictable.
