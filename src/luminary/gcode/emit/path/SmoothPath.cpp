    ...
    if (distance >= 0)
    {
        for (const SmoothPathElement &el : path)
        {
            ...
            for (auto segment_it = el.path.begin() + 1; segment_it != el.path.end(); ++segment_it)
            {
                ...
                {
                    // Circular segment
                    const float angle = Geometry::ArcWelder::arc_angle(prev_point.cast<float>(), point.cast<float>(),
                                                                       segment_it->radius);
                    const double len = std::abs(segment_it->radius) * angle;
                    if (len > distance)
                    {
                        const Point center_pt = Geometry::ArcWelder::arc_center(prev_point.cast<float>(),
                                                                                point.cast<float>(), segment_it->radius,
                                                                                segment_it->ccw())
                                                    .cast<coord_t>();
...
void SmoothPathCache::interpolate_add(const ExtrusionPath &path, const InterpolationParameters &params)
{
    ...
    if (path.role().is_sparse_infill())
        ...
        tolerance *= 4.;
    m_cache[&path.polyline] = Luminary::Geometry::ArcWelder::fit_path(path.polyline.points, tolerance,
                                                                      params.fit_circle_tolerance);
}
...
Geometry::ArcWelder::Path SmoothPathCache::resolve_or_fit(const ExtrusionPath &path, bool reverse,
...
SmoothPath SmoothPathCache::resolve_or_fit_split_with_seam(const ExtrusionLoop &loop, const bool reverse,
                                                           const double resolution, const Point &seam_point,
                                                           const double seam_point_merge_distance_threshold) const
{
    ...
    if (!out.empty())
    {
        ...
        for (const SmoothPathElement &el : out)
            if (Geometry::ArcWelder::PathSegmentProjection this_proj =
                    Geometry::ArcWelder::point_to_path_projection(el.path, seam_point, proj.distance2);
                this_proj.valid())
            {
        ...
        std::pair<Geometry::ArcWelder::Path, Geometry::ArcWelder::Path> split =
            Geometry::ArcWelder::split_at(path, proj, seam_point_merge_distance_threshold);
