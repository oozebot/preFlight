...
#include "luminary/core/algorithm/AStar.hpp"
...
private:
    ...
    bool is_jump_point(CellPositionType pos, CellPositionType forward_dir) const
    {
        ...
        {
            // diagonal
            CellPositionType horizontal_check_dir = CellPositionType(forward_dir.x(), coord_t(0));
            CellPositionType vertical_check_dir = CellPositionType(coord_t(0), forward_dir.y());

            if (!is_passable(pos - horizontal_check_dir) &&
                is_passable(pos + forward_dir - coord_t(2) * horizontal_check_dir))
            {
                return true;
            }

            if (!is_passable(pos - vertical_check_dir) &&
                is_passable(pos + forward_dir - coord_t(2) * vertical_check_dir))
            {
                return true;
            }
...
public:
    ...
    void foreach_reachable(const Node &from, Fn &&fn) const
    {
        ...
        // At most the eight directions; a fixed array avoids a heap allocation per node.
        CellPositionType dirs_to_check[8];
        size_t dirs_count = 0;
        auto add_dir = [&dirs_to_check, &dirs_count](const CellPositionType &dir)
        {
            dirs_to_check[dirs_count++] = dir;
        };

        if (abs(forward_dir.x()) + abs(forward_dir.y()) == 0)
        { // special case for starting point
            for (const CellPositionType &dir : all_directions)
                add_dir(dir);
        }
        else if (abs(forward_dir.x()) + abs(forward_dir.y()) == 2)
        {
            ...
            if (!is_passable(pos - horizontal_check_dir) &&
                is_passable(pos + forward_dir - coord_t(2) * horizontal_check_dir))
            {
                add_dir(forward_dir - coord_t(2) * horizontal_check_dir);
            }

            if (!is_passable(pos - vertical_check_dir) &&
                is_passable(pos + forward_dir - coord_t(2) * vertical_check_dir))
            {
                add_dir(forward_dir - coord_t(2) * vertical_check_dir);
            }

            add_dir(horizontal_check_dir);
            add_dir(vertical_check_dir);
            add_dir(forward_dir);
        }
        else
        { // horizontal or vertical
        ...
        {
            const CellPositionType &dir = dirs_to_check[i];
    ...
    {
        return n.position == target ? -1.f : (target - n.position).template cast<double>().norm();
    }
    ...
    const std::vector<CellPositionType> all_directions{CellPositionType(1, 0),  CellPositionType(1, 1),
                                                       CellPositionType(0, 1),  CellPositionType(-1, 1),
                                                       CellPositionType(-1, 0), CellPositionType(-1, -1),
                                                       CellPositionType(0, -1), CellPositionType(1, -1)};
};
...
{
    // The box grows by the pixels the lines will draw; the bitmap is drawn when queried.
    ...
    for (const Line &l : obstacles)
    {
        ...
        obstacle_lines.emplace_back(start, end);
        inpassable_dirty = true;
    }
}

void JPSPathFinder::build_inpassable()
{
    const size_t cols = size_t(max_search_box.max.x() - max_search_box.min.x() + 1);
    const size_t rows = size_t(max_search_box.max.y() - max_search_box.min.y() + 1);
    inpassable.assign(cols * rows, 0);
    auto store_obstacle = [&](coord_t x, coord_t y)
    {
        inpassable[size_t(y - max_search_box.min.y()) * cols + size_t(x - max_search_box.min.x())] = 1;
        return true;
    };
    for (const Line &l : obstacle_lines)
        double_dda_with_offset(l.a.x(), l.a.y(), l.b.x(), l.b.y(), store_obstacle);
    inpassable_dirty = false;
}

bool JPSPathFinder::is_inpassable(const Pixel &p) const
{
    if (p.x() < max_search_box.min.x() || p.x() > max_search_box.max.x() || p.y() < max_search_box.min.y() ||
        p.y() > max_search_box.max.y())
        return false;
    const size_t cols = size_t(max_search_box.max.x() - max_search_box.min.x() + 1);
    return inpassable[size_t(p.y() - max_search_box.min.y()) * cols + size_t(p.x() - max_search_box.min.x())] != 0;
}
...
void JPSPathFinder::add_obstacles(const Layer *layer, const Point &global_origin)
{
    ...
    for (const Line &l : layer->curled_lines)
    {
        obstacles.push_back(Line{l.a + global_origin, l.b + global_origin});
    }
...
Polyline JPSPathFinder::find_path(const Point &p0, const Point &p1)
{
    ...
    if (obstacle_lines.empty() || (start - end).cast<float>().norm() < 3.0)
    {
    ...
    // A straight line that crosses no obstacle pixel is the shortest path that passes the same
    // test the pruning below applies to the search's result, so it is returned without a
    // search (most travels on a layer with obstacles).
    DBG_COUNT("JPS_SEARCH");
    {
        bool straight_clear = true;
        dda(start.x(), start.y(), end.x(), end.y(),
            [&](coord_t x, coord_t y)
            {
                if (Pixel(x, y) != start && Pixel(x, y) != end && is_inpassable(Pixel(x, y)))
                {
        ...
        if (straight_clear)
        {
            DBG_COUNT("JPS_STRAIGHT_CLEAR");
            return Polyline{p0, p1};
        }
    }

    if (is_inpassable(start))
    {
        dda(start.x(), start.y(), end.x(), end.y(),
            [&](coord_t x, coord_t y)
            {
                if (!is_inpassable(Pixel(x, y)) || start == end)
    ...
    if (is_inpassable(end))
    {
        dda(end.x(), end.y(), start.x(), start.y(),
            [&](coord_t x, coord_t y)
            {
                if (!is_inpassable(Pixel(x, y)) || start == end)
                { // new start not found yet, and xy passable
    ...
    {
        return search_box.contains(pixel) && (pixel == start || pixel == end || !is_inpassable(pixel));
    };
    ...
    {
        // The search exhausted its box without reaching the end; the best-effort path below
        // may still cross curled regions.
        DBG_COUNT("JPS_NO_PATH_BEST_EFFORT");
        ...
        auto coordiante_func = [&astar_cache](size_t idx, size_t dim)
        {
            return float(astar_cache[idx].node.position[dim]);
        };
    ...
    auto scaled_point = [](const Point &p)
    {
        return Point::new_scale(p.x(), p.y());
    };
    ::Luminary::SVG svg(
        debug_out_path(("path_jps" + std::to_string(print_z) + "_" + std::to_string(rand() % 1000)).c_str()).c_str(),
    ...
    std::reverse(out_path.begin(), out_path.end());
    {
        ...
        {
            if ((out_path[i] - out_path[i - 1]).cast<float>().normalized() !=
                (out_path[i + 1] - out_path[i]).cast<float>().normalized())
            {
