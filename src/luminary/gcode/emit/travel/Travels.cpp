...
static Lines extrusion_entity_to_lines(const ExtrusionEntity &e_entity)
{
    ...
    {
        throw Luminary::InvalidArgument("Invalid argument supplied to extrusion_entity_to_lines()");
    }
            ...
            for (const PrintInstance &instance : object->instances())
            {
                ...
                for (const ExPolygon &polygon : slices)
                    for (const Line &line : polygon.lines())
                        lines.emplace_back(unscaled(Point{line.a + instance.shift}),
                                           unscaled(Point{line.b + instance.shift}), object_layer_idx, instance_idx);
            }
        }
    }
            ...
            for (const PrintInstance &instance : layer->object()->instances())
            {
                ...
                for (const LayerSlice &lslice : layer->lslices_ex)
                {
                    for (const LayerIsland &island : lslice.islands)
                    {
                        ...
                        {
                            assert(dynamic_cast<const ExtrusionEntityCollection *>(
                                layerm.perimeters().entities[perimeter_id]));
                            const auto *eec = static_cast<const ExtrusionEntityCollection *>(
                                layerm.perimeters().entities[perimeter_id]);
                            for (const ExtrusionEntity *ee : *eec)
                            {
                                if (ee->role().is_external_perimeter())
                                {
                                    for (const Line &line : extrusion_entity_to_lines(*ee))
                                        lines.emplace_back(unscaled(Point{line.a + instance.shift}),
                                                           unscaled(Point{line.b + instance.shift}), object_layer_idx,
                                                           instance_idx, ee);
                                }
...
void TravelObstacleTracker::mark_extruded(const ExtrusionEntity *extrusion_entity, size_t object_layer_idx,
                                          size_t instance_idx)
{
...
{
    return m_extruded_extrusion.find({line.object_layer_idx, line.instance_idx, line.extrusion_entity}) !=
           m_extruded_extrusion.end();
}

} // namespace Luminary::GCode

namespace Luminary::GCode::Impl::Travels
{
...
std::vector<DistancedPoint> slice_xy_path(tcb::span<const Point> xy_path, tcb::span<const double> sorted_distances)
{
    ...
    for (const Point &point : xy_path.subspan(1))
    {
        ...
        for (const double distance_to_add : sorted_distances.subspan(offset))
        {
            ...
            {
                Point to_place = scaled(
                    place_at_segment(unscaled_point, unscaled_previous_point, distance_to_add - total_distance));
...
Points3 generate_elevated_travel(const tcb::span<const Point> xy_path,
                                 const std::vector<double> &ensure_points_at_distances, const double initial_elevation,
                                 const std::function<double(double)> &elevation)
{
    ...
    {
        result.emplace_back(point.point.x(), point.point.y(),
...
struct Intersection
{
    ...
    {
        return this->object_layer_idx == print_istance.object_layer_idx &&
               this->instance_idx == print_istance.instance_idx;
    }
};

double get_first_crossed_line_distance(tcb::span<const Line> xy_path,
                                       ...
                                       const bool ignore_starting_object_intersection)
{
    ...
    for (const Line &line : xy_path)
    {
        ...
        const std::vector<std::pair<Vec2d, size_t>> intersections = distancer.intersections_with_line<true>(
            unscaled_line);

        if (intersections.empty())
            continue;

        if (!objects_to_print.empty() && ignore_starting_object_intersection &&
            first_intersection.object_layer_idx == -1)
        {
            const ObjectOrExtrusionLinef &intersection_line = distancer.get_line(intersections.front().second);
            const Point shift = objects_to_print[intersection_line.object_layer_idx]
                                    .layer()
                                    ->object()
                                    ->instances()[intersection_line.instance_idx]
                                    .shift;
            const Point shifted_first_point = path_first_point - shift;
            const bool contain_first_point = expolygons_contain(
                objects_to_print[intersection_line.object_layer_idx].layer()->lslices, shifted_first_point);

            first_intersection = {intersection_line.object_layer_idx, intersection_line.instance_idx,
                                  contain_first_point};
        }
        ...
        for (const auto &intersection : intersections)
        {
            ...
            if (skip_intersection && first_intersection.is_print_instance_equal(intersection_line) &&
                first_intersection.is_inside)
            {
...
SmoothingParams get_smoothing_params(const double lift_height, const double slope_end, unsigned extruder_id,
                                     const double travel_length, const FullPrintConfig &config)
{
    ...
    const double max_xy_velocity = Vec2d{config.machine_max_feedrate_x.get_at(extruder_id),
...
ElevatedTravelParams get_elevated_traval_params(const Polyline &xy_path, const FullPrintConfig &config,
                                                ...
                                                const GCode::TravelObstacleTracker &obstacle_tracker)
{
    ...
    SmoothingParams smoothing_params{get_smoothing_params(elevation_params.lift_height, elevation_params.slope_end,
                                                          extruder_id, unscaled(xy_path.length()), config)};
...
Points3 generate_travel_to_extrusion(const Polyline &xy_path, const FullPrintConfig &config, const unsigned extruder_id,
                                     ...
                                     const Point &xy_path_coord_origin)
{
    ...
    ElevatedTravelParams elevation_params{
        get_elevated_traval_params(Polyline{std::move(global_xy_path)}, config, extruder_id, obstacle_tracker)};
    ...
    Points3 result{generate_elevated_travel(xy_path.points, ensure_points_at_distances, initial_elevation,
