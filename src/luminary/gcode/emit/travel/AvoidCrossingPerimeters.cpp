...
struct AllIntersectionsVisitor
{
    ...
    void reset() { intersection_set.clear(); }
             ...
             ++it_contour_and_segment)
        {
                ...
                intersection_set.find(*it_contour_and_segment) == intersection_set.end())
            {
                intersections.push_back(
                    {it_contour_and_segment->first, it_contour_and_segment->second, intersection_point});
...
struct FirstIntersectionVisitor
{
    ...
    const EdgeGrid::Grid &grid;
    const Luminary::Point *pt_current = nullptr;
    const Luminary::Point *pt_next = nullptr;
    ...
    bool operator()(coord_t iy, coord_t ix)
    {
             ...
             ++it_contour_and_segment)
        {
            ...
            if (closest_lines_set.find(*it_contour_and_segment) == closest_lines_set.end() &&
                line_alg::distance_to_squared(Line(segment.first, segment.second), center, &closest_point) <=
                    this->max_distance_squared)
            {
...
static std::vector<ClosestLine> get_closest_lines_in_radius(const EdgeGrid::Grid &grid, const Point &center,
                                                            float search_radius)
{
    ...
    std::sort(visitor.closest_lines.begin(), visitor.closest_lines.end(),
              [&center](const auto &l, const auto &r)
              {
                  return (center - l.point).template cast<double>().squaredNorm() <
                         (center - r.point).template cast<double>().squaredNorm();
              });
                                                          ...
                                                          const float search_radius)
{
    ...
    {
        float dist_from_line_begin = (closest_line.point -
                                      boundary.boundaries[closest_line.border_idx][closest_line.line_idx])
                                         .cast<float>()
                                         .norm();
    ...
    if (!start_lines.empty() && !end_lines.empty())
    {
        ...
        if (cl_indices.first != std::numeric_limits<size_t>::max())
        {
            ...
            new_intersections.push_back(
                {cl_start.border_idx, cl_start.line_idx, cl_start.point, compute_distance(cl_start), true});
            new_intersections.push_back(
                {cl_end.border_idx, cl_end.line_idx, cl_end.point, compute_distance(cl_end), true});
        ...
        auto find_closest_line_index = [&closest_lines](const Intersection &intersection) -> size_t
        {
            for (const ClosestLine &closest_line : closest_lines)
                if (closest_line.border_idx == intersection.border_idx)
                    return &closest_line - &closest_lines.front();
    ...
    if (!new_intersections.empty() && !start_lines.empty())
    {
        ...
        if (cl_start_idx != std::numeric_limits<size_t>::max())
        {
            ...
            new_intersections.front() = {cl_start.border_idx, cl_start.line_idx, cl_start.point,
                                         compute_distance(cl_start), true};
        }
        else
        {
            ...
            size_t start_closest_lines_idx = find_closest_line_with_same_boundary_idx(start_lines, new_intersections,
                                                                                      true);
            const ClosestLine &cl_start = (start_closest_lines_idx != std::numeric_limits<size_t>::max())
                                              ? start_lines[start_closest_lines_idx]
                                              : start_lines.front();
            new_intersections.insert(new_intersections.begin(), {cl_start.border_idx, cl_start.line_idx, cl_start.point,
                                                                 compute_distance(cl_start), true});
        }
    }
    ...
    if (!new_intersections.empty() && !end_lines.empty())
    {
        ...
        else
        {
            ...
            size_t end_closest_lines_idx = find_closest_line_with_same_boundary_idx(end_lines, new_intersections,
                                                                                    false);
            const ClosestLine &cl_end = (end_closest_lines_idx != std::numeric_limits<size_t>::max())
...
static Point find_first_different_vertex(const Polygon &polygon, const size_t point_idx, const Point &point)
{
    ...
    if constexpr (forward)
        for (; point == polygon.points[line_idx] && line_idx != int(point_idx);
             line_idx = line_idx + 1 < int(polygon.points.size()) ? line_idx + 1 : 0)
            ;
    else
        for (; point == polygon.points[line_idx] && line_idx != int(point_idx);
             line_idx = line_idx - 1 >= 0 ? line_idx - 1 : int(polygon.points.size()) - 1)
            ;
...
{
    return polygon.points[point_idx] +
           (get_polygon_vertex_inward_normal(polygon, point_idx) * double(offset)).cast<coord_t>();
}
...
static Point get_middle_point_offset(const Polygon &polygon, const size_t left_idx, const size_t right_idx,
                                     const Point &middle, const coord_t offset)
{
    const Point &left = find_first_different_vertex<false>(polygon, left_idx, middle);
    const Point &right = find_first_different_vertex<true>(polygon, right_idx, middle);
...
static void export_travel_to_svg(const Polygons &boundary, const Line &original_travel, const Polyline &result_travel,
                                 const std::vector<Intersection> &intersections, const std::string &path)
{
    coordf_t stroke_width = scale_(0.05);
    BoundingBox bbox = get_extents(boundary);
    ::Luminary::SVG svg(path, bbox);
...
static void export_travel_to_svg(const Polygons &boundary, const Line &original_travel,
...
static Direction get_shortest_direction(const AvoidCrossingPerimeters::Boundary &boundary,
                                        const Intersection &intersection_first, const Intersection &intersection_second,
                                        float contour_length)
{
    ...
    if (reversed)
        std::swap(total_length_forward, total_length_backward);

    total_length_forward -= (intersection_first.point - poly[intersection_first.line_idx]).cast<float>().norm();
    total_length_backward -=
        (poly[(intersection_first.line_idx + 1) % poly.size()] - intersection_first.point).cast<float>().norm();

    total_length_forward -=
        (poly[(intersection_second.line_idx + 1) % poly.size()] - intersection_second.point).cast<float>().norm();
...
static std::vector<TravelPoint> simplify_travel(const AvoidCrossingPerimeters::Boundary &boundary,
...
static size_t avoid_perimeters_inner(const AvoidCrossingPerimeters::Boundary &boundary, const Point &start,
                                     const Point &end, const Layer &layer, std::vector<TravelPoint> &result_out)
{
    ...
    std::vector<Intersection> intersections;
    {
        ...
        std::sort(intersections.begin(), intersections.end(), [dir](const auto &l, const auto &r)
    ...
    for (auto it_first = intersections.begin(); it_first != intersections.end(); ++it_first)
    {
        ...
        auto it_second_r = std::find_if(intersections.rbegin(), it_last_item,
                                        [&intersection_first](const Intersection &intersection)
        ...
        size_t right_idx = intersection_first.line_idx + 1 == boundaries[intersection_first.border_idx].points.size()
                               ? 0
        ...
        result.push_back({get_middle_point_offset(boundaries[intersection_first.border_idx], left_idx, right_idx,
                                                  intersection_first.point, coord_t(SCALED_EPSILON)),
        ...
        if (it_second_r != it_last_item)
        {
            ...
            Direction shortest_direction =
                get_shortest_direction(boundary, intersection_first, intersection_second,
            ...
            if (shortest_direction == Direction::Forward)
                for (int line_idx = int(intersection_first.line_idx); line_idx != int(intersection_second.line_idx);
                    ...
                    result.push_back({get_polygon_vertex_offset(
                                          boundaries[intersection_first.border_idx],
                                          (line_idx + 1 == int(boundaries[intersection_first.border_idx].points.size()))
                                              ? 0
                                              : (line_idx + 1),
                ...
                for (int line_idx = int(intersection_first.line_idx); line_idx != int(intersection_second.line_idx);
                     line_idx = line_idx - 1 >= 0 ? line_idx - 1
                                                  : int(boundaries[intersection_first.border_idx].size()) - 1)
                    result.push_back({get_polygon_vertex_offset(boundaries[intersection_second.border_idx],
                                                                line_idx + 0, coord_t(SCALED_EPSILON)),
                                      int(intersection_first.border_idx)});

            // Append the farthest intersection into the path
            left_idx = intersection_second.line_idx;
            right_idx = (intersection_second.line_idx >= (boundaries[intersection_second.border_idx].points.size() - 1))
                            ? 0
                            : (intersection_second.line_idx + 1);
            result.push_back({get_middle_point_offset(boundaries[intersection_second.border_idx], left_idx, right_idx,
                                                      intersection_second.point, coord_t(SCALED_EPSILON)),
...
#ifdef AVOID_CROSSING_PERIMETERS_DEBUG_OUTPUT
    {
        ...
        export_travel_to_svg(boundaries, Line(start, end), result, intersections,
                             debug_out_path("AvoidCrossingPerimetersInner-initial-%d-%d.svg", layer.id(), iRun++));
    }
#endif /* AVOID_CROSSING_PERIMETERS_DEBUG_OUTPUT */
...
static size_t avoid_perimeters(const AvoidCrossingPerimeters::Boundary &boundary, const Point &start, const Point &end,
                               const Layer &layer, Polyline &result_out)
{
    ...
    {
        static int iRun = 0;
        export_travel_to_svg(boundary.boundaries, Line(start, end), path, {},
                             debug_out_path("AvoidCrossingPerimeters-final-%d-%d.svg", layer.id(), iRun++));
    }
#endif /* AVOID_CROSSING_PERIMETERS_DEBUG_OUTPUT */
...
static bool any_expolygon_contains(const ExPolygons &lslices_offset,
                                   ...
                                   const EdgeGrid::Grid &grid_lslices_offset, const Line &travel)
{
...
// Check if anyone of ExPolygons contains whole travel.
// called by need_wipe()
static bool any_expolygon_contains(const ExPolygons &ex_polygons, const std::vector<BoundingBox> &ex_polygons_bboxes,
                                   const EdgeGrid::Grid &grid_lslice_offset, const Polyline &travel)
{
    assert(ex_polygons.size() == ex_polygons_bboxes.size());
    if (std::any_of(travel.points.begin(), travel.points.end(),
                    [&grid_lslice_offset](const Point &point) { return !grid_lslice_offset.bbox().contains(point); }))
        return false;
    ...
    if (!any_intersection)
    {
        for (const ExPolygon &ex_polygon : ex_polygons)
        {
            ...
            if (std::all_of(travel.points.begin(), travel.points.end(),
                            [&bbox](const Point &point) { return bbox.contains(point); }) &&
...
static bool need_wipe(const GCodeGenerator &gcodegen, const ExPolygons &lslices_offset,
                      const std::vector<BoundingBox> &lslices_offset_bboxes, const EdgeGrid::Grid &grid_lslices_offset,
                      const Line &original_travel, const Polyline &result_travel, const size_t intersection_count)
{
    ...
    if (intersection_count > 0)
    {
        ...
        if (z_lift_enabled)
        {
            if (any_expolygon_contains(lslices_offset, lslices_offset_bboxes, grid_lslices_offset, original_travel))
            {
                ...
                wipe_needed = !(result_travel.size() > 2 && result_travel.first_point() == original_travel.a &&
                                result_travel.last_point() == original_travel.b) &&
                              !any_expolygon_contains(lslices_offset, lslices_offset_bboxes, grid_lslices_offset,
                                                      result_travel);
            }
            else
            {
        ...
        {
            wipe_needed = !any_expolygon_contains(lslices_offset, lslices_offset_bboxes, grid_lslices_offset,
                                                  result_travel);
        }
    }
...
static void precompute_polygon_distances(const Polygon &polygon, std::vector<float> &polygon_distances_out)
{
    ...
    for (size_t point_idx = 1; point_idx < polygon.size(); ++point_idx)
        polygon_distances_out[point_idx] = polygon_distances_out[point_idx - 1] +
                                           float((polygon[point_idx] - polygon[point_idx - 1]).cast<double>().norm());
    polygon_distances_out.back() = polygon_distances_out[polygon.size() - 1] +
                                   float((polygon.points.back() - polygon.points.front()).cast<double>().norm());
}

static void precompute_expolygon_distances(const ExPolygon &ex_polygon,
                                           std::vector<std::vector<float>> &expolygon_distances_out)
{
...
static std::vector<float> contour_distance(const EdgeGrid::Grid &grid, const std::vector<float> &poly_distances,
                                           const size_t contour_idx, const Polygon &polygon, double compensation,
                                           double search_radius)
{
    ...
    if (polygon.size() > 2)
    {
        struct Visitor
        {
            Visitor(const EdgeGrid::Grid &grid, const size_t contour_idx, const std::vector<float> &polygon_distances,
                    double dist_same_contour_accept, double dist_same_contour_reject)
                : grid(grid)
                , idx_contour(contour_idx)
                , contour(grid.contours()[contour_idx])
                , boundary_parameters(polygon_distances)
...
static ExPolygons inner_offset(const ExPolygons &ex_polygons, double offset)
{
    ...
    for (ExPolygon &ex_poly : ex_poly_result)
    {
        ...
        if (Vec2d bbox_size = bbox.size().cast<double>(); bbox_size.x() * bbox_size.y() < Luminary::sqr(scale_(0.1f)))
            continue;
        ...
        for (const double &min_contour_width : min_contour_width_values)
        {
            ...
            for (size_t idx_contour = 0; idx_contour <= ex_poly.holes.size(); ++idx_contour)
            {
                ...
                std::vector<float> distances = contour_distance(grid, ex_poly_distances[idx_contour], idx_contour, poly,
...
static void init_boundary(AvoidCrossingPerimeters::Boundary *boundary, Polygons &&boundary_polygons)
{
    ...
    // 1 mm cells: the grid only accelerates intersection queries, it does not affect the path.
...
Polyline AvoidCrossingPerimeters::travel_to(const GCodeGenerator &gcodegen, const Point &point,
                                            bool *could_be_wipe_disabled)
{
    ...
    if (!use_external && (is_support_layer || (!m_lslices_offset.empty() &&
                                               !any_expolygon_contains(m_lslices_offset, m_lslices_offset_bboxes,
                                                                       m_grid_lslices_offset, travel))))
    {
        ...
        {
            travel_intersection_count = avoid_perimeters(m_internal, startf.cast<coord_t>(), endf.cast<coord_t>(),
                                                         *gcodegen.layer(), result_pl);
    ...
    else if (use_external)
    {
        ...
        {
            travel_intersection_count = avoid_perimeters(m_external, startf.cast<coord_t>(), endf.cast<coord_t>(),
                                                         *gcodegen.layer(), result_pl);
            result_pl.points.front() = start;
            result_pl.points.back() = end;
        }
    ...
    if (opt_max_detour.value > 0)
    {
        ...
        double max_detour_length = opt_max_detour.percent ? direct_length * 0.01 * opt_max_detour.value
                                                          : scale_(opt_max_detour.value);
        if (detour > max_detour_length)
        {
            // The straight line crosses whatever the detour avoided
            DBG_COUNT("TRAVEL_ACP_MAX_DETOUR_FALLBACK");
            result_pl = {start, end};
    ...
    else if (max_detour_length_exceeded)
    {
        ...
        *could_be_wipe_disabled = !need_wipe(gcodegen, m_lslices_offset, m_lslices_offset_bboxes, m_grid_lslices_offset,
                                             travel, result_pl, travel_intersection_count);
