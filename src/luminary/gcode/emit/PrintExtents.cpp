...
{
    BoundingBox bbox = extrusion_polyline_extents(extrusion_path.polyline,
                                                  coord_t(scale_(0.5 * extrusion_path.width())));
    BoundingBoxf bboxf;
    if (!empty(bbox))
    {
...
static BoundingBoxf extrusionentity_extents(const ExtrusionEntity *extrusion_entity)
{
    ...
    throw Luminary::RuntimeError("Unexpected extrusion_entity type in extrusionentity_extents()");
    return BoundingBoxf();
}
...
BoundingBoxf get_wipe_tower_extrusions_extents(const Print &print, const coordf_t max_print_z)
{
    ...
    Transform2d trafo = Eigen::Translation2d(print.model().wipe_tower().position.x(),
                                             print.model().wipe_tower().position.y()) *
