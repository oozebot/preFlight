...
#include "luminary/layer/wipe_tower/WipeTower.hpp"
#include "luminary/gcode/wipe_tower/WipeTowerIntegration.hpp"
#include "luminary/gcode/emit/travel/Travels.hpp"
...
#include "luminary/toolpath/travel/TravelOptimization.hpp"
...
#include <list>      // std::list, which the precompiled header had been supplying
#include <math.h>
#include <optional>
#include <set>
#include <string>
...
// Intel redesigned some TBB interface considerably when merging TBB with their oneAPI set of libraries.
...
using tbb_filtermode = tbb::filter_mode;
#else
#include <tbb/pipeline.h>
using tbb_filtermode = tbb::filter;
#endif
...
void GCodeGenerator::PlaceholderParserIntegration::validate_output_vector_variables()
{
    if (this->opt_position->values.size() != 3)
        throw Luminary::RuntimeError("\"position\" output variable must not be resized by the script.");
    if (this->num_extruders > 0)
    {
        if (this->opt_e_position && this->opt_e_position->values.size() != this->num_extruders)
            throw Luminary::RuntimeError("\"e_position\" output variable must not be resized by the script.");
        if (this->opt_e_retracted->values.size() != this->num_extruders)
            throw Luminary::RuntimeError("\"e_retracted\" output variable must not be resized by the script.");
        if (this->opt_e_restart_extra->values.size() != this->num_extruders)
            throw Luminary::RuntimeError("\"e_restart_extra\" output variable must not be resized by the script.");
    }
}
...
GCodeGenerator::ObjectsLayerToPrint GCodeGenerator::collect_layers_to_print(const PrintObject &object)
{
    ...
    while (idx_object_layer < object.layers().size() || idx_support_layer < object.support_layers().size())
    {
        ...
        {
            if (!has_extrusions)
                throw Luminary::SlicingError(_u8L("There is an object with no extrusions in the first layer.") + "\n" +
    ...
    if (!warning_ranges.empty())
    {
        ...
        for (i = 0; i < std::min(warning_ranges.size(), size_t(3)); ++i)
            warning += Luminary::format(_u8L("Empty layer between %1% and %2%."), warning_ranges[i].first,
        ...
        warning += "\n";
        warning +=
            Luminary::format(_u8L("Object name: %1%"), object.model_object()->name) + "\n\n" +
            _u8L(
                               ...
                               ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    print->set_gcode_export_started();
    ...
    m_processor.set_preprocessing_timeout_seconds(print->preprocessing_timeout_seconds());
#endif
    ...
    try
    {
        ...
        if (file.is_error())
        {
            ...
            throw Luminary::RuntimeError(format(_u8L("G-code export to %1% failed\nIs the disk full?"), path) + "\n");
        }
    }
    catch (std::exception &)
    {
    ...
    if (!m_placeholder_parser_integration.failed_templates.empty())
    {
        ...
        // path is empty in virtual-file mode; do not print a blank destination.
        std::string msg = (path == nullptr || *path == '\0')
                              ? _u8L("G-code export failed due to invalid custom G-code sections:") + "\n\n"
                              : format(_u8L("G-code export to %1% failed due to invalid custom G-code sections:"),
                                       path) +
                                    "\n\n";
        ...
        // The two markers are passed as arguments so the message quotes the exact text written into the G-code.
        msg += "\n" + format(_u8L("The error messages are enclosed between\n        %1%\nand\n        %2%\nin the "
                                  "G-code output for all macro processing errors."),
                             "!!!!! Failed to process the custom G-code template ...",
                             "!!!!! End of an error report for the custom G-code template ...");
        throw Luminary::PlaceholderParserError(msg);
    }
    ...
    //    DoExport::update_print_estimated_times_stats(m_processor, print->print_statistics());
    DoExport::update_print_estimated_stats(m_processor, m_writer.extruders(), print->print_statistics());
    if (result != nullptr)
    {
    ...
    if (path != nullptr && strlen(path) > 0)
    {
        ...
        if (write_result.gcode_object)
        {
            ...
            if (output_file == nullptr)
                throw Luminary::RuntimeError(format(_u8L("Cannot open output file for writing: %1%"), path));
            ...
            {
                boost::nowide::remove(path);
                throw Luminary::RuntimeError(format(_u8L("Failed to write G-code to %1%\nIs the disk full?"), path));
            }
        }
    }
...
std::vector<const PrintInstance *> chain_print_object_instances(const Print &print)
{
    ...
    std::vector<size_t> ordered = chain_points(object_reference_points);
...
// The G-code resolution is an object setting; plate paths (skirt, brim) take the project's.
static inline GCode::SmoothPathCache::InterpolationParameters interpolation_parameters(
    const PrintConfig &print_config, const PrintObjectConfig &object_config)
{
    return {scaled<double>(object_config.gcode_resolution.value),
...
static inline GCode::SmoothPathCache smooth_path_interpolate_global(const Print &print)
{
    const GCode::SmoothPathCache::InterpolationParameters interpolation_params =
        interpolation_parameters(print.config(), print.default_object_config());
...
void GCodeGenerator::_do_export(Print &print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if (const char *extras = boost::nowide::getenv("PREFLIGHT_PREPARED_BY_INFO"); extras)
    {
        ...
        {
            BOOST_LOG_TRIVIAL(error) << "Value in PREFLIGHT_PREPARED_BY_INFO env variable is invalid. Closing.";
            std::terminate();
        }
    }
    ...
    if (export_to_binary_gcode)
    {
        ...
        // file data
        binary_data.file_metadata.raw_data.emplace_back("Producer", std::string(PREFLIGHT_APP_NAME) + " " +
                                                                        std::string(PREFLIGHT_VERSION));
    ...
    // The curled-overhang router serves every object that asks for it; the bed is shared.
    if (std::any_of(print.objects().begin(), print.objects().end(),
                    [](const PrintObject *object) { return object->config().avoid_crossing_curled_overhangs; }))
    {
    ...
    if (!export_to_binary_gcode)
    {
        ...
        try
        {
            std::string header = Luminary::header_generated();
            file.write_format("; %s\n", header.c_str());
            if (!prepared_by_info.empty())
            {
    ...
    if (print.config().complete_objects.value)
    {
        ...
        if (initial_extruder_id == static_cast<unsigned int>(-1))
            ...
            throw Luminary::SlicingError(_u8L("No extrusions were generated for objects."));
    ...
    if (!(has_wipe_tower && print.config().single_extruder_multi_material_priming))
    {
        ...
        // Skipped when the extruder is primed on the multi-material priming towers at the edge of the print bed.
    ...
    this->prebuild_role_indexes(print);
    pf_export.stage("  export: role index prepass");
    if (print.config().complete_objects.value)
    {
    ...
    const std::string filament_stats_string_out = DoExport::update_print_stats_and_format_filament_stats(
        ...
        // Modifies
        print.print_statistics(), export_to_binary_gcode, m_processor.get_binary_data());
    ...
    {
        bgcode::binarize::BinaryData &binary_data = m_processor.get_binary_data();
        if (print.print_statistics().total_toolchanges > 0)
    ...
    else
    {
        ...
        file.write("\n");
        file.write_format(PrintStatistics::TotalFilamentUsedGValueMask.c_str(), print.print_statistics().total_weight);
        file.write_format(PrintStatistics::TotalFilamentCostValueMask.c_str(), print.print_statistics().total_cost);
        file.write_format(PrintStatistics::TotalFilamentUsedWipeTowerValueMask.c_str(),
                          print.print_statistics().total_wipe_tower_filament_weight);
        if (print.print_statistics().total_toolchanges > 0)
...
void GCodeGenerator::smooth_path_interpolate(const ObjectLayerToPrint &object_layer_to_print,
                                             const PrintConfig &print_config, GCode::SmoothPathCache &out)
{
    const PrintObject *object = object_layer_to_print.object();
    if (object == nullptr)
        return;
    const GCode::SmoothPathCache::InterpolationParameters params = interpolation_parameters(print_config,
                                                                                            object->config());
...
// The serial generator stage of the export pipeline queries the layers' role indexes
// (interlocking flow, over-bridge speed) and would build each one on first use, one layer
// at a time; build them in parallel first. A print whose regions cannot query them pays
// nothing.
void GCodeGenerator::prebuild_role_indexes(const Print &print)
{
    bool queried = false;
    for (const PrintObject *object : print.objects())
        for (const PrintRegion &region : object->all_regions())
            if (region.config().interlock_perimeters_enabled.value || region.config().over_bridge_speed.value > 0)
                queried = true;
    if (!queried)
        return;
    for (const PrintObject *object : print.objects())
    {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, layers.size()),
                          [&layers, &print](const tbb::blocked_range<size_t> &range)
                          {
...
{
    PerfAccumTimer t_smooth, t_generator, t_spiral, t_equalizer, t_cooling, t_output;
    PerfAccumTimer t_set_status;
    // The layer progress takes ~17 distinct values; the status (a gettext lookup and a UI event)
    // is posted only when the percent changes.
    int last_progress = -1;
    const std::string progress_message = _u8L("Generating G-code layers");
    ...
    const auto smooth_path_interpolator = tbb::make_filter<void, std::pair<size_t, GCode::SmoothPathCache>>(
        tbb_filtermode::serial_in_order,
        [this, &print, &layers_to_print, &layer_to_print_idx, &t_smooth, &t_set_status, &elapsed, &last_progress,
         &progress_message](tbb::flow_control &fc) -> std::pair<size_t, GCode::SmoothPathCache>
        {
            ...
            else
            {
                ...
                if (layers_to_print.size() > 0)
                {
                    ...
                    if (progress != last_progress)
                    {
                        ...
                        if constexpr (PERF_TIMING)
                        {
                            ...
                            const_cast<Print &>(print).set_status(progress, progress_message);
                ...
                for (const ObjectLayerToPrint &l : layers_to_print[idx].second)
                    GCodeGenerator::smooth_path_interpolate(l, print.config(), smooth_path_cache);
    ...
    const auto spiral_vase = tbb::make_filter<LayerResult, LayerResult>(
        tbb_filtermode::serial_in_order,
        [spiral_vase = this->m_spiral_vase.get(), &layers_to_print, &t_spiral](LayerResult in) -> LayerResult
        {
            ...
            LayerResult out{spiral_vase->process_layer(std::move(in.gcode), last_layer), in.layer_id,
                            in.spiral_vase_enable, in.cooling_buffer_flush};
            t_spiral.add(ts0, std::chrono::steady_clock::now());
            return out;
        });
    const auto pressure_equalizer = tbb::make_filter<LayerResult, LayerResult>(
        tbb_filtermode::serial_in_order,
        [pressure_equalizer = this->m_pressure_equalizer.get(), &t_equalizer](LayerResult in) -> LayerResult
        {
            auto te0 = std::chrono::steady_clock::now();
            LayerResult out = pressure_equalizer->process_layer(std::move(in));
            t_equalizer.add(te0, std::chrono::steady_clock::now());
            return out;
        });
    ...
    const auto find_replace = tbb::make_filter<std::string, std::string>(
        tbb_filtermode::serial_in_order, [find_replace = this->m_find_replace.get()](std::string s) -> std::string
    ...
    perf_print("[TIMING]   gcode pipeline breakdown (%zu layers):\n"
               ...
               "[TIMING]     spiral_vase:    %7.1fms\n"
               "[TIMING]     equalizer:      %7.1fms\n"
               "[TIMING]     cooling:        %7.1fms\n"
               "[TIMING]     output/write:   %7.1fms\n"
               "[TIMING]     process_buffer: %7.1fms (inside output_stream.write)\n",
               layers_to_print.size(), t_set_status.ms(), t_smooth.ms(), t_generator.ms(), t_spiral.ms(),
               t_equalizer.ms(), t_cooling.ms(), t_output.ms(), output_stream.process_buffer_ms());
}
                                    ...
                                    GCodeOutputStream &output_stream)
{
    ...
    const auto smooth_path_interpolator = tbb::make_filter<void, std::pair<size_t, GCode::SmoothPathCache>>(
        tbb_filtermode::serial_in_order,
        [this, &print, &layers_to_print,
         &layer_to_print_idx](tbb::flow_control &fc) -> std::pair<size_t, GCode::SmoothPathCache>
        {
            if (layer_to_print_idx >= layers_to_print.size())
            {
                if (layer_to_print_idx == layers_to_print.size() + (m_pressure_equalizer ? 1 : 0))
            ...
            else
            {
                ...
                GCode::SmoothPathCache smooth_path_cache;
                GCodeGenerator::smooth_path_interpolate(layers_to_print[idx], print.config(), smooth_path_cache);
                return {idx, std::move(smooth_path_cache)};
            }
        });
    const auto generator = tbb::make_filter<std::pair<size_t, GCode::SmoothPathCache>, LayerResult>(
    ...
    const auto find_replace = tbb::make_filter<std::string, std::string>(
        tbb_filtermode::serial_in_order, [find_replace = this->m_find_replace.get()](std::string s) -> std::string
        { return find_replace->process_layer(std::move(s)); });
    const auto output = tbb::make_filter<std::string, void>(tbb_filtermode::serial_in_order,
        ...
        if (auto it = custom_gcode_placeholders.find(name); it != custom_gcode_placeholders.end())
        {
            ...
            for (const std::string &key : config_override->keys())
            {
                ...
                if (std::find(placeholders.begin(), placeholders.end(), key) == placeholders.end())
                    throw Luminary::PlaceholderParserError(format("\"%s\" placeholder for \"%s\" custom G-code \n"
                ...
                if (!custom_gcode_specific_config_def.has(key))
                    throw Luminary::PlaceholderParserError(format("Definition of \"%s\" placeholder \n"
                                                                  "needs to be added to CustomGcodeSpecificConfigDef",
                                                                  key.c_str()));
            }
        }
        else
            throw Luminary::PlaceholderParserError(
    ...
    const PrintConfig &config)
{
    ...
    if (color_change && color_change_extruder < 0)
    {
        // A colour change item without an extruder (malformed project data) has nothing to
        // emit; the tag would name extruder -1.
        DBG_COUNT("COLOR_CHANGE_EXTRUDER_INVALID");
        return gcode;
    }
    ...
    {
        // On a single extruder multi material printer a colour change for an extruder that is not
        // printing this layer becomes a pause, so the user can swap that filament.
    ...
    else
    {
        ...
        gcode += "\n";
        // A firmware colour change returns with the extruder primed. Update the writer's retract
        // state only; the G-code this call returns is discarded.
                                                 ...
                                                 unsigned int first_extruder_id, const PrintConfig &config)
{
    ...
    else
    {
        ...
        { // Pause print
            ...
            {
                // The message is one G-code line; a line break in it would be executed as G-code.
                std::string message_line = pause_print_msg;
                std::replace_if(
                    message_line.begin(), message_line.end(), [](char c) { return c == '\n' || c == '\r'; }, ' ');
                gcode += "M117 " + message_line + "\n";
            }
    ...
    std::vector<coordf_t> &skirt_done)
{
    ...
    if (print.has_skirt() && !print.skirt().entities.empty() && layer_tools.has_skirt &&
        ...
        // Skirt heights are recorded once for the whole print, so in a sequential print the skirt rises
        // with the first object and resumes only when a later object prints above it.
        (skirt_done.size() < (size_t) print.config().skirt_height.value || print.has_infinite_skirt()))
    {
        bool valid = !skirt_done.empty() && skirt_done.back() < layer_tools.print_z - EPSILON;
        assert(valid);
        // This print_z has not been extruded yet (sequential print)
        // Guard against an empty skirt_done: the skirt for this print_z may not have been emitted
        // yet, and skirt_done.back() is read below.
        if (valid)
        {
...
// Precedence: the object's own override, then the filament's override, then the project value.
static inline double get_seam_gap_distance_value(const PrintConfig &config, const PrintObjectConfig &object_config,
                                                 const PrintObjectConfig &default_object_config,
                                                 const unsigned extruder_id)
{
    const double nozzle_diameter = config.nozzle_diameter.get_at(extruder_id);
    if (object_config.seam_gap_distance != default_object_config.seam_gap_distance)
        return object_config.seam_gap_distance.get_abs_value(nozzle_diameter);
    ...
    return default_object_config.seam_gap_distance.get_abs_value(nozzle_diameter);
}
...
struct SmoothPathGenerator
{
    ...
    const PrintConfig &config;
    // The project's object config: the smooth paths are generated before each instance applies its
    // own config, so object-level keys are read from the layer's object here, and from this for
                                                          ...
                                                          std::optional<InstancePoint> &previous_position)
    {
        ...
        const PrintObjectConfig &object_config = layer != nullptr ? layer->object()->config() : default_object_config;
        const double scaled_resolution = scaled<double>(object_config.gcode_resolution.value);
        ...
        if (auto loop = dynamic_cast<const ExtrusionLoop *>(extrusion_entity))
        {
            ...
            if (const double extrusion_clipping = get_seam_gap_distance_value(config, object_config,
                                                                              default_object_config, extruder_id);
            ...
            else if (enable_loop_clipping && extrusion_clipping < 0.)
            {
                ...
                clip_end(smooth_path_extension, smooth_path_extension_cut_length,
                         scaled<double>(GCode::ExtrusionOrder::min_gcode_segment_length));
                Luminary::append(result, smooth_path_extension);
            }
...
// Fingerprint of one layer's extrusions in emission order: every smooth path element hashed from
// its smaller end with its role family, summed (`hash`, order-insensitive) and chained in
// emission order (`ord`). A layer whose `hash` holds while `ord` moves only reordered its paths;
// a moved seam or a geometry change moves both.
static void dbg_gcode_order_fp(const double print_z,
                               const std::vector<GCode::ExtrusionOrder::ExtruderExtrusions> &extrusions)
{
    using namespace GCode::ExtrusionOrder;
    DetermFingerprint fp;
    size_t paths = 0;
    auto add = [&fp, &paths](const GCode::SmoothPath &path)
    {
        ++paths;
        for (const GCode::SmoothPathElement &element : path)
        {
            Points pts;
            pts.reserve(element.path.size());
            for (const Geometry::ArcWelder::Segment &segment : element.path)
                pts.push_back(segment.point);
            fp.add_path(pts, element.path_attributes.role);
        }
    };
    auto add_ranges = [&add](const std::vector<InfillRange> &ranges)
    {
        for (const InfillRange &range : ranges)
            for (const GCode::SmoothPath &path : range.items)
                add(path);
    };
    auto add_slices = [&add, &add_ranges](const std::vector<SliceExtrusions> &slices)
    {
        for (const SliceExtrusions &slice : slices)
        {
            for (const IslandExtrusions &island : slice.common_extrusions)
            {
                if (island.infill_first)
                    add_ranges(island.infill_ranges);
                for (const Perimeter &perimeter : island.perimeters)
                    add(perimeter.smooth_path);
                if (!island.infill_first)
                    add_ranges(island.infill_ranges);
            }
            add_ranges(slice.ironing_extrusions);
        }
    };
    for (const ExtruderExtrusions &extruder : extrusions)
    {
        for (const auto &skirt : extruder.skirt)
            add(skirt.second);
        for (const BrimPath &brim : extruder.brim)
            add(brim.path);
        for (const OverridenExtrusions &overriden : extruder.overriden_extrusions)
            add_slices(overriden.slices_extrusions);
        for (const NormalExtrusions &normal : extruder.normal_extrusions)
        {
            for (const SupportPath &support : normal.support_extrusions)
                add(support.path);
            add_slices(normal.slices_extrusions);
        }
    }
    dbg_log(DBG_PERIMETERS, print_z, "DETERM",
            "GCODE_ORDER_FP paths=%zu pts=%zu len=%.3fmm roles=%s hash=%016llx ord=%016llx", paths, fp.points,
    ...
    const size_t single_object_instance_idx)
{
    ...
    if (debug_enabled(DBG_PERIMETERS))
        dbg_gcode_order_fp(print_z, extrusions);
    ...
    // add tag for processor
    // Include the layer number in the LAYER_CHANGE comment. m_layer_index starts at -1 and is
    // incremented in change_layer() below, so +2 gives the 1-based layer number.
    ...
    // export layer height
    // Round to 4 decimal places to clean up floating point artifacts (0.333302 -> 0.3333)
    ...
    if (!instances_to_print.empty() && instances_to_print.front().print_object.config().avoid_crossing_perimeters)
    {
    ...
    // The obstacle map holds every object on the layer as soon as one object routes around
    // curled overhangs: a travel of that object may cross any of them.
    if (std::any_of(layers.begin(), layers.end(),
                    [](const ObjectLayerToPrint &layer_to_print)
                    {
                        return layer_to_print.object() != nullptr &&
                               layer_to_print.object()->config().avoid_crossing_curled_overhangs;
                    }))
    {
    ...
    if (has_custom_gcode_to_emit && extruder_id_for_custom_gcode == -1)
    {
        ...
        {
            // The color change for this layer is emitted immediately before the first extrusion rather than here.
    ...
    for (ExtruderExtrusions &extruder_extrusions : extrusions)
    {
        ...
        if (!extruder_extrusions.skirt.empty())
        {
            ...
            for (const auto &[_, smooth_path] : extruder_extrusions.skirt)
            {
                ...
                // The skirt is shared by all objects, so it prints at the first object's
                                         ...
                                         const ObjectLayerToPrint &layer_to_print, const bool is_first)
{
    ...
    if (print_object.config().avoid_crossing_perimeters && !is_first)
    {
...
void GCodeGenerator::encode_full_config(const Print &print, std::vector<std::pair<std::string, std::string>> &config)
{
    ...
    static constexpr auto banned_keys = {"compatible_printers"sv, "compatible_prints"sv,
                                         // Print host credentials must not land in the exported G-code.
                                                ...
                                                const std::size_t wipe_offset)
{
    ...
    if (!is_loop && wipe_offset > 0)
    {
        throw std::runtime_error("Wipe offset is not supported for non looped paths!");
    }

    // set_path drops the path when wiping is off; the copies are only made when it is on.
    if (!m_wipe.enabled())
        m_wipe.reset_path();
    else if (is_loop)
    {
...
// Nip/Tuck seams: offsets external perimeter path inward at the seam
...
// Direction of travel at one end of the seam: leaving the seam for the path start, arriving at
// the seam for the path end. Taken as the chord from the seam over at least min_len_scaled of
// path, walked across smooth-path elements, so a sub-width stub segment at the seam (the seam
// placed a few microns before a vertex) cannot set it. Zero when the path has no extent.
static Vec2d seam_end_direction(const GCode::SmoothPath &smooth_path, bool from_start, double min_len_scaled)
{
    bool have_origin = false;
    Point origin, last, reach;
    double acc = 0;
    auto visit = [&](const Point &p) -> bool
    {
        if (!have_origin)
        {
            origin = last = reach = p;
        ...
        acc += (p - last).cast<double>().norm();
        last = reach = p;
        return acc < min_len_scaled;
    };
    bool go = true;
    if (from_start)
    {
        for (auto it = smooth_path.begin(); go && it != smooth_path.end(); ++it)
            for (auto sit = it->path.begin(); go && sit != it->path.end(); ++sit)
                go = visit(sit->point);
    }
    else
    {
        for (auto it = smooth_path.rbegin(); go && it != smooth_path.rend(); ++it)
            for (auto sit = it->path.rbegin(); go && sit != it->path.rend(); ++sit)
                go = visit(sit->point);
    }
    if (!have_origin)
        return Vec2d::Zero();
    if (acc < min_len_scaled)
        DBG_COUNT("SEAM_NOTCH_DIR_SHORT");
    Vec2d dir = (reach - origin).cast<double>();
    if (!from_start)
        dir = -dir;
...
static Vec2d compute_seam_inward_direction(const GCode::SmoothPath &smooth_path, bool reversed, double min_len_scaled)
{
    // Travel directions leaving and arriving at the seam, each a chord over one extrusion width
    Vec2d dir_start = seam_end_direction(smooth_path, true, min_len_scaled);
    Vec2d dir_end = seam_end_direction(smooth_path, false, min_len_scaled);
    ...
    {
        // Directions are nearly parallel: use the perpendicular to the path direction
    ...
    // LEFT of dir_start = rotate 90 deg CCW = (-dir_start.y, dir_start.x)
...
// One V leg of the notch: a linear inward ramp from full depth at the seam to zero at
// leg_len_scaled along the path, walked from the path start or from the path end. The walk
// crosses smooth-path elements, so a loop split by width or overhang changes near the seam
// still gets the whole leg; the junction point two elements share is offset in both copies,
// which keeps the path continuous.
static void apply_notch_leg(GCode::SmoothPath &smooth_path, bool from_start, double leg_len_scaled, double depth_scaled,
                            const Vec2d &inward)
{
    if (smooth_path.empty() || leg_len_scaled <= 0)
        return;
    const double max_seg_len = leg_len_scaled * 0.15; // fine steps along the ramp
    // Points up to this arc distance belong to the leg. Coordinates are integers, so the arc
    // length to an inserted leg-end point may differ from leg_len_scaled by a unit or two.
    double leg_reach = leg_len_scaled + 4.0;

    // Points of the path in walk order from the seam, with their arc distance from it,
    // up to the first point past `until`.
    struct Stop
    {
        size_t elem;
        size_t idx;
    ...
    auto collect = [&](double until)
    {
        std::vector<Stop> stops;
        double acc = 0;
        const Point *last = nullptr;
        auto push = [&](size_t e, size_t i)
        {
            const Point &p = smooth_path[e].path[i].point;
            if (last != nullptr)
                acc += (p - *last).cast<double>().norm();
            last = &p;
            stops.push_back({e, i, acc});
            return acc <= until;
        };
        bool go = true;
        if (from_start)
        {
            for (size_t e = 0; go && e < smooth_path.size(); ++e)
                for (size_t i = 0; go && i < smooth_path[e].path.size(); ++i)
                    go = push(e, i);
        }
        else
        {
            for (size_t e = smooth_path.size(); go && e-- > 0;)
                for (size_t i = smooth_path[e].path.size(); go && i-- > 0;)
                    go = push(e, i);
        }
        return stops;
    };

    // Split the segment the leg end falls in, so the ramp ends exactly there.
    {
        std::vector<Stop> stops = collect(leg_len_scaled);
        if (stops.size() < 2)
            return;
        const Stop &beyond = stops.back();
        if (beyond.dist > leg_len_scaled)
        {
            const Stop &before = stops[stops.size() - 2];
            // A junction step has zero length, so the crossing lies inside one element.
            if (before.elem == beyond.elem)
            {
                auto &path = smooth_path[beyond.elem].path;
                const size_t hi = std::max(before.idx, beyond.idx);
                const double frac = (leg_len_scaled - before.dist) / (beyond.dist - before.dist);
                if (frac > 0.001 && frac < 0.999)
                {
                    Geometry::ArcWelder::Segment s;
                    s.point = path[before.idx].point +
                              ((path[beyond.idx].point - path[before.idx].point).cast<double>() * frac).cast<coord_t>();
                    s.radius = 0;
                    // The split segment's attributes live on its end point in path order
                    s.e_fraction = path[hi].e_fraction;
                    s.height_fraction = path[hi].height_fraction;
                    path.insert(path.begin() + hi, s);
                }
                else if (frac >= 0.999)
                    leg_reach = beyond.dist + 1.0; // the existing point is the leg end
            }
            else
                DBG_COUNT("SEAM_NOTCH_LEG_JUNCTION_GAP");
        }
        else
            DBG_COUNT("SEAM_NOTCH_LEG_SHORT"); // the path ran out before the leg length
    }

    // Straighten the arcs inside the leg and subdivide it, element by element.
    {
        std::vector<Stop> stops = collect(leg_reach);
        size_t k = stops.size() - 1;
        if (stops[k].dist > leg_reach)
            --k; // the first point past the leg end
        if (k == 0)
        {
            DBG_COUNT("SEAM_NOTCH_LEG_EMPTY"); // no point of the path lies within the leg
            return;
        }
        Stop leg_end = stops[k];
        if (leg_end.elem != stops.front().elem)
            DBG_COUNT("SEAM_NOTCH_LEG_ACROSS_ELEMENTS");
        // Segments inside the leg end (in path order) at stops[1..k] walking forward and at
        // stops[0..k-1] walking backward; a segment's radius is stored on its end point.
        const size_t lin_from = from_start ? 1 : 0;
        const size_t lin_to = from_start ? k : k - 1;
        for (size_t j = lin_from; j <= lin_to && j < stops.size(); ++j)
            if (stops[j].idx >= 1)
                smooth_path[stops[j].elem].path[stops[j].idx].radius = 0;
        if (from_start)
        {
            for (size_t e = stops.front().elem; e <= leg_end.elem; ++e)
            {
                auto &path = smooth_path[e].path;
                const size_t end_idx = (e == leg_end.elem) ? leg_end.idx + 1 : path.size();
                const size_t inserted = subdivide_smooth_path_region(path, 1, end_idx, max_seg_len);
                if (e == leg_end.elem)
                    leg_end.idx += inserted;
            }
        }
        else
        {
            for (size_t e = leg_end.elem; e <= stops.front().elem; ++e)
            {
                auto &path = smooth_path[e].path;
                const size_t start_idx = (e == leg_end.elem) ? leg_end.idx + 1 : 1;
                subdivide_smooth_path_region(path, start_idx, path.size(), max_seg_len);
            }
        }
    }

    // Offset every point of the leg by its distance from the seam. Distances come from the
    // unmodified points, and both copies of a junction point get the same offset.
    {
        const std::vector<Stop> stops = collect(leg_reach);
        for (const Stop &s : stops)
        {
            if (s.dist > leg_reach)
                break;
            const double t = std::min(1.0, s.dist / leg_len_scaled);
            const double offset = depth_scaled * (1.0 - t);
            smooth_path[s.elem].path[s.idx].point += (inward * offset).cast<coord_t>();
        }
    }
}

// Apply V-notch offset to the start and end of an external perimeter's SmoothPath.
// half_width_scaled: the length of one V leg along the path in scaled coordinates
                                    ...
                                    const Vec2d &inward, SeamNotchType notch_type)
{
    ...
    // Tuck cuts only the end inward, Nip only the start
    if (notch_type != sntTuck)
        apply_notch_leg(smooth_path, true, half_width_scaled, depth_scaled, inward);
    if (notch_type != sntNip)
        apply_notch_leg(smooth_path, false, half_width_scaled, depth_scaled, inward);
}
                                  ...
                                  int layer_index)
{
    ...
    // The end directions at the seam are chords over at least a tenth of the width: long enough
    // to step over the micron-scale jog the seam projection can leave at a vertex, short enough
    // to keep reading the first real segment on either side.
    const double dir_min_len = scale_(0.1 * ext_width);

    // Check corner sharpness at the seam: sharp corners naturally hide seams. The travel
    // directions arriving at and leaving the seam are the same ones
    // compute_seam_inward_direction() uses. Their dot product is cos(deviation_from_straight);
    // skip the notch when the corner is sharper than the configured threshold.
    const double corner_threshold_deg = config.seam_notch_angle.value;
    if (corner_threshold_deg > 0)
    {
        const Vec2d dir_start = seam_end_direction(ext_perim.smooth_path, true, dir_min_len);
        const Vec2d dir_end = seam_end_direction(ext_perim.smooth_path, false, dir_min_len);
        if (dir_start.squaredNorm() > 1e-10 && dir_end.squaredNorm() > 1e-10)
        {
    ...
    // Notch width is a multiple of external perimeter extrusion width and spans the whole V
    // across both sides of the seam, so one leg is half of it (Nip or Tuck cut one leg only)
    const double notch_width_mm = config.seam_notch_width.value * ext_width;
    const double leg_mm = notch_width_mm / 2.0;
    const double half_width_scaled = scale_(leg_mm);

    // Check minimum loop length, do not notch tiny features
    ...
    Vec2d inward = compute_seam_inward_direction(ext_perim.smooth_path, ext_perim.reversed, dir_min_len);
    if (inward.squaredNorm() < 0.5) // degenerate, no direction to cut along
    {
        DBG_COUNT("SEAM_NOTCH_DIR_DEGENERATE");
        return;
    }
    ...
    if (inner_perim != nullptr)
    {
        ...
        // Intersection: depth * (1 - x/hw) = spacing gives x = hw * (1 - spacing/depth)
        const double x_int = (notch_depth > spacing && leg_mm > 0) ? leg_mm * (1.0 - spacing / notch_depth) : 0.0;
...
{
    // One V leg is half the configured notch width
    const double leg_mm = config.seam_notch_width.value * ext_width / 2.0;
    const double notch_depth = ext_width * 0.9;
                                               ...
                                               const InstanceToPrint &print_instance)
{
    ...
    // Nip/tuck needs >=2 regular walls so the external has an adjacent inner to notch. Two configs
    ...
    // Apply the seam notch if a non-Regular seam type is selected, there are multiple perimeters, not in
    ...
    for (const GCode::ExtrusionOrder::Perimeter &perimeter : perimeters)
    {
        ...
        // A loop no longer than the circumference of the configured diameter is a small perimeter.
        const double small_perimeter_length = scale_(m_config.small_perimeter_diameter.value) * PI;
...
GCodeGenerator::GCodeOutputStream::~GCodeOutputStream()
{
                                                     ...
                                                     const std::function<std::string()> &insert_gcode)
{
    ...
    {
        // The position is unknown after start or tool-change G-code. With first_travel_combine_z the
        // Z-first lift, the XY travel and the Z drop become one XYZ move, and the firmware interpolates
        // from wherever the custom G-code left the nozzle.
        const bool combine_z = m_config.first_travel_combine_z.value && !this->last_position;
        ...
        if (combine_z)
        {
            DBG_COUNT("FIRST_TRAVEL_COMBINED_Z");
            gcode += this->writer().travel_to_xyz_force(gcode_point, comment);
        }
        else
        {
                                     ...
                                     const EmitModifiers &emit_modifiers)
{
    ...
    if (path_attr.role == ExtrusionRole::InterlockingPerimeter && m_layer != nullptr && !path.empty())
    {
        ...
        if (!segment.points.empty() && segment.points.size() >= 2)
        {
            ...
            // Flow logic:
            //   - Over interlocking below, or over a void with no feature: full over-extrusion flow
            ...
            if (index_below)
            {
                ...
                {
                    if (!has_zone_below || !index_below->in_interlocking_zone(pt))
                        return false;
                    if (!has_zone_above || !index_above->in_interlocking_zone(pt))
                        return false;
                    return true;
                };
    ...
    // Breaks on first bridge hit per segment, so the remaining sample points are not checked.
            ...
            if (index_below.has_bridge_zone())
            {
                ...
                for (size_t i = 0; i < path.size(); ++i)
                {
                    ...
                    if (seg_len < scale_(8.0))
                    {
                        ...
                        segment_over_bridge[i] = index_below.in_bridge_zone(current_point) ||
                                                 index_below.in_bridge_zone(mid) ||
                                                 index_below.in_bridge_zone(end_point);
                    }
                    else
                    {
                        ...
                        for (int s = 0; s <= num_steps; ++s)
                        {
                            ...
                            if (index_below.in_bridge_zone(pt))
                            {
    ...
    {
        // First layer speeds are ceilings: a feature already configured slower keeps its own speed.
        // Solid infill (bottom and top) answers to first_layer_infill_speed when that is set,
        // otherwise to first_layer_speed like every other feature. Zero disables the ceiling.
        const double first_layer_infill_speed{m_config.get_abs_value("first_layer_infill_speed", speed)};
        const double first_layer_speed{m_config.get_abs_value("first_layer_speed", speed)};
        const bool solid = path_attr.role == ExtrusionRole::SolidInfill ||
                           path_attr.role == ExtrusionRole::TopSolidInfill;
        if (solid && first_layer_infill_speed > 0)
            speed = std::min(speed, first_layer_infill_speed);
        else if (first_layer_speed > 0)
            speed = std::min(speed, first_layer_speed);
    }
    else if (this->object_layer_over_raft())
    {
        const double over_raft_speed{m_config.get_abs_value("first_layer_speed_over_raft", speed)};
        if (over_raft_speed > 0)
            speed = std::min(speed, over_raft_speed);
    }
    ...
    // Pre-scan to check whether any segment will produce actual movement.
        ...
        if (p != prev)
        {
            ...
            if (!segment_flow_multipliers.empty() && dest_index < segment_flow_multipliers.size())
            {
                    ...
                    !has_crossings_here)
                {
                    ...
                    // When over-extruding, reduce speed proportionally to maintain mm^3/s
                    if (path_attr.role == ExtrusionRole::InterlockingPerimeter &&
                        dest_index < segment_flow_multipliers.size())
                    {
                                                  ...
                                                  const std::function<bool()> &use_short_distance_acceleration)
{
    ...
    {
        // These flavors carry one acceleration value for print and travel, so re-assert the
        // travel value before the next move.
        gcode += this->m_writer.set_travel_acceleration(travel_acceleration);
    }
            ...
            for (const BoundingBox &bbox : support_layer->support_islands_bboxes)
                if (bbox.overlap(bbox_travel))
                {
                    ...
                    if (trimmed.empty())
                        ...
                        // Travel that stays inside a support island prints no retraction at any
                        // length, so a wide island can ooze into the next extrusion.
                        return false;
