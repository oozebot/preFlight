...
unsigned int LayerTools::extruder(const ExtrusionEntityCollection &extrusions, const PrintRegion &region) const
{
    ...
    assert(region.config().infill_extruder.value > 0);
    assert(region.config().solid_infill_extruder.value > 0);
    assert(region.config().interlocking_perimeter_extruder.value > 0);
    // 1 based extruder ID.
    unsigned int extruder = this->extruder_override == 0
                                ? (extrusions.role().is_infill()
                                       ? (extrusions.entities.front()->role().is_solid_infill()
                                              ? region.config().solid_infill_extruder
                                              : region.config().infill_extruder)
                                       :
                                       // Check if this is an interlocking perimeter (has Interlocking modifier)
                                       // If so, use interlocking_perimeter_extruder instead of perimeter_extruder
                                       (extrusions.role() == ExtrusionRole::InterlockingPerimeter
                                            ? region.config().interlocking_perimeter_extruder.value
...
static double calc_max_layer_height(const PrintConfig &config, double max_object_layer_height)
{
    ...
    // The Fast (0.35mm layer height) print profile sets a higher layer height than what is normally allowed
    // by the nozzle. This works by increasing the extrusion width.
...
ToolOrdering::ToolOrdering(const PrintObject &object, unsigned int first_extruder, bool prime_multi_material)
{
    ...
    this->fill_wipe_tower_partitions(object.print()->config(),
...
ToolOrdering::ToolOrdering(const Print &print, unsigned int first_extruder, bool prime_multi_material)
{
    ...
    if (num_extruders > 1 &&
        print.object_extruders().size() == 1 && // the current Print's configuration is CustomGCode::MultiAsSingle
    ...
    if (num_extruders > 1 && print.model().custom_gcode_per_print_z().mode == CustomGCode::MultiExtruder &&
...
[[nodiscard]] static bool is_overriddable(const ExtrusionEntityCollection &eec, const LayerTools &lt,
                                          const PrintConfig &print_config, const PrintObject &object,
                                     ...
                                     const std::vector<std::pair<double, unsigned int>> &per_layer_color_changes)
{
    ...
    std::vector<std::pair<double, unsigned int>>::const_iterator it_per_layer_extruder_override =
        per_layer_extruder_switches.begin();
    unsigned int extruder_override = 0;

    std::vector<std::pair<double, unsigned int>>::const_iterator it_per_layer_color_changes =
        per_layer_color_changes.begin();
    ...
    for (auto layer : object.layers())
    {
        ...
        for (; it_per_layer_color_changes != per_layer_color_changes.end() &&
               it_per_layer_color_changes->first < layer->print_z + EPSILON;
             ++it_per_layer_color_changes)
        {
            if (std::abs(it_per_layer_color_changes->first - layer->print_z) < EPSILON)
            {
                assert(layer_tools.extruder_needed_for_color_changer ==
                       0); // Just on color change per layer is allowed.
        ...
        for (const LayerRegion *layerm : layer->regions())
        {
            ...
            if (!layerm->perimeters().empty())
            {
                ...
                { // in this case complete_objects is false (see ToolOrdering constructors)
                    something_nonoverriddable = false;
                    for (const ExtrusionEntity *eec :
                         layerm->perimeters()) // let's check if there are nonoverriddable entities
                        if (is_overriddable(dynamic_cast<const ExtrusionEntityCollection &>(*eec), layer_tools,
                ...
                if (something_nonoverriddable)
                    layer_tools.extruders.emplace_back(extruder_override == 0 ? region.config().perimeter_extruder.value
                                                                              : extruder_override);
        ...
        else
        {
            ...
            if (lt == m_layer_tools[0] && m_print_config_ptr && m_print_config_ptr->wipe_tower)
            {
                for (size_t i = 0; i < lt.extruders.size(); ++i)
                    if (m_print_config_ptr->filament_soluble.get_at(lt.extruders[i] - 1))
                    { // 1-based...
                                              ...
                                              coordf_t max_layer_height)
{
    ...
    // A layer needs the wipe tower when it prints an object and changes tools, or while it is
    // still below the object bottom (raft and priming).
    for (LayerTools &lt : m_layer_tools)
        ...
        if (lt.print_z < object_bottom_z + EPSILON && lt_next.print_z >= object_bottom_z + EPSILON)
        {
            ...
            if (j < m_layer_tools.size())
            {
                ...
                if (gap > max_layer_height + EPSILON)
                {
                    ...
                    for (j = i + 1; j < m_layer_tools.size() && m_layer_tools[j].print_z < lt_new.print_z - EPSILON;
                         ++j)
                        ;
    ...
    // If the model contains empty layers, there might be layers
    ...
    for (unsigned int i = 0; i + 1 < m_layer_tools.size(); ++i)
    {
        ...
        if (lt.extruders.empty() || lt_next.extruders.empty())
            break;
        if (!lt_next.has_wipe_tower &&
            (lt_next.extruders.front() != lt.extruders.back() || lt_next.extruders.size() > 1))
...
void ToolOrdering::collect_extruder_statistics(bool prime_multi_material)
{
    ...
    if (prime_multi_material && !m_all_printing_extruders.empty())
    {
        ...
        m_all_printing_extruders.erase(std::remove_if(m_all_printing_extruders.begin(), m_all_printing_extruders.end(),
                                                      [this](const unsigned int eid)
                                                      { return eid == m_first_printing_extruder; }),
...
void ToolOrdering::assign_custom_gcodes(const Print &print)
{
    ...
    auto num_extruders = unsigned(print.config().nozzle_diameter.size());
    CustomGCode::Mode mode = (num_extruders == 1)                   ? CustomGCode::SingleExtruder
    ...
    // or vice versa.
    bool ignore_tool_and_color_changes = (mode == CustomGCode::MultiExtruder) !=
                                         (model_mode == CustomGCode::MultiExtruder);
    // If printing on a single extruder machine, make the tool changes trigger color change (M600) events.
    bool tool_changes_as_color_changes = mode == CustomGCode::SingleExtruder &&
                                         model_mode == CustomGCode::MultiAsSingle;
    ...
    for (auto it_lt = m_layer_tools.rbegin(); it_lt != m_layer_tools.rend(); ++it_lt)
    {
        ...
        for (; custom_gcode_it != custom_gcode_per_print_z.gcodes.rend() &&
               (custom_gcode_it->print_z > lt.print_z + EPSILON || custom_gcode_it->type == CustomGCode::ToolChange);
             ++custom_gcode_it)
            ;
...
void WipingExtrusions::set_extruder_override(const ExtrusionEntity *entity, size_t copy_id, int extruder,
...
[[nodiscard]] static int first_nonsoluble_extruder_on_layer(const PrintConfig &print_config,
...
[[nodiscard]] static int last_nonsoluble_extruder_on_layer(const PrintConfig &print_config,
                                                           const LayerTools &layer_tools)
{
    for (auto extruders_it = layer_tools.extruders.rbegin(); extruders_it != layer_tools.extruders.rend();
         ++extruders_it)
        if (!print_config.filament_soluble.get_at(*extruders_it))
            return (*extruders_it);
...
float WipingExtrusions::mark_wiping_extrusions(const Print &print, const LayerTools &lt, unsigned int old_extruder,
                                               unsigned int new_extruder, float volume_to_wipe)
{
    ...
    std::sort(object_list.begin(), object_list.end(), [](const PrintObject *a, const PrintObject *b)
              { return a->config().wipe_into_objects && !b->config().wipe_into_objects; });
    ...
    {
        if (!perimeters_done && (i == (int) object_list.size() || !object_list[i]->config().wipe_into_objects))
        { // we passed the last dedicated object in list
        ...
        for (unsigned int copy = 0; copy < num_of_copies; ++copy)
        {
            for (const LayerRegion *layerm : this_layer->regions())
            {
                ...
                if (object->config().infill_first != perimeters_done || wipe_into_infill_only)
                {
                    for (const ExtrusionEntity *ee : layerm->fills())
                    { // iterate through all infill Collections
                        ...
                        if (wipe_into_infill_only && !object->config().infill_first)
                ...
                if (object->config().wipe_into_objects && object->config().infill_first == perimeters_done)
                {
        ...
        { // iterate through copies first, so that we mark neighbouring infills to minimize travel moves
            for (const LayerRegion *layerm : this_layer->regions())
            {
                ...
                { // iterate through all infill Collections
                    ...
                    if (object->config().infill_first ||
                        object->config()
                            .wipe_into_objects // in this case the perimeter is overridden, so we can override by the last one safely
                        ||
                        lt.is_extruder_order(
                            lt.perimeter_extruder(region),
                            last_nonsoluble_extruder) // !infill_first, but perimeter is already printed when last extruder prints
                        || !lt.has_extruder(lt.infill_extruder(
                               region))) // forced override: with infill_first the infill can end up after its perimeter
                        set_extruder_override(fill, copy,
                                              (object->config().infill_first ? first_nonsoluble_extruder
                                                                             : last_nonsoluble_extruder),
                                              num_of_copies);
                    else
                    {
                ...
                { // iterate through all perimeter Collections
                    ...
                    if (is_overriddable(*fill, lt, print.config(), *object, region) &&
                        !is_entity_overridden(fill, copy))
                        set_extruder_override(fill, copy,
                                              (object->config().infill_first ? last_nonsoluble_extruder
                                                                             : first_nonsoluble_extruder),
                                              num_of_copies);
                }
            }
        }
...
int ToolOrdering::toolchanges_count() const
{
    ...
    tools_in_order.erase(std::remove(tools_in_order.begin(), tools_in_order.end(), (unsigned int) (-1)),
                         tools_in_order.end());
