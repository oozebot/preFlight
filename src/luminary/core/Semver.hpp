...
public:
    ...
    Semver(int major, int minor, int patch, boost::optional<const std::string &> metadata,
    ...
    Semver(const std::string &str) : ver(semver_zero())
    {
        ...
        if (!parsed)
        {
            throw Luminary::RuntimeError(std::string("Could not parse version string: ") + str);
        }
        ver = parsed->ver;
    ...
    {
        static semver_t ver = {std::numeric_limits<int>::max(), std::numeric_limits<int>::max(),
