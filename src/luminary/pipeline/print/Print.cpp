...
#include "luminary/pipeline/brim/Brim.hpp"
#include "luminary/geometry/clipper/ClipperUtils.hpp"
#include "luminary/gcode/extruder/Extruder.hpp"
...
#include "luminary/gcode/emit/GCode.hpp"
#include "luminary/layer/wipe_tower/WipeTower.hpp"
#include "luminary/pipeline/validation/ConflictChecker.hpp"
...
// The conditional row of the print-level invalidation table.
enum class PrintInvalidationRule : uint8_t
{
    None,
    // "filament_colour", "filament_transmission_distance": the colour-mixing segmentation bakes
    // the filament palette into the slices, so a plate carrying colour-mixing paint re-slices.
    ColorMixingPainted,
};

struct PrintKeyInvalidation
{
    std::vector<PrintStep> print_steps;
    std::vector<PrintObjectStep> object_steps;
    PrintInvalidationRule rule{PrintInvalidationRule::None};
};

// One row per print, filament or printer key. A key with no row invalidates every print step
// (not the object steps); a key in two rows is a mistake, asserted at build time.
const std::unordered_map<std::string, PrintKeyInvalidation> &print_key_invalidation_table()
{
    static const std::unordered_map<std::string, PrintKeyInvalidation> table = []()
    {
        std::unordered_map<std::string, PrintKeyInvalidation> t;
        auto add = [&t](std::initializer_list<const char *> keys, std::vector<PrintStep> print_steps,
                        std::vector<PrintObjectStep> object_steps = {},
                        PrintInvalidationRule rule = PrintInvalidationRule::None)
        {
            for (const char *key : keys)
            {
                ...
                t[key] = PrintKeyInvalidation{print_steps, object_steps, rule};
            }
        };
        // Keys that influence the G-code generator only, or are notes with no influence at all.
        add({"autoemit_temperature_commands", "bed_shape", "bed_temperature", "before_layer_gcode",
             "between_objects_gcode", "binary_gcode", "bridge_fan_speed", "overhang_fan_speed_0",
             "overhang_fan_speed_1", "overhang_fan_speed_2", "overhang_fan_speed_3", "chamber_temperature",
             "chamber_minimal_temperature", "colorprint_heights", "cooling", "cooling_slowdown_logic",
             "cooling_perimeter_transition_distance", "deretract_speed", "disable_fan_first_layers",
             "duplicate_distance", "end_gcode", "end_filament_gcode", "extrusion_axis", "extruder_clearance_height",
             "extruder_clearance_radius", "extruder_colour", "extruder_offset", "extrusion_multiplier", "fan_always_on",
             "fan_below_layer_time", "full_fan_speed_layer", "filament_abrasive", "filament_diameter",
             "filament_density", "filament_notes", "filament_cost", "filament_seam_gap_distance",
             "filament_spool_weight", "first_layer_acceleration_over_raft", "first_layer_bed_temperature",
             "first_layer_speed_over_raft", "first_travel_combine_z", "gcode_comments", "gcode_label_objects",
             "nozzle_high_flow", "layer_gcode", "min_fan_speed", "max_fan_speed", "max_print_height", "min_print_speed",
             "max_volumetric_extrusion_rate_slope_positive", "max_volumetric_extrusion_rate_slope_negative", "notes",
             "custom_parameters_printer", "output_filename_format", "post_process", "gcode_substitutions",
             "printer_notes", "travel_ramping_lift", "travel_initial_part_length", "travel_slope", "travel_max_lift",
             "travel_lift_before_obstacle", "retract_before_travel", "retract_before_wipe", "wipe_length",
             ...
             "retract_speed", "seam_type", "seam_notch_width", "seam_notch_angle",
             "single_extruder_multi_material_priming", "slowdown_below_layer_time", "standby_temperature_delta",
             "start_gcode", "start_filament_gcode", "custom_parameters_filament", "toolchange_gcode",
             "travel_acceleration", "travel_short_distance_acceleration", "thumbnails", "thumbnails_format",
             "use_firmware_retraction", "use_relative_e_distances", "use_volumetric_e", "variable_layer_height", "wipe",
             "wipe_tower_acceleration",
             // Editing-time check only; nothing generated depends on it.
             "nozzle_width_warning_min"},
            {psGCodeExport});
        add({"filament_colour", "filament_transmission_distance"}, {psGCodeExport}, {},
            PrintInvalidationRule::ColorMixingPainted);
        // The overhang split runs only while a printing extruder has dynamic fan speeds on and
        // it rewrites the perimeter extrusions in place, so the toggle regenerates the
        // perimeters like its region twin enable_dynamic_overhang_speeds does. The
        // generated-width check runs while perimeters and fills are produced.
        add({"enable_dynamic_fan_speeds", "automatic_extrusion_widths", "nozzle_width_warning_max"}, {},
            {posPerimeters});
        add({"skirts", "skirt_height", "draft_shield", "skirt_distance", "min_skirt_length", "ooze_prevention"},
            {psSkirtBrim});
        // Spiral Vase forces different kind of slicing than the normal model: holes are closed
        // and only the largest area contour is kept at each layer, so toggling it re-slices.
        add({"first_layer_height", "nozzle_diameter", "spiral_vase", "filament_shrinkage_compensation_x",
             "filament_shrinkage_compensation_y", "filament_shrinkage_compensation_z", "prefer_clockwise_movements"},
            {}, {posSlice});
        add({"complete_objects",
             "filament_type",
             ...
             "travel_speed_z",
             "z_offset"},
            {psWipeTower, psSkirtBrim});
        // Soluble support interface / non-soluble base interface produces non-soluble interface
        // layers below soluble interface layers, so the material switch may change the supports.
        add({"filament_soluble"}, {psWipeTower}, {posSupportMaterial});
        add({"min_layer_height", "max_layer_height"}, {psSkirtBrim}, {posPerimeters, posInfill, posSupportMaterial});
        // Read inside the export only: the manual fan speeds and the fan spin-up ramps by the
        // cooling buffer, the outer-wall slowdown exemption, pressure advance, the first-layer
        // speed ceilings, the bed-temperature extruder (its apply-time read is a warning), the
        // wipe extension, the cost, note and identity keys and the placeholder variables, the
        // preprocessing switches and scripts (run inside the export), and the machine limits
        // (emitted or fed to the time estimator at export). The export script runs on the
        // saved file and reads nothing here; its row keeps the config trailer current.
        add({"enable_manual_fan_speeds",
...
std::vector<std::string> print_key_invalidation_keys()
{
    std::vector<std::string> keys;
    for (const auto &[key, row] : print_key_invalidation_table())
        keys.push_back(key);
    return keys;
}

// The steps a changed print-level key invalidates according to the table; false when the key
// has no row.
bool Print::table_invalidation_steps_for_key(const t_config_option_key &opt_key, std::vector<PrintStep> &steps,
                                             std::vector<PrintObjectStep> &object_steps) const
{
    const auto &table = print_key_invalidation_table();
    auto it = table.find(opt_key);
    if (it == table.end())
        return false;
    const PrintKeyInvalidation &row = it->second;
    append(steps, row.print_steps);
    append(object_steps, row.object_steps);
    if (row.rule == PrintInvalidationRule::ColorMixingPainted &&
        std::any_of(m_objects.begin(), m_objects.end(),
                    [](const PrintObject *object)
                    {
                        const ModelObject *mo = object->model_object();
                        return mo != nullptr &&
                               std::any_of(mo->volumes.begin(), mo->volumes.end(),
                                           [](const ModelVolume *mv) { return mv->is_color_mixing_painted(); });
                    }))
        object_steps.emplace_back(posSlice);
    return true;
}
                                               ...
                                               const std::vector<t_config_option_key> &opt_keys)
{
    ...
    for (const t_config_option_key &opt_key : opt_keys)
        if (!this->table_invalidation_steps_for_key(opt_key, steps, osteps))
        {
            // A key without a row re-runs every print step; counted so an unmapped key shows up.
            DBG_COUNT("PRINT_KEY_CATCH_ALL");
            invalidate_all = true;
        }
...
std::string Print::validate(std::vector<std::string> *warnings) const
{
    ...
    if (warnings)
    {
        // The draft shield is the skirt raised to the object's height; without a skirt loop it
        // is silently absent.
        if (m_config.draft_shield != dsDisabled && m_config.skirts.value == 0)
            warnings->push_back(_u8L("Draft shield is enabled but Skirt loops is 0, so no draft shield is printed."));
    ...
    {
        // Check if any speed triggers auto-calculation (auto_speed enabled on any object, or any
        // individual speed set to 0)
        bool needs_volumetric = std::any_of(m_objects.begin(), m_objects.end(), [](const PrintObject *object)
                                            { return object->config().auto_speed.value; });
        if (!needs_volumetric)
        {
            for (const PrintObject *object : m_objects)
            {
    ...
    for (const PrintObject *object : m_objects)
        if (object->config().avoid_crossing_perimeters && object->config().avoid_crossing_curled_overhangs)
        {
            return _u8L("Avoid crossing perimeters option and avoid crossing curled overhangs option cannot be both "
                        "enabled together.");
        }
    ...
    if (m_config.spiral_vase)
    {
        ...
        // The spiral keeps its Z ramp with absolute extruder addressing, but the entry and exit
        // tapers and the XY blend into the previous layer are skipped.
        if (warnings && !m_config.use_relative_e_distances.value)
            warnings->push_back(
                _u8L("With absolute extruder addressing, Spiral Vase mode does not taper its first and last layer and "
                     "does not blend layers in XY. Enable \"Use relative E distances\" in Printer Settings to get "
                     "both, if your firmware supports it."));
    }
    ...
    {
        const PrintObject &print_object = *m_objects[print_object_idx];
        // The layer stack has to be generated to know the final print height.
    ...
    for (size_t print_object_idx = 0; print_object_idx < m_objects.size(); ++print_object_idx)
    {
        ...
        if (style != smsOrganic && style != smsBaobab && !print_object.has_organic_enforcers() &&
...

    {
                                                              ...
                                                              std::string &err_msg) -> bool
        {
            ...
            else if (extrusion_width_max > max_nozzle_diameter * 3. + EPSILON)
            {
                // A width at exactly three nozzle diameters is printable; only a wider one is refused.
        ...
        for (PrintObject *object : m_objects)
        {
            if (object->has_support_material())
            {
                ...
                {
                    // Sparse support layers follow the object layer heights and the wipe tower takes every
                    // support z from the tool ordering, so only the non-soluble case is constrained.
            ...
            for (const char *opt_key :
                ...
                for (const PrintRegion &region : object->all_regions())
                    if (!validate_extrusion_width(region.config(), opt_key, layer_height, pct_of_nozzle, err_msg))
                        return err_msg;
            // The Serpentine bead is a configured width like the others; the value only matters
            // where the feature is on.
            for (const PrintRegion &region : object->all_regions())
                if (region.config().serpentine_enabled &&
                    !validate_extrusion_width(region.config(), "serpentine_extrusion_width", layer_height,
                                              pct_of_nozzle, err_msg))
                    return err_msg;
        }
    }
...
{
    // The skirt and brim are plate features: they take the project's first-layer width.
    ConfigOptionFloatOrPercent width = m_default_object_config.first_layer_extrusion_width;
    if (width.value == 0)
...
void Print::auto_assign_extruders(ModelObject *model_object) const
{
    ...
    for (size_t volume_id = 0; volume_id < model_object->volumes.size(); ++volume_id)
    {
        ...
        // A modifier with a material assigned gets an extruder id here as well.
...
void Print::process()
{
    ...
    // Debug counters are per slice: a GUI re-slice starts from zero.
    DbgCounters::inst().reset();
    ...
    for (const PrintObject *obj : m_objects)
    {
        ...
        const bool alerts_off = !obj->config().support_alerts.value;
        if (alerts_off && !automatic)
            DBG_COUNT("SUPPORT_ALERTS_OFF");
        dbg_log(DBG_STABILITY, 0., "STAB", "GATE %s master=%d auto=%d enforce=%d alerts=%d -> %s",
                obj->model_object()->name.c_str(), int(obj->has_support()),
                int(obj->config().support_material_auto.value),
                int(obj->config().support_material_enforce_layers.value), int(!alerts_off),
                automatic ? "skip (automatic supports)" : (alerts_off ? "skip (alerts off)" : "analyze"));
        if (!automatic && !alerts_off)
    ...
    if (this->set_started(psWipeTower))
    {
        ...
        // The previous wipe tower's synthetic support layers would pass its duplicate guard again.
        for (PrintObject *object : m_objects)
            object->remove_synthetic_support_layers();
    ...
    if (this->set_started(psSkirtBrim))
    {
        ...
        const float accum = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                            ...
                            g_progress_config.slicing.skirt_brim;
        ...
        {
            // Run scoped: the skirt and brim as generated, before the export orders them.
            determ_fp_extrusions(DBG_PERIMETERS, 0., "BRIM_FP", "skirt", 0, m_skirt);
            determ_fp_extrusions(DBG_PERIMETERS, 0., "BRIM_FP", "brim", 0, m_brim);
        }
    ...
    // Machine-readable totals of every debug counter that fired during this slice.
    if (g_debug_mask != 0)
        DbgCounters::inst().dump("PROCESS");
}
...
void Print::_make_skirt()
{
    ...
    Polygon convex_hull = Luminary::Geometry::convex_hull(points);
...
void Print::alert_when_supports_needed()
{
    if (this->set_started(psAlertWhenSupportsNeeded))
    {
        ...
        const float accum = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                            ...
                            g_progress_config.slicing.perimeters + g_progress_config.slicing.prepare_infill +
                            g_progress_config.slicing.bridge_over_infill + g_progress_config.slicing.making_infill +
                            g_progress_config.slicing.supports_alert;
        const_cast<Print *>(this)->set_status(static_cast<int>((accum / total) * 33.0f),
        ...
        // Two alerts, split by remedy: geometry that supports would hold (floating extrusions
        // and bridges, floating parts) and what supports cannot fix (bed adhesion, fragile
        // parts). A bed-adhesion alert on a part standing inside a supported region then no
        // longer reads as a support problem.
        using Issue = std::pair<SupportSpotsGenerator::SupportPointCause, bool>;
        using ObjectIssues = std::vector<std::pair<const PrintObject *, std::vector<Issue>>>;
        auto is_support_cause = [](SupportSpotsGenerator::SupportPointCause cause)
        {
            return cause == SupportSpotsGenerator::SupportPointCause::LongBridge ||
                   cause == SupportSpotsGenerator::SupportPointCause::FloatingBridgeAnchor ||
                   cause == SupportSpotsGenerator::SupportPointCause::FloatingExtrusion ||
                   cause == SupportSpotsGenerator::SupportPointCause::UnstableFloatingPart;
        };
        auto emit_alert = [this, &issue_to_alert_message, &elements_to_translated_list, &single_line_list_rule,
                           &multiline_list_rule](const ObjectIssues &issues, const std::string &header,
                                                 const std::vector<std::string> &advice)
        {
            if (issues.empty())
                return;
            std::map<Issue, std::vector<const PrintObject *>> po_by_issue;
            for (const auto &obj : issues)
                for (const auto &issue : obj.second)
                    po_by_issue[issue].push_back(obj.first);

            std::vector<std::pair<std::string, std::vector<std::string>>> message_elements;
            if (issues.size() > po_by_issue.size())
            {
                // there are more objects than causes, group by issues
                for (const auto &issue : po_by_issue)
                {
            ...
            else
            {
                ...
                for (const auto &obj : issues)
                {
                    auto &pair = message_elements.emplace_back(obj.first->model_object()->name,
                                                               std::vector<std::string>{});
                    for (const auto &issue : obj.second)
            ...
            this->active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL,
                                          Luminary::format(header,
                                                           elements_to_translated_list(lines, multiline_list_rule)));
        };

        ObjectIssues support_issues, adhesion_issues;
        bool recommend_brim = false;
        bool weak_part = false;
        for (const auto &obj : objects_isssues)
        {
            std::vector<Issue> for_supports, for_adhesion;
            for (const auto &issue : obj.second)
            {
                if (is_support_cause(issue.first))
                {
                    for_supports.push_back(issue);
                    continue;
                }
                for_adhesion.push_back(issue);
                if (issue.first == SupportSpotsGenerator::SupportPointCause::SeparationFromBed &&
                    !obj.first->has_brim())
                    recommend_brim = true;
                if (issue.first == SupportSpotsGenerator::SupportPointCause::WeakObjectPart)
                    weak_part = true;
            }
            if (!for_supports.empty())
                support_issues.emplace_back(obj.first, std::move(for_supports));
            if (!for_adhesion.empty())
                adhesion_issues.emplace_back(obj.first, std::move(for_adhesion));
        }

        // TRN Alert header for geometry that needs supports. The argument is the list of detected issues.
        emit_alert(support_issues, _u8L("Detected unsupported geometry:\n%1%"), {_u8L("Consider enabling supports.")});
        std::vector<std::string> adhesion_advice;
        if (recommend_brim)
            // TRN Alert advice when a part's bed adhesion is insufficient.
            adhesion_advice.push_back(_u8L("Consider enabling a brim."));
        if (weak_part)
            // TRN Alert advice when a part is thin enough to break during printing.
            adhesion_advice.push_back(_u8L("Consider thickening the part or adding perimeters."));
        // TRN Alert header for issues supports cannot fix: bed adhesion and fragile parts. The argument is the list of detected issues.
        emit_alert(adhesion_issues, _u8L("Detected bed adhesion or part strength issues:\n%1%"), adhesion_advice);
...
void Print::_make_wipe_tower()
{
    ...
    // Fix for print validation issue
    {
        ...
        if (idx_begin != size_t(-1))
        {
            ...
            for (size_t i = idx_begin; i < idx_end; ++i)
            {
                ...
                // The layer carries only the wipe tower, so it gets no support layer id.
    ...
    WipeTower wipe_tower(model().wipe_tower().position.cast<float>(), model().wipe_tower().rotation, m_config,
                         m_default_object_config, m_default_region_config, wipe_volumes,
    ...
    m_wipe_tower_data.priming = Luminary::make_unique<std::vector<WipeTower::ToolChangeResult>>(
    ...
    m_wipe_tower_data.final_purge = Luminary::make_unique<WipeTower::ToolChangeResult>(
...
void MultipleBeds::ensure_wipe_towers_on_beds(Model &model, const std::vector<std::unique_ptr<Print>> &prints)
{
    for (size_t bed_idx = 0; bed_idx < get_number_of_beds(); ++bed_idx)
    {
        ...
        Polygon plg(Points{Point::new_scale(-brim, -brim), Point::new_scale(brim + width, -brim),
                           Point::new_scale(brim + width, brim + depth), Point::new_scale(-brim, brim + depth)});
        plg.rotate(Geometry::deg2rad(mwt.rotation));
        plg.translate(scaled(mwt.position));
        if (std::all_of(plg.points.begin(), plg.points.end(),
                        [this](const Point &pt) { return !m_build_volume_bb.contains(unscale(pt)); }))
