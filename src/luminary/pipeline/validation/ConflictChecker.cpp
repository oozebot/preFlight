...
static std::vector<ExtrusionPaths> getFakeExtrusionPathsFromWipeTower(const WipeTowerData &wtd)
{
    ...
    for (float hh = 0.f; hh < h; hh += lh)
    {
        ...
        ExtrusionPath path(
            {minCorner, {maxCorner.x(), minCorner.y()}, maxCorner, {minCorner.x(), maxCorner.y()}, minCorner},
        ...
        if (cone_base_R > 0.)
        {
            ...
            { // Cone brim.
                ...
                {
                    path.polyline.clear();
                    for (double alpha = 0; alpha < 2.01 * M_PI;
                         alpha += 2 * M_PI / 20.) // see load_wipe_tower_preview, where the same is a bit clearer
                        path.polyline.points.emplace_back(
                            Point::new_scale(width / 2. + cone_base_R * std::cos(alpha) / cone_scale_x *
                                                              (1. + cone_scale_x * bw / cone_base_R),
...
ExtrusionPaths getExtrusionPathsFromLayer(LayerRegionPtrs layerRegionPtrs)
{
    ...
    for (auto regionPtr : layerRegionPtrs)
    {
        ...
        if (!regionPtr->perimeters().empty())
        {
            getExtrusionPathsFromEntity(&regionPtr->fills(), paths);
        }
    }
    return paths;
}
...
std::pair<std::vector<ExtrusionPaths>, std::vector<ExtrusionPaths>> getAllLayersExtrusionPathsFromObject(
    const PrintObject *obj)
{
    ...
    {
        objPaths.push_back(getExtrusionPathsFromLayer(layerPtr->regions()));
    }
    ...
    const WipeTowerData &wipe_tower_data) // find the first intersection point of lines in different objects
{
    ...
    {
        std::pair<std::vector<ExtrusionPaths>, std::vector<ExtrusionPaths>> layers =
            getAllLayersExtrusionPathsFromObject(obj);
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, layersLines.size()),
    ...
    if (find)
    {
        std::sort(conflict.begin(), conflict.end(),
                  [](const std::pair<ConflictComputeResult, double> &i1,
                     const std::pair<ConflictComputeResult, double> &i2) { return i1.second < i2.second; });
        ...
        if (ptr1 == &wtptr || ptr2 == &wtptr)
        {
            ...
            return std::make_optional<ConflictResult>("WipeTower", obj2->model_object()->name, conflictHeight, nullptr,
                                                      ptr2);
        }
        const PrintObject *obj1 = reinterpret_cast<const PrintObject *>(ptr1);
        const PrintObject *obj2 = reinterpret_cast<const PrintObject *>(ptr2);
        return std::make_optional<ConflictResult>(obj1->model_object()->name, obj2->model_object()->name,
                                                  conflictHeight, ptr1, ptr2);
    }
    else
        return {};
}
...
{
    if (l1._obj_id == l2._obj_id && l1._inst_id == l2._inst_id)
    {
        return {};
    } // lines are from same instance
    ...
    if (intersect)
    {
        ...
        if (dist > 0.01)
        {
            return std::make_optional<ConflictComputeResult>(l1._obj_id, l2._obj_id);
