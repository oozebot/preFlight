...
#include "luminary/slice/elephant_foot/ElephantFootCompensation.hpp"
...
#include "luminary/colour/dither/ColorDithering.hpp"
#include "luminary/colour/mixing/ColorMixer.hpp"
#include "luminary/colour/mixing/FilamentOptics.hpp"
#include "luminary/colour/palette/MixedColorPalette.hpp"
...
#include "luminary/slice/interlocking/InterlockingGenerator.hpp"
    ...
    const std::vector<float> &zs, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    params_base.resolution = print_object_config.resolution.value;
...
void PrintObject::slice()
{
    ...
    {
        // Per-layer fingerprint of the slice result: two runs on the same input must print the
        // same line, or the drift starts at or before slicing.
        for (const Layer *layer : m_layers)
        {
            ...
            {
                area += ep.area();
                for (const Polygon *p : {&ep.contour})
            ...
            dbg_log(DBG_PERIMETERS, layer->print_z, "DETERM",
                    "SLICE_FP layer=%zu polys=%zu pts=%zu area=%.4fmm2 hash=%016llx", layer->id(),
                    layer->lslices.size(), pts, area * 1e-12, (unsigned long long) h);
        }
    }
...
void apply_mm_segmentation(PrintObject &print_object, ThrowOnCancel throw_on_cancel)
{
    ...
    // Run-to-run drift bracket (debug only): fingerprint of each producer's per-layer output.
    auto seg_fingerprint = [](const ExPolygons &expolys, size_t &pts, double &area)
    {
        uint64_t h = 1469598103934665603ull;
        pts = 0;
        area = 0.;
    ...
    auto seg_dump = [&](const char *tag, const std::vector<std::vector<ExPolygons>> &seg)
    {
        if (!debug_enabled(DBG_PERIMETERS))
            return;
        for (size_t layer_id = 0; layer_id < seg.size() && layer_id < print_object.layer_count(); ++layer_id)
            for (size_t bucket = 0; bucket < seg[layer_id].size(); ++bucket)
            {
                if (seg[layer_id][bucket].empty())
                    continue;
                size_t pts = 0;
                double area = 0.;
                const uint64_t h = seg_fingerprint(seg[layer_id][bucket], pts, area);
                dbg_log(DBG_PERIMETERS, print_object.get_layer(int(layer_id))->print_z, "DETERM",
                        "%s layer=%zu bucket=%zu polys=%zu pts=%zu area=%.4fmm2 hash=%016llx", tag, layer_id, bucket,
                        seg[layer_id][bucket].size(), pts, area * 1e-12, (unsigned long long) h);
            }
    };
    seg_dump("SEG_MM_FP", segmentation);
    ...
    // assignments into the main segmentation array.
    {
        ...
        if (has_color_mixing)
        {
            ...
            {
                // Build compact remap from original_state to compact_state (1, 2, 3, ...)
                // and the reverse map from compact_state to original_state
                ...
                {
                    // A colour that does not parse as "#RRGGBB" counts as mid-grey rather than
                    // black or uninitialised optics.
                    ColorRGB rgb(0.5f, 0.5f, 0.5f);
                    unsigned int r = 0, g = 0, b = 0;
                    if (filament_colors.values[i].size() >= 7 && filament_colors.values[i][0] == '#' &&
                        sscanf(filament_colors.values[i].c_str() + 1, "%02x%02x%02x", &r, &g, &b) == 3)
                        rgb = ColorRGB((float) r / 255.f, (float) g / 255.f, (float) b / 255.f);
                    else
                        DBG_COUNT("COLORMIX_FILAMENT_COLOR_UNPARSED");
                ...
                // A painted volume without a recipe table (foreign 3MF, programmatic paint) is
                // resolved against a table seeded from the current palette. The table is local
                // to this slice: the ModelVolume is shared by every PrintObject of the model
                // object, and those slice concurrently. Seeding the persistent table is the
                // gizmo's job.
                std::vector<ColorMixingRecipe> seeded_recipes;
                if (recipes == nullptr)
                    for (const ModelVolume *mv : print_object.model_object()->volumes)
                        if (mv->is_model_part() && mv->is_color_mixing_painted())
                        {
                            DBG_COUNT("COLORMIX_RECIPES_SEEDED_LOCAL");
                            seeded_recipes = seeded_color_mixing_recipes(palette, *mv);
                            if (!seeded_recipes.empty())
                ...
                auto resolve_state = [&](int original_state) -> const StateResolution &
                {
                    ...
                    if (recipes && recipe_idx >= 0 && recipe_idx < (int) recipes->size())
                    {
                        ...
                        else
                        {
                            ...
                            if (best < 0)
                            {
                                // The recipe's own palette entry no longer predicts its colour
                                // (filaments changed since it was painted): the closest entry is used.
                                DBG_COUNT("COLORMIX_RECIPE_REMATCHED");
                    ...
                    for (size_t idx = 0; idx < color_layer.size(); ++idx)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                DBG_COUNT("COLORMIX_EMPTY_PATTERN_FALLBACK");
                                physical_extruder = r.fallback_extruder;
                            }
                        }
                        if (physical_extruder >= (int) num_extruders)
                            DBG_COUNT("COLORMIX_LOCKED_EXTRUDER_CLAMPED"); // a lock to an extruder no longer present
            ...
            for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id)
            {
                     ...
                     ++parent_layer_region_idx)
                {
                    ...
                    for (int extruder_id = 1; extruder_id <= int(by_extruder.size()); ++extruder_id)
                    {
                        ...
                        {
                            // The painted regions were built for fewer extruders than the
                            // segmentation now selects (an extruder was added since).
                            DBG_COUNT("MM_PAINTED_REGION_MISSING");
                            continue;
                        }
                        ...
                        // A region does not steal from itself; record the extruder and move on.
                    ...
                    if (!self_trimmed)
                    {
                        ...
                        // This could, on some models, produce bulges with the model's base color.
                        if (!mine.empty())
                        {
...
void PrintObject::slice_volumes()
{
    ...
    std::vector<VolumeSlices> volume_slices = slice_volumes_inner(print->config(), this->config(),
                                                                  this->trafo_centered(), this->model_object()->volumes,
                                                                  m_shared_regions->layer_ranges, slice_zs,
                                                                  throw_on_cancel_callback);
    // Fingerprint helper for the run-to-run drift bracket (debug only): the mesh slicer's raw
    // output per volume and layer, then the per-region union.
    auto fingerprint_layer = [](const ExPolygons &expolys, size_t &pts, double &area)
    {
        uint64_t h = 1469598103934665603ull;
        pts = 0;
        area = 0.;
    ...
    if (debug_enabled(DBG_PERIMETERS))
        for (size_t vi = 0; vi < volume_slices.size(); ++vi)
            for (size_t layer_id = 0; layer_id < volume_slices[vi].slices.size() && layer_id < slice_zs.size();
                 ++layer_id)
            {
                size_t pts = 0;
                double area = 0.;
                const uint64_t h = fingerprint_layer(volume_slices[vi].slices[layer_id], pts, area);
                dbg_log(DBG_PERIMETERS, slice_zs[layer_id], "DETERM",
                        "SLICE_RAW_FP layer=%zu volume=%zu polys=%zu pts=%zu area=%.4fmm2 hash=%016llx", layer_id, vi,
                        volume_slices[vi].slices[layer_id].size(), pts, area * 1e-12, (unsigned long long) h);
            }
    std::vector<std::vector<ExPolygons>> region_slices = slices_to_regions(this->model_object()->volumes,
                                                                           *m_shared_regions, slice_zs,
    ...
    for (size_t region_id = 0; region_id < region_slices.size(); ++region_id)
    {
        ...
        for (size_t layer_id = 0; layer_id < by_layer.size(); ++layer_id)
        {
            ...
            if (debug_enabled(DBG_PERIMETERS) && layer_id < slice_zs.size())
            {
                size_t pts = 0;
                double area = 0.;
                const uint64_t h = fingerprint_layer(by_layer[layer_id], pts, area);
                dbg_log(DBG_PERIMETERS, slice_zs[layer_id], "DETERM",
                        "SLICE_UNION_FP layer=%zu region=%zu polys=%zu pts=%zu area=%.4fmm2 hash=%016llx", layer_id,
                        region_id, by_layer[layer_id].size(), pts, area * 1e-12, (unsigned long long) h);
            }
    ...
    if (this->model_object()->is_fuzzy_skin_painted())
    {
        ...
        // apply_fuzzy_skin_segmentation() is not called here
    }
    ...
    BOOST_LOG_TRIVIAL(debug) << "Slicing volumes - make_slices in parallel - begin";
    {
                ...
                for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id)
                {
                    ...
                    if (layer->m_regions.size() == 1)
                    {
                        ...
                        if (elfoot > 0)
                        {
                            ...
                            if (delta > elfoot)
                            {
                                ...
                                elfoot -= delta;
                            layerm->m_slices.set(union_ex(Luminary::elephant_foot_compensation(
                    ...
                    else
                    {
                        if (xy_compensation_scaled < 0.f || elfoot > 0.f)
                        {
                            ...
                            Polygons trimming;
                            // Per object: a static here would keep the first object's closing radius for
                            // every later object with a different one.
                            ...
                            if (elfoot > 0.f)
                            {
                                ...
                                trimming = to_polygons(Luminary::elephant_foot_compensation(
        ...
        if (elephant_foot_compensation_scaled > 0.f && !m_layers.empty())
        {
            ...
            // The first layer keeps its uncompensated slices here; the support raft is fitted against them.
