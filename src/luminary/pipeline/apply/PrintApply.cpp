                                               ...
                                               DynamicPrintConfig &filament_overrides)
{
    ...
    for (const t_config_option_key &opt_key : current_config.keys())
    {
        ...
        if (opt_new == nullptr)
            // new_full_config need not carry every key the current config has.
            continue;
    ...
    const bool has_painted_fuzzy_skin)
{
    ...
    auto get_create_region = [&region_set, &all_regions](PrintRegionConfig &&config) -> PrintRegion *
    {
        ...
        auto it = Luminary::lower_bound_by_predicate(
...
// One synchronization pass; shrinkage_applied receives the shrinkage compensation the object
// transformations were built with, so apply() can tell whether a second pass is needed.
Print::ApplyStatus Print::apply_once(const Model &model, DynamicPrintConfig new_full_config,
                                     std::vector<std::string> *warnings, Vec3d &shrinkage_applied)
{
#ifdef _DEBUG
    ...
    for (size_t idx_model_object = 0; idx_model_object < model.objects.size(); ++idx_model_object)
    {
        ...
        bool solid_or_modifier_differ = model_volume_list_changed(model_object, model_object_new,
                                        ...
                                        (model_object_new.is_color_mixing_painted() && num_extruders_changed) ||
    ...
    // The object transformations carry the filament shrinkage compensation, which depends on the
    // extruders the print objects use; it is read once here from the objects that exist now.
    const Vec3d shrinkage_compensation_applied = this->shrinkage_compensation();
    {
        ...
        for (ModelObject *model_object : m_model.objects)
        {
            ...
            model_object_status.print_instances = print_objects_from_model_object(*model_object,
                                                                                  shrinkage_compensation_applied);
    ...
    if (print_regions_reshuffled)
    {
        ...
        for (PrintObject *print_object : m_objects)
            if (print_object_regions != print_object->m_shared_regions)
            {
                ...
                for (std::unique_ptr<Luminary::PrintRegion> &print_region : print_object_regions->all_regions)
    ...
    shrinkage_applied = shrinkage_compensation_applied;
                                ...
                                std::vector<std::string> *warnings)
{
    ...
    // would re-slice from the mesh. Apply once more with the same inputs, after the first
    // pass has released its bookkeeping, so the objects carry the compensation their
    // extruders call for from the start.
    const bool may_reapply = m_objects.empty();
    Vec3d shrinkage_applied;
    ApplyStatus status = this->apply_once(model,
                                          may_reapply ? DynamicPrintConfig(new_full_config)
                                                      : std::move(new_full_config),
                                          warnings, shrinkage_applied);
    if (may_reapply && shrinkage_applied != this->shrinkage_compensation())
    {
        const ApplyStatus again = this->apply_once(model, std::move(new_full_config), warnings, shrinkage_applied);
        status = std::max(status, again);
    }
    return status;
}
