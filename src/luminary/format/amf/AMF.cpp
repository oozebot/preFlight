...
// VERSION NUMBERS
// 0 : .amf, .amf.xml and .zip.amf files with no version definition in them.
...
// The version number was rolled back to 2, the next change should use 4.
...
const char *PREFLIGHT_CONFIG_TYPE = "slic3rpe_config";
...
{
    AMFParserContext(XML_Parser parser, DynamicPrintConfig *config, ConfigSubstitutionContext *config_substitutions,
                     Model *model)
    ...
    struct Instance
    {
        ...
        {
            return deltax_set || deltay_set || deltaz_set || rx_set || ry_set || rz_set || scalex_set || scaley_set ||
                   scalez_set || mirrorx_set || mirrory_set || mirrorz_set;
        }
    };
    ...
    case 2:
        ...
        else if (m_path[1] == NODE_TYPE_CUSTOM_GCODE)
        {
            if (strcmp(name, "code") == 0)
            {
                ...
                {
                    // It means that data was saved in old version format
                    // read old data ...
    ...
    else
    {
        ...
        case 4:
            if (m_path.back() == NODE_TYPE_DELTAX || m_path.back() == NODE_TYPE_DELTAY ||
                m_path.back() == NODE_TYPE_DELTAZ || m_path.back() == NODE_TYPE_RX || m_path.back() == NODE_TYPE_RY ||
                m_path.back() == NODE_TYPE_RZ || m_path.back() == NODE_TYPE_SCALEX ||
                m_path.back() == NODE_TYPE_SCALEY || m_path.back() == NODE_TYPE_SCALEZ ||
                m_path.back() == NODE_TYPE_SCALE || m_path.back() == NODE_TYPE_MIRRORX ||
                m_path.back() == NODE_TYPE_MIRRORY || m_path.back() == NODE_TYPE_MIRRORZ ||
        ...
        case 6:
            switch (m_path.back())
            {
            case NODE_TYPE_VERTEX1:
                m_value[0].append(s, len);
                break;
            case NODE_TYPE_VERTEX2:
                m_value[1].append(s, len);
                break;
            case NODE_TYPE_VERTEX3:
                m_value[2].append(s, len);
                break;
            default:
                break;
            }
        case 7:
            switch (m_path.back())
            {
            case NODE_TYPE_COORDINATE_X:
                m_value[0].append(s, len);
                break;
            case NODE_TYPE_COORDINATE_Y:
                m_value[1].append(s, len);
                break;
            case NODE_TYPE_COORDINATE_Z:
                m_value[2].append(s, len);
                break;
            default:
                break;
...
void AMFParserContext::endElement(const char * /* name */)
{
    ...
    case NODE_TYPE_VERTEX:
        ...
        m_object_vertices.emplace_back(float(atof(m_value[0].c_str())), float(atof(m_value[1].c_str())),
                                       float(atof(m_value[2].c_str())));
        m_value[0].clear();
        m_value[1].clear();
        m_value[2].clear();
        break;
    ...
    case NODE_TYPE_VOLUME:
    {
...

        {
            ...
            for (Vec3i &face : m_volume_facets)
                face -= Vec3i(coord_t(min_id), coord_t(min_id), coord_t(min_id));
        ...
        if (bool has_transform = !m_volume_transform.isApprox(Transform3d::Identity(), 1e-10); has_transform)
            m_volume->source.transform = Luminary::Geometry::Transformation(m_volume_transform);
    ...
    case NODE_TYPE_CUSTOM_GCODE_MODE:
    {
        ...
        m_model.custom_gcode_per_print_z().mode = mode == CustomGCode::SingleExtruderMode
    ...
    case NODE_TYPE_METADATA:
        if ((m_config != nullptr) &&
            strncmp(m_value[0].c_str(), PREFLIGHT_CONFIG_TYPE, strlen(PREFLIGHT_CONFIG_TYPE)) == 0)
        {
            // The config is stored as ini lines each prefixed with a semicolon. The legacy gcode parser is
            // used because files in the field can hold duplicate ini keys, which it tolerates.
            ConfigBase::load_from_gcode_string_legacy(*m_config, std::move(m_value[1].c_str()),
        ...
        else if (strncmp(m_value[0].c_str(), "slic3r.", 7) == 0)
        {
            ...
            if (print_config_def.options.find(opt_key) != print_config_def.options.end())
            {
                ...
                else if (m_path.size() == 5 && m_path[3] == NODE_TYPE_RANGE && m_object &&
                         !m_object->layer_config_ranges.empty())
                {
            ...
            else if (m_path.size() == 3 && m_path[1] == NODE_TYPE_OBJECT && m_object &&
                     strcmp(opt_key, "layer_height_profile") == 0)
            {
            ...
            else if (m_path.size() == 5 && m_path[1] == NODE_TYPE_OBJECT && m_path[3] == NODE_TYPE_RANGE && m_object &&
                     strcmp(opt_key, "layer_height_range") == 0)
            {
            ...
            else if (m_path.size() == 5 && m_path[3] == NODE_TYPE_VOLUME && m_volume)
            {
                if (strcmp(opt_key, "modifier") == 0)
                {
                    ...
                    m_volume->set_type((atoi(m_value[1].c_str()) == 1) ? ModelVolumeType::PARAMETER_MODIFIER
                                                                       : ModelVolumeType::MODEL_PART);
                }
                else if (strcmp(opt_key, "volume_type") == 0)
                    m_volume->set_type(ModelVolume::type_from_string(m_value[1]));
                else if (strcmp(opt_key, "matrix") == 0)
                    m_volume_transform = Luminary::Geometry::transform3d_from_string(m_value[1]);
    ...
    for (const auto &object : m_object_instances_map)
    {
        ...
        for (const Instance &instance : object.second.instances)
            if (instance.anything_set())
            {
                ...
                mi->set_rotation(Vec3d(instance.rx_set ? (double) instance.rx : 0.0,
                                       instance.ry_set ? (double) instance.ry : 0.0,
...
bool load_amf_file(const char *path, DynamicPrintConfig *config, ConfigSubstitutionContext *config_substitutions,
                   Model *model)
{
                                ...
                                Model *model, bool check_version)
{
    ...
    struct CallbackData
    {
        ...
        CallbackData(XML_Parser &parser, AMFParserContext &ctx, const mz_zip_archive_file_stat &stat)
            : parser(parser), ctx(ctx), stat(stat)
        {
        }
    };
    ...
    try
    {
        res = mz_zip_reader_extract_to_callback(
            ...
            [](void *pOpaque, mz_uint64 file_ofs, const void *pBuf, size_t n) -> size_t
            {
                ...
                {
                    char error_buf[1024];
                    ::sprintf(error_buf, "Error (%s) while parsing '%s' at line %d", data->ctx.error_message(),
                              data->stat.m_filename, (int) XML_GetCurrentLineNumber(data->parser));
                    throw Luminary::FileIOError(error_buf);
                }
    ...
    {
        // std::string msg = _(L("The selected amf file has been saved with a newer version of " + std::string(PREFLIGHT_APP_NAME) + " and is not compatible."));
        // throw Luminary::FileIOError(msg.c_str());
        const std::string msg =
            (boost::format(
                 _u8L("The selected amf file has been saved with a newer version of %1% and is not compatible.")) %
             std::string(PREFLIGHT_APP_NAME))
                .str();
        throw Luminary::FileIOError(msg);
    }
...
bool load_amf_archive(const char *path, DynamicPrintConfig *config, ConfigSubstitutionContext *config_substitutions,
                      Model *model, bool check_version)
{
    if ((path == nullptr) || (model == nullptr))
        return false;
              ...
              Model *model, bool check_version)
{
    ...
    else if (boost::iends_with(path, ".amf"))
    {
        ...
        return (zip_mask == "PK") ? load_amf_archive(path, config, config_substitutions, model, check_version)
                                  : load_amf_file(path, config, config_substitutions, model);
    }
    else
        return false;
}
