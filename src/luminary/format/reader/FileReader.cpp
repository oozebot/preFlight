...
#include "luminary/format/mesh/STEP.hpp"
#include "luminary/format/svg/SVG.hpp"

#include <boost/filesystem.hpp>

#include "luminary/core/I18N.hpp"

namespace Luminary::FileReader
{
                                  ...
                                  LoadStats *stats = nullptr)
{
    ...
    {
        // Only the geometry is kept here, so the writing application version is not checked.
        ...
        throw Luminary::RuntimeError(L(
            "Unknown file format. Input file must have .stl, .obj, .step/.stp, .svg, .amf(.xml) or extension .3mf(.zip)."));

    if (!result)
        throw Luminary::RuntimeError(L("Loading of a model file failed."));

    if (model.objects.empty() && temp_config.empty())
        throw Luminary::RuntimeError(L("The supplied file couldn't be read because it's empty"));
                                ...
                                LoadStats *stats = nullptr)
{
    ...
    if (is_project_file(input_file))
    {
        ...
        throw Luminary::RuntimeError(L("Unknown file format. Input file must have .3mf extension."));
...
TriangleMesh load_mesh(const std::string &input_file)
{
    ...
    {
        throw Luminary::RuntimeError(L("Error! Invalid model"));
    }
...
static bool looks_like_multipart_object(const Model &model)
{
    ...
    for (const ModelObject *obj : model.objects)
    {
        ...
        // AMF files reach this point before default instances are added, so fall back to the
        // untransformed bounding box when there is no instance.
