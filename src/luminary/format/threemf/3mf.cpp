...
#include "luminary/model/annotations/BrimEarsPoint.hpp"
...
#include "luminary/model/schema/emboss/EmbossShape.hpp"
#include "luminary/geometry/serialize/ExPolygonSerialize.hpp"

#include "luminary/gizmo/emboss/svg/NSVGUtils.hpp"
...
// VERSION NUMBERS
// 0 : .3mf files with no version definition in them.
...
// The version number was rolled back to 1, the next change should use 3.
...
class version_error : public Luminary::FileIOError
{
public:
    version_error(const std::string &what_arg) : Luminary::FileIOError(what_arg) {}
    version_error(const char *what_arg) : Luminary::FileIOError(what_arg) {}
};
...
Luminary::Transform3d get_transform_from_3mf_specs_string(const std::string &mat_str)
{
    ...
    Luminary::Transform3d ret = Luminary::Transform3d::Identity();
...
class _3MF_Importer : public _3MF_Base
{
    ...
    // Raw Application metadata string (e.g. "preFlight-1.4.0")
...
private:
    ...
    // The uncompressed size in the archive directory is untrusted; a part is read into memory
    // whole only under this cap (the model part streams).
    bool _part_size_ok(const mz_zip_archive_file_stat &stat);
                                          ...
                                          ConfigSubstitutionContext &config_substitutions)
{
    ...
    for (mz_uint i = 0; i < num_entries; ++i)
    {
        if (mz_zip_reader_file_stat(&archive, i, &stat))
        {
            ...
            {
                DBG_COUNT_LOAD("THREEMF_PART_SKIPPED");
                add_error("Skipped archive part " + name + ": " + e.what());
            }
        }
    }
    ...
    for (const IdToModelObjectMap::value_type &object : m_objects)
    {
        ...
        {
            // config data has been found, this model carries a print configuration
        ...
        {
            // config data not found, this model carries no print configuration
    ...
    for (int obj_id = 0; obj_id < int(model.objects.size()); ++obj_id)
    {
        ...
        if (o->volumes.size() == 1)
        {
            ...
            const Luminary::Geometry::Transformation &first_inst_trafo = o->instances.front()->get_transformation();
            ...
            {
                const Luminary::Geometry::Transformation &vol_trafo = v->get_transformation();
                for (int inst_id = 0; inst_id < int(o->instances.size()); ++inst_id)
                {
                    ModelInstance *i = o->instances[inst_id];
                    const Luminary::Geometry::Transformation &inst_trafo = i->get_transformation();
    ...
    if (!m_generator_version.has_value() && model.objects.empty())
    {
        ...
        throw Luminary::RuntimeError(msg);
    }
...
bool _3MF_Importer::_extract_relationships_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat)
{
    ...
    {
        char error_buf[1024];
        ::snprintf(error_buf, sizeof(error_buf), "Error (%s) while parsing xml file at line %d",
...
bool _3MF_Importer::_part_size_ok(const mz_zip_archive_file_stat &stat)
{
    static constexpr mz_uint64 MAX_PART_SIZE = mz_uint64(64) * 1024 * 1024;
    if (stat.m_uncomp_size <= MAX_PART_SIZE)
        return true;
    DBG_COUNT_LOAD("THREEMF_PART_TOO_LARGE");
    add_error(std::string("Archive part ") + stat.m_filename + " claims " + std::to_string(stat.m_uncomp_size) +
              " bytes, above the " + std::to_string(MAX_PART_SIZE) + " byte limit");
    return false;
}
    ...
    try
    {
        res = mz_zip_reader_extract_to_callback(
            ...
            [](void *pOpaque, mz_uint64 file_ofs, const void *pBuf, size_t n) -> size_t
            {
                ...
                {
                    char error_buf[1024];
                    ::snprintf(error_buf, sizeof(error_buf), "Error (%s) while parsing '%s' at line %d",
                                                       ...
                                                       const std::string &archive_filename)
{
    ...
    if (stat.m_uncomp_size > 0)
    {
        ...
        // used because 3MFs in the field can hold duplicate ini keys, which it tolerates.
...
bool _3MF_Importer::_handle_end_metadata()
{
    if (m_curr_metadata_name == SLIC3RPE_3MF_VERSION)
    {
        ...
        {
            // std::string msg = _u8L("The selected 3mf file has been saved with a newer version of " + std::string(PREFLIGHT_APP_NAME) + " and is not compatible.");
...
{
    Luminary::Geometry::Transformation t(transform);
                                      ...
                                      ConfigSubstitutionContext &config_substitutions)
{
    ...
    for (const ObjectMetadata::VolumeMetadata &volume_data : volumes)
    {
        ...
        for (const Metadata &metadata : volume_data.metadata)
        {
            ...
            {
                volume_matrix_to_object = Luminary::Geometry::transform3d_from_string(metadata.value);
        ...
        if (m_version == 0)
        {
            ...
            if (object.instances.size() == 1)
            {
                ...
                object.instances.front()->set_transformation(Luminary::Geometry::Transformation());
            }
        }
        ...
        if (has_transform)
            volume->source.transform = Luminary::Geometry::Transformation(volume_matrix_to_object);
                                        ...
                                        const ThumbnailData *thumbnail_data)
{
    ...
    // Adds the print config file ("Metadata/Slic3r_PE.config").
    // This file contains the content of FullPrintConfig.
    if (config != nullptr)
    {
    ...
    // Adds the model config file ("Metadata/Slic3r_PE_model.config").
                                               ...
                                               IdToObjectDataMap &objects_data)
{
    ...
    if (!mz_zip_writer_add_staged_open(
            ...
            m_zip64 ?
                ...
                (uint64_t(1) << 30) * 16
                    :
                    // Maximum expected 3MF file size is 4GB-1, for interoperability with the Windows 10
                // 3D model fixing API.
                (uint64_t(1) << 32) - 1,
...

    {
        ...
        std::string date = Luminary::Utils::utc_timestamp(Luminary::Utils::get_current_time_utc());
        ...
        stream << " <" << METADATA_TAG << " name=\"Application\">" << PREFLIGHT_APP_KEY << "-" << PREFLIGHT_VERSION
               << "</" << METADATA_TAG << ">\n";
