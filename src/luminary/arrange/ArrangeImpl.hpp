...
#include <luminary/core/parallel/ExecutionTBB.hpp>
#include <luminary/geometry/hull/ConvexHull.hpp>

#include <luminary/arrange/packing/ArrangeBase.hpp>
#include <luminary/arrange/packing/ArrangeFirstFit.hpp>
#include <luminary/arrange/packing/PackStrategyNFP.hpp>
#include <luminary/arrange/packing/TMArrangeKernel.hpp>
#include <luminary/arrange/packing/GravityKernel.hpp>
#include <luminary/arrange/packing/RectangleOverfitPackingStrategy.hpp>
#include <luminary/arrange/packing/Beds.hpp>

#include <luminary/arrange/Arrange.hpp>
#include <luminary/arrange/scene/MutableItemTraits.hpp>
#include <luminary/arrange/scene/SegmentedRectangleBed.hpp>

#ifndef NDEBUG
#include <luminary/arrange/packing/SVGDebugOutputKernelWrapper.hpp>
#endif
...
// by XL printers.
template<class It, class ConstIt, class SelectionStrategy, class PackStrategy, class... SBedArgs>
void arrange(SelectionStrategy &&selstrategy, PackStrategy &&packingstrategy, const Range<It> &items,
             const Range<ConstIt> &fixed, const SegmentedRectangleBed<SBedArgs...> &bed)
{
    // Dispatch:
    arrange(std::forward<SelectionStrategy>(selstrategy), std::forward<PackStrategy>(packingstrategy), items, fixed,
    ...
    {
        if (auto occup_it = bed_occupied.find(bedidx); occup_it != bed_occupied.end() && occup_it->second)
            continue;
        ...
        Point corr{coord_t(0), coord_t(0)};
...
using VariantKernel = boost::variant<TMArrangeKernel, GravityKernel>;

template<>
struct KernelTraits_<VariantKernel>
{
    template<class ArrItem>
    static double placement_fitness(const VariantKernel &kernel, const ArrItem &itm, const Vec2crd &transl)
    {
        double ret = NaNd;
        boost::apply_visitor([&](auto &k) { ret = k.placement_fitness(itm, transl); }, kernel);

        return ret;
    }

    template<class ArrItem, class Bed, class Ctx, class RemIt>
    static bool on_start_packing(VariantKernel &kernel, ArrItem &itm, const Bed &bed, const Ctx &packing_context,
                                 const Range<RemIt> &remaining_items)
    {
        bool ret = false;

        boost::apply_visitor([&](auto &k) { ret = k.on_start_packing(itm, bed, packing_context, remaining_items); },
                             kernel);
    ...
    {
        bool ret = false;
        boost::apply_visitor([&](auto &k) { ret = k.on_item_packed(itm); }, kernel);
...
struct firstfit::ItemArrangedVisitor<ArrItem, DataStoreOnly<ArrItem>>
{
    ...
    static void on_arranged(ArrItem &itm, const Bed &bed, const Range<PIt> &packed, const Range<RIt> &remaining)
    {
...
inline RectPivots xlpivots_to_rect_pivots(ArrangeSettingsView::XLPivots xlpivot)
{
    if (xlpivot == arr2::ArrangeSettingsView::xlpRandom)
    {
        ...
        std::mt19937 rng(rd());
        std::uniform_int_distribution<std::mt19937::result_type> dist(0, arr2::ArrangeSettingsView::xlpRandom - 1);
    ...
    switch (xlpivot)
    {
    case arr2::ArrangeSettingsView::xlpCenter:
        rectpivot = RectPivots::Center;
        break;
    case arr2::ArrangeSettingsView::xlpFrontLeft:
        rectpivot = RectPivots::BottomLeft;
        break;
    case arr2::ArrangeSettingsView::xlpFrontRight:
        rectpivot = RectPivots::BottomRight;
        break;
    case arr2::ArrangeSettingsView::xlpRearLeft:
        rectpivot = RectPivots::TopLeft;
        break;
    case arr2::ArrangeSettingsView::xlpRearRight:
        rectpivot = RectPivots::TopRight;
        break;
    default:;
    }
...
void fill_rotations(const Range<It> &items, const Bed &bed, const ArrangeSettingsView &settings)
{
    ...
    for (auto &itm : items)
    {
        ...
        std::vector<double> rotations = {minbbr, minbbr + PI / 4., minbbr + PI / 2., minbbr + PI, minbbr + 3 * PI / 4.};
...
// An arranger put together to fulfill all the requirements of preFlight based
...
class DefaultArranger : public Arranger<ArrItem>
{
    ...
    void arrange_(const Range<It> &items, const Range<FixIt> &fixed, const Bed &bed, ArrangerCtl<ArrItem> &ctl)
    {
        auto cmpfn = [](const auto &itm1, const auto &itm2)
        {
            ...
            return pa == pb ? area(envelope_convex_hull(itm1)) > area(envelope_convex_hull(itm2)) : pa > pb;
        };
        ...
        bool with_wipe_tower = std::any_of(items.begin(), items.end(), [](auto &itm) { return is_wipe_tower(itm); });
        ...
        if (!with_wipe_tower && m_settings.get_arrange_strategy() == ArrangeSettingsView::asAuto && IsRectangular<Bed>)
        {
...
public:
    explicit DefaultArranger(const ArrangeSettingsView &settings) { m_settings.set_from(settings); }

    void arrange(std::vector<ArrItem> &items, const std::vector<ArrItem> &fixed, const ExtendedBed &bed,
                 ArrangerCtl<ArrItem> &ctl) override
    {
        visit_bed(
            [this, &items, &fixed, &ctl](auto rawbed)
            {
                if constexpr (IsSegmentedBed<decltype(rawbed)>)
                    rawbed.pivot = xlpivots_to_rect_pivots(m_settings.get_xl_alignment());
...
template<class ArrItem>
std::unique_ptr<Arranger<ArrItem>> Arranger<ArrItem>::create(const ArrangeSettingsView &settings)
{
...
template<class ArrItem>
ArrItem ConvexItemConverter<ArrItem>::convert(const Arrangeable &arrbl, coord_t offs) const
{
...
template<class ArrItem>
ArrItem AdvancedItemConverter<ArrItem>::convert(const Arrangeable &arrbl, coord_t offs) const
{
    auto bed_index = arrbl.get_bed_index();
...
template<class ArrItem>
ArrItem AdvancedItemConverter<ArrItem>::get_arritem(const Arrangeable &arrbl, coord_t offs) const
{
    coord_t infl = offs + coord_t(std::ceil(this->safety_dist() / 2.));
...
template<class ArrItem>
ArrItem BalancedItemConverter<ArrItem>::get_arritem(const Arrangeable &arrbl, coord_t offs) const
{
...
template<class ArrItem>
std::unique_ptr<ArrangeableToItemConverter<ArrItem>> ArrangeableToItemConverter<ArrItem>::create(
    ArrangeSettingsView::GeometryHandling gh, coord_t safety_d)
{
