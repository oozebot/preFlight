...
#include <luminary/arrange/helper/MinAreaBoundingBox.hpp>
...
public:
    ...
    virtual void arrange(std::vector<ArrItem> &items, const std::vector<ArrItem> &fixed, const ExtendedBed &bed,
                         Ctl &ctl) = 0;

    void arrange(std::vector<ArrItem> &items, const std::vector<ArrItem> &fixed, const ExtendedBed &bed,
                 ArrangeTaskCtl &ctl);

    void arrange(std::vector<ArrItem> &items, const std::vector<ArrItem> &fixed, const ExtendedBed &bed, Ctl &&ctl)
    {
...
template<class ArrItem>
using ArrangerCtl = typename Arranger<ArrItem>::Ctl;
...
template<class ArrItem>
void Arranger<ArrItem>::arrange(std::vector<ArrItem> &items, const std::vector<ArrItem> &fixed, const ExtendedBed &bed,
                                ArrangeTaskCtl &ctl)
{
...
template<class ArrItem>
class ArrangeableToItemConverter
{
public:
    ...
    static std::unique_ptr<ArrangeableToItemConverter> create(ArrangeSettingsView::GeometryHandling geometry_handling,
                                                              coord_t safety_d);

    static std::unique_ptr<ArrangeableToItemConverter> create(const Scene &sc)
    {
        return create(sc.settings().get_geometry_handling(), scaled(sc.settings().get_distance_from_objects()));
    }
};
...
public:
    ...
    void write(std::string_view key, std::any d) override { set_data(dstore, std::string{key}, std::move(d)); }
};
...
public:
    BasicItemConverter(coord_t safety_d = 0, coord_t simpl_tol = 0) : m_safety_d{safety_d}, m_simplify_tol{simpl_tol} {}

    coord_t safety_dist() const noexcept { return m_safety_d; }

    coord_t simplification_tolerance() const override { return m_simplify_tol; }
};
...
struct ImbueableItemTraits_
{
    static constexpr const char *Key = "object_id";

    static void imbue_id(ArrItem &itm, const ObjectID &id) { set_arbitrary_data(itm, Key, id); }
...
bool apply_arrangeitem(const ArrItem &itm, ArrangeableModel &mdl)
{
    ...
    {
        mdl.visit_arrangeable(*id,
                              [&itm, &ret](Arrangeable &arrbl)
                              {
...
{
    return MinAreaBoundigBox{envelope_convex_hull(itm), MinAreaBoundigBox::pcConvex}.angle_to_X();
}
...
template<class ArrItem>
auto get_corrected_bed(const ExtendedBed &bed, const ArrangeableToItemConverter<ArrItem> &converter)
{
    auto bedcpy = bed;
    visit_bed([tol = -converter.simplification_tolerance()](auto &rawbed) { rawbed = offset(rawbed, tol); }, bedcpy);
