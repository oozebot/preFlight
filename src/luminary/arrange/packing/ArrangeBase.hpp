...
template<class It>
constexpr bool is_const_iterator(const It &it) noexcept
{
    return IsConstIterator<It>;
}
...
using PackStrategyTag = typename PackStrategyTag_<remove_cvref_t<Strategy>>::Tag;

template<class PackStrategy, class En = void>
struct PackStrategyTraits_
{
    template<class ArrItem>
    using Context = DefaultPackingContext<ArrItem>;
...
template<class PS>
using PackStrategyTraits = PackStrategyTraits_<StripCVRef<PS>>;

template<class PS, class ArrItem>
using PackStrategyContext = typename PackStrategyTraits<PS>::template Context<StripCVRef<ArrItem>>;
...
bool pack(Strategy &&strategy, const Bed &bed, ArrItem &item, const PackStrategyContext<Strategy, ArrItem> &context,
...
template<class Strategy, class Bed, class ArrItem, class RemIt>
bool pack(Strategy &&strategy, const Bed &bed, ArrItem &item, const PackStrategyContext<Strategy, ArrItem> &context,
          const Range<RemIt> &remaining_items, const UnimplementedPacking &)
{
...
void arrange(SelectionStrategy &&selstrategy, PackStrategy &&packingstrategy, const Range<It> &items,
             const Range<ConstIt> &fixed, const TBed &bed)
{
    ...
    arrange(std::forward<SelectionStrategy>(selstrategy), std::forward<PackStrategy>(packingstrategy), items, fixed,
            bed, SelStrategyTag<SelectionStrategy>{});
}
...
void arrange(SelectionStrategy &&selstrategy, PackStrategy &&packingstrategy, const Range<It> &items, const TBed &bed)
{
    ...
    arrange(std::forward<SelectionStrategy>(selstrategy), std::forward<PackStrategy>(packingstrategy), items,
            crange(dummy), bed);
}
...
template<class It, class ConstIt, class TBed, class SelectionStrategy, class PackStrategy>
void arrange(SelectionStrategy &&selstrategy, PackStrategy &&packingstrategy, const Range<It> &items,
             const Range<ConstIt> &fixed, const TBed &bed, const UnimplementedSelection &)
{
...
int get_max_bed_index(const Range<It> &items)
{
    auto it = std::max_element(items.begin(), items.end(),
                               [](auto &i1, auto &i2) { return get_bed_index(i1) < get_bed_index(i2); });
