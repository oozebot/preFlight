...
struct ItemArrangedVisitor
{
    ...
    static void on_arranged(ArrItem &itm, const Bed &bed, const Range<PIt> &packed_items,
...
template<class CompareFn = DefaultItemCompareFn, class OnArrangedFn = DefaultOnArrangedFn,
...
struct SelectionStrategy
{
    ...
    SelectionStrategy(CompareFn cmp = {}, OnArrangedFn on_arranged = {}, StopCondition stopcond = {})
        : cmpfn{cmp}, on_arranged_fn{std::move(on_arranged)}, cancel_fn{std::move(stopcond)}
    {
    }
};
...
void arrange(SelStrategy &&sel, PackStrategy &&ps, const Range<It> &items, const Range<ConstIt> &fixed, const TBed &bed,
             const firstfit::SelectionTag &)
{
    ...
    remove_unpackable_items(ps, sorted_items, bed, [&is_cancelled]() { return is_cancelled(); });
