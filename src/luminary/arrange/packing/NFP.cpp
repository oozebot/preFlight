...
static bool line_cmp(const Line &e1, const Line &e2)
{
    ...
    if (q[0] == q[1])
    {                                 // only bother if p1 and p2 are in the same quadrant
...
Polygon ifp_convex_convex(const Polygon &fixed, const Polygon &movable)
{
    ...
    // find the first hole
    auto it = std::find_if(ifp.begin(), ifp.end(), [](const Polygon &subifp) { return subifp.is_clockwise(); });
...
ExPolygons ifp_convex(const arr2::IrregularBed &bed, const Polygon &convexpoly)
{
    ...
    {
        Polygons triangles = Luminary::convex_decomposition_tess(part);
