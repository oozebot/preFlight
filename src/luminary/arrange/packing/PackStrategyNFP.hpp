...
#include <luminary/arrange/packing/EdgeCache.hpp>
#include <luminary/arrange/packing/KernelTraits.hpp>
#include <luminary/arrange/packing/NFPArrangeItemTraits.hpp>

#include "luminary/arrange/helper/NLoptOptimizer.hpp"
#include "luminary/core/parallel/ExecutionSeq.hpp"
...
struct DummyArrangeKernel
{
    ...
    bool on_start_packing(ArrItem &itm, const Bed &bed, const Context &packing_context,
...
using OptAlg = typename Strategy::OptAlg;

template<class ArrangeKernel = DummyArrangeKernel, class ExecPolicy = ExecutionSeq,
         class OptMethod = opt::AlgNLoptSubplex, class StopCond = DefaultStopCondition>
struct PackStrategyNFP
{
    ...
    PackStrategyNFP(opt::Optimizer<OptMethod> slv, ArrangeKernel k = {}, ExecPolicy execpolicy = {}, double accur = 1.,
                    StopCond stop_cond = {})
        : kernel{std::move(k)}
    ...
    PackStrategyNFP(ArrangeKernel k = {}, ExecPolicy execpolicy = {}, double accur = 1., StopCond stop_cond = {})
        : PackStrategyNFP{opt::Optimizer<OptMethod>{}, std::move(k), std::move(execpolicy), accur, std::move(stop_cond)}
    {
...
double pick_best_spot_on_nfp(ArrItem &item, const ExPolygons &nfp, const Bed &bed,
...
bool pack(Strategy &strategy, const Bed &bed, ArrItem &item,
          const PackStrategyContext<Strategy, ArrItem> &packing_context, const Range<RemIt> &remaining_items,
          const NFPPackingTag &)
{
    using KernelT = KernelTraits<decltype(strategy.kernel)>;
