...
#include "RectangleOverfitKernelWrapper.hpp"
...
{
    Vec2crd operator()(const BoundingBox &bedbb, const BoundingBox &pilebb) { return bedbb.center() - pilebb.center(); }
};
...
struct RectangleOverfitPackingContext : public DefaultPackingContext<ArrItem>
{
    ...
    explicit RectangleOverfitPackingContext(const BoundingBox limits, int bedidx,
...
struct RectangleOverfitPackingStrategy
{
    ...
    RectangleOverfitPackingStrategy(PackStrategyNFP<Args...> s, PostAlignmentFn post_align_fn)
...
{
    template<class ArrItem>
    using Context = typename RectangleOverfitPackingStrategy<Args...>::template Context<StripCVRef<ArrItem>>;
          ...
          const RectangleOverfitPackingStrategyTag &)
{
    ...
    if (fixed_items_range(packing_context).empty())
    {
        ...
        PackStrategyNFP modded_strategy{base.solver, RectangleOverfitKernelWrapper{base.kernel, packing_context.limits},
                                        base.ep, base.accuracy};

        ret = pack(modded_strategy, InfiniteBed{packing_context.limits.center()}, item, packing_context,
