...
///|/
/*================================================================*/
// Interface of the sequential printing SMT model for preFlight
/*================================================================*/
...
enum PrinterType
{
    ...
    SEQ_PRINTER_TYPE_COMPACT,
    SEQ_PRINTER_TYPE_BED_250x210,
    SEQ_PRINTER_TYPE_BED_360x360
};

const int SEQ_BED_250x210_X_SIZE = 2500;
const int SEQ_BED_250x210_Y_SIZE = 2100;

const coord_t SEQ_BED_250x210_NOZZLE_LEVEL = 0;
const coord_t SEQ_BED_250x210_EXTRUDER_LEVEL = 2000000;
const coord_t SEQ_BED_250x210_HOSE_LEVEL = 18000000;
const coord_t SEQ_BED_250x210_GANTRY_LEVEL = 26000000;

const int SEQ_BED_360x360_X_SIZE = 3600;
const int SEQ_BED_360x360_Y_SIZE = 3600;

const coord_t SEQ_BED_360x360_NOZZLE_LEVEL = 0;
const coord_t SEQ_BED_360x360_EXTRUDER_LEVEL = 2000000;
const coord_t SEQ_BED_360x360_HOSE_LEVEL = 18000000;
const coord_t SEQ_BED_360x360_GANTRY_LEVEL = 26000000;

/*----------------------------------------------------------------*/

bool PrinterGeometry::convert_Geometry2PlateBounds(Luminary::BoundingBox &plate_bounding_box,
                                                   Luminary::Polygon &plate_bounding_polygon) const
{
    ...
    if (fabs(plate.area() - plate_box.polygon().area()) > EPSILON)
    {
        ...
        {
            coord_t x = plate.points[i].x() / SEQ_SLICER_SCALE_FACTOR;
            coord_t y = plate.points[i].y() / SEQ_SLICER_SCALE_FACTOR;
            plate_bounding_polygon.points.insert(plate_bounding_polygon.points.begin() + i, Point(x, y));
        }
    ...
    {
        plate_bounding_box = BoundingBox(Point(plate_box.min.x() / SEQ_SLICER_SCALE_FACTOR,
                                               plate_box.min.y() / SEQ_SLICER_SCALE_FACTOR),
                                         Point(plate_box.max.x() / SEQ_SLICER_SCALE_FACTOR,
                                               plate_box.max.y() / SEQ_SLICER_SCALE_FACTOR));
...
{
    if (check_ScheduledObjectsForSequentialConflict(solver_configuration, printer_geometry, objects_to_print,
                                                    scheduled_plates))
    {
        return false;
    }
...
std::optional<std::pair<int, int>> check_ScheduledObjectsForSequentialConflict(
    const SolverConfiguration &solver_configuration, const PrinterGeometry &printer_geometry,
    const std::vector<ObjectToPrint> &objects_to_print, const std::vector<ScheduledPlate> &scheduled_plates)
{
    ...
    for (unsigned int i = 0; i < objects_to_print.size(); ++i)
    {
        ...
        std::vector<Luminary::Polygon> scale_down_unreachable_polygons;

        flat_index_map[objects_to_print[i].id] = i;

        Polygon scale_down_object_polygon;

        prepare_ExtruderPolygons(solver_configuration, printer_geometry, objects_to_print[i], convex_level_polygons,
                                 box_level_polygons, extruder_convex_level_polygons, extruder_box_level_polygons,
                                 false);

        prepare_ObjectPolygons(solver_configuration, convex_level_polygons, box_level_polygons,
                               extruder_convex_level_polygons, extruder_box_level_polygons, scale_down_object_polygon,
    ...
    for (const auto &scheduled_plate : scheduled_plates)
    {
        ...
        std::vector<Luminary::Polygon> plate_polygons;
        std::vector<std::vector<Luminary::Polygon>> plate_unreachable_polygons;
	    ...
	    */
        ...
        if (auto conflict = check_PointsOutsidePolygons(dec_values_X, dec_values_Y, dec_values_T, plate_polygons,
                                                        plate_unreachable_polygons))
        {
            return std::pair<int, int>(scheduled_plate.scheduled_objects[conflict.value().first].id,
        ...
        if (auto conflict = check_PolygonLineIntersections(dec_values_X, dec_values_Y, dec_values_T, plate_polygons,
                                                           plate_unreachable_polygons))
        {
            return std::pair<int, int>(scheduled_plate.scheduled_objects[conflict.value().first].id,
                                       scheduled_plate.scheduled_objects[conflict.value().second].id);
                                                               ...
                                                               std::function<void(int)> progress_callback)
{
    ...
    schedule_ObjectsForSequentialPrint(solver_configuration, printer_geometry, objects_to_print, scheduled_plates,
                                       progress_callback);
                                        ...
                                        std::function<void(int)> progress_callback)
{
    ...
    for (unsigned int i = 0; i < objects_to_print.size(); ++i)
    {
        ...
        prepare_ExtruderPolygons(solver_configuration, printer_geometry, objects_to_print[i], convex_level_polygons,
                                 box_level_polygons, extruder_convex_level_polygons, extruder_box_level_polygons, true);

        prepare_ObjectPolygons(solver_configuration, convex_level_polygons, box_level_polygons,
                               extruder_convex_level_polygons, extruder_box_level_polygons, solvable_object.polygon,
    ...
    int progress_object_phases_total = SEQ_MAKE_EXTRA_PROGRESS(
    ...
    do
    {
        ...
        optimized = optimize_SubglobalConsequentialPolygonNonoverlappingBinaryCentered(
            solver_configuration, poly_positions_X, poly_positions_Y, times_T, solvable_objects, trans_bed_lepox,
            decided_polygons, remaining_polygons, progress_object_phases_done, progress_object_phases_total,
            progress_callback);
        ...
        if (optimized)
        {
#ifdef DEBUG
            {
                ...
                for (unsigned int i = 0; i < decided_polygons.size(); ++i)
                {
                    printf("  [ID:%d,RID:%d] x:%.3f, y:%.3f (t:%.3f)\n", original_index_map[decided_polygons[i]],
                           decided_polygons[i], poly_positions_X[decided_polygons[i]].as_double(),
                           poly_positions_Y[decided_polygons[i]].as_double(), times_T[decided_polygons[i]].as_double());
                }
            ...
            for (unsigned int i = 0; i < decided_polygons.size(); ++i)
            {
                scheduled_polygons.insert(
                    std::pair<double, int>(times_T[decided_polygons[i]].as_double(), decided_polygons[i]));
            }
            ...
            for (const auto &scheduled_polygon : scheduled_polygons)
            {
                ...
                scaleUp_PositionForSlicer(poly_positions_X[scheduled_polygon.second],
                                          poly_positions_Y[scheduled_polygon.second], X, Y);
                                       ...
                                       std::function<void(int)> progress_callback)
{
    ...
    PrinterType printer_type = SEQ_PRINTER_TYPE_BED_250x210;
            ...
            if (!objects_to_print[i].pgns_at_height[j].second.points.empty())
            {
                ...
                if (solver_configuration.decimation_precision != SEQ_DECIMATION_PRECISION_UNDEFINED)
                {
                    decimate_PolygonForSequentialSolver(solver_configuration,
                                                        objects_to_print[i].pgns_at_height[j].second, decimated_polygon,
                                                        true);
                }
                else
                {
                ...
                switch (printer_type)
                {
                case SEQ_PRINTER_TYPE_BED_250x210:
                {
                    switch (height)
                    {
                    case SEQ_BED_250x210_NOZZLE_LEVEL: // nozzle
                    {
                        nozzle_polygon = decimated_polygon;
                        break;
                    }
                    case SEQ_BED_250x210_EXTRUDER_LEVEL: // extruder
                    {
                        extruder_polygon = decimated_polygon;
                        break;
                    }
                    case SEQ_BED_250x210_HOSE_LEVEL: // hose
                    {
                        hose_polygon = decimated_polygon;
                        break;
                    }
                    case SEQ_BED_250x210_GANTRY_LEVEL: // gantry
                    {
                ...
                case SEQ_PRINTER_TYPE_BED_360x360:
                {
                    switch (height)
                    {
                    case SEQ_BED_360x360_NOZZLE_LEVEL: // nozzle
                    {
                        nozzle_polygon = decimated_polygon;
                        break;
                    }
                    case SEQ_BED_360x360_EXTRUDER_LEVEL: // extruder
                    {
                        extruder_polygon = decimated_polygon;
                        break;
                    }
                    case SEQ_BED_360x360_HOSE_LEVEL: // hose (no hose on this bed)
                    {
                        hose_polygon = decimated_polygon;
                        break;
                    }
                    case SEQ_BED_360x360_GANTRY_LEVEL: // gantry
                    {
                        gantry_polygon = decimated_polygon;
                        break;
                    }
...
void setup_ExtruderUnreachableZones(const SolverConfiguration &solver_configuration,
                                    std::vector<std::vector<Luminary::Polygon>> &convex_unreachable_zones,
                                    std::vector<std::vector<Luminary::Polygon>> &box_unreachable_zones)
{
    PrinterType printer_type = SEQ_PRINTER_TYPE_BED_250x210;
...
int schedule_ObjectsForSequentialPrint(const SolverConfiguration &solver_configuration,
                                       const std::vector<ObjectToPrint> &objects_to_print,
                                       const std::vector<std::vector<Luminary::Polygon>> &convex_unreachable_zones,
                                       const std::vector<std::vector<Luminary::Polygon>> &box_unreachable_zones,
                                       std::vector<ScheduledPlate> &scheduled_plates,
                                       std::function<void(int)> progress_callback)
{
#ifdef PROFILE
    ...
    for (unsigned int i = 0; i < objects_to_print.size(); ++i)
    {
        ...
        prepare_UnreachableZonePolygons(solver_configuration, convex_level_polygons, box_level_polygons,
                                        convex_unreachable_zones, box_unreachable_zones,
                                        solvable_object.unreachable_polygons);
