...
///|/ Copyright (c) Prusa Research 2023 - 2024 Pavel Surynek
///|/
...
struct PrinterGeometry
{
    ...
    std::map<coord_t, std::vector<Luminary::Polygon>> extruder_slices;

    bool convert_Geometry2PlateBounds(Luminary::BoundingBox &plate_bounding_box,
...
struct SolverConfiguration
{
    ...
    Luminary::BoundingBox plate_bounding_box;
    Luminary::Polygon plate_bounding_polygon;
...
struct ObjectToPrint
{
    ...
    std::vector<std::pair<coord_t, Luminary::Polygon>> pgns_at_height;
};

struct ScheduledObject
{
    ScheduledObject(int _id, coord_t _x, coord_t _y) : id(_id), x(_x), y(_y) { /* */ }
...
std::vector<ScheduledPlate> schedule_ObjectsForSequentialPrint(
    const SolverConfiguration &solver_configuration, const PrinterGeometry &printer_geometry,
    const std::vector<ObjectToPrint> &objects_to_print,
    std::function<void(int)> progress_callback = [](int progress) {});

void schedule_ObjectsForSequentialPrint(
    const SolverConfiguration &solver_configuration, const PrinterGeometry &printer_geometry,
    const std::vector<ObjectToPrint> &objects_to_print, std::vector<ScheduledPlate> &scheduled_plates,
    std::function<void(int)> progress_callback = [](int progress) {});
...
int schedule_ObjectsForSequentialPrint(
    const SolverConfiguration &solver_configuration, const std::vector<ObjectToPrint> &objects_to_print,
    std::vector<ScheduledPlate> &scheduled_plates, std::function<void(int)> progress_callback = [](int progress) {});
