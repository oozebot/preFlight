...
// resources_dir, which this file reached transitively until Slicing.hpp stopped carrying the
// Utils.hpp umbrella.
...
static Sequential::PrinterGeometry get_printer_geometry(const ConfigBase &config)
{
    ...
    const std::string printer_notes = config.opt_string("printer_notes");
    {
        ...
        if (slices.empty())
        {
            ...
            slices.push_back(ExtruderSlice{
                0, CONVEX, {{{-5000000, -5000000}, {5000000, -5000000}, {5000000, 5000000}, {-5000000, 5000000}}}});
            slices.push_back(ExtruderSlice{1000000, BOX, {{{-r, -r}, {r, -r}, {r, r}, {-r, r}}}});
            slices.push_back(ExtruderSlice{
                h, BOX, {{{-scaled(bed_x), -r}, {scaled(bed_x), -r}, {scaled(bed_x), r}, {-scaled(bed_x), r}}}});
        }
    }
...
static std::vector<Sequential::ObjectToPrint> get_objects_to_print(const Model &model,
                                                                   ...
                                                                   int selected_bed)
{
    ...
    Luminary::sort_remove_duplicates(heights);

    // Now collect all objects and projections of convex hull above respective heights.
    std::vector<std::pair<Sequential::ObjectToPrint, std::vector<Sequential::ObjectToPrint>>>
        objects; // first = object id, the vector = ids of its instances
    for (const ModelObject *mo : model.objects)
    {
        ...
        for (const ModelInstance *mi : mo->instances)
        {
            ...
            if (mi->printable)
            {
                ...
                for (double height : heights)
                {
                    ...
                    Polygon pgn = its_convex_hull_2d_above(raw_mesh.its, mi->get_matrix_no_offset().cast<float>(),
                                                           height - mi->get_offset().z());
...
{
    m_plates = Sequential::schedule_ObjectsForSequentialPrint(m_solver_configuration, m_printer_geometry, m_objects,
                                                              progress_fn);
    ...
    if (m_selected_bed != -1 && s_multiple_beds.get_number_of_beds() > 1)
    {
        ...
        for (const Sequential::ObjectToPrint &otp : m_objects)
        {
            auto it = std::find_if(m_plates.begin(), m_plates.end(),
                                   [&otp](const auto &plate)
                                   {
                                       return std::any_of(plate.scheduled_objects.begin(),
                                                          plate.scheduled_objects.end(),
            ...
            for (ModelObject *mo : model.objects)
                for (ModelInstance *mi : mo->instances)
                    if (mi->id().id == object.id)
                    {
                        ...
                        mi->set_offset(Vec3d(unscaled(object.x) + bed_offset.x(), unscaled(object.y) + bed_offset.y(),
                                             mi->get_offset().z()));
                    }
        ++plate_idx;
    }
        ...
        for (ModelInstance *mi : mo->instances)
            if (!mi->printable)
            {
                ...
                if (it == s_multiple_beds.get_inst_map().end() ||
                    (m_selected_bed != -1 && it->second != m_selected_bed))
                    continue;
...
std::optional<std::pair<std::string, std::string>> check_seq_conflict(const Model &model, const ConfigBase &config)
{
    ...
    for (const ModelObject *mo : model.objects)
    {
        ...
        for (const ModelInstance *mi : mo->instances)
        {
            ...
            auto it2 = std::find_if(objects.begin(), objects.end(),
                                    [&mi](const Sequential::ObjectToPrint &otp) { return otp.id == mi->id().id; });
            if (it2 == objects.end())
                continue;

            Vec3d offset = s_multiple_beds.get_bed_translation(s_multiple_beds.get_active_bed());
            plate.scheduled_objects.emplace_back(mi->id().id, scaled(mi->get_offset().x() - offset.x()),
                                                 scaled(mi->get_offset().y() - offset.y()));
        }
    }

    std::optional<std::pair<int, int>> conflict = Sequential::check_ScheduledObjectsForSequentialConflict(
        solver_config, printer_geometry, objects, std::vector<Sequential::ScheduledPlate>(1, plate));
    if (conflict)
    {
