...
// arrangement. No dependency on the rest of the engine is introduced here. (No
...
public:
    ...
    [[nodiscard]] std::unique_ptr<ArrangeResult> process(Ctl &&ctl) { return process(ctl); }

    [[nodiscard]] std::unique_ptr<ArrangeResult> process() { return process(DummyCtl{}); }
};
