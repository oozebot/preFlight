...
int calculate_items_needed_to_fill_bed(const ExtendedBed &bed, const ArrItem &prototype_item, size_t prototype_count,
                                       const std::vector<ArrItem> &fixed)
{
    ...
    auto needed_items = static_cast<int>(std::ceil((bed_area - fixed_area) / poly_area));
...
void extract(FillBedTask<ArrItem> &task, const Scene &scene, const ArrangeableToItemConverter<ArrItem> &itm_conv)
{
    ...
    auto collect_task_items = [&prototype_geometry_id, &task, &itm_conv, &bed_constraint](const Arrangeable &arrbl)
    {
        try
        {
        ...
        {
            BOOST_LOG_TRIVIAL(error) << "ObjectID " << std::to_string(arrbl.id().id) << ": " << ex.what();
        }
    };

    scene.model().for_each_arrangeable(collect_task_items);

    int needed_items = calculate_items_needed_to_fill_bed(task.bed, *task.prototype_item, task.selected.size(),
                                                          task.unselected);
...
template<class ArrItem>
std::unique_ptr<FillBedTask<ArrItem>> FillBedTask<ArrItem>::create(const Scene &sc,
...
std::unique_ptr<FillBedTaskResult> FillBedTask<ArrItem>::process_native(Ctl &ctl)
{
    ...
    public:
        ...
        void update_status(int remaining) override { parent.update_status(remaining); }

        bool was_canceled() const override { return parent.was_canceled() || do_stop; }
