...
#include <luminary/core/containers/AnyPtr.hpp>
...
#include <luminary/arrange/packing/PackingContext.hpp>
#include <luminary/arrange/packing/NFPArrangeItemTraits.hpp>
#include <luminary/arrange/packing/NFP.hpp>
#include <luminary/arrange/packing/ArrangeBase.hpp>
#include <luminary/arrange/packing/ArrangeItemTraits.hpp>
#include <luminary/arrange/packing/DataStoreTraits.hpp>

#include <luminary/arrange/scene/MutableItemTraits.hpp>
#include <luminary/arrange/Arrange.hpp>
#include <luminary/arrange/scene/ArrangeTask.hpp>
#include <luminary/arrange/scene/FillBedTask.hpp>
#include <luminary/arrange/scene/MultiplySelectionTask.hpp>
#include <luminary/arrange/scene/ArbitraryDataStore.hpp>
...
{
    return std::all_of(pp.begin(), pp.end(), [](const Polygon &p) { return polygon_is_convex(p); });
}
...
public:
    ...
    explicit DecomposedShape(Polygons sh) : m_shape{std::move(sh)} { assert(check_polygons_are_convex(m_shape)); }
...
public:
    ...
    explicit ArrangeItem(DecomposedShape shape, DecomposedShape envelope)
        : m_shape(std::move(shape)), m_envelope{std::make_unique<DecomposedShape>(std::move(envelope))}
    {
    }
...
{
    static const Vec2crd &get_translation(const ArrangeItem &itm) { return itm.translation(); }

    static double get_rotation(const ArrangeItem &itm) { return itm.rotation(); }

    static int get_bed_index(const ArrangeItem &itm) { return itm.bed_idx(); }

    static int get_priority(const ArrangeItem &itm) { return itm.priority(); }

    static std::optional<int> get_bed_constraint(const ArrangeItem &itm) { return itm.bed_constraint(); }

    // Setters:

    static void set_translation(ArrangeItem &itm, const Vec2crd &v) { itm.translation(v); }

    static void set_rotation(ArrangeItem &itm, double v) { itm.rotation(v); }

    static void set_bed_index(ArrangeItem &itm, int v) { itm.bed_idx(v); }

    static void set_bed_constraint(ArrangeItem &itm, std::optional<int> v) { itm.bed_constraint(v); }
};
...
struct DataStoreTraits_<ArrangeItem>
{
    ...
    static bool has_key(const ArrangeItem &itm, const std::string &key) { return itm.datastore().has_key(key); }
};
...
static Polygons calculate_nfp_unnormalized(const ArrangeItem &item, const Range<FixedIt> &fixed_items,
                                           StopCond &&stop_cond = {})
{
    size_t cap = 0;
    ...
    for (const ArrangeItem &fixed : fixed_items)
    {
        ...
        {
            Point max_fixed = Luminary::reference_vertex(fixed_poly);
            for (size_t mi = 0; mi < item_outlines.size(); ++mi)
            {
                ...
                Vec2crd max_nfp = Luminary::reference_vertex(subnfp);
...
struct NFPArrangeItemTraits_<ArrangeItem>
{
    ...
    static ExPolygons calculate_nfp(const ArrangeItem &item, const Context &packing_context, const Bed &bed,
                                    StopCond &&stopcond)
    {
    ...
    static const Vec2crd &reference_vertex(const ArrangeItem &item) { return item.envelope().reference_vertex(); }

    static BoundingBox envelope_bounding_box(const ArrangeItem &itm) { return itm.envelope().bounding_box(); }

    static BoundingBox fixed_bounding_box(const ArrangeItem &itm) { return itm.shape().bounding_box(); }
    ...
    static const Polygons &envelope_outline(const ArrangeItem &itm) { return itm.envelope().transformed_outline(); }

    static const Polygons &fixed_outline(const ArrangeItem &itm) { return itm.shape().transformed_outline(); }

    static const Polygon &envelope_convex_hull(const ArrangeItem &itm) { return itm.envelope().convex_hull(); }

    static const Polygon &fixed_convex_hull(const ArrangeItem &itm) { return itm.shape().convex_hull(); }
    ...
    static Vec2crd fixed_centroid(const ArrangeItem &itm) { return itm.shape().centroid(); }

    static Vec2crd envelope_centroid(const ArrangeItem &itm) { return itm.envelope().centroid(); }
};
...
struct MutableItemTraits_<ArrangeItem>
{
    ...
    static void set_convex_shape(ArrangeItem &itm, const Polygon &shape) { itm.set_shape(DecomposedShape{shape}); }
    static void set_shape(ArrangeItem &itm, const ExPolygons &shape) { itm.set_shape(decompose(shape)); }
    static void set_convex_envelope(ArrangeItem &itm, const Polygon &envelope)
    {
        itm.set_envelope(DecomposedShape{envelope});
    }
    static void set_envelope(ArrangeItem &itm, const ExPolygons &envelope) { itm.set_envelope(decompose(envelope)); }
