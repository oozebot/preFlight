...
#include <luminary/arrange/packing/NFPConcave_Tesselate.hpp>
...
const Polygons &DecomposedShape::transformed_outline() const
{
    ...
    if (!m_transformed_outline_valid)
    {
        ...
        m_area = std::accumulate(m_transformed_outline.begin(), m_transformed_outline.end(), 0.,
...
const Vec2crd &DecomposedShape::reference_vertex() const
{
    ...
    {
        m_reference_vertex = Luminary::reference_vertex(transformed_outline());
        m_refs.clear();
        m_mins.clear();
        ...
        {
            m_refs.emplace_back(Luminary::reference_vertex(poly));
            m_mins.emplace_back(Luminary::min_vertex(poly));
        }
