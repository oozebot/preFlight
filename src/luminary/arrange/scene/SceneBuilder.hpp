...
public:
    ...
    bool assign_bed(VBedPlaceable &&obj, int bed_idx) { return assign_bed(obj, bed_idx); }
...
class FixedSelection : public Luminary::arr2::SelectionMask
{
...
public:
    ...
    explicit FixedSelection(std::initializer_list<std::vector<bool>> seld, bool wp = false)
    ...
    {
        return obj_id < int(m_seldata.size()) ? m_seldata[obj_id] : std::vector<bool>{};
    }
...
struct ArrangeableWipeTowerBase : public Arrangeable
{
    ...
    ArrangeableWipeTowerBase(
        const ObjectID &objid, Polygon shape, int bed_index,
        SelectionPredicate selection_predicate = [](int) { return false; })
        : oid{objid}, poly{std::move(shape)}, bed_index{bed_index}, selection_pred{std::move(selection_predicate)}
    {
    }
    ...
    bool is_selected() const override { return selection_pred(bed_index); }
    ...
    std::optional<int> bed_constraint() const override { return this->bed_index; }

    void transform(const Vec2d &transl, double rot) override {}

    void imbue_data(AnyWritable &datastore) const override { datastore.write("is_wipe_tower", {}); }
};
...
struct InstPos
{
    size_t obj_idx = 0, inst_idx = 0;
};

using BedConstraints = std::map<ObjectID, int>;

// Implementing ArrangeableModel interface for preFlight's Model, ModelObject, ModelInstance data
// hierarchy
...
// SceneBuilder implementation for preFlight API.
...
public:
    ...
    Transform3d get_physical_bed_trafo(int bed_index) const override { return Transform3d::Identity(); }
...
public:
    GridStriderVBedHandler(const BoundingBox &bedbb, const Vec2crd &gap)
        : m_xstrider{bedbb, gap.x()}, m_ystrider{bedbb, gap.y()}
    {
    }
...
// virtual beds.
void transform_instance(ModelInstance &mi, const Vec2d &transl_unscaled, double rot,
                        const Transform3d &physical_tr = Transform3d::Identity());

BoundingBoxf3 instance_bounding_box(const ModelInstance &mi, bool dont_translate = false);

BoundingBoxf3 instance_bounding_box(const ModelInstance &mi, const Transform3d &tr, bool dont_translate = false);

constexpr double UnscaledCoordLimit = 1000.;

ExPolygons extract_full_outline(const ModelInstance &inst, const Transform3d &tr = Transform3d::Identity());

Polygon extract_convex_outline(const ModelInstance &inst, const Transform3d &tr = Transform3d::Identity());
...
public:
    ...
    void displace(const Vec2d &transl, double rot) override { transform_instance(*m_mi, transl, rot); }
};
...
public:
    explicit ArrangeableModelInstance(InstPtr *mi, VBedHPtr *vbedh, const SelectionMask *selmask, const InstPos &pos,
...
{
    std::remove_reference_t<decltype(std::declval<Mdl>().objects[0]->instances[0])> ret = nullptr;
...
public:
    explicit ArrangeableFullModel(Mdl *mdl, Dup *md, VBH *vbh) : m_mdl{mdl}, m_dup{md}, m_vbh{vbh}
    {
    ...
    int get_bed_index() const override { return m_vbh->get_bed_index(*this); }
    ...
    void displace(const Vec2d &transl, double rot) override { transform(transl, rot); }
};
...
public:
    explicit DuplicableModel(AnyPtr<Model> mdl, AnyPtr<VirtualBedHandler> vbh, const BoundingBox &bedbb);
    ~DuplicableModel();
