...
#include <luminary/arrange/scene/SceneBuilder.hpp>
#include <luminary/arrange/scene/Scene.hpp>
...
coord_t get_skirt_inset(const Print &fffprint)
{
    ...
    {
        float skirtflow = fffprint.objects().empty() ? 0 : fffprint.skirt_flow().width();
        skirt_inset = fffprint.config().skirts.value * skirtflow + fffprint.config().skirt_distance.value;
    }
...
{
    return std::accumulate(m.objects.begin(), m.objects.end(), size_t(0),
                           [](size_t s, const Luminary::ModelObject *mo) { return s + mo->instances.size(); });
}

void transform_instance(ModelInstance &mi, const Vec2d &transl_unscaled, double rot, const Transform3d &physical_tr)
{
...
BoundingBoxf3 instance_bounding_box(const ModelInstance &mi, const Transform3d &tr, bool dont_translate)
{
    BoundingBoxf3 bb;
    const Transform3d inst_matrix = dont_translate ? mi.get_transformation().get_matrix_no_offset()
    ...
    for (ModelVolume *v : mi.get_object()->volumes)
    {
        if (v->is_model_part())
        {
            bb.merge(v->mesh().transformed_bounding_box(tr * inst_matrix * v->get_matrix()));
        }
    }
...
{
    return std::abs(bb.min.x()) < UnscaledCoordLimit && std::abs(bb.min.y()) < UnscaledCoordLimit &&
           std::abs(bb.max.x()) < UnscaledCoordLimit && std::abs(bb.max.y()) < UnscaledCoordLimit;
}
...
ExPolygons extract_full_outline(const ModelInstance &inst, const Transform3d &tr)
{
    ...
    if (check_coord_bounds(to_2d(instance_bounding_box(inst, tr))))
    {
        for (const ModelVolume *v : inst.get_object()->volumes)
        {
            ...
            vol_outline = project_mesh(v->mesh().its, tr * inst.get_matrix() * v->get_matrix(), [] {});
            switch (v->type())
            {
...
{
    bool ret = false;
    visit_bed([&ret](auto &rawbed) { ret = std::is_convertible_v<decltype(rawbed), InfiniteBed>; }, ebed);
...
void SceneBuilder::build_arrangeable_slicer_model(ArrangeableSlicerModel &amodel)
{
    ...
    const bool has_wipe_tower{
        std::any_of(m_wipetower_handlers.begin(), m_wipetower_handlers.end(),
    ...
    {
        wth->set_selection_predicate([&amodel](int wipe_tower_index)
...
Transform3d GridStriderVBedHandler::get_physical_bed_trafo(int bed_idx) const
{
    ...
    Transform3d ret = m_xstrider.get_physical_bed_trafo(crd.x()) * m_ystrider.get_physical_bed_trafo(crd.y());
...
{
    auto ret = Luminary::reserve_vector<bool>(m_seldata.size());
    std::transform(m_seldata.begin(), m_seldata.end(), std::back_inserter(ret),
                   [](auto &a) { return std::any_of(a.begin(), a.end(), [](bool b) { return b; }); });
    return ret;
}
...
void ArrangeableSlicerModel::for_each_arrangeable(std::function<void(Arrangeable &)> fn)
{
...
void ArrangeableSlicerModel::for_each_arrangeable(std::function<void(const Arrangeable &)> fn) const
{
    for_each_arrangeable_(*this, fn);
...
std::optional<int> get_bed_constraint(const ObjectID &id, const BedConstraints &bed_constraints)
{
...
bool should_include_instance(const ObjectID &instance_id, const std::set<ObjectID> &considered_instances)
{
...
void ArrangeableSlicerModel::visit_arrangeable(const ObjectID &id, std::function<void(const Arrangeable &)> fn) const
{
    visit_arrangeable_(*this, id, fn);
}

void ArrangeableSlicerModel::visit_arrangeable(const ObjectID &id, std::function<void(Arrangeable &)> fn)
{
    visit_arrangeable_(*this, id, fn);
}
...
bool ArrangeableModelInstance<InstPtr, VBedHPtr>::is_selected() const
{
    ...
    if (m_selmask)
    {
        ...
        if (m_pos_within_model.inst_idx < sel.size() && sel[m_pos_within_model.inst_idx])
            ret = true;
    }
...
ObjectID ArrangeableFullModel<Mdl, Dup, VBH>::geometry_id() const
{
    return m_mdl->id();
}
