...
#if defined(PREFLIGHT_DEBUG) || defined(PREFLIGHT_DEBUG_SLICE_PROCESSING)
#include "luminary/geometry/diagnostics/SVG.hpp"
// debug_out_path names the file each dump below writes; only these guarded blocks call it.
...
#if (!defined(NDEBUG) || defined(PREFLIGHT_DEBUG_SLICE_PROCESSING))
    ...
    IntersectionLine &line_out)
{
    ...
    if (num_points == 2)
    {
        ...
        line_out.color = facet_color;
        // Rounding the double intersection points into coord_t can collapse an edge to zero length,
        // so the endpoints are not asserted distinct.
...
struct OpenPolyline
{
    ...
    OpenPolyline(const IntersectionReference &start, const IntersectionReference &end, Points &&points,
        ...
        , length(Luminary::length(this->points))
        , consumed(false)
    {
    }
    void reverse()
...
#ifdef PREFLIGHT_TRIANGLEMESH_DEBUG
...
std::vector<OpenPolyline *> open_polylines_sorted(std::vector<OpenPolyline> &open_polylines, bool update_lengths)
{
    ...
    for (OpenPolyline &opl : open_polylines)
        ...
        {
            if (update_lengths)
                opl.length = Luminary::length(opl.points);
                                            ...
                                            bool try_connect_reversed)
{
    ...
    for (OpenPolyline *opl : sorted_by_length)
    {
        ...
        for (;;)
        {
            ...
            bool loop_closed = current_loop_closing_distance2 < Luminary::sqr(coordf_t(max_gap_scaled));
...
// Canonical order for the intersection lines of one layer, independent of the thread order
// they were collected in.
static void sort_intersection_lines(IntersectionLines &lines)
{
    std::sort(lines.begin(), lines.end(),
              [](const IntersectionLine &l, const IntersectionLine &r)
              {
                  if (l.a != r.a)
                      return l.a.x() < r.a.x() || (l.a.x() == r.a.x() && l.a.y() < r.a.y());
                  if (l.b != r.b)
                      return l.b.x() < r.b.x() || (l.b.x() == r.b.x() && l.b.y() < r.b.y());
                  if (l.edge_a_id != r.edge_a_id)
                      return l.edge_a_id < r.edge_a_id;
                  if (l.edge_b_id != r.edge_b_id)
                      return l.edge_b_id < r.edge_b_id;
                  if (l.a_id != r.a_id)
                      return l.a_id < r.a_id;
                  return l.b_id < r.b_id;
              });
}
...
static typename PolygonsType<mesh_info>::type make_loops(
    ...
    IntersectionLines &lines,
    // Number of open polylines the chaining left for the closers (debug telemetry).
    size_t *open_pieces = nullptr)
{
    ...
    // The lines of a layer are collected from a parallel pass over the facets, so their order
    // depends on thread timing. Chaining starts loops from the first unused line and closes
    // open pieces in encounter order, so the order decides each loop's start vertex and, on
    // meshes with open edges, which pieces get joined. Sort by geometry first so every run
    // chains the same sequence.
    sort_intersection_lines(lines);
    std::vector<OpenPolyline> open_polylines;
    chain_lines_by_triangle_connectivity<mesh_info>(lines, loops, open_polylines);
    if (open_pieces)
        *open_pieces = open_polylines.size();
...
static std::vector<typename PolygonsType<mesh_info>::type> make_loops(
    ...
    // Per-layer count of open polylines the chaining left for the closers (debug telemetry).
    std::vector<size_t> *open_counts = nullptr)
{
    using PolygonsType = typename PolygonsType<mesh_info>::type;
    ...
    if (open_counts)
        open_counts->assign(lines.size(), 0);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, lines.size()),
                      [&lines, &layers, &params, throw_on_cancel, open_counts](const tbb::blocked_range<size_t> &range)
                      {
                          for (size_t line_idx = range.begin(); line_idx < range.end(); ++line_idx)
                          {
                              ...
                              polygons = make_loops<mesh_info>(lines[line_idx],
                                                               open_counts ? &(*open_counts)[line_idx] : nullptr);
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(lines.at_slice.size())),
                    ...
                    // Edges in between slice_below and slice_above.
                                  ...
                                  if (!in.empty())
                                  {
                                      ...
                                      std::vector<OpenPolyline> open_polylines;
                                      // Same reason as in make_loops: the collected line order is
                                      // thread dependent and the chaining is order dependent.
                                      sort_intersection_lines(in);
                            ...
                            Clipper2Lib::FillRule fill_type, ExPolygons *slices)
{
    ...
    */
    ...
    // Closing offset to merge facets that nearly touch.
    ...
    // Detect thin walls in the raw polygon loops before union_ex merges them.
        ...
        for (const Polygon *hole : cw_loops)
        {
            ...
            for (const Polygon *contour : ccw_loops)
            {
                ...
                if (hbb.min.x() > cbb.max.x() + margin || hbb.max.x() < cbb.min.x() - margin ||
                    ...
                    continue;
                // CW loop vertices against CCW loop segments
    ...
    const std::vector<float> &zs, const MeshSlicingParams &params, std::function<void()> throw_on_cancel)
{
    ...
    // Layers where the chaining left open pieces for the closers: the closed geometry there
    // depends on the line order, which is now canonical, so these are the layers that may
    // differ from a build with the old thread-dependent order.
    // Always counted (one increment per open piece) so the SLICE_OPEN_LOOPS counter does not
    // depend on which --debug categories are on.
    std::vector<size_t> open_counts;
    std::vector<PolygonsType> layers = make_loops<mesh_info>(lines, params, throw_on_cancel, &open_counts);
    for (size_t i = 0; i < open_counts.size() && i < zs.size(); ++i)
        if (open_counts[i] > 0)
        {
            DBG_COUNT("SLICE_OPEN_LOOPS");
            dbg_log(DBG_PERIMETERS, double(zs[i]), "DETERM", "SLICE_OPEN_LOOPS layer=%zu pieces=%zu", i,
                    open_counts[i]);
        }
...

    {
...

        {
            ...
            else
            {
                ...
                for (size_t i = 0; i < mesh.vertices.size(); ++i)
                {
                    // Only z is used for the side test.
                      ...
                      [&layers_p, &params, &layers, throw_on_cancel](const tbb::blocked_range<size_t> &range)
                      {
                          ...
                          for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id)
                          {
                              ...
                              const auto this_mode = layer_id < params.slicing_mode_normal_below_layer
                                                         ? params.mode_below
                                                         : params.mode;
                              Luminary::make_expolygons(layers_p[layer_id], params.closing_radius, params.extra_offset,
...
Polygons project_mesh(const indexed_triangle_set &mesh, const Transform3d &trafo, std::function<void()> throw_on_cancel)
{
    ...
    // The facet projections are collected from a parallel loop, so their order is thread
    // dependent, and Clipper rounds intersection vertices per union, so the union of many
    // overlapping polygons depends on that order. Sort them canonically first.
    auto canonical = [](Polygons &polys)
    {
                  ...
                  [](const Polygon &a, const Polygon &b)
                  {
                      ...
                      for (size_t i = 0; i < a.points.size(); ++i)
                      {
                          if (a.points[i].x() != b.points[i].x())
                              return a.points[i].x() < b.points[i].x();
                          if (a.points[i].y() != b.points[i].y())
                              return a.points[i].y() < b.points[i].y();
                      }
                      return false;
                  });
    };
    canonical(top.front());
    canonical(bottom.back());
    ...
    Polygons out = union_(union_parallel_reduce(top.front()), union_parallel_reduce(bottom.back()));
    if (debug_enabled(DBG_PERIMETERS))
    {
        // Run-to-run fingerprints of the projection input (sorted) and output, so a differing
        // object outline can be attributed to the projection or to the union.
        auto fp = [](const Polygons &polys, size_t &pts)
        {
        ...
        size_t nt = 0, nb = 0, no = 0;
        const uint64_t ht = fp(top.front(), nt), hb = fp(bottom.back(), nb), ho = fp(out, no);
        dbg_log(DBG_PERIMETERS, 0., "DETERM", "PROJ_FP top=%zu/%zu/%016llx bottom=%zu/%zu/%016llx out=%zu/%zu/%016llx",
                top.front().size(), nt, (unsigned long long) ht, bottom.back().size(), nb, (unsigned long long) hb,
                out.size(), no, (unsigned long long) ho);
    }
    return out;
}
