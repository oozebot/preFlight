...
struct MeshSlicingParams
{
    enum class SlicingMode : uint32_t
    {
        ...
        // slice_mesh_ex() applies FillRule::NonZero rule to the result of slice_mesh().
        Regular,
        // For slicing 3DLabPrints plane models (aka to be compatible with S3D default strategy).
        // slice_mesh_ex() applies FillRule::EvenOdd rule. slice_mesh() slices EvenOdd as Regular.
        EvenOdd,
        // Maintain all contours, orient all contours CCW.
        // slice_mesh_ex() applies FillRule::NonZero rule, thus holes will be closed.
        Positive,
...
std::vector<Polygons> slice_mesh(
    const indexed_triangle_set &mesh, const std::vector<float> &zs, const MeshSlicingParams &params,
    std::function<void()> throw_on_cancel = [] {});

std::vector<ColorPolygons> slice_mesh(
    const indexed_triangle_set_with_color &mesh, const std::vector<float> &zs, const MeshSlicingParams &params,
    std::function<void()> throw_on_cancel = [] {});

// Specialized version for a single slicing plane only, running on a single thread.
Polygons slice_mesh(const indexed_triangle_set &mesh, float plane_z, const MeshSlicingParams &params);

ColorPolygons slice_mesh(const indexed_triangle_set_with_color &mesh, float plane_z, const MeshSlicingParams &params);

std::vector<ExPolygons> slice_mesh_ex(
    const indexed_triangle_set &mesh, const std::vector<float> &zs, const MeshSlicingParamsEx &params,
    std::function<void()> throw_on_cancel = [] {});

inline std::vector<ExPolygons> slice_mesh_ex(
    const indexed_triangle_set &mesh, const std::vector<float> &zs, std::function<void()> throw_on_cancel = [] {})
{
    return slice_mesh_ex(mesh, zs, MeshSlicingParamsEx{}, throw_on_cancel);
}

inline std::vector<ExPolygons> slice_mesh_ex(
    const indexed_triangle_set &mesh, const std::vector<float> &zs, float closing_radius,
    std::function<void()> throw_on_cancel = [] {})
{
...
Polygons project_mesh(const indexed_triangle_set &mesh, const Transform3d &trafo,
                      std::function<void()> throw_on_cancel);
