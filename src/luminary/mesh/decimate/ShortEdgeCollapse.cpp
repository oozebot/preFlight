...
#include "luminary/mesh/decimate/NormalUtils.hpp"
...
void its_short_edge_collpase(indexed_triangle_set &mesh, size_t target_triangle_count)
{
    ...
    // This score is not updated, even though the decimation does change the mesh. It saves computation time, and there are no strong reasons to update.
    ...
    auto remove_face = [&triangles_neighbors, &face_removal_flags](int face_idx, int other_face_idx)
    {
        ...
        if (n_a > 0)
            for (coord_t &n : triangles_neighbors[n_a])
            {
                if (n == face_idx)
                {
                    n = n_b;
        ...
        if (n_b > 0)
            for (coord_t &n : triangles_neighbors[n_b])
            {
                if (n == face_idx)
                {
                    n = n_a;
    ...
    float decimation_ratio =
        1.0f; // decimation ratio updated in each iteration. it is number of removed triangles / number of all
