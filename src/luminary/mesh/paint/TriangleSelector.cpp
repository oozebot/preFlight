...
{
    const float sphere_radius_sqr = Luminary::sqr(sphere_radius);
                                      ...
                                      const Vec3f &cylinder_Q, const float cylinder_radius)
{
    ...
    const float eq_c = d_dot_d * (m_dot_m - Luminary::sqr(cylinder_radius)) - m_dot_d * m_dot_d;
...
std::pair<int, int> TriangleSelector::triangle_subtriangles(int itriangle, int vertexi, int vertexj) const
{
    return itriangle == -1
               ? std::make_pair(-1, -1)
               : Luminary::TriangleSelector::triangle_subtriangles(m_triangles[itriangle], vertexi, vertexj);
}
...
{
    m_edge_limit_sqr = Luminary::sqr(edge_limit);
}
...
TriangleSelector::TriangleSplittingData TriangleSelector::serialize() const
{
    ...
    // An explicit function object supports the recursive call of Serializer::serialize() and calls
    // directly, where a type-erased std::function would call through a pointer.
    struct Serializer
    {
...
void TriangleSelector::deserialize(const TriangleSplittingData &data, bool needs_reset)
{
    ...
    for (auto [triangle_id, ibit] : data.triangles_to_split)
    {
        ...
        // Painting saved against another mesh (a split or replaced volume) indexes past this
        // one; the entry is dropped instead of written past m_triangles.
        if (triangle_id < 0 || triangle_id >= int(m_triangles.size()) || ibit < 0 || ibit >= bitstream_size)
        {
            DBG_COUNT("PAINT_DESERIALIZE_OUT_OF_RANGE");
            continue;
        }
        ...
        while (true)
        {
            ...
            if (code < 0)
            {
                DBG_COUNT("PAINT_DESERIALIZE_TRUNCATED");
            ...
            if (!is_split)
            {
                if ((code & 0b1100) == 0b1100)
                {
                    ...
                    if (second == 15 && ibit + 16 <= bitstream_size)
                    {
                        ...
                        {
                            DBG_COUNT("PAINT_STATE_OVER_CAP");
    ...
    : source{source_}, trafo{trafo_.cast<float>()}, clipping_plane{clipping_plane_}
{
    ...
    if (is_approx(sf.x(), sf.y()) && is_approx(sf.y(), sf.z()))
    {
        ...
        radius_sqr = float(Luminary::sqr(radius_world / sf.x()));
    ...
    else
    {
        ...
        radius = radius_world;
        radius_sqr = Luminary::sqr(radius_world);
...
bool TriangleSelector::HeightRange::is_mesh_point_inside(const Vec3f &point) const
{
    ...
    return Luminary::is_in_range<float>(transformed_point_z, m_z_range_bottom, m_z_range_top);
}
