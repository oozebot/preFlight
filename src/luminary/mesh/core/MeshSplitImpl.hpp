...
std::vector<Vec3i> create_face_neighbors_index(ExPolicy &&ex, const indexed_triangle_set &its)
{
    ...
    // Pass 1, parallel and confined to the face's own row: for every edge the lowest higher
    // face sharing it with the opposite orientation. Writing the partner's row from here raced
    // with the partner's own worker on edges shared by three or more faces, so the pairing
    // differed run to run.
    std::vector<Vec3i> candidates(indices.size(), Vec3i(coord_t(no_value), coord_t(no_value), coord_t(no_value)));
    execution::for_each(
        ex, size_t(0), indices.size(),
        [&candidates, &indices, &vertex_triangles](size_t face_idx)
        {
            Vec3i &candidate = candidates[face_idx];
            ...
            for (int edge_index = 0; edge_index < 3; ++edge_index)
            {
                ...
                for (const size_t other_face : vertex_triangles[edge_indices[0]])
                {
                    ...
                    if (edge_indices[0] != face_indices[(vertex_index + 1) % 3])
                        continue;
                    candidate[edge_index] = other_face;
                    break;
                }
            }
        },
    ...
    // Pass 2, sequential in face order: an edge already paired by a lower face keeps that
    // pairing, otherwise the face takes its candidate and writes itself into the candidate's row.
    ...
    for (size_t face_idx = 0; face_idx < indices.size(); ++face_idx)
        for (int edge_index = 0; edge_index < 3; ++edge_index)
        {
            coord_t &neighbor_edge = neighbors[face_idx][edge_index];
            if (neighbor_edge != no_value)
                continue;
            const coord_t other_face = candidates[face_idx][edge_index];
            if (other_face == no_value)
                continue;
            const Vec2i edge_indices = its_triangle_edge(indices[face_idx], edge_index);
            neighbor_edge = other_face;
            neighbors[other_face][its_triangle_vertex_index(indices[other_face], edge_indices[1])] = face_idx;
        }
