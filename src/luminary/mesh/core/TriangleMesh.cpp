...
{
    BoundingBoxf3 bbox = Luminary::bounding_box(its);
...
static void trianglemesh_repair_on_import(stl_file &stl)
{
    ...
    // checking exact
#ifdef PREFLIGHT_TRACE_REPAIR
    BOOST_LOG_TRIVIAL(trace) << "\tstl_check_faces_exact";
#endif /* PREFLIGHT_TRACE_REPAIR */
    ...
    // Repair can invalidate face connectivity, which slicing reads; rebuild it when facets were degenerate.
...
void TriangleMesh::rotate(float angle, const Axis &axis)
{
    if (angle != 0.f)
    {
        angle = Luminary::Geometry::rad2deg(angle);
        switch (axis)
        {
        case X:
...
// Unions the projection of every facet, so the cost grows with the facet count.
...
Polygon TriangleMesh::convex_hull()
{
    ...
    return Luminary::Geometry::convex_hull(pp);
}
    ...
    for (int i = 0; i < int(face_neighbors.size()); ++i)
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
            ...
            if (n > i)
            {
                ...
                // A flipped neighbour holds the shared edge in the same orientation; both get the same edge ID.
                if (k == -1)
                {
...
void its_merge(indexed_triangle_set &its, indexed_triangle_set &&its_add)
{
    ...
    Luminary::append(verts, std::move(its_add.vertices));
    ...
    for (auto &face : its_add.indices)
        for (int i = 0; i < 3; ++i)
            face[i] += offset;
    Luminary::append(its.indices, std::move(its_add.indices));
}
                         ...
                         const std::vector<stl_vertex> &vertices)
{
    ...
    // A full disk shows up as a stream error or a failed close, not at fopen.
    const bool written = ferror(fp) == 0;
