...
// Every module returns a value in [-1, 1]; the consumer multiplies it by the fuzzy thickness. The
// ridged and cellular formulas overshoot on a small share of samples and are clamped here.
inline double clamp_noise(const double v)
{
    if (v < -1.0 || v > 1.0)
    {
        DBG_COUNT("FUZZY_NOISE_CLAMPED");
...
public:
    ...
    double getValue(double x, double y, double z) const override
    {
        // The module frequency scales the input once; the octave frequency is relative to it, so the
        // spectral weights (frequency^-0.9) are the same at every feature scale.
        x *= m_frequency;
        y *= m_frequency;
        z *= m_frequency;
        ...
        return clamp_noise(result * 1.25 - 1.0);
    }
...
public:
    ...
    double getValue(double x, double y, double z) const override
    {
        ...
        // Distance to the nearest feature point, mapped so one cell width reaches 1
        return clamp_noise(std::sqrt(minDist) * m_displacement * 2.0 - 1.0);
    }
