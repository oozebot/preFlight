...
public:
    ...
    // The semicircular ends have radius = layer_height / 2. Adjacent beads bond over their whole
    // face when they overlap by (1 - pi/4), about 21.46% of the layer height.
    //
    // The user-facing percentage is halved so that 100% = complete overlap, and doubled again
    // internally:
    //   - User sees 10.73% (the bonding value), internally 21.46% of height
    ...
    {
        // (1 - pi/4) / 2 * 100 = 10.73% (user-facing value, doubled internally)
        return (1.0 - 0.25 * M_PI) * 50.0; // about 10.73%
    }
