...
const constexpr coord_t POINT_IS_ON_LINE_THRESHOLD_SQR = Luminary::sqr(scaled<coord_t>(EPSILON));
                                                        ...
                                                        const ClipperZUtils::ZPath &subject, const size_t region_idx)
{
    ...
    for (ClipperZUtils::ZPoint &clipper_pt : intersection)
    {
        ...
        // A clip point lands inside the intersection rarely, so a linear search for the source line is enough.
                                          ...
                                          const size_t region_config_idx)
{
    ...
    // In Clipper2, bot and top refer to Y ordering, not path ordering, so the line segment index is
    // min(bot.point_index, top.point_index): the first vertex of the segment in the original path
    // order.
                                      ...
                                      const size_t default_clip_idx = 0)
{
    ...
    for (const ExPolygons &expolygons_clip : expolygons_clips)
    {
        ...
        Luminary::append(line_region_ranges,
