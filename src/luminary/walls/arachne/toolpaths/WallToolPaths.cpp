...
#include "luminary/walls/arachne/skeleton/SkeletalTrapezoidation.hpp"
#include "luminary/walls/arachne/predicates/linearAlg2D.hpp"
#include "luminary/walls/arachne/spatial/SparseLineGrid.hpp"
#include "luminary/geometry/transform/Geometry.hpp"
#include "luminary/walls/arachne/paths/PolylineStitcher.hpp"
#include "luminary/geometry/clipper/ClipperUtils.hpp"
#include "luminary/walls/arachne/beading/BeadingStrategy.hpp"
...
#include "luminary/geometry/index/arachne/PolygonsPointIndex.hpp"
...
WallToolPaths::WallToolPaths(const Polygons &outline, const coord_t bead_width_0, const coord_t bead_width_x,
    ...
    , print_thin_walls(Luminary::Arachne::fill_outline_gaps)
...
const std::vector<VariableWidthLines> &WallToolPaths::generate()
{
    ...
    const coord_t smallest_segment = Luminary::Arachne::meshfix_maximum_resolution;
    const coord_t allowed_distance = Luminary::Arachne::meshfix_maximum_deviation;
    ...
    // Step 3: NOW convert to ExPolygons and apply union (reconstructs hole structure)
    ...
    // Filter out degenerate polygons that break Voronoi: The Voronoi diagram generator is very
    // sensitive to polygons with < 3 vertices or zero/near-zero area. These cause "missing Voronoi
    // vertex" errors and infinite loops.
    ...
    {
        if (poly.size() < 3 || std::abs(poly.area()) < min_area)
            continue;
    ...
    const auto beading_strat = BeadingStrategyFactory::makeStrategy(bead_width_0, bead_width_x, wall_transition_length,
                                                                    transitioning_angle, print_thin_walls,
                                                                    min_bead_width, min_feature_size,
                                                                    wall_split_middle_threshold,
                                                                    wall_add_middle_threshold, max_bead_count,
...
void WallToolPaths::simplifyToolPaths(std::vector<VariableWidthLines> &toolpaths)
{
    ...
    {
        const int64_t maximum_resolution = Luminary::Arachne::meshfix_maximum_resolution;
        const int64_t maximum_deviation = Luminary::Arachne::meshfix_maximum_deviation;
        const int64_t maximum_extrusion_area_deviation =
            Luminary::Arachne::meshfix_maximum_extrusion_area_deviation; // unit: um^2
...
void WallToolPaths::separateOutInnerContour()
{
    ...
    inner_contour = union_unknown_winding(inner_contour);
}
