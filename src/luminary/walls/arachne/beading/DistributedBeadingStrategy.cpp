...
DistributedBeadingStrategy::DistributedBeadingStrategy(
    const coord_t optimal_width, const coord_t default_transition_length, const double transitioning_angle,
    const double wall_split_middle_threshold, const double wall_add_middle_threshold, const int distribution_radius)
    : BeadingStrategy(optimal_width, wall_split_middle_threshold, wall_add_middle_threshold, default_transition_length,
                      transitioning_angle)
{
                                                                       ...
                                                                       const coord_t bead_count) const
{
    ...
    const auto calc_weight = [middle, &self = std::as_const(*this)](const size_t bead_idx) -> float
    {
        ...
        return std::max(0.f, 1.f - self.one_over_distribution_radius_squared * Luminary::sqr(dev_from_middle));
    };
    ...
    const float max_allowed_weight = -static_cast<float>(optimal_width) / static_cast<float>(to_be_divided) /
                                     total_weight;
                                                                        ...
                                                                        coord_t bead_count) const
{
    ...
    if (bead_count > 2)
    {
        ...
        for (size_t bead_idx = 0; bead_idx < bead_count; ++bead_idx)
        {
            const coord_t splitup_left_over_weight = static_cast<coord_t>(static_cast<float>(to_be_divided) *
                                                                          normalized_weights[bead_idx]);
            const coord_t width = (bead_idx == bead_count - 1)
                                      ? thickness - accumulated_width
                                      : std::max(coord_t(0), optimal_width + splitup_left_over_weight);
            ...
            {
                ret.toolpath_locations.emplace_back(ret.toolpath_locations.back() +
                                                    (ret.bead_widths.back() + width) / 2);
            }
...
coord_t DistributedBeadingStrategy::getOptimalBeadCount(coord_t thickness) const
{
    ...
    const coord_t minimum_line_width = optimal_width *
