...
BeadingStrategy::Beading RedistributeBeadingStrategy::compute(coord_t thickness, coord_t bead_count) const
{
    ...
    // The vectors are built in order rather than insert()-ed at begin(), which would shift every
    // element on each insertion.
    {
        ...
        new_widths.push_back(actual_outer_thickness);
        new_locations.push_back(actual_outer_thickness / 2);
        ...
        {
            new_widths.push_back(actual_outer_thickness);
            new_locations.push_back(thickness - actual_outer_thickness / 2);
        }
    ...
    ret.total_thickness = thickness;
    ret.left_over = thickness -
                    std::accumulate(ret.bead_widths.cbegin(), ret.bead_widths.cend(), static_cast<coord_t>(0));
    return ret;
}
