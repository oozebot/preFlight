...
struct ExtrusionLine
{
    ...
    /*!
     * Computes and returns the total area error (in um^2) of the AB and BC segments of an ABC straight ExtrusionLine
...
static inline Luminary::ThickPolyline to_thick_polyline(const Arachne::ExtrusionLine &line_junctions)
{
    assert(line_junctions.size() >= 2);
    Luminary::ThickPolyline out;
...
static inline Luminary::ThickPolyline to_thick_polyline(const ClipperZUtils::ZPath &path)
{
    assert(path.size() >= 2);
    Luminary::ThickPolyline out;
...
void extrusion_paths_append(ExtrusionPaths &dst, const Arachne::ExtrusionLine &extrusion, const ExtrusionRole role,
                            const Flow &flow, uint32_t perimeter_index);
