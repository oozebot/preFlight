...
#include "luminary/walls/arachne/spatial/SparsePointGrid.hpp"
...
public:
                       ...
                       coord_t snap_distance = scaled<coord_t>(0.01))
    {
        ...
        for (size_t line_idx = 0; line_idx < lines.size(); line_idx++)
        {
            ...
            {            // Implementation only works for this order; the chain is re-reversed only when it is closed.
                ...
                if (closest_is_closing_polygon)
                {
                    ...
                    { // re-reverse chain to retain original direction
                        // not sure if this code could ever be reached, since if a polygon can be closed that should be already possible in the forward direction
                        chain.reverse();
                    }
