...
namespace Luminary
{
// Stage timers, defined in PerimeterGenerator.cpp.
extern PerfAccumTimer g_st_central, g_st_bead_count, g_st_noncentral, g_st_trans_ribs, g_st_extra_ribs, g_st_seg_init,
...
SkeletalTrapezoidation::SkeletalTrapezoidation(const Polygons &polys, const BeadingStrategy &beading_strategy,
                                               ...
                                               coord_t ext_perimeter_spacing, double debug_print_z, int debug_layer_id)
...
void SkeletalTrapezoidation::constructFromPolygons(const Polygons &polys)
{
    ...
    if (dropped_vertices > 0 || dropped_polys > 0)
        dbg_log(Luminary::DBG_PERIMETERS, debug_print_z, "SKEL", "CLEAN dropped_vertices=%zu dropped_polys=%zu",
    ...
    if (has_missing_twin())
    {
        ...
        if (voronoi_diagram.get_state() != VD::State::REPAIR_UNSUCCESSFUL)
        {
            ...
            for (const double fix_angle : fix_angles)
            {
                ...
                dbg_log(Luminary::DBG_PERIMETERS, debug_print_z, "SKEL", "TWIN_REPAIR angle=%.4f issue=%d accepted=%d",
        ...
        if (!repaired)
        {
            ...
            dbg_log(Luminary::DBG_PERIMETERS, debug_print_z, "SKEL", "TWIN_GIVEUP polys=%zu area_mm2=%.3f",
                                               ...
                                               bool filter_outermost_central_edges)
{
    ...
    Luminary::g_st_central.add(st_t0, st_t1);

    updateBeadCount();
    Luminary::g_st_bead_count.add(st_t1, std::chrono::steady_clock::now());
    ...
    Luminary::g_st_noncentral.add(st_t2, std::chrono::steady_clock::now());
    ...
    Luminary::g_st_trans_ribs.add(st_t3, std::chrono::steady_clock::now());
    ...
    generateExtraRibs();
    Luminary::g_st_extra_ribs.add(st_t4, std::chrono::steady_clock::now());
...
void SkeletalTrapezoidation::updateBeadCount()
{
    ...
    // Set the bead count at locally maximal R, including inside central regions.
...
{
    // Only the transition mid at the near end of this edge is consulted; mids further along the recursion are not.
...
{
    auto st_seg_t0 = std::chrono::steady_clock::now();
    ...
    { // Store beading
        // Nodes beaded by interpolating two bead counts: the only nodes whose widths depend
        // on the transition ramp rather than on a single compute() result.
        ...
        {
            DBG_COUNT("TRANSITION_BEADING");
            dbg_log(Luminary::DBG_PERIMETERS, debug_print_z, "SKEL", "TRANSITION_BEADING layer=%d nodes=%zu",
                    debug_layer_id, transition_nodes);
        }
    }
    ...
    auto st_seg_t1 = std::chrono::steady_clock::now();
    Luminary::g_st_seg_init.add(st_seg_t0, st_seg_t1);
    ...
    auto st_seg_t2 = std::chrono::steady_clock::now();
    Luminary::g_st_seg_propagate.add(st_seg_t1, st_seg_t2);
    generateJunctions(node_beadings, edge_junctions);
    auto st_seg_t3 = std::chrono::steady_clock::now();
    Luminary::g_st_seg_junctions.add(st_seg_t2, st_seg_t3);
    ...
    Luminary::g_st_seg_connect.add(st_seg_t3, std::chrono::steady_clock::now());
                                                                    ...
                                                                    coord_t switching_radius) const
{
    ...
    // The merged beading takes the thicker side's bead count, which can be smaller than the
    // left side's when the strategy stack caps the count. The inset then has no counterpart to
    // solve a new ratio against; the merged result already holds the lower count. When the
    // merged beading is the left side the index is in range and the compare below is false by
    // construction (the loop stopped on a location under the switching radius), so only the
    // short-merged case is guarded and counted.
    if (size_t(next_inset_idx) >= ret.toolpath_locations.size())
    {
        DBG_COUNT("ST_INTERPOLATE_INSET_OOB");
        return ret;
    }
                                               ...
                                               ptr_vector_t<LineJunctions> &edge_junctions)
{
    for (edge_t &edge_ : graph.edges)
    {
        ...
        // At most one junction per toolpath location: one allocation instead of a growth
        // sequence per edge.
        ret.reserve(num_junctions);
        size_t junction_idx;
        ...
        for (junction_idx = start_junction_idx; junction_idx < num_junctions; junction_idx--)
        {
            ...
            // Those rounding errors can make the middle extrusion go missing.
             ...
             junction_idx--) //When junction_idx underflows, it'll be more than num_junctions too.
        {
            ...
            if (Luminary::debug_enabled(Luminary::DBG_PERIMETERS) && beading->preserve_innermost_position)
            {
                dbg_log(Luminary::DBG_PERIMETERS, debug_print_z, "SKEL",
