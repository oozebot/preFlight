...
BeadingStrategyPtr BeadingStrategyFactory::makeStrategy(
    ...
    // The outer wall inset strategy it selected is gone: every caller passes zero, so it never ran.
    // The parameter keeps its place rather than its name, because the one caller passes twenty-odd
    // positional arguments and removing one would shift every argument after it.
    ...
    const coord_t max_bead_width_external, const double debug_print_z)
{
    ...
    // Outermost: absorb the fixed-width stack's left-over thickness into the center
    // bead(s), so compute() returns final widths and positions. Sits above Limited
    // because the pair flanking its 0-width marker wall is the innermost pair to adjust.
    {
        const coord_t absorb_ext_width = (ext_perimeter_width > 0) ? ext_perimeter_width : ext_perimeter_spacing;
        auto absorb = std::make_unique<CenterAbsorbBeadingStrategy>(std::move(ret), absorb_ext_width,
                                                                    ext_perimeter_spacing);
        absorb->set_debug_context(debug_print_z, layer_id);
        ret = std::move(absorb);
    }
    return ret;
}
