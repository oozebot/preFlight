                                                                        ...
                                                                        coord_t bead_count) const
{
    ...
    // Widths are fixed, so their sum plus left_over is not the thickness; left_over is the
    // slot remainder that the outermost strategy absorbs. The structural invariant is that
    // widths and locations are parallel and the locations are ordered.
    assert(ret.bead_widths.size() == ret.toolpath_locations.size());
    assert(std::is_sorted(ret.toolpath_locations.begin(), ret.toolpath_locations.end()));
