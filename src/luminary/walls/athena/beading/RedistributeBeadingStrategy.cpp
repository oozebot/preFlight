...
BeadingStrategy::Beading RedistributeBeadingStrategy::compute(coord_t thickness, coord_t bead_count) const
{
    ...
    if (ext_to_first_internal_spacing > 0 && bead_count >= 2)
    {
        ...
        // Uses the spacing-based leftover, which assumes a symmetric layout.
    ...
    else
    {
        ...
        // The vectors are built in order rather than insert()-ed at begin(), which would shift
        // every element on each insertion.
