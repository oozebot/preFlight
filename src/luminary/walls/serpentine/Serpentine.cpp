...
#include "luminary/geometry/medial_axis/MedialAxis.hpp"
...
public:
    ...
    {
        if (!Luminary::debug_enabled(Luminary::DBG_SERPENTINE))
            return;
        char buf[512];
        va_list args;
        va_start(args, fmt);
    ...
    void flush()
    {
        if (!Luminary::debug_enabled(Luminary::DBG_SERPENTINE) || m_text.empty())
            return;
              ...
              ExPolygons *out_interior)
{
    ...
    if (!anchor.is_point && params.band_clamp == 0 && params.depth_clamp == 0.)
    {
        ...
        ExPolygons capsule = union_ex(
            offset(spine_fit, (float) r_track, JoinType::Round, DefaultLineMiterLimit, EndType::Round));
    ...
    if (params.band_clamp == 0 && params.depth_clamp == 0. && !island.holes.empty())
    {
        ...
        for (const Polygon &h : island.holes)
        {
            ...
            Polygons grown = offset(v, (float) (0.5 * bw), JoinType::Miter, 2.0);
            if (grown.size() == 1)
            {
    ...
    {
        Polygons inner = offset(work_island.contour, -(float) (bw - ov), JoinType::Miter, 2.0);
    ...
    for (size_t loop_idx = 0; loop_idx < loops_src.size(); ++loop_idx)
    {
        ...
        // A thin bent wall keeps a few slits alive by the luck of the layer's phase (2 of 44 on
        // one layer, none on the next): a tour of one to five teeth on a long contour is a plain
        // outline with dents, and the island flips between it and ordinary walls layer by layer.
        // Fewer than eight teeth where four times as many slits were laid is not a pattern; the
        // island prints as walls on every layer. A sparse real tour (60 teeth of 320 slits on a
        // large thin box) keeps its count above the bar. Relaxed and band layouts keep every
        // mouth by design and are not judged.
        if (is_contour && !relaxed_layout && band == 0. && mouths.size() < 8 && mouths.size() * 4 < n)
        {
            dbg.line("REJECT island: too few live mouths on the contour (%zu of %zu slits)", mouths.size(), n);
            DBG_COUNT("SRP_FEW_MOUTHS");
            // The tag line goes out under the perimeters category.
            dbg_log(Luminary::DBG_PERIMETERS, params.print_z, "SRP",
                    "SRP_FEW_MOUTHS %zu of %zu slits alive on the contour; normal walls", mouths.size(), n);
            return false;
        }
