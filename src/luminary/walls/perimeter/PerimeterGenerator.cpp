...
#include "luminary/walls/athena/toolpaths/WallToolPaths.hpp"
#include "luminary/walls/athena/paths/ExtrusionLine.hpp"
#include "luminary/core/diagnostics/DebugCounters.hpp"
#include "luminary/geometry/diagnostics/DebugGeom.hpp"
#include "luminary/walls/athena/order/PerimeterOrder.hpp"
...
#include "luminary/walls/fuzzy_skin/FuzzySkin.hpp"
...
namespace Luminary
{
// Perimeter-stage sub-timers, thread-summed; PrintObject prints them after make_perimeters.
PerfAccumTimer g_pg_main_walls, g_pg_voronoi, g_pg_inner_contour, g_pg_traverse, g_pg_interlock, g_pg_extra_perims;
PerfAccumTimer g_pg_il_phase1, g_pg_il_phase2, g_pg_il_lanes;
PerfAccumTimer g_pg_wtp_prep, g_pg_beading,
    g_pg_thin_retry; // WallToolPaths: outline prep, generateToolpaths, thin retry
// SkeletalTrapezoidation::generateToolpaths stages.
PerfAccumTimer g_st_central, g_st_bead_count, g_st_noncentral, g_st_trans_ribs, g_st_extra_ribs, g_st_seg_init,
    g_st_seg_propagate, g_st_seg_junctions, g_st_seg_connect;
} // namespace Luminary
...
#include "luminary/platform/paths/Paths.hpp" // debug_out_path
#endif
                                                                    ...
                                                                    const double nominal_mm3_per_mm)
{
    ...
    for (int i = 0; i < (int) lines.size(); ++i)
    {
        ...
        if (thickness_delta > tolerance)
        {
            ...
            // A new vector is built with the corrected elements and swapped in place, which keeps
            // this O(n); an erase plus one insert per segment costs O(n) each, O(n^2) in total. The
            // element ordering and the semantics are the same either way.
            {
        ...
        // ThickLine.a_width and b_width are extrusion widths, not spacing values, so the Arachne and
        // Athena paths take the width directly; converting spacing to width here would add extra
        // width and break the exact width enforcement. Classic and bridge paths keep the Flow.
...
{
    using namespace Luminary::Feature::FuzzySkin;
...
{
    using namespace Luminary::Feature::FuzzySkin;
    PerfScopedTimer pg_timer(g_pg_traverse);
                                     ...
                                     const Arachne::Perimeters &perimeters, const ExPolygons &infill_area)
{
    ...
    ::Luminary::SVG svg(path.c_str(), bbox);
...
// The distancers are built once per path by the caller; this runs for every pair.
bool paths_touch(const ExtrusionPath &path_one, const AABBTreeLines::LinesDistancer<Line> &lines_one,
                 const ExtrusionPath &path_two, const AABBTreeLines::LinesDistancer<Line> &lines_two,
...
Polylines reconnect_polylines(const Polylines &polylines, double limit_distance)
{
    ...
    // Connects nearby polyline endpoints during perimeter generation, once per layer. The iterator
    // from find() is reused instead of a second at() on the same key, halving the hash lookups.
                                     ...
                                     double extrusion_spacing, const bool target_clockwise)
{
    ...
    // Sorts extra perimeter paths by their touch dependencies into print order. A topological sort
    // keeps this O(n^2) and keeps the travel optimization; iterative dependency resolution would be
    // O(n^3 log n).

    const size_t n = extra_perims.size();

    // Phase 1: Build dependency graph, O(n^2) without a spatial index
    std::vector<std::unordered_set<size_t>> dependencies(n);

    std::vector<AABBTreeLines::LinesDistancer<Line>> path_distancers;
    path_distancers.reserve(n);
    for (const ExtrusionPath &path : extra_perims)
        path_distancers.emplace_back(path.as_polyline().lines());

    for (size_t path_idx = 0; path_idx < n; path_idx++)
    {
        for (size_t prev_path_idx = 0; prev_path_idx < path_idx; prev_path_idx++)
        {
            if (paths_touch(extra_perims[path_idx], path_distancers[path_idx], extra_perims[prev_path_idx],
                            path_distancers[prev_path_idx], extrusion_spacing * 1.5f))
            {
    ...
    // Phase 3 below is the topological sort: it checks dependencies.empty() and updates the sets as
    // paths are consumed, so no separate dependency resolution pass is needed.

    // Phase 3: Greedy path selection with travel distance optimization, O(n^2)
    ...
    const PrintConfig &print_config, const double dbg_z)
{
...
#ifdef EXTRA_PERIM_DEBUG_FILES
    {
        ...
        bbox.offset(scale_(1.));
        ::Luminary::SVG svg(debug_out_path("inset_overhang_area").c_str(), bbox);
    ...
    for (const ExPolygon &overhang : union_ex(to_expolygons(inset_overhang_area)))
    {
...
#ifdef EXTRA_PERIM_DEBUG_FILES
        {
            ...
            bbox.offset(scale_(1.));
            ::Luminary::SVG svg(debug_out_path("bridge_check").c_str(), bbox);
        ...
        else
        {
            ...
            while (continuation_loops >= 0)
            {
                ...
                if (prev == perimeter_polygon)
                {
                    ...
                    ::Luminary::SVG svg(debug_out_path("perimeter_polygon").c_str(), bbox);
                    for (const Line &line : to_lines(perimeter_polygon))
            ...
            ::Luminary::SVG svg(debug_out_path("pre_final").c_str(), bbox);
            for (const Line &line : to_lines(perimeter_polygon))
    ...
    BoundingBox bbox = get_extents(inset_overhang_area);
    bbox.offset(scale_(2.));
    ::Luminary::SVG svg(debug_out_path(("final" + std::to_string(rand())).c_str()).c_str(), bbox);
    for (const Line &line : to_lines(inset_overhang_area_left_unfilled))
        svg.draw(line, "blue", scale_(0.05));
    for (const Line &line : to_lines(inset_overhang_area))
        svg.draw(line, "green", scale_(0.05));
    ...
    ExPolygons &out_fill_expolygons)
{
    ...
    // Only fuzzy skin reads the raw lower slices (its per-segment overhang test), so the copy of
    // the whole lower layer is skipped unless fuzzy skin can be active on this region.
    Polygons lower_slices_raw;
    if (lower_slices != nullptr &&
        (params.config.fuzzy_skin.value != FuzzySkinType::None || !params.perimeter_regions.empty()))
    {
    ...
    // The floor under which an odd open wall is dropped: the setting, or the engine floor on the
    // first and topmost layers, whose surfaces would otherwise open gaps.
    MinWallLength min_wall_length;
    min_wall_length.percent = params.object_config.min_wall_length.percent;
    min_wall_length.value = min_wall_length.percent ? params.object_config.min_wall_length.value
                                                    : scaled<double>(params.object_config.min_wall_length.value);
    min_wall_length.surface_layer = params.layer_id == 0 || upper_slices == nullptr;
    ...
    wall_tool_paths.set_debug_context(params.layer_id, (params.layer != nullptr) ? params.layer->print_z : 0.0);
    ...
    if (inner_loop_number >= 0)
    {
        ...
        if (!top_expolygons.empty())
        {
            ...
            inner_wall_tool_paths.set_min_wall_length(min_wall_length);
            inner_wall_tool_paths.set_debug_context(params.layer_id,
                                                    (params.layer != nullptr) ? params.layer->print_z : 0.0);
        ...
        else
        {
            ...
            Arachne::WallToolPaths no_single_perimeter_tool_paths(last_p, ext_perimeter_spacing, perimeter_spacing,
                                                                  ...
                                                                  params.print_config);
            no_single_perimeter_tool_paths.set_min_wall_length(min_wall_length);
            no_single_perimeter_tool_paths.set_debug_context(params.layer_id,
                                                             (params.layer != nullptr) ? params.layer->print_z : 0.0);
        ...
        params.config.perimeters > 0 && params.layer_id > params.object_config.raft_layers)
    {
        ...
        g_pg_extra_perims.add(pg_t1, std::chrono::steady_clock::now());
...
void PerimeterRegion::merge_compatible_perimeter_regions(PerimeterRegions &perimeter_regions)
{
    ...
    for (auto it_curr_region = perimeter_regions.begin(); it_curr_region != perimeter_regions.end();)
    {
        ...
        for (; it_next_region != perimeter_regions.end() &&
             ...
             ++it_next_region)
        {
            Luminary::append(current_merge.expolygons, std::move(it_next_region->expolygons));
...
// --debug perimeters only [PERIM]; the --debug-geom sidecar records appear under either.
static inline bool pg_dbg_active()
{
    return Luminary::debug_enabled(Luminary::DBG_PERIMETERS | Luminary::DBG_INTERLOCK);
}
...
static void dbg_perim_contours(const char *phase, double z, int layer_id, const ExPolygons &contours, const char *label)
{
    ...
    dbg_log(Luminary::DBG_PERIMETERS, z, "PERIM", "%s %s ep=%zu area=%8.4fmm2 bbox=(%.2f,%.2f)-(%.2f,%.2f)", phase,
            label, contours.size(), total_area, unscaled<double>(bb.min.x()), unscaled<double>(bb.min.y()),
                            ...
                            coord_t perimeter_width)
{
    ...
    dbg_log(Luminary::DBG_PERIMETERS, z, "PERIM", "LOOPS_TOTAL: %d loops, ext_w=%.4fmm perim_w=%.4fmm", total_loops,
...
static void dbg_il_regions(double z, const char *phase, const ExPolygons &regions, const char *label)
{
    ...
    if (regions.empty())
    {
        dbg_log(Luminary::DBG_INTERLOCK, z, "INTERLOCK", "%s %s EMPTY", phase, label);
        return;
    }
    BoundingBox bb = get_extents(regions);
    dbg_log(Luminary::DBG_INTERLOCK, z, "INTERLOCK", "%s %s ep=%zu area=%8.4fmm2 bbox=(%.2f,%.2f)-(%.2f,%.2f)", phase,
    ...
    for (size_t i = 0; i < regions.size(); i++)
    {
        ...
        double a = std::abs(ep.area()) * 1e-12;
        BoundingBox epbb = get_extents(ep);
        dbg_log(Luminary::DBG_INTERLOCK, z, "INTERLOCK",
                "  %s %s [%zu] area=%8.4fmm2 holes=%zu pts=%zu "
                "bbox=(%.2f,%.2f)-(%.2f,%.2f)",
                phase, label, i, a, ep.holes.size(), ep.contour.points.size(), unscaled<double>(epbb.min.x()),
                unscaled<double>(epbb.min.y()), unscaled<double>(epbb.max.x()), unscaled<double>(epbb.max.y()));
...
// Geometry debug (--debug perimeters/interlock with --debug-geom): ExPolygons and polylines go to the geometry
// sidecar as WKT in mm (tag = label, category "perimeters"), where shapely loads them next to the gcode layer model.
static void dbg_wkt_expolys(double z, const char *label, const ExPolygons &eps)
{
    if (!eps.empty())
        dbg_geom(Luminary::DBG_PERIMETERS | Luminary::DBG_INTERLOCK, z, label, eps);
}

static void dbg_wkt_polyline(double z, const char *label, size_t inset, bool odd,
                             const std::vector<Athena::ExtrusionJunction> &js)
{
    const uint32_t cat = Luminary::DBG_PERIMETERS | Luminary::DBG_INTERLOCK;
    if (!dbg_geom_active(cat, z) || js.size() < 2)
        return;
    Polyline pl;
    pl.points.reserve(js.size());
    for (const Athena::ExtrusionJunction &j : js)
        pl.points.push_back(j.p);
    dbg_geom(cat, z, label, pl, "inset=" + std::to_string(inset) + " odd=" + (odd ? "1" : "0"));
}
...
static void dbg_il_collect_result(double z, const char *label, const ExtrusionEntityCollection &coll)
{
    ...
    dbg_log(Luminary::DBG_INTERLOCK, z, "INTERLOCK", "COLLECTED %s entities=%zu loops=%d paths=%d path_len=%.2fmm",
            label, coll.entities.size(), loops, paths, total_len);
}
...
// Portions of `probe` that lose coverage within m_top layers above
...
void PerimeterGenerator::process_athena(
    ...
    ExtrusionEntityCollection &out_gap_fill,
    // Infills without the gap fills
    ExPolygons &out_fill_expolygons, bool serpentine_allowed, int serpentine_piece_depth)
{
    // The island as received, once per top-level call (the pieces of a split island are subsets
    // of it): the reference the envelope check measures the extruded footprint against.
    if (serpentine_allowed && serpentine_piece_depth == 0 && params.layer != nullptr)
        dbg_wkt_expolys(params.layer->print_z, "slice", ExPolygons{surface.expolygon});
    ...
    // Convert thin wall precision enum to nanometer snap grid value
    ...
    bool serp_tried = false; // a single-piece island was handed to the Serpentine generator
    if (serpentine_allowed && params.config.serpentine_enabled && !params.spiral_vase)
    {
        ...
        if (params.layer_id == 0 && params.object_config.first_layer_extrusion_width.value > 0)
            sp_flow = Flow::new_from_config_width(frExternalPerimeter, params.object_config.first_layer_extrusion_width,
        ...
        if (serp_relaxed && params.config.serpentine_limit_depth)
            dbg_log(Luminary::DBG_SERPENTINE, sp_params.print_z, "SRP",
                    "serpentine relaxed spacing overrides the depth limit (both enabled)");

        // Hub audit for a full-mode tour. A bead wide relative to the island's hub leaves the tooth
        // tips unable to meet at the centre, and the tour rings an uncovered pocket there (a 5.5 mm
        // bolt head at a 0.55 mm bead keeps a 2 mm hollow on every layer). The pocket that holds the
        // island's deepest point (the centre of its largest inscribed disc, where the teeth converge)
        // and is at least 1.25 beads across is handed to fill, grown by the infill overlap so the fill
        // bonds into the beads around it; the small-area rule types it solid downstream. Every other
        // uncovered region stays: tooth-tip wedges and rim scallops are under a bead wide, and the
        // corner and convergence voids of a complex island are the pattern's own texture (filling
        // them all would scatter fill patches over every layer). Relaxed spacing leaves open cells on
        // purpose and is not audited.
        auto serp_hand_pockets_to_fill = [&](const ExPolygon &island)
        {
            // A ring has no hub: its teeth run from the contour to the bore, so the deepest point
            // lies in the tooth zone and the void holding it is a wedge between two teeth. Only
            // hole-free islands, where the teeth converge, are audited.
            if (serp_relaxed || !island.holes.empty() || out_loops.entities.empty())
                return;
            Polygons covered;
            out_loops.entities.back()->polygons_covered_by_width(covered, 0.f);
            const float pocket_half_min = 0.625f * float(sp_params.bead_width);
            ExPolygons voids;
            for (ExPolygon &v : diff_ex(ExPolygons{island}, union_(covered)))
                if (!offset_ex(v, -pocket_half_min).empty())
                    voids.push_back(std::move(v));
            if (voids.empty())
                return;
            // The deepest point: bisect the erosion radius to the largest one that leaves material,
            // then take the centroid of what is left (a disc of about a quarter bead).
            const BoundingBox island_bb = get_extents(island);
            float lo = 0.f, hi = 0.5f * float(std::min(island_bb.size().x(), island_bb.size().y()));
            ExPolygons deepest{island};
            for (int it = 0; it < 12 && hi - lo > 0.25f * float(sp_params.bead_width); ++it)
            {
                const float mid = 0.5f * (lo + hi);
                ExPolygons left = offset_ex(island, -mid);
                if (left.empty())
                    hi = mid;
                else
                {
            ...
            const ExPolygon &core = *std::max_element(deepest.begin(), deepest.end(),
                                                      [](const ExPolygon &a, const ExPolygon &b)
                                                      { return std::abs(a.area()) < std::abs(b.area()); });
            const Point hub = core.contour.centroid();
            // A hub pocket is what the converging tooth tips could not close: round (about 1.1 times
            // its inscribed disc) and a few beads across (a 5.5 mm bolt head reads 2.3 to 3.7 beads).
            // A wedge between two splayed teeth is many times its inscribed disc, and a core the
            // teeth never reached (a floor, a bracket junction) is far wider than four beads; both
            // stay uncovered as before. The inscribed radius comes from bisecting the erosion.
            ExPolygons pockets;
            for (ExPolygon &v : voids)
            {
                bool holds_hub = false;
                for (const ExPolygon &grown : offset_ex(v, 0.5f * float(sp_params.bead_width)))
                    if (grown.contains(hub))
                    {
                        holds_hub = true;
                        break;
                    }
                if (!holds_hub)
                    continue;
                const BoundingBox vb = get_extents(v);
                float r_lo = pocket_half_min, r_hi = 0.5f * float(std::min(vb.size().x(), vb.size().y()));
                for (int it = 0; it < 10 && r_hi - r_lo > 0.1f * float(sp_params.bead_width); ++it)
                {
                    const float mid = 0.5f * (r_lo + r_hi);
                    if (offset_ex(v, -mid).empty())
                        r_hi = mid;
                    else
                        r_lo = mid;
                }
                if (2.f * r_lo <= 4.f * float(sp_params.bead_width) &&
                    std::abs(v.area()) <= 2.0 * M_PI * double(r_lo) * double(r_lo))
                    pockets.push_back(std::move(v));
            }
            if (pockets.empty())
                return;
            double pocket_area = 0.;
            for (const ExPolygon &p : pockets)
                pocket_area += p.area();
            const coord_t bond = coord_t(
                scale_(params.config.get_abs_value("infill_overlap", unscale<double>(ext_perimeter_spacing))));
            ExPolygons handed = intersection_ex(offset_ex(pockets, float(bond)), ExPolygons{island});
            DBG_COUNT("SRP_HUB_POCKET");
            dbg_log(Luminary::DBG_PERIMETERS, sp_params.print_z, "SRP",
                    "SRP_HUB_POCKET %zu uncovered hub pocket(s) inside the tour, %.3f mm2, at least %.2f mm across; "
                    "handed to fill",
                    pockets.size(), pocket_area * SCALING_FACTOR * SCALING_FACTOR,
                    unscale<double>(2 * pocket_half_min));
            dbg_wkt_expolys(sp_params.print_z, "srp_pocket", handed);
            append(out_fill_expolygons, std::move(handed));
        };
        ...
        // The inset erases an island thinner than a bead (empty) or splits an island at a
        // sub-bead neck or a hole near the edge (size > 1). A tour on the raw slice would print
        // about half a bead oversize per side in either case, so a split island is generated piece
        // by piece, each piece grown back to the model edge, and the sub-bead material no piece
        // reaches prints as normal walls; an erased island, or a piece that splits again, prints as
        // normal walls at size. The tag lines go out under the perimeters category.
        ...
        if (serp_in.size() != 1)
        {
            // What a bead can cover is the island eroded by half a bead on every edge, contour and
            // holes alike, then grown back: a tour's beads reach exactly that far. The pieces are
            // the eroded island's components at least two beads across; a narrower component is
            // neck material, not an island of its own, and a thin wall that the erosion shatters
            // into crumbs stays one island of normal walls instead of many. Eroding the holes as
            // well matters: a neck between the outer edge and a hole would otherwise survive the
            // contour-only inset as a zero-width slit that a piece's tour drops as degenerate.
            const float half_bead = 0.5f * float(sp_params.bead_width);
            ExPolygons viable;
            if (serp_in.size() > 1 && serpentine_piece_depth == 0)
            {
                const coord_t min_piece = 2 * sp_params.bead_width;
                for (const ExPolygon &piece : offset_ex(surface.expolygon, -half_bead))
                {
                    const BoundingBox bb = get_extents(piece);
                    if (std::min(bb.size().x(), bb.size().y()) >= min_piece)
                        viable.push_back(piece);
                }
            }
            if (!viable.empty())
            {
                // Each piece grown back by the half bead it was eroded by and clipped to the slice, so
                // its boundary bead lands on the model edge. Grown pieces meet across a short neck;
                // the overlap goes to the first piece so no material is covered twice, and the
                // material beyond every piece's reach (necks, crumbs) is the rest.
                ExPolygons claimed;
                ExPolygons piece_islands;
                for (const ExPolygon &piece : viable)
                {
                    ExPolygons grown = intersection_ex(offset_ex(piece, half_bead), ExPolygons{surface.expolygon});
                    if (!claimed.empty())
                        grown = diff_ex(grown, claimed);
                    append(claimed, grown);
                    claimed = union_ex(claimed);
                    append(piece_islands, std::move(grown));
                }
                ExPolygons rest = diff_ex(ExPolygons{surface.expolygon}, claimed);
                // Sub-0.05 mm2 crumbs of the boolean are noise, not material.
                const double min_rest_area = scale_(0.05) * scale_(0.05);
                rest.erase(std::remove_if(rest.begin(), rest.end(), [min_rest_area](const ExPolygon &ex)
                                          { return std::abs(ex.area()) < min_rest_area; }),
                           rest.end());
                DBG_COUNT("SRP_PIECEWISE");
                dbg_wkt_expolys(sp_params.print_z, "srp_piece", piece_islands);
                dbg_wkt_expolys(sp_params.print_z, "srp_rest", rest);
                dbg_log(Luminary::DBG_PERIMETERS, sp_params.print_z, "SRP",
                        "SRP_PIECEWISE half-bead inset split the island into %zu pieces, %zu wide enough for a "
                        "tour; generating %zu piece island(s) at size, %zu region(s) as normal walls",
                        serp_in.size(), viable.size(), piece_islands.size(), rest.size());
                for (ExPolygon &ex : piece_islands)
                    process_athena(params, Surface(surface, std::move(ex)), lower_slices, upper_slices,
                                   lower_slices_polygons_cache, out_loops, out_gap_fill, out_fill_expolygons, true,
                                   serpentine_piece_depth + 1);
                for (ExPolygon &ex : rest)
                    process_athena(params, Surface(surface, std::move(ex)), lower_slices, upper_slices,
                                   lower_slices_polygons_cache, out_loops, out_gap_fill, out_fill_expolygons, false,
                                   serpentine_piece_depth + 1);
                return;
            }
            if (serpentine_piece_depth == 0)
            {
                DBG_COUNT("SRP_INSET_ERASED");
                dbg_log(Luminary::DBG_PERIMETERS, sp_params.print_z, "SRP",
                        "SRP_INSET_ERASED half-bead inset left no piece wide enough for a tour (%zu pieces); "
                        "normal walls at size",
                        serp_in.size());
            }
            else
            {
                DBG_COUNT("SRP_PIECE_RESPLIT");
                dbg_log(Luminary::DBG_PERIMETERS, sp_params.print_z, "SRP",
                        "SRP_PIECE_RESPLIT half-bead inset of a piece island left %zu pieces; normal walls at "
                        "size",
                        serp_in.size());
            }
            DBG_COUNT("SRP_WALL_FALLBACK");
            if (params.layer != nullptr)
                params.layer->object()->record_serpentine_wall_fallback();
            process_athena(params, surface, lower_slices, upper_slices, lower_slices_polygons_cache, out_loops,
                           out_gap_fill, out_fill_expolygons, false, serpentine_piece_depth);
            return;
        }
        const ExPolygon &serp_island = serp_in.front();
        serp_tried = true;
        ...
        if (serp_limit_depth)
        {
            ...
            if (band_params.fill_core.empty() && serp_island.holes.empty())
            {
                ...
                if (Serpentine::generate(serp_island, sp_params, out_loops))
                {
                    ...
                    serp_hand_pockets_to_fill(serp_island);
                    return;
                }
        ...
        else
        {
            ...
            if (params.config.serpentine_outer_loop.value)
            {
                ...
                for (const Polygon &h : serp_island.holes)
                {
                    ...
                    Polygons grown = offset(v, 0.5f * float(sp_params.bead_width), JoinType::Miter, 2.0);
                    if (grown.size() == 1)
                    {
    ...
    // An island the generator gave up on prints as ordinary walls; the depth handoff is not that,
    // its Athena wall is part of the pattern. Counted for the one notification per object.
    if (serp_tried && !serp_band_handoff)
    {
        DBG_COUNT("SRP_WALL_FALLBACK");
        if (params.layer != nullptr)
            params.layer->object()->record_serpentine_wall_fallback();
    ...
    // Arachne treats widths as suggestions and recalculates them, which drifts an extrusion width
    // off the user's setting (0.5mm becomes 0.499mm), so the exact values are enforced here.
    ...
    // The depth handoff's interior returned neither a wall nor an inner contour: the band prints
    // alone and the core the band was meant to enclose is lost. Counted to learn whether it happens.
    if (serp_band_handoff && perimeters.empty() && infill_contour.empty())
    {
        DBG_COUNT("SRP_BAND_HANDOFF_EMPTY");
        dbg_log(Luminary::DBG_SERPENTINE, dbg_z, "SRP",
                "SRP_BAND_HANDOFF_EMPTY the depth handoff's Athena pass produced no wall and no inner contour");
    }
    ...
    if (inner_loop_number >= 0 && !serp_band_handoff)
    {
        ...
        if (!top_expolygons.empty())
        {
            ...
            inner_wall_tool_paths.set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);
            inner_wall_tool_paths.set_min_wall_length(min_wall_length);
        ...
        else
        {
            ...
            Athena::WallToolPaths no_single_perimeter_tool_paths(last_p, ext_perimeter_spacing, perimeter_spacing,
                                                                 ...
                                                                 max_perimeter_width);
            no_single_perimeter_tool_paths.set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);
            no_single_perimeter_tool_paths.set_min_wall_length(min_wall_length);
            perimeters = no_single_perimeter_tool_paths.getToolPaths();
            preFlight::PreciseWalls::enforce_exact_widths(perimeters, ext_perimeter_width, perimeter_width, tw_snap);
            infill_contour = union_ex(no_single_perimeter_tool_paths.getInnerContour());
    ...
    auto il_compute_footprint = [&](const ExPolygons &contour, bool pwi_active)
    {
            ...
            if (pg_dbg_active())
            {
                ...
                for (const ExPolygon &p : flow)
                {
                    ...
                    if (pa >= piece_log_min)
                    {
                        ...
                        dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK",
                ...
                dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "OPENING_SPLIT fold=%.4fmm2 flow=%.4fmm2",
        ...
        if (pwi_active && !non_il_regions.empty())
        {
            ...
            if (tiny_n > 0)
                dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "FACE_TINY n=%zu area=%.4fmm2", tiny_n,
                        tiny_total * 1e-12);
            ...
            if (!il_suppressed_nowall.empty() && !il_regions.empty())
            {
                ...
                if (pg_dbg_active())
                {
                    ...
                    dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "NOWALL_SPLIT fill=%.4fmm2 flow=%.4fmm2",
            ...
            if (annulus_n > 0 && pg_dbg_active())
                dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "ANNULUS_GATE n=%zu area=%.6fmm2", annulus_n,
                        annulus_area * 1e-12);
        }
            ...
            if (!non_il_regions.empty() || !il_suppressed_nowall.empty())
            {
                ...
                if (lower_slices != nullptr)
                {
                    ...
                    if (!over_void.empty())
                    {
                        ...
                        if (pg_dbg_active())
                        {
                            ...
                            if (noanchor_n > 0)
                                dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "BRIDGE_NOANCHOR n=%zu",
                                        noanchor_n);
                        }
                        ...
                        if (speck_n > 0)
                        {
                            ...
                            dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "WALL_SPECK n=%zu moved_to_fill",
                                    speck_n);
                        }
                ...
                if (!extra_outline_ex.empty())
                {
                    ...
                    Athena::WallToolPaths extra_wall_tool_paths(extra_outline, perimeter_spacing, perimeter_spacing,
                                                                ...
                                                                min_bead_width_factor, tw_snap, max_perimeter_width);
                    extra_wall_tool_paths.set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);
                    extra_wall_tool_paths.set_min_wall_length(min_wall_length);
                    ...
                    if (hairline_n > 0)
                        dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK", "WALL_HAIRLINE n=%zu len=%.2fmm",
                                hairline_n, hairline_len * 1e-6);
                ...
                if (pg_dbg_active())
                {
                    ...
                    dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "ILADD",
    ...
    // the derived Parameters::ext_mm3_per_mm is not recomputed here, so it is
    ...
    {
        PerfScopedTimer pg_timer(g_pg_interlock);
        ...
        if (num_interlocking_shells > 0)
        {
            ...
            if (!il_regions.empty())
            {
...

                {
                    ...
                    il_walls.set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);

                    // Phase 1 must run first so getInnerContour() works.
                    auto pg_il_t0 = std::chrono::steady_clock::now();
                    il_walls.generate();
                    g_pg_il_phase1.add(pg_il_t0, std::chrono::steady_clock::now());
                    ...
                    // On odd layers the alternating parameters are the Phase 1 parameters (the
                    // ic_* values are defined as the odd-layer ones), so on a single-island
                    // contour the Phase 1 toolpaths are the Phase 2 result and the second
                    // Voronoi/beading run is skipped.
                    const bool phase2_is_phase1 = il_bead_width_0 == ic_bead_width_0 && il_external == ic_external &&
                                                  il_innermost == ic_innermost;
                    if (infill_contour.size() <= 1)
                    {
                        // Single ExPolygon: generate entities with alternating params
                        std::unique_ptr<Athena::WallToolPaths> il_entity_walls;
                        const std::vector<Athena::VariableWidthLines> *il_paths_ptr = nullptr;
                        if (phase2_is_phase1)
                        {
                            DBG_COUNT("IL_PHASE2_REUSED");
                            dbg_log(DBG_INTERLOCK, dbg_z, "INTERLOCK", "IL_PHASE2_REUSED layer=%zu",
                                    size_t(params.layer_id));
                            il_paths_ptr = &il_walls.getToolPaths();
                        }
                        else
                        {
                            il_entity_walls = std::make_unique<Athena::WallToolPaths>(
                                il_outline, il_bead_width_0, il_internal, size_t(actual_shells), 0, params.layer_height,
                                params.object_config, params.print_config, il_bead_width_0, perimeter_width,
                                il_external, il_internal, il_innermost, params.layer_id, 1.0, 10000);
                            il_entity_walls->set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);
                            auto pg_il_t1 = std::chrono::steady_clock::now();
                            il_paths_ptr = &il_entity_walls->generate();
                            g_pg_il_phase2.add(pg_il_t1, std::chrono::steady_clock::now());
                        }
                        const auto &il_paths = *il_paths_ptr;
                    ...
                    else
                    {
                        for (size_t ep_idx = 0; ep_idx < infill_contour.size(); ++ep_idx)
                        {
                            ...
                            ep_walls.set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);
                            auto pg_il_t1 = std::chrono::steady_clock::now();
                    ...
                    if (!il_emitted_cl.empty())
                        il_pattern_cover = offset_ex(union_ex(offset(il_emitted_cl, float(il_internal),
                                                                     JoinType::Square, 0., EndType::Round)),
                    ...
                    if (!il_wall_lanes.empty())
                    {
                        ...
                        Athena::WallToolPaths lane_walls(to_polygons(il_wall_lanes), perimeter_spacing,
                                                         ...
                                                         perimeter_width, 0, perimeter_spacing, 0, params.layer_id,
                                                         min_bead_width_factor, tw_snap, max_perimeter_width);
                        lane_walls.set_debug_print_z((params.layer != nullptr) ? params.layer->print_z : 0.0);
                        auto pg_il_t2 = std::chrono::steady_clock::now();
                        Athena::Perimeters lane_paths = lane_walls.getToolPaths();
                        g_pg_il_lanes.add(pg_il_t2, std::chrono::steady_clock::now());
                        ExPolyIL lane_il;
                    ...
                    if (pg_dbg_active())
                        dbg_wkt_expolys(dbg_z, "il_inner", union_ex(il_inner));
                    {
                        ...
                        // The geometric fallback extends Athena's inner contour into corners, so a
                        // piece the Athena contour mostly covers is clipped to that contour's
                        // neighbourhood. A piece the Athena contour barely covers is a core whose
                        // marker was lost (stitched open and dropped), not a narrow feature Athena
                        // rejected: rejection emits nothing, while a piece that still holds a full
                        // bead after the shell-depth offset is printable fill. Keep those whole. The
                        // split is by covered fraction, not by contact: a marker fragment at the
                        // edge of a lost core must not reduce the core to its own neighbourhood.
                        if (!geometric_inner.empty())
                        {
                            const ExPolygons expanded_inner = il_inner.empty()
                                                                  ? ExPolygons()
                                                                  : offset_ex(union_ex(il_inner), perimeter_width * 2);
                            const ExPolygons geo_all = union_ex(geometric_inner);
                            if (pg_dbg_active())
                                dbg_wkt_expolys(dbg_z, "il_geo", geo_all);
                            ExPolygons geo_kept;
                            // Counts for the three outcomes: kept whole (unmarked core), clipped
                            // to the marker neighbourhood (remainder dropped; the lowest covered
                            // fraction shows how close a piece came to the 50% line), too thin
                            // for a bead (dropped).
                            size_t unmarked_n = 0, clipped_n = 0, thin_n = 0;
                            double unmarked_a = 0., clipped_a = 0., thin_a = 0., clipped_min_frac = 1.;
                            for (const ExPolygon &piece : geo_all)
                            {
                                const double piece_area = std::abs(piece.area());
                                ExPolygons covered = expanded_inner.empty()
                                                         ? ExPolygons()
                                                         : intersection_ex(ExPolygons{piece}, expanded_inner);
                                double covered_area = 0.;
                                for (const ExPolygon &c : covered)
                                    covered_area += std::abs(c.area());
                                // The marker-adjacent part is always fill (corner extension).
                                append(geo_kept, std::move(covered));
                                if (covered_area >= 0.5 * piece_area)
                                {
                                    if (covered_area < piece_area)
                                    {
                                        ++clipped_n;
                                        clipped_a += piece_area - covered_area;
                                        clipped_min_frac = std::min(clipped_min_frac, covered_area / piece_area);
                                    }
                                    continue;
                                }
                                if (offset_ex(ExPolygons{piece}, -float(perimeter_width) / 2.f).empty())
                                {
                                    ++thin_n;
                                    thin_a += piece_area - covered_area;
                                    continue;
                                }
                                ++unmarked_n;
                                unmarked_a += piece_area;
                                geo_kept.push_back(piece);
                            }
                            if (unmarked_n > 0)
                                DBG_COUNT("IL_CORE_UNMARKED");
                            if (clipped_n > 0)
                                DBG_COUNT("IL_CORE_CLIPPED");
                            if (pg_dbg_active())
                            {
                                if (unmarked_n > 0)
                                    dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK",
                                            "IL_CORE_UNMARKED layer=%d n=%zu area=%.4fmm2", params.layer_id, unmarked_n,
                                            unmarked_a * 1e-12);
                                if (clipped_n > 0)
                                    dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK",
                                            "IL_CORE_CLIPPED layer=%d n=%zu uncovered=%.4fmm2 minfrac=%.3f",
                                            params.layer_id, clipped_n, clipped_a * 1e-12, clipped_min_frac);
                                if (thin_n > 0)
                                    dbg_log(Luminary::DBG_INTERLOCK, dbg_z, "INTERLOCK",
                                            "IL_CORE_THIN layer=%d n=%zu area=%.4fmm2", params.layer_id, thin_n,
                                            thin_a * 1e-12);
                            }
                            geometric_inner = to_polygons(geo_kept);
                        }
...
skip_interlocking:
    ...
    if (pg_dbg_active())
    {
        ...
        dbg_log(Luminary::DBG_PERIMETERS, dbg_z, "PERIM", "SIMPLIFY before=%zu after=%zu resolution=%.4fmm", ic_pts,
