...
// One [DETERM] fingerprint line per non-empty layer of a support layer list, stamped with its
// print z.
inline void determ_fp_layers(uint32_t cat, const char *tag, const char *kind,
                             const FFFSupport::SupportGeneratorLayersPtr &layers)
{
    if (!debug_enabled(cat))
        return;
    for (const FFFSupport::SupportGeneratorLayer *layer : layers)
        if (layer != nullptr && !layer->polygons.empty())
        {
            DetermFingerprint fp;
            fp.add(layer->polygons);
            dbg_log(cat, layer->print_z, "DETERM",
                    "%s kind=%s bz=%.4f polys=%zu pts=%zu area=%.4fmm2 hash=%016llx ord=%016llx", tag, kind,
                    layer->bottom_z, fp.count, fp.points, fp.area_mm2(), (unsigned long long) fp.hash,
