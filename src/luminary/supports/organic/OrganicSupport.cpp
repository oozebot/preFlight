...
#include "luminary/supports/tree/TreeModelVolumes.hpp"
#include "luminary/supports/tree/TreeSupport.hpp"
#include "luminary/supports/tree/TreeSupportCommon.hpp"
...
struct Tree
{
    ...
    Luminary::deque<Branch> branches;
                                              ...
                                              indexed_triangle_set &result)
{
    ...
    // A planted trunk fitted to a foot narrower than itself tapers down to that foot over its
    // lowest layers. The taper widens upward at maximum_move_distance per layer, the branch lean
    // limit: a leaning branch's underside already overhangs at that angle, so the flare prints
    // wherever the branch does. The foot lives on the tree root, which is path.front() of every
    // branch that starts there; an element created by a merge carries no foot.
    const coord_t foot = path.front()->state.plant_foot_radius;
    auto radius_of = [&path, &config, foot](size_t i) -> coord_t
    {
        const coord_t r = support_element_radius(config, *path[i]);
        return foot > 0 ? std::min(r, foot + coord_t(i) * config.maximum_move_distance) : r;
    };
    ...
    for (size_t ipath = 1; ipath < path.size(); ++ipath)
    {
        ...
        if (ipath == 1)
        {
            ...
            float radius = unscaled<float>(radius_of(ipath - 1));
            float angle_step = 2. * acos(1. - eps / radius);
        ...
        if (ipath + 1 == path.size())
        {
            ...
            float radius = unscaled<float>(radius_of(ipath));
            float angle_step = 2. * acos(1. - eps / radius);
            auto nsteps = int(ceil(M_PI / (2. * angle_step)));
            angle_step = M_PI / (2. * nsteps);
...
//
//     ring(d) = disk(stem_centre(d), trunk_radius(d) + reach(d))  intersect  interface
//
                                 ...
                                 const double reach_mm)
{
    ...
    for (size_t d = reach_at.size() - 1;; --d)
    {
        ...
        {
            // The builder's clip: (envelope disk intersect region) union tube, plus the straight bridge
...
static size_t baobab_extrude_canopy(const std::vector<std::pair<LayerIndex, Polygons>> &bands,
                                    ...
                                    const TreeModelVolumes &volumes, const SlicingParameters &slicing_params,
                                    const double taper_rad, const double growth_budget, const double reach_mm,
                                    const LayerIndex tip_extension, LayerIndex &canopy_bottom, BaobabCanopyStats &stats,
                                    ...
                                    std::vector<std::pair<LayerIndex, Polygons>> &rings_out, indexed_triangle_set &out)
{
    ...
    // below by at most the bead budget, clipped to (disk intersect interface) union tube. Foundation is
    ...
    // (disk intersect interface at every depth) a leaning trunk's deepest rings detach and float;
    ...
    for (size_t d = depth_converged;; --d)
    {
        ...
        else
        {
            ...
            clip.emplace_back(trunk_disk);
            // The ring may not stand on the model: what the model occupies at this layer (with
            // the xy clearance, and the model above within the gap) leaves the clip, so a ring
            // over a model face grows onto it only by the bead budget from the ring beside it,
            // never as a sheet resting on the face. The slice clip after slicing removes the
            // same area, but only from the ring that overlaps, not from the ring above it.
            Polygons clip_free = union_(clip);
            {
                const double area_before = area(clip_free);
                // The full xy distance, the same clearance the final object trim applies: with
                // the minimum distance here, the layer touching a model face is trimmed back
                // further than the ring above it, and that ring's edge oversteps by the
                // difference.
                clip_free = diff(clip_free, volumes.getCollision(0, layer, false));
                if (area(clip_free) < area_before - tiny_area_threshold)
                    DBG_COUNT("BAOBAB_RING_MODEL_CLIPPED");
            }
            chain = intersection_ex(offset_ex(chain, scaled<float>(growth_budget)), clip_free);
            if (chain.empty())
                chain = union_ex(Polygons{trunk_disk});
        }
...
// Nudges spheres away from an object using per layer AABB trees of lines.
    ...
    const std::vector<size_t> &linear_data_layers, std::function<void()> throw_on_cancel)
{
    ...
    for (LayerIndex layer_idx = 0; layer_idx < LayerIndex(layer_collision_cache.size()); ++layer_idx)
        ...
        {
            // Collision lower bound is taken at a zero radius for every layer.
            l.min_element_radius = 0;
                        ...
                        !collision_sphere.locked)
                    {
                        ...
                        if (collision_sphere.last_collision_depth > 0)
                        {
                            ...
                            if (collision_sphere.last_collision_depth > EPSILON)
                                ...
                                ++num_moved;
                            // Shift out of the collision by its depth plus the extra gap, capped per pass.
    ...
    // Passes run, and an exit at the iteration cap while spheres were still being nudged out of a collision.
    DBG_COUNT_ADD("ORGANIC_SMOOTH_ITERATIONS", iterations_run);
    if (!converged)
        DBG_COUNT("ORGANIC_SMOOTH_NOT_CONVERGED");
                           ...
                           BaobabCanopyFootprints *baobab_canopy_footprints)
{
    ...
    determ_fp_move_bounds("TREE_SMOOTH_FP", print_object.slicing_parameters(), config, move_bounds);
    ...
    // Trunk economy. Routing merges tips by proximity, so a mouth is carried by however
    ...
    if (baobab && trees.size() > 1)
    {
        ...
        // Which samples does a live set serve? Reaches are re-sized to the set's nearest-tip
        // needs (mirroring the build) but credited only up to one gather spacing, the canopy a
        // trunk is designed to carry: without the cap a survivor's simulated mouth grows to
        // cover the whole interface and every other tree reads as redundant, and the print
        // ends on one trunk under a funnel it cannot stabilise. Each live tip's canopy is
        // simulated at that reach, and a sample is served when it falls inside the rim of a
        // tip that can legally serve its band (terrace law: tip layer at or below the band).
        // Also returns the live rims.
        const double reach_credit_mm = unscaled<double>(config.settings.support_tree_branch_distance);
        auto serve_map = [&](const size_t r, const std::vector<char> &excluded, std::vector<char> &served,
                             ExPolygons &rims) -> bool
        {
            std::vector<size_t> live;
            for (size_t i = 0; i < etips.size(); ++i)
                if (!excluded[etips[i].tree] && region_of(etips[i].layer) == r)
                    live.push_back(i);
            served.assign(region_samples[r].size(), 0);
            rims.clear();
            if (live.empty())
                return false;
            ...
            for (double &n : need)
                n = std::min(n, reach_credit_mm);
            for (size_t k = 0; k < live.size(); ++k)
                append(rims, rim_of(live[k], need[k] + 0.5));
            for (size_t si = 0; si < region_samples[r].size(); ++si)
            {
                const EconomySample &s = region_samples[r][si];
                for (size_t k = 0; served[si] == 0 && k < live.size(); ++k)
                {
        ...
        // A tree is redundant when every sample only it serves lies within the rim bridge span
        // of the remaining rims: the base layer under the interface carries the interface's
        // whole outline, and that outline spans the gap from the neighbouring rim. A tree that
        // serves nothing at all is redundant by the same test.
        const double bridge_mm = unscaled<double>(config.settings.support_baobab_trunk_consolidation);
        size_t bridged_samples = 0;
        auto prunable = [&](const size_t r, const std::vector<char> &served_now, const std::vector<char> &trial,
                            size_t &bridged) -> bool
        {
            std::vector<char> served_trial;
            ExPolygons rims_trial;
            if (!serve_map(r, trial, served_trial, rims_trial))
                return false;
            ExPolygons reach;
            bool reach_built = false;
            for (size_t si = 0; si < region_samples[r].size(); ++si)
            {
                if (!served_now[si] || served_trial[si])
                    continue;
                if (!reach_built)
                {
                    reach = offset_ex(rims_trial, scaled<float>(bridge_mm));
                    reach_built = true;
                }
                bool inside = false;
                for (const ExPolygon &piece : reach)
                    if (piece.contains(region_samples[r][si].scaled_pos))
                    {
                        inside = true;
                        break;
                    }
        ...
        // The prune trial simulates a rim for every live tip, so the loop is quadratic in tips
        // at best. A dense lattice (a large interface) has no redundant trunks to find: every
        // tip serves its own cell. Above the cap the economy is skipped whole.
        constexpr size_t economy_tip_cap = 64;
        const bool economy_skipped = etips.size() > economy_tip_cap;
        if (economy_skipped)
            DBG_COUNT("BAOBAB_ECONOMY_SKIPPED_LARGE");
        for (bool changed = !economy_skipped; changed;)
        {
            changed = false;
            // Current coverage per region, once per pass instead of once per candidate.
            std::vector<std::vector<char>> served_now(region_floor.size());
            for (size_t r = 0; r < region_floor.size(); ++r)
            {
                ExPolygons rims_unused;
                serve_map(r, pruned, served_now[r], rims_unused);
            }
            ...
            for (const size_t candidate : order)
            {
                ...
                trial[candidate] = 1;
                // A rim is credited at most one gather spacing from its tip: a candidate tip
                // farther than reach + bridge from every other live tree's tip cannot be
                // covered, so the trial is settled without a single simulation.
                bool coverable = true;
                for (const EconomyTip &t : etips)
                {
                    if (t.tree != candidate)
                    ...
                    for (const EconomyTip &o : etips)
                        if (o.tree != candidate && !pruned[o.tree])
                            best2 = std::min(best2, (t.pos - o.pos).squaredNorm());
                    if (best2 > sqr(reach_credit_mm + bridge_mm + 1.0))
                    {
                        coverable = false;
                        break;
                    }
                ...
                for (size_t r = 0; ok && r < region_floor.size(); ++r)
                {
                    ...
                    if (candidate_holds)
                        ok = prunable(r, served_now[r], trial, bridged);
                }
                if (ok)
                {
                    DBG_COUNT("BAOBAB_TREE_PRUNED");
                    if (bridged > 0)
                    {
                        DBG_COUNT("BAOBAB_TREE_PRUNED_BRIDGED");
        ...
        dbg_log(DBG_BAOBAB, 0., "BAOBAB-ECONOMY",
                "regions=%zu trees_before=%zu pruned=%zu rejected_chain=%zu trees_after=%zu bridge_mm=%.1f "
                "bridged_samples=%zu tips=%zu%s",
                region_floor.size(), trees.size(), n_pruned, rejected_chain, trees.size() - n_pruned, bridge_mm,
                bridged_samples, etips.size(), economy_skipped ? " SKIPPED_LARGE" : "");
        if (n_pruned > 0)
        {
                ...
                for (size_t branch_idx = 0; branch_idx < tree.branches.size(); ++branch_idx)
                {
                    ...
                    if (baobab && branch.has_tip)
                    {
                        ...
                        if (bands.empty())
                        {
                            ...
                            canopy_outcome.reason = "no_interface";
                                     ...
                                     bands, canopy_path, config, volumes, slicing_params, baobab_taper,
                                     baobab_growth_budget, canopy_reach, baobab_tip_extension, baobab_canopy_bottom,
                                     baobab_canopy_stats, canopy_outcome, canopy_rings, partial_mesh);
                                 layers == 0)
                    ...
                    for (LayerIndex i = 0; i < LayerIndex(slices.size()); ++i)
                    {
                        ...
                        slices[i] = diff_clipped(
                            slices[i], volumes.getCollision(0, layer_begin + i,
                                                            true)); // collision taken at the minimum xy distance
    ...
    // Per-layer fingerprint of the branch slices before the union: the sum over trees, so the
    // order the trees were sliced in does not matter.
    if (debug_enabled(DBG_SUPPORT))
        for (size_t layer_idx = 0; layer_idx < slices.size(); ++layer_idx)
        {
            const Slice &slice = slices[layer_idx];
            if (slice.polygons.empty() && slice.tip_polygons.empty() && slice.bottom_contacts.empty())
                continue;
            DetermFingerprint base, tips, bottom;
            base.add(slice.polygons);
            tips.add(slice.tip_polygons);
            bottom.add(slice.bottom_contacts);
            dbg_log(DBG_SUPPORT, layer_z(print_object.slicing_parameters(), config, layer_idx), "DETERM",
                    "TREE_SLICE_FP layer=%zu branches=%zu base=%zu/%zu/%.4f/%016llx tips=%zu/%zu/%016llx "
                    "bottom=%zu/%zu/%016llx ord=%016llx/%016llx/%016llx",
                    layer_idx, slice.num_branches, base.count, base.points, base.area_mm2(),
                    (unsigned long long) base.hash, tips.count, tips.points, (unsigned long long) tips.hash,
                    bottom.count, bottom.points, (unsigned long long) bottom.hash, (unsigned long long) base.ordered,
                    (unsigned long long) tips.ordered, (unsigned long long) bottom.ordered);
        }
    ...
    // The base layer directly under an interface stack carries the interface's whole outline.
    const bool baobab_rim_outline = baobab && interface_placer.support_parameters.has_top_contacts;
    // An interface stack is a top contact with its interface and base-interface layers below it,
    // three arrays sharing the support layer index. For each stack bottom (a slot with interface
    // above and none one index down) the base layer whose print z fits under the stack's bottom z
    // receives the stack's whole outline.
    std::vector<Polygons> baobab_rim_outlines(baobab_rim_outline ? num_layers : 0);
    if (baobab_rim_outline && num_layers > 0)
    {
        const SupportGeneratorLayersPtr *stacks[3] = {&top_contacts, &interface_placer.top_interfaces_ref(),
                                                      &interface_placer.top_base_interfaces_ref()};
        auto stack_layer = [&stacks](const size_t idx) -> const SupportGeneratorLayer *
        {
            const SupportGeneratorLayer *lowest = nullptr;
            for (const SupportGeneratorLayersPtr *layers : stacks)
                if (idx < layers->size())
                    if (const SupportGeneratorLayer *l = (*layers)[idx];
                        l != nullptr && !l->polygons.empty() && (lowest == nullptr || l->bottom_z < lowest->bottom_z))
                        lowest = l;
            return lowest;
        };
        size_t idx_end = 0;
        for (const SupportGeneratorLayersPtr *layers : stacks)
            idx_end = std::max(idx_end, layers->size());
        for (size_t idx = 0; idx < idx_end; ++idx)
        {
            const SupportGeneratorLayer *bottom = stack_layer(idx);
            if (bottom == nullptr || (idx > 0 && stack_layer(idx - 1) != nullptr))
                continue;
            size_t target = std::min(idx, num_layers - 1);
            while (target > 0 &&
                   layer_z(print_object.slicing_parameters(), config, target) > bottom->bottom_z + EPSILON)
                --target;
            if (layer_z(print_object.slicing_parameters(), config, target) > bottom->bottom_z + EPSILON)
                continue;
            for (const SupportGeneratorLayersPtr *layers : stacks)
                if (idx < layers->size())
                    if (const SupportGeneratorLayer *l = (*layers)[idx]; l != nullptr)
                        polygons_append(baobab_rim_outlines[target], l->polygons);
        }
    }
    ...
    tbb::parallel_for(
        ...
        [&print_object, &config, &slices, &bottom_contacts, &top_contacts, &intermediate_layers, &layer_storage,
         &throw_on_cancel, &excluded_areas, &volumes, route_tips_to_top_contacts, baobab, baobab_rim_outline,
         &baobab_rim_outlines, baobab_region_max_hole_area,
         &baobab_canopy_stats](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                // The layer directly under an interface stack holds the interface's whole outline, so
                // the interface attaches to support everywhere. A canopy alone does not guarantee
                // that: it grows from its own trunk, and a trunk leaning away from the interface
                // leaves only a pin at the rim.
                if (baobab_rim_outline && !baobab_rim_outlines[layer_idx].empty())
                {
                    const double base_z = layer_z(print_object.slicing_parameters(), config, layer_idx);
                    // The same hole closing as the printed interface and the canopy regions.
                    ExPolygons outline = union_ex(baobab_rim_outlines[layer_idx]);
                    if (baobab_region_max_hole_area > 0.)
                        for (ExPolygon &piece : outline)
                            piece.holes.erase(std::remove_if(piece.holes.begin(), piece.holes.end(),
                                                             [baobab_region_max_hole_area](const Polygon &hole)
                                                             {
                                                                 return std::abs(hole.area()) <
                                              ...
                                              piece.holes.end());
                    const double outline_area = debug_enabled(DBG_BAOBAB) ? area(outline) : 0.;
                    Polygons rim = diff_clipped(to_polygons(std::move(outline)),
                                                volumes.getCollision(0, LayerIndex(layer_idx), true));
                    if (layer_idx < excluded_areas.size() && !excluded_areas[layer_idx].empty())
                        rim = diff(rim, excluded_areas[layer_idx]);
                    if (!rim.empty())
                    {
                        DBG_COUNT("BAOBAB_RIM_OUTLINE");
                        const double area_before = debug_enabled(DBG_BAOBAB) ? area(base_layer_polygons) : 0.;
                        polygons_append(base_layer_polygons, std::move(rim));
                        base_layer_polygons = union_(base_layer_polygons);
                        if (debug_enabled(DBG_BAOBAB))
                            dbg_log(DBG_BAOBAB, base_z, "BAOBAB-RIM",
                                    "layer=%zu interface_area=%.1f base_before=%.1f base_after=%.1f", layer_idx,
                                    outline_area * SCALING_FACTOR * SCALING_FACTOR,
                                    area_before * SCALING_FACTOR * SCALING_FACTOR,
                                    area(base_layer_polygons) * SCALING_FACTOR * SCALING_FACTOR);
                    }
                }
