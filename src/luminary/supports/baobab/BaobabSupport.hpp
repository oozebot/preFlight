...
struct BaobabSeedStats
{
    ...
    // Supplements moved off the outline onto the region inset by the trunk's growth, so the
    // trunk descends plumb under the interface; and pocket samples whose inset region lay
    // beyond the gather spacing, left to the interface outline on the base layer below.
    std::atomic<size_t> points_supplemental_inset{0};
    std::atomic<size_t> points_supplemental_unreachable{0};
};

// Plant decisions over one tree pass: trunks that landed on the model, and trunks refused because
// no model substance backed their landing. A trunk that reaches the bed with neither never
// overlapped a placeable landing. Summed and reported once.
struct BaobabPlantStats
{
    std::atomic<size_t> planted{0};
    std::atomic<size_t> refused_unbacked{0};
    // A surface within one gather spacing of the branch that even the narrowest foot could not
    // reach: the branch was beside the surface, not above it.
    std::atomic<size_t> no_landing{0};
    // Planted trunks whose landing was narrower than the trunk, and the narrowest foot fitted.
    std::atomic<size_t> fitted{0};
    std::atomic<int64_t> foot_min{std::numeric_limits<int64_t>::max()};
    // Per-event lines emitted; the stream caps them, the counters above do not.
    std::atomic<size_t> logged{0};
};

// Cap on per-event BAOBAB-PLANT lines per tree pass.
static constexpr size_t BAOBAB_PLANT_LOG_MAX = 200;
...
struct BaobabCanopyStats
{
    ...
    // A ring is disk intersect region, so a ring hole can only be a region hole.
