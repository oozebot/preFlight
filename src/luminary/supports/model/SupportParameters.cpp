...
///|/ Copyright (c) Prusa Research 2016 - 2023 Vojtěch Bubník @bubnikv, Pavel Mikuš @Godrak, Oleksandra Iushchenko @YuSanka, Tomáš Mészáros @tamasmeszaros
...
SupportParameters::SupportParameters(const PrintObject &object)
{
    ...
    this->first_layer_flow = Luminary::support_material_1st_layer_flow(&object,
                                                                       float(slicing_params.first_print_layer_height));
    this->support_material_flow = Luminary::support_material_flow(&object, float(slicing_params.layer_height));
    this->support_material_interface_flow =
        Luminary::support_material_interface_flow(&object, float(slicing_params.layer_height));
    ...
    for (size_t region_id = 0; region_id < object.num_printing_regions(); ++region_id)
    {
        ...
        external_perimeter_width =
            std::max(external_perimeter_width,
    ...
    this->support_material_bottom_interface_flow = this->support_material_interface_flow.with_flow_ratio(
        bridge_flow_ratio);

    this->can_merge_support_regions = object_config.support_material_extruder.value ==
                                      object_config.support_material_interface_extruder.value;
    if (!this->can_merge_support_regions && (object_config.support_material_extruder.value == 0 ||
                                             object_config.support_material_interface_extruder.value == 0))
    {
        ...
        if (object_extruders.size() == 1 &&
            *object_extruders.begin() ==
                std::max<unsigned int>(object_config.support_material_extruder.value,
                                       object_config.support_material_interface_extruder.value))
            // Object is printed with the same extruder as the support.
            this->can_merge_support_regions = true;
    }

    double interface_spacing = object_config.support_material_interface_spacing.value +
    ...
    double raft_interface_spacing = object_config.support_material_interface_spacing.value +
    ...
    this->contact_fill_pattern = (object_config.support_material_interface_pattern == smipAuto &&
                                  slicing_params.soluble_interface) ||
...
Flow support_material_1st_layer_flow(const PrintObject *object, float layer_height)
{
    ...
    const float nozzle_diameter = extruder >= 0 ? static_cast<float>(print_config.nozzle_diameter.get_at(extruder))
                                                : min_nozzle_diameter(*object);
    const auto &width = (object->config().first_layer_extrusion_width.value > 0)
