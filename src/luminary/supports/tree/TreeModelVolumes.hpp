...
static constexpr const coord_t SUPPORT_TREE_EXPONENTIAL_THRESHOLD = scaled<coord_t>(1. *
...
public:
    ...
    explicit TreeModelVolumes(const PrintObject &print_object, const BuildVolume &build_volume, coord_t max_move,
    ...
    // Determinism fingerprint of every cache, one [DETERM] line each (run scoped).
    void determ_fingerprint(const char *tag) const;
    ...
    std::optional<std::pair<coord_t, std::reference_wrapper<const Polygons>>> get_collision_lower_bound_area(
     ...
     * \return Polygons object
     */
    const Polygons &getAvoidance(coord_t radius, LayerIndex layer_idx, AvoidanceType type, bool to_model,
    ...
    coord_t ceilRadius(const coord_t radius, const bool min_xy_dist) const
    {
        ...
        return min_xy_dist
                   ...
                   radius > 0 ? this->ceilRadius(radius + m_current_min_xy_dist_delta)
                              : m_current_min_xy_dist_delta;
    }
    /*!
...
private:
    ...
    public:
        ...
        // Get a collision area at a given layer for a radius that is a lower or equial to the key radius.
        std::optional<std::pair<coord_t, std::reference_wrapper<const Polygons>>> get_lower_bound_area(
            const TreeModelVolumes::RadiusLayerPair &key) const
        {
            std::lock_guard<std::mutex> guard(m_mutex);
            if (key.second >= LayerIndex(m_data.size()))
                return {};
    ...
    /*!
     * \brief The outlines above with every enclosed hole narrower than the mesh's minimum opening filled, one
     * vector per m_layer_outlines entry. The collision offsets are built from these, so a branch never routes
     * through a hole it is not meant to thread; the placeable areas keep reading the raw outlines, so a filled
     * hole never becomes a surface to land on.
     */
    std::vector<std::vector<Polygons>> m_layer_outlines_routing;
    /*!
