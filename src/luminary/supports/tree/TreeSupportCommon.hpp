...
struct TreeSupportMeshGroupSettings
{
    ...
    // The minimum angle of overhangs for which support is added. At a value of 0 deg all overhangs are supported, 90 deg will not provide any support.
    ...
    // Type is double for scaled^2 area units (like minimum_roof_area).
    ...
    // Baobab rim bridge span: how far the interface outline on the layer under the interface
    // may span from a neighbouring canopy rim without a trunk of its own; the trunk economy
    // prunes a tree whose whole duty lies within it. Baobab only; unused by stock organic.
    coord_t support_baobab_trunk_consolidation{scaled<coord_t>(4.)};
    ...
    // Minimum opening: a hole of the model narrower than this that a branch could enter from free
    // space above is solid to every branch radius, so a branch never threads a screw hole or a slot
    // on its way down; a cavity closed above keeps its support. 0 disables it.
    coord_t support_tree_min_opening{scaled<coord_t>(8.)};
                                 ...
                                 const double z)
{
    ...
    if (layers.empty())
        return LayerIndex(config.raft_layers.size()) +
               std::max<LayerIndex>(0, LayerIndex(std::ceil((z - EPSILON - slicing_params.object_print_z_min -
                                  ...
                                  const double z)
{
    ...
    if (layers.empty())
        return LayerIndex(config.raft_layers.size()) +
               std::max<LayerIndex>(0, LayerIndex(std::floor((z + EPSILON - slicing_params.object_print_z_min -
                                                              slicing_params.first_object_layer_height) /
                                                             slicing_params.layer_height)));
    return LayerIndex(config.raft_layers.size()) + LayerIndex(layers.size()) - 1 +
...
public:
    InterfacePlacer(const SlicingParameters &slicing_parameters, const SupportParameters &support_parameters,
        ...
        , anchor_dtt(std::make_shared<std::unordered_map<const SupportGeneratorLayer *, size_t>>())
    ...
    // The dtt_roof each contact / interface slot is anchored to, shared across copies; a key is
    // only ever looked up, never iterated.
    const std::shared_ptr<std::unordered_map<const SupportGeneratorLayer *, size_t>> anchor_dtt;
    SupportGeneratorLayersPtr &top_contacts_mutable() { return this->top_contacts; }
    // The interface layers below the top contacts, by support layer index.
    const SupportGeneratorLayersPtr &top_interfaces_ref() const { return this->top_interfaces; }
    const SupportGeneratorLayersPtr &top_base_interfaces_ref() const { return this->top_base_interfaces; }
...
public:
    ...
    void add_roof_unguarded(Polygons &&new_roofs, const size_t insert_layer_idx, const size_t dtt_roof)
    {
        ...
        // Stacks from different contacts can share a slot, each with its own dtt_roof and so
        // its own anchor surface. The slot is anchored to the smallest dtt_roof it has received
        // (the contact nearest above) whatever order the threads deliver the roofs in; a smaller
        // one arriving later re-anchors the slot from its grid position.
        const bool allocate = l == nullptr;
        if (allocate)
        ...
        const auto anchored = anchor_dtt->find(l);
        const size_t anchored_dtt = anchored == anchor_dtt->end() ? std::numeric_limits<size_t>::max()
                                                                  : anchored->second;
        if (allocate || dtt_roof < anchored_dtt)
        {
            (*anchor_dtt)[l] = dtt_roof;
            if (!allocate)
                layer_initialize(*l, slicing_parameters, config, insert_layer_idx);
