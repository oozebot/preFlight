...
// The machine border is not part of the tree collision.
static Polygons calculateMachineBorderCollision(Polygon machine_border)
{
    // No border is returned, so tree support legs are not kept inside the bed boundary.
    return {};
}

TreeModelVolumes::TreeModelVolumes(const PrintObject &print_object, const BuildVolume &build_volume,
                                   const coord_t max_move, const coord_t max_move_slow)
...
{
    {
        ...
        std::vector<Polygons> &routing = m_layer_outlines_routing.emplace_back();
        ...
        tbb::parallel_for(
            tbb::blocked_range<size_t>(num_raft_layers, num_layers,
                                       std::clamp<size_t>(num_layers / (8 * tbb::this_task_arena::max_concurrency()), 1,
                                                          16)),
        ...
        // Decided over the whole stack: a narrow hole is filled only where a branch could enter it
        // from free space above, so a cavity closed by a ceiling keeps the support under it.
        routing = fill_narrow_passages(outlines, mesh_settings.support_tree_min_opening);
    }
                                          ...
                                          std::function<void()> throw_on_cancel)
{
    ...
    for (size_t outline_idx : layer_outline_indices)
        if (const std::vector<Polygons> &outlines = m_layer_outlines[outline_idx].second; !outlines.empty())
        {
            ...
            // The collision offsets come from the routing outlines (narrow holes filled); the
            // z-distance accumulation and the placeable areas below keep the raw outlines.
            assert(outline_idx < m_layer_outlines_routing.size() &&
                   m_layer_outlines_routing[outline_idx].size() == outlines.size());
            const std::vector<Polygons> &routing = m_layer_outlines_routing[outline_idx];
            ...
            tbb::parallel_for(tbb::blocked_range<LayerIndex>(collision_areas_offsetted.begin(),
                              ...
                              [&routing, &machine_border = std::as_const(m_machine_border),
                               ...
                               &throw_on_cancel](const tbb::blocked_range<LayerIndex> &range)
                              {
                                  for (LayerIndex layer_idx = range.begin(); layer_idx != range.end(); ++layer_idx)
                                  {
                                      ...
                                      append(collision_areas, routing[layer_idx]);
                                      if (offset_value == 0)
        ...
        [this, &data, radius, start_layer, &throw_on_cancel](const tbb::blocked_range<LayerIndex> &range)
        {
            for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                data[layer_idx - start_layer] = offset(
                    ...
                    -(radius + m_current_min_xy_dist + m_current_min_xy_dist_delta), JoinType::Miter, 1.2);
                throw_on_cancel();
            }
        });
...
coord_t TreeModelVolumes::ceilRadius(const coord_t radius) const
{
    ...
    if (radius > m_radius_0)
    {
        ...
        {
            const int num_steps = round_up_divide(initial_radius_delta, SUPPORT_TREE_COLLISION_RESOLUTION);
...
void TreeModelVolumes::determ_fingerprint(const char *tag) const
{
    if (!debug_enabled(DBG_SUPPORT))
        return;
    auto one = [tag](const char *name, const RadiusLayerPolygonCache &cache)
    {
        DetermFingerprint fp;
        size_t entries = 0;
        for (const auto &[key, polys] : cache.sorted())
        {
            ++entries;
            fp.add(DetermFingerprint::mix(DetermFingerprint::mix(uint64_t(key.first), uint64_t(key.second)),
                                          DetermFingerprint::polygons(polys.get())));
            for (const Polygon &poly : polys.get())
            {
        ...
        dbg_log(DBG_SUPPORT, 0., "DETERM", "%s cache=%s entries=%zu pts=%zu area=%.4fmm2 hash=%016llx ord=%016llx", tag,
                name, entries, fp.points, fp.area_mm2(), (unsigned long long) fp.hash, (unsigned long long) fp.ordered);
    };
    one("collision", m_collision_cache);
    one("collision_holefree", m_collision_cache_holefree);
    one("avoidance", m_avoidance_cache);
    one("avoidance_slow", m_avoidance_cache_slow);
    one("avoidance_to_model", m_avoidance_cache_to_model);
    one("avoidance_to_model_slow", m_avoidance_cache_to_model_slow);
    one("placeable", m_placeable_areas_cache);
    one("avoidance_holefree", m_avoidance_cache_holefree);
    one("avoidance_holefree_to_model", m_avoidance_cache_holefree_to_model);
    one("wall_restrictions", m_wall_restrictions_cache);
    one("wall_restrictions_min", m_wall_restrictions_cache_min);
}
