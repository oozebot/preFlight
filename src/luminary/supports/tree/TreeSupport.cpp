...
#include "luminary/supports/organic/OrganicSupport.hpp"
...
#include "luminary/geometry/contours/MultiPoint.hpp"
#include "luminary/geometry/contours/Polygon.hpp"
#include "luminary/geometry/contours/Polyline.hpp"
#include "luminary/geometry/mutable_polygon/MutablePolygon.hpp"
...
#include "luminary/core/containers/Indices.hpp"
#include "luminary/core/diagnostics/PerfTiming.hpp"

// #define TREESUPPORT_DEBUG_SVG

using namespace Luminary::FFFSupport;
    ...
    const Print &print, const std::vector<size_t> &print_object_ids)
{
    ...
    // The soluble flag is shared by all objects: any object printing with no gap sets it.
    ...
    for (size_t object_id : print_object_ids)
    {
        ...
        TreeSupportSettings next_settings{TreeSupportMeshGroupSettings{print_object}, print_object.slicing_parameters(),
                                          &print_object};
        // Each object forms its own group.
...
// blockers_out receives the support blockers on the same layer grid as the returned overhangs, so
// tip placement can honor them after it grows a strip beyond the layer they were subtracted on.
[[nodiscard]] static const std::vector<Polygons> generate_overhangs(const TreeSupportSettings &settings,
                                                                    const PrintObject &print_object,
                                                                    std::vector<Polygons> &blockers_out,
                                                                    std::function<void()> throw_on_cancel)
{
    ...
    // Painted-region census: layers with painted overhang, and those whose tip-wide band fell
    // under the minimum area.
    std::atomic<size_t> painted_layers{0};
    std::atomic<size_t> painted_dropped_small{0};
    // Steepest painted surface (degrees from horizontal) still treated as an overhang; the
    // per-layer strip of anything steeper is slicing noise on a wall. Safe range 60 to 85.
    const double painted_max_slope_deg = 75.;
    tbb::parallel_for(
        ...
        [&print_object, &config, &print_config, &enforcers_layers, &blockers_layers, support_auto,
         ...
         &throw_on_cancel, &out, &painted_layers, &painted_dropped_small,
         painted_max_slope_deg](const tbb::blocked_range<LayerIndex> &range)
        {
            for (LayerIndex layer_id = range.begin(); layer_id < range.end(); ++layer_id)
            {
                ...
                if (!enforcers_layers.empty() && !enforcers_layers[layer_id].empty())
                {
                    ...
                    // A painted region is the instruction to support whatever lies under it, so
                    // it ignores the support threshold angle: a painted slope between that angle
                    // and painted_max_slope is still an overhang. Above painted_max_slope the
                    // per-layer strip is slicing noise on a wall, and noise expanded to a tip
                    // band would support the whole painted wall.
                    Polygons enforcer_raw_overhangs;
                    if (raw_overhangs_calculated)
                        enforcer_raw_overhangs = raw_overhangs;
                    else
                    {
                        const float wall_offset = scaled<float>(lower_layer.height /
                                                                tan(M_PI * (painted_max_slope_deg + 1.) / 180.));
                        enforcer_raw_overhangs = diff(current_layer.lslices, offset(lower_layer.lslices, wall_offset));
                    }
                    if (Polygons enforced_overhangs = intersection(enforcer_raw_overhangs, enforcers_layers[layer_id]);
                        !enforced_overhangs.empty())
                    {
                        painted_layers.fetch_add(1, std::memory_order_relaxed);
                        enforced_overhangs = diff(offset(union_ex(enforced_overhangs), enforcer_overhang_offset),
                                                  lower_layer.lslices);
                        // Judged on the tip band, after the expansion: a painted slope on a
                        // faceted surface leaves its strip in pieces far under the minimum
                        // area, and the wall filter above already removed the noise this
                        // filter would otherwise guard against.
                        if (config.support_material_min_area.value > 0)
                        {
                            ...
                            if (enforced_overhangs.empty())
                                painted_dropped_small.fetch_add(1, std::memory_order_relaxed);
                        }
                        if (!enforced_overhangs.empty())
                        {
    ...
    if (debug_enabled(DBG_SUPPORT))
        dbg_log(DBG_SUPPORT, 0., "TREE-OVERHANG-PAINT", "layers=%zu dropped_small=%zu",
                painted_layers.load(std::memory_order_relaxed), painted_dropped_small.load(std::memory_order_relaxed));
    ...
    blockers_out.assign(num_layers, Polygons{});
    for (size_t layer_id = 0; layer_id < num_object_layers && layer_id < blockers_layers.size(); ++layer_id)
        blockers_out[layer_id + num_raft_layers] = blockers_layers[layer_id];

    if (!settings.support_layer_of.empty() && !settings.grid_matches_object)
    {
        auto remap = [&](std::vector<Polygons> &src_layers, size_t &dbg_src_layers, size_t &dbg_shared_slots)
        {
            ...
            for (size_t i = 0; i < num_raft_layers && i < src_layers.size(); ++i)
                remapped[i] = std::move(src_layers[i]);
            for (size_t layer_id = 0; layer_id < num_object_layers && layer_id < settings.support_layer_of.size();
                 ++layer_id)
                if (Polygons &src = src_layers[layer_id + num_raft_layers]; !src.empty())
            ...
            src_layers = std::move(remapped);
        };
        size_t dbg_src_layers = 0, dbg_shared_slots = 0, dbg_blocker_layers = 0, dbg_blocker_shared = 0;
        remap(out, dbg_src_layers, dbg_shared_slots);
        remap(blockers_out, dbg_blocker_layers, dbg_blocker_shared);
    ...
    for (Polyline part : input)
    {
        ...
        else
        {
            ...
            if (part.front() == part.back())
            {
                ...
                // If the polyline was a polygon, there is a high chance it was an overhang. Overhangs that are <60 deg tend to be very thin areas, so lets get the beginning and end of them and ensure that they are supported.
    ...
    coord_t support_infill_distance)
{
    ...
    filler->angle = roof ?
                         // Alternates by layer index, not by the support layer's interface id.
...
[[nodiscard]] static Polygons safe_union(const Polygons first, const Polygons second = {})
{
    ...
    // This does not cause a problem on large areas, but influence areas (representing centerpoints) can be very
    // thin, so a union that comes back empty is retried on slightly offset polylines below.
...
*/
    ...
    if (!first.empty() || !second.empty())
    {
        ...
        if (result.empty())
        {
            ...
            result = union_(offset(to_polylines(first), scaled<float>(0.002), JoinType::Miter, 1.2),
                            offset(to_polylines(second), scaled<float>(0.002), JoinType::Miter, 1.2));
        }
    }
                                              ...
                                              size_t min_amount_offset)
{
    ...
    for (int i = 0; i < steps; ++i)
    {
        ...
        // ensure that if many offsets are done the performance does not suffer extremely by the new vertices of JoinType::Round.
        if (i % 10 == 7)
...
// Baobab seeds one tip per gather cell on a square lattice through the object origin, aligned
// with the support fill angle. The lattice depends on nothing but the object: every overhang
// band of a sloped face samples the same rails, moving the object on the plate changes nothing,
// and rotating it by a multiple of 90 degrees maps the lattice onto itself. A cell is kept when
// its point lies inside the region inset by half a support line, the margin the fill lines keep
// from the outline. Each seed is emitted as a one-point polyline.
static Polylines baobab_seed_lattice(const Polygons &area, const coord_t spacing, const double angle,
                                     const coord_t inset)
{
    Polylines out;
    if (area.empty() || spacing <= 0)
        return out;
    const Polygons region = inset > 0 ? offset(area, -float(inset)) : area;
    if (region.empty())
        return out;
    // Convert the region once; the per-cell point-in-polygon test then allocates nothing.
    Clipper2Lib::Paths64 paths;
    paths.reserve(region.size());
    for (const Polygon &poly : region)
        paths.emplace_back(Points_to_ClipperPath(poly.points));
    const BoundingBox bbox = get_extents(region);
    const Vec2d u(std::cos(angle), std::sin(angle));
    const Vec2d v(-u.y(), u.x());
    // Lattice index range covering the box: project its corners onto the lattice axes.
    double i_lo = std::numeric_limits<double>::max(), i_hi = std::numeric_limits<double>::lowest();
    double j_lo = i_lo, j_hi = i_hi;
    for (const Point &corner :
         {bbox.min, Point(bbox.max.x(), bbox.min.y()), bbox.max, Point(bbox.min.x(), bbox.max.y())})
    {
        const Vec2d c = corner.cast<double>() / double(spacing);
        i_lo = std::min(i_lo, c.dot(u));
        i_hi = std::max(i_hi, c.dot(u));
        j_lo = std::min(j_lo, c.dot(v));
        j_hi = std::max(j_hi, c.dot(v));
    }
    for (int i = int(std::floor(i_lo)); i <= int(std::ceil(i_hi)); ++i)
        for (int j = int(std::floor(j_lo)); j <= int(std::ceil(j_hi)); ++j)
        {
            const Vec2d pos = (u * double(i) + v * double(j)) * double(spacing);
            const Point p(coord_t(std::lround(pos.x())), coord_t(std::lround(pos.y())));
            if (!bbox.contains(p))
                continue;
            // Inside an odd number of contours, or on an edge.
            const Clipper2Lib::Point64 cp(p.x(), p.y());
            int inside = 0;
            bool on_edge = false;
            for (const Clipper2Lib::Path64 &path : paths)
            {
                const Clipper2Lib::PointInPolygonResult r = Clipper2Lib::PointInPolygon(cp, path);
            ...
            if (on_edge || (inside % 2) == 1)
                out.emplace_back(Polyline{p});
        }
    return out;
}
...
// it - but the rim must cover the entire interface outline. The lattice does not reach into
// outline corners, so a corner pocket can sit beyond the gather spacing from the whole grid.
// Sample the region outline (contours and holes) and seed it by farthest-point selection:
// repeatedly seed the outline sample farthest from every seed while that distance exceeds the
// gather spacing. Each edge is sampled from its own start vertex and the selection order is
// set by distances alone, so the result does not depend on where a contour's vertex array
// happens to begin.
// The point of a polygon set's outline nearest to `from`.
static Point baobab_nearest_outline_point(const Polygons &polygons, const Point &from)
{
    Point best = from;
    double best_d2 = std::numeric_limits<double>::max();
    const Vec2d f = from.cast<double>();
    for (const Polygon &poly : polygons)
        for (size_t i = 0; i < poly.points.size(); ++i)
        {
            const Vec2d a = poly.points[i].cast<double>();
            const Vec2d b = poly.points[(i + 1) % poly.points.size()].cast<double>();
            const Vec2d ab = b - a;
            const double len2 = ab.squaredNorm();
            const double t = len2 > 0. ? std::clamp((f - a).dot(ab) / len2, 0., 1.) : 0.;
            const Vec2d p = a + ab * t;
            if (const double d2 = (p - f).squaredNorm(); d2 < best_d2)
            {
                best_d2 = d2;
                best = p.cast<coord_t>();
            }
...
static void baobab_supplement_outline_seeds(const Polygons &overhang_area, const coord_t gather_spacing,
                                            const coord_t inset, Polylines &polylines, BaobabSeedStats *stats)
{
    if (gather_spacing <= 0)
        return;
    Points seeds;
    ...
    for (const Polygon &contour : overhang_area)
        for (size_t i = 0; i < contour.points.size(); ++i)
        {
            ...
            for (double t = 0.; t < len; t += step)
                candidates.emplace_back((a + (b - a) * (t / len)).cast<coord_t>());
        }
    if (candidates.empty())
        return;
    // The seed stands where a trunk can descend plumb: the region inset by the trunk's growth
    // over the tip radius. A seed on the outline stands at the model wall, and the branch has
    // to lean out of the footprint as it thickens.
    const Polygons interior = inset > 0 ? offset(overhang_area, -float(inset)) : overhang_area;
    // Nearest-seed distance per candidate, kept current as seeds are added, so each added seed
    // costs one pass over the candidates rather than a rescan of every seed.
    std::vector<double> min_d2(candidates.size(), std::numeric_limits<double>::max());
    auto account = [&candidates, &min_d2](const Point &seed)
    {
        const Vec2d s = seed.cast<double>();
        for (size_t c = 0; c < candidates.size(); ++c)
            min_d2[c] = std::min(min_d2[c], (candidates[c].cast<double>() - s).squaredNorm());
    };
    auto farthest = [&min_d2]()
    {
        return size_t(std::max_element(min_d2.begin(), min_d2.end()) - min_d2.begin());
    };
    // Where a pocket sample lands as a seed: itself when it lies in the inset region, else the
    // inset region's nearest point when that is within the gather spacing, else nowhere (the
    // base layer under the interface carries the outline there). A region too small to hold a
    // plumb trunk keeps the outline seed: a leaning trunk beats none.
    auto place = [&interior, radius2](const Point &sample, Point &seed) -> bool
    {
        if (interior.empty() || contains(interior, sample))
        {
            seed = sample;
            return true;
        }
        seed = baobab_nearest_outline_point(interior, sample);
        return (seed - sample).cast<double>().squaredNorm() <= radius2;
    };
    size_t added = 0, moved = 0, unreachable = 0;
    auto add = [&](const size_t best)
    {
        Point seed;
        if (place(candidates[best], seed))
        {
            seeds.emplace_back(seed);
            polylines.emplace_back(Polyline{seed});
            account(seed);
            ++added;
            if (seed != candidates[best])
                ++moved;
        }
        else
        {
            DBG_COUNT("BAOBAB_SEED_UNREACHABLE");
            ++unreachable;
        }
        // Settled either way: a pocket beyond every reachable seed is left to the outline layer.
        min_d2[best] = 0.;
    };
    if (seeds.empty())
    {
        // With no seeds at all, the first pick is the sample farthest from the region centre.
        const Vec2d centre = get_extents(overhang_area).center().cast<double>();
        for (size_t c = 0; c < candidates.size(); ++c)
            min_d2[c] = (candidates[c].cast<double>() - centre).squaredNorm();
        const size_t best = farthest();
        std::fill(min_d2.begin(), min_d2.end(), std::numeric_limits<double>::max());
        add(best);
    }
    for (const Point &seed : seeds)
        account(seed);
    for (size_t best = farthest(); min_d2[best] > radius2; best = farthest())
        add(best);
    if (stats != nullptr)
    {
        if (added > 0)
        {
            ...
            stats->points_supplemental_inset.fetch_add(moved, std::memory_order_relaxed);
        }
        stats->points_supplemental_unreachable.fetch_add(unreachable, std::memory_order_relaxed);
    }
}
    ...
    RichInterfacePlacer &interface_placer)
{
    ...
    // Baobab seeds its tips on the gather lattice instead, so widening it cannot cost an
    // interface layer.
    auto generate_tip_lines = [&interface_placer, &mesh_group_settings](const Polygons &area,
                                                                        LayerIndex layer_idx) -> Polylines
    {
        return generate_support_infill_lines(area, interface_placer.support_parameters, true, layer_idx,
                                             mesh_group_settings.support_roof_line_distance);
    };
    // Baobab tips come from baobab_seed_lattice(); the fill-line sampling below is the stock
    // organic path.
    const double lattice_angle = double(interface_placer.support_parameters.base_angle);
    const coord_t lattice_inset = interface_placer.config.support_line_width / 2;
    // Outline supplements stand where the trunk's growth over the tip radius still clears the
    // model, so they can descend plumb under the interface.
    const coord_t supplement_inset = std::max<coord_t>(0, interface_placer.config.branch_radius -
                                                              interface_placer.config.min_radius);

    // Spacing of tip points along a resampled outline: Baobab keeps its gather spacing so a
    // small region seeds sparse tips rather than a dense ring. Safe to widen here because the
    // only closed-outline caller below is the sparse-region fallback, whose outline is short,
    // and the dense-tip stock path resamples open fill lines with min_points = 1.
    ...
    if (large_horizontal_roof)
    {
        ...
        for (dtt_roof = 0; dtt_roof < num_support_roof_layers && layer_idx - dtt_roof >= 1; ++dtt_roof)
        {
            ...
            Polygons forbidden_next;
            {
                ...
                forbidden_next = offset(union_ex(forbidden_next_raw), scaled<float>(0.005), JoinType::Miter, 1.2);
            }
            ...
            if (area(overhang_area_next) < mesh_group_settings.minimum_roof_area)
            {
                ...
                if (dtt_roof > 0)
                {
                    ...
                    Polylines roof_tip_lines =
                        baobab_tip_spacing > 0
                            ? baobab_seed_lattice(last_overhang, baobab_tip_spacing, lattice_angle, lattice_inset)
                            : ensure_maximum_distance_polyline(generate_tip_lines(last_overhang,
                    ...
                    baobab_supplement_outline_seeds(last_overhang, baobab_tip_spacing, supplement_inset, roof_tip_lines,
                                                    baobab_stats);
                    overhang_lines =
    ...
    if (overhang_lines.empty())
    {
        ...
        // Baobab seeds the gather lattice; with continuous tips (no interface layers) it keeps
        // the dense stock sampling, as support lines so the line distance carries the gather
        // spacing (the roof branch derives density from interface_density and discards it).
        ...
        Polylines polylines =
            baobab_tip_spacing > 0 && !continuous_tips
                ? baobab_seed_lattice(overhang_area, baobab_tip_spacing, lattice_angle, lattice_inset)
        ...
        if (point_count <= min_support_points)
        {
            ...
            // will support the overhang (if this is not done it may only be half). This WILL NOT be the case when supporting an angle of about < 60 deg so there is a fallback,
            // as some support is better than none.
            Polygons reduced_overhang_area = offset(union_ex(overhang_area),
                                                    -interface_placer.config.support_line_width / 2.2, JoinType::Miter,
                                                    1.2);
            polylines = ensure_maximum_distance_polyline(
                to_polylines(!reduced_overhang_area.empty() &&
                                     area(offset(diff_ex(overhang_area, reduced_overhang_area),
                                                 ...
                                                 JoinType::Miter, 1.2)) < sqr(scaled<double>(0.001))
        ...
        baobab_supplement_outline_seeds(overhang_area, baobab_tip_spacing, supplement_inset, polylines, baobab_stats);
...
static void generate_initial_areas(const PrintObject &print_object, const TreeModelVolumes &volumes,
                                   ...
                                   const std::vector<Polygons> &blockers, std::vector<SupportElements> &move_bounds,
                                   InterfacePlacer &interface_placer, std::function<void()> throw_on_cancel)
{
    using AvoidanceType = TreeModelVolumes::AvoidanceType;
    ...
    BaobabSeedStats *const baobab_stats = (baobab && (debug_enabled(DBG_BAOBAB) || debug_enabled(DBG_SUPPORT)))
                                              ? &baobab_seed_stats
                                              : nullptr;
    // Census of the overhang layers this pass was handed and what became of them: placed by
    // the stock push-out, pushed to slope_reach, or lost. Areas in um2.
    struct OverhangCensus
    {
        std::atomic<size_t> layers_raw{0}, layers_placed{0}, layers_pushed{0}, layers_lost{0}, dropped_min_area{0};
        std::atomic<int64_t> area_raw{0}, area_placed{0}, area_pushed{0};
    };
    OverhangCensus overhang_census_storage;
    OverhangCensus *const overhang_census = debug_enabled(DBG_SUPPORT) ? &overhang_census_storage : nullptr;
    auto census_area = [](const Polygons &polys) -> int64_t
    {
        return int64_t(std::llround(unscaled<double>(unscaled<double>(area(polys))) * 1e6));
    };
    ...
    // Reach for sloped overhangs. A surface between the threshold angle and vertical leaves a
    // strip per layer no wider than one layer's outline advance, lying inside the forbidden
    // zone of the layer below, and the push-out above stops at extra_outset. Pushing the strip
    // to xy distance plus tip radius clears that zone; half a line width more gives the tip a
    // band to stand in. The branch then follows the slope down at xy distance from the wall.
    const coord_t slope_reach = config.xy_distance + config.min_radius + config.support_line_width / 2;
    ...
    tbb::parallel_for(
        ...
        [&volumes, &config, &raw_overhangs, &blockers, &mesh_group_settings, min_xy_dist, roof_enabled,
         num_support_roof_layers, extra_outset, slope_reach, z_distance_delta, circle_length_to_half_linewidth_change,
         connect_length, baobab_tip_spacing, baobab_stats, overhang_census, census_area, &rich_interface_placer,
         &throw_on_cancel](const tbb::blocked_range<size_t> &range)
        {
            for (size_t raw_overhang_idx = range.begin(); raw_overhang_idx < range.end(); ++raw_overhang_idx)
            {
                ...
                if (overhang_census != nullptr)
                {
                    overhang_census->layers_raw.fetch_add(1, std::memory_order_relaxed);
                    overhang_census->area_raw.fetch_add(census_area(overhang_raw), std::memory_order_relaxed);
                }
                ...
                Polygons relevant_forbidden;
                {
                    ...
                    relevant_forbidden = offset(union_ex(relevant_forbidden_raw), scaled<float>(0.005), JoinType::Miter,
                                                1.2);
                }
                ...
                // Rim seeded for strips the stock push-out could not place (census only).
                Polygons pushed;
                {
                    ...
                    Polygons remaining_overhang = intersection(
                        diff(mesh_group_settings.support_offset == 0
                                 ? overhang_raw
                                 : offset(union_ex(overhang_raw), mesh_group_settings.support_offset, JoinType::Miter,
                                          1.2),
                             offset(union_ex(overhang_regular), config.support_line_width * 0.5, JoinType::Miter, 1.2)),
                        relevant_forbidden);
                    ...
                    // Strips the push-out above could not place are pushed once more, to
                    // slope_reach. Of what clears the forbidden zone only the rim within half a
                    // line of it seeds tips, so every tip stands as close to the wall as the xy
                    // distance allows. Blockers painted on the overhang hold here as well.
                    // A strip within slope_reach of an area the stock placement covers is that
                    // area's job (its tips reach the wall as far as the xy distance allows);
                    // pushing it would only seed a lobe beside the wall with a branch of its own.
                    if (!remaining_overhang.empty() && !overhang_regular.empty())
                        remaining_overhang = diff(remaining_overhang, expand(overhang_regular, float(slope_reach)));
                    if (!remaining_overhang.empty())
                    {
                        const Polygons &raw_collision = volumes.getCollision(0, layer_idx, true);
                        const coord_t offset_step = config.xy_min_distance + config.support_line_width;
                        pushed = intersection(diff(safe_offset_inc(remaining_overhang, slope_reach, raw_collision,
                                                                   offset_step, 0, 1),
                                                   relevant_forbidden),
                                              expand(relevant_forbidden, float(config.support_line_width / 2)));
                        if (const size_t overhang_idx = layer_idx + z_distance_delta;
                            overhang_idx < blockers.size() && !blockers[overhang_idx].empty())
                            pushed = diff(pushed, blockers[overhang_idx], ApplySafetyOffset::Yes);
                        if (!pushed.empty())
                            overhang_regular = union_(overhang_regular, pushed);
                    }
                }

                throw_on_cancel();

                bool roofs_placed = false;
                if (roof_enabled)
                {
                    ...
                    roofs_placed = !overhang_roofs.empty();
                ...
                const bool had_regular = !overhang_regular.empty();
                if (mesh_group_settings.minimum_support_area > 0)
                    remove_small(overhang_regular, mesh_group_settings.minimum_support_area);
                // Census after every sink: a layer counts as placed only by what reaches the
                // sampler, and as lost when neither roofs nor tips remain.
                if (overhang_census != nullptr)
                {
                    if (!pushed.empty())
                    {
                        overhang_census->layers_pushed.fetch_add(1, std::memory_order_relaxed);
                        overhang_census->area_pushed.fetch_add(census_area(pushed), std::memory_order_relaxed);
                    }
                    if (had_regular && overhang_regular.empty())
                        overhang_census->dropped_min_area.fetch_add(1, std::memory_order_relaxed);
                    if (!overhang_regular.empty() || roofs_placed)
                    {
                        overhang_census->layers_placed.fetch_add(1, std::memory_order_relaxed);
                        overhang_census->area_placed.fetch_add(census_area(overhang_regular),
                                                               std::memory_order_relaxed);
                    }
                    else
                        overhang_census->layers_lost.fetch_add(1, std::memory_order_relaxed);
                }
    ...
    if (baobab_stats != nullptr)
    {
        ...
        dbg_log(DBG_BAOBAB, 0., "BAOBAB-SEED",
                ...
                "points_supplemental_inset=%zu points_supplemental_unreachable=%zu",
                ...
                baobab_stats->points_supplemental_inset.load(std::memory_order_relaxed),
                baobab_stats->points_supplemental_unreachable.load(std::memory_order_relaxed));
        // Seed census for the support stream: the tip set is the input every later stage of the
        // tree depends on, so two placements of one model should agree here first.
        dbg_log(DBG_SUPPORT, 0., "TREE-SEED",
                "gather_spacing=%.3f points_gathered=%zu points_from_fallback=%zu points_supplemental=%zu",
                unscaled<double>(baobab_tip_spacing), gathered, fallback_points,
                baobab_stats->points_supplemental.load(std::memory_order_relaxed));
    }
    if (overhang_census != nullptr)
        dbg_log(DBG_SUPPORT, 0., "TREE-OVERHANG",
                "object=%zu layers_raw=%zu area_raw=%.1f layers_placed=%zu area_placed=%.1f layers_pushed=%zu "
                "area_pushed=%.1f layers_lost=%zu dropped_min_area=%zu slope_reach=%.3f",
                size_t(print_object.id().id), overhang_census->layers_raw.load(std::memory_order_relaxed),
                double(overhang_census->area_raw.load(std::memory_order_relaxed)) * 1e-6,
                overhang_census->layers_placed.load(std::memory_order_relaxed),
                double(overhang_census->area_placed.load(std::memory_order_relaxed)) * 1e-6,
                overhang_census->layers_pushed.load(std::memory_order_relaxed),
                double(overhang_census->area_pushed.load(std::memory_order_relaxed)) * 1e-6,
                overhang_census->layers_lost.load(std::memory_order_relaxed),
                overhang_census->dropped_min_area.load(std::memory_order_relaxed), unscaled<double>(slope_reach));
...
// Depth below a landing surface at which a Baobab trunk asks for model before it plants there.
// A fixed depth, independent of the trunk radius, so the decision does not vary with how far a
// given trunk's radius has grown.
static constexpr double BAOBAB_PLANT_BACKING_MM = 1.0;

// Layer whose collision the landing must overlap for the surface to count as backed. The landing
// sits z_distance_bottom_layers + 1 above the surface, and the collision at a layer carries the
// outlines z_distance_top_layers above it, so probing this far down asks for model at
// BAOBAB_PLANT_BACKING_MM under the surface. The collision is a window, not a point: it holds
// the outlines from z_distance_bottom_layers below the probe layer to z_distance_top_layers
// above it, offset by the support xy distance, so model anywhere in that window and laterally
// within the xy distance of the landing satisfies the probe.
static LayerIndex baobab_plant_backing_layer(const TreeSupportSettings &config, const LayerIndex landing_layer)
{
    const auto depth = LayerIndex(std::ceil(BAOBAB_PLANT_BACKING_MM / unscaled<double>(config.layer_height)));
    return std::max(LayerIndex(0), landing_layer - LayerIndex(config.z_distance_bottom_layers) - 1 -
                                       LayerIndex(config.z_distance_top_layers) - depth);
}

// Radius used to test where a planted trunk's foot can stand: the trunk's collision radius unless
// a narrower foot was fitted to the landing.
static coord_t plant_foot_collision_radius(const TreeSupportSettings &config, const SupportElementState &state)
{
    const coord_t r = support_element_collision_radius(config, state);
    return state.plant_foot_radius > 0 ? std::min(r, state.plant_foot_radius) : r;
}

// Widest foot, down to the nominal trunk radius, whose placeable area the influence area still
// reaches at this layer; 0 when none does. A surface narrower than the trunk (a ridge, a rib,
// a lip) still carries it: the trunk tapers to the fitted foot over its lowest layers instead
// of walking past the only surface it could have stood on. The floor is the nominal trunk
// radius, the section the trunk already has below its tip.
static coord_t baobab_fit_plant_foot(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                     const Polygons &influence_area, const coord_t radius, const LayerIndex layer_idx,
                                     Polygons &landing, const std::function<void()> &throw_on_cancel)
{
    // Placeable areas are cached per rounded radius and shrink as the radius grows, so walk the
    // rounding ladder upward from the narrowest foot: an empty landing there settles it in one
    // intersection, and every wider bucket is tested once. The foot reported is the bucket the
    // landing was tested with, capped at the trunk radius.
    const coord_t floor = std::min(radius, config.branch_radius);
    coord_t best = 0;
    landing.clear();
    for (coord_t r = volumes.ceilRadius(floor, true);; r = volumes.ceilRadius(r + 1, true))
    {
        Polygons hit = intersection_clipped(influence_area, volumes.getPlaceableAreas(r, layer_idx, throw_on_cancel));
        if (hit.empty())
            break;
        best = r;
        landing = std::move(hit);
        if (r >= radius)
            break;
    }
    return std::min(best, radius);
}
...
static void increase_areas_one_layer(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                     ...
                                     // Baobab plant tally, or nullptr when not collecting.
                                     BaobabPlantStats *plant_stats)
{
    using AvoidanceType = TreeModelVolumes::AvoidanceType;
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<size_t> &range)
        {
            for (size_t merging_area_idx = range.begin(); merging_area_idx < range.end(); ++merging_area_idx)
            {
                ...
                // substance under the surface (see baobab_plant_backing_layer), so thin ledges
                // never qualify. Ending the branch here (no offspring) hands it to the existing
                // gracious rest machinery, which settles it on the best placeable layer and
                // emits the bottom contact interface under the foot.
                if (elem.distance_to_top >= baobab_plant_min_depth)
                {
                    Polygons landing;
                    const coord_t foot = baobab_fit_plant_foot(volumes, config, parent.influence_area, radius,
                                                               layer_idx, landing, throw_on_cancel);
                    const bool was_to_bp = parent.state.to_buildplate;
                    const char *outcome = nullptr;
                    bool planted = false;
                    if (!landing.empty())
                    {
                        const LayerIndex backing_layer = baobab_plant_backing_layer(config, layer_idx);
                                 ...
                                 .empty())
                        {
                            ...
                            if (foot < radius)
                                parent.state.plant_foot_radius = foot;
                            planted = true;
                            outcome = "planted";
                            if (plant_stats != nullptr)
                            {
                                plant_stats->planted.fetch_add(1, std::memory_order_relaxed);
                                if (foot < radius)
                                {
                                    plant_stats->fitted.fetch_add(1, std::memory_order_relaxed);
                                    baobab_atomic_min(plant_stats->foot_min, int64_t(foot));
                                }
                            }
                        }
                        else
                        {
                            // A landing existed but nothing under it.
                            outcome = "refused_unbacked";
                            if (plant_stats != nullptr)
                                plant_stats->refused_unbacked.fetch_add(1, std::memory_order_relaxed);
                        }
                    }
                    else if (plant_stats != nullptr)
                    {
                        // A surface within one gather spacing of the branch that even the narrowest
                        // foot could not reach: the branch is beside the surface, not above it.
                        const Polygons &near_surface = volumes.getPlaceableAreas(std::min(radius, config.branch_radius),
                                                                                 layer_idx, throw_on_cancel);
                        if (!near_surface.empty() && get_extents(parent.influence_area)
                                                         .inflated(double(config.settings.support_tree_branch_distance))
                                                         .overlap(get_extents(near_surface)))
                        {
                            outcome = "no_landing";
                            plant_stats->no_landing.fetch_add(1, std::memory_order_relaxed);
                        }
                    }
                    // One line per decision, capped per pass: where the branch can be (influence
                    // area) against where the narrowest foot could stand (placeable area), so a
                    // trunk that walked past a shelf is distinguishable from one refused on it.
                    if (outcome != nullptr && plant_stats != nullptr &&
                        plant_stats->logged.fetch_add(1, std::memory_order_relaxed) < BAOBAB_PLANT_LOG_MAX)
                    {
                        const Polygons &placeable = volumes.getPlaceableAreas(std::min(radius, config.branch_radius),
                                                                              layer_idx, throw_on_cancel);
                        const BoundingBox ib = get_extents(parent.influence_area);
                        const BoundingBox pb = get_extents(placeable);
                        dbg_log(DBG_SUPPORT | DBG_BAOBAB, 0., "BAOBAB-PLANT",
                                "%s layer=%d r=%.2f foot=%.2f dtt=%zu to_bp=%d infl_area=%.1f "
                                "infl_bb=[%.1f,%.1f,%.1f,%.1f] place_area=%.1f place_bb=[%.1f,%.1f,%.1f,%.1f]",
                                outcome, int(layer_idx), unscaled<double>(radius), unscaled<double>(foot),
                                size_t(elem.distance_to_top), int(was_to_bp),
                                area(parent.influence_area) * SCALING_FACTOR * SCALING_FACTOR,
                                unscaled<double>(ib.min.x()), unscaled<double>(ib.min.y()),
                                unscaled<double>(ib.max.x()), unscaled<double>(ib.max.y()),
                                area(placeable) * SCALING_FACTOR * SCALING_FACTOR, unscaled<double>(pb.min.x()),
                                unscaled<double>(pb.min.y()), unscaled<double>(pb.max.x()),
                                unscaled<double>(pb.max.y()));
                    }
                    if (planted)
                        continue;
                }
             ...
             */
                ...
                else
                {
                    ...
                    // A branch that ends here rests on whatever is under it, non-graciously.
                    if (debug_enabled(DBG_SUPPORT))
                    {
                        const BoundingBox ib = get_extents(parent.influence_area);
                        dbg_log(DBG_SUPPORT, 0., "TREE-LOST",
                                "layer=%d dtt=%zu to_bp=%d r=%.2f infl_bb=[%.1f,%.1f,%.1f,%.1f]", int(layer_idx),
                                size_t(parent.state.distance_to_top), int(parent.state.to_buildplate),
                                unscaled<double>(radius), unscaled<double>(ib.min.x()), unscaled<double>(ib.min.y()),
                                unscaled<double>(ib.max.x()), unscaled<double>(ib.max.y()));
                    }
#ifdef TREE_SUPPORTS_TRACK_LOST
                                               ...
                                               SupportElementMerging &src)
{
    ...
    if (area(offset(intersect, scaled<float>(-0.025), JoinType::Miter, 1.2)) <= tiny_area_threshold)
        return false;
                                  ...
                                  std::function<void()> throw_on_cancel)
{
    ...
    // The tree is built in full although the lowest level branches are always bucketed. The extra
    // time is negligible.
...
static void create_layer_pathing(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                 ...
                                 BaobabPlantStats *plant_stats = nullptr)
{
                                                   ...
                                                   SupportElement &first_elem, std::function<void()> throw_on_cancel)
{
    ...
    // check for every layer upwards, up to the point where this influence area was created (either by initial insert or merge) if the branch could be placed on it, and highest up layer index.
    {
        ...
        for (LayerIndex layer_check = elem->state.layer_idx;
             !intersection(elem->influence_area,
                           volumes.getPlaceableAreas(plant_foot_collision_radius(config, elem->state), layer_check,
                                                     throw_on_cancel))
                  .empty();
    ...
    else
    {
        ...
        Polygons landing =
            intersection(last_successfull_layer->influence_area,
                         volumes.getPlaceableAreas(plant_foot_collision_radius(config, last_successfull_layer->state),
        ...
        if (config.settings.support_baobab_plant_on_model && !landing.empty())
        {
            ...
            const Polygons backed = intersection(
                landing,
                volumes.getCollision(0, baobab_plant_backing_layer(config, last_successfull_layer->state.layer_idx),
         ...
         &throw_on_cancel](const tbb::blocked_range<size_t> &range)
        {
            for (size_t idx = range.begin(); idx < range.end(); ++idx)
            {
                                     ...
                                     &movement_directions](coord_t aoffset, double &max_speed)
                {
                    ...
                    // Rounding errors can leave a branch a little further from the model than it has
                    // to be, which puts the tip slightly off the overhang in x and y. The small
                    // offset below takes that back; elsewhere 0.05mm is not noticeable.
                    poly = diff_clipped(offset(union_(poly), std::min(coord_t(50), support_line_width / 4),
                                               JoinType::Miter, 1.2),
                                        collision);
                    return poly;
                };
                    ...
                    if (nozzle_path.size() > 1)
                    {
                        ...
                        if (nozzle_path.size() > 1)
                        {
                            ...
                            polygons = diff_clipped(
                                offset(polygons_with_correct_center, config.support_line_width / 2, JoinType::Miter,
                                       1.2),
                                // Clipping may split the region again, undoing the single part selection above.
                                collision);
                        }
                    }
                }
            ...
            [&](const tbb::blocked_range<size_t> &range)
            {
                for (size_t processing_idx = range.begin(); processing_idx < range.end(); ++processing_idx)
                {
                    ...
                    if (do_something)
                    {
                        ...
                        Polygons max_allowed_area = offset(draw_area.polygons, float(max_outer_wall_distance),
                                                           JoinType::Miter, 1.2);
                        for (int32_t parent_idx : draw_area.element->parents)
                        {
                            const SupportElement &parent = layer_above[parent_idx];
                                   ...
                                   ++processing_idx)
                              {
                                  ...
                                  for (int32_t parent_idx : draw_area.element->parents)
                                  {
                                      ...
                                      Polygons result = offset(linear_data[processing_base_above + parent_idx].polygons,
                                                               max_outer_line_increase, JoinType::Miter, 1.2);
        ...
        [&](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                if (!base_layer_polygons.empty())
                {
                    ...
                    base_layer_polygons = smooth_outward(union_(base_layer_polygons), config.support_line_width);
                ...
                if (config.support_bottom_layers > 0 && !base_layer_polygons.empty())
                {
                    ...
                    if (!floor_layer.empty())
                    {
                        ...
                        base_layer_polygons = diff_clipped(
                            base_layer_polygons, offset(support_bottom->polygons, scaled<float>(0.01), JoinType::Miter,
 ...
 */
// Determinism fingerprints of the assembled support layers: the islands (TREE_LAYERS_FP) and
// the extrusions (TREE_PATHS_FP, role folded into each path's hash), one line per layer.
static void determ_fp_support_layers(const char *tag, const SupportLayerPtrs &support_layers, const bool paths)
{
    if (!debug_enabled(DBG_SUPPORT))
        return;
    for (const SupportLayer *layer : support_layers)
    {
        ...
        if (paths)
        {
            ...
            for (const ExtrusionEntity *ee : layer->support_fills.flatten().entities)
            {
                const Polyline pl = ee->as_polyline();
                const bool iface = ee->role().is_support_interface();
                fp.add(DetermFingerprint::mix(DetermFingerprint::polyline(pl.points), uint64_t(iface)));
                fp.points += pl.points.size();
                length += pl.length();
                interface_paths += iface;
            }
            dbg_log(DBG_SUPPORT, layer->print_z, "DETERM",
                    "%s paths=%zu iface=%zu pts=%zu len=%.3fmm hash=%016llx ord=%016llx", tag, fp.count,
                    interface_paths, fp.points, unscaled<double>(length), (unsigned long long) fp.hash,
        ...
        else
        {
            if (layer->support_islands.empty())
                continue;
            for (const ExPolygon &island : layer->support_islands)
            {
                fp.add(island.contour);
                fp.add(island.holes);
            }
            dbg_log(DBG_SUPPORT, layer->print_z, "DETERM",
                    "%s islands=%zu polys=%zu pts=%zu area=%.4fmm2 hash=%016llx ord=%016llx", tag,
                    layer->support_islands.size(), fp.count, fp.points, fp.area_mm2(), (unsigned long long) fp.hash,
                    (unsigned long long) fp.ordered);
        }
    }
}
                                   ...
                                   const std::vector<Polygons> &additional_excluded_areas = {})
{
    ...
    for (std::pair<TreeSupportSettings, std::vector<size_t>> &processing : grouped_meshes)
    {
        ...
        TreeModelVolumes volumes{print_object, build_volume, config.maximum_move_distance,
        ...
        std::vector<Polygons> overhangs = generate_overhangs(config, *print.get_object(processing.second.front()),
                                                             blockers, throw_on_cancel);
        pf_tree.stage("  tree: generate_overhangs");
        if (debug_enabled(DBG_SUPPORT))
            for (size_t layer_idx = 0; layer_idx < overhangs.size(); ++layer_idx)
                if (!overhangs[layer_idx].empty())
                {
                    DetermFingerprint fp;
                    fp.add(overhangs[layer_idx]);
                    const size_t blocked = layer_idx < blockers.size() ? blockers[layer_idx].size() : 0;
                    dbg_log(DBG_SUPPORT, layer_z(print_object.slicing_parameters(), config, layer_idx), "DETERM",
                            "TREE_OVERHANG_FP layer=%zu polys=%zu pts=%zu area=%.4fmm2 blockers=%zu hash=%016llx "
                            "ord=%016llx",
                            layer_idx, fp.count, fp.points, fp.area_mm2(), blocked, (unsigned long long) fp.hash,
                            (unsigned long long) fp.ordered);
                }
        ...
        volumes.determ_fingerprint("TREE_VOLUMES_FP");
        ...
        if (has_support)
        {
            ...
            // value is the area where support may be placed. As this is calculated in CreateLayerPathing it is saved
            // and reused when the branches are drawn
            std::vector<SupportElements> move_bounds(num_support_layers);

            // ### Place tips of the support tree
            for (size_t mesh_idx : processing.second)
                generate_initial_areas(*print.get_object(mesh_idx), volumes, config, overhangs, blockers, move_bounds,
                                       interface_placer, throw_on_cancel);

            auto t_gen = std::chrono::high_resolution_clock::now();
            pf_tree.stage("  tree: generate_initial_areas");
            determ_fp_move_bounds("TREE_SEED_FP", print_object.slicing_parameters(), config, move_bounds);
            ...
            {
                BaobabPlantStats plant_stats;
                const bool plant_tally = is_baobab_object(print_object) &&
                                         config.settings.support_baobab_plant_on_model &&
                                         (debug_enabled(DBG_SUPPORT) || debug_enabled(DBG_BAOBAB));
                create_layer_pathing(volumes, config, move_bounds, throw_on_cancel,
                                     plant_tally ? &plant_stats : nullptr);
                // Plant tally for this group. A trunk that reaches the bed without appearing in
                // any count never overlapped a placeable landing.
                if (plant_tally)
                    dbg_log(DBG_SUPPORT | DBG_BAOBAB, 0., "TREE-PLANT",
                            "planted=%zu refused_unbacked=%zu no_landing=%zu fitted=%zu foot_min=%.2f",
                            plant_stats.planted.load(), plant_stats.refused_unbacked.load(),
                            plant_stats.no_landing.load(), plant_stats.fitted.load(),
                            plant_stats.fitted.load() > 0 ? unscaled<double>(plant_stats.foot_min.load()) : 0.);
                // Element census: one line per layer whose branch count differs from the layer
                // above, with the union extents of the influence areas, so merges and the walk
                // of each column read directly from the stream.
                if (debug_enabled(DBG_SUPPORT))
                {
                    size_t last_count = std::numeric_limits<size_t>::max();
                    for (int layer_idx = int(move_bounds.size()) - 1; layer_idx >= 0; --layer_idx)
                    {
                        const SupportElements &layer = move_bounds[layer_idx];
                        if (layer.size() == last_count)
                            continue;
                        last_count = layer.size();
                        BoundingBox bb;
                        for (const SupportElement &el : layer)
                            bb.merge(get_extents(el.influence_area));
                        dbg_log(DBG_SUPPORT, layer_z(print_object.slicing_parameters(), config, size_t(layer_idx)),
                                "TREE-ELEMS", "layer=%d count=%zu infl_bb=[%.1f,%.1f,%.1f,%.1f]", layer_idx,
                                layer.size(), unscaled<double>(bb.min.x()), unscaled<double>(bb.min.y()),
            ...
            determ_fp_move_bounds("TREE_PATH_FP", print_object.slicing_parameters(), config, move_bounds);
            ...
            determ_fp_move_bounds("TREE_NODE_FP", print_object.slicing_parameters(), config, move_bounds);
            ...
            organic_draw_branches(*print.get_object(processing.second.front()), volumes, config, move_bounds,
                                  ...
                                  throw_on_cancel, additional_excluded_areas, overhangs, &baobab_canopy_footprints);
            pf_tree.stage("  tree: organic_draw_branches");

            determ_fp_layers(DBG_SUPPORT, "TREE_DRAW_FP", "base", intermediate_layers);
            determ_fp_layers(DBG_SUPPORT, "TREE_DRAW_FP", "contacts", top_contacts);
            determ_fp_layers(DBG_SUPPORT, "TREE_DRAW_FP", "bottom", bottom_contacts);
            determ_fp_layers(DBG_SUPPORT, "TREE_DRAW_FP", "interfaces", interface_layers);
            determ_fp_layers(DBG_SUPPORT, "TREE_DRAW_FP", "base_interfaces", base_interface_layers);
            if (debug_enabled(DBG_BAOBAB))
                for (const auto &[z_um, rings] : baobab_canopy_footprints)
                {
                    DetermFingerprint fp;
                    fp.add(rings);
                    dbg_log(DBG_BAOBAB, double(z_um) * 0.001, "DETERM",
                            "BAOBAB_CANOPY_FP rings=%zu pts=%zu area=%.4fmm2 hash=%016llx ord=%016llx", fp.count,
                            fp.points, fp.area_mm2(), (unsigned long long) fp.hash, (unsigned long long) fp.ordered);
                }
            ...
            determ_fp_layers(DBG_SUPPORT, "TREE_IFACE_FP", "base", intermediate_layers);
            determ_fp_layers(DBG_SUPPORT, "TREE_IFACE_FP", "contacts", top_contacts);
            determ_fp_layers(DBG_SUPPORT, "TREE_IFACE_FP", "interfaces", interface_layers);
            determ_fp_layers(DBG_SUPPORT, "TREE_IFACE_FP", "base_interfaces", base_interface_layers);
            ...
            if (closing_radius > 0)
            {
                ...
                // Close holes in lower interface layers
            ...
            // A hole the object pierces is not dead space: it is the clearance pocket around a
            // feature standing inside the canopy. Where the rings first close around the
            // feature, filling that hole adds a band between the pocket and the re-trimmed
            // clearance that the layer below never had (its pocket was still open), and that
            // band prints over air. Such holes stay; only object-free voids are filled.
            if (baobab_object)
            {
                const SlicingParameters &slicing_params = print_object.slicing_parameters();
                auto object_polygons_at = [&print_object, &slicing_params](const SupportGeneratorLayer &layer)
                {
                    Polygons out;
                    for (const Layer *object_layer : print_object.layers())
                    {
                        ...
                        if (object_layer->print_z <
                            ...
                            continue;
                        polygons_append(out, to_polygons(object_layer->lslices));
                    }
                    return out;
                };
                for (SupportGeneratorLayer *layer : intermediate_layers)
                    if (layer != nullptr && !layer->polygons.empty())
                    {
                        ...
                        for (ExPolygon &expoly : expolys)
                        {
                            ...
                            if (!object_known)
                            {
                                object_here = object_polygons_at(*layer);
                                object_known = true;
                            }
                            const size_t holes_before = expoly.holes.size();
                            expoly.holes.erase(
                                std::remove_if(expoly.holes.begin(), expoly.holes.end(),
                                               [&object_here](const Polygon &hole)
                                               {
                                                   if (object_here.empty())
                                                       return true;
                                                   Polygon region = hole;
                                                   region.reverse();
                                                   if (intersection(Polygons{region}, object_here).empty())
                                                       return true;
                                                   DBG_COUNT("BAOBAB_BASE_HOLE_KEPT_OBJECT");
                                                   return false;
                                               }),
                                expoly.holes.end());
                            holed = holed || expoly.holes.size() != holes_before;
                        }
                        if (holed)
                            layer->polygons = to_polygons(expolys);
                    }
            ...
            determ_fp_layers(DBG_SUPPORT, "TREE_TRIM_FP", "base", intermediate_layers);
            determ_fp_layers(DBG_SUPPORT, "TREE_TRIM_FP", "contacts", top_contacts);
            determ_fp_layers(DBG_SUPPORT, "TREE_TRIM_FP", "interfaces", interface_layers);
            determ_fp_layers(DBG_SUPPORT, "TREE_TRIM_FP", "base_interfaces", base_interface_layers);
        ...
        determ_fp_support_layers("TREE_LAYERS_FP", print_object.support_layers(), false);
        ...
        determ_fp_support_layers("TREE_PATHS_FP", print_object.support_layers(), true);
