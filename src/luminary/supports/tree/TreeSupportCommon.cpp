...
TreeSupportMeshGroupSettings::TreeSupportMeshGroupSettings(const PrintObject &print_object)
{
    ...
    assert(config.support_material_style == smsOrganic || config.support_material_style == smsBaobab);
    ...
    this->resolution = scaled<coord_t>(config.gcode_resolution.value);
    // Arache feature
    ...
    // Area is in mm^2, need to convert to scaled^2 units for area comparisons
    ...
    this->support_tree_min_opening = scaled<coord_t>(std::max(0., config.support_tree_min_opening.value));
