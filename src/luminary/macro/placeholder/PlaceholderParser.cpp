...
#include "luminary/macro/orca_aliases/OrcaGCodeAliases.hpp"
...
#define PREFLIGHT_REGEX_NAMESPACE std
#else /* USE_CPP11_REGEX */
#define PREFLIGHT_REGEX_NAMESPACE boost
...
{
    this->set("version", std::string(PREFLIGHT_VERSION));
...
{
    // The documented PREFLIGHT_ prefix and its SLIC3R_ twin, the pair ConfigBase::setenv_ exports.
    for (char **env = environ; *env; ++env)
    {
        if (strncmp(*env, "PREFLIGHT_", 10) == 0 || strncmp(*env, "SLIC3R_", 7) == 0)
        {
...
// Using an encoding which accepts unsigned chars. The ascii encoding faults on bytes above 127
// because it indexes 7 bit classification tables with negative char values. Character classification
// is therefore latin-1, one byte at a time.
...
struct OptWithPos
{
    ...
    // Canonical preFlight key name, set when an Orca alias was resolved.
...
struct expr
{
    ...
    static void regex_op(const expr &lhs, IteratorRange &rhs, char op, expr &out)
    {
        ...
        try
        {
            ...
            bool result = PREFLIGHT_REGEX_NAMESPACE::regex_match(*subject, PREFLIGHT_REGEX_NAMESPACE::regex(pattern));
            if (op == '!')
                result = !result;
            out.set_b(result);
        }
        catch (PREFLIGHT_REGEX_NAMESPACE::regex_error &ex)
        {
    ...
    static void one_of_test(const expr &match, const expr &pattern, expr &out)
    {
        ...
        if (!out.b())
        {
            ...
            if (RegEx)
            {
                try
                {
                    out.set_b(PREFLIGHT_REGEX_NAMESPACE::regex_match(match.s(),
                                                                     PREFLIGHT_REGEX_NAMESPACE::regex(pattern.s())));
                }
                catch (PREFLIGHT_REGEX_NAMESPACE::regex_error &)
                {
                    // Syntax error in the regular expression
...
struct MyContext : public ConfigOptionResolver
{
    ...
    // Resolve with optional alias tracking. When resolved_key is non-null and an
    ...
    // Whitelist of nullable variables that default to 0 when nil,
    ...
    static void legacy_variable_expansion(const MyContext *ctx, IteratorRange &opt_key, std::string &output)
    {
        ...
        // Check locals first so [bracket] syntax matches {brace} behavior
                                 ...
                                 NewOldVariable &out)
    {
        if (!ctx->skipping())
        {
            ...
            // Allow {local} to shadow config keys. The resolve_variable
                                      ...
                                      const Iterator &it_begin, const Iterator &it_end, const Iterator &it_error)
    {
        ...
        {
            // The tag contains an explanatory string.
            msg += ": ";
        ...
        else
        {
            ...
            else if (const spirit::utf8_string *expected = boost::get<spirit::utf8_string>(&info.value);
                     expected != nullptr && !expected->empty())
            {
                // Literal parsers (tags "literal-char", "literal-string") carry the expected token in the info value.
                msg += ". Expecting \"";
                msg += *expected;
                msg += "\"";
            }
        ...
        for (size_t i = 0; i < error_pos; ++i)
            msg += ' ';
        msg += "^\n";
        // Braces and brackets are template syntax everywhere in the text, including after a G-code ";".
        // A failure past a ";" on the error line is almost always prose in a comment, so explain the rule
        // and the literal-output workaround.
        if (auto semicolon = error_line.find(';'); semicolon != std::string::npos && semicolon < size_t(error_pos))
            msg +=
                "Note: {} and [] are template syntax everywhere in custom G-code, including inside \";\" comments.\n"
                "To output them as literal text, wrap them in quotes inside braces, for example {\"[literal text]\"}.\n";
    }
        ...
        for (size_t i = 1; i < table.table.size(); ++i)
        {
            ...
            if (!evaluated && x >= x0 && x <= x1)
            {
                ...
                if (x == x0)
                    ...
                    out.set_d(Luminary::lerp(y0, y1, (x - x0) / (x1 - x0)));
                evaluated = true;
            }
        }
        if (!evaluated)
...
*/
        ...
        // Legacy variable expansion, in the form of [scalar_variable] or [vector_variable_index].
...
static std::string process_macro(const std::string &templ, client::MyContext &context)
{
    ...
    if (!context.error_message.empty())
    {
        ...
        throw Luminary::PlaceholderParserError(context.error_message);
    }
    return output;
}
...
// Throws Luminary::RuntimeError on syntax or runtime error.
