...
public:
    ...
    explicit AppConfig(EAppMode mode) : m_mode(mode) { this->reset(); }
    ...
    // Load preFlight.ini from a user profile directory (or a datadir, if configured).
    // return error string or empty strinf
    std::string load();
    // Load from an explicit path.
    std::string load(const std::string &path);
    // Store preFlight.ini into a user profile directory (or a datadir, if configured).
    void save();
    ...
    bool get_bool(const std::string &section, const std::string &key) const { return this->get(section, key) == "1"; }
    ...
    bool get_bool(const std::string &key) const { return this->get(key) == "1"; }
    ...
    bool set(const std::string &key, const std::string &value) { return this->set("", key, value); }
    ...
    bool has(const std::string &key) const { return this->has("", key); }
    ...
    // Get the default config path from Luminary::data_dir().
    std::string config_path() const;

    // Returns true if the user's data directory predates the config updating mechanism (no updating)
    bool legacy_datadir() const { return m_legacy_datadir; }
    void set_legacy_datadir(bool value) { m_legacy_datadir = value; }

    // Get the version check url.
    // This returns a hardcoded string unless it is overriden by "version_check_url" in the ini file.
    std::string version_check_url() const;
    // Get the release notes URL for displaying in the update dialog.
    std::string release_notes_url() const;
    // Get the url to the vendor index archive zip.
    std::string index_archive_url() const;
    // Get the url to the folder with vendor profile files.
    std::string profile_folder_url() const;

    // Returns the original application version found in the ini file before it was overwritten
    ...
    bool set_mouse_device(const std::string &name, double translation_speed, double translation_deadzone,
