...
// Define this to store the print/filament/printer presets in a "presets" subdirectory of the config dir.
// #define PREFLIGHT_PROFILE_USE_PRESETS_SUBDIR
    ...
    , physical_printers(PhysicalPrinter::printer_options(), this)
{
    ...
    // The one default printer is FFF.
    {
        Preset &preset = this->printers.default_preset(0);
        for (const char *key :
             {"printer_settings_id", "printer_vendor", "printer_model", "printer_variant", "thumbnails",
              // The print host keys exist on the printer preset so older printer profiles still parse;
              // they are carried over to the Physical Printer profile.
...
PresetBundle &PresetBundle::operator=(const PresetBundle &rhs)
{
    ...
    m_parked_extruder_filaments = rhs.m_parked_extruder_filaments;
...
void PresetBundle::reset(bool delete_files)
{
    ...
    this->m_parked_extruder_filaments.clear();
...
{
    boost::filesystem::path data_dir = boost::filesystem::path(Luminary::data_dir());
    std::initializer_list<boost::filesystem::path> paths = {
        data_dir, data_dir / "vendor", data_dir / "cache", data_dir / "cache" / "vendor", data_dir / "shapes",
#ifdef PREFLIGHT_PROFILE_USE_PRESETS_SUBDIR
        // Store the print/filament/printer presets into a "presets" directory.
        data_dir / "presets", data_dir / "presets" / "print", data_dir / "presets" / "filament",
        data_dir / "presets" / "printer", data_dir / "presets" / "physical_printer"
#else
        // Store the print/filament/printer presets directly in the config dir.
        data_dir / "print", data_dir / "filament", data_dir / "printer", data_dir / "physical_printer"
#endif
    };
    for (const boost::filesystem::path &path : paths)
    {
        ...
        if (!boost::filesystem::is_directory(subdir) && !boost::filesystem::create_directory(subdir))
            throw Luminary::RuntimeError(std::string("preFlight was unable to create its data directory at ") +
                                         subdir.string());
    }
}
...
#ifdef PREFLIGHT_PROFILE_USE_PRESETS_SUBDIR
        // Store the print/filament/printer presets into a "presets" directory.
        data_dir / "presets" / "print", data_dir / "presets" / "filament", data_dir / "presets" / "printer",
        data_dir / "presets" / "physical_printer"
#else
        // Store the print/filament/printer presets directly in the config dir.
        from_data_dir / "print", from_data_dir / "filament", from_data_dir / "printer",
    ...
    ForwardCompatibilitySubstitutionRule compatibility_rule, const std::set<std::string> &installed_models)
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(dir))
        if (Luminary::is_ini_file(dir_entry))
        {
    ...
    // Make all presets editable (no locked system presets)
...
const PresetCollection &PresetBundle::get_presets(Preset::Type type) const
{
    ...
    return type == Preset::TYPE_PRINT ? prints : type == Preset::TYPE_FILAMENT ? filaments : printers;
}
                                                                    ...
                                                                    const std::string &alias) const
{
    ...
    const PresetCollection &presets = preset_type == Preset::TYPE_PRINT ? prints : filaments;
                                   ...
                                   const PresetPreferences &preferred_selection /* = PresetPreferences()*/)
{
    ...
    // Update visibility of filament presets
    ...
    {
        // Don't run the following code, as we want to activate default filament profiles when installing and selecting a new printer.
        // Only run this code if just a filament was installed by Config Wizard for an active Printer.
...
void PresetBundle::export_selections(AppConfig &config)
{
    ...
    // All filament selections are saved in extruder_filaments, for single and multi material printers
    // alike, so filaments and extruders_filaments need no cross check here.
...
{
    return this->full_fff_config();
}

DynamicPrintConfig PresetBundle::full_config_secure() const
{
    DynamicPrintConfig config = this->full_config();
    // Drop the print host credentials carried by older printer profiles.
                                                   ...
                                                   ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    if (is_gcode_file(path))
    {
        ...
        if (file == nullptr)
            throw Luminary::RuntimeError(format("Error opening file %1%", path));
    ...
    {
        throw Luminary::RuntimeError(std::string("The Config Bundle cannot be loaded: ") + path +
                                     "\n\tReason: " + err.what());
    }
    catch (const boost::property_tree::file_parser_error &err)
    {
        throw Luminary::RuntimeError(format("Failed loading the Config Bundle \"%1%\": %2% at line %3%", err.filename(),
                                            err.message(), err.line()));
    }
    catch (const std::runtime_error &err)
    {
        throw Luminary::RuntimeError(std::string("Failed loading the preset file: ") + path +
                                     "\n\tReason: " + err.what());
    }
    ...
    try
    {
        ...
        case CONFIG_FILE_TYPE_UNKNOWN:
            throw Luminary::RuntimeError(std::string("Unknown configuration file type: ") + path);
        case CONFIG_FILE_TYPE_APP_CONFIG:
            throw Luminary::RuntimeError(std::string("Invalid configuration file: ") + path +
    ...
    {
        throw Luminary::RuntimeError(format("Invalid configuration file %1%: %2%", path, e.what()));
    }
                                           ...
                                           DynamicPrintConfig &&config)
{
...

    {
        ...
        else
        {
            ...
            // The edited filament is the first extruder's unless load_external_preset selected a
            // modified filament for another extruder above (only one preset can carry the
            // project's modifications). load_external_preset selects an unmodified filament only
            // when inserting the new presets moved the selected index (selected and edited names
            // then differ), so the first extruder is selected by name here in every other case;
            // otherwise the filament tab keeps the preset of the previous project and the first
            // extruder follows it instead of the project.
            if (!any_modified)
            {
                if (this->filaments.get_selected_preset_name() != extr_names[0] ||
                    this->filaments.get_edited_preset().name != extr_names[0])
                    this->filaments.select_preset_by_name(extr_names[0], true, true);
            }
            else if (this->filaments.get_selected_preset_name() != this->filaments.get_edited_preset().name)
                                           ...
                                           const std::vector<std::string> &system_profiles)
{
    ...
    for (const Prst &prst : presets)
    {
        ...
        if (Luminary::unescape_strings_cstyle(prst.node->get<std::string>("inherits", ""), inherits_names))
        {
        ...
        if (!inherits_system.empty())
        {
            ...
            prst.node->put("inherits", Luminary::escape_string_cstyle(inherits_system.front()));
        }
    }
    ...
    const std::string &path, LoadConfigBundleAttributes flags, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    pt::ptree tree;
    {
        ...
        {
            throw Luminary::RuntimeError(format("Failed loading config bundle \"%1%\"\nError: \"%2%\" at line %3%",
    ...
    for (const auto &section : tree)
    {
        ...
        if (ph_printers != nullptr)
        {
            ...
            auto file_path = (boost::filesystem::path(data_dir())
                              ...
                              / "presets"
#else
            // Store the physical printers directly in the config dir.
#endif
...
void PresetBundle::update_multi_material_filament_presets()
{
    ...
    {
        // Park the filaments of the slots this printer does not have, so a later switch to a printer
        // with more nozzles restores them instead of copying the neighbouring slot.
        if (m_parked_extruder_filaments.size() < this->extruders_filaments.size())
            m_parked_extruder_filaments.resize(this->extruders_filaments.size());
        for (size_t id = num_extruders; id < this->extruders_filaments.size(); ++id)
            m_parked_extruder_filaments[id] = this->extruders_filaments[id].get_selected_preset_name();
        this->extruders_filaments.resize(num_extruders);
    }
    else
        // Append the rest of filament presets: the parked filament of the slot when it still exists,
        // otherwise the neighbouring slot's.
        for (size_t id = extruders_filaments.size(); id < num_extruders; id++)
        {
            std::string name = id == 0 ? filaments.first_visible().name
                                       : extruders_filaments[id - 1].get_selected_preset_name();
            if (id < m_parked_extruder_filaments.size() && !m_parked_extruder_filaments[id].empty())
            {
                if (const Preset *parked = this->filaments.find_preset(m_parked_extruder_filaments[id]);
                    parked != nullptr && parked->is_visible)
                    name = parked->name;
                m_parked_extruder_filaments[id].clear();
            }
            extruders_filaments.emplace_back(ExtruderFilaments(&filaments, id, name));
        }
...
{
    // Filaments carry no printer compatibility, so the filament selection policy is not consulted.
    UNUSED(select_other_filament_if_incompatible);
    ...
    std::function<bool(const std::string &, const std::string &, std::string &)> secret_callback)
{
    ...
    // Put a comment at the first line including the time stamp and the application version.
