...
public:
    ...
    // Load ini files of all types (print, filament, printer) from Luminary::data_dir() / presets.
    ...
    // Returns a vector of values of a given config option for all currently used filaments.
    // Modified value is returned for modified preset.
    ...
    static std::array<Preset::Type, 3> types_list()
    {
        return {Preset::TYPE_PRINTER, Preset::TYPE_PRINT, Preset::TYPE_FILAMENT};
    }

private:
    // Filament names of the slots a printer switch dropped, by slot index, so a later switch to a
    // printer with more nozzles restores the project's assignment. Cleared when a project or a
    // config bundle establishes a new assignment.
    std::vector<std::string> m_parked_extruder_filaments;
