...
// A map is used rather than an unordered_map because iterators must stay valid: Preset and the
// configuration wizard hold pointers to VendorProfiles.
...
public:
    ...
    // into the default configuration location.
    ...
    {
        assert(this->type == TYPE_FILAMENT);
    ...
    {
        assert(this->type == TYPE_PRINT || this->type == TYPE_FILAMENT);
...
public:
    ...
    PresetCollection(Preset::Type type, const std::vector<std::string> &keys,
                     const Luminary::StaticPrintConfig &defaults, const std::string &default_name = "- default -");
    ...
    void add_default_preset(const std::vector<std::string> &keys, const Luminary::StaticPrintConfig &defaults,
...
private:
    ...
    {
        auto it = Luminary::lower_bound_by_predicate(m_presets.begin() + m_num_default_presets, m_presets.end(),
...
// The printer collection picks its default preset from the config and looks system presets up by
// model and variant.
...
public:
    PrinterPresetCollection(Preset::Type type, const std::vector<std::string> &keys,
                            const Luminary::StaticPrintConfig &defaults,
...
private:
    ...
    {
        return Luminary::lower_bound_by_predicate(m_extr_filaments.begin(), m_extr_filaments.end(),
