...
#include "luminary/macro/placeholder/PlaceholderParser.hpp"
...
VendorProfile VendorProfile::from_ini(const ptree &tree, const boost::filesystem::path &path, bool load_all)
{
    ...
    for (auto &section : tree)
    {
        if (boost::starts_with(section.first, printer_model_key))
        {
            ...
            if (Luminary::unescape_strings_cstyle(variants_field, variants))
            {
    ...
    // Load filaments to be installed by default
                              ...
                              const PresetWithVendorProfile &active_printer)
{
    ...
    if (!has_compatible_prints && !condition.empty())
    {
        ...
        {
            // A condition that fails to parse counts as compatible, so the preset stays selectable.
...
void Preset::set_visible_from_appconfig(const AppConfig &app_config)
{
    ...
    {
        // User .ini files in the printer folder are the source of truth.
        // The vendor pointer is only for bed model/texture resolution.
        is_visible = true;
    }
    else if (type == TYPE_FILAMENT)
    {
        // All filament presets are always visible
        is_visible = true;
    }
}
...
// Preset::print_options, filament_options and printer_options are defined in SettingsSpec.cpp,
// the translation unit that owns the row specification they read.
...
PresetCollection::PresetCollection(Preset::Type type, const std::vector<std::string> &keys,
                                   const Luminary::StaticPrintConfig &defaults, const std::string &default_name)
...
void PresetCollection::add_default_preset(const std::vector<std::string> &keys,
                                          const Luminary::StaticPrintConfig &defaults, const std::string &preset_name)
{
    // Insert just the default preset.
                                    ...
                                    ForwardCompatibilitySubstitutionRule substitution_rule)
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(dir))
        if (Luminary::is_ini_file(dir_entry))
        {
            ...
            // User preset files take priority over vendor bundle presets.
            ...
            try
            {
                ...
                {
                    throw Luminary::RuntimeError(std::string("The selected preset cannot be loaded: ") + preset.file +
                                                 "\n\tReason: " + err.what());
                }
                catch (const std::runtime_error &err)
                {
                    throw Luminary::RuntimeError(std::string("Failed loading the preset file: ") + preset.file +
                                                 "\n\tReason: " + err.what());
                }
...
static bool profile_print_params_same(const DynamicPrintConfig &cfg_old, const DynamicPrintConfig &cfg_new)
{
    ...
    for (const char *key :
         {"compatible_prints", "compatible_prints_condition", "compatible_printers", "compatible_printers_condition",
          "inherits", "print_settings_id", "filament_settings_id", "printer_settings_id", "filament_vendor",
          "printer_model", "printer_variant", "default_print_profile", "default_filament_profile", "print_host",
    ...
    if (found)
    {
        if (select != LoadAndSelect::Never)
        {
            ...
            if (!profile_print_params_same(it->config, cfg))
            {
                ...
                for (const char *key :
                     {"print_settings_id", "filament_settings_id", "printer_settings_id", "filament_vendor",
...
const Preset *PresetCollection::get_preset_parent(const Preset &child) const
{
    ...
    return
        ...
        (preset == nullptr /* || preset->is_default */ ||
         ...
         preset->is_external ||
         // this should not happen, however people are creative
         preset == &child)
            ? nullptr
            : preset;
}
...
const std::string &PresetCollection::get_preset_name_by_alias(const std::string &alias) const
{
    for (
        ...
        auto it =
            Luminary::lower_bound_by_predicate(m_map_alias_to_profile_name.begin(), m_map_alias_to_profile_name.end(),
...
static std::vector<std::string> s_PhysicalPrinter_opts{
    ...
    "preset_names", "printer_technology", "host_type", "print_host", "print_host_webui", "printhost_apikey",
...
// Only the Klipper host type serves its web interface apart from Moonraker, so a web interface address
// left over from a host-type switch never redirects another host type's Printer tab.
std::string PhysicalPrinter::web_interface_host(const DynamicPrintConfig &config)
{
    ...
    {
        const std::string webui = config.opt_string("print_host_webui");
        if (!webui.empty())
            return webui;
    }
...
{
    return config.opt_string("print_host").empty() && config.opt_string("print_host_webui").empty() &&
           config.opt_string("printhost_apikey").empty() && config.opt_string("printhost_cafile").empty() &&
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(dir))
        if (Luminary::is_ini_file(dir_entry))
        {
            ...
            try
            {
                ...
                {
                    throw Luminary::RuntimeError(std::string("The selected preset cannot be loaded: ") + printer.file +
                                                 "\n\tReason: " + err.what());
                }
                catch (const std::runtime_error &err)
                {
                    throw Luminary::RuntimeError(std::string("Failed loading the preset file: ") + printer.file +
                                                 "\n\tReason: " + err.what());
                }
...
{
    if (case_sensitive_search)
        return Luminary::lower_bound_by_predicate(m_printers.begin(), m_printers.end(),
...
const std::string &ExtruderFilaments::get_preset_name_by_alias(const std::string &alias) const
{
    ...
    for (
        // Find the 1st profile name with the alias.
        auto it = Luminary::lower_bound_by_predicate(aliases_map.begin(), aliases_map.end(),
                                                     [&alias](auto &l) { return l.first < alias; });
        // Continue over all profile names with the same alias.
...
std::string system_printer_bed_model(const Preset &preset)
{
    ...
    {
        out = Luminary::data_dir() + "/vendor/" + preset.vendor->id + "/" + pm->bed_model;
...
std::string system_printer_bed_texture(const Preset &preset)
{
    ...
    {
        out = Luminary::data_dir() + "/vendor/" + preset.vendor->id + "/" + pm->bed_texture;
        if (!boost::filesystem::exists(boost::filesystem::path(out)))
            out.clear();
    }
    return out;
...
bool vendor_profile_has_all_resources(const VendorProfile &vp)
{
    ...
    std::string vendor_folder = Luminary::data_dir() + "/vendor/" + vp.id + "/";
    std::string cache_folder = Luminary::data_dir() + "/cache/" + vp.id + "/";
