...
std::string OrcaKeyMapper::transform_small_perimeter_threshold(const std::string &value)
{
    // Orca: circle radius in mm -> preFlight: circle diameter in mm
    size_t parsed = 0;
    const double radius = string_to_double_decimal_point(value, &parsed);
    if (parsed == 0)
        return value;
    return float_to_string_decimal_point(radius * 2.);
}

std::string OrcaKeyMapper::transform_min_wall_length(const std::string &value)
{
    // Orca: a multiple of the wall's own width -> preFlight: the same share as a percentage
    size_t parsed = 0;
    const double factor = string_to_double_decimal_point(value, &parsed);
    if (parsed == 0)
        return value;
    return float_to_string_decimal_point(factor * 100.) + "%";
}
...
void OrcaKeyMapper::build_process_mappings()
{
    m_mappings[Preset::TYPE_PRINT] = {
        ...
        XFORM("small_perimeter_threshold", "small_perimeter_diameter", transform_small_perimeter_threshold),
        XFORM("min_length_factor", "min_wall_length", transform_min_wall_length),
        DIRECT("travel_speed"),
        DIRECT("bridge_speed"),
        ...
        // preFlight emits no arcs.
        ORCA_ONLY("enable_arc_fitting"),
        ...
        // Thin features are absorbed by the perimeter generator; there is no switch.
        ORCA_ONLY("detect_thin_wall"),
        ...
        //    speed_0 = 0% overlap (bridge) ... speed_3 = 75% overlap
