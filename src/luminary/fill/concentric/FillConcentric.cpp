                                        ...
                                        ToThickPolylineFn to_thick_polyline_fn)
{
    ...
    // A parent must have a larger area than its child, so the tree cannot contain a cycle, and its
    // bounding box must contain the child's, so a sibling is never picked as the parent.
                                          ...
                                          Polylines &polylines_out)
{
    ...
    {
        distance = Luminary::FillConcentric::_adjust_solid_spacing(bounding_box.size()(0), distance);
    ...
    // offset2_ex does a morphological closing (shrink then expand), and Clipper2 fragments complex
    // shapes with holes instead of keeping them unified, so the polygon count grows exponentially:
    //   - Iteration 20: 184 loops to 1,005 loops (5x jump)
    //   - Iteration 30: 3,366 loops
    //   - Iteration 50: 44,378 loops (then hangs in union_pt)
    //
    // A plain offset_ex shrink avoids the fragmentation and still shrinks the geometry to empty
    // without a MAX_LOOPS cap, at the cost of slightly less smooth infill paths.
    ...
    // Compute a physically-derived loop limit from the geometry. The maximum number
    ...
    // The test is on area, not point count: a square shrinking inward keeps its 4 points while its
    // area falls, so a point-count check would exit after 5 iterations on any rectangle.
    ...
    // Do not use union_pt_chained_outside_in here. It feeds all accumulated loops
    ...
    // The loops leave here as open polylines, already split, so the outermost loop starts at the point
    // closest to (0, 0) rather than at a seam the G-code generator picks.
}
        ...
        if (params.perimeter_generator == PerimeterGeneratorType::Athena)
        {
            ...
            if (Luminary::debug_enabled(Luminary::DBG_FILL))
            {
                ...
                dbg_log(Luminary::DBG_FILL, this->z, "FILL",
                        "CONC_IN area=%.3fmm2 holes=%zu spacing=%.4f loop_sets=%zu", band_area, expolygon.holes.size(),
                        unscaled<double>(min_spacing), loops.size());
