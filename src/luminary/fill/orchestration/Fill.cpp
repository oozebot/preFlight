...
// debug_out_path, for the PREFLIGHT_DEBUG_SLICE_PROCESSING block.
...
#include "luminary/fill/rectilinear/FillRectilinear.hpp"
#include "luminary/fill/patterns/lightning/FillLightning.hpp"
#include "luminary/fill/ensuring/FillEnsuring.hpp"
...
// Debug output (Luminary::dbg_log) is in DebugOutput.hpp, via FillBase.hpp
...
static void dbg_fill_input(const Layer &layer)
{
    ...
    dbg_log(Luminary::DBG_FILL, z, "FILL", "========== INPUT SURFACES (layer %d, height=%.3f) ==========", lid,
            layer.height);
    for (size_t region_id = 0; region_id < layer.regions().size(); ++region_id)
    {
        ...
        for (const Surface &surface : layerm.fill_surfaces())
        {
            ...
            dbg_log(Luminary::DBG_FILL, z, "FILL",
        ...
        dbg_log(Luminary::DBG_FILL, z, "FILL", "INPUT r=%zu TOTAL: %d surfaces, %.4fmm2", region_id, idx, region_total);
    }
}
...
static void dbg_fill_phase(const char *phase, const Layer &layer, const std::vector<SurfaceFill> &fills)
{
    ...
    int total_ep = 0;
    double total_area = 0;
    dbg_log(Luminary::DBG_FILL, z, "FILL", "========== %s (layer %d) ==========", phase, lid);
    ...
    dbg_log(Luminary::DBG_FILL, z, "FILL", "%s TOTAL: %d expolygons, %.4fmm2", phase, total_ep, total_area);
}
...
std::vector<SurfaceFill> group_fills(const Layer &layer)
{
        ...
        for (const Surface &surface : layerm.fill_surfaces())
            ...
            else
            {
                ...
                if (surface.is_solid())
                {
                    ...
                    // The type and area tests come first: the erosion offset is the expensive part.
                    if (!is_bridge && surface.is_solid() &&
                        (surface.surface_type == stTop || surface.surface_type == stBottom))
                    {
                        ...
                        {
                            dbg_log(Luminary::DBG_FILL, layer.print_z, "FILL", "SKIP_NARROW type=%-18s area=%8.4fmm2",
                ...
                else
                {
                    ...
                    if (params.spacing < min_round_bead_spacing)
                    {
                        ...
                        dbg_log(Luminary::DBG_FILL, layer.print_z, "FILL",
                    ...
                    // Only anchor_length goes to 0, not anchor_length_max: an anchor_length_max of 0
                    // makes dont_connect() true, which disables zigzag patterns entirely. Perimeter
                    // anchoring goes off while infill-to-infill zigzag connections stay.
...

    {
        ...
        // Track TopSolidInfill polygons separately so we can apply
    ...
    // Compute the total fill boundary from the layer's fill_expolygons: the area
    ...
    // Compute the sparse fill threshold once, for hole removal
    ...
    // Consolidate all stSolidOverBridge SurfaceFill entries into one.
    ...
    // Remove small holes from stInternalSolid ExPolygons.
    ...
    // Remove thin holes from stSolidOverBridge ExPolygons.
    ...
    // Transfer stInternalSolid that physically touches stSolidOverBridge into SOB.
    ...
    // After the stSolidOverBridge modifications (hole removal and thin region merge),
    ...
    // Absorb sparse infill regions that are enclosed by solid or bridge fills
    ...
    for (SurfaceFill &absorber : surface_fills)
    {
        ...
        // Merge nearby ExPolygons into a unified region. Grow/union/shrink
    ...
    // After grow/union/shrink, solid fills may have expanded into adjacent fills.
    ...
    // Remove tiny stSolidOverBridge expolygons that are too small for meaningful
    ...
    // Merge fragmented bridge infill into unified regions per angle.
    ...
    if (sparse_erode_radius > 0)
    {
        ...
        // (from bridge_over_infill) and stBottomBridge (from the bridge direction detector) that overlap
    ...
    // A narrow region with no void beside it gets no widening here.
    if (has_internal_voids)
    {
        // Internal voids are generated only when "infill_every_layers" is active.
        ...
        if (!voids.empty() && !surfaces_polygons.empty())
        {
            ...
            // The voids are not appended to collapsed: the expansion costs time and two expanded void
            // regions that merge would add area that should not be filled.
    ...
    */
    ...
    // Narrow-to-Athena width split. Runs on settled geometry so only genuinely
    ...
    // concentric SurfaceFill. This replaces the old all-or-nothing per-surface pattern flip.
    {
        ...
        for (size_t fi = 0; fi < surface_fills.size(); ++fi)
        {
            ...
            for (const ExPolygon &ep : sf.expolygons)
            {
                ...
                if (Luminary::debug_enabled(Luminary::DBG_FILL))
                {
                    ...
                    auto dump_ring = [&](const char *tag, const Polygon &poly)
                    {
                        ...
                        dbg_log(Luminary::DBG_FILL, layer.print_z, "FILL", "NARROW_PTS %s n=%zu %s", tag,
            ...
            dbg_log(Luminary::DBG_FILL, layer.print_z, "FILL", "NARROW_SPLIT type=%-18s wide_ep=%zu narrow_ep=%zu",
...
void Layer::clear_ironing()
{
    // make_ironing() appends its collections after everything make_fills() stored, so the
    // ironing entities are the tail of each region's fills; truncating the tails keeps every
    // index that the island fill ranges hold for the regular fills.
    std::vector<uint32_t> first_ironing(m_regions.size());
    bool any = false;
    for (size_t region_idx = 0; region_idx < m_regions.size(); ++region_idx)
    {
        ExtrusionEntitiesPtr &entities = m_regions[region_idx]->m_fills.entities;
        size_t keep = entities.size();
        while (keep > 0 && entities[keep - 1]->role() == ExtrusionRole::Ironing)
            --keep;
        first_ironing[region_idx] = uint32_t(keep);
        if (keep < entities.size())
        {
            any = true;
            for (size_t i = keep; i < entities.size(); ++i)
                delete entities[i];
            entities.erase(entities.begin() + keep, entities.end());
        }
    }
    if (!any)
        return;
    this->invalidate_role_index();
    // add_fill_range() merges an ironing range into an adjacent fill range of the same region,
    // so clip every range at the region's first ironing index instead of matching whole ranges.
    for (LayerSlice &lslice : lslices_ex)
        for (LayerIsland &island : lslice.islands)
        {
            LayerExtrusionRanges kept;
            for (const LayerExtrusionRange &range : island.fills)
            {
                const uint32_t end = std::min(*range.end(), first_ironing[range.region()]);
                if (*range.begin() < end)
                    kept.push_back(LayerExtrusionRange{range.region(), {*range.begin(), end}});
            }
            island.fills = std::move(kept);
        }
}
                       ...
                       FillLightning::Generator *lightning_generator)
{
    ...
    {
        extern Luminary::PerfAccumTimer g_mf_group_fills;
        g_mf_group_fills.add(t0, std::chrono::steady_clock::now());
    }
    const Luminary::BoundingBox bbox = this->object()->bounding_box();
    const auto resolution = this->object()->config().gcode_resolution.value;
    ...
    for (LayerSlice &lslice : this->lslices_ex)
    {
        for (LayerIsland &island : lslice.islands)
        {
            ...
            for (SurfaceFill &surface_fill : surface_fills)
            {
                ...
                {
                    extern Luminary::PerfAccumTimer g_mf_intersection;
                ...
                if (Luminary::debug_enabled(Luminary::DBG_FILL))
                {
                    ...
                    dbg_log(Luminary::DBG_FILL, this->print_z, "FILL",
                ...
                if (use_per_expolygon_path || params.use_advanced_perimeters)
                {
                    ...
                    for (ExPolygon &expoly : island_expolygons)
                    {
                        ...
                        {
                            extern Luminary::PerfAccumTimer g_mf_fill_surface;
                ...
                else
                {
                    ...
                    if (!all_polylines.empty())
                    {
                        ...
                        dbg_log(Luminary::DBG_FILL, this->print_z, "FILL", "PRE_CHAIN all_polylines=%zu flow_tags=%zu",
                                all_polylines.size(), flow_tags.size());
                        auto [chained, index_map] = chain_polylines_with_indices(std::move(all_polylines), chain_start);
                        dbg_log(Luminary::DBG_FILL, this->print_z, "FILL", "POST_CHAIN chained=%zu index_map=%zu",
                ...
                if (!eec->empty())
                {
                    dbg_log(Luminary::DBG_FILL, this->print_z, "FILL", "FILL_OK type=%-18s entities=%zu",
                ...
                {
                    dbg_log(Luminary::DBG_FILL, this->print_z, "FILL",
                            "FILL_EMPTY type=%-18s (no extrusions generated)",
                            dbg_stype(surface_fill.surface.surface_type));
                    delete eec;
                }
    ...
    if (debug_enabled(DBG_PERIMETERS))
        for (size_t ri = 0; ri < m_regions.size(); ++ri)
            determ_fp_extrusions(DBG_PERIMETERS, this->print_z, "FILL_OUT_FP", "fills", ri, m_regions[ri]->fills());
...
void Layer::make_ironing()
{
    ...
    for (uint32_t region_id = 0; region_id < uint32_t(this->regions().size()); ++region_id)
        if (LayerRegion *layerm = this->get_region(region_id); !layerm->slices().empty())
        {
            ...
            {
                // just_infill stays false; ironing covers the infill and the perimeters together.
    ...
    for (size_t i = 0; i < by_extruder.size();)
    {
        ...
        {
            // Nothing to do: just_infill is never set.
        }
        else
        {
