...
protected:
    ...
    bool fill_surface_by_lines(const Surface *surface, const FillParams &params, float angleBase, float pattern_shift,
    ...
    bool fill_surface_by_multilines(const Surface *surface, FillParams params,
                                    const std::initializer_list<SweepParams> &sweep_params, Polylines &polylines_out);
...
// Combines aligned (same angle every layer) with monotonic (single direction extrusion)
// Useful for transparent/clear parts where optical clarity is important
...
protected:
    // The grid fill will keep the angle constant between the layers, see the implementation of Luminary::Fill.
