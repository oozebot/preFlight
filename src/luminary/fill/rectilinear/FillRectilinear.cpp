...
// The modulo index helpers, and debug_out_path for the INFILL_DEBUG_OUTPUT and PREFLIGHT_DEBUG
// blocks.
...
#if defined(PREFLIGHT_DEBUG) || defined(INFILL_DEBUG_OUTPUT)
#include "luminary/geometry/diagnostics/SVG.hpp"
#endif
...
struct SegmentIntersection
{
    ...
    coord_t pos() const
    {
        // Guard against division by zero from degenerate or out-of-bounds intersections.
        assert(pos_q > 0);
        if (pos_q == 0)
    ...
    // These accessors test both the link type and its quality. Connectivity and phony validation
    // mark a link Invalid while leaving its type Horizontal, Up or Down, so a type-only test would
    // return the index of a link that must not be followed and let iright=-1 reach path generation.
    // The test is != Invalid rather than == Valid, because TooLong means the link exists but is not
    // to be followed, and region building still needs it, as MonotonicLines does.
...
public:
        ...
        coord_t aoffset2 = 0)
    {
        ...
        // Both offsets may be zero; sample_grid_pattern() for lightning infill calls it that way and gets
        // the source expolygon back as the outer contour.
        // assert(aoffset1 < 0);
        assert(aoffset2 <= 0);
        ...
        for (size_t i = 0; i < n_contours; ++i)
        {
            ...
            polygons_ccw[i] = Luminary::Geometry::is_ccw(contour(i));
        }
    }
    ...
    // Outer and inner name the offset operation, not contour against hole:
    // - outer = boundaries from offset1 (aoffset1), both its contours and its holes
    // - inner = boundaries from offset2 (aoffset2), both its contours and its holes
    // offset() returns flat lists where a hole can sit at any index, so an index comparison against
    // n_contours_outer cannot tell the two offsets apart; the from_inner_offset flag records which
    // offset each polygon came from.
...
#ifdef PREFLIGHT_DEBUG
    void export_to_svg(Luminary::SVG &svg)
    {
    ...
    // offset_ex keeps the contour and hole relationship. A flat Polygons list from offset() mixes
    // the two with nothing to tell them apart, so is_contour_outer() classifies a hole as an outer
    // contour and the inner and outer markers come out inverted, which crashes on bridge holes.
    ...
    const ExPolygonWithOffset &poly_with_offset, size_t n_vlines, coord_t x0, coord_t line_spacing)
{
    ...
    // A region narrower than one spacing has no vertical line to intersect.
    if (n_vlines == 0)
        return segs;
...
#undef INFILL_DEBUG_ASSERT
    ...
    catch (const InfillFailedException & /* ex */)
    {
        ...
        ::Luminary::SVG svg(debug_out_path("slice_region_by_vertical_lines-failed-%d.svg", iRun++), bbox);
    ...
    for (;;)
    {
        ...
        if (try_connect)
        {
            ...
            if (intersection_horizontal_valid)
            {
                ...
                // The connecting segments at the other end of this line stay unconsumed; a perimeter segment
                // is not followed once the vertical segment on its far side is taken.
                                                      ...
                                                      const SegmentIntersection &start)
{
    ...
    // Bounds-check the pointer walk to prevent out-of-bounds reads
...
#ifndef NDEBUG
#define PREFLIGHT_DEBUG_MONOTONOUS_REGIONS
#endif

#ifdef PREFLIGHT_DEBUG_MONOTONOUS_REGIONS
            ...
            if (!start->consumed_vertical_up)
            {
                ...
                while (++i_vline < int(segs.size()))
                {
                    ...
                    if (left != right_left)
                        ...
                        break;
                    // Verify at least one horizontal contour link exists from
                                      ...
                                      std::vector<SegmentedIntersectionLine> &segs)
{
    ...
    for (MonotonicRegion &region : regions)
    {
        if (region.left.vline > 0)
        {
            ...
            if (lbegin != nullptr)
            {
                // Bounds pointers for safe iteration over vline_left intersections.
...
// #define PREFLIGHT_DEBUG_ANTS

template<typename... TArgs>
inline void print_ant(const std::string &fmt, TArgs &&...args)
{
#ifdef PREFLIGHT_DEBUG_ANTS
    std::cout << Luminary::format(fmt, std::forward<TArgs>(args)...) << std::endl;
#endif
}
                                                                ...
                                                                std::mt19937_64 &rng)
{
    ...
    for (int round = 0; round < num_rounds && num_rounds_no_change < num_rounds_no_change_exit; ++round)
    {
        ...
        for (int ant = 0; ant < num_ants; ++ant)
        {
            ...
            // The first region is picked at random rather than by pheromone level; only one step out of the
            // whole path is affected.
            ...
            *(queue.begin() + first_idx) = std::move(queue.back());
            ...
            while (!queue.empty() || !path.back().region->right_neighbors.empty())
            {
                ...
                // If no candidates from either right_neighbors or queue,
            ...
            // Skip incomplete paths where an ant got stuck in a dead-end.
                                 ...
                                 const FillParams &params)
{
    ...
    for (const MonotonicRegionLink &path_segment : path)
    {
        ...
        if (polyline != nullptr && &path_segment != path.data())
        {
            ...
            if (!extended)
            {
                ...
                // Guard against an out-of-bounds pointer move. Degenerate geometry
    ...
    if (polyline != nullptr)
    {
        ...
        assert(ip->is_inner());
        // Same bounds guard as above: degenerate geometry can violate intersection ordering.
                                            ...
                                            float pattern_shift, Polylines &polylines_out)
{
        ...
        surface->surface_type == stInternalSolid)
    {
        ...
        if (stretch > 0)
        {
            ...
            {
                DBG_COUNT("STRETCH_REJECT");
                dbg_log(Luminary::DBG_FILL, this->z, "FILL", "STRETCH_REJECT inner ep=%zu->%zu",
    ...
    if (params.full_infill() && !params.dont_adjust)
    {
        ...
        // n_vlines is unsigned: a region narrower than one spacing has no lines, and
        // (n_vlines - 1) would wrap and put x0 so far from the region that the per-segment
        // line-index loops in slice_region_by_vertical_lines run for billions of iterations.
        coord_t pattern_half_width = (n_vlines == 0) ? 0 : (coord_t(n_vlines) - 1) * line_spacing / 2;
    ...
    ::Luminary::SVG svg(debug_out_path("FillRectilinear-%d.svg", iRun), bbox_svg); // , scale_(1.));
    poly_with_offset.export_to_svg(svg);
    {
        ::Luminary::SVG svg(debug_out_path("FillRectilinear-initial-%d.svg", iRun), bbox_svg); // , scale_(1.));
        poly_with_offset.export_to_svg(svg);
    }
    iRun++;
#endif /* PREFLIGHT_DEBUG */
    ...
    if (Luminary::debug_enabled(Luminary::DBG_FILL))
    {
        ...
        for (const auto &s : segs)
            if (!s.intersections.empty())
                ++n_segs_with_isect;
        dbg_log(Luminary::DBG_FILL, this->z, "FILL",
    ...
    // Clip generated polylines against the fill boundary to prevent lines from
    ...
    if (polylines_out.size() > n_polylines_out_initial && !poly_with_offset.expolygons_outer.empty())
    {
        ...
        if (pre_clip_length - post_clip_length > 0.05)
            DBG_COUNT("CLIP_LOST");
        dbg_log(Luminary::DBG_FILL, this->z, "FILL", "CLIP pre=%zu(%.1fmm) post=%zu(%.1fmm) lost=%.1fmm",
                pre_clip_count, pre_clip_length, post_clip_count, post_clip_length, pre_clip_length - post_clip_length);
    }

#ifdef PREFLIGHT_DEBUG
    {
        {
            ::Luminary::SVG svg(debug_out_path("FillRectilinear-final-%03d.svg", iRun), bbox_svg); // , scale_(1.));
            poly_with_offset.export_to_svg(svg);
            for (size_t i = n_polylines_out_initial; i < polylines_out.size(); ++i)
        ...
        {
            ::Luminary::SVG svg(debug_out_path("FillRectilinear-final-%03d-%03d.svg", iRun, i_polyline),
                                bbox_svg); // , scale_(1.));
                     ...
                     coord_t line_spacing, coord_t pattern_shift, Polylines &fill_lines)
{
    ...
    refpt.x() -= (pattern_shift_scaled >= 0) ? pattern_shift_scaled : (line_spacing + pattern_shift_scaled);
    bounding_box.merge(Luminary::align_to_grid(bounding_box.min, Point(line_spacing, line_spacing), refpt));
                                                 ...
                                                 Polylines &polylines_out)
{
    ...
    if (params.dont_connect() || fill_lines.size() <= 1)
    {
        ...
        dbg_log(Luminary::DBG_FILL, this->z, "FILL", "MULTILINE_RESULT raw=%zu out=%zu (dont_connect)", total_raw,
    ...
    else
    {
        ...
        dbg_log(Luminary::DBG_FILL, this->z, "FILL", "MULTILINE_RESULT raw=%zu connected=%zu", total_raw,
                polylines_out.size() - before_out);
    }
