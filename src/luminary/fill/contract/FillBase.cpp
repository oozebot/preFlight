...
// The modulo index helpers, and debug_out_path for the INFILL_DEBUG_OUTPUT blocks.
#include "luminary/core/containers/Indices.hpp"
#include "luminary/platform/paths/Paths.hpp"
#include "FillBase.hpp"
#include "luminary/fill/concentric/FillConcentric.hpp"
#include "luminary/fill/patterns/procedural/FillHoneycomb.hpp"
#include "luminary/fill/patterns/procedural/Fill3DHoneycomb.hpp"
#include "luminary/fill/patterns/procedural/FillGyroid.hpp"
#include "luminary/fill/planepath/FillPlanePath.hpp"
#include "luminary/fill/line/FillLine.hpp"
...
Fill *Fill::new_from_type(const InfillPattern type)
{
    ...
    default:
        throw Luminary::InvalidArgument("unknown type");
    }
}
...
Polylines Fill::fill_surface(const Surface *surface, const FillParams &params)
{
    ...
    Luminary::ExPolygons expp = offset_ex(surface->expolygon, offset);
...
coord_t Fill::_adjust_solid_spacing(const coord_t width, const coord_t distance)
{
    ...
    // The factor_max check below still has to run, so number_of_intervals==0 does not return early
    if (distance <= 0)
    {
        return 1;
    }
...
std::pair<float, Point> Fill::_infill_direction(const Surface *surface) const
{
    ...
    // Bounding box is the bounding box of a perl object Luminary::Print::Object (c++ object Luminary::PrintObject)
                 ...
                 ++it_contour_and_segment)
            {
                                                         ...
                                                         this->radius, interval))
                {
                    ...
                    else
                    {
                        ...
                        auto it_low = Luminary::lower_bound_by_predicate(intersections.begin(), intersections.end(),
                                                                         [param_overlap1](
                                                                             const ContourIntersectionPoint *l)
                                                                         { return l->param < param_overlap1; });
                        auto it_high = Luminary::lower_bound_by_predicate(intersections.begin(), intersections.end(),
                                                                          [param_overlap2](
                                                                              const ContourIntersectionPoint *l)
