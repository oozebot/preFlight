...
// debug_out_path, for the ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT blocks.
                                           ...
                                           const coordf_t hook_length, const coordf_t hook_length_max)
{
    ...
    std::vector<std::pair<const Polyline *, const Polyline *>> lines_touching_at_endpoints;
    {
        ...
        double r2_close = Luminary::sqr(1200.);
                    ...
                    if (!closest.empty())
                    {
                        ...
                        if (pl.points.empty())
                        {
                            ...
                            // Such an infill line should not make a T-joint anyways.
                                 ...
                                 // The caller flattens each polyline to its two end points, so front and back
                                 // bound the whole line.
                                  ...
                                  Polylines &polylines_out)
{
    ...
    Polylines all_polylines;
    {
        ...
        if (Luminary::debug_enabled(Luminary::DBG_FILL))
            dbg_log(Luminary::DBG_FILL, this->z, "FILL", "ADAPTIVE octree_lines=%zu (dir0=%zu dir1=%zu dir2=%zu)",
                    lines.size(),
        ...
        dbg_log(Luminary::DBG_FILL, this->z, "FILL", "ADAPTIVE crop pre=%zu post=%zu", pre_crop, all_polylines.size());
    }
    ...
    dbg_log(Luminary::DBG_FILL, this->z, "FILL", "ADAPTIVE hooks pre=%zu post=%zu", pre_hooks,
    ...
    if (params.dont_connect() || all_polylines_with_hooks.size() <= 1)
    {
        ...
        dbg_log(Luminary::DBG_FILL, this->z, "FILL", "ADAPTIVE chain pre_len=%.1fmm post=%zu(%.1fmm) lost=%.1fmm",
    ...
    else
    {
        ...
        for (size_t i = pre_connect_out; i < polylines_out.size(); ++i)
            post_len += unscale<double>(polylines_out[i].length());
        dbg_log(Luminary::DBG_FILL, this->z, "FILL", "ADAPTIVE connect pre_len=%.1fmm post=%zu(%.1fmm) lost=%.1fmm",
                pre_connect_len, polylines_out.size() - pre_connect_out, post_len, pre_connect_len - post_len);
    }
                             ...
                             const BoundingBoxf3 &current_bbox, int depth)
{
    ...
    // const double r2_cube = Luminary::sqr(0.5 * this->cubes_properties[depth].height + EPSILON);
