...
#include "luminary/toolpath/ordering/PathSorting.hpp"
                                   ...
                                   bool stop_vibrations, bool fill_gaps, bool connect_extrusions)
{
    ...
    if (fill_gaps)
    {
        ...
        // ::Luminary::SVG svg(debug_out_path(("surface" + std::to_string(surface->area())).c_str()).c_str(), bbox);
        ...
        for (ExPolygon &ex_poly : gaps_for_additional_filling)
        {
            ...
            wall_tool_paths.set_debug_print_z(fill->z);
        ...
        for (size_t ep_idx = 0; ep_idx < connection_endpoints.size(); ep_idx++)
        {
            ...
            if (!ep1.used)
            {
                ...
                for (size_t close_endpoint_idx : close_endpoints)
                {
                    ...
                    if (std::abs(Luminary::angle(v1, v2)) > PI / 6.0)
                    {
                        continue;
                    }
    ...
    if (connect_extrusions)
    {
        ...
        {
            connected_thick_polylines.push_back(std::move(thick_polylines.front()));
            for (size_t tp_idx = 1; tp_idx < thick_polylines.size(); tp_idx++)
            {
                ...
                {
                    connected_thick_polylines.push_back(std::move(tp));
                }
            }
        }
        thick_polylines = std::move(connected_thick_polylines);
    }
