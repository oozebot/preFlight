...
// As written, this struct will only be valid for a single layer, will have to be updated for the next.
// Reasons for implementing this with some separate closures:
...
public:
     ...
     */
    void propagateToNextLayer(std::vector<NodeSPtr> &next_trees, const Polygons &next_outlines,
                              const EdgeGrid::Grid &outline_locator, coord_t prune_distance, coord_t smooth_magnitude,
...
protected:
     ...
     */
    bool realign(const Polygons &outlines, const EdgeGrid::Grid &outline_locator,
                 std::vector<NodeSPtr> &rerooted_parts);

    struct RectilinearJunction
    {
        coord_t
            total_recti_dist; //!< rectilinear distance along the tree from the last junction above to the junction below
     ...
     */
    RectilinearJunction straighten(coord_t magnitude, const Point &junction_above, coord_t accumulated_dist,
...
protected:
    ...
    std::optional<Point>
        m_last_grounding_location; //<! The last known grounding location, see 'getLastGroundingLocation()'.
    ...
    friend void export_to_svg(const NodeSPtr &root_node, Luminary::SVG &svg);
    friend void export_to_svg(const std::string &path, const Polygons &contour,
                              const std::vector<NodeSPtr> &root_nodes);
...
bool lineSegmentPolygonsIntersection(const Point &a, const Point &b, const EdgeGrid::Grid &outline_locator,
                                     Point &result, coord_t within_max_dist);
