...
NodeSPtr Node::addChild(NodeSPtr &new_child)
{
    ...
    //assert(p != new_child->p); // No problem for now. Issue to solve later. Low prio.
...
void Node::propagateToNextLayer(std::vector<NodeSPtr> &next_trees, const Polygons &next_outlines,
                                const EdgeGrid::Grid &outline_locator, const coord_t prune_distance,
                                const coord_t smooth_magnitude, const coord_t max_remove_colinear_dist) const
{
...
// Depth-first, as currently implemented.
...
bool lineSegmentPolygonsIntersection(const Point &a, const Point &b, const EdgeGrid::Grid &outline_locator,
                                     Point &result, const coord_t within_max_dist)
{
    struct Visitor
    {
...
bool Node::realign(const Polygons &outlines, const EdgeGrid::Grid &outline_locator,
                   std::vector<NodeSPtr> &rerooted_parts)
{
    ...
    if (contains(outlines, m_p))
    {
        ...
        m_children.erase(std::remove_if(m_children.begin(), m_children.end(),
                                        ...
                                        {
                                            bool connect_branch = child->realign(outlines, outline_locator,
                                                                                 rerooted_parts);
                                            // Find an intersection of the line segment from p to child->p, at maximum outline_locator.resolution() * 2 distance from p.
                                            if (connect_branch &&
                                                lineSegmentPolygonsIntersection(child->m_p, m_p, outline_locator, coll,
...
Node::RectilinearJunction Node::straighten(const coord_t magnitude, const Point &junction_above,
                                           const coord_t accumulated_dist, const int64_t max_remove_colinear_dist2)
{
    constexpr coord_t junction_magnitude_factor_numerator = 3;
    constexpr coord_t junction_magnitude_factor_denominator = 4;

    const coord_t junction_magnitude = magnitude * junction_magnitude_factor_numerator /
    ...
    if (m_children.size() == 1)
    {
        ...
        RectilinearJunction junction_below = child_p->straighten(magnitude, junction_above,
        ...
        for (auto &child_p : m_children)
        {
            ...
            if (below.total_recti_dist < magnitude)
            {
                prevent_junction_moving =
                    true; // prevent flipflopping in branches due to straightening and junctoin moving clashing
            }
        }
    ...
    for (auto child_it = m_children.begin(); child_it != m_children.end();)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                const Point n =
                    b + (ba.cast<double>().normalized() * (pruning_distance - dist_pruned_child)).cast<coord_t>();
...
void Node::convertToPolylines(size_t long_line_idx, Polylines &output) const
{
    ...
    // The child that continues the long line is scattered by the node's own position rather
    // than a global random generator: layers are converted from several threads at once, and a
    // shared generator made the split depend on their interleaving, so the fill differed run
    // to run.
    const size_t first_child_idx = PointHash{}(m_p) % m_children.size();
