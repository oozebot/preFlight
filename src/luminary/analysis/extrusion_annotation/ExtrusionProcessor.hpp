...
namespace Luminary::ExtrusionProcessor
{
...
struct PropertiesEstimationConfig
{
    ...
    // The curvature windows are only read by the support spots search; the overhang split
    // turns them off.
    bool compute_curvature{true};
};
                                                      ...
                                                      const PropertiesEstimationConfig &config)
{
...

    {
        ...
        auto [distance, nearest_line, x] = unscaled_prev_layer.template distance_from_lines_extra<SIGNED_DISTANCE>(
            start_point.position.cast<AABBScalar>());
    ...
    for (size_t i = 1; i < input_points.size(); i++)
    {
        ...
        auto [distance, nearest_line, x] = unscaled_prev_layer.template distance_from_lines_extra<SIGNED_DISTANCE>(
            next_point.position.cast<AABBScalar>());
                ...
                (next.distance > -boundary_offset && next.distance < boundary_offset + 2.0f))
            {
                ...
                if (line_len > 4.0f)
                {
                    ...
                    if (t0 < 1.0)
                    {
                        ...
                        auto [p0_dist, p0_near_l, p0_x] =
                            unscaled_prev_layer.template distance_from_lines_extra<SIGNED_DISTANCE>(
                                p0.cast<AABBScalar>());
                        ExtendedPoint new_p{};
                        new_p.position = p0;
            ...
            for (size_t i = 0; i + 1 < points.size(); i++)
            {
                ...
                for (size_t j = 1; j < new_point_count + 1; j++)
                {
                    ...
                    auto [p_dist, p_near_l, p_x] =
                        unscaled_prev_layer.template distance_from_lines_extra<SIGNED_DISTANCE>(pos.cast<AABBScalar>());
                    ExtendedPoint new_p{};
                    new_p.position = pos;
        ...
        for (float window_size : {3.0f, 9.0f, 16.0f})
        {
            for (int point_idx = 0; point_idx < int(points.size()); ++point_idx)
            {
                ...
                Vec2d back_position = current.position;
                {
                    ...
                    while (dist_backwards < window_size * 0.5 &&
                ...
                Vec2d front_position = current.position;
                {
                    ...
                    while (dist_forwards < window_size * 0.5 &&
                ...
                float new_curvature = angle(current.position - back_position, front_position - current.position) /
                                      window_size;
...
OverhangSpeeds calculate_overhang_speed(const ExtrusionAttributes &attributes, const FullPrintConfig &config,
                                        size_t extruder_id, float external_perimeter_reference_speed,
                                        float default_speed, const std::optional<float> &current_fan_speed);
