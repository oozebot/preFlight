    ...
    const AABBTreeLines::LinesDistancer<CurledLine> &prev_layer_curled_lines)
{
    ...
    config.compute_curvature = false;
    ...
    for (size_t i = 0; i < extended_points.size(); i++)
    {
        ...
        const double dist_limit = 10.0 * path.width();
        {
            ...
            if (!line_indices.empty())
            {
                ...
                // Curled lines are sampled near the segment midpoint, so a long segment is slowed as a
                // whole or not at all. For segments over 8 mm the projected curled length decides whether
                // the slowdown is worth it.
                if (len > 8)
                {
    ...
    for (size_t i = 1; i < extended_points.size(); i++)
    {
        ...
        // start_distance is seeded from the max-collapsed segment distance, so each sub-path runs at
    ...
    const AABBTreeLines::LinesDistancer<CurledLine> &prev_layer_curled_lines)
{
    ...
    for (const auto *e : ecc->entities)
    {
        ...
        {
            throw Luminary::InvalidArgument("Unknown extrusion entity type");
        }
    }
    return result;
};
