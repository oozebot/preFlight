...
namespace Luminary::SupportSpotsGenerator
{
...
float estimate_curled_up_height(float distance, float curvature, float layer_height, float flow_width,
                                float prev_line_curled_height, const Params &params)
{
    ...
    if (entity->role().is_bridge() && !entity->role().is_perimeter())
    {
        ...
        for (size_t i = 0; i < annotated_points.size(); ++i)
        {
            ...
            else if (curr_point.distance > flow_width)
            {
                ...
                {
                    dbg_log(Luminary::DBG_STABILITY, layer_region->layer()->print_z, "STAB",
                       ...
                       const std::optional<Polygons> &brim)
{
    ...
    // The leaf entities are visited in place, depth first in their stored order (the order a
    // flattened copy would list them), without cloning the layer's paths.
    std::function<void(const ExtrusionEntity *)> visit = [&](const ExtrusionEntity *entity)
    {
        if (const auto *sub = dynamic_cast<const ExtrusionEntityCollection *>(entity); sub != nullptr)
        {
            for (const ExtrusionEntity *e : sub->entities)
                visit(e);
            return;
        }
        {
...
{
    // Brim area is estimated per slice, so brims of neighbouring slices that meet are each counted in full.
    ExPolygons brim;
...
// The object part of one slice (extrusion integrals and, on the first layer, the brim) depends
// on that layer alone; every layer's parts are built here in parallel so the stability walk
// over the layers only merges them.
using PrecomputedObjectParts = std::vector<std::vector<ObjectPart>>;

static ObjectPart make_object_part(const Layer *layer, size_t slice_idx, const Params &params)
{
    const LayerSlice &slice = layer->lslices_ex.at(slice_idx);
    ...
    const std::optional<Polygons> brim{
        has_brim(layer, params)
            ...
            : std::nullopt};
    return ObjectPart{extrusion_collections, connected_to_bed, layer->print_z, layer->height, brim};
}

static PrecomputedObjectParts precompute_object_parts(const PrintObject *po, const Params &params)
{
    PrecomputedObjectParts result(po->layer_count());
    for (size_t lidx = 0; lidx < po->layer_count(); ++lidx)
        if (const Layer *layer = po->get_layer(lidx); layer != nullptr)
            result[lidx].resize(layer->lslices_ex.size());
    tbb::parallel_for(tbb::blocked_range<size_t>(0, po->layer_count()),
                      [po, &params, &result](const tbb::blocked_range<size_t> &r)
                      {
                          for (size_t lidx = r.begin(); lidx < r.end(); ++lidx)
                          {
                              const Layer *layer = po->get_layer(lidx);
                              if (!layer)
                                  continue;
                              for (size_t slice_idx = 0; slice_idx < layer->lslices_ex.size(); ++slice_idx)
                                  result[lidx][slice_idx] = make_object_part(layer, slice_idx, params);
                          }
                      });
    return result;
}
...
SliceMappings update_active_object_parts(const Layer *layer, const Params &params,
                                         ...
                                         const std::vector<ObjectPart> &precomputed_parts,
                                         ...
                                         ActiveObjectParts &active_object_parts, PartialObjects &partial_objects)
{
    ...
    {
        const LayerSlice &slice = layer->lslices_ex.at(slice_idx);
        const ObjectPart &new_part = precomputed_parts[slice_idx];
    ...
    const PrintTryCancel &cancel_func, const Params &params)
{
    ...
    const PrecomputedObjectParts precomputed_parts = precompute_object_parts(po, params);
    ...
    for (size_t layer_idx = 0; layer_idx < po->layer_count(); ++layer_idx)
    {
        ...
        slice_mappings = update_active_object_parts(layer, params, precomputed_slices_connections[layer_idx],
                                                    precomputed_parts[layer_idx], slice_mappings, active_object_parts,
                                                    partial_objects);
...
void estimate_malformations(LayerPtrs &layers, const Params &params)
{
    ...
    for (Layer *l : layers)
    {
        ...
        {
            for (const ExtrusionEntity *extrusion : layer_region->perimeters_unsplit().flatten().entities)
            {
...
std::vector<std::pair<SupportPointCause, bool>> gather_issues(const SupportPoints &support_points,
                                                              PartialObjects &partial_objects)
{
    if (Luminary::debug_enabled(Luminary::DBG_STABILITY))
    {
        for (const SupportPoint &sp : support_points)
            dbg_log(Luminary::DBG_STABILITY, sp.position.z(), "STAB", "POINT cause=%s pos=(%.2f,%.2f) radius=%.2f",
    ...
    if (Luminary::debug_enabled(Luminary::DBG_STABILITY))
    {
        dbg_log(Luminary::DBG_STABILITY, 0.0, "STAB", "ISSUES total_points=%zu issues=%zu", support_points.size(),
                result.size());
        for (const auto &[cause, critical] : result)
            dbg_log(Luminary::DBG_STABILITY, 0.0, "STAB", "ISSUE cause=%s critical=%d", dbg_cause_name(cause),
                    int(critical));
    }
