...
static constexpr auto snapshot_subdirs = {"print",   "sla_print",        "filament", "sla_material",
                                          "printer", "physical_printer", "vendor"};
...
bool Snapshot::equal_to_active(const AppConfig &app_config) const
{
    ...
    boost::filesystem::path snapshot_dir = boost::filesystem::path(Slic3r::data_dir()) / SLIC3R_SNAPSHOTS_DIR /
                                           this->id;
                                         ...
                                         const boost::filesystem::path &path_dst)
{
    if (!boost::filesystem::is_directory(path_dst) && !boost::filesystem::create_directory(path_dst))
        throw Slic3r::RuntimeError(std::string("preFlight was unable to create a directory at ") + path_dst.string());
        ...
        if (Slic3r::is_ini_file(dir_entry))
            if (std::string error_message;
                copy_file(dir_entry.path().string(), (path_dst / dir_entry.path().filename()).string(), error_message,
                          false) != SUCCESS)
                throw Slic3r::RuntimeError(format("Failed copying \"%1%\" to \"%2%\": %3%", path_src.string(),
