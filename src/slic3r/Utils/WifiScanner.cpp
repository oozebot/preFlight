...
DBusMessage *dbus_query(DBusConnection *connection, const char *service, const char *object, const char *interface,
                        const char *method)
{
    DBusError error;
...
void iter_access_points(DBusConnection *connection, DBusMessage *access_points, Slic3r::WifiSsidPskMap &wifi_map)
{
    ...
    while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_OBJECT_PATH)
    {
        ...
        DBusMessage *msg = dbus_message_new_method_call("org.freedesktop.NetworkManager", object_path,
...
void iter_devices(DBusConnection *connection, DBusMessage *devices, Slic3r::WifiSsidPskMap &wifi_map)
{
    ...
    while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_OBJECT_PATH)
    {
        ...
        DBusMessage *reply = dbus_query(connection,
...
WifiScanner::WifiScanner()
{
    ...
    wlanEnumInterfacesFunc = reinterpret_cast<WlanEnumInterfacesFunc>(
        GetProcAddress(m_wlanapi_handle, "WlanEnumInterfaces"));
    wlanQueryInterfaceFunc = reinterpret_cast<WlanQueryInterfaceFunc>(
        GetProcAddress(m_wlanapi_handle, "WlanQueryInterface"));
    ...
    wlanGetProfileListFunc = reinterpret_cast<WlanGetProfileListFunc>(
        GetProcAddress(m_wlanapi_handle, "WlanGetProfileList"));
    wlanGetAvailableNetworkListFunc = reinterpret_cast<WlanGetAvailableNetworkListFunc>(
        GetProcAddress(m_wlanapi_handle, "WlanGetAvailableNetworkList"));
    ...
    for (DWORD i = 0; i < interface_list->dwNumberOfItems; i++)
    {
        if (interface_list->InterfaceInfo[i].isState == wlan_interface_state_connected)
        {
            ...
            if (wlanQueryInterfaceFunc(handle, &interface_list->InterfaceInfo[i].InterfaceGuid,
                ...
                pConnectInfo && pConnectInfo->wlanAssociationAttributes.dot11Ssid.uSSIDLength)
            {
        ...
        if (wlanGetAvailableNetworkListFunc(handle, &interface_info_entry->InterfaceGuid, 0, NULL,
                                            &available_network_list) != ERROR_SUCCESS)
        {
            continue;
        }
        ...
        for (unsigned int j = 0; j < available_network_list->dwNumberOfItems; j++)
        {
            ...
            if (wlanGetProfileListFunc(handle, &interface_list->InterfaceInfo[i].InterfaceGuid, NULL, &profile_list) !=
                ERROR_SUCCESS)
            {
                continue;
            }
            ...
            for (DWORD k = 0; k < profile_list->dwNumberOfItems; k++)
            {
                ...
                if ((ret = wlanGetProfileFunc(handle, &interface_list->InterfaceInfo[i].InterfaceGuid,
                                              profile_list->ProfileInfo[k].strProfileName, NULL, &xmlstr, &flags,
