...
bool SelfTest(wxSecretStore &store, const wxString &service)
{
    ...
    wxString user;
    ok = PrintResult(store.Load(service, user, secret) && user == userTest && secret == secret1);
    ...
    if (PrintResult(store.Save(service, user, secret2)))
    {
        ...
        if (!PrintResult(store.Load(service, user, secret) && secret == secret2))
            ok = false;
    }
    else
        ok = false;
...
bool check_secrets()
{
    ...
    return SelfTest(store, "preflight");
#else
