...
std::string proccess_failed_archives(const std::vector<std::string> &failed_archives, const VendorMap &vendors,
                                     const SharedArchiveRepositoryVector &repos)
{
    ...
    for (const std::string &failed_archive : failed_archives)
    {
        ...
        if (const auto it = std::find_if(repos.begin(), repos.end(), [failed_archive](const auto *rep)
...
bool PresetUpdaterWrapper::wizard_sync(const PresetBundle *preset_bundle, const Semver &old_slic3r_version,
                                       wxWindow *parent, bool full_sync, const wxString &headline)
{
    ...
    {
        const SharedArchiveRepositoryVector &repos = m_preset_archive_database->get_selected_archive_repositories();
        m_preset_updater->config_update(old_slic3r_version, PresetUpdater::UpdateParams::SHOW_TEXT_BOX_YES_NO, repos,
                                        m_ui_status.get());
    }
...
PresetUpdater::UpdateResult PresetUpdaterWrapper::check_updates_on_user_request(const PresetBundle *preset_bundle,
                                                                                const Semver &old_slic3r_version,
                                                                                wxWindow *parent)
{
    assert(!m_modal_thread.joinable());
    cancel_worker_thread();
    ...
    GUI::ProgressUpdaterDialog *dialog = new GUI::ProgressUpdaterDialog(m_ui_status.get(), parent,
                                                                        _L("Checking for Configuration Updates"));
    m_ui_status->set_handler(dialog);
    VendorMap vendors_copy = preset_bundle->vendors;
    ...
    updater_result = m_preset_updater->config_update(old_slic3r_version, PresetUpdater::UpdateParams::SHOW_TEXT_BOX,
                                                     m_preset_archive_database->get_selected_archive_repositories(),
                                                     m_ui_status.get());
...
PresetUpdater::UpdateResult PresetUpdaterWrapper::check_updates_on_startup(const Semver &old_slic3r_version)
{
    ...
    // preset_updater::config_update does show wxDialog
    m_preset_updater->update_index_db();
    return m_preset_updater->config_update(old_slic3r_version, PresetUpdater::UpdateParams::SHOW_NOTIFICATION,
                                           m_preset_archive_database->get_selected_archive_repositories(),
                                           m_ui_status.get());
}
...
bool PresetUpdaterWrapper::install_bundles_rsrc_or_cache_vendor(std::vector<std::string> bundles,
                                                                bool snapshot /* = true*/) const
{
    m_ui_status->reset(PresetUpdaterUIStatus::PresetUpdaterRetryPolicy::PURP_NO_RETRY);
    const SharedArchiveRepositoryVector &repos = m_preset_archive_database->get_selected_archive_repositories();
...
void PresetUpdaterWrapper::sync_preset_updater(wxEvtHandler *end_evt_handler, const PresetBundle *preset_bundle)
{
    ...
    auto worker_body = [this, vendors_copy, end_evt_handler]()
    {
        const SharedArchiveRepositoryVector &repos = m_preset_archive_database->get_selected_archive_repositories();
        m_preset_updater->sync_blocking(vendors_copy, repos, m_ui_status.get());
        if (this->m_ui_status->get_canceled())
        {
            return;
        }
...
namespace GUI
{
ProgressUpdaterDialog::ProgressUpdaterDialog(PresetUpdaterUIStatus *ui_status, wxWindow *parent,
                                             const wxString first_line)
    // TRN: Text of progress dialog.
    : wxGenericProgressDialog(first_line, _L("Initializing"), 100, parent,
