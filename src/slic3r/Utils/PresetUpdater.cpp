...
void copy_file_fix(const fs::path &source, const fs::path &target)
{
    ...
    if (cfr != CopyFileResult::SUCCESS)
    {
        ...
        throw Slic3r::CriticalException(
            GUI::format(_L("Copying of file %1% to %2% failed: %3%"), source, target, error_message));
    }
...
struct Update
{
    ...
    Update(fs::path &&source, fs::path &&target, const Version &version, std::string vendor, std::string changelog_url,
           bool forced = false, std::vector<std::string> new_printers = {})
...
struct Incompat
{
    ...
    Incompat(fs::path &&bundle, const Version &version, std::string vendor)
        : bundle(std::move(bundle)), version(version), vendor(std::move(vendor))
    {
    }
...
struct PresetUpdater::priv
{
    ...
    bool perform_updates(Updates &&updates, const SharedArchiveRepositoryVector &repositories,
                         PresetUpdaterUIStatus *ui_status, bool snapshot = true) const;
    void set_waiting_updates(Updates u);
    // checks existence and downloads resource to cache
    void get_missing_resource(const ArchiveRepository *archive, const std::string &vendor, const std::string &filename,
                              const std::string &repository_id_from_ini, PresetUpdaterUIStatus *ui_status) const;
    // checks existence and downloads resource to vendor or copy from cache to vendor
    void get_or_copy_missing_resource(const ArchiveRepository *archive, const std::string &vendor,
                                      const std::string &filename, const std::string &repository_id_from_ini,
...
void PresetUpdater::priv::clear_cache_vendor() const
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(cache_vendor_path))
    {
        ...
        if (ec)
        {
            BOOST_LOG_TRIVIAL(error) << "Failed to remove " << dir_entry
                                     << " during cache cleanup. Reason: " << ec.what();
            ec.clear();
        }
    }
}
...
void PresetUpdater::priv::get_missing_resource(const ArchiveRepository *archive, const std::string &vendor,
                                               const std::string &filename, const std::string &repository_id_from_ini,
                                               PresetUpdaterUIStatus *ui_status) const
{
    ...
    { // Already in vendor. No need to do anything.
        BOOST_LOG_TRIVIAL(info) << "Resource " << vendor << " / " << filename
                                << " found in vendor folder. No need to download.";
        return;
    }
    if (fs::exists(file_in_rsrc))
    { // In resources dir since installation. No need to do anything.
        BOOST_LOG_TRIVIAL(info) << "Resource " << vendor << " / " << filename
                                << " found in resources folder. No need to download.";
        return;
    }
    if (fs::exists(file_in_cache))
    { // In cache/venodr_name/ dir. No need to do anything.
        BOOST_LOG_TRIVIAL(info) << "Resource " << vendor << " / " << filename
                                << " found in cache folder. No need to download.";
        return;
    }
    BOOST_LOG_TRIVIAL(info) << "Resources check could not find " << vendor << " / " << filename
                            << " bed texture. Downloading.";
...
void PresetUpdater::priv::get_or_copy_missing_resource(const ArchiveRepository *archive, const std::string &vendor,
...
void PresetUpdater::priv::sync_config(const VendorMap &vendors, const ArchiveRepository *archive_repository,
                                      PresetUpdaterUIStatus *ui_status)
{
    ...
    {
        BOOST_LOG_TRIVIAL(error) << "Download of vendor profiles archive zip of "
                                 << archive_repository->get_manifest().id << " repository has failed.";
                ...
                if (stat.m_uncomp_size > 0)
                {
                    ...
                    if (!fs::exists(tmp_path.parent_path()))
                    {
                        BOOST_LOG_TRIVIAL(error)
                            << "Failed to unzip file " << name << ". Directories are not supported. Skipping file.";
                        continue;
                    }
                    ...
                    if (!exists || ec)
                    {
                        BOOST_LOG_TRIVIAL(error) << "Failed to find unzipped file at " << tmp_path
                    ...
                    if (name.substr(name.size() - 3) == "idx")
                        vendors_with_status.emplace_back(
                            name.substr(0, name.size() - 4),
                            VendorStatus::IN_ARCHIVE); // asume for now its only in archive - if not, it will change later.
                }
            }
        }
        close_zip_reader(&archive);
    ...
    for (auto &index : index_db)
    {
        ...
        auto archive_it = std::find_if(vendors_with_status.begin(), vendors_with_status.end(),
                                       [&index](const std::pair<std::string, VendorStatus> &element)
                                       { return element.first == index.vendor(); });
        ...
        // Load the fresh index up
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Could not load downloaded index %1% for vendor %2%: invalid index?",
                                                   idx_path_temp, vendor.name);
                continue;
            }
            if (new_index.version() < index.version())
            {
                BOOST_LOG_TRIVIAL(info) << format(
                    "The downloaded index %1% for vendor %2% is older than the active one. Ignoring the downloaded index.",
                    idx_path_temp, vendor.name);
                continue;
            }
        ...
        BOOST_LOG_TRIVIAL(debug) << format("Got index for vendor: %1%: current version: %2%, recommended version: %3%",
                                           vendor.name, vendor.config_version.to_string(), recommended.to_string());

        if (vendor.config_version >= recommended)
        {
            continue;
        }
        ...
        {
            BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1% at %2%, message: %3%", vendor.id,
                                               bundle_path, e.what());
            continue;
        }
        ...
        for (const auto &model : vp.models)
        {
            for (const std::string &res :
                 {model.bed_texture, model.bed_model, model.thumbnail /*id +"_thumbnail.png"*/})
            {
                if (!res.empty())
                {
                    try
                    ...
                    {
                        BOOST_LOG_TRIVIAL(error)
                            << "Failed to get " << res << " for " << vp.id << " " << model.id << ": " << e.what();
                    }
                }
                if (ui_status->get_canceled())
                    return;
    ...
    for (const std::pair<std::string, VendorStatus> &vendor : vendors_with_status)
    {
        if (vendor.second == VendorStatus::IN_ARCHIVE)
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Could not load downloaded index %1% for vendor %2%: invalid index?",
                                                   idx_path_in_archive, vendor.first);
                continue;
            }
            const auto recommended_it = index.recommended();
            if (recommended_it == index.end())
            {
                BOOST_LOG_TRIVIAL(error) << format("No recommended version for vendor: %1%, invalid index? (%2%)",
                                                   vendor.first, idx_path_in_archive);
                continue;
            }
            const auto recommended = recommended_it->config_version;
            ...
            else
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1% at %2%, message: %3%",
                                                       vendor.first, ini_path_in_archive, e.what());
                    // Delete the file
                    boost::system::error_code ec;
            ...
            for (const auto &model : vp.models)
            {
                if (!model.thumbnail.empty())
                {
                    ...
                    {
                        BOOST_LOG_TRIVIAL(error) << "Failed to get " << model.thumbnail << " for " << vp.id << " "
                                                 << model.id << ": " << e.what();
                    }
                }
                if (ui_status->get_canceled())
        ...
        else if (vendor.second == VendorStatus::IN_CACHE)
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Could not load downloaded index %1% for vendor %2%: invalid index?",
                                                   idx_path_in_cache, vendor.first);
                continue;
            }
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("No recommended version for vendor: %1%, invalid index? (%2%)",
                                                   vendor.first, idx_path_in_cache);
                continue;
            }
            ...
            if (!fs::exists(ini_path_in_archive) || fs::is_empty(ini_path_in_archive))
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1% at %2%, message: %3%",
                                                       vendor.first, ini_path_in_rsrc, e.what());
                ...
                {
                    BOOST_LOG_TRIVIAL(error)
                        << format("Failed to get new vendor .ini file when checking missing resources: %1%",
                                  ini_path_in_archive.string());
                    continue;
                }
            }
            else
            {
                ...
                {
                    const std::string source_subpath = GUI::format("%1%/%2%.ini", vp.id,
                                                                   recommended_archive.to_string());
                    if (!archive_repository->get_file(source_subpath, ini_path_in_archive, vp.repo_id, ui_status))
                    {
                        BOOST_LOG_TRIVIAL(error)
                            << format("Failed to open vendor .ini file when checking missing resources: %1%",
                                      ini_path_in_archive);
                        continue;
                    }
                }
        ...
        else if (vendor.second == VendorStatus::INSTALLED || vendor.second == VendorStatus::NEW_VERSION)
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1% at %2%, message: %3%",
                                                   vendor.first, path_in_vendor, e.what());
                continue;
            }
            for (const auto &model : vp.models)
            {
...
// Version of slic3r that was running the last time and which was read out from preFlight.ini is provided
// as a parameter.
Updates PresetUpdater::priv::get_config_updates(const Semver &old_slic3r_version) const
{
    ...
    for (const Index &idx : index_db)
    {
        ...
        {
            BOOST_LOG_TRIVIAL(error) << format("No recommended version for vendor: %1%, invalid index? Giving up.",
                                               idx.vendor());
        ...
        BOOST_LOG_TRIVIAL(debug) << format("Vendor: %1%, version installed: %2%%3%, version cached: %4%", vp.name,
        ...
        if (!ver_current_found)
        {
            ...
            auto message = format("Preset bundle `%1%` version not found in index: %2%", idx.vendor(),
                                  vp.config_version.to_string());
        ...
        bool current_not_supported =
            false; //if slcr is incompatible but situation is not downgrade, we do forced updated and this bool is information to do it
        ...
        if (ver_current_found && !ver_current->is_current_slic3r_supported())
        {
            if (ver_current->is_current_slic3r_downgrade())
            {
                ...
                BOOST_LOG_TRIVIAL(warning)
                    << "Current Slic3r incompatible with installed bundle: " << bundle_path.string();
        ...
        {
            BOOST_LOG_TRIVIAL(warning) << format(
                "Recommended config version for the currently running preFlight is older than the currently installed config for vendor %1%. This should not happen.",
                idx.vendor());
            continue;
        }
        ...
        // Config bundle update situation. The recommended config bundle version for this preFlight version from the index from the cache is newer
        ...
        // 1) The last config index downloaded by any previously running preFlight instance
        // 2) The last config index installed by any previously running preFlight instance (older or newer) from its resources.
        // 3) The last config index installed by the currently running preFlight instance from its resources.
        ...
        if (fs::exists(path_in_cache))
        {
            try
            {
                ...
                if (new_vp.config_version == recommended->config_version)
                {
                    ...
                    new_update = Update(std::move(path_in_cache), std::move(bundle_path), *recommended, vp.name,
                                        vp.changelog_url, current_not_supported, std::move(new_printers));
            ...
            {
                BOOST_LOG_TRIVIAL(info) << format("Failed to load the config bundle `%1%`: %2%", path_in_cache.string(),
                                                  ex.what());
            }
        }
        ...
        if (!found && fs::exists(path_in_rsrc) && fs::exists(path_idx_in_rsrc))
        {
            ...
            {
                BOOST_LOG_TRIVIAL(info) << format("Cannot load the config bundle at `%1%`: %2%", path_in_rsrc.string(),
                                                  ex.what());
            }
            if (rsrc_vp.valid())
            {
                ...
                catch (const std::exception &ex)
                {
                    BOOST_LOG_TRIVIAL(info)
                        << format("Cannot load the config index at `%1%`: %2%", path_idx_in_rsrc.string(), ex.what());
                }
                recommended = rsrc_idx.recommended();
                if (recommended != rsrc_idx.end() && recommended->config_version == rsrc_vp.config_version &&
                    recommended->config_version > vp.config_version)
                {
                    new_update = Update(std::move(path_in_rsrc), std::move(bundle_path), *recommended, vp.name,
                                        vp.changelog_url, current_not_supported);
                ...
                {
                    BOOST_LOG_TRIVIAL(warning) << format(
                        "The recommended config version for vendor `%1%` in resources does not match the recommended\n"
                        " config version for this version of preFlight. Corrupted installation?",
                        idx.vendor());
                }
            }
        }
        ...
        if (found)
        {
            ...
            if (fs::exists(bundle_path_idx))
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(error)
                        << format("Cannot load the installed index at `%1%`: %2%", bundle_path_idx, err.what());
                }
            }
#if 0
        ...
        {
            BOOST_LOG_TRIVIAL(warning) << format(
                "Index for vendor %1% indicates update (%2%) but the new bundle was found neither in cache nor resources",
                idx.vendor(), recommended->config_version.to_string());
        }
    }
...
bool PresetUpdater::priv::perform_updates(Updates &&updates, const SharedArchiveRepositoryVector &repositories,
                                          PresetUpdaterUIStatus *ui_status, bool snapshot) const
{
    if (updates.incompats.size() > 0)
    {
        if (snapshot)
        {
            ...
            if (!GUI::Config::take_config_snapshot_cancel_on_error(*GUI::wxGetApp().app_config,
                                                                   Snapshot::SNAPSHOT_DOWNGRADE, "",
    ...
    else if (updates.updates.size() > 0)
    {
        ...
        wxProgressDialog progress_dialog(_L("Installing profiles"), _L("Installing profiles"), 100, nullptr,
                                         wxPD_AUTO_HIDE);
        ...
        for (const auto &update : updates.updates)
        {
            ...
            BOOST_LOG_TRIVIAL(info) << format("Deleting %1% obsolete presets",
                                              bundle.obsolete_presets.prints.size() +
                                                  bundle.obsolete_presets.filaments.size() +
                                                  bundle.obsolete_presets.printers.size());
            ...
            for (const auto &name : bundle.obsolete_presets.prints)
            {
                obsolete_remover("print", name);
            }
            for (const auto &name : bundle.obsolete_presets.filaments)
            {
                obsolete_remover("filament", name);
            }
            for (const auto &name : bundle.obsolete_presets.sla_prints)
            {
                obsolete_remover("sla_print", name);
            }
            for (const auto &name : bundle.obsolete_presets.sla_materials /*filaments*/)
            {
                obsolete_remover("sla_material", name);
            }
            for (const auto &name : bundle.obsolete_presets.printers)
            {
                obsolete_remover("printer", name);
            }
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1%, message: %2%", update.target,
                                                   e.what());
                continue;
            }
                ...
                for (const std::string &resource : {model.bed_texture, model.bed_model, model.thumbnail})
                {
                    ...
                    try
                    {
                        auto it = std::find_if(repositories.begin(), repositories.end(),
                                               [&vp](const auto *i) { return vp.repo_id == i->get_manifest().id; });
                        if (it != repositories.end())
                            get_or_copy_missing_resource((*it), vp.id, resource, vp.repo_id, ui_status);
                        else
                        {
                            BOOST_LOG_TRIVIAL(error)
                                << "Failed to prepare " << resource << " for " << vp.id << " " << model.id
                                << ": Missing record for source with repo_id " << vp.repo_id;
                        }
                    }
                    catch (const std::exception &e)
                    {
                        BOOST_LOG_TRIVIAL(error) << "Failed to prepare " << resource << " for " << vp.id << " "
                                                 << model.id << ": " << e.what();
                    }
                }
            }
...
void PresetUpdater::sync_blocking(const VendorMap &vendors, const SharedArchiveRepositoryVector &repositories,
                                  PresetUpdaterUIStatus *ui_status)
{
...
PresetUpdater::UpdateResult PresetUpdater::config_update(const Semver &old_slic3r_version, UpdateParams params,
                                                         ...
                                                         PresetUpdaterUIStatus *ui_status) const
{
    ...
    else if (updates.updates.size() > 0)
    {
        ...
        if (incompatible_version)
        {
            ...
            for (const auto &update : updates.updates)
            {
                std::string changelog_url = update.version.config_version.prerelease() == nullptr ? update.changelog_url
                                                                                                  : std::string();
                std::string printers;
            ...
            if (res == wxID_OK)
            {
                ...
                if (!p->perform_updates(std::move(updates), repositories, ui_status) || !reload_configs_update_gui())
                    return R_INCOMPAT_EXIT;
        ...
        if (params == UpdateParams::SHOW_NOTIFICATION)
        {
            ...
            GUI::wxGetApp().plater()->get_notification_manager()->push_notification(
                new_printer ? GUI::NotificationType::PresetUpdateAvailableNewPrinter
                            : GUI::NotificationType::PresetUpdateAvailable);
        }
        else
        {
            BOOST_LOG_TRIVIAL(info) << format("Update of %1% bundles available. Asking for confirmation ...",
...
bool PresetUpdater::install_bundles_rsrc_or_cache_vendor(std::vector<std::string> bundles,
                                                         const SharedArchiveRepositoryVector &repositories,
                                                         PresetUpdaterUIStatus *ui_status, bool snapshot) const
{
    Updates updates;
    ...
    for (const auto &bundle : bundles)
    {
        ...
        {
            BOOST_LOG_TRIVIAL(error) << GUI::format("Couldn't locate idx file %1% when performing updates.",
                                                    idx_path.string());
        ...
        if (!boost::filesystem::exists(idx_path))
        {
            std::string msg = GUI::format(
                _L("Couldn't locate index file for vendor %1% when performing updates. The profile will not be installed."),
                bundle);
        ...
        {
            std::string msg = GUI::format(
                _L("Couldn't load index file for vendor %1% when performing updates. The profile will not be installed. Reason: Corrupted index file %2%."),
                bundle, idx_path.string());
            BOOST_LOG_TRIVIAL(error) << msg;
            GUI::show_error(nullptr, msg);
            continue;
        }
        ...
        if (is_in_cache_vendor)
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1%, message: %2%",
                                                   path_in_cache_vendor, e.what());
            ...
            {
                BOOST_LOG_TRIVIAL(error) << format("Corrupted profile file for vendor %1%, message: %2%", path_in_rsrc,
                                                   e.what());
                //continue;
            ...
            {
                std::string msg = GUI::format(
                    _L("Couldn't open profile file for vendor %1% when performing updates. The profile will not be installed. This installation might be corrupted."),
                    bundle);
                BOOST_LOG_TRIVIAL(error) << msg;
                GUI::show_error(nullptr, msg);
                continue;
                                                   ...
                                                   PresetUpdaterUIStatus *ui_status)
{
    ...
    {
        BOOST_LOG_TRIVIAL(debug) << "User agreed to perform the update";
        if (p->perform_updates(std::move(p->waiting_updates), repositories, ui_status) && reload_configs_update_gui())
        {
