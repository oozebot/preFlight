...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type,
                                            const std::vector<wxString> paths_to_open, bool single_instance,
                                            bool delete_after_load)
{
    ...
    path += "\\";
    path += (instance_type == NewSlicerInstanceType::Slicer) ? "preFlight.exe" : "preFlight-gcodeviewer.exe";
    ...
    // Don't call with wxEXEC_HIDE_CONSOLE, preFlight in GUI mode would just show the splash screen. It would not open the main window though, it would
    ...
    {
        // Maybe one day we will be able to run preFlightGCodeViewer, but for now the Apple notarization
        // process refuses Apps with multiple binaries and Vojtech does not know any workaround.
        // ((instance_type == NewSlicerInstanceType::Slicer) ? "preFlight" : "preFlightGCodeViewer");
        // Just run preFlight and give it a --gcodeviewer parameter.
        bin_path = bin_path.parent_path() / "preFlight";
        ...
        try
        {
            ...
            bp2::process proc(ctx, bin_path, args);
            proc.detach();
        }
...
#else // Linux or Unix
    {
        ...
        if (args.empty())
        {
            ...
            my_path = (bin_path.parent_path() /
                       ((instance_type == NewSlicerInstanceType::Slicer) ? "preFlight" : "preFlight-gcodeviewer"))
                          .string();
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const wxString *path_to_open,
                                            bool single_instance, bool delete_after_load)
{
