...
struct DnsHeader
{
    ...
    uint32_t rrcount() const { return ancount + nscount + arcount; }
};
...
private:
                  ...
                  const Bonjour::TxtKeys &txt_keys)
    {
        ...
        case DnsRR_SRV::TAG:
        {
            ...
            if (srv)
            {
                this->sdmap.insert_srv(std::move(rr.name), std::move(*srv));
            }
            break;
        }
        case DnsRR_TXT::TAG:
        {
            ...
            if (txt)
            {
                this->sdmap.insert_txt(std::move(rr.name), std::move(*txt));
            }
            break;
        }
        }
...
{
    os << boost::format("DnsMessage(ID: %1%, Q: %2%, A: %3%, AAAA: %4%, services: [") % msg.header.id %
              (msg.question ? msg.question->name.c_str() : "none") % (msg.rr_a ? msg.rr_a->ip.to_string() : "none") %
...
UdpSocket::UdpSocket(Bonjour::ReplyFn replyfn, const asio::ip::address &multicast_address,
                     const asio::ip::address &interface_address, std::shared_ptr<boost::asio::io_context> io_service)
    : replyfn(replyfn), multicast_address(multicast_address), socket(*io_service), io_service(io_service)
{
    try
    {
        // open socket
        boost::asio::ip::udp::endpoint listen_endpoint(multicast_address.is_v4() ? udp::v4() : udp::v6(),
                                                       BonjourRequest::MCAST_PORT);
        ...
        if (interface_address.is_v4())
        {
            ...
            socket.set_option(
                boost::asio::ip::multicast::join_group(multicast_address.to_v4(), interface_address.to_v4()));
            // send to interface
        ...
        {
            // listen for multicast on given interface
            socket.set_option(boost::asio::ip::multicast::join_group(multicast_address.to_v6(),
                                                                     interface_address.to_v6().scope_id()));
            // send to interface
...
void UdpSocket::async_receive()
{
    try
    {
        ...
        socket.async_receive_from(asio::buffer(session->buffer, session->buffer.size()), session->remote_endpoint,
                                  boost::bind(&UdpSocket::receive_handler, this, session, asio::placeholders::error,
...
void UdpSocket::receive_handler(SharedSession session, const boost::system::error_code &error, size_t bytes)
{
    ...
    // from boost documentation asio::post:
...
void Bonjour::priv::lookup_perform()
{
    ...
    if (!ec)
    {
        ...
        for (const auto &intrfc : interfaces)
            sockets.emplace_back(new LookupSocket(txt_keys, service, service_dn, protocol, replyfn,
                                                  BonjourRequest::MCAST_IP4, intrfc, io_service));
    }
    else
    {
    ...
    if (sockets.empty())
        sockets.emplace_back(
            new LookupSocket(txt_keys, service, service_dn, protocol, replyfn, BonjourRequest::MCAST_IP4, io_service));
    // ipv6 interfaces
    interfaces.clear();
    ...
    if (!ec)
    {
        ...
        for (const auto &intrfc : interfaces)
            sockets.emplace_back(new LookupSocket(txt_keys, service, service_dn, protocol, replyfn,
                                                  BonjourRequest::MCAST_IP6, intrfc, io_service));
        if (interfaces.empty())
            sockets.emplace_back(new LookupSocket(txt_keys, service, service_dn, protocol, replyfn,
                                                  BonjourRequest::MCAST_IP6, io_service));
    }
    else
    {
...
void Bonjour::priv::resolve_perform()
{
    ...
    if (!ec)
    {
        ...
        for (const auto &intrfc : interfaces)
            sockets.emplace_back(
                new ResolveSocket(hostname, reply_callback, BonjourRequest::MCAST_IP4, intrfc, io_service));
    }
    else
    {
        BOOST_LOG_TRIVIAL(info) << "Failed to resolve ipv4 interfaces: " << ec.message();
    ...
    if (!ec)
    {
        ...
        for (const auto &intrfc : interfaces)
            sockets.emplace_back(
                new ResolveSocket(hostname, reply_callback, BonjourRequest::MCAST_IP6, intrfc, io_service));
        if (interfaces.empty())
    ...
    , txt_data(std::move(txt_data))
{
    ...
    if (port != 443 && port != 80)
    {
        port_suffix = std::to_string(port).insert(0, 1, ':');
    }
...
{
    os << boost::format("BonjourReply(%1%, %2%, %3%, %4%") % reply.ip.to_string() % reply.service_name %
              reply.hostname % reply.full_address;
...
Bonjour::Ptr Bonjour::lookup()
{
    ...
    if (self->p)
    {
        auto io_thread = std::thread([self]() { self->p->lookup_perform(); });
...
Bonjour::Ptr Bonjour::resolve()
{
    ...
    if (self->p)
    {
        auto io_thread = std::thread([self]() { self->p->resolve_perform(); });
        self->p->io_thread = std::move(io_thread);
    }
