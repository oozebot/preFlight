...
TopBarItemsCtrl::ButtonWithPopup::ButtonWithPopup(wxWindow *parent, const std::string &icon_name,
                                                  int icon_width /* = 20*/, int icon_height /* = 20*/)
...
{
    // Function body removed - login functionality disabled
}
...
void TopBarItemsCtrl::CreateSearch()
{
    ...
    m_search = new ::TextInput(this, wxGetApp().searcher().default_string, "", "search", wxDefaultPosition,
                               wxSize(20 * em_unit(this), -1), wxTE_PROCESS_ENTER);
    m_search->SetMaxSize(wxSize(30 * em_unit(this), -1));
    ...
    m_search->SetOnDropDownIcon([this]() { TriggerSearch(); });
    ...
    ctrl->Bind(wxEVT_KEY_DOWN,
               [this](wxKeyEvent &e)
               {
                   ...
                   if (e.GetKeyCode() == WXK_TAB)
                       m_search->Navigate(e.ShiftDown() ? wxNavigationKeyEvent::IsBackward
    ...
    ctrl->Bind(wxEVT_LEFT_DOWN,
               [this](wxMouseEvent &event)
               {
                   if (!wxGetApp().searcher().get_dialog() || !wxGetApp().searcher().get_dialog()->IsShown())
                   {
                       TriggerSearch();
                   }
                   event.Skip();
...
*/
    ...
    right_sizer->Add(search_sizer, 1, wxEXPAND | wxALIGN_CENTER_VERTICAL | wxRIGHT, m_btn_margin);

    m_sizer->Add(right_sizer, 1, wxEXPAND | wxALIGN_CENTER_VERTICAL);
...
bool TopBarItemsCtrl::InsertPage(size_t n, const wxString &text, bool bSelect /* = false*/,
