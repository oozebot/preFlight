...
public:
    ...
    {
        using Validator =
            std::function<std::pair<BulkExportDialog::ItemStatus, wxString>(boost::filesystem::path, std::string)>;
        Item(wxWindow *parent, wxFlexGridSizer *sizer, const std::optional<const boost::filesystem::path> &path,
             const int bed_index, Validator validator);
