...
void Downloader::start_download(const std::string &full_url)
{
    ...
    if (boost::starts_with(escaped_url, "preflight://open?file="))
    {
        escaped_url = escaped_url.substr(22);
    }
    else if (boost::starts_with(escaped_url, "preflight://open/?file="))
    {
        escaped_url = escaped_url.substr(23);
    }
    else
    {
    ...
    if (!boost::starts_with(escaped_url, "https://") ||
        !is_any_subdomain(escaped_url, {"printables.com", "thingiverse.com", "cults3d.com"}))
    {
        std::string msg = format(_L("Download won't start. Download URL doesn't point to allowed subdomains : %1%"),
                                 escaped_url);
    ...
    ntf_mngr->push_download_URL_progress_notification(id, m_downloads.back()->get_filename(),
                                                      std::bind(&Downloader::user_action_callback, this,
                                                                std::placeholders::_1, std::placeholders::_2));
    m_downloads.back()->start();
    BOOST_LOG_TRIVIAL(debug) << "started download";
}

void Downloader::start_download_printables(const std::string &url, bool load_after, const std::string &printables_url,
                                           GUI_App *app)
{
    assert(m_initialized);
    ...
    {
        std::string msg = format(_L("Download won't start. Download URL doesn't point to https://printables.com : %1%"),
                                 url);
        BOOST_LOG_TRIVIAL(error) << msg;
        NotificationManager *ntf_mngr = wxGetApp().notification_manager();
        ntf_mngr->push_notification(NotificationType::CustomNotification,
    ...
    // );
    // Use regular notification without Printables link
    ntf_mngr->push_download_URL_progress_notification(id, m_downloads.back()->get_filename(),
                                                      std::bind(&Downloader::user_action_callback, this,
                                                                std::placeholders::_1, std::placeholders::_2));
    m_downloads.back()->start();
