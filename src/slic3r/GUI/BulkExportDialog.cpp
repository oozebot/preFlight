...
void BulkExportDialog::Item::init_input_name_ctrl(wxFlexGridSizer *row_sizer, const std::string &path)
{
    ...
    const long style = 0L;
#endif
    m_text_ctrl = new wxTextCtrl(m_parent, wxID_ANY, from_u8(path), wxDefaultPosition,
                                 wxSize(45 * wxGetApp().em_unit(), -1), style);
...
void BulkExportDialog::Item::init_selection_ctrl(wxFlexGridSizer *row_sizer, int bed_index)
{
    ...
    m_checkbox->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent &event) { this->selected = event.IsChecked(); });

    row_sizer->Add(m_checkbox, 0, wxALIGN_CENTER_VERTICAL);
    m_checkbox->SetValue(this->selected);
}

BulkExportDialog::Item::Item(wxWindow *parent, wxFlexGridSizer *sizer,
                             const std::optional<const boost::filesystem::path> &path_opt, const int bed_index,
                             Validator validator)
    : bed_index(bed_index)
    , m_parent(parent)
...
struct PathValidator
{
    ...
    {
        const int64_t count{std::count_if(items.get().begin(), items.get().end(),
                                          [&](const auto &item) { return item->path == path; })};

        return count >= 2;
    }

    std::pair<BulkExportDialog::ItemStatus, wxString> operator()(const fs::path &path, const std::string &filename)
    {
        for (size_t i = 0; i < std::strlen(unusable_symbols.c_str()); i++)
        {
            if (filename.find_first_of(unusable_symbols[i]) != std::string::npos)
            {
                return {ItemStatus::NoValid,
        ...
        if (filename.empty())
        {
            return {ItemStatus::NoValid, _L("The name cannot be empty.")};
        }

        if (path.string().length() >= max_path_length)
        {
            return {ItemStatus::NoValid, _L("The name is too long.")};
        }

        if (filename.find_first_of(' ') == 0)
        {
            return {ItemStatus::NoValid, _L("The name cannot start with space character.")};
        }

        if (filename.find_last_of(' ') == filename.length() - 1)
        {
            return {ItemStatus::NoValid, _L("The name cannot end with space character.")};
        }

        if (is_duplicate(path))
        {
            return {ItemStatus::NoValid, _L("This name is already used, use another.")};
        }

        if (fs::exists(path))
        {
            return {ItemStatus::Warning, _L("The file already exists!")};
        }
...
std::string get_bmp_name(const BulkExportDialog::ItemStatus status)
{
    ...
    case ItemStatus::Warning:
        return "exclamation_manifold";
    case ItemStatus::NoValid:
        return "exclamation";
...
BulkExportDialog::BulkExportDialog(const std::vector<std::pair<int, std::optional<fs::path>>> &paths,
                                   const wxString &title, const std::string &unusable_symbols)
    : DPIDialog(nullptr, wxID_ANY, title, wxDefaultPosition,
...
{
    m_items.push_back(
        std::make_unique<Item>(this, m_sizer, path, bed_index, PathValidator{m_items, m_unusable_symbols}));
}
...
void BulkExportDialog::accept()
{
    if (has_warnings())
    {
        MessageDialog dialog(nullptr, _L("Some of the selected files already exist. Do you want to replace them?"),
