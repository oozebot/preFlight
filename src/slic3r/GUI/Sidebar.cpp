...
// OrangeSlider class removed - now using custom Slider widget
...
ObjectInfo::ObjectInfo(wxWindow *parent) : wxStaticBoxSizer(new wxStaticBox(parent, wxID_ANY, _L("Info")), wxVERTICAL)
{
    ...
    auto init_info_label =
        [parent, grid_sizer](wxStaticText **info_label, wxString text_label, wxSizer *sizer_with_icon = nullptr)
    {
...
void Sidebar::show_preset_comboboxes()
{
    ...
    // Indices 0-3 are FFF mode items
    // Indices 4-7 are SLA mode items
...
Sidebar::Sidebar(Plater *parent)
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(43 * wxGetApp().em_unit(), -1)), m_plater(parent)
{
    m_scrolled_panel = new wxScrolledWindow(this);
    m_scrolled_panel->SetScrollRate(0, 5);

    m_scrolled_panel->ShowScrollbars(wxSHOW_SB_NEVER, wxSHOW_SB_ALWAYS);
    ...
    auto init_combo =
        [this, margin_5](PlaterPresetComboBox **combo, wxString label, Preset::Type preset_type, bool filament)
    {
    ...
    params_sizer->Add(m_frequently_changed_parameters->get_sizer(), 0,
    ...
    is_msw ? scrolled_sizer->Add(m_presets_panel, 0, wxEXPAND | size_margin, margin_5)
    ...
    auto init_scalable_btn =
        [this](ScalableButton **btn, const std::string &icon_name, wxString tooltip = wxEmptyString)
    {
#ifdef __APPLE__
        int bmp_px_cnt = 16;
#else
        ...
        *btn = new ScalableButton(this, wxID_ANY, bmp, "", wxBU_EXACTFIT);
        ...
        (*btn)->Bind(wxEVT_ENTER_WINDOW,
                     [tooltip, btn, this](wxMouseEvent &event)
                     {
                         show_rich_tip(tooltip, *btn);
                         event.Skip();
                     });
        (*btn)->Bind(wxEVT_LEAVE_WINDOW,
                     [btn, this](wxMouseEvent &event)
                     {
                         hide_rich_tip(*btn);
                         event.Skip();
                     });
    ...
    init_scalable_btn(&m_btn_send_gcode, "export_gcode",
                      _L("Send to printer") + " " + GUI::shortkey_ctrl_prefix() + "Shift+G");
    init_scalable_btn(&m_btn_export_gcode_removable, "export_to_sd",
    ...
    complect_btns_sizer->Add(m_btn_connect_gcode, 1, wxEXPAND);
    ...
    sizer->Add(m_scrolled_panel, 1, wxEXPAND | wxRIGHT, 7); // 7px right padding

    const int buttons_sizer_flags{wxEXPAND | wxLEFT | wxBOTTOM | wxRIGHT};
    sizer->Add(m_btns_sizer, 0, buttons_sizer_flags, margin_5);
    // Add bottom spacing to ensure border visibility
    sizer->AddSpacer(margin_5);

    m_autoslicing_btns_sizer = new wxBoxSizer(wxHORIZONTAL);

    init_scalable_btn(&m_btn_export_all_gcode_removable, "export_to_sd",
                      _L("Export all to SD card / Flash drive") + " " + GUI::shortkey_ctrl_prefix() + "U");
    ...
    Bind(wxEVT_PAINT,
         [this](wxPaintEvent &)
         {
             ...
             // Get appropriate border color for dark mode
             wxColour border_color;
#ifdef _WIN32
             if (wxGetApp().dark_mode())
             {
                 border_color = wxColour(60, 60, 60); // Dark gray for dark mode
             }
             else
             {
                 border_color = wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE);
             }
#else
        border_color = wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE);
#endif

             // Make border more visible by using a darker color
             wxColour visible_border = border_color.ChangeLightness(80); // Darken by 20%
             dc.SetPen(wxPen(visible_border, 3));                        // 3-pixel wide border for better visibility
             dc.SetBrush(*wxTRANSPARENT_BRUSH);

             // Draw border on right side only (since we're on the left)
             // Move border in by 4 pixels to ensure visibility
             dc.DrawLine(size.x - 4, 0, size.x - 4, size.y);
         });

    m_btn_export_gcode->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { m_plater->export_gcode(false); });
    m_btn_reslice->Bind(wxEVT_BUTTON,
                        [this](wxCommandEvent &)
                        {
...
#ifdef _WIN32
    m_btn_reslice->Bind(wxEVT_ENTER_WINDOW,
                        [this](wxMouseEvent &event)
                        {
                            show_rich_tip(m_reslice_btn_tooltip, m_btn_reslice);
                            event.Skip();
                        });
    m_btn_reslice->Bind(wxEVT_LEAVE_WINDOW,
                        [this](wxMouseEvent &event)
                        {
    ...
    m_btn_export_all_gcode->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { this->m_plater->export_all_gcodes(false); });

    m_btn_export_all_gcode_removable->Bind(wxEVT_BUTTON,
                                           [this](wxCommandEvent &) { this->m_plater->export_all_gcodes(true); });

    m_btn_connect_gcode_all->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { this->m_plater->connect_gcode_all(); });
}
...
void Sidebar::init_filament_combo(PlaterPresetComboBox **combo, int extr_idx)
{
    *combo = new PlaterPresetComboBox(m_presets_panel, Slic3r::Preset::TYPE_FILAMENT);
    ...
    this->m_filaments_sizer->Add(combo_and_btn_sizer, 1,
                                 wxEXPAND |
#ifdef __WXGTK3__
                                     wxRIGHT,
...
void Sidebar::on_select_preset(wxCommandEvent &evt)
{
    ...
    // Fix for preset combo box issue
...
{
    SetMinSize(wxSize(43 * wxGetApp().em_unit(), -1));
...
void Sidebar::sys_color_changed()
{
    ...
    for (wxWindow *win : std::vector<wxWindow *>{this, m_sliced_info->GetStaticBox(), m_object_info->GetStaticBox(),
                                                 m_btn_reslice, m_btn_export_gcode})
...
void Sidebar::show_info_sizer()
{
    ...
    if (selection.is_single_volume())
    {
        ...
        if (vol_idxs.size() != 1)
            ...
            // Fix for filament combo visibility
            return;
    ...
    {
        {
            ...
            info_text = wxString::Format("%.2f",
                                         imperial_units ? ps.total_extruded_volume * koef : ps.total_extruded_volume);
                ...
                for (const auto &[filament_id, filament_vol] : ps.filament_stats)
                {
                    ...
                    if (const Preset *preset = extruders_filaments[filament_id].get_selected_preset())
                    {
                        ...
                        else
                        {
                            ...
                            filament_weight = filament_vol * filament_density /* *2.4052f*/ *
                                              0.001; // assumes 1.75mm filament diameter;
            ...
            info_text = ps.total_cost == 0.0 ? "N/A"
                        : is_wipe_tower
                            ? wxString::Format("%.2f \n%.2f \n%.2f", ps.total_cost,
                                               (ps.total_cost - ps.total_wipe_tower_cost), ps.total_wipe_tower_cost)
            ...
            m_sliced_info->SetTextAndShow(siWTNumberOfToolchanges, ps.total_toolchanges > 0
...
bool Sidebar::show_reslice(bool show) const
{
    ...
    return m_btn_reslice->Show(false);
}
...
void Sidebar::set_btn_label(const ActionButtonType btn_type, const wxString &label) const
{
    ...
    case ActionButtonType::Export:
        ...
        break;
    case ActionButtonType::SendGCode: /*m_btn_send_gcode->SetLabelText(label);*/
        break;
    case ActionButtonType::Connect: /*m_btn_connect_gcode->SetLabelText(label);*/
        break;
    }
}
...
void Sidebar::collapse(bool collapse)
{
    ...
    {
        // When showing: show immediately and layout once
        this->Show(true);
        if (GetContainingSizer())
        {
            GetContainingSizer()->Layout();
