...
static void smooth_normals_corner(const TriangleMesh &mesh, std::vector<stl_normal> &normals)
{
    ...
    Eigen::MatrixXd vertices =
        MapMatrixXfUnaligned(mesh.its.vertices.front().data(), Eigen::Index(mesh.its.vertices.size()), 3).cast<double>();
    ...
    Eigen::MatrixXd in_normals =
        MapMatrixXfUnaligned(face_normals.front().data(), Eigen::Index(face_normals.size()), 3).cast<double>();
...
void GLModel::render(const std::pair<size_t, size_t> &range)
{
    ...
    if (position)
    {
        ...
        if (position_id != -1)
        {
            glsafe(::glVertexAttribPointer(position_id, Geometry::position_stride_floats(data.format), GL_FLOAT,
                                           GL_FALSE, vertex_stride_bytes,
    ...
    if (normal)
    {
        ...
        if (normal_id != -1)
        {
            glsafe(::glVertexAttribPointer(normal_id, Geometry::normal_stride_floats(data.format), GL_FLOAT, GL_FALSE,
                                           vertex_stride_bytes,
    ...
    if (tex_coord)
    {
        ...
        if (tex_coord_id != -1)
        {
            glsafe(::glVertexAttribPointer(tex_coord_id, Geometry::tex_coord_stride_floats(data.format), GL_FLOAT,
                                           GL_FALSE, vertex_stride_bytes,
    ...
    if (extra)
    {
        ...
        if (extra_id != -1)
        {
            glsafe(::glVertexAttribPointer(extra_id, Geometry::extra_stride_floats(data.format), GL_FLOAT, GL_FALSE,
                                           vertex_stride_bytes,
                                           (const void *) Geometry::extra_offset_bytes(data.format)));
    ...
    glsafe(::glDrawElements(mode, range.second - range.first, index_type,
                            (const void *) (range.first * Geometry::index_stride_bytes(data))));
...
void GLModel::render_instanced(unsigned int instances_vbo, unsigned int instances_count)
{
    ...
    glsafe(
        ::glVertexAttribPointer(scales_id, 2, GL_FLOAT, GL_FALSE, instance_stride, (const void *) (3 * sizeof(float))));
    ...
    if (position)
    {
        glsafe(::glVertexAttribPointer(position_id, Geometry::position_stride_floats(data.format), GL_FLOAT, GL_FALSE,
                                       vertex_stride_bytes,
                                       (const void *) Geometry::position_offset_bytes(data.format)));
        glsafe(::glEnableVertexAttribArray(position_id));
    }
    ...
    {
        glsafe(::glVertexAttribPointer(normal_id, Geometry::normal_stride_floats(data.format), GL_FLOAT, GL_FALSE,
                                       vertex_stride_bytes, (const void *) Geometry::normal_offset_bytes(data.format)));
        glsafe(::glEnableVertexAttribArray(normal_id));
    }
...
bool GLModel::send_to_gpu()
{
    ...
    s_statistics.gpu_memory.vertices.max = std::max(s_statistics.gpu_memory.vertices.current,
                                                    s_statistics.gpu_memory.vertices.max);
#endif // ENABLE_GLMODEL_STATISTICS
    ...
    if (m_render_data.vertices_count <= 256)
    {
        ...
        glsafe(::glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices_count * sizeof(unsigned char), reduced_indices.data(),
                              GL_STATIC_DRAW));
    }
    else if (m_render_data.vertices_count <= 65536)
    {
        ...
        glsafe(::glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices_count * sizeof(unsigned short), reduced_indices.data(),
                              GL_STATIC_DRAW));
    }
    else
    {
    ...
    s_statistics.gpu_memory.indices.max = std::max(s_statistics.gpu_memory.indices.current,
                                                   s_statistics.gpu_memory.indices.max);
#endif // ENABLE_GLMODEL_STATISTICS
...
void GLModel::render_statistics()
{
    ...
    imgui.set_next_window_pos(0.5f * wxGetApp().plater()->get_current_canvas3D()->get_canvas_size().get_width(), 0.0f,
                              ImGuiCond_Once, 0.5f, 0.0f);
...
bool contains(const BuildVolume &volume, const GLModel &model, bool ignore_bottom)
{
    ...
    case BuildVolume::Type::Circle:
    {
        ...
        return volume.max_print_height() == 0.0
                   ? all_vertices_inside(model.get_geometry(),
                                         [c, r2](const Vec3f &p) { return (to_2d(p) - c).squaredNorm() <= r2; })
                   :

                   all_vertices_inside(model.get_geometry(),
                                       [c, r2, z = volume.max_print_height() + epsilon](const Vec3f &p)
    ...
    case BuildVolume::Type::Custom:
        return volume.max_print_height() == 0.0
                   ? all_vertices_inside(model.get_geometry(),
                                         [&volume](const Vec3f &p)
                                         {
                                             return Geometry::inside_convex_polygon(
                                                 volume.top_bottom_convex_hull_decomposition_bed(),
                                                 to_2d(p).cast<double>());
                                         })
                   : all_vertices_inside(model.get_geometry(),
                                         [&volume, z = volume.max_print_height() + epsilon](const Vec3f &p)
                                         {
                                             return Geometry::inside_convex_polygon(
                                                        volume.top_bottom_convex_hull_decomposition_bed(),
                                                        to_2d(p).cast<double>()) &&
...
GLModel::Geometry stilized_arrow(unsigned int resolution, float tip_radius, float tip_height, float stem_radius,
                                 float stem_height)
{
    ...
    for (unsigned int i = 0; i < resolution; ++i)
    {
        data.add_vertex(Vec3f(tip_radius * sines[i], tip_radius * cosines[i], stem_height),
                        Vec3f(sines[i], cosines[i], 0.0f));
    }
    ...
    for (unsigned int i = 0; i < resolution; ++i)
    {
        data.add_vertex(Vec3f(stem_radius * sines[i], stem_radius * cosines[i], stem_height),
                        (Vec3f) (-Vec3f::UnitZ()));
    }
    ...
    for (unsigned int i = 0; i < resolution; ++i)
    {
        data.add_vertex(Vec3f(stem_radius * sines[i], stem_radius * cosines[i], 0.0f),
                        Vec3f(sines[i], cosines[i], 0.0f));
    }
...
GLModel::Geometry circular_arrow(unsigned int resolution, float radius, float tip_height, float tip_width,
                                 float stem_width, float thickness)
{
    ...
    for (unsigned int i = 0; i <= resolution; ++i)
    {
        ...
        data.add_vertex(Vec3f(inner_radius * ::sin(angle), inner_radius * ::cos(angle), half_thickness),
                        (Vec3f) Vec3f::UnitZ());
    }
    ...
    {
        const float angle = float(i) * step_angle;
        data.add_vertex(Vec3f(outer_radius * ::sin(angle), outer_radius * ::cos(angle), half_thickness),
                        (Vec3f) Vec3f::UnitZ());
    }
...
GLModel::Geometry straight_arrow(float tip_width, float tip_height, float stem_width, float stem_height,
                                 float thickness)
{
    GLModel::Geometry data;
    data.format = {GLModel::Geometry::EPrimitiveType::Triangles, GLModel::Geometry::EVertexLayout::P3N3};
...
GLModel::Geometry smooth_cylinder(unsigned int resolution, float radius, float height)
{
    ...
    for (unsigned int i = 0; i < sectorCount; ++i)
    {
        data.add_triangle(cap_center_id, (i < sectorCount - 1) ? cap_center_id + i + 2 : cap_center_id + 1,
                          cap_center_id + i + 1);
    }
    ...
    for (unsigned int i = 0; i < sectorCount; ++i)
    {
        data.add_triangle(cap_center_id, cap_center_id + i + 1,
                          (i < sectorCount - 1) ? cap_center_id + i + 2 : cap_center_id + 1);
    }

    return data;
}

GLModel::Geometry smooth_torus(unsigned int primary_resolution, unsigned int secondary_resolution, float radius,
                               float thickness)
{
