...
ArrangeSettingsDialogImgui::ArrangeSettingsDialogImgui(ImGuiWrapper *imgui, AnyPtr<arr2::ArrangeSettingsDb> db)
...
void ArrangeSettingsDialogImgui::render(float pos_x, float pos_y, bool current_bed)
{
    ...
    if (!wxGetApp().preset_bundle->prints.get_edited_preset().config.opt_bool("complete_objects") ||
        wxGetApp().preset_bundle->prints.get_edited_preset().printer_technology() == ptSLA)
    {
        Settings settings;
        read_settings(settings, m_db.get());

        ImGuiPureWrap::text(
            GUI::format(_L("Press %1%left mouse button to enter the exact value"), shortkey_ctrl_prefix()));
        ...
        if (m_show_xl_combo_predicate() && settings.xl_align >= 0 &&
            ImGuiPureWrap::combo(_u8L("Alignment"),
                                 {_u8L("Center"), _u8L("Rear left"), _u8L("Front left"), _u8L("Front right"),
                                  _u8L("Rear right"), _u8L("Random")},
                                 settings.xl_align))
        {
            if (settings.xl_align >= 0 && settings.xl_align < ArrangeSettingsView::xlpCount)
                m_db->set_xl_alignment(static_cast<ArrangeSettingsView::XLPivots>(settings.xl_align));
        }
                                 ...
                                 settings.geom_handling))
        {
            if (settings.geom_handling >= 0 && settings.geom_handling < ArrangeSettingsView::ghCount)
                m_db->set_geometry_handling(static_cast<ArrangeSettingsView::GeometryHandling>(settings.geom_handling));
        }
