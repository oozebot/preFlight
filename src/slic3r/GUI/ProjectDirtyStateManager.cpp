...
void ProjectDirtyStateManager::update_from_presets()
{
    ...
    for (const PresetCollection *preset_collection : app.get_active_preset_collections())
    {
        ...
        m_presets_dirty |= (has_project && !m_initial_presets[type].empty() &&
                            m_initial_presets[type] != preset_collection->get_selected_preset_name()) ||
                           preset_collection->saved_is_dirty();
    }
...
void ProjectDirtyStateManager::render_debug_window() const
{
    ...
    const UndoRedo::Snapshot *main_last_saveable_snapshot = get_last_saveable_snapshot(EStackType::Main, main_stack,
                                                                                       m_state.gizmos,
                                                                                       m_last_save.main);
