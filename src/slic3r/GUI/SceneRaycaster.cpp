                                                                  ...
                                                                  const Transform3d &trafo, bool use_back_faces)
{
    ...
    case EType::Bed:
    {
        return m_bed.emplace_back(
            std::make_shared<SceneRaycasterItem>(encode_id(type, id), raycaster, trafo, use_back_faces));
    }
    case EType::Volume:
    {
        return m_volumes.emplace_back(
            std::make_shared<SceneRaycasterItem>(encode_id(type, id), raycaster, trafo, use_back_faces));
    }
    case EType::Gizmo:
    {
        return m_gizmos.emplace_back(
            std::make_shared<SceneRaycasterItem>(encode_id(type, id), raycaster, trafo, use_back_faces));
    }
    case EType::FallbackGizmo:
    {
        return m_fallback_gizmos.emplace_back(
            std::make_shared<SceneRaycasterItem>(encode_id(type, id), raycaster, trafo, use_back_faces));
    }
    default:
    {
                                              ...
                                              const ClippingPlane *clipping_plane) const
{
    ...
    class VolumeKeeper
    {
        ...
        Vec3f m_closest_hit_pos{std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
    ...
    public:
        ...
        bool check_hit_result(const HitResult &hit)
        {
            ...
            if (m_selected_volume_already_found && hit.type == SceneRaycaster::EType::Volume &&
                hit.position.isApprox(m_closest_hit_pos))
                return false;

            if (hit.type == SceneRaycaster::EType::Volume)
                m_selected_volume_already_found = *m_selected_volume_id ==
                                                  (unsigned int) decode_id(hit.type, hit.raycaster_id);
    ...
    auto test_raycasters = [this, is_closest, clipping_plane, &volume_keeper](EType type, const Vec2d &mouse_pos,
                                                                              const Camera &camera, HitResult &ret)
    {
        const ClippingPlane *clip_plane = (clipping_plane != nullptr && type == EType::Volume) ? clipping_plane
                                                                                               : nullptr;
        ...
        for (const std::shared_ptr<SceneRaycasterItem> &item : *raycasters)
        {
            ...
            if (type == EType::Bed)
            {
                ...
                for (int i = 0; i < s_multiple_beds.get_number_of_beds(); ++i)
                {
                    ...
                    sth_hit = item->get_raycaster()->closest_hit(mouse_pos, trafo, camera, current_hit.position,
                                                                 current_hit.normal, clip_plane);
                    if (sth_hit)
                    {
                        bed_idx_hit = i;
                        break;
            ...
            if (sth_hit)
            {
                ...
                current_hit.normal = (trafo.matrix().block(0, 0, 3, 3).inverse().transpose() *
                                      current_hit.normal.cast<double>())
                                         .normalized()
                                         .cast<float>();
...
void SceneRaycaster::render_hit(const Camera &camera)
{
    ...
    const Transform3d sphere_view_model_matrix = camera.get_view_matrix() *
                                                 Geometry::translation_transform(
                                                     (*m_last_hit).position.cast<double>()) *
    ...
    m.matrix().block(0, 0, 3,
                     3) = q.setFromTwoVectors(Vec3d::UnitZ(), (*m_last_hit).normal.cast<double>()).toRotationMatrix();
...
int SceneRaycaster::encode_id(EType type, int id)
{
    return base_id(type) + id;
}
int SceneRaycaster::decode_id(EType type, int id)
{
