...
Slic3r::GCodeExtrusionRole convert(EGCodeExtrusionRole role)
{
    ...
    {
        return Slic3r::GCodeExtrusionRole::InterlockingPerimeter;
    }
...
EGCodeExtrusionRole convert(Slic3r::GCodeExtrusionRole role)
{
    ...
    case Slic3r::GCodeExtrusionRole::InterlockingPerimeter:
    {
                       ...
                       std::function<void(float)> progress_callback)
{
    ...
    for (size_t i = 1; i < moves.size(); ++i)
    {
        ...
        if (option_type == EOptionType::COUNT || option_type == EOptionType::Travels ||
            option_type == EOptionType::Wipes)
        {
            // When width or height changes significantly (>5%), insert a path break to prevent
            // the tapering effect in the preview. This is needed for interlocking perimeters
            // which have abrupt width changes when transitioning flow rates.
            ...
            const bool significant_width_change = (prev.width > 0.0f && curr.width > 0.0f) &&
                                                  std::abs(curr.width - prev.width) / prev.width >
...
static void convert_lines_to_vertices(const Slic3r::Lines &lines, const std::vector<float> &widths,
...
static void convert_to_vertices(const Slic3r::ExtrusionPath &extrusion_path, float print_z, size_t layer_id,
                                size_t extruder_id, size_t color_id, EGCodeExtrusionRole extrusion_role,
                                const Slic3r::Point &shift, std::vector<PathVertex> &vertices)
{
...
static void convert_to_vertices(const Slic3r::ExtrusionMultiPath &extrusion_multi_path, float print_z, size_t layer_id,
                                size_t extruder_id, size_t color_id, EGCodeExtrusionRole extrusion_role,
                                const Slic3r::Point &shift, std::vector<PathVertex> &vertices)
{
    Slic3r::Lines lines;
...
static void convert_to_vertices(const Slic3r::ExtrusionLoop &extrusion_loop, float print_z, size_t layer_id,
                                size_t extruder_id, size_t color_id, EGCodeExtrusionRole extrusion_role,
                                const Slic3r::Point &shift, std::vector<PathVertex> &vertices)
{
    Slic3r::Lines lines;
...
// forward declaration
static void convert_to_vertices(const Slic3r::ExtrusionEntityCollection &extrusion_entity_collection, float print_z,
                                size_t layer_id, size_t extruder_id, size_t color_id,
                                EGCodeExtrusionRole extrusion_role, const Slic3r::Point &shift,
                                std::vector<PathVertex> &vertices);

static void convert_to_vertices(const Slic3r::ExtrusionEntity &extrusion_entity, float print_z, size_t layer_id,
                                size_t extruder_id, size_t color_id, EGCodeExtrusionRole extrusion_role,
                                const Slic3r::Point &shift, std::vector<PathVertex> &vertices)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                auto *extrusion_entity_collection = dynamic_cast<const Slic3r::ExtrusionEntityCollection *>(
                    &extrusion_entity);
    ...
    for (size_t i = 0; i < skirt_height; ++i)
    {
        if (i == 0)
            convert_to_vertices(print.brim(), data.layers_zs[i], i, 0, 0, EGCodeExtrusionRole::Skirt,
                                Slic3r::Point(0, 0), data.vertices);
        convert_to_vertices(print.skirt(), data.layers_zs[i], i, 0, 0, EGCodeExtrusionRole::Skirt, Slic3r::Point(0, 0),
                            data.vertices);
    }
}
...
{
public:
    ObjectHelper(const std::vector<Slic3r::CustomGCode::Item> &color_print_values, size_t tool_colors_count,
                                       ...
                                       size_t extruders_count, VerticesData &data)
{
    ...
    std::sort(layers.begin(), layers.end(),
              [](const Slic3r::Layer *l1, const Slic3r::Layer *l2) { return l1->print_z < l2->print_z; });
