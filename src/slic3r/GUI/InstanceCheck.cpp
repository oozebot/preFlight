...
static void delete_lockfile()
{
    ...
    if (s_created_lockfile)
    {
        ...
        if (wxPlatformInfo::Get().GetOSMajorVersion() > 12)
            send_message_mac_closing(GUI::wxGetApp().get_instance_hash_string(),
                                     GUI::wxGetApp().get_instance_hash_string());
#endif
    }
}
    ...
    for (++it; it != args.end(); ++it)
    {
        ...
        else if (it->rfind("preflight://login", 0) == 0)
        {
            wxPostEvent(m_callback_evt_handler,
                        LoginOtherInstanceEvent(GUI::EVT_LOGIN_OTHER_INSTANCE, std::string(*it)));
        }
    }
    if (!paths.empty())
    {
        wxPostEvent(m_callback_evt_handler,
                    LoadFromOtherInstanceEvent(GUI::EVT_LOAD_MODEL_OTHER_INSTANCE,
                                               std::vector<boost::filesystem::path>(std::move(paths))));
    }
    if (!downloads.empty())
    {
        wxPostEvent(m_callback_evt_handler,
                    StartDownloadOtherInstanceEvent(GUI::EVT_START_DOWNLOAD_OTHER_INSTANCE,
