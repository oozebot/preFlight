...
std::shared_ptr<ArchiveViewNode> ArchiveViewModel::AddFile(std::shared_ptr<ArchiveViewNode> parent,
                                                           const wxString &name, bool container)
{
...
FileArchiveDialog::FileArchiveDialog(wxWindow *parent_window, mz_zip_archive *archive,
    ...
    , m_selected_paths_w_size(selected_paths_w_size)
{
                                                     ...
                                                     std::vector<std::shared_ptr<ArchiveViewNode>> &stack) -> size_t
    {
        ...
        if (stack.size() > struct_size &&
            (file.has_extension() || file.filename().string() != stack[struct_size]->get_name()))
        {
            reduce_stack(stack, struct_size);
        }
        if (!file.has_extension() && stack.size() == struct_size)
            stack.push_back(
                avc->get_model()->AddFile((stack.empty() ? std::shared_ptr<ArchiveViewNode>(nullptr) : stack.back()),
                                          boost::nowide::widen(file.filename().string()),
                                          true)); // filename string to wstring?
    ...
    std::sort(filtered_entries.begin(), filtered_entries.end(),
              [](const std::pair<boost::filesystem::path, size_t> &p1,
                 const std::pair<boost::filesystem::path, size_t> &p2)
    ...
    for (const auto &entry : filtered_entries)
    {
        ...
        if (!stack.empty())
            parent = stack.back();
        if (std::regex_match(path.extension().string(), pattern_drop))
        { // this leaves out non-compatible files
            std::shared_ptr<ArchiveViewNode> new_node =
                m_avc->get_model()->AddFile(parent, boost::nowide::widen(path.filename().string()), false);
...
void FileArchiveDialog::on_open_button()
{
    ...
    std::function<void(ArchiveViewNode *)> deep_fill =
        [&paths = m_selected_paths_w_size, &deep_fill](ArchiveViewNode *node)
    {
        if (node == nullptr)
            return;
