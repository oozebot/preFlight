...
void PressureEqualizer::process_layer(const std::string &gcode)
{
    ...
    for (auto current_extrusion_end_it = m_gcode_lines.cbegin(); current_extrusion_end_it != m_gcode_lines.cend();)
    {
        ...
        const std::ptrdiff_t current_extrusion_begin_idx = std::distance(m_gcode_lines.cbegin(),
                                                                         current_extrusion_begin_it);
        for (auto current_line_it = current_extrusion_begin_it; current_line_it != current_extrusion_end_it;
             ++current_line_it)
        {
            ...
            const size_t start_idx = size_t(
                std::max<std::ptrdiff_t>(current_extrusion_begin_idx, current_line_idx - max_look_back_limit));
...
void PressureEqualizer::adjust_volumetric_rate(const size_t first_line_idx, const size_t last_line_idx)
{
    ...
    feedrate_per_extrusion_role[int(m_gcode_lines[line_idx].extrusion_role)] =
    ...
    feedrate_per_extrusion_role.fill(std::numeric_limits<float>::max());
    feedrate_per_extrusion_role[size_t(m_gcode_lines[line_idx].extrusion_role)] =
    ...
    while (line_idx != last_line_idx)
    {
        ...
        for (size_t iRole = 1; iRole < size_t(GCodeExtrusionRole::Count); ++iRole)
        {
            ...
            if (line.adjustable_flow)
            {
                float rate_end = sqrt(rate_start * rate_start + 2 * line.volumetric_extrusion_rate * line.dist_xyz() *
                                                                    rate_slope / line.feedrate());
