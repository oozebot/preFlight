...
GCodeFindReplace::GCodeFindReplace(const std::vector<std::string> &gcode_substitutions)
{
    ...
    for (size_t i = 0; i < gcode_substitutions.size(); i += 4)
    {
        ...
        try
        {
            ...
            if (out.regexp)
            {
                out.regexp_pattern.assign(out.whole_word ? std::string("\\b") + out.plain_pattern + "\\b"
                                                         : out.plain_pattern,
        ...
        {
            throw RuntimeError(
                std::string("Invalid gcode_substitutions parameter, failed to compile regular expression: ") +
                ex.what());
        }
...
static void find_and_replace_whole_word(std::string &inout, const std::string &match, const std::string &replace,
                                        FindFn find_fn)
{
...
std::string GCodeFindReplace::process_layer(const std::string &ain)
{
    ...
    for (const Substitution &substitution : m_substitutions)
    {
        if (substitution.regexp)
        {
            temp.clear();
            temp.reserve(in->size());
            boost::regex_replace(ToStringIterator(temp), in->begin(), in->end(), substitution.regexp_pattern,
                                 substitution.format,
                                 (substitution.single_line ? boost::match_single_line | boost::match_default
                                                           : boost::match_not_dot_newline | boost::match_default) |
                                     boost::format_all);
            std::swap(out, temp);
        }
        else
                ...
                if (substitution.whole_word)
                    find_and_replace_whole_word(out, substitution.plain_pattern, substitution.format,
                                                [](const std::string &str, size_t start_pos, const std::string &match)
                                                {
                                                    ...
                                                    boost::iterator_range<std::string::const_iterator> r1(begin,
                                                                                                          str.end());
                                                    boost::iterator_range<std::string::const_iterator> r2(match.begin(),
                                                                                                          match.end());
                                                    auto res = boost::ifind_first(r1, r2);
                                                    return res ? std::make_pair(size_t(res.begin() - str.begin()),
                                                                                size_t(res.end() - str.begin()))
                ...
                if (substitution.whole_word)
                    find_and_replace_whole_word(out, substitution.plain_pattern, substitution.format,
                                                [](const std::string &str, size_t start_pos, const std::string &match)
                                                {
                                                    ...
                                                    return std::make_pair(pos, pos + (pos == std::string::npos
                                                                                          ? 0
                                                                                          : match.size()));
                                                });
                else
