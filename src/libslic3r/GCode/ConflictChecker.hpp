...
public:
    ...
    void raise()
    {
        if (valid())
        {
            if (_piles[_curPileIdx].empty() == false)
            {
                _curHeight += _piles[_curPileIdx].front().height();
            }
            _curPileIdx++;
        }
    }
    ...
    LineWithIDs curLines() const
    {
        ...
        for (const ExtrusionPath &path : _piles[_curPileIdx])
        {
            ...
            for (int i = 0; i < (int) _offsets.size(); ++i)
            {
                ...
                {
                    lines.emplace_back(line, _id, i, path.role());
                }
            }
        }
        return lines;
    ...
    friend bool operator>(const LinesBucket &left, const LinesBucket &right)
    {
        return left._curHeight > right._curHeight;
    }
    friend bool operator<(const LinesBucket &left, const LinesBucket &right)
    {
        return left._curHeight < right._curHeight;
...
{
    static ConflictResultOpt find_inter_of_lines_in_diff_objs(SpanOfConstPtrs<PrintObject> objs,
                                                              const WipeTowerData &wtd);
