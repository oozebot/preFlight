...
const char *support_surface_type_to_color_name(const SupporLayerType surface_type)
{
    ...
    case SupporLayerType::GapTransition:
        return "rgb(0,128,255)"; // light blue (similar to base)
...
void export_support_surface_type_legend_to_svg(SVG &svg, const Point &pos)
{
    ...
    svg.draw_legend(Point(pos_x, pos_y), "top contact",
                    support_surface_type_to_color_name(SupporLayerType::TopContact));
    pos_x += step_x;
    svg.draw_legend(Point(pos_x, pos_y), "top iface",
                    support_surface_type_to_color_name(SupporLayerType::TopInterface));
    pos_x += step_x;
    svg.draw_legend(Point(pos_x, pos_y), "base", support_surface_type_to_color_name(SupporLayerType::Base));
    pos_x += step_x;
    svg.draw_legend(Point(pos_x, pos_y), "bottom iface",
                    support_surface_type_to_color_name(SupporLayerType::BottomInterface));
    pos_x += step_x;
    svg.draw_legend(Point(pos_x, pos_y), "bottom contact",
                    support_surface_type_to_color_name(SupporLayerType::BottomContact));
    // 2nd row
    pos_x = pos_x0;
    pos_y = pos(1) + scale_(2.8);
    svg.draw_legend(Point(pos_x, pos_y), "raft interface",
                    support_surface_type_to_color_name(SupporLayerType::RaftInterface));
    pos_x += step_x;
    ...
    pos_x += step_x;
    svg.draw_legend(Point(pos_x, pos_y), "intermediate",
                    support_surface_type_to_color_name(SupporLayerType::Intermediate));
}
...
void export_print_z_polygons_to_svg(const char *path, SupportGeneratorLayer **const layers, int n_layers)
{
    ...
    for (int i = 0; i < n_layers; ++i)
        svg.draw(union_ex(layers[i]->polygons), support_surface_type_to_color_name(layers[i]->layer_type),
                 transparency);
    for (int i = 0; i < n_layers; ++i)
...
void export_print_z_polygons_and_extrusions_to_svg(const char *path, SupportGeneratorLayer **const layers, int n_layers,
