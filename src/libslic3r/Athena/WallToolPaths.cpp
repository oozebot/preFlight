    ...
    for (VariableWidthLines &inset : toolpaths)
    {
        ...
        for (size_t i = 0; i < inset.size(); ++i)
        {
            ...
            for (size_t j = i + 1; j < inset.size(); ++j)
            {
                ...
                {
                    dbg_log(Slic3r::DBG_PERIMETERS, debug_print_z, "PERIM",
...
void WallToolPaths::separateOutInnerContour()
{
    ...
    Slic3r::UnknownWindingStats uw_stats;
    ...
    if (Slic3r::debug_enabled(Slic3r::DBG_PERIMETERS) &&
        ...
        dbg_log(Slic3r::DBG_PERIMETERS, debug_print_z, "PERIM", "WTP_UNION dup_dropped=%zu holes=%zu near_dup=%zu",
