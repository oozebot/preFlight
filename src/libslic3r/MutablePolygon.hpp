...
public:
    ...
    MutablePolygon(const Polygon &rhs, size_t reserve = 0)
        : MutablePolygon(rhs.points.begin(), rhs.points.end(), reserve)
    {
    }
    MutablePolygon(std::initializer_list<Point> rhs, size_t reserve = 0)
        : MutablePolygon(rhs.begin(), rhs.end(), reserve)
    {
    }
    ...
    void assign(const Polygon &rhs, size_t reserve = 0) { assign(rhs.points.begin(), rhs.points.end(), reserve); }
...
inline ExPolygons remove_duplicates(ExPolygons expolygons, coord_t scaled_eps, double max_angle)
{
    ...
    for (ExPolygon &expolygon : expolygons)
    {
        ...
        expolygon.holes.erase(std::remove_if(expolygon.holes.begin(), expolygon.holes.end(),
                                             [](const auto &p) { return p.empty(); }),
                              expolygon.holes.end());
    }
    expolygons.erase(std::remove_if(expolygons.begin(), expolygons.end(), [](const auto &p) { return p.empty(); }),
                     expolygons.end());
    return expolygons;
}
...
inline Polygons smooth_outward(Polygons polygons, coord_t clip_dist_scaled)
{
    ...
    polygons.erase(std::remove_if(polygons.begin(), polygons.end(), [](const auto &p) { return p.empty(); }),
                   polygons.end());
    return polygons;
}
