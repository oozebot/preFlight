...
// binary cascade. Other counts (fallback only) map from a virtual power-of-2
// ring with the surplus mouths at level 1.
static void assign_ring_levels(std::vector<Slit> &slits)
{
    ...
    for (size_t v = 0; v < V; ++v)
    {
        ...
        size_t r = (size_t) std::llround((double) v * (double) n / (double) V) % n;
        slits[r].level = level;
    }
}
              ...
              ExPolygons *out_interior)
{
    ...
    Anchor anchor = build_anchor(island, bw, params.band_clamp == 0, dbg);
    ...
    // mode has no deep spokes, so no ring.
    ...
    for (size_t loop_idx = 0; loop_idx < loops_src.size(); ++loop_idx)
    {
        ...
        // The grid anchors to a stable geometric reference (the max-X vertex,
        ...
        for (size_t i = 1; i < lp.pts.size(); ++i)
            if (lp.pts[i].x() > lp.pts[ref_i].x() + 1.0 ||
                ...
                ref_i = i;
        const double ref_arc = lp.cum[ref_i];
        ...
        size_t n = pick_slit_count(lp.total, pitch_t);
        const bool count_fallback = (n == 0);
        if (count_fallback)
        ...
        // deep tooth brakes against its already-resolved shallower neighbors). A
        // uniform comb collapses to the first convergence depth. Depth mode keeps
        // the identical pattern and only clips the deep teeth at the bead below.
        assign_ring_levels(slits);
        size_t n_degenerate = 0, n_crowded = 0, n_floored = 0, n_truncated = 0;
        for (size_t k = 0; k < n; ++k)
        {
            Slit &s = slits[k];
            s.arc = wrap_arc(lp, phase + (double) k * pitch);
            ...
            {
                const double tip_standoff = center_loop ? r_loop + bw - tip_bond : 0.5 * (bw - tip_bond);
        ...
        const double hub_reach = (center_loop ? r_loop + bw - tip_bond : 0.5 * (bw - tip_bond)) + 0.25 * bw;
        for (size_t self = 0; self < n; ++self)
        {
            ...
            if (s.depth > 3.0 * bw && s.depth < s.max_t - frun_margin)
            {
                ...
                t_apex = std::min(t_apex, s.depth + 3.0 * bw);
        ...
        if (mouths.size() > 1)
        {
            ...
            for (size_t e = 0; e < mouths.size(); ++e)
            {
                ...
                for (int side = 0; side < 2; ++side)
                {
                    ...
                    double splay = 0.5 * (clearance - (bw - ov));
