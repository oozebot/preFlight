                                                                  ...
                                                                  coord_t &base_idx)
{
    ...
    for (const ExPolygon &expoly : src)
    {
        for (size_t icontour = 0; icontour < expoly.num_contours(); ++icontour)
        {
            ...
            offsetter.Clear();
            offsetter.AddPath(Slic3rPoints_to_ClipperPath(expoly.contour_or_hole(icontour).points), JoinType::Square,
                              EndType::Polygon);
            Clipper2Lib::Paths64 clipper_result;
            offsetter.Execute(icontour == 0 ? expansion : -expansion, clipper_result);
            expansion_cache = ClipperPaths_to_Slic3rPolygons(clipper_result);
...
static Polygons wavefront_initial(Clipper2Lib::ClipperOffset &co, const Polygons &polylines, float offset)
{
    ...
    for (const Polygon &path : polylines)
    {
        assert(path.size() >= 2);
        co.Clear();
        Clipper2Lib::Path64 clipper_path = Slic3rPoints_to_ClipperPath(path.points);
        ...
        append(out, ClipperPaths_to_Slic3rPolygons(out_this));
    }
    return out;
}
...
{
    Clipper2Lib::Clipper64 clipper;
    clipper.AddSubject(Slic3rPolygons_to_ClipperPaths(wavefront));
    clipper.AddClip(Slic3rPolygons_to_ClipperPaths(clipping));
