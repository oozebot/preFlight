...
bool check_custom_parameters(const std::string &cp_print, const std::string &cp_printer,
                             const std::vector<std::string> &cp_filaments, std::string *error)
{
                                                        ...
                                                        const std::vector<std::string> &cp_filaments)
{
    ...
    for (const CustomParameterValues &map : parsed_filaments)
    {
        for (const auto &[key, v] : map)
        {
            ...
            {
                if (const int *val = std::get_if<int>(&v))
                    config.set_key_value(full_key, new ConfigOptionIntsNullable(parsed_filaments.size(),
                                                                                ConfigOptionIntsNullable::nil_value()));
                else if (const double *val = std::get_if<double>(&v))
                    config.set_key_value(full_key,
                                         new ConfigOptionFloatsNullable(parsed_filaments.size(),
