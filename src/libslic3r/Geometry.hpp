...
// For int32_t coords: 30 bits + sign → 61 bits + sign in products → fits in int64_t
// For int64_t coords: 62 bits + sign → 125 bits + sign in products → needs 128-bit arithmetic
//
...
inline bool segments_intersect(const Slic3r::Point &ip1, const Slic3r::Point &ip2, const Slic3r::Point &jp1,
                               const Slic3r::Point &jp2)
{
    assert(ip1 != ip2);
    assert(jp1 != jp2);

    auto segments_could_intersect = [](const Slic3r::Point &ip1, const Slic3r::Point &ip2, const Slic3r::Point &jp1,
                                       const Slic3r::Point &jp2) -> std::pair<int, int>
    {
