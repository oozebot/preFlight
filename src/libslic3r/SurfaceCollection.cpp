...
{
    // Convert types to set for O(1) lookup instead of O(m) linear search
    std::unordered_set<SurfaceType> types_set(types.begin(), types.end());
...
void SurfaceCollection::keep_types(std::initializer_list<SurfaceType> types)
{
    ...
    size_t j = 0;
    for (size_t i = 0; i < surfaces.size(); ++i)
        if (types_set.count(surfaces[i].surface_type))
        { // O(1) lookup
            if (j < i)
                std::swap(surfaces[i], surfaces[j]);
            ++j;
...
void SurfaceCollection::remove_types(std::initializer_list<SurfaceType> types)
{
    ...
    size_t j = 0;
    for (size_t i = 0; i < surfaces.size(); ++i)
        if (types_set.count(surfaces[i].surface_type) == 0)
        { // O(1) lookup (inverted check)
            if (j < i)
                std::swap(surfaces[i], surfaces[j]);
            ++j;
        }
