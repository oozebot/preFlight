...
template<class EP>
constexpr bool IsExecutionPolicy = IsExecutionPolicy_<remove_cvref_t<EP>>::value;
...
{
    return AsTraits<EP>::reduce(ep, from, to, init, std::forward<MergeFn>(mergefn), std::forward<AccessFn>(accessfn),
...
{
    return reduce(ep, from, to, init, std::plus<T>{}, std::forward<AccessFn>(accessfn),
                  std::max(granularity, size_t(1)));
}
