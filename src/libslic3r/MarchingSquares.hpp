...
class Grid
{
    ...
    Coord rastercoord(const Coord &crd) const { return {(crd.r - 1) * m_window.r, (crd.c - 1) * m_window.c}; }
    ...
    void set_visited(size_t idx, Dir d = Dir::none) { m_tags[idx] |= (1 << (_t(d)) << 4); }
    ...
    size_t search_start_cell(size_t i = 0) const
    {
        ...
        // direction.
        while ((i < m_tags.size()) && (is_visited(i) || is_ambiguous(i)))
            ++i;
...
public:
    explicit Grid(const Rst &rst, const Coord &cellsz, const Coord &overlap)
        ...
        , m_gridsize{2 + (long(rows(rst)) - overlap.r) / m_window.r, 2 + (long(cols(rst)) - overlap.c) / m_window.c}
    ...
    void tag_grid(ExecutionPolicy &&policy, TRasterValue<Rst> isoval)
    {
        ...
        for_each(std::forward<ExecutionPolicy>(policy), m_tags.begin(), m_tags.end(),
                 [this, isoval](uint8_t &tag, size_t idx) { tag = get_tag_for_cell(coord(idx), isoval); });
    }
    ...
    std::vector<Ring> scan_rings()
    {
        ...
        while ((startidx = search_start_cell(startidx)) < m_tags.size())
        {
            ...
            if (next == Dir::none)
                m_tags[startidx] = _t(SquareTag::none);
    ...
    {
        for_each(std::forward<ExecutionPolicy>(policy), rings.begin(), rings.end(),
                 [this, isov](Ring &ring, size_t)
                 {
...
std::vector<marchsq::Ring> execute_with_policy(ExecutionPolicy &&policy, const Raster &raster,
                                               TRasterValue<Raster> isoval, Coord windowsize = {})
{
