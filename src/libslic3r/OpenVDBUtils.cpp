...
void VoxelGridDeleter::operator()(VoxelGrid *ptr)
{
    delete ptr;
}
...
inline Vec3f to_vec3f(const openvdb::Vec3s &v)
{
    return Vec3f{v.x(), v.y(), v.z()};
}
inline Vec3d to_vec3d(const openvdb::Vec3s &v)
{
    return to_vec3f(v).cast<double>();
}
inline Vec3i to_vec3i(const openvdb::Vec3I &v)
{
    return Vec3i{coord_t(v[2]), coord_t(v[1]), coord_t(v[0])};
}
...
VoxelGridPtr mesh_to_grid(const indexed_triangle_set &mesh, const MeshToGridParams &params)
{
    ...
    {
        auto subgrid = openvdb::tools::meshToVolume<openvdb::FloatGrid>(interrupter, TriangleMeshDataAdapter{m, trafo},
    ...
    if (meshparts.empty())
    {
        ...
        grid = openvdb::tools::meshToVolume<openvdb::FloatGrid>(interrupter, TriangleMeshDataAdapter{mesh, trafo},
                                                                openvdb::math::Transform{}, params.exterior_bandwidth(),
                                                                params.interior_bandwidth());
    }
...
indexed_triangle_set grid_to_mesh(const VoxelGrid &vgrid, double isovalue, double adaptivity,
...
VoxelGridPtr dilate_grid(const VoxelGrid &vgrid, float exteriorBandWidth, float interiorBandWidth)
{
    ...
    if (interiorBandWidth > 0.f)
        new_grid = openvdb::tools::dilateSdf(vgrid.grid, scale * interiorBandWidth, openvdb::tools::NN_FACE_EDGE,
                                             DilateIterations,
    ...
    if (exteriorBandWidth > 0.f)
        new_grid = openvdb::tools::dilateSdf(arg, scale * exteriorBandWidth, openvdb::tools::NN_FACE_EDGE,
                                             DilateIterations,
