...
indexed_triangle_set its_subdivide(const indexed_triangle_set &its, float max_length)
{
    ...
    struct Edges
    {
        Vec3f data[3];
        Vec3f lengths;
        Edges(const Vec3crd &indices, const std::vector<Vec3f> &vertices) : lengths(-1.f, -1.f, -1.f)
        {
        ...
        float abs_sum(const Vec3f &v) { return abs(v[0]) + abs(v[1]) + abs(v[2]); }
        bool is_dividable(const float &max_length)
        {
            ...
            Vec3i biggest_index = (sum[0] > sum[1])
                                      ? ((sum[0] > sum[2])
                                             ? ((sum[2] > sum[1]) ? Vec3i(coord_t(0), coord_t(2), coord_t(1))
                                                                  : Vec3i(coord_t(0), coord_t(1), coord_t(2)))
                                             : Vec3i(coord_t(2), coord_t(0), coord_t(1)))
                                      : ((sum[1] > sum[2])
                                             ? ((sum[2] > sum[0]) ? Vec3i(coord_t(1), coord_t(2), coord_t(0))
                                                                  : Vec3i(coord_t(1), coord_t(0), coord_t(2)))
                                             : Vec3i(coord_t(2), coord_t(1), coord_t(0)));
            for (int i = 0; i < 3; i++)
            {
                int index = biggest_index[i];
                if (sum[index] <= max_length)
                    return false;
    ...
    struct TriangleLengths
    {
        Vec3crd indices;
        Vec3f l; // lengths
        TriangleLengths(const Vec3crd &indices, const Vec3f &lengths) : indices(indices), l(lengths) {}
        ...
        std::pair<TriangleLengths, TriangleLengths> divide(int divide_index, float max_length,
                                                           std::vector<Vec3f> &vertices, EdgeDivides &edge_divides)
        {
            ...
            if (index_offset > 0)
            {
                ...
                if (edge_divides.find(new_key) == edge_divides.end())
                {
                    // insert new
                    edge_divides.insert(
                        {new_key, (new_key_swap) ? VerticesSequence(new_index - sign, !vs.positive_order) : vs});
                }
            }
            ...
            if (index_offset < int(count_edge_vertices) - 1)
            {
                ...
                {
                    edge_divides.insert(
                        {new_key, (new_key_swap) ? VerticesSequence(vs.start_index + sign * (count_edge_vertices - 1),
                                                                    !vs.positive_order)
                                                 : VerticesSequence(new_index + sign, vs.positive_order)});
                }
            }

            return {TriangleLengths(indices1, lengths1), TriangleLengths(indices2, lengths2)};
        }
    };
    ...
    for (const Vec3crd &indices : its.indices)
    {
        ...
        do
        {
            ...
            {
                auto [tl1, tl2] = tl.divide(divide_index, max_length, result.vertices, edge_divides);
                tl = tl1;
                tls.push(tl2);
            }
        } while (true);
