...
GCodeExtrusionRole extrusion_role_to_gcode_extrusion_role(ExtrusionRole role)
{
    ...
    if (role == ExtrusionRole::WipeTower)
...
std::string gcode_extrusion_role_to_string(GCodeExtrusionRole role)
{
    ...
    case GCodeExtrusionRole::InterlockingPerimeter:
        return L("Interlocking perimeter");
...
GCodeExtrusionRole string_to_gcode_extrusion_role(const std::string_view role)
{
    if (role == L("Perimeter"))
    ...
    else if (role == L("Interlocking perimeter"))
        return GCodeExtrusionRole::InterlockingPerimeter;
        ...
        return GCodeExtrusionRole::GapFill;
             ...
             role == L("Skirt/Brim")) // "Skirt" is for backward compatibility with 2.3.1 and earlier
        return GCodeExtrusionRole::Skirt;
    else if (role == L("Support material"))
        return GCodeExtrusionRole::SupportMaterial;
    else if (role == L("Support interface") ||
             role == L("Support material interface")) // "Support material interface" for backward compat
        return GCodeExtrusionRole::SupportMaterialInterface;
