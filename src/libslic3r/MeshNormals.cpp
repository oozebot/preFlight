...
Vec3d get_normal(const AABBMesh &mesh, const Vec3d &picking_point, double eps)
{
    ...
    auto cmpfn = [](const Vec3d &v1, const Vec3d &v2)
    {
    ...
    if (vertex_idx >= 0)
    { // The point is right on a vertex of the triangle
    ...
    { // point lies safely within its triangle
...
template<class Ex>
Eigen::MatrixXd normals(Ex ex_policy, const PointSet &points, const AABBMesh &mesh, double eps,
...
template Eigen::MatrixXd normals(ExecutionSeq policy, const PointSet &points, const AABBMesh &convert_mesh, double eps,
                                 std::function<void()> throw_on_cancel, const std::vector<unsigned> &selected_points);

template Eigen::MatrixXd normals(ExecutionTBB policy, const PointSet &points, const AABBMesh &convert_mesh, double eps,
                                 std::function<void()> throw_on_cancel, const std::vector<unsigned> &selected_points);
