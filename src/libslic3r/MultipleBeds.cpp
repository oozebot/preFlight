...
ObjectInstances get_object_instances(const Model &model)
{
    ...
    std::transform(model.objects.begin(), model.objects.end(), std::back_inserter(result),
                   [](ModelObject *object) { return std::pair{object, object->instances}; });
...
void restore_object_instances(Model &model, const ObjectInstances &object_instances)
{
    ...
    std::transform(object_instances.begin(), object_instances.end(), std::back_inserter(objects),
...
void MultipleBeds::remove_instances_outside_outside_bed(Model &model, const int bed_index) const
{
    for (ModelObject *mo : model.objects)
    {
        mo->instances.erase(std::remove_if(mo->instances.begin(), mo->instances.end(),
                                           [&](const ModelInstance *instance)
                                           { return !this->is_instance_on_bed(instance->id(), bed_index); }),
                            mo->instances.end());
    }

    model.objects.erase(std::remove_if(model.objects.begin(), model.objects.end(),
                                       [](const ModelObject *object) { return object->instances.empty(); }),
                        model.objects.end());
}
...
bool MultipleBeds::is_glvolume_on_thumbnail_bed(const Model &model, int obj_idx, int instance_idx) const
{
    ...
    if (obj_idx < 0 || instance_idx < 0 || obj_idx >= int(model.objects.size()) ||
        instance_idx >= int(model.objects[obj_idx]->instances.size()))
        return false;
...
void MultipleBeds::update_shown_beds(Model &model, const BuildVolume &build_volume, bool only_remove /*=false*/)
{
    ...
    const int max_bed{std::accumulate(this->m_inst_to_bed.begin(), this->m_inst_to_bed.end(), 0,
