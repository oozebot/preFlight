...
{
public:
    SurfaceFeature(SurfaceFeatureType type, const Vec3d &pt1, const Vec3d &pt2, std::optional<Vec3d> pt3 = std::nullopt,
                   double value = 0.0)
        : m_type(type), m_pt1(pt1), m_pt2(pt2), m_pt3(pt3), m_value(value)
    {
    }

    explicit SurfaceFeature(const Vec3d &pt) : m_type{SurfaceFeatureType::Point}, m_pt1{pt} {}
...
{
    AngleAndEdges(double angle_, const Vec3d &center_, const std::pair<Vec3d, Vec3d> &e1_,
                  const std::pair<Vec3d, Vec3d> &e2_, double radius_, bool coplanar_)
...
struct MeasurementResult
{
    ...
    {
        return angle.has_value() || distance_infinite.has_value() || distance_strict.has_value() ||
...
inline Vec3d edge_direction(const Vec3d &from, const Vec3d &to)
{
    return (to - from).normalized();
}
inline Vec3d edge_direction(const std::pair<Vec3d, Vec3d> &e)
{
...
inline bool are_parallel(const Vec3d &v1, const Vec3d &v2)
{
    return std::abs(std::abs(v1.dot(v2)) - 1.0) < EPSILON;
}
inline bool are_perpendicular(const Vec3d &v1, const Vec3d &v2)
{
