...
public:
     ...
     * to consider.
     */
    bool tryGetNextPoint(Point *out_unsupported_location, size_t *out_unsupported_cell_idx,
                         const size_t start_idx = 0) const
    {
...
protected:
    ...
    public:
        ...
        void initialize(const std::vector<UnsupportedCell> &unsupported_points,
                        const std::function<Point(const Point &)> &map_cell_to_grid)
        {
            ...
            m_grid_range = BoundingBox(map_cell_to_grid(unsupported_points_bbox.min),
    ...
    friend void export_distance_field_to_svg(const std::string &path, const Polygons &outline, const Polygons &overhang,
