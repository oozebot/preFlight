...
void FillHoneycomb::_fill_surface_single(const FillParams &params, unsigned int thickness_layers,
                                         ...
                                         Polylines &polylines_out)
{
    ...
    if (it_m == this->cache.end())
    {
        ...
        m.x_offset = min_spacing / coord_t(2);
        m.y_offset = coord_t(m.x_offset * sqrt(3) / 3);
        m.hex_center = Point(m.hex_width / coord_t(2), m.hex_side);
    }
    CacheData &m = it_m->second;
    ...
    Polylines all_polylines;
    {
        ...
        while (x <= bounding_box.max(0))
        {
            ...
            for (size_t i = 0; i < 2; ++i)
            {
                ...
                for (coord_t y = bounding_box.min(1); y <= bounding_box.max(1);
    ...
    if (params.dont_connect() || all_polylines.size() <= 1)
        append(polylines_out,
               chain_polylines_by_region(std::move(all_polylines), coord_t(scale_(this->spacing) * 3 / params.density),
