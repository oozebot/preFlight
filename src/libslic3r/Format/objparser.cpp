...
#define EATWS()                           \
    ...
    case 'v':
    {
        ...
        default:
        {
            ...
            // Example of OBJ parsing edge case,
    ...
    try
    {
        ...
        while ((len = ::fread(buf + lenPrev, 1, half_buf - 1, pFile)) != 0)
        {
            if (std::feof(pFile))
            {
                ...
                // Fix for OBJ vertex color parsing
...
bool objbinsave(const char *path, const ObjData &data)
{
    ...
    bool result = savevector(pFile, data.coordinates) && savevector(pFile, data.textureCoordinates) &&
                  savevector(pFile, data.normals) && savevector(pFile, data.parameters) &&
                  savevector(pFile, data.mtllibs) && savevectornameidx(pFile, data.usemtls) &&
...
bool objbinload(const char *path, ObjData &data)
{
    ...
    bool result = loadvector(pFile, data.coordinates) && loadvector(pFile, data.textureCoordinates) &&
                  loadvector(pFile, data.normals) && loadvector(pFile, data.parameters) &&
                  loadvector(pFile, data.mtllibs) && loadvectornameidx(pFile, data.usemtls) &&
...
extern bool objequal(const ObjData &data1, const ObjData &data2)
{
    ...
    return vectorequal(data1.coordinates, data2.coordinates) &&
