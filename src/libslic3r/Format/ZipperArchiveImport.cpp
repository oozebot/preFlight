...
ZipperArchive read_zipper_archive(const std::string &zipfname, const std::vector<std::string> &includes,
...
std::pair<DynamicPrintConfig, ConfigSubstitutions> extract_profile(const ZipperArchive &arch,
                                                                   DynamicPrintConfig &profile_out)
{
    DynamicPrintConfig profile_in, profile_use;
    ConfigSubstitutions config_substitutions = profile_in.load(arch.profile,
                                                               ForwardCompatibilitySubstitutionRule::Enable);

    if (profile_in.empty())
    { // missing profile... do guess work
        ...
        if (auto lh_opt = arch.config.find("layerHeight"); lh_opt != arch.config.not_found())
        {
            ...
            if (pos)
            { // TODO: verify that pos is 0 when parsing fails
