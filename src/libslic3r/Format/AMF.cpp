...
void AMFParserContext::endElement(const char * /* name */)
{
    ...
    case NODE_TYPE_METADATA:
        if ((m_config != nullptr) && strncmp(m_value[0].c_str(), SLIC3R_CONFIG_TYPE, strlen(SLIC3R_CONFIG_TYPE)) == 0)
        {
            ...
            // parsing older 3MFs which can have duplicated entries in the INI.
            // Fix for duplicated INI entries issue. We'll revert it for now.
        ...
        else if (strncmp(m_value[0].c_str(), "slic3r.", 7) == 0)
        {
                     ...
                     strcmp(opt_key, "layer_height_profile") == 0)
            {
                ...
                m_object->layer_height_profile.set(std::move(data));
                     ...
                     strcmp(opt_key, "sla_support_points") == 0)
            {
                // Parse object's layer height profile, a semicolon separated list of floats.
                unsigned char coord_idx = 0;
                Eigen::Matrix<float, 5, 1, Eigen::DontAlign> point(
                    Eigen::Matrix<float, 5, 1, Eigen::DontAlign>::Zero());
                char *p = m_value[1].data();
                for (;;)
                {
                    char *end = strchr(p, ';');
                    ...
                    if (++coord_idx == 5)
                    {
                        m_object->sla_support_points.push_back(
                            sla::SupportPoint{Vec3f(point[0], point[1], point[2]), point[3]});
                        coord_idx = 0;
                    }
                    if (end == nullptr)
                        break;
