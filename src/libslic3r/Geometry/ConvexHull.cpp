...
void visit_antipodals(Idx &ia, Idx &ib, Fn &&fn)
{
    ...
    while (visitor_continue && !finished)
    {
        ...
        if constexpr (mode == AntipodalVisitMode::Full)
            ...
            {
                visitor_continue = fn(current == &ia ? ia.idx() : ia.next(), current == &ib ? ib.idx() : ib.next(),
                                      current_dir_a);
            }
...
bool convex_polygons_intersect(const Polygon &A, const Polygon &B)
{
    ...
    visit_antipodals<AntipodalVisitMode::EdgesOnly>(imin, imax,
                                                    [&imin, &imax, &found_divisor](size_t ia, size_t ib,
                                                                                   const Point &dir)
                                                    {
...
bool inside_convex_polygon(const std::pair<std::vector<Vec2d>, std::vector<Vec2d>> &top_bottom_decomposition,
                           const Vec2d &pt)
{
