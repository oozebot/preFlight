...
// CRITICAL: Clipper2 uses int64_t internally (Path64). Using int32_t causes silent truncation
// and overflow with high-detail models (1M+ triangles), breaking boost::polygon Voronoi.
//
// OLD: int32_t (saves 32% RAM but causes truncation bugs with Clipper2)
// NEW: int64_t (matches Clipper2, eliminates overflow)
//
