...
GCodeSender::GCodeSender()
    : io(), serial(io), can_send(false), sent(0), open(false), error(false), connected(false), queue_paused(false)
{
#ifdef DEBUG_SERIAL
...
void GCodeSender::on_read(const boost::system::error_code &error, size_t bytes_transferred)
{
    ...
    if (!line.empty())
    {
        ...
        // parse incoming line
        if (!this->connected && (boost::starts_with(line, "start") || boost::starts_with(line, "Grbl ") ||
...
#ifdef DEBUG_SERIAL
            fs << "!! line num out of sync: toresend = " << toresend << ", sent = " << sent
               << ", last_sent.size = " << last_sent.size() << std::endl;
#endif
