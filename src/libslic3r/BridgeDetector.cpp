...
bool BridgeDetector::detect_angle(double bridge_direction_override)
{
    ...
    for (size_t i_angle = 0; i_angle < candidates.size(); ++i_angle)
    {
        ...
        Lines lines;
        {
            ...
            for (coord_t y = bbox.min(1); y <= bbox.max(1); y += this->spacing)
                lines.push_back(
                    Line(Point((coord_t) round(c * bbox.min(0) - s * y), (coord_t) round(c * y + s * bbox.min(0))),
        ...
        double max_length = 0;
        {
            ...
            for (size_t i = 0; i < clipped_lines.size(); ++i)
            {
                ...
                if (expolygons_contain(this->_anchor_regions, line.a) &&
                    expolygons_contain(this->_anchor_regions, line.b))
                {
    ...
    size_t i_best = 0;
    for (size_t i = 1; i < candidates.size() && candidates[i_best].coverage - candidates[i].coverage < this->spacing;
         ++i)
...
void BridgeDetector::unsupported_edges(double angle, Polylines *unsupported) const
{
    ...
    for (ExPolygons::const_iterator it_expoly = this->expolygons.begin(); it_expoly != this->expolygons.end();
         ++it_expoly)
    {
