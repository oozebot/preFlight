...
void SVG::draw(const ThickLine &line, const std::string &fill, const std::string &stroke, coordf_t stroke_width)
{
    ...
    fprintf(this->f,
            ...
            to_svg_x(line.b(0) + db * perp(0) - origin(0)), to_svg_y(line.b(1) + db * perp(1) - origin(1)),
            to_svg_x(line.a(0) + da * perp(0) - origin(0)), to_svg_y(line.a(1) + da * perp(1) - origin(1)),
            fill.c_str(), stroke.c_str(), (stroke_width == 0) ? 1.f : to_svg_coord(stroke_width));
}
...
void SVG::draw_outline(const ExPolygon &expolygon, std::string stroke_outer, std::string stroke_holes,
...
void SVG::draw_outline(const ExPolygons &expolygons, std::string stroke_outer, std::string stroke_holes,
                       coordf_t stroke_width)
{
    for (ExPolygons::const_iterator it = expolygons.begin(); it != expolygons.end(); ++it)
...
void SVG::draw_outline(const Surface &surface, std::string stroke_outer, std::string stroke_holes,
                       coordf_t stroke_width)
{
...
void SVG::draw_outline(const Surfaces &surfaces, std::string stroke_outer, std::string stroke_holes,
                       coordf_t stroke_width)
{
    for (Surfaces::const_iterator it = surfaces.begin(); it != surfaces.end(); ++it)
        draw_outline(*it, stroke_outer, stroke_holes, stroke_width);
...
void SVG::draw_outline(const SurfacesPtr &surfaces, std::string stroke_outer, std::string stroke_holes,
                       coordf_t stroke_width)
{
    for (SurfacesPtr::const_iterator it = surfaces.begin(); it != surfaces.end(); ++it)
        draw_outline(*(*it), stroke_outer, stroke_holes, stroke_width);
...
void SVG::draw(const ThickPolylines &thickpolylines, const std::string &fill, const std::string &stroke,
               coordf_t stroke_width)
{
...
void SVG::draw(const Clipper2Lib::Path64 &polygon, double scale, std::string stroke, coordf_t stroke_width)
{
    this->stroke = stroke;
    this->path(this->get_path_d(polygon, scale, true), false, stroke_width, 1.f);
}

void SVG::draw(const Clipper2Lib::Paths64 &polygons, double scale, std::string stroke, coordf_t stroke_width)
{
    for (Clipper2Lib::Paths64::const_iterator it = polygons.begin(); it != polygons.end(); ++it)
...
std::string SVG::get_path_d(const Clipper2Lib::Path64 &path, double scale, bool closed) const
{
    std::ostringstream d;
    d << "M ";
    for (Clipper2Lib::Path64::const_iterator p = path.begin(); p != path.end(); ++p)
    {
        // Clipper2: Point64 uses .x, .y (members, not methods)
...
{
    fprintf(this->f, R"(<text x="%f" y="%f" font-family="sans-serif" font-size="%fpx" fill="%s">%s</text>)",
            to_svg_x(float(pt.x() - origin.x())), to_svg_y(float(pt.y() - origin.y())), font_size, color, text);
}

void SVG::draw_legend(const Point &pt, const char *text, const char *color, const coordf_t font_size)
{
    fprintf(this->f, R"(<circle cx="%f" cy="%f" r="%f" fill="%s"/>)", to_svg_x(float(pt.x() - origin.x())),
            to_svg_y(float(pt.y() - origin.y())), font_size, color);
    fprintf(this->f, R"(<text x="%f" y="%f" font-family="sans-serif" font-size="%fpx" fill="%s">%s</text>)",
            to_svg_x(float(pt.x() - origin.x())) + 20.f, to_svg_y(float(pt.y() - origin.y())), font_size, "black",
            text);
}
...
void SVG::export_expolygons(const char *path, const BoundingBox &bbox, const Slic3r::ExPolygons &expolygons,
                            std::string stroke_outer, std::string stroke_holes, coordf_t stroke_width)
{
    SVG svg(path, bbox);
    svg.draw(expolygons);
...
void SVG::export_expolygons(
    const char *path, const std::vector<std::pair<Slic3r::ExPolygons, ExPolygonAttributes>> &expolygons_with_attributes)
{
    ...
    Point legend_size(scale_(1.) + num_columns * step_x,
    ...
    for (const auto &exp_with_attr : expolygons_with_attributes)
    {
        ...
        {
            svg.draw_legend(Point(pos_x, pos_y), exp_with_attr.second.legend.c_str(),
                            exp_with_attr.second.color_fill.c_str());
