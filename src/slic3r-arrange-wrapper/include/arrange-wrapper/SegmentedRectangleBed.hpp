...
template<class T>
constexpr bool IsSegmentedBed = IsSegmentedBed_<StripCVRef<T>>::value;
...
struct SegmentedRectangleBed
{
    ...
    SegmentedRectangleBed(const BoundingBox &bb, size_t segments_x, size_t segments_y, const Vec2crd &gap,
...
struct SegmentedRectangleBed<std::integral_constant<size_t, SegX>, std::integral_constant<size_t, SegY>>
{
    ...
    explicit SegmentedRectangleBed(const BoundingBox &b, const Vec2crd &gap,
                                   const RectPivots pivot = RectPivots::Center)
        : bb{b}, gap{gap}
    {
    }
