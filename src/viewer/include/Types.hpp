...
//
// Parameters of the host application's scene passes (Full lighting tier).
// When enabled, the visible segments shader modulates its baked lighting by the
// given shadow map (sampled through shadow_vp, a world -> shadow-texture matrix)
// and the screen-space ambient-occlusion texture.
//
...
enum class EViewType : uint8_t
{
    ...
    Tool,
    // Color print by filament colors (plus the color changes)
    ColorPrint,
    COUNT
};
