...
// Frequency-aware band for legend display.
...
public:
    ...
    // Part of the frequency-aware band system.
    //
...
private:
    ...
    //
    // Frequency map: rounded value to vertex count.
    // Collected during update(), used by finalize() to compute bands.
    //
    std::map<float, size_t> m_value_counts;
    //
    // Computed bands (up to palette size), populated by finalize().
    //
    std::vector<ColorBand> m_bands;
    //
    // Whether finalize() has been called after the last update cycle.
    //
    ...
    void reset();
    //
    // Compute frequency-based bands from collected value data.
    // Called by ViewerImpl after all update() calls are complete.
    //
    void finalize();
    //
    // Create fixed evenly-spaced bands (e.g. 0-10, 11-20, ..., 91-100 for fan speed).
