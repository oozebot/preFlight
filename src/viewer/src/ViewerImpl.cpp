...
void ViewerImpl::init(const std::string &opengl_context_version)
{
    ...
    // Scene passes (Full lighting tier): shadow/AO uniforms of the visible shader
    // and the G-buffer variant program.
    m_uni_segments_shadow_vp_id = glGetUniformLocation(m_segments_shader_id, "shadow_vp");
    m_uni_segments_scene_passes_id = glGetUniformLocation(m_segments_shader_id, "scene_passes");
    m_uni_segments_shadow_tex_id = glGetUniformLocation(m_segments_shader_id, "shadow_tex");
    m_uni_segments_ao_tex_id = glGetUniformLocation(m_segments_shader_id, "ao_tex");
    m_uni_segments_viewport_size_id = glGetUniformLocation(m_segments_shader_id, "viewport_size");
...
void ViewerImpl::update_color_ranges()
{
    ...
    // Compute frequency-based bands for all ranges
...
void ViewerImpl::draw_enabled_segments()
{
#ifdef ENABLE_OPENGL_ES
    int curr_bound_texture = 0;
    glsafe(glGetIntegerv(GL_TEXTURE_BINDING_2D, &curr_bound_texture));
                                 ...
                                 const Vec3 &camera_position)
{
    ...
    // draw is rejected.
    glsafe(glUniform1i(m_uni_segments_shadow_tex_id, 4));
    glsafe(glUniform1i(m_uni_segments_ao_tex_id, 5));
    int prev_tex2d_unit4 = 0;
    int prev_tex2d_unit5 = 0;
    const bool scene_passes_on = m_scene_pass_params.enabled && m_uni_segments_scene_passes_id != -1;
    if (scene_passes_on)
    {
        glsafe(glUniform1i(m_uni_segments_scene_passes_id, 1));
        glsafe(glUniformMatrix4fv(m_uni_segments_shadow_vp_id, 1, GL_FALSE, m_scene_pass_params.shadow_vp.data()));
        glsafe(glUniform2fv(m_uni_segments_viewport_size_id, 1, m_scene_pass_params.viewport_size.data()));
    ...
    else if (m_uni_segments_scene_passes_id != -1)
        glsafe(glUniform1i(m_uni_segments_scene_passes_id, 0));
#endif // !ENABLE_OPENGL_ES

    glsafe(glDisable(GL_CULL_FACE));

    draw_enabled_segments();
                                      ...
                                      const Vec3 &camera_position, bool gbuffer)
{
    ...
    (void) gbuffer;
#else
    const unsigned int shader_id = gbuffer ? m_segments_gbuffer_shader_id : m_segments_shader_id;
    if (shader_id == 0 || m_enabled_segments_count == 0)
        return;
    ...
    {
        // Depth pass reuses the visible segments program with no color attachment;
        // the light's position orients the imposters toward the light. The scene-pass
        // modulation must stay off so the shader never samples the shadow map being
        // rendered.
        glsafe(glUniform1i(m_uni_segments_positions_tex_id, 0));
        glsafe(glUniform1i(m_uni_segments_height_width_angle_tex_id, 1));
        glsafe(glUniform1i(m_uni_segments_colors_tex_id, 2));
        ...
        glsafe(glUniform3fv(m_uni_segments_camera_position_id, 1, camera_position.data()));
        glsafe(glUniform4fv(m_uni_segments_clipping_plane_id, 1, m_clipping_plane.data()));
        if (m_uni_segments_scene_passes_id != -1)
            glsafe(glUniform1i(m_uni_segments_scene_passes_id, 0));
