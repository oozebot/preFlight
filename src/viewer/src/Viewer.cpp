...
void Viewer::set_scene_pass_params(const ScenePassParams &params)
{
    m_impl->set_scene_pass_params(params);
}

void Viewer::render_segments_pass(const Mat4x4 &view_matrix, const Mat4x4 &projection_matrix,
                                  const Vec3 &camera_position, bool gbuffer)
{
    m_impl->render_segments_pass(view_matrix, projection_matrix, camera_position, gbuffer);
}
