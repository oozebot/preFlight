                                                      ...
                                                      std::optional<std::pair<double, double>> deflections)
{
    try
    {
        ...
        for (const NamedSolid &namedSolid : namedSolids)
        {
            ...
            // The fallbacks match the application's default import precision, so an import without
            // explicit values meshes the same as the GUI's default preset.
            double angularDeflection = deflections.has_value() ? deflections.value().second : 1.0;
