...
inline InfiniteBed offset(const InfiniteBed &bed, coord_t)
{
    return bed;
}
...
inline BoundingBox bounding_box(const RectangleBed &bed)
{
    return bed.bb;
}
...
public:
    ...
    explicit CircleBed(const Point &c, double r, const Vec2crd &g) : m_center(c), m_radius(r), m_gap(g) {}
...
inline ExPolygons to_expolygons(const IrregularBed &bed)
{
    return bed.poly;
}
...
template<class Bed>
static constexpr bool IsRectangular = IsRectangular_<Bed>::value;

} // namespace arr2

inline BoundingBox &bounding_box(BoundingBox &bb)
{
    return bb;
}
inline const BoundingBox &bounding_box(const BoundingBox &bb)
{
    return bb;
}
inline BoundingBox bounding_box(const Polygon &p)
{
    return get_extents(p);
}
