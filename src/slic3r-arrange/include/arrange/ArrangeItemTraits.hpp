...
{
    static Vec2crd get_translation(const ArrItem &ap) { return ap.get_translation(); }

    static double get_rotation(const ArrItem &ap) { return ap.get_rotation(); }

    static int get_bed_index(const ArrItem &ap) { return ap.get_bed_index(); }

    static std::optional<int> get_bed_constraint(const ArrItem &ap) { return ap.get_bed_constraint(); }

    static int get_priority(const ArrItem &ap) { return ap.get_priority(); }

    // Setters:

    static void set_translation(ArrItem &ap, const Vec2crd &v) { ap.set_translation(v); }
    ...
    static void set_bed_constraint(ArrItem &ap, std::optional<int> v) { ap.set_bed_constraint(v); }
};

template<class T>
using ArrangeItemTraits = ArrangeItemTraits_<StripCVRef<T>>;
...
template<class T>
std::optional<int> get_bed_constraint(const T &itm)
{
...
template<class T>
void set_bed_constraint(T &itm, std::optional<int> v)
{
