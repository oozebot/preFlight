...
int load_printer_geometry_from_stream(std::istream &geometry_stream, PrinterGeometry &printer_geometry)
{
    ...
    while (geometry_stream)
    {
        ...
        if (find_and_remove(line, "POLYGON_AT_HEIGHT"))
        {
            ...
            std::map<coord_t, std::vector<Polygon>>::iterator extruder_slice = printer_geometry.extruder_slices.find(
                height);
            ...
            else
            {
                ...
                current_polygon =
                    &printer_geometry.extruder_slices.insert(std::pair(height, polygons)).first->second.back();
            }
        }
    ...
    printer_geometry.plate = {Point(coord_t(0), coord_t(0)), Point(coord_t(x_size), coord_t(0)),
                              Point(coord_t(x_size), coord_t(y_size)), Point(coord_t(0), coord_t(y_size))};

    return 0;
}

void save_import_data_to_file(const std::string &filename, const std::map<double, int> &scheduled_polygons,
                              const map<int, int> &original_index_map, const vector<Rational> &poly_positions_X,
