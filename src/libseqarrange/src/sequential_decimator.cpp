                            ...
                            const std::vector<Slic3r::Polygon> &decimated_polygons)
{
    ...
    for (unsigned int i = 0; i < decimated_polygons.size(); ++i)
    {
        ...
        Slic3r::Polygon shift_polygon = scaleUp_PolygonForSlicer(
            1, decimated_polygons[i], (command_parameters.x_position * SEQ_SLICER_SCALE_FACTOR) * 10,
...
int decimate_Polygons(const CommandParameters &command_parameters)
{
    ...
    for (unsigned int i = 0; i < decimated_polygons.size(); ++i)
    {
        ...
        shift_polygon = scaleUp_PolygonForSlicer(1, shift_polygon,
    ...
    solver_configuration.plate_bounding_box = BoundingBox(Point(0, 0),
                                                          Point(SEQ_PRUSA_MK3S_X_SIZE, SEQ_PRUSA_MK3S_Y_SIZE));
    ...
    for (unsigned int i = 0; i < decimated_polygons.size(); ++i)
    {
        ...
        {
            transformed_polygon = transform_UpsideDown(
                solver_configuration,
                scaleUp_PolygonForSlicer(1, shift_polygon,
                                         (solver_configuration.plate_bounding_box.min.x() +
                                          rand() % (solver_configuration.plate_bounding_box.max.x() -
                                                    solver_configuration.plate_bounding_box.min.x())) *
    ...
    Polygon bed_polygon(
        {{solver_configuration.plate_bounding_box.min.x(), solver_configuration.plate_bounding_box.min.y()},
    ...
    Polygon display_bed_polygon = scaleUp_PolygonForSlicer(SEQ_SVG_SCALE_FACTOR, bed_polygon, 0, 0);
