...
///|/ Copyright (c) Prusa Research 2019 - 2023 Vojtěch Bubník @bubnikv
///|/
...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"preFlight_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc))
        {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"preFlight_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0,
                                      640, 480, 0, 0, wc.hInstance, (LPVOID) this);
            if (hwnd)
            {
...
protected:
    ...
    void check(HWND hWnd)
    {
        ...
        if (wglCreateContext == nullptr || wglMakeCurrent == nullptr || wglDeleteContext == nullptr ||
            glGetString == nullptr)
        {
    ...
    int wmain(int argc, wchar_t **argv)
    {
        ...
        if (load_mesa)
        {
            ...
            if (!res)
            {
                MessageBox(
                    nullptr,
                    L"preFlight was unable to automatically switch to MESA OpenGL library\nPlease, try to run the application using the '--sw-renderer' option.\n",
                    L"preFlight Warning", MB_OK);
                return -1;
            }
            else
            {
